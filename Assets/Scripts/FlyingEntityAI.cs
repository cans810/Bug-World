...
public class FlyingEntityAI : EnemyAI
{
    ...
    [Header("Attack Height Settings")]
    [SerializeField] private float attackHeightOffset = 0.5f; // Height above player when attacking
    [SerializeField] private float attackDescentSpeed = 3f; // Speed to descend when attacking
    [SerializeField] private float attackAscentSpeed = 2f; // Speed to ascend after attacking
    ...
    private void RunFlyingAI()
    {
        ...
        if (livingEntity.HasTargetsInRange())
        {
            ...
            if (potentialTarget != null && !potentialTarget.IsDead)
            {
                ...
                {
                    // Only set new target if we don't already have one
                    if (currentTarget == null || currentTarget != potentialTarget)
                    {
        ...
        // Only run wandering logic if we're not attacking or descending
        if (!isWandering && !isDescending && wanderingBehavior != null && !wanderingBehavior.IsCurrentlyMoving())
        {
    ...
    private void HandleFlyingHeight()
    {
        if (isDescending)
        {
            // Calculate target height based on player's position plus offset
            float targetHeight = currentTarget != null ? 
                currentTarget.transform.position.y + attackHeightOffset : 
                flyingHeight;
            ...
            {
                Vector3 newPos = transform.position;
                newPos.y = Mathf.MoveTowards(transform.position.y, targetHeight, attackDescentSpeed * Time.deltaTime);
        ...
        else if (isAscending)
        {
            ...
            {
                Vector3 newPos = transform.position;
                newPos.y = Mathf.MoveTowards(transform.position.y, flyingHeight, attackAscentSpeed * Time.deltaTime);
    ...
    private void StartDescending()
    {
        ...
        // Log for debugging
        Debug.Log($"{gameObject.name} is descending to attack player at Y: {currentTarget?.transform.position.y + attackHeightOffset}");
    }
    ...
    {
        // Only attack if we're at the player's height (with some tolerance)
        if (currentTarget != null && 
            Mathf.Abs(transform.position.y - (currentTarget.transform.position.y + attackHeightOffset)) < 0.1f)
        {
