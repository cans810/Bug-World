...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    private void ShowArrowToArea(LevelAreaTarget area)
    {
        ...
        // Use the existing ArrowIndicatorManager
        if (ArrowIndicatorManager.Instance != null)
        {
            // Hide any existing arrow
            if (isArrowActive && currentTarget != null)
            {
                ArrowIndicatorManager.Instance.RemoveArrow(currentTarget);
            }
            
            // Create a new arrow pointing to the area target
            currentTarget = area.areaTarget;
            ArrowIndicatorManager.Instance.CreateArrowToTarget(currentTarget, arrowColor);
            isArrowActive = true;
    ...
    private IEnumerator HideArrowAfterDelay(float delay)
    {
        ...
        // Hide the arrow using ArrowIndicatorManager
        if (isArrowActive && currentTarget != null && ArrowIndicatorManager.Instance != null)
        {
            ArrowIndicatorManager.Instance.RemoveArrow(currentTarget);
            isArrowActive = false;
            currentTarget = null;
