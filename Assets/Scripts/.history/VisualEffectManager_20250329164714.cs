...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private void InitializeUIElementPools()
    {
        ...
        if (uiXPIconPrefab != null)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                icon.SetActive(false);
                uiElementPools["uiXP"].Enqueue(icon);
            }
        }
        
        Debug.Log("UI Element pools initialized correctly within canvasSafeArea");
    }
    ...
    private GameObject GetUIElementFromPool(string type)
    {
        if (!uiElementPools.ContainsKey(type) || uiElementPools[type].Count == 0)
        {
            ...
            if (prefab != null && canvasSafeArea != null)
            {
                // Find the appropriate pool container
                Transform poolContainer = canvasSafeArea.transform.Find(type == "uiChitin" ? "UIChitinPool" : "UIXPPool");
                if (poolContainer == null)
                {
                    // Create pool container if it doesn't exist
                    GameObject container = new GameObject(type == "uiChitin" ? "UIChitinPool" : "UIXPPool");
                    container.transform.SetParent(canvasSafeArea.transform, false);
                    poolContainer = container.transform;
                }
                
                // Create the icon in the pool container
                GameObject icon = Instantiate(prefab, poolContainer);
            ...
            Debug.LogError($"UI Element pool empty and no prefab for {type}");
            return null;
        }
