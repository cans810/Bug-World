...
public class ChitinDropper : MonoBehaviour
{
    ...
    private IEnumerator SimpleChitinAnimation(GameObject chitinObject)
    {
        ...
        Debug.Log("[DROPPER DEBUG] Starting simple chitin animation");
        ...
        {
            // Check if the object has been destroyed or is being collected
            ChitinCollectible collectible = chitinObject.GetComponent<ChitinCollectible>();
            if (chitinObject == null || (collectible != null && collectible.IsBeingCollected()))
            {
                Debug.Log("[DROPPER DEBUG] Chitin is being collected, stopping dropper animation");
                yield break;
            }
        ...
        if (behavior != null)
        {
            Debug.Log("[DROPPER DEBUG] Calling ForceStick on ChitinBehavior");
            behavior.ForceStick();
        }
    }
} 