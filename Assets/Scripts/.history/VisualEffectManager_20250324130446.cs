...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        int symbolCount = 20; // Increased from 12 to 20 for more symbols
        ...
        {
            Debug.LogError("Level up symbol pool is missing or empty! Check inspector assignment.");
            onCompleteCallback?.Invoke();
            return;
        }
        
        // Create a vertical ray direction
        ...
        for (int i = 0; i < symbolCount; i++)
        {
            ...
            // Very tight horizontal clustering but more vertical staggering
            Vector3 randomOffset = new Vector3(
                Random.Range(-0.1f, 0.1f),   // Even tighter horizontal spread
                0.15f * i,                    // More vertical staggering (0.15 instead of 0.1)
                Random.Range(-0.1f, 0.1f)     // Even tighter depth spread
            );
            ...
            // Use even less delay between symbols to make a more consistent line
            StartCoroutine(AnimateLevelUpSymbolInRay(symbol, playerObj.transform, rayDirection, i * 0.01f, () => {
                completedAnimations++;
    ...
    // Modify the animation to make symbols move slower
    private IEnumerator AnimateLevelUpSymbolInRay(GameObject symbol, Transform playerTransform, Vector3 rayDirection, float delay, System.Action onComplete)
    {
        // Reduced delay
        ...
        // Start very small
        symbol.transform.localScale = startScale * 0.1f;
        
        // Store starting position
        Vector3 startPosition = symbol.transform.position;
        
        // Animation parameters
        float animDuration = 2.5f;  // Increased from 1.2f to 2.5f for slower movement
        float maxHeight = 12.0f;    // Even higher for more vertical travel
        float elapsed = 0f;
        
        // Create a very subtle curved path
        Vector3 rayOffset = new Vector3(
            Random.Range(-0.05f, 0.05f), // Extremely small random offset for tighter line
            0,
            Random.Range(-0.05f, 0.05f)
        ).normalized * 0.2f;
        
        // Ray velocity slightly varies with symbol index
        float rayVelocityMultiplier = 1.0f + (delay * 2.0f); // Reduced from 5.0f to 2.0f
        ...
        while (elapsed < animDuration && playerTransform != null)
        {
            ...
            // Smoother vertical movement for more consistent speed
            float verticalProgress = Mathf.Pow(normalizedTime, 0.8f); // Less acceleration (0.8 vs 0.7)
            float currentHeight = Mathf.Lerp(0, maxHeight * rayVelocityMultiplier, verticalProgress);
            
            // Even straighter ray
            float rayIntegrity = 0.98f; // Increased from 0.95f for straighter line
            float horizontalDrift = Mathf.Sin(normalizedTime * 2f) * 0.05f * (1f - rayIntegrity);
            
            // Calculate new position with minimal drift
            Vector3 newPosition = startPosition + new Vector3(
                horizontalDrift + rayOffset.x * currentHeight * 0.05f, // Reduced from 0.1f
                currentHeight,
                rayOffset.z * currentHeight * 0.05f // Reduced from 0.1f
            );
            ...
            // More gradual growth with subtle pulse
            float baseGrowth = normalizedTime < 0.4f ? // Increased from 0.3f for slower growth
                Mathf.Lerp(0.1f, 1.0f, normalizedTime / 0.4f) : 
                1.0f;
                
            // Subtle pulse
            float pulse = 1.0f + 0.05f * Mathf.Sin(normalizedTime * 12f); // Less intense pulse
            
            symbol.transform.localScale = startScale * baseGrowth * pulse;
            
            // Fade in quickly, stay visible longer, then fade out near the end
            float alpha;
            if (normalizedTime < 0.1f) {
                alpha = Mathf.Lerp(0f, 1f, normalizedTime / 0.1f);
            }
            else if (normalizedTime > 0.8f) { // Increased from 0.7f for longer visibility
                alpha = Mathf.Lerp(1f, 0f, (normalizedTime - 0.8f) / 0.2f);
            }
            else {
                alpha = 1f;
            }
