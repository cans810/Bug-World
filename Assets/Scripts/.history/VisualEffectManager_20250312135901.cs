...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private Vector3 chitinScale = new Vector3(0.005f, 0.005f, 0.005f);
    ...
    private void InitializePool(string type, GameObject prefab)
    {
        ...
        // Create a container for the pool objects that won't affect their scale
        ...
        for (int i = 0; i < poolSizePerType; i++)
        {
            ...
            // Reset scale to 1 and set the desired scale before parenting
            symbol.transform.localScale = type == "chitin" ? chitinScale : baseScale;
            
            // Parent to the container after setting scale
            symbol.transform.SetParent(poolContainer.transform, false);
    ...
    private IEnumerator SpawnChitinCollectEffect(int count)
    {
        ...
        // Get the screen dimensions with adjusted offset
        Vector2 screenTopRight = new Vector2(Screen.width - 200f, Screen.height - 150f); // Moved more left and down
    ...
    private IEnumerator AnimateChitinCollection(GameObject symbol)
    {
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            // Scale pulse effect using chitinScale as base
            float pulseScale = 1f + Mathf.Sin(normalizedTime * Mathf.PI * 2) * 0.2f;
            symbol.transform.localScale = chitinScale * pulseScale;

            // Play sound effect slightly before reaching nest
        ...
        symbol.transform.localScale = chitinScale; // Ensure it resets to chitin scale
        symbol.SetActive(false);
