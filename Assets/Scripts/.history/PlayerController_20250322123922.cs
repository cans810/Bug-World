...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Check if the collider is in the BigBorder layer
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // Set the border state and normal
            isAtMapBorder = true;
            
            // Calculate the normal pointing back INTO the BigBorder (toward center)
            // For exiting, we need to reverse the direction compared to entering
            ...
            if (livingEntity != null)
            {
                // Apply a stronger force to keep player inside the BigBorder
            ...
            // Show the specific message for BigBorder
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            ...
            // Calculate the normal pointing back into the playable area
            Vector3 playerToColliderCenter = transform.position - other.bounds.center;
            borderNormal = playerToColliderCenter.normalized;
            
            // Apply a force to keep player inside boundary
            if (livingEntity != null)
            {
                livingEntity.MoveInDirection(borderNormal, 0.1f);
            }
