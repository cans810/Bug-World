    ...
    private void ChaseAndAttackTarget()
    {
        ...
        if (distanceToTarget <= minAttackDistance)
        {
            ...
            if (!isPreparingAttack && Time.time >= lastAttackTime + attackInterval)
            {
                ...
                // Could trigger a "prepare attack" animation here if you have one
                // For example: animController.SetPreparingAttack(true);
            }
        ...
        else
        {
            ...
            {
                isPreparingAttack = false;
                // Reset any preparation animations if needed
                // For example: animController.SetPreparingAttack(false);
            }
