...
public class ArrowIndicatorManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning("Multiple ArrowIndicatorManager instances found!");
            Destroy(gameObject);
            return;
        }
        Instance = this;
        
        Debug.Log("ArrowIndicatorManager initialized");
    }
    
    private void Start()
    {
        mainCamera = Camera.main;
        Debug.Log($"ArrowIndicatorManager using camera: {(mainCamera != null ? mainCamera.name : "null")}");
        
        // Don't modify the camera's properties
        // mainCamera.clearFlags = ...  // Remove any lines that change camera settings
    }
    
    private void Update()
    {
        // Add this at the start of Update
        if (Time.frameCount % 100 == 0) // Log every 100 frames to avoid spam
        {
            Debug.Log("ArrowIndicatorManager.Update running");
        }
    ...
    {
        // Temporarily disable arrow updates to see if this is causing the issue
        return;
    ...
    private void UpdateArrowTransform(GameObject arrow, GameObject target)
    {
        ...
        if (!isOnScreen)
        {
            ...
            // If your arrow points left by default, add 180 degrees to make it point right
            float angle = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg;
            arrow.transform.rotation = Quaternion.Euler(0, 0, angle + 180); // Add 180 to flip direction
        ...
        else
        {
            ...
            else
            {
                ...
                arrow.transform.rotation = Quaternion.Euler(0, 0, angle); // Remove the +180 to flip direction
            }
        }
    }
