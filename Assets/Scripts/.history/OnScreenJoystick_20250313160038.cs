...
public class OnScreenJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
        ...
        if (useDirectInput)
        {
            ...
            if (UnityEngine.Input.touchCount > 0)
            {
                ...
                if (touch.phase == TouchPhase.Began && !isDragging)
                {
                    ...
                    if (touchInArea)
                    {
                        // Manually trigger the pointer down logic
                        Vector2 touchPosition;
                        if (RectTransformUtility.ScreenPointToLocalPointInRectangle(
                            canvasRectTransform,
                            touch.position,
                            ...
                            out touchPosition))
                        {
                            if (showDebugInfo)
                            {
                                Debug.Log($"Direct input: Touch began at {touch.position}, local position: {touchPosition}");
                            }
                            ...
                            // We don't want to process input yet since the handle should be centered initially
            ...
            if (UnityEngine.Input.GetMouseButtonDown(0) && !isDragging)
            {
                ...
                if (clickInArea)
                {
                        ...
                        out clickPosition))
                    {
                        if (showDebugInfo)
                        {
                            Debug.Log($"Direct input: Mouse click at {UnityEngine.Input.mousePosition}, local position: {clickPosition}");
                        }
                        ...
                        joystickBackground.anchoredPosition = clickPosition;
                        joystickHandle.anchoredPosition = Vector2.zero;
    ...
    private void HandleDirectInput(Vector2 inputPosition)
    {
        ...
        {
            // Calculate the direction from the center of the joystick background
            Vector2 direction = touchPosition;  // touchPosition is already relative to joystick center
            
            // Calculate input based on the direction and handle range
            float maxRadius = joystickBackground.sizeDelta.x * 0.5f * handleRange;
            
            if (direction.magnitude > maxRadius)
            {
                input = direction.normalized;
                joystickHandle.anchoredPosition = direction.normalized * maxRadius;
            }
            else
            {
            ...
            if (showDebugInfo && Time.frameCount % 30 == 0)
            {
                Debug.Log($"Direct input: Joystick input: {input}, magnitude: {input.magnitude}, handle pos: {joystickHandle.anchoredPosition}");
            }
        }
        else if (showDebugInfo)
        {
            Debug.LogWarning($"Failed to convert screen point {inputPosition} to local point in joystick background");
        }
    }
} 