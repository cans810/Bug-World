...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Store the initial Y position as the fixed position
    ...
    private void Update()
    {
        ...
        // Only enforce Y position every few frames to avoid movement disruption
        if (enforceFixedYPosition && !livingEntity.IsDead && Time.frameCount % 10 == 0)
        {
            EnforceYPosition();
        }
    }
    ...
    private void EnterCombatStance()
    {
        ...
        // Ensure we're at the correct Y position when entering combat
    ...
    private void ExitCombatStance()
    {
        ...
        // Ensure we're at the correct Y position when exiting combat
        if (enforceFixedYPosition)
        {
            EnforceYPosition();
        }
    ...
    private void HandlePlayerEnterHitbox(LivingEntity playerEntity)
    {
        ...
        // Ensure we're at the correct Y position
        if (enforceFixedYPosition)
        {
            EnforceYPosition();
        }
    ...
    private void EnforceYPosition()
    {
        ...
        if (Mathf.Abs(position.y - fixedYPosition) > 0.05f) // Increased tolerance
        {
