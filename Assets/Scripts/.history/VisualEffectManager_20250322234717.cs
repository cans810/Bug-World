...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Shorter duration for snappier movement
        float duration = 0.9f;
        bool hasBumpedUI = false;
        float elapsed = 0f;
        
        // Record starting position for midpoint calculation
        Vector3 originalStartPos = startPos;
        
        while (elapsed < duration)
        {
            // Recalculate target position each frame to handle UI movement
            Ray currentRay = Camera.main.ScreenPointToRay(targetScreenPos);
            Vector3 targetPos = currentRay.GetPoint(10f);
            
            elapsed += Time.deltaTime;
            float normalizedTime = Mathf.Clamp01(elapsed / duration);
            
            // Use a lower arc for more direct movement
            float arcHeight = 0.5f;
            Vector3 midPoint = Vector3.Lerp(originalStartPos, targetPos, 0.5f) + Vector3.up * arcHeight;
            
            // Calculate position using bezier curve
            float curveValue = chitinFlyCurve.Evaluate(normalizedTime);
            Vector3 a = Vector3.Lerp(originalStartPos, midPoint, curveValue);
            Vector3 b = Vector3.Lerp(midPoint, targetPos, curveValue);
            symbol.transform.position = Vector3.Lerp(a, b, curveValue);
            ...
            {
                float fadeAmount = (normalizedTime - 0.85f) / 0.15f;
                spriteRenderer.color = new Color(startColor.r, startColor.g, startColor.b, Mathf.Lerp(1f, 0f, fadeAmount));
            }
        ...
        // Ensure we actually reach and briefly stay at the destination
        Ray finalRay = Camera.main.ScreenPointToRay(targetScreenPos);
        Vector3 finalTargetPos = finalRay.GetPoint(10f);
        symbol.transform.position = finalTargetPos;
        yield return null;
        
        // Clean up after reaching destination
        if (symbol != null)
        {
            symbol.SetActive(false);
            spriteRenderer.color = startColor;
