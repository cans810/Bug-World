...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if (isAtMapBorder && moveDirection.magnitude > 0.1f)
        {
            ...
            // If trying to move through the border
            if (movementWithBorder < 0)
            {
                // Allow movement parallel to the border but not through it
                Vector3 tangent = Vector3.Cross(borderNormal, Vector3.up);
                float parallelComponent = Vector3.Dot(moveDirection, tangent);
                moveDirection = tangent * parallelComponent;
                
                // Recalculate target speed
                targetSpeed = moveDirection.magnitude * livingEntity.moveSpeed * 0.8f;
            }
            else
            {
                // Moving away from border, clear border state
                isAtMapBorder = false;
        ...
        if (moveDirection.magnitude > 0.01f)
        {
            if (livingEntity != null)
            {
                livingEntity.MoveInDirection(moveDirection, targetSpeed / livingEntity.moveSpeed);
            }
        }
        ...
        if (animController != null)
        {
            bool shouldBeWalking = moveDirection.magnitude > 0.1f;
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        {
            // Allow free movement through MapBorder1
            ...
            // For other borders, prevent exit if player is inside them
            if (!CanEnterBorder(other.gameObject.name))
            {
                isAtMapBorder = false;
                borderNormal = Vector3.zero;
                return;
            ...
            // We're trying to exit a permitted area
            isAtMapBorder = true;
            ...
            if (other is SphereCollider sphereCollider)
            {
                ...
                // Calculate direction back INTO the current area
