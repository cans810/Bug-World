    ...
    private void OptimizeAllRigidbodies()
    {
        ...
        foreach (Rigidbody rb in allRigidbodies)
        {
            ...
            try
            {
                ...
                rb.angularDrag = 0.05f; // Reduce angular drag for smoother rotation
                rb.drag = 0.5f; // Add some drag to prevent sliding
                
                // Freeze rotation on X and Z axes to prevent tipping
                rb.constraints = RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationZ;
                
                // Check if this is a player or ally
                LivingEntity entity = rb.GetComponent<LivingEntity>();
                if (entity != null)
                {
                    // Increase mass for player and allies to prevent pushing
                    if (rb.gameObject.CompareTag("Player") || rb.gameObject.CompareTag("Ally"))
                    {
                        rb.mass = 10f;
                        rb.drag = 1f; // Higher drag for player/allies
                    }
                    else
                    {
                        // For enemies
