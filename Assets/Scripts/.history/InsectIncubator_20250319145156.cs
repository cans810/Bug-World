...
public class InsectIncubator : MonoBehaviour
{
    ...
    // Add method to decrement egg count when an egg hatches
    public void DecrementEggCount()
    {
        ...
        // Reset the EggExists flag if no more eggs
        if (currentEggCount == 0)
        {
            EggExists = false;
        }
        
        Debug.Log($"Egg hatched. Current egg count: {currentEggCount}");
    }

    // Add a method to get the egg spawn position
    public Transform GetEggSpawnPosition()
    {
        return eggPos;
    }
}
