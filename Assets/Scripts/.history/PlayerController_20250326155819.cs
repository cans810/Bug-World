...
public class PlayerController : MonoBehaviour
{
    ...
    private void ShowMetamorphosisArrows()
    {
        ...
        if (sizeField != null)
            sizeField.SetValue(centerArrow, 0.1f); // Larger size for visibility
    ...
    // Helper method to create circular pattern of arrows
    private void CreateCircularArrows(GameObject parent)
    {
        ...
        for (int i = 0; i < arrowCount; i++)
        {
            ...
            // Position it to always be visible from the camera
            Vector3 screenPos = mainCamera.WorldToScreenPoint(position);
            // Ensure it's within screen bounds (80% of screen dimensions)
            ...
            position = mainCamera.ScreenToWorldPoint(new Vector3(screenPos.x, screenPos.y, 10f));
            ...
            var sizeField = typeof(PlayerFollowingArrowIndicator).GetField("arrowSize", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (sizeField != null)
                sizeField.SetValue(arrow, 1.0f); // Larger size for visibility
                
            var heightField = typeof(PlayerFollowingArrowIndicator).GetField("floatHeight", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (heightField != null)
                heightField.SetValue(arrow, 3.0f); // Higher float height for visibility
