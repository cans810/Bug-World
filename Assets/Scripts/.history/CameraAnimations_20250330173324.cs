...
public class CameraAnimations : MonoBehaviour
{
    ...
    // Add this method to animate to a specific position (for multiple objects)
    public void AnimateToPosition(Vector3 targetPosition)
    {
        if (isAnimating)
        {
            Debug.LogWarning("Cannot start new animation while another is in progress");
            return;
        }
        
        StartCoroutine(AnimateToPositionCoroutine(targetPosition));
    }

    // Add the coroutine implementation
    private IEnumerator AnimateToPositionCoroutine(Vector3 targetPosition)
    {
        isAnimating = true;
        ...
        // Calculate target position - above the position looking down
        Vector3 abovePosition = targetPosition + Vector3.up * heightOffset;
        Quaternion lookDownRotation = Quaternion.Euler(lookDownAngle, cameraTransform.rotation.eulerAngles.y, 0);
        
        // Disable player camera control
        if (playerCamera != null)
        {
            playerCamera.SetCameraControlEnabled(false);
        }
        
        // First animate to position above the target
        float elapsedTime = 0f;
        while (elapsedTime < animationDuration)
        {
            ...
            cameraTransform.position = Vector3.Lerp(originalPosition, abovePosition, t);
            cameraTransform.rotation = Quaternion.Slerp(originalRotation, lookDownRotation, t);
        ...
        cameraTransform.position = abovePosition;
        cameraTransform.rotation = lookDownRotation;
        ...
        while (elapsedTime < returnDuration)
        {
            ...
            cameraTransform.position = Vector3.Lerp(abovePosition, originalPosition, t);
            cameraTransform.rotation = Quaternion.Slerp(lookDownRotation, originalRotation, t);
        ...
        if (playerCamera != null)
        {
            playerCamera.SetCameraControlEnabled(true);
        }
