...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public void PlayXPRewardEffect(Vector3 panelPosition, int xpAmount)
    {
        ...
        // Start spawning symbols from the panel position
        StartCoroutine(SpawnXPSymbols(panelPosition, symbolCount, xpAmount));
    }

    private IEnumerator SpawnXPSymbols(Vector3 panelPosition, int count, int totalXPAmount)
    {
        // Get target position from XP panel
        Vector2 targetScreenPos = GetScreenPositionFromPanel(xpPanel);
        
        Debug.Log($"Spawning {count} XP symbols from position {panelPosition} to target {targetScreenPos}");
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (spriteRenderer != null)
            {
                // Set a bright blue color with full opacity for XP
                spriteRenderer.color = new Color(0.2f, 0.6f, 1f, 1f);
                // Ensure the sprite renders on top of everything
                spriteRenderer.sortingOrder = 1000;
            }
            
            // Start the animation to the UI position using our existing method
            StartCoroutine(AnimateXPSymbolToScreen(symbol, targetScreenPos, totalXPAmount));
            
            // Stagger the spawns
            yield return new WaitForSeconds(0.08f);
        }
        
        // Play XP sound after a slight delay
        yield return new WaitForSeconds(0.3f);
        PlayXPSound();
    }

    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpAmount)
    {
        // Similar to AnimateCoinSymbolToScreen but for XP
        Vector3 startPos = symbol.transform.position;
        ...
        // Store initial scale for scaling animation
        Vector3 initialScale = symbol.transform.localScale;
        Vector3 finalScale = initialScale * 0.7f;
        ...
        while (elapsed < duration)
        {
            ...
            // When XP is close to destination, animate the UI text
            if (!hasBumpedUI && normalizedTime > 0.75f)
            {
                // Find UIHelper to animate XP text
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null && xpPanel != null)
                {
                    // Pulse the XP panel
                    RectTransform panelRect = xpPanel.GetComponent<RectTransform>();
                    if (panelRect != null)
                    {
                        StartCoroutine(PulseUIElement(panelRect));
                    }
