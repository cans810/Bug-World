...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        ...
        while (true)
        {
            ...
            while (isWandering)
            {
                ...
                // If next position would exceed border, stop and turn smoothly
                if (restrictToSpawnArea && nextDistanceFromSpawn >= maxWanderRadius * 0.98f)
                {
                    isWandering = false;
                    UpdateAnimationState(false);
                    StartCoroutine(SmoothTurnAndMoveInward());
                    yield break;
                }
    ...
    // New method for smooth turning at border
    private IEnumerator SmoothTurnAndMoveInward()
    {
        ...
        // Add some randomness to the inward direction (but biased toward center)
        float randomAngle = Random.Range(-60f, 60f); // Reduced angle range to favor inward movement
        ...
        // Smooth rotation
        float rotationDuration = 1.0f; // Longer duration for smoother turn
        float rotationTimer = 0f;
        
        while (rotationTimer < rotationDuration)
        {
            // Use SmoothStep for more natural rotation acceleration/deceleration
        ...
        // Brief pause after turning
        yield return new WaitForSeconds(0.3f);
        
        // Resume wandering with new direction
        isWandering = true;
        UpdateAnimationState(true);
        StartCoroutine(WanderingRoutine());
    }

    // Modify EnforceBoundaryLimits to be less aggressive
    private void EnforceBoundaryLimits()
    {
        ...
        // Only apply emergency correction if significantly beyond border
        if (currentDistanceFromSpawn > maxWanderRadius * 1.1f)
        {
            ...
            correctedPosition.y = spawnPosition.y;
            
            transform.position = correctedPosition;
            
            // Force a smooth turn inward
            if (!isWaiting)
            {
                isWandering = false;
                UpdateAnimationState(false);
