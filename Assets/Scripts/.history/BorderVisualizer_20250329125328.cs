...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Points needed for a smooth visualization
        int segments = 60;
        perimeterLineRenderer.positionCount = segments;
        
        // For a Y-oriented capsule, the perimeter is just a circle in the XZ plane
        ...
        int circleSegments = segments / 2; // Half for the two semicircles (1/4 for each semicircle)
        int straightSegments = segments - circleSegments; // Half for the two straight sections (1/4 for each)
        
        // Points per section
        int pointsPerSemicircle = circleSegments / 2;
        int pointsPerStraight = straightSegments / 2;
        
        // We'll build the perimeter in four parts:
        // 1. First semicircle (right/front end)
        // 2. First straight section (bottom)
        // 3. Second semicircle (left/back end)
        // 4. Second straight section (top)
        
        // Calculate the endpoints of the capsule's central axis
        Vector3 direction1 = (direction == 0) ? Vector3.right : Vector3.forward;
        Vector3 offset = direction1 * (straightLength / 2);
        Vector3 end1 = center + offset; // Right/front end
        Vector3 end2 = center - offset; // Left/back end
        
        int pointIndex = 0;
        
        // Part 1: First semicircle (right/front end)
        for (int i = 0; i <= pointsPerSemicircle; i++)
        {
            float t = (float)i / pointsPerSemicircle;
            float angle = Mathf.PI / 2 + t * Mathf.PI; // 90° to 270°
            ...
            perimeterLineRenderer.SetPosition(pointIndex++, new Vector3(x, perimeterHeight, z));
        }
        
        // Part 2: First straight section (bottom)
        for (int i = 0; i <= pointsPerStraight; i++)
        {
            ...
            if (direction == 0) // X-axis
            {
                x = end1.x - t * straightLength;
                z = center.z - radius;
            }
            else // Z-axis
            {
                x = center.x - radius;
                z = end1.z - t * straightLength;
            }
            
            perimeterLineRenderer.SetPosition(pointIndex++, new Vector3(x, perimeterHeight, z));
        }
        
        // Part 3: Second semicircle (left/back end)
        for (int i = 0; i <= pointsPerSemicircle; i++)
        {
            float t = (float)i / pointsPerSemicircle;
            float angle = 3 * Mathf.PI / 2 + t * Mathf.PI; // 270° to 90°
        ...
        // Part 4: Second straight section (top) to close the loop
        int remainingPoints = segments - pointIndex;
        for (int i = 0; i < remainingPoints; i++)
        {
            float t = (float)i / (remainingPoints - 1);
            
            float x, z;
            if (direction == 0) // X-axis
            {
                x = end2.x + t * straightLength;
                z = center.z + radius;
            }
            else // Z-axis
            {
                x = center.x + radius;
                z = end2.z + t * straightLength;
            }
