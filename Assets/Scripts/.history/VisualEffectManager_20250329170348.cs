...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Add this method to spawn coins in world space
    public void PlayCoinEffectFromWorldPosition(Vector3 startPosition, int coinAmount = 1)
    {
        Debug.Log($"Spawning {coinAmount} coin symbols at world position {startPosition}");
        ...
        // Find the coin counter panel position in world space
        Vector3 targetPosition = Vector3.zero;
        if (coinPanel != null)
        {
            RectTransform coinRectTransform = coinPanel.GetComponent<RectTransform>();
            if (coinRectTransform != null)
            {
                targetPosition = GetWorldPositionFromUI(coinRectTransform);
                Debug.Log($"Coin panel world position: {targetPosition}");
            }
        }
        else
        {
            // Fallback position if panel not found - use screen top right
            targetPosition = Camera.main.ViewportToWorldPoint(new Vector3(0.85f, 0.95f, 5f));
            Debug.LogWarning("Coin panel not found, using fallback position");
        }
        
        // Spawn coin symbols one by one with slight delay
        StartCoroutine(SpawnCoinSymbols(startPosition, targetPosition, coinAmount));
    }

    // Helper coroutine to spawn multiple coin symbols
    private IEnumerator SpawnCoinSymbols(Vector3 startPos, Vector3 targetPos, int count)
    {
        for (int i = 0; i < count; i++)
        {
            // Get coin symbol from pool
            GameObject coinSymbol = GetSymbolFromPool("coin", coinSymbolPrefab);
            
            // Position it in the world - NOT as child of canvas
            coinSymbol.transform.SetParent(null); // Detach from any parent
            ...
            coinSymbol.transform.position = startPos + randomOffset;
            
            // Set initial scale and make sure it's visible
            coinSymbol.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
            ...
            if (spriteRenderer != null)
            {
                // Make sure the sprite is visible and has high sorting order
                spriteRenderer.enabled = true;
                spriteRenderer.sortingOrder = 1000;
                // Ensure it's using the correct sorting layer (UI or Default)
                spriteRenderer.sortingLayerName = "UI";
            }
            
            // Log the coin's state
            Debug.Log($"Spawned coin symbol at {coinSymbol.transform.position} - Active: {coinSymbol.activeSelf}, " +
                      $"Renderer enabled: {(spriteRenderer != null ? spriteRenderer.enabled.ToString() : "N/A")}");
            
            // Start flying animation
            StartCoroutine(FlyCoinToCounter(coinSymbol, targetPos));
            
            // Slight delay between coin spawns
            yield return new WaitForSeconds(0.1f);
        }
    }

    // Animation coroutine for coins
    private IEnumerator FlyCoinToCounter(GameObject coinSymbol, Vector3 targetPosition)
    {
        ...
        while (elapsed < duration)
        {
            ...
            // Scale animation
            float scale = Mathf.Lerp(0.3f, 0.1f, normalizedTime);
            coinSymbol.transform.localScale = new Vector3(scale, scale, scale);
