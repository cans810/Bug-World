...
public class MissionManager : MonoBehaviour
{
    ...
    [SerializeField] private string progressTextPath = "ProgressText"; // Path to the progress text component
    ...
    // Dictionary to track mission panel progress texts
    private Dictionary<string, TextMeshProUGUI> missionProgressTexts = new Dictionary<string, TextMeshProUGUI>();
    ...
    private void ShowMissionPanel(MissionData mission)
    {
        ...
        // Find and initialize the progress text
        TextMeshProUGUI progressText = missionPanelInstance.transform.Find(progressTextPath)?.GetComponent<TextMeshProUGUI>();
        if (progressText != null)
        {
            // Set initial text with current/target format
            progressText.text = $"{mission.currentAmount}/{mission.targetAmount}";
            
            // Store reference for later updates
            missionProgressTexts[mission.id] = progressText;
        }
        else
        {
            Debug.LogWarning($"Progress text not found at path: {progressTextPath}");
        }
    ...
    private void UpdateMissionPanelProgress(string missionId)
    {
        ...
        // Update the progress text if we have a reference to it
        if (missionProgressTexts.ContainsKey(missionId))
        {
            TextMeshProUGUI progressText = missionProgressTexts[missionId];
            if (progressText != null)
            {
                // Update the text with current/target format
                progressText.text = $"{mission.currentAmount}/{mission.targetAmount}";
                
                // Optionally change text color when completed
                if (mission.CheckIsCompleted())
                {
        ...
        if (mission.CheckIsCompleted() && panel != null)
        {
            ...
            if (panelButton != null)
            {
                ...
                // Optional: Add a text component to show "Claim Reward" or similar
                TextMeshProUGUI buttonText = panel.transform.Find("ClaimText")?.GetComponent<TextMeshProUGUI>();
                if (buttonText != null)
                {
    ...
    public void GiveRewardsForMission(string missionId, GameObject panel)
    {
        ...
        // Play visual effects for the rewards
        ...
        if (visualEffectManager != null)
        {
            ...
            if (rectTransform != null && canvas != null)
            {
                ...
                // Play the coin and XP animations
                if (coinReward > 0)
                {
        ...
        if (missionProgressTexts.ContainsKey(missionId))
        {
            missionProgressTexts.Remove(missionId);
        }
    }
