...
public class ChitinDropper : MonoBehaviour
{
    ...
    // Show red dot in the Scene view, even when not selected
    private void OnDrawGizmos()
    {
        DrawChitinDropMarker();
    }

    // Also draw when selected (for consistency)
    private void OnDrawGizmosSelected()
    {
        DrawChitinDropMarker();
    }

    // Helper method to draw the marker consistently
    private void DrawChitinDropMarker()
    {
        // Force position update by getting current transform position
        Vector3 spawnPosition = transform.position;
        
        // Draw a larger red sphere at the spawn position for better visibility
        Gizmos.color = Color.red;
        Gizmos.DrawSphere(spawnPosition, 0.25f);
        
        // Draw a white wire sphere around it to make it more visible
