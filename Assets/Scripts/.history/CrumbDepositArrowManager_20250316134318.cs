...
public class CrumbDepositArrowManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (arrowPrefab != null)
        {
            ...
            var field = typeof(ArrowIndicator).GetField("arrowPrefab", 
                System.Reflection.BindingFlags.Instance | 
                System.Reflection.BindingFlags.NonPublic);
                
            if (field != null)
            {
                field.SetValue(arrowIndicator, arrowPrefab);
            }
            else
            {
                // If reflection fails, try to find a serialized field
                var serializedFields = typeof(ArrowIndicator).GetFields(
                    System.Reflection.BindingFlags.Instance | 
                    System.Reflection.BindingFlags.Public);
                    
                foreach (var serializedField in serializedFields)
                {
                    if (serializedField.Name.Contains("arrowPrefab"))
                    {
                        serializedField.SetValue(arrowIndicator, arrowPrefab);
                        break;
                    }
                }
            }
        ...
        // Set the color
        var colorField = typeof(ArrowIndicator).GetField("arrowColor", 
            System.Reflection.BindingFlags.Instance | 
            System.Reflection.BindingFlags.NonPublic);
            
        if (colorField != null)
        {
            colorField.SetValue(arrowIndicator, arrowColor);
        }
