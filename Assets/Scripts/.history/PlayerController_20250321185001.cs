...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("MapBorder"))
        {
            ...
            // Get the border visualizer to check level requirements
            BorderVisualizer borderVisualizer = other.GetComponent<BorderVisualizer>();
            if (borderVisualizer != null)
            {
                int requiredLevel = borderVisualizer.GetRequiredLevel();
                int playerLevel = playerInventory != null ? playerInventory.CurrentLevel : 0;
                
                // Check if player meets level requirement
                bool canPassBorder = (requiredLevel == 0 || playerLevel >= requiredLevel);
                
                // Get our position and the border's position to determine direction
                Vector3 playerPos = transform.position;
                Vector3 borderPos = other.transform.position;
                
                // Calculate direction from border to player
                Vector3 borderToPlayerDir = (playerPos - borderPos).normalized;
                
                // Get the border's forward direction (assumes border is facing outward from area)
                Vector3 borderForward = other.transform.forward;
                
                // Dot product to determine if player is entering or exiting
                // Positive means player is moving in same direction as border faces (entering)
                float dotProduct = Vector3.Dot(borderToPlayerDir, borderForward);
                
                // Player is entering if dot product is positive
                bool isEntering = (dotProduct > 0);
                
                Debug.Log($"Border collision: {borderName}, Required level: {requiredLevel}, " +
                          $"Player level: {playerLevel}, Can pass: {canPassBorder}, " +
                          $"Is entering: {isEntering}");
                
                if (canPassBorder)
                {
                    // Handle player allowed to pass border
                    if (isEntering)
                    {
                        // Only update border name when player is entering a new area
                        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
                        if (playerAttributes != null)
                        {
                            playerAttributes.borderObjectName = borderName;
                            Debug.Log($"Player entered new area: {borderName}");
                        }
                        
                        // Notify level area manager that this area has been visited
                        if (levelAreaArrowManager != null)
                        {
                            levelAreaArrowManager.MarkAreaAsVisited(borderName);
                        }
                    }
                }
                else
                {
                    // Only show message if we're not already showing one
                    if (Time.time > lastBorderMessageTime + borderMessageCooldown && !isShowingBoundaryMessage)
                    {
                        // Player doesn't meet level requirement
                        string message = $"Requires Level {requiredLevel} to Enter";
                        ...
                        if (uiHelper != null)
                        {
                            ...
                            StartCoroutine(ResetBoundaryMessageFlag(borderMessageCooldown));
                        }
                        
                        // Add haptic feedback for better player experience
                        HapticFeedback.MediumFeedback();
                        
                        // Handle border collision physics (stop player from passing)
                        isAtMapBorder = true;
                        borderNormal = -borderToPlayerDir; // Use direction from player to border as normal
                        
                        Debug.Log($"Border collision normal: {borderNormal}");
                    }
                }
            }
        }
        
        // Handle other trigger types if needed
        // ...
    }

    private IEnumerator ResetBoundaryMessageFlag(float delay)
    {
