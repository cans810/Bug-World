...
public class AllyEggController : MonoBehaviour
{
    ...
    // Add this method to ensure AllyAI is enabled after hatching
    private IEnumerator EnsureAllComponentsEnabled(GameObject insect)
    {
        // Wait a short delay after hatching to ensure all systems are ready
        yield return new WaitForSeconds(1.0f);
        
        // Ensure AllyAI component is enabled
        AllyAI allyAI = insect.GetComponent<AllyAI>();
        if (allyAI != null && !allyAI.enabled)
        {
            Debug.Log($"Re-enabling disabled AllyAI on {insect.name}");
            allyAI.enabled = true;
            allyAI.ForceEnableMovement();
        }
        
        // Ensure AIWandering component is properly enabled
        AIWandering wandering = insect.GetComponent<AIWandering>();
        if (wandering != null)
        {
            wandering.SetWanderingEnabled(true);
            Debug.Log($"Ensuring wandering is enabled on {insect.name}");
        }
    }

    // Modify the OnEggHatched method to include the safety check
    public void OnEggHatched(GameObject newInsect)
    {
        // Existing code...
        
        // Start the safety coroutine to ensure all components are enabled
        StartCoroutine(EnsureAllComponentsEnabled(newInsect));
    }
}
