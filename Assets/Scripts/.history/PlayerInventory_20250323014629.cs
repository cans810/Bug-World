...
public class PlayerInventory : MonoBehaviour
{
    ...
    private IEnumerator PlayBarrierRemovalAnimation(BarrierRocksController barrier)
    {
        ...
        if (uiHelper != null)
        {
            uiHelper.ShowInformText("Your level 30 power is clearing the path ahead...");
        }
        ...
        // Fix the sound effect method call using the proper method name
        if (SoundEffectManager.Instance != null)
        {
            // Looks like PlaySound expects a Vector3 position as second parameter
            SoundEffectManager.Instance.PlaySound("RockBreak", barrier.transform.position);
            
            // If you need to pass volume, it's probably the third parameter
            // SoundEffectManager.Instance.PlaySound("RockBreak", barrier.transform.position, 0.8f);
        }
        
        // Fallback using AudioSource if needed
        else
        {
            AudioSource audioSource = GetComponent<AudioSource>();
            if (audioSource != null)
            {
                // Try to play a sound directly if SoundEffectManager isn't available
                if (levelUpSound != null) // Use any available sound clip as fallback
                {
    ...
    // Update the CheckForRockBarrierRemoval method to use the camera animation
