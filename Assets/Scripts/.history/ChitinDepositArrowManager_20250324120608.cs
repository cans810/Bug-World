...
public class ChitinDepositArrowManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Check if player is near the nest to auto-hide the arrow
        if (isArrowActive && nestTarget != null)
        {
            GameObject player = GameObject.FindGameObjectWithTag("Player");
            if (player != null)
            {
                // Calculate distance between player and nest
                float distance = Vector3.Distance(player.transform.position, nestTarget.transform.position);
                
                // If player is within range, hide the arrow and clear first-time flag
                if (distance < 5f) // 5 units seems reasonable, adjust as needed
                {
                    Debug.Log("Player near nest - hiding arrow");
                    ClearFirstTimeFlag();
                    
                    // If this is the first time tutorial, show a message about depositing
                    if (isShowingFirstTimeArrow)
                    {
                        UIHelper helper = FindObjectOfType<UIHelper>();
                        if (helper != null)
                        {
                            helper.ShowInformText("Press [E] to deposit your chitin!");
                        }
                    }
                }
            }
