...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        float halfLength = straightLength / 2;
        
        // Use fewer points for a cleaner outline
        int segments = 36;
        perimeterLineRenderer.positionCount = segments;
        
        // Divide the perimeter into 4 parts: top arc, right side, bottom arc, left side
        int pointsPerQuarter = segments / 4;
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            if (i < pointsPerQuarter) // Top semicircle
            {
                // Map from 0 to pointsPerQuarter -> angles from 0 to PI
                float angle = i * Mathf.PI / pointsPerQuarter;
                point = new Vector3(
                    center.x + radius * Mathf.Sin(angle),
                    perimeterHeight,
                    center.z + halfLength + radius * Mathf.Cos(angle)
                );
            }
            else if (i < 2 * pointsPerQuarter) // Right straight edge
            {
                // Calculate t based on position in this quarter
                float t = (i - pointsPerQuarter) / (float)pointsPerQuarter;
                point = new Vector3(
                    center.x + radius,
                    perimeterHeight,
                    center.z + halfLength - t * straightLength
                );
            }
            else if (i < 3 * pointsPerQuarter) // Bottom semicircle
            {
                // Map from 0 to pointsPerQuarter -> angles from PI to 2*PI
                float angle = Mathf.PI + (i - 2 * pointsPerQuarter) * Mathf.PI / pointsPerQuarter;
                point = new Vector3(
                    center.x + radius * Mathf.Sin(angle),
                    perimeterHeight,
                    center.z - halfLength + radius * Mathf.Cos(angle)
            ...
            else // Left straight edge
            {
                // Calculate t based on position in this quarter
                float t = (i - 3 * pointsPerQuarter) / (float)pointsPerQuarter;
                point = new Vector3(
                    center.x - radius,
                    perimeterHeight,
                    center.z - halfLength + t * straightLength
