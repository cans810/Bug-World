...
public class UIHelper : MonoBehaviour
{
    ...
    // Find the existing OnDiscoveredEntitiesButtonClicked method in the file
    public void OnDiscoveredEntitiesButtonClicked()
    {
        Debug.Log("Discovered Entities button clicked!");
        
        // Replace the simple call with our improved panel transition logic
        // Check if AttributesPanel is active
        if (attributesPanel != null && attributesPanel.activeSelf)
        {
            // Hide AttributesPanel first with animation
            StartCoroutine(TransitionBetweenPanels(attributesPanel, DiscoveredEntitiesPanel));
        }
        else
        {
            // Just show discovered entities panel normally
            if (DiscoveredEntitiesPanel != null)
            {
                DiscoveredEntitiesPanel.SetActive(true);
    ...
    // Then add method to show attributes panel (if it doesn't exist)
    public void ShowAttributesPanelWithTransition()
    {
        // Check if DiscoveredEntitiesPanel is active
        if (DiscoveredEntitiesPanel != null && DiscoveredEntitiesPanel.activeSelf)
        {
            // Hide DiscoveredEntitiesPanel first with animation
            StartCoroutine(TransitionBetweenPanels(DiscoveredEntitiesPanel, attributesPanel));
        }
        else
        {
            // Just show attributes panel normally
            if (attributesPanel != null)
            {
                attributesPanel.SetActive(true);
    ...
    // Coroutine to handle the transition between panels
    private IEnumerator TransitionBetweenPanels(GameObject panelToHide, GameObject panelToShow)
    {
        // Hide the first panel with animation
        Animator hideAnimator = panelToHide.GetComponent<Animator>();
        if (hideAnimator != null)
        {
            hideAnimator.SetBool("Hide", true);
            hideAnimator.SetBool("ShowUp", false);
            
            // Wait for animation to start
            yield return null;
            
            // Wait for a portion of the animation
            yield return new WaitForSeconds(0.5f);
        }
        else
        {
            // If no animator, just deactivate immediately
            panelToHide.SetActive(false);
        }
        
        // Now show the second panel
        if (panelToShow != null)
        {
            panelToShow.SetActive(true);
            
            // If animator exists, play show animation
            Animator showAnimator = panelToShow.GetComponent<Animator>();
            if (showAnimator != null)
            {
                showAnimator.SetBool("ShowUp", true);
                showAnimator.SetBool("Hide", false);
            }
        }
        
        // If the hide animation is still playing, complete it by fully deactivating
        // the panel after a short delay
        if (hideAnimator != null)
        {
