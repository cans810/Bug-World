...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnXPSymbols(Vector3 nestPosition, int count, int totalXPAmount)
    {
        ...
        // Flag to track if we've notified about the first symbol reaching destination
        bool firstSymbolNotified = false;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // For the first symbol, we'll pass a flag to indicate it's the first one
            bool isFirstSymbol = i == 0 && !firstSymbolNotified;
            if (isFirstSymbol) {
                firstSymbolNotified = true;
            }
            
            StartCoroutine(AnimateXPSymbolToScreen(symbol, screenTopLeft, xpPerSymbol, isFirstSymbol));
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpValue, bool isFirstSymbol = false)
    {
        ...
        // Track if we've notified about reaching destination
        bool hasNotifiedDestination = false;
        ...
        while (elapsed < duration)
        {
            ...
            // If this is the first symbol and it's about to reach its destination (95% of the way there)
            // notify the PlayerInventory to update the XP display
            if (isFirstSymbol && !hasNotifiedDestination && normalizedTime >= 0.95f)
            {
                // Find the player inventory and tell it to update the XP display
                PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
                if (playerInventory != null)
                {
                    // Trigger the XP display update for the total amount
                    playerInventory.TriggerXPDisplayUpdate(totalXPAmount);
                    Debug.Log("First XP symbol reached destination - triggering XP display update");
                }
                hasNotifiedDestination = true;
            }
        ...
        // We'll only do this if it's NOT the first symbol, since the first one
        // already triggered the update for the total amount
        if (incrementXPOneByOne && !isFirstSymbol)
        {
    ...
    // Add a field to track the total XP amount for the current effect
    private int totalXPAmount;
} 