...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator ShowLevelUpPanelAfterDelay(float delay, int newLevel)
    {
        ...
        {
            // Simply show the panel - it will check for level rewards internally
            levelUpHelper.ShowUpperPanel();
            
            // Log the level up
            Debug.Log($"Showing level up panel for level {newLevel}");
            
            // Play the level up sound effect if not already played
            if (SoundEffectManager.Instance != null)
            {
        ...
        else
        {
            ...
            // As a fallback, inform the player through the UI helper
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null)
            {
