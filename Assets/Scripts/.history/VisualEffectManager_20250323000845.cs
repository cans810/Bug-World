...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // SHORTER animation duration for snappier movement
        float duration = 1.0f; // Reduced from 1.8f to 1.0f
        ...
        while (elapsed < duration)
        {
            // Get the current target position - RECALCULATE every frame to handle UI movement
            Ray currentRay = Camera.main.ScreenPointToRay(targetScreenPos);
            Vector3 currentTargetPos = currentRay.GetPoint(10f);
            ...
            // Create a much flatter arc for more direct movement
            float arcHeight = 0.5f; // Fixed lower height
            Vector3 fixedMidPoint = Vector3.Lerp(startPos, currentTargetPos, 0.5f) + Vector3.up * arcHeight;
            
            // Bezier curve movement with MORE DIRECT path
            float curveValue = chitinFlyCurve.Evaluate(normalizedTime);
            Vector3 a = Vector3.Lerp(startPos, fixedMidPoint, curveValue);
            Vector3 b = Vector3.Lerp(fixedMidPoint, currentTargetPos, curveValue);
            
            // Move directly to target in final stage (last 20%)
            if (normalizedTime > 0.8f)
            {
                float finalT = (normalizedTime - 0.8f) / 0.2f;
                // Move QUICKLY to target at the end - use a stronger lerp factor
                symbol.transform.position = Vector3.Lerp(symbol.transform.position, currentTargetPos, finalT * 0.5f);
            }
            else
            {
                symbol.transform.position = Vector3.Lerp(a, b, curveValue);
            ...
            // Only fade out in the very last 10% of movement
            if (normalizedTime > 0.9f)
            {
                float fadeAmount = (normalizedTime - 0.9f) / 0.1f;
            ...
            if (!hasBumpedUI && normalizedTime > 0.65f)
            {
