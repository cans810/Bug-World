...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (distanceToPlayer <= followDistance)
        {
            ...
            // Face the player's direction (not position) when close
        ...
        else
        {
            ...
            // VERY IMPORTANT: Use direct transform-based movement exactly like EnemyAI
            
            // Get direction to player
            Vector3 directionToPlayer = playerTransform.position - transform.position;
            directionToPlayer.y = 0; // Keep on the same Y level
            directionToPlayer.Normalize();
            
            // First rotate to face player
    ...
    {
        // COMPLETELY DELEGATE control to AIWandering
        if (wanderingBehavior != null)
        {
            ...
            // IMPORTANT: Do not set hasAppliedMovementThisFrame here!
            // Removing this flag allows AIWandering to control movement
            
            // DEBUG
            Debug.Log($"AllyAI {gameObject.name}: Delegating movement to AIWandering");
        }
        else
        {
    ...
    // Override the Update method for better control
    public void OnAIWanderingStateChanged(bool isWandering)
    {
        // Only do this check when we're in wander mode
        if (currentMode == AIMode.Wander && lootModeEnabled && 
            carriedLoot == null && !isInCombat && !isChasing)
        {
            // Check for loot when the wandering AI changes state
            // This prevents checking every frame but still catches loot
            CheckForLoot();
        }
    }

    // This method should be called by AIWandering when it changes state
    public void NotifyWanderingStateChange(bool isWandering, bool isWaiting)
    {
        // Only handle if we're in wander mode and we're not carrying loot
        if (currentMode == AIMode.Wander && carriedLoot == null)
        {
            // When the AI starts moving (not waiting) after being still, check for loot
