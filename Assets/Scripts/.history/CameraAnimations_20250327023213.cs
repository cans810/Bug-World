...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator MetamorphosisCameraAnimation(Transform playerTransform, System.Action onCameraInPosition)
    {
        ...
        // UPDATED: Position camera above and to the right of the player
        Vector3 playerRight = playerTransform.right;
        Vector3 playerUp = Vector3.up;
        
        // Calculate positioning values based on player scale
        float rightOffset = playerTransform.localScale.x * 3.0f; // Position to the right
        float heightOffset = playerTransform.localScale.y * 4.0f; // Higher position for looking down
        float backOffset = -playerTransform.localScale.z * 1.5f; // Slightly behind
        
        // Combine offsets for final position
        Vector3 horizontalOffset = (playerRight * rightOffset) + (playerTransform.forward * backOffset);
        Vector3 targetPosition = playerTransform.position + horizontalOffset;
        targetPosition.y = playerTransform.position.y + heightOffset;
        
        // Calculate rotation to look down at the player from this offset position
