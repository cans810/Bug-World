...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        {
            // Check player's current speed instead of using IsMoving()
            AnimationController playerAnimController = playerTransform.GetComponent<AnimationController>();
            if (playerAnimController != null)
            {
                isPlayerMoving = playerAnimController.IsWalking();
            }
            else
            {
        ...
        // If player is not moving and we're close enough, stop and look around
    ...
    {
        // If we're not already looking around, start a new look
        if (!isLookingAround)
        {
            // Generate a random rotation around the Y axis within maxLookAngle
            float randomAngle = Random.Range(-maxLookAngle, maxLookAngle);
            Vector3 baseDirection = playerTransform != null ? 
                (playerTransform.position - transform.position).normalized : 
                transform.forward;
                
            // Create a rotation that's a variation from the base direction
            targetIdleRotation = Quaternion.Euler(0, randomAngle, 0) * Quaternion.LookRotation(baseDirection);
            
            // Set timer for this look
            idleLookTimer = Random.Range(2f, 4f);
        ...
        transform.rotation = Quaternion.Slerp(
            transform.rotation,
            targetIdleRotation,
            idleLookSpeed * Time.deltaTime
        );
        
        // Decrease timer
        idleLookTimer -= Time.deltaTime;
        
        // If timer is up, reset looking state
        if (idleLookTimer <= 0)
        {
            isLookingAround = false;
        }
