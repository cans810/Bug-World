...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        int capsuleDirection = capsuleCollider.direction;
        
        int segments = 60;
        perimeterLineRenderer.positionCount = segments;
        
        // Calculate direction vector based on capsule orientation
        Vector3 directionVector;
        if (capsuleDirection == 0) directionVector = Vector3.right;
        else if (capsuleDirection == 1) directionVector = Vector3.up;
        else directionVector = Vector3.forward;
        
        // Calculate the half height (distance from center to where the hemisphere begins)
        float halfHeight = (height - 2 * radius) * 0.5f;
        
        // Calculate the two points at the centers of the hemispheres
        Vector3 point1 = center + directionVector * halfHeight;
        Vector3 point2 = center - directionVector * halfHeight;
        
        // Project these points onto the XZ plane
        ...
        // For a Y-oriented capsule, the projection is just a circle on the XZ plane
        if (capsuleDirection == 1) // Y-axis
        {
            for (int i = 0; i < segments; i++)
            {
                float angle = (float)i / segments * 2f * Mathf.PI;
        ...
        // For X or Z oriented capsules, we need to create a stadium shape
        // First, find the perpendicular vector on the XZ plane
        Vector3 perpVector;
        if (capsuleDirection == 0) // X-axis
            perpVector = Vector3.forward;
        else // Z-axis
            perpVector = Vector3.right;
        
        // Calculate points for the stadium shape    
        for (int i = 0; i < segments; i++)
        {
            float angle = (float)i / segments * 2f * Mathf.PI;
            Vector3 position;
            
            // First half of the circle - one end cap plus one straight edge
            if (angle < Mathf.PI)
            {
                // For 0 to π/2: Part of first hemisphere
                if (angle < Mathf.PI/2)
                {
                    float circleAngle = angle;
                    if (capsuleDirection == 0) // X-axis
                    {
                        position = projectedPoint1 + perpVector * radius * Mathf.Sin(circleAngle);
                    }
                    else // Z-axis
                    {
                        position = projectedPoint1 + perpVector * radius * Mathf.Sin(circleAngle);
                ...
                // For π/2 to π: Part of second hemisphere
                else
                {
                    float circleAngle = angle - Mathf.PI/2;
                    if (capsuleDirection == 0) // X-axis
                    {
                        position = projectedPoint2 + perpVector * radius * Mathf.Sin(circleAngle);
                    }
                    else // Z-axis
                    {
                        position = projectedPoint2 + perpVector * radius * Mathf.Sin(circleAngle);
            ...
            // Second half of the circle - one end cap plus one straight edge
            else
            {
                // For π to 3π/2: Continue second hemisphere
                if (angle < 3*Mathf.PI/2)
                {
                    float circleAngle = angle - Mathf.PI;
                    if (capsuleDirection == 0) // X-axis
                    {
                        position = projectedPoint2 - perpVector * radius * Mathf.Sin(circleAngle);
                    }
                ...
                // For 3π/2 to 2π: Back to first hemisphere
                else
                {
                    float circleAngle = angle - 3*Mathf.PI/2;
                    if (capsuleDirection == 0) // X-axis
                    {
                        position = projectedPoint1 - perpVector * radius * Mathf.Sin(circleAngle);
                    }
            ...
            perimeterLineRenderer.SetPosition(i, position);
        }
    }
