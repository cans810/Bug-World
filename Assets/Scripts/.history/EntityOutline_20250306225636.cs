...
public class EntityOutline : MonoBehaviour
{
    ...
    private void Start()
    {
        // If entity reference is not set, use this GameObject
        if (entity == null)
            entity = gameObject;
        ...
        // Force a check after a short delay to ensure outline is properly applied
        Invoke("CheckAndRestoreOutline", 0.5f);
    }
    ...
    private void CheckAndRestoreOutline()
    {
        ...
        {
            needsRestore = true;
            Debug.Log($"Outline missing or disabled on {entity.name}", entity);
        }
        else
        {
            // Check if materials are properly set
            Renderer[] renderers = entity.GetComponentsInChildren<Renderer>(true); // Include inactive children
            ...
            {
                needsRestore = true;
                Debug.Log($"No outline material found on {entity.name} - performing reset", entity);
            }
        }
    ...
    {
        // Try to find outline component even if it's on a child
        outline = entity.GetComponent<Outline>();
        if (outline == null)
        {
            // Try to find on any children
            outline = entity.GetComponentInChildren<Outline>(true);
        }
        
        // If still null, add to main entity
        if (outline == null)
        ...
        // Add another delayed check to handle any race conditions
    ...
    private void FixOutlineColor()
    {
        ...
        Renderer[] renderers = entity.GetComponentsInChildren<Renderer>(true); // Include inactive renderers
        foreach (Renderer renderer in renderers)
        {
            ...
            try {
                ...
                Debug.LogWarning($"Error while fixing outline color on {renderer.name}: {e.Message}", renderer);
            }
        }
    }
