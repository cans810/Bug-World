...
public class PlayerInventory : MonoBehaviour
{
    ...
    public void CollectChitin(GameObject chitinObject)
    {
        ...
        // Fix the hasYetToCollectChitin flag logic
        if (hasYetToCollectChitin && ChitinCount > 0)
        {
            // Only show guidance the first time
            hasYetToCollectChitin = false;
            
            // Show the guidance (this should only happen once now)
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null)
            {
                uiHelper.ShowInformText("Go to your nest to deposit your chitin!");
                
                // Show direction arrow to nest
                LevelAreaArrowManager arrowManager = FindObjectOfType<LevelAreaArrowManager>();
                if (arrowManager != null)
                {
                    // Call method to show arrow to nest
                    arrowManager.ShowArrowToNest();
                }
            }
            
            // Save the game to persist this flag
