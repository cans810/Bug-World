...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Add this method to visualize mission coin rewards
    public void PlayMissionCoinRewardEffect(RectTransform missionPanelRect, int coinAmount)
    {
        if (coinSymbolPrefab == null)
        {
            Debug.LogError("Coin Symbol prefab not assigned!");
        ...
        // Make sure we have the coin panel reference
        if (coinPanel == null)
        {
            Debug.LogWarning("Coin panel reference is missing. Finding it now...");
            coinPanel = GameObject.Find("CoinPanel");
            
            if (coinPanel == null)
            {
                Debug.LogError("Could not find CoinPanel in the scene!");
                return;
            }
        }
        
        // Make sure we have a valid canvas safe area
        if (canvasSafeArea == null)
        {
            Debug.LogWarning("Canvas Safe Area reference is missing. Finding it now...");
            canvasSafeArea = GameObject.Find("SafeArea");
            
            if (canvasSafeArea == null)
            {
                Debug.LogError("Could not find SafeArea in the scene!");
                return;
            }
        }
        
        // Convert the mission panel position to world space
        Vector3 missionPanelWorldPos = GetWorldPositionFromUI(missionPanelRect);
        
        // Debug positions
        Debug.Log($"Mission panel position: {missionPanelWorldPos}, Target coin panel: {coinPanel.name}");
        
        // Spawn 3-5 coin symbols based on reward amount
        ...
        StartCoroutine(SpawnCoinSymbolsFromMission(missionPanelWorldPos, symbolCount, coinAmount));
    ...
    // Helper coroutine to spawn multiple coin symbols with staggered timing
    private IEnumerator SpawnCoinSymbolsFromMission(Vector3 startPosition, int count, int totalCoins)
    {
        for (int i = 0; i < count; i++)
        {
            // Get a coin symbol from the pool
            GameObject coinSymbol = GetSymbolFromPool("coin", coinSymbolPrefab);
            ...
            coinSymbol.transform.position = startPosition + offset;
            
            // Set coin symbol properties
            coinSymbol.transform.localScale = new Vector3(0.4f, 0.4f, 0.4f); // Larger scale for visibility
            coinSymbol.SetActive(true);
            
            // Ensure looking at camera
            coinSymbol.transform.rotation = cameraTransform.rotation;
            
            // Start flying animation to coin panel
            StartCoroutine(FlyCoinSymbolToUI(coinSymbol, coinPanel.GetComponent<RectTransform>()));
            
            // Add short delay between spawning each symbol
            yield return new WaitForSeconds(0.08f);
        }
    }

    // Animation for coin symbol flying to the coin panel
    private IEnumerator FlyCoinSymbolToUI(GameObject coinSymbol, RectTransform targetPanel)
    {
        if (coinSymbol == null || targetPanel == null) yield break;
        
        // Get target position (coin panel in world space)
        Vector3 targetPosition = GetWorldPositionFromUI(targetPanel);
        ...
        Vector3 startScale = coinSymbol.transform.localScale;
        
        // Cache sprite renderer
        SpriteRenderer spriteRenderer = coinSymbol.GetComponent<SpriteRenderer>();
        Color originalColor = spriteRenderer != null ? spriteRenderer.color : Color.white;
        
        // Animate the coin to the target
        while (elapsed < duration)
        {
            if (coinSymbol == null) yield break; // Safety check
            
            elapsed += Time.deltaTime;
            float t = Mathf.SmoothStep(0, 1, elapsed / duration); // Smooth easing
            
            // Calculate arc path with higher peak for better visibility
            float height = Mathf.Sin(t * Mathf.PI) * 2f; // Higher arc
            Vector3 arcOffset = Vector3.up * height;
            
            // Update position with arc
            coinSymbol.transform.position = Vector3.Lerp(startPos, targetPosition, t) + arcOffset;
            
            // Scale down slightly near the end
            if (t > 0.8f)
            {
                float scaleT = (t - 0.8f) / 0.2f;
                coinSymbol.transform.localScale = Vector3.Lerp(startScale, startScale * 0.5f, scaleT);
                
                // Fade out at the very end
                if (spriteRenderer != null)
                {
                    Color newColor = originalColor;
                    newColor.a = Mathf.Lerp(1f, 0f, scaleT);
        ...
        // Make panel flash or pulse when coin arrives
        RectTransform panelRect = targetPanel;
        if (panelRect != null)
        {
            StartCoroutine(PulseUIElement(panelRect));
        }
        ...
        if (coinSymbol != null)
        {
            ...
            coinSymbol.transform.localScale = startScale;
