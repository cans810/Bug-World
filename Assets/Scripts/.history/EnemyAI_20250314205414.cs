...
public class EnemyAI : MonoBehaviour
{
    ...
    // Replace the existing boundary collision handlers with this simpler version
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            // Get collision normal and reflect movement
            ReflectAtBoundary(collision.contacts[0].normal);
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Reflect movement
            ReflectAtBoundary(normal);
        }
    }

    private void ReflectAtBoundary(Vector3 normal)
    {
        // Ensure the normal is on the horizontal plane and normalized
        normal.y = 0;
        normal.Normalize();
        
        // Move slightly away from boundary to prevent getting stuck
        ...
        // Add a small random angle to break up uniform movement
        reflectionDirection = Quaternion.Euler(0, Random.Range(-15f, 15f), 0) * reflectionDirection;
        
        // Simply rotate to face the reflection direction
        transform.rotation = Quaternion.LookRotation(reflectionDirection);
        
        // Don't touch the wandering behavior - let it continue naturally in the new direction
    }
} 