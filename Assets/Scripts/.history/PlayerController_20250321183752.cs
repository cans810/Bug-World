...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Add this to the Start method or where movement is initialized
        ApplySpeedAttribute();
    }
    ...
    {
        // Get the PlayerAttributes component
        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
        if (playerAttributes != null)
        {
            // Apply the speed multiplier to movement speed
            float speedMultiplier = playerAttributes.SpeedMultiplier;
            
            // Modify the movement speed based on the attribute
            // Use your existing movement system's variables here
            maxSpeed = baseMaxSpeed * speedMultiplier;
            acceleration = baseAcceleration * speedMultiplier;
            
            Debug.Log($"Applied speed attribute multiplier: {speedMultiplier:F2}x");
        }
    }

    // Call this method whenever attributes change (subscribe to the event)
