...
public class AllyAI : MonoBehaviour
{
    ...
    private void DropLoot(Vector3 dropPosition)
    {
        ...
        if (originalMode == AIMode.Wander && wanderingBehavior != null)
        {
            ...
            if (wanderingBehavior is AIWandering wander)
            {
                // Try to access the public fields/methods that might exist
                try {
                    // Use reflection to force a new wander target
                    System.Reflection.FieldInfo timeField = wander.GetType().GetField("currentWanderTime", 
                        System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                    if (timeField != null)
                    {
                        timeField.SetValue(wander, 999f); // Force timeout of current wander target
                    }
                    
                    Debug.Log($"Reset wandering timer for {gameObject.name}");
                }
                catch (System.Exception e) {
                    Debug.LogWarning($"Could not reset wander target: {e.Message}");
                }
            }
        }
