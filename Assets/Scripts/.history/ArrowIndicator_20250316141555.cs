...
public class ArrowIndicator : MonoBehaviour
{
    ...
    private void UpdateArrowPosition()
    {
        ...
        // Calculate direction from target to player in world space
        Vector3 directionToPlayer = playerWorldPos - targetWorldPos;
        ...
        Vector2 screenDirection = new Vector2(playerScreenPos.x - targetScreenPos.x, 
        ...
        // Debug the angle
        Debug.Log($"Arrow angle: {angle}, Direction: {screenDirection}");

        // Force the arrow to face the camera and apply rotation
        arrowObject.transform.rotation = Quaternion.identity; // Reset rotation
        arrowObject.transform.Rotate(0, 0, angle); // Apply only Z rotation
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            // Calculate direction from target to player in screen space (reversed direction)
            ...
            if (!isOnScreen)
            {
                ...
                Vector2 arrowScreenPos = ClampToScreenEdge(screenCenter, directionToPlayer, minX, maxX, minY, maxY);
                ...
                // Make sure the sprite renderer is set to face the camera
                SpriteRenderer renderer = arrowObject.GetComponent<SpriteRenderer>();
                if (renderer != null)
                {
                    renderer.transform.forward = mainCamera.transform.forward;
                }
            }
            else
            {
                ...
                // Use the same angle calculation for consistency
                arrowObject.transform.rotation = Quaternion.Euler(0, 0, angle);
            }
