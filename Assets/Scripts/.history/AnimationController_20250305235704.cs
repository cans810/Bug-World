...
public class AnimationController : MonoBehaviour
{
    ...
    public void SetAttackWalking(bool attackWalking)
    {
        ...
        {
            // Add debugging
            Debug.Log($"Setting AttackWalking to {attackWalking} - HasAnimation={HasAnimation(attackWalkingParam)}");
            
            // Set the parameter if it exists
            if (HasAnimation(attackWalkingParam))
            {
                // First, reset other states that shouldn't be active during attack walking
                if (attackWalking)
                {
                    ResetStates();
                    isAttackWalking = true;
                    isAttacking = false;
                    isWalking = false;
                    isIdle = false;
                }
                ...
                Debug.Log($"Setting Animator.SetBool('{attackWalkingParam}', {attackWalking})");
                animator.SetBool(attackWalkingParam, attackWalking);
                
                // Update all animation parameters
                UpdateAnimatorParameters();
                
                // Verify parameter was set
                Debug.Log($"After setting, AttackWalking param = {animator.GetBool(attackWalkingParam)}");
                Debug.Log($"States after update: Idle={isIdle}, Walking={isWalking}, Attacking={isAttacking}, AttackWalking={isAttackWalking}");
            }
            else
            {
                // Fallback behavior - this can be customized as needed
                Debug.Log($"No AttackWalking animation, using fallback (setting Walk={attackWalking}, Attack={attackWalking})");
                // Use standard walking and attacking animations as fallback
                isWalking = attackWalking;
                isAttacking = attackWalking;
                isIdle = !attackWalking;
                UpdateAnimatorParameters();
            }
        }
    }
