...
public class GameManager : MonoBehaviour
{
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (entitiesController != null)
        {
            // Initialize and load discovered entities data
            entitiesController.LoadDiscoveredEntities(gameData);
            
            // Force refresh the UI
            entitiesController.UpdateDiscoveredEntitiesUI();
            
            Debug.Log($"Loaded discovered entities data - Flea discovered: {gameData.hasDiscoveredFlea}");

            // Load equipped entity state after other entity data is loaded
