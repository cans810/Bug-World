...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modified method to prevent teleporting - just stop in place without forcing position
    private void HandlePlayerEnterHitbox(LivingEntity playerEntity)
    {
        ...
        // Stop all physics movement but don't change kinematic state
        Rigidbody rb = GetComponent<Rigidbody>();
        if (rb != null)
        {
            rb.velocity = Vector3.zero;
        ...
        // Update animation to idle first
        if (animController != null)
        {
            animController.SetIdle();
        }
        
        // Start attacking in place without trying to force position
        StartCoroutine(AttackInPlace());
    }

    // Simplified coroutine that doesn't try to force position
