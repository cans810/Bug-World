...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Check if current target has died - immediately stop chasing if so
        if (currentTarget != null && currentTarget.IsDead)
        {
            // Target is dead, return to wandering immediately
            ReturnToWandering();
            return;
        }
    ...
    {
        // Immediately check if target is dead or null before proceeding
        if (currentTarget == null || currentTarget.IsDead)
        {
            ReturnToWandering();
            return;
    ...
    // Modify ReturnToWandering to be more forceful and clean up state
    private void ReturnToWandering()
    {
        ...
        // Clear any residual animation states
        if (animController != null)
        {
            animController.SetAttacking(false);
        }
        
        // Force the Rigidbody to stop if it exists
        ...
        // Re-enable wandering behavior with a small delay
        StartCoroutine(EnableWanderingAfterDelay(0.5f));
    }
    
    // New helper method to enable wandering after a short delay
    private IEnumerator EnableWanderingAfterDelay(float delay)
    {
