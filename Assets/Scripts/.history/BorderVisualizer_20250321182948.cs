...
public class BorderVisualizer : MonoBehaviour
{
    ...
    // Improved FadeOutVisualization coroutine to better handle all visualization types
    private IEnumerator FadeOutVisualization()
    {
        ...
        if (visualSphere == null)
        {
            Debug.LogWarning($"No visualization to fade for border: {gameObject.name}");
            hasBeenDisabled = true;
            isFading = false;
            yield break;
        }
        
        // Get all renderers in the visualization (handles both sphere and capsule)
        Renderer[] renderers = visualSphere.GetComponentsInChildren<Renderer>(true);
        
        // If no renderers found, also check for LineRenderer components
        if (renderers.Length == 0)
        {
            LineRenderer[] lineRenderers = visualSphere.GetComponentsInChildren<LineRenderer>(true);
            if (lineRenderers.Length > 0)
            {
                // Handle line renderers separately
                float lineElapsedTime = 0f;
                Color startColor = lineRenderers[0].startColor;
                
                while (lineElapsedTime < fadeOutDuration)
                {
                    float alpha = 1f - (lineElapsedTime / fadeOutDuration);
                    ...
                    lineElapsedTime += Time.deltaTime;
                    yield return null;
                }
                ...
                foreach (LineRenderer line in lineRenderers)
                {
                    Color finalColor = line.startColor;
                    finalColor.a = 0f;
            ...
            // No regular renderers found, just disable
            visualSphere.SetActive(false);
            hasBeenDisabled = true;
            isFading = false;
            Debug.Log($"Border visualization disabled (no regular renderers): {gameObject.name}");
            yield break;
        }
        
        // Regular fade out for standard renderers
        ...
        while (elapsedTime < fadeOutDuration)
        {
            ...
            foreach (Material mat in materials)
            {
                ...
                Color newColor = new Color(
                    ...
                    originalColor.g, 
                    originalColor.b, 
                    originalColor.a * (1f - normalizedTime)
                );
                mat.color = newColor;
            }
        ...
        foreach (Material mat in materials)
        {
            Color finalColor = originalColors[mat];
            mat.color = new Color(finalColor.r, finalColor.g, finalColor.b, 0f);
        }
        
        // After fading, disable the visualization
        yield return new WaitForSeconds(0.2f);
        visualSphere.SetActive(false);
        hasBeenDisabled = true;
        isFading = false;
        
        // Ensure the collider is still active but with isTrigger = true
        ...
        Debug.Log($"Border visualization fade complete and disabled: {gameObject.name}");
    }
    ...
    // Add this method to check if player can enter this border area
    public bool CanPlayerEnterArea(int playerLevel)
    {
        return requiredPlayerLevel <= playerLevel || requiredPlayerLevel == 0;
    }

    // Modify the OnTriggerEnter method in BorderVisualizer
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            // Only show border name if player meets level requirement
            if (CanPlayerEnterArea(playerLevel))
            {
                // Show border name or trigger border effects
                PlayerController playerController = other.GetComponent<PlayerController>();
                if (playerController != null)
                {
                    playerController.SetCurrentBorder(gameObject.name, requiredPlayerLevel, transform.position);
                }
                
                // Your existing border enter code...
            }
            else
            {
                // Player doesn't meet level requirement, show level requirement message
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText($"Level {requiredPlayerLevel} required to enter this area");
                }
                
                // Explicitly set border as inaccessible
                PlayerController playerController = other.GetComponent<PlayerController>();
                if (playerController != null)
                {
                    playerController.SetBorderInaccessible(gameObject.name, requiredPlayerLevel);
                }
            }
        }
    }
