...
public class ChitinBehavior : MonoBehaviour
{
    ...
    [SerializeField] private float minArcHeight = 0.5f;  // REDUCED height for smaller arc
    ...
    [SerializeField] private float horizontalForce = 0.5f; // REDUCED scatter force
    ...
    private void ApplyInitialForce()
    {
        if (rb != null && !hasAppliedForce)
        {
            ...
            rb.angularVelocity = new Vector3(
                UnityEngine.Random.Range(-spinSpeed, spinSpeed),
                UnityEngine.Random.Range(-spinSpeed, spinSpeed),
                UnityEngine.Random.Range(-spinSpeed, spinSpeed)
            );
