...
public class AIWandering : MonoBehaviour
{
        ...
        while (!validWaypointFound && attempts < maxAttempts)
        {
            ...
            else
            {
                ...
                if (hasTerritory && respectTerritory)
                {
                    ...
                    if (isRingTerritory)
                    {
                        // For ring territories, check if position is within the ring using stricter boundaries
                        float outerBuffer = Mathf.Min(2f, (territoryRadius - innerTerritoryRadius) * 0.1f); // 10% of ring width or 2 units
                        withinTerritory = distanceToCenter >= (innerTerritoryRadius + outerBuffer) && 
                                          distanceToCenter <= (territoryRadius - outerBuffer);
                    }
                    else
                    {
                        // For regular territories, use a larger buffer from the edge
                        float outerBuffer = Mathf.Min(3f, territoryRadius * 0.1f); // 10% of territory radius or 3 units
                        withinTerritory = distanceToCenter <= (territoryRadius - outerBuffer);
                    }
                }
    ...
    private void EnforceTerritory()
    {
        ...
        // Check if we're near or past a boundary
        bool nearBoundary = false;
        Vector3 awayFromBoundary = Vector3.zero;
        
        if (isRingTerritory)
        {
            // For ring territories, check both inner and outer boundaries
            if (distanceToCenter <= innerTerritoryRadius + territoryRespectDistance)
            {
                // Too close to inner boundary, get direction away from center
                nearBoundary = true;
                awayFromBoundary = (flatPosition - flatCenter).normalized;
            }
            else if (distanceToCenter >= territoryRadius - territoryRespectDistance)
            {
                // Too close to outer boundary, get direction toward center
                nearBoundary = true;
                awayFromBoundary = (flatCenter - flatPosition).normalized;
            }
        }
        else
        {
            // For regular territories, only check outer boundary
        ...
        if (nearBoundary)
        {
            // Immediately reverse direction - set waypoint in the opposite direction
            float turnDistance = Random.Range(minWanderDistance, maxWanderDistance);
            
            // Make sure the new waypoint is well inside acceptable bounds
            currentWaypoint = transform.position + awayFromBoundary * turnDistance;
            
            // Immediately face the new direction
            if (livingEntity != null)
            {
                livingEntity.RotateTowards(awayFromBoundary, 10f); // Fast rotation
            }
            
            // Debug visualization
            Debug.DrawLine(transform.position, currentWaypoint, Color.red, 0.5f);
            Debug.DrawRay(transform.position, awayFromBoundary * 2f, Color.yellow, 0.5f);
        }
    }
}