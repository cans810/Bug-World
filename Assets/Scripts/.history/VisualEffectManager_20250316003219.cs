...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private float xpSymbolSpawnOffsetY = 100f;
    ...
    public void PlayXPGainEffect(Vector3 nestPosition, int xpAmount)
    {
        ...
        // Calculate number of symbols based on XP amount (1-5 symbols)
        int symbolCount = Mathf.Clamp(xpAmount / 5, 1, 5);
    ...
    {
        // Use fixed screen coordinates for top left (XP counter location)
        Vector2 screenTopLeft = new Vector2(100f, Screen.height - 50f); // Adjust these values to match your UI
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Make sure the symbol is visible - use a slightly larger scale
            symbol.transform.localScale = baseScale * 1.5f;
            
            // Create random offset around the nest position
            Vector2 randomOffset = Random.insideUnitCircle * xpSpreadRadius;
            Vector3 spawnPos = nestPosition + new Vector3(randomOffset.x, 1f + xpSymbolSpawnOffsetY, randomOffset.y);
            
            symbol.transform.position = spawnPos;
            symbol.transform.rotation = cameraTransform.rotation;
            symbol.SetActive(true);
            
            // Debug log to confirm symbols are being spawned
            Debug.Log($"Spawning XP symbol at {spawnPos}, target: {screenTopLeft}");
            
            // Animate the XP symbol to fly to the UI position
            StartCoroutine(AnimateXPSymbol(symbol, screenTopLeft));
    ...
    private IEnumerator AnimateXPSymbol(GameObject symbol, Vector2 targetScreenPos)
    {
        Vector3 startPos = symbol.transform.position;
        ...
        // Make sure the symbol is visible - use a bright color
        Color startColor = spriteRenderer.color;
        spriteRenderer.color = new Color(1f, 0.8f, 0.2f, 1f); // Bright gold color
        
        float elapsed = 0f;
        
        // Create a curved path with midpoint above
        Vector3 midPoint = Vector3.Lerp(startPos, targetPos, 0.5f);
        midPoint += Vector3.up * Random.Range(2f, 4f); // Higher arc for better visibility
        
        // Log the path for debugging
        Debug.Log($"XP Symbol path: {startPos} -> {midPoint} -> {targetPos}");
        
        while (elapsed < xpFlyDuration)
        {
            elapsed += Time.deltaTime;
            float normalizedTime = elapsed / xpFlyDuration;
            float curveValue = chitinFlyCurve.Evaluate(normalizedTime);
            ...
            {
                float alpha = Mathf.Lerp(1f, 0f, (normalizedTime - 0.8f) / 0.2f);
                spriteRenderer.color = new Color(1f, 0.8f, 0.2f, alpha);
            }
