...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this field to cache the nest reference
    public GameObject cachedNestObject;
    ...
    // Simplify the main animation method
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Basic validation
        if (chitinCount <= 0 || chitinIconPrefab == null || cachedNestObject == null)
        {
            Debug.LogError("[CRITICAL] Invalid animation parameters");
        ...
        // Get positions
        Vector3 startPos = chitinCountText != null ? chitinCountText.transform.position : new Vector3(Screen.width * 0.9f, Screen.height * 0.9f, 0);
        Vector3 targetPos = Camera.main.WorldToScreenPoint(cachedNestObject.transform.position);
        targetPos.z = 0;
        
        Debug.Log($"[CRITICAL] Animation from {startPos} to {targetPos}");
        
        // Create and animate icons one by one
        for (int i = 0; i < chitinCount; i++)
        {
            // Create icon
            GameObject icon = Instantiate(chitinIconPrefab, transform);
            icon.name = $"ChitinIcon_{i}";
            
            // Position with offset
            Vector3 offset = new Vector3(i * 20, i * 20, 0);
            icon.transform.position = startPos + offset;
            
            Debug.Log($"[CRITICAL] Created icon {i+1} at {icon.transform.position}");
            
            // Start animation immediately
            StartCoroutine(AnimateIconSafe(icon, icon.transform.position, targetPos, icon.transform.localScale, 1.5f, i+1));
            
            // Wait a bit before spawning next icon
    ...
    // Get the nest reference
    private GameObject GetNestObject()
    {
        return cachedNestObject;
    }

    // Modified helper method with more debugging
    private IEnumerator AnimateIconSafe(GameObject icon, Vector3 startPosition, Vector3 targetPos, Vector3 originalScale, float duration, int iconIndex)
    {
        if (icon == null) yield break;
        
        Debug.Log($"[CRITICAL] ANIMATION STARTING for icon {iconIndex} from {startPosition} to {targetPos}");
        
        // Make sure the icon is visible
        Image iconImage = icon.GetComponent<Image>();
        if (iconImage != null)
        {
            iconImage.color = Color.red; // Make it bright red for visibility
            iconImage.raycastTarget = false; // Ensure it doesn't block raycasts
            
            // Make sure it has a sprite
            if (iconImage.sprite == null)
            {
                // Try to find a default sprite
                iconImage.sprite = Resources.FindObjectsOfTypeAll<Sprite>()
                    .FirstOrDefault(s => s.name.Contains("Square") || s.name.Contains("Circle"));
                
                if (iconImage.sprite == null)
                {
                    Debug.LogError($"[CRITICAL] Icon {iconIndex} has no sprite!");
                }
            }
            
            Debug.Log($"[CRITICAL] Set icon {iconIndex} color to red for visibility");
        }
        ...
        {
            rectTransform.sizeDelta = new Vector2(100, 100); // Make it large
            Debug.Log($"[CRITICAL] Set icon {iconIndex} size to 100x100");
        }
        
        // Simple, direct animation with no fancy error handling
        float startTime = Time.time;
        float endTime = startTime + duration;
        
        while (Time.time < endTime && icon != null)
        {
            // Calculate progress
            float elapsed = Time.time - startTime;
            float normalizedTime = elapsed / duration;
            
            // Get current nest position
            if (cachedNestObject != null)
            {
                targetPos = Camera.main.WorldToScreenPoint(cachedNestObject.transform.position);
                targetPos.z = 0;
            }
            
            // Calculate position with arc
            float arcHeight = 200f * Mathf.Sin(Mathf.PI * normalizedTime);
            Vector3 newPos = Vector3.Lerp(startPosition, targetPos, normalizedTime);
            newPos.y += arcHeight;
            ...
            icon.transform.Rotate(0, 0, 360 * Time.deltaTime); // Rotate 360 degrees per second
            
            // Log every few frames
            if (Mathf.FloorToInt(elapsed * 10) % 3 == 0)
            {
                Debug.Log($"[CRITICAL] MOVING icon {iconIndex}: {newPos}, progress: {normalizedTime:F2}");
            }
            
            yield return null;
        }
        
        Debug.Log($"[CRITICAL] ANIMATION COMPLETE for icon {iconIndex}");
        ...
        // Ensure the icon is in front
        Canvas canvas = GetComponent<Canvas>();
        if (canvas != null)
        {
            // Set a high sorting order
            canvas.sortingOrder = 100;
            Debug.Log($"[CRITICAL] Set canvas sorting order to 100");
        }

        // Or alternatively, add a Canvas component to each icon
        Canvas iconCanvas = icon.AddComponent<Canvas>();
    ...
    private void ValidateChitinIconPrefab()
    {
        ...
        // Make sure the prefab has a tag for finding it later
        if (string.IsNullOrEmpty(chitinIconPrefab.tag) || chitinIconPrefab.tag == "Untagged")
        {
            Debug.LogWarning("Setting chitin icon prefab tag to 'ChitinIcon'");
            // Note: This won't actually change the prefab, just inform the developer
        }
    }

    // Add this method back to fix the error in PlayerInventory
    ...
    // Add this method to your UIHelper class
    public void CreateTestButton()
    {
        GameObject buttonObj = new GameObject("TestChitinButton");
    ...
    // Call this from the Unity Editor using a context menu item
