...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Play XP gained sound when the symbol reaches its destination
        // Make sure the sound is audible and plays for each symbol
        if (SoundEffectManager.Instance != null)
        {
            // Use 2D sound (false for is3D) since this is a UI effect
            // Increase volume slightly to ensure it's audible
            SoundEffectManager.Instance.PlaySound(xpGainedSoundName, Vector3.zero, false);
            
            // Add debug log to verify sound is playing
            Debug.Log($"Playing XP gained sound: {xpGainedSoundName}");
        }
        else
        {
            Debug.LogWarning("SoundEffectManager not found - cannot play XP gained sound");
        }
        
        // Reset and return to pool
        spriteRenderer.color = startColor;
        symbol.transform.localScale = initialScale; // Reset to original scale
        symbol.SetActive(false);
