...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            // Try to find and subscribe to the player inventory
        ...
        // Hide border visualizations for areas the player has already unlocked
        StartCoroutine(HideUnlockedBorderVisualizations());
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            else if (requiredLevel > 0 && playerLevel >= requiredLevel)
            {
                ...
                // Hide the border visualization since player now has access
                HideBorderVisualization(other.gameObject);
    ...
    // Update the HideBorderVisualization method to use a simpler player identifier
    private void HideBorderVisualization(GameObject borderObject)
    {
        if (borderObject == null) return;
        
        // Find the BorderVisualizer component on the border object
        BorderVisualizer visualizer = borderObject.GetComponent<BorderVisualizer>();
        
        if (visualizer != null)
        {
            // Check if we've already hidden this border
            string borderName = borderObject.name;
            
            // Use a simple constant identifier for the player
            string playerIdentifier = "Player";
            
            // Store hidden borders in player prefs to persist between sessions
            string hiddenBordersKey = "HiddenBorders_" + playerIdentifier;
            string hiddenBorders = PlayerPrefs.GetString(hiddenBordersKey, "");
            
            // If this border is not already hidden
            if (!hiddenBorders.Contains(borderName))
            {
                // Log for debugging
                if (showDebugMessages)
                {
                    Debug.Log($"Hiding border visualization for {borderName} - player level high enough");
                }
                
                // Disable the visualization
                visualizer.ShowInGame = false;
                
                // Add to hidden borders list
                hiddenBorders += (hiddenBorders.Length > 0 ? "," : "") + borderName;
                PlayerPrefs.SetString(hiddenBordersKey, hiddenBorders);
                PlayerPrefs.Save();
            }
        }
    }

    private IEnumerator HideUnlockedBorderVisualizations()
    {
        ...
        // Find all map borders in the scene
        GameObject[] mapBorders = GameObject.FindGameObjectsWithTag("MapBorder");
        
        // First hide all borders the player has already unlocked
        foreach (GameObject border in mapBorders)
        {
            // Get the required level for this border
            int requiredLevel = GetRequiredLevelForBorder(border.name);
            
            // If player level is high enough, hide the visualization
            if (requiredLevel > 0 && playerLevel >= requiredLevel)
            {
                HideBorderVisualization(border);
            }
        }
        
        // Now check for newly unlocked areas based on the player's current level
        // and trigger a camera animation to show them
        CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
        if (cameraAnimations != null && playerLevel > 1) // Only for players above level 1
        {
            // Check if this level unlocks a new area
            bool newAreaUnlocked = cameraAnimations.IsNewAreaUnlockedAtLevel(playerLevel);
            
            if (newAreaUnlocked)
            {
                Debug.Log($"Found newly unlocked area for level {playerLevel}, showing camera animation");
                cameraAnimations.ShowAreaByLevel(playerLevel);
            }
        }
    }
    ...
    {
        // Return the required level based on border name
        if (borderName == "MapBorder2") return 5;
        else if (borderName == "MapBorder3") return 10;
        else if (borderName == "MapBorder4") return 15;
        else if (borderName == "MapBorder5") return 25;
        else if (borderName == "MapBorder6") return 30;
        else if (borderName == "MapBorder7") return 35;
        else if (borderName == "MapBorder9") return 40;
        else if (borderName == "MapBorder10") return 45;
        else if (borderName == "MapBorder11") return 50;
        else if (borderName == "MapBorder12") return 55;
        
        return 0; // No level requirement
    }

    // Add this method to handle level up events
    private void OnPlayerLevelUp(int newLevel)
    {
        Debug.Log($"PlayerController received level up event: Level {newLevel}");
        
        // Find all map borders that the player can now pass through
        GameObject[] mapBorders = GameObject.FindGameObjectsWithTag("MapBorder");
        ...
        foreach (GameObject border in mapBorders)
        {
            ...
            // If this border is now unlocked by the new level
            if (requiredLevel == newLevel)
            {
                Debug.Log($"Unlocked border {border.name} at level {newLevel}");
                
                // Hide the border visualization
                HideBorderVisualization(border);
                
                // No need to manually trigger camera animation here as it should
                // be handled by PlayerInventory.HandleLevelUpSequence
            }
        }
    }
}