...
public class CameraAnimations : MonoBehaviour
{
    ...
    // Update the FinishHatchingAnimation method to notify all controllers
    private void FinishHatchingAnimation(Transform insectTransform)
    {
        if (insectTransform == null)
        {
            Debug.LogError("FinishHatchingAnimation called with null insect transform");
            return;
        }
        
        Debug.Log($"Finishing hatching animation for: {insectTransform.name}");
        
        // Re-enable the AllyAI component directly
        AllyAI allyAI = insectTransform.GetComponent<AllyAI>();
        if (allyAI != null)
        {
            allyAI.enabled = true;
        ...
        // Find all egg controllers in the scene instead of using SendMessage
        AllyEggController[] allEggControllers = FindObjectsOfType<AllyEggController>();
        foreach (var controller in allEggControllers)
        {
            controller.OnCameraAnimationComplete(insectTransform.gameObject);
        }
        
        // Also keep the SendMessage as a fallback for any that might be missed
        GameObject.FindObjectOfType<AllyEggController>()?.gameObject.SendMessage(
            "OnCameraAnimationComplete", insectTransform.gameObject, SendMessageOptions.DontRequireReceiver);
    }
} 