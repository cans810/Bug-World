...
public class AttributeDisplay : MonoBehaviour
{
    ...
    public void ShowPanelFromPlayerAction(bool show)
    {
        ...
        if (attributesPanel == null)
        {
            Debug.LogError("AttributeDisplay: attributesPanel is null!");
            return;
        }
        
        Debug.Log($"AttributeDisplay panel exists: {attributesPanel != null}, currently active: {attributesPanel.activeSelf}");
        
        // Force enable the panel
        if (show)
        {
            // Direct activation without using ShowPanel 
            attributesPanel.SetActive(true);
            ...
            Debug.Log("AttributeDisplay: Panel activated directly");
        }
        else
        {
            attributesPanel.SetActive(false);
            Debug.Log("AttributeDisplay: Panel deactivated directly");
        }
    }
    ...
    // Call this in Start() and after each level up to ensure it stays hidden
    private void OnEnable()
    {
        // Always ensure panel is hidden on enable
        CancelPendingPanelDisplay();
    }
    ...
    // Add this direct method for button clicks - make it public and static for easy access
    public static void ShowAttributesPanelDirect()
    {
        Debug.Log("AttributeDisplay.ShowAttributesPanelDirect called - direct button handler");
        
        // Find the attribute display
        AttributeDisplay display = FindObjectOfType<AttributeDisplay>();
        if (display != null)
        {
            // Call the player-initiated method
            display.ShowPanelFromPlayerAction(true);
        }
        else
        {
            Debug.LogError("Failed to find AttributeDisplay component");
        }
    }
} 