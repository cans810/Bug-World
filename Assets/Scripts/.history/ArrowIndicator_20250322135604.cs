    ...
    private void UpdateArrowPosition()
    {
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            if (!isOnScreen)
            {
                ...
                Vector3 newArrowWorldPos = mainCamera.ScreenToWorldPoint(new Vector3(arrowScreenPos.x, arrowScreenPos.y, 1f));
                
                // Set target position for smoothing
                targetPosition = newArrowWorldPos;
                
                // Apply smooth position transition using SmoothDamp
            ...
            else
            {
                ...
                Vector3 newArrowWorldPos = mainCamera.ScreenToWorldPoint(new Vector3(targetScreenPos.x, targetScreenPos.y, 1f));
            ...
            // IMPROVED ROTATION AND BILLBOARDING WITH SMOOTHING
            
            // First ensure the arrow faces the camera (billboard effect)
            Quaternion lookRotation = Quaternion.LookRotation(
                arrowObject.transform.position - mainCamera.transform.position,
                mainCamera.transform.rotation * Vector3.up
            );
            
            // Smooth the rotation angle using SmoothDampAngle
            ...
            Quaternion finalRotation = lookRotation * Quaternion.Euler(0, 0, currentAngle);
            arrowObject.transform.rotation = finalRotation;
            
            // Apply pulsing effect to scale only
