...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Set up axes based on capsule orientation
        Vector3 heightAxis = Vector3.up; // Default Y axis (direction = 1)
        Vector3 radiusAxis1 = Vector3.right;
        Vector3 radiusAxis2 = Vector3.forward;
        
        if (direction == 0) // X axis
        {
            heightAxis = Vector3.right;
            radiusAxis1 = Vector3.up;
            radiusAxis2 = Vector3.forward;
        }
        else if (direction == 2) // Z axis
        {
            heightAxis = Vector3.forward;
            radiusAxis1 = Vector3.right;
            radiusAxis2 = Vector3.up;
        }
        
        // Calculate endpoints of capsule center line
        float halfHeightWithoutCaps = (height - 2 * radius) * 0.5f;
        Vector3 endpoint1 = center + heightAxis * halfHeightWithoutCaps;
        Vector3 endpoint2 = center - heightAxis * halfHeightWithoutCaps;
        
        // For the perimeter, we only need to draw the outer silhouette
        int segments = 60;
        perimeterLineRenderer.positionCount = segments;
        
        // Calculate the outer perimeter points
        for (int i = 0; i < segments; i++)
        {
            float angle = (float)i / segments * 2f * Mathf.PI;
            
            // Calculate point on circle perpendicular to height axis
            Vector3 circlePoint = radiusAxis1 * Mathf.Cos(angle) + radiusAxis2 * Mathf.Sin(angle);
            circlePoint *= radius; // Scale by radius
            
            // Determine if this point is on the "side" of the capsule or on one of the endpoints
            float dotProduct = Vector3.Dot(circlePoint.normalized, heightAxis);
            Vector3 point;
            
            if (dotProduct > 0.01f)
            {
                // Point is on the positive hemisphere
                point = endpoint1 + circlePoint;
            }
            else if (dotProduct < -0.01f)
            {
                // Point is on the negative hemisphere
                point = endpoint2 + circlePoint;
            }
            else
            {
                // Point is on the cylindrical middle section - project onto correct plane
                // Find the projection onto the plane perpendicular to height axis
                Vector3 projection = circlePoint - heightAxis * Vector3.Dot(circlePoint, heightAxis);
                
                // If the point is on the "side" (perpendicular to height axis)
                if (Mathf.Abs(dotProduct) < 0.01f)
                {
                    // This point is on the "silhouette" from above - use the correct endpoint
                    if (Vector3.Dot(circlePoint, radiusAxis1) > 0)
                        point = center + projection + heightAxis * halfHeightWithoutCaps * Mathf.Sign(circlePoint.x);
                    else
                        point = center + projection - heightAxis * halfHeightWithoutCaps * Mathf.Sign(circlePoint.x);
                }
                else
                {
                    // This is not a silhouette point - use center
                    point = center + projection;
                }
            }
            
            // Project all points to the specified height
