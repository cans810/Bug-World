...
public class HealthBarController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (lookAtTarget != null)
        {
            // Improved camera-facing logic that ensures the healthbar is always readable
            Vector3 lookDirection = transform.position - lookAtTarget.position;
            lookDirection.y = 0; // Optional: Keep health bar upright by removing y-axis rotation
            
            if (lookDirection.sqrMagnitude > 0.001f) // Check to prevent errors with zero vectors
            {
