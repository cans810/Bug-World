...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this method to UIHelper
    public void DebugShowCurrentValues()
    {
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null)
        {
            Debug.Log("=== CURRENT VALUES BEFORE SAVE ===");
            Debug.Log($"Level: {playerInventory.CurrentLevel}");
            Debug.Log($"XP: {playerInventory.TotalExperience}");
            Debug.Log($"Chitin: {playerInventory.ChitinCount}");
            Debug.Log($"Crumb: {playerInventory.CrumbCount}");
            Debug.Log($"Egg: {playerInventory.CurrentEggCount}");
            Debug.Log($"Coin: {playerInventory.CoinCount}");
            
            PlayerAttributes playerAttributes = FindObjectOfType<PlayerAttributes>();
            if (playerAttributes != null)
            {
                Debug.Log($"Strength: {playerAttributes.StrengthPoints}");
                Debug.Log($"Vitality: {playerAttributes.VitalityPoints}");
                Debug.Log($"Agility: {playerAttributes.AgilityPoints}");
                Debug.Log($"Incubation: {playerAttributes.IncubationPoints}");
                Debug.Log($"Available Points: {playerAttributes.AvailablePoints}");
            }
            Debug.Log("=================================");
        }
    }

    // Update the SaveGame method to show current values before saving
    public void SaveGame()
    {
        Debug.Log("SaveGame method called from UI");
        
        // Show current values before saving
        DebugShowCurrentValues();
        
        // Find the GameManager
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null)
        {
            // Make sure player data is up to date before saving
            PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
            if (playerInventory != null)
            {
    ...
    // Update the UpdateCoinDisplay method
    public void UpdateCoinDisplay(int coinCount)
    {
        if (coinCountText != null)
        {
            coinCountText.text = string.Format(coinFormat, coinCount);
            Debug.Log($"Updated coin display: {coinCount}");
            
            // Animate if animation is enabled
    ...
    // Add this method to UIHelper
    public void DebugLoadTest()
    {
        Debug.Log("=== DEBUG LOAD TEST ===");
        ...
        if (loadedData != null)
        {
            ...
            Debug.Log($"Level: {loadedData.currentLevel}");
            Debug.Log($"XP: {loadedData.currentXP}");
            Debug.Log($"Chitin: {loadedData.currentChitin}");
            Debug.Log($"Crumb: {loadedData.currentCrumb}");
            Debug.Log($"Egg: {loadedData.currentEgg}");
            Debug.Log($"Coin: {loadedData.currentCoin}");
            Debug.Log($"Strength: {loadedData.currentStrength}");
            Debug.Log($"Vitality: {loadedData.currentVitality}");
            Debug.Log($"Agility: {loadedData.currentAgility}");
            Debug.Log($"Incubation: {loadedData.currentIncubation}");
            Debug.Log($"Available Points: {loadedData.availableAttributePoints}");
            ...
            {
                // Set the game data
                gameManager.SetGameData(loadedData);
                
                // Apply the loaded data
                gameManager.ApplyLoadedDataToGameState();
                
                // Show current values after loading
                DebugShowCurrentValues();
            }
        }
        
        Debug.Log("=== DEBUG LOAD TEST COMPLETE ===");
    }
    ...
    public void DebugAddCoins(int amount)
    {
        ...
        if (playerInventory != null)
        {
            Debug.Log($"Before adding coins: {playerInventory.CoinCount}");
            playerInventory.AddCoins(amount);
            Debug.Log($"After adding coins: {playerInventory.CoinCount}");
            
            // Save the game
            SaveGame();
            
            // Load the game data to verify
            GameData loadedData = SaveSystem.LoadGame();
            Debug.Log($"Saved coin count: {loadedData.currentCoin}");
        }
    }

    // Add this master debug method to UIHelper
    public void MasterDebugFixEverything()
    {
        Debug.Log("======= MASTER DEBUG FIX =======");
        
        // Save current game state
        Debug.Log("1. Saving current game state...");
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null)
        {
            gameManager.SaveGame();
        ...
        Debug.Log("2. Force reloading from save file...");
        GameData loadedData = SaveSystem.LoadGame();
        
        if (loadedData != null)
        {
            Debug.Log("3. Save file loaded successfully.");
            
            // Set the data to game manager
            if (gameManager != null)
            {
                gameManager.SetGameData(loadedData);
                
                // Apply data and force UI update
                Debug.Log("4. Applying saved data to game state...");
                gameManager.ApplyLoadedDataToGameState();
                
                // Force UI update one more time
                Debug.Log("5. Final force UI update...");
                PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
                if (playerInventory != null)
                {
                    playerInventory.ForceUpdateUI();
            ...
            // Show what we've got
            Debug.Log("6. Final data check:");
            DebugShowCurrentValues();
        }
        
        Debug.Log("======= MASTER DEBUG COMPLETE =======");
        ShowInformText("Game data restored from save!");
    }

    // Add this to UIHelper
    public void NuclearReset()
    {
        Debug.Log("===== NUCLEAR RESET INITIATED =====");
        
        // Force delete any save file manually
        string path = Path.Combine(Application.persistentDataPath, "savedata.json"); // Use actual filename
        if (File.Exists(path))
        {
            File.Delete(path);
