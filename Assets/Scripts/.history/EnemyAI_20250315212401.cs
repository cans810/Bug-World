...
public class EnemyAI : MonoBehaviour
{
    ...
    // Make sure the enemy checks player state in the AttackInPlace coroutine
    private IEnumerator AttackInPlace()
    {
        while (isPlayerInHitbox && !livingEntity.IsDead)
        {
            // Get current player reference and check if it's alive
            LivingEntity playerEntity = livingEntity.GetCurrentTarget();
