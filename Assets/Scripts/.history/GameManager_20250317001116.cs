...
public class GameManager : MonoBehaviour
{
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (entitiesController != null)
        {
            ...
            Debug.Log("Loaded discovered entities data: Ant=" + gameData.hasDiscoveredAnt + 
                      ", Fly=" + gameData.hasDiscoveredFly + 
                      ", Ladybug=" + gameData.hasDiscoveredLadybug);
                      
            // Add a delay to verify entity UI creation
            StartCoroutine(VerifyEntityUICreationAfterDelay(entitiesController));
        }
        else
        {
            Debug.LogWarning("DiscoveredEntitiesController not found when loading game data!");
        }
    ...
    {
        Debug.Log($"Setting entity discovered status: {entityType} = {discovered}");
        
        switch (entityType.ToLower())
        {
            case "ant":
                gameData.hasDiscoveredAnt = discovered;
                Debug.Log($"Updated hasDiscoveredAnt to {discovered}");
                break;
            case "fly":
                gameData.hasDiscoveredFly = discovered;
                Debug.Log($"Updated hasDiscoveredFly to {discovered}");
                break;
            case "ladybug":
                gameData.hasDiscoveredLadybug = discovered;
                Debug.Log($"Updated hasDiscoveredLadybug to {discovered}");
                break;
            default:
        ...
        // Consider auto-saving when a discovery is made
        // SaveGame();
    }
    ...
    private void VerifySaveData()
    {
        ...
        Debug.Log($"Discovered Ant: {gameData.hasDiscoveredAnt}");
        Debug.Log($"Discovered Fly: {gameData.hasDiscoveredFly}");
        Debug.Log($"Discovered Ladybug: {gameData.hasDiscoveredLadybug}");
        Debug.Log("==============================");
    }

    // Add this coroutine to verify entity UI creation after a delay
    private IEnumerator VerifyEntityUICreationAfterDelay(DiscoveredEntitiesController controller)
    {
        // Wait for a short time to allow any initialization to complete
        yield return new WaitForSeconds(0.5f);
        
        // Force a refresh of the entity UI
        controller.ForceRefreshEntityUI();
        
        Debug.Log("Verified entity UI creation after loading");
    }
}
