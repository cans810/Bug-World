...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Check if we have a canvas safe area
        if (canvasSafeArea == null)
        {
            Debug.LogError("Canvas Safe Area reference is missing. UI animations will not work correctly.");
            return;
        }

        // Create UI chitin pool
        if (uiChitinIconPrefab != null)
        {
            // Create the pool container as a child of canvasSafeArea
            GameObject uiChitinPool = new GameObject("UIChitinPool");
            uiChitinPool.transform.SetParent(canvasSafeArea.transform, false);
            
            uiElementPools["uiChitin"] = new Queue<GameObject>();
            
            for (int i = 0; i < 10; i++) // Pool size of 10
            {
                GameObject icon = Instantiate(uiChitinIconPrefab, uiChitinPool.transform);
                RectTransform iconRect = icon.GetComponent<RectTransform>();
                if (iconRect != null)
                {
                    // Make sure the icon has the correct anchoring
                    iconRect.anchorMin = new Vector2(0.5f, 0.5f);
                    iconRect.anchorMax = new Vector2(0.5f, 0.5f);
                    iconRect.pivot = new Vector2(0.5f, 0.5f);
                }
                icon.SetActive(false);
                uiElementPools["uiChitin"].Enqueue(icon);
            }
        }
        ...
        {
            // Create the pool container as a child of canvasSafeArea
            GameObject uiXPPool = new GameObject("UIXPPool");
            uiXPPool.transform.SetParent(canvasSafeArea.transform, false);
            
            uiElementPools["uiXP"] = new Queue<GameObject>();
            
            for (int i = 0; i < 10; i++) // Pool size of 10
            {
                GameObject icon = Instantiate(uiXPIconPrefab, uiXPPool.transform);
                RectTransform iconRect = icon.GetComponent<RectTransform>();
                if (iconRect != null)
                {
                    // Make sure the icon has the correct anchoring
