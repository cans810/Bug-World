...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    [Header("Area Unlocking")]
    [SerializeField] private GameObject[] unlockedAreaPrefabs; // Prefabs for each unlockable area
    [SerializeField] private int[] areaUnlockLevels = { 5, 10, 15, 20, 25 }; // Levels at which areas unlock
    ...
    // Add this method to the class to check if revival is in progress
    ...
    // Modify InstantiateRewards to check revival state 
    ...
    private void RemoveBorderForArea(GameObject areaTarget, int level)
    {
        ...
        foreach (BorderVisualizer border in allBorders)
        {
            ...
            if (isMatch)
            {
                ...
                // Call the DisableBorderVisualization method directly
                border.DisableBorderVisualization();
                
                // Enable spawning for this area
                border.SetSpawningEnabled(true);
                
                // Explicitly deactivate entity icons (for redundancy)
                border.DeactivateEntityIcons();
                
                // Add the border name to the unlocked borders list in GameData
                GameManager gameManager = FindObjectOfType<GameManager>();
                if (gameManager != null && gameManager.gameData != null)
                {
                    // Ensure the list exists
                    if (gameManager.gameData.unlockedBorderAreas == null)
                    {
                        gameManager.gameData.unlockedBorderAreas = new List<string>();
                    }
                    
                    // Add the border to the unlocked list if not already there
                    if (!gameManager.gameData.unlockedBorderAreas.Contains(borderName))
                    {
                        gameManager.gameData.unlockedBorderAreas.Add(borderName);
                        Debug.Log($"<color=green>Added {borderName} to unlocked borders list</color>");
                    }
                    
                    // Save the game to persist this change
                    gameManager.SaveGame();
                }
                
                Debug.Log($"<color=green>Border {borderName} disabled and entity spawning enabled!</color>");
            }
        }
    }
    ...
    private void HandleLevelUp(int newLevel)
    {
        ...
        // Check if an area should be unlocked at this level
        bool unlockArea = ShouldUnlockAreaAtLevel(newLevel);
        
        // Show the level up panel with small delay to let other level-up effects finish
        StartCoroutine(ShowLevelUpPanelWithUnlock(0.5f, newLevel, unlockArea));
    }

    // Modify to include area unlocking
    private IEnumerator ShowLevelUpPanelWithUnlock(float delay, int newLevel, bool unlockArea)
    {
        yield return new WaitForSeconds(delay);
        ...
        // Handle area unlocking if needed
        if (unlockArea)
        {
            int areaIndex = GetAreaIndexForLevel(newLevel);
            if (areaIndex >= 0 && areaIndex < unlockedAreaPrefabs.Length)
            {
                // Create the area if it doesn't exist yet
                InstantiateNewArea(areaIndex);
                
                // Show the new area unlocked reward
                ShowNewAreaUnlockedReward($"Area {areaIndex + 1} Unlocked!");
            }
        }
        ...
        Debug.Log($"Level up panel shown for level {newLevel} with all rewards, area unlock: {unlockArea}");
    }

    // Method to instantiate a new area
    private void InstantiateNewArea(int areaIndex)
    {
        if (areaIndex < 0 || areaIndex >= unlockedAreaPrefabs.Length)
        {
        ...
        // Check if the area prefab exists
        GameObject areaPrefab = unlockedAreaPrefabs[areaIndex];
        if (areaPrefab == null)
        {
            Debug.LogError($"No area prefab defined for index {areaIndex}");
            return;
        }
        
        // Check if the area already exists (by name or tag)
        string areaName = $"Area{areaIndex + 1}";
        GameObject existingArea = GameObject.FindGameObjectWithTag(areaName);
        
        if (existingArea != null)
        {
            Debug.Log($"Area {areaName} already exists, not creating duplicate");
            return;
        }
        ...
        Debug.Log($"Created new area: {areaName}");
        
        // Optionally focus the camera on the new area
        // StartCoroutine(FocusCameraOnNewArea(newArea.transform));
    }

    // Method to show the new area unlocked reward in the level up panel
    private void ShowNewAreaUnlockedReward(string areaName)
    {
        if (newAreaUnlockedReward != null)
        {
            ...
            if (rewardText != null)
            {
                rewardText.text = areaName;
            }
            
            // Make sure the reward is visible
            newAreaUnlockedReward.SetActive(true);
        }
    }

    // Add a method to check if an area should be unlocked at the current level
    private bool ShouldUnlockAreaAtLevel(int level)
    {
        foreach (int unlockLevel in areaUnlockLevels)
        {
    ...
    // Add a method to get the index of the area to unlock at the given level
    private int GetAreaIndexForLevel(int level)
    {
        for (int i = 0; i < areaUnlockLevels.Length; i++)
        {
            if (level == areaUnlockLevels[i])
                return i;
        }
        return -1; // No area to unlock at this level
    }
}
