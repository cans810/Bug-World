...
public class ChitinDropper : MonoBehaviour
{
    ...
    private void SpawnChitinWithPhysics()
    {
        ...
        Vector3 horizontalForce = new Vector3(
            ...
            0,
            randomDirection.y
        ) * Random.Range(0.05f, 0.2f); // Drastically reduced horizontal force
        ...
        if (collider != null)
        {
            // Set the collider to be a trigger initially
        ...
        // Start coroutine to monitor position and disable trigger when landing
        StartCoroutine(MonitorChitinFall(chitin));
    }
    ...
    private IEnumerator MonitorChitinFall(GameObject chitin)
    {
        if (chitin == null) yield break;
        
        Collider collider = chitin.GetComponent<Collider>();
        if (collider == null) yield break;
        
        // Wait until the chitin falls below y = 0.3
        while (chitin != null && chitin.transform.position.y > 0.3f)
        {
            yield return null;
        }
        
        // If the chitin still exists, disable the trigger
        if (chitin != null && collider != null)
        {
            collider.isTrigger = false;
            Debug.Log($"Chitin reached landing height, disabled trigger on {chitin.name}");
        }
    }
} 