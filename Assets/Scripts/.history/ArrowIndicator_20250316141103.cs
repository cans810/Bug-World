    ...
    private void UpdateArrowPosition()
    {
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            if (!isOnScreen)
            {
                ...
                // Make sure arrow faces the camera (but preserve its shape)
                arrowObject.transform.rotation = Quaternion.LookRotation(
                    mainCamera.transform.forward,  // Forward direction (facing camera)
                    Vector3.up                     // Up direction (keeps arrow upright)
                );
                
                // Then apply our z-rotation separately
