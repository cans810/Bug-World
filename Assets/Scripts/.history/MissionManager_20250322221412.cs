...
public class MissionManager : MonoBehaviour
{
    ...
    private IEnumerator PlayCoinAnimationThenHide(GameObject panel, int coinAmount)
    {
        ...
        // Get the panel's world position for spawning coins
        ...
        if (rectTransform != null)
        {
            ...
            Vector3 screenPos = RectTransformUtility.WorldToScreenPoint(Camera.main, panel.transform.position);
            ...
            {
                // Randomly position coins around the panel center
                float offsetX = Random.Range(-50f, 50f);
                float offsetY = Random.Range(-25f, 25f);
                
                // Call the visual effect manager to spawn a coin at this position
                // Use the existing "SpawnCoinSymbol" method or similar
                visualEffectManager.SpawnCoinSymbolFromUI(
                    screenPos + new Vector3(offsetX, offsetY, 0),
                    new Vector2(Screen.width - 150f, Screen.height - 75f), // Target position (top right)
                    i * 0.1f // Delay each coin slightly
                );
        ...
        // Wait for coins to travel to the top-right
