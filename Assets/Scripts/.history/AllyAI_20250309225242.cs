...
public class AllyAI : MonoBehaviour
{
    ...
    private void PickUpLoot(Transform loot)
    {
        ...
        // Position the loot properly on the ally
        // Check if it's a chitin or crumb and position accordingly
        if (loot.GetComponent<ChitinCollectible>() != null)
        {
            // For chitin, position it directly above the ally
            loot.localPosition = new Vector3(0f, 1f, 0f);
        }
        else
        {
            // For crumbs, use the existing positioning logic
            // (assuming this is what's already working for crumbs)
            loot.localPosition = new Vector3(0f, 0.5f, 0f);
        }
