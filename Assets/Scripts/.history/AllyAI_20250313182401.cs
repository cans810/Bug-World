...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float movementSmoothTime = 0.1f; // Longer smooth time for movement
    ...
    private void FollowPlayerBehavior()
    {
        ...
        else
        {
            ...
            // Smoothly face and move towards player
            FaceTarget(playerTransform.position);
            MoveWithBoundaryCheck(transform.forward);
        }
    }
    ...
    private void WanderBehavior()
    {
        if (wanderingBehavior != null)
        {
            ...
            // Add smoothing to wandering movement
            if (wanderingBehavior.IsCurrentlyMoving())
            {
                Vector3 targetPosition = transform.position + transform.forward * livingEntity.moveSpeed * Time.deltaTime;
    ...
    public void SetMode(AIMode newMode)
    {
        ...
        {
            // Enable AIWandering and let it take complete control
            if (wanderingBehavior != null)
            {
                ...
                // Enable the AIWandering with a clean state
                wanderingBehavior.enabled = true;
                wanderingBehavior.SetWanderingEnabled(true);
        ...
        {
            // For all other modes, disable AIWandering completely
            if (wanderingBehavior != null)
            {
                wanderingBehavior.SetWanderingEnabled(false);
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        // Calculate the next position with smoothing
        Vector3 targetPosition = transform.position + direction * livingEntity.moveSpeed * Time.deltaTime;
        ...
        // Boundary check
        if (mapBoundary != null && !mapBoundary.IsWithinBounds(targetPosition))
        {
            Vector3 safePosition = mapBoundary.GetNearestPointInBounds(targetPosition);
            Vector3 safeDirection = (safePosition - transform.position).normalized;
            
            // Smoothly rotate towards the safe direction
            FaceTarget(transform.position + safeDirection);
            
            // Move towards the safe position
            transform.position = Vector3.SmoothDamp(
                transform.position,
                safePosition,
                ref movementVelocity,
    ...
    {
        if (baseTransform == null || carriedLoot == null) return;
        ...
        // Smoothly face and move towards base
