...
public class InsectIncubator : MonoBehaviour
{
    ...
    public GameObject CreateInsectEgg(string entityType)
    {
        ...
        {
            entityType = "ant"; // Default to ant if not specified
        }

        // Store the entity type for this egg
        currentEntityTypeToHatch = entityType;
        
        // Check if we have the egg prefab
        if (insectEggPrefab == null)
        {
            Debug.LogError("No egg prefab assigned to InsectIncubator!");
            return null;
        }
        
        // Get egg spawn position
        Vector3 spawnPos = eggPos.position;
        
        // Play spawn effect if available
        if (eggSpawnEffect != null)
        {
            eggSpawnEffect.transform.position = spawnPos;
            eggSpawnEffect.Play();
        }
        ...
        {
            AudioSource.PlayClipAtPoint(eggCreationSound, spawnPos);
        }
        
        // Instantiate the egg prefab and set its entity type
        GameObject eggObject = Instantiate(insectEggPrefab, spawnPos, Quaternion.identity);
        eggObject.transform.SetParent(transform); // Parent it to the incubator
        
        // Update the egg controller to use the specified entity type
        AllyEggController eggController = eggObject.GetComponent<AllyEggController>();
        if (eggController != null)
        {
            eggController.SetEntityType(entityType);
            
            // Set incubation time (use default time)
            float incubationTime = 300f; // 5 minutes in seconds
            eggController.SetIncubationTime(incubationTime);
            
            Debug.Log($"Created egg of type {entityType} with incubation time {incubationTime}s");
        }
        else
        {
            Debug.LogError("Created egg object has no AllyEggController component!");
        }
        
        // Mark that an egg exists
        EggExists = true;
        
        // Set globally that egg exists so we can reference this from other scripts
        CustomEvents.EggCreated();
        
        return eggObject; // Return the created egg
    }
    ...
    // Update this method to return the created egg for camera focus
    public GameObject CreateMultipleEggs(string entityType, int count)
    {
        ...
        // Reference to last created egg to return
        GameObject lastCreatedEgg = null;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create egg
            GameObject egg = CreateInsectEgg(entityType);
            ...
            if (egg == null)
            {
                ...
                return lastCreatedEgg; // Return the last successfully created egg, if any
            }
            
            // Store reference to this egg
            lastCreatedEgg = egg;
        }
        ...
        Debug.Log($"Successfully created {count} {entityType} eggs");
        return lastCreatedEgg; // Return the last created egg for camera focus
    }
}
