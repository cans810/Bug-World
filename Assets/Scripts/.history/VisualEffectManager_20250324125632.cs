...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public void PlayLevelUpEffect(Vector3 playerPosition, System.Action onCompleteCallback = null)
    {
        ...
        for (int i = 0; i < symbolCount; i++)
        {
            ...
            // Start a specialized animation that rises vertically - using 0.05f instead of 0.1f for faster spawn
            StartCoroutine(AnimateLevelUpSymbolRisingUpward(symbol, playerObj.transform, randomOffset, i * 0.05f, () => {
                completedAnimations++;
    ...
    // Modify the animation coroutine to make it faster with growing effect
    ...
    {
        // Reduced initial delay for faster start
        ...
        // Set initial scale to be much smaller (start tiny)
        symbol.transform.localScale = startScale * 0.2f;
        
        // Store starting position
        Vector3 startPosition = symbol.transform.position;
        
        // Animation parameters
        float animDuration = 1.5f;  // Shorter duration for faster effect (was 2.5f)
        float maxHeight = 7.0f;     // Higher maximum height for more dramatic rise
        float elapsed = 0f;
        ...
        while (elapsed < animDuration && playerTransform != null)
        {
            ...
            // Calculate vertical rise with easing (faster initial movement)
            ...
            float horizontalOffset = Mathf.Sin(normalizedTime * 4f) * 0.4f * (1f - normalizedTime);
            
            // Calculate new position - start moving immediately
            ...
            // Gradual scale growth throughout most of the animation
            float scaleMultiplier;
            if (normalizedTime < 0.7f) {
                // Grow from 0.2 to 1.4 during the first 70% of animation
                scaleMultiplier = Mathf.Lerp(0.2f, 1.4f, normalizedTime / 0.7f);
            }
            else {
                // Shrink slightly at the end from 1.4 to 1.0
                scaleMultiplier = Mathf.Lerp(1.4f, 1.0f, (normalizedTime - 0.7f) / 0.3f);
            }
            
            symbol.transform.localScale = startScale * scaleMultiplier;
            
            // Fade in quickly, stay visible, then fade out at the very end
            float alpha = normalizedTime < 0.1f ? 
                Mathf.Lerp(0f, 1f, normalizedTime / 0.1f) : 
                Mathf.Lerp(1f, 0f, (normalizedTime - 0.8f) / 0.2f);
                
            // Ensure alpha stays at 1 during the middle part of the animation
            if (normalizedTime >= 0.1f && normalizedTime <= 0.8f) alpha = 1f;
