...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Make sure the OnLevelUp event is properly giving attribute points
    // Find the code that handles leveling up and ensure it's adding points to PlayerAttributes

    // This might be in HandleLevelUp or a similar method
    private void HandleLevelUp(int newLevel)
    {
        Debug.Log($"PlayerInventory: Leveled up to {newLevel}!");
        
        // Find PlayerAttributes and add points
        PlayerAttributes playerAttributes = FindObjectOfType<PlayerAttributes>();
        if (playerAttributes != null)
        {
            playerAttributes.AddAttributePoints(1); // Make sure points are being added
            Debug.Log($"PlayerInventory: Added attribute point, now has {playerAttributes.AvailablePoints}");
        }
        else
        {
            Debug.LogError("PlayerInventory: Could not find PlayerAttributes when leveling up!");
        }
        
        // Notify listeners about level up
        if (OnLevelUp != null)
        {
            OnLevelUp(newLevel);
        }
