...
public class PlayerInventory : MonoBehaviour
{
    ...
    private IEnumerator PlayBarrierRemovalAnimation(BarrierRocksController barrier)
    {
        ...
        // Modified: Calculate a more top-down, wider angle viewing position
        Vector3 directionToBarrier = (barrierPosition - playerTransform.position).normalized;
        
        // Increase height (more top-down) and distance (wider view)
        Vector3 viewPosition = barrierPosition - directionToBarrier * 16f + Vector3.up * 15f;
        
        // Adjust rotation for a more top-down view
        Quaternion viewRotation = Quaternion.LookRotation((barrierPosition - viewPosition).normalized);
        
        // Optional: Also adjust the field of view for a wider perspective
        float originalFOV = mainCamera.fieldOfView;
        float cinematicFOV = 65f; // Wider FOV for better view (default is usually 60)
