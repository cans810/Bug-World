...
public class UIHelper : MonoBehaviour
{
    ...
    private GameObject cachedNestObject;
    ...
    private GameObject GetNestObject()
    {
        ...
        {
            cachedNestObject = GameObject.Find("Player Nest 1");
        }
    ...
    private IEnumerator AnimateIconSafe(GameObject icon, Vector3 startPosition, Vector3 targetPos, Vector3 originalScale, float duration, int iconIndex)
    {
        ...
        // Get the nest reference once
        GameObject nestObject = GetNestObject();
        ...
        while (!animationComplete && icon != null)
        {
            try
            {
                ...
                // Update target position every 5 frames for performance
                if (frameCount % 5 == 0 && nestObject != null)
                {
                    targetPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                    targetPos.z = 0;
                }
