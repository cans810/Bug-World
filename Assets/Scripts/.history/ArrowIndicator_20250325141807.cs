...
public class ArrowIndicator : MonoBehaviour
{
    ...
    [SerializeField] private float arrowSize = 0.075f;
    ...
    private void UpdateArrowPosition()
    {
        ...
        if (distance > reachDistance)
        {
            ...
            if (isOnScreen)
            {
                // If target is on screen, position arrow above target
                // Add a small offset in Y to position above the target
                Vector3 offsetTargetScreenPos = new Vector3(
                    targetScreenPos.x, 
                    targetScreenPos.y + 60f,
                    targetScreenPos.z);
                    
                arrowWorldPos = mainCamera.ScreenToWorldPoint(
                    new Vector3(offsetTargetScreenPos.x, offsetTargetScreenPos.y, 1f));
                    
                // Point straight down when above target
                angle = 0;
            }
            else
            {
                // If target is off screen, use fixed position
    ...
    // Helper method to check if a point is on screen
