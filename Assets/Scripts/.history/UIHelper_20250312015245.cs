...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            ...
            // Set the layer name instead of naming the object
            icon.layer = LayerMask.NameToLayer("ChitinIcon");
            
            // Make sure it's active and enabled
            icon.SetActive(true);
            
            allIcons.Add(icon);
            Debug.Log($"[CRITICAL] Created icon {i+1} of {chitinCount} at position {icon.transform.position} on layer 'ChitinIcon'");
        }
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            Debug.Log($"[CRITICAL] Starting animation for icon {i+1} on layer 'ChitinIcon'");
