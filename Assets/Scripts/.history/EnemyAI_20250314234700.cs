...
public class EnemyAI : MonoBehaviour
{
    ...
    // Enhance the border detection system to match the player's but with bouncing behavior
    private void OnTriggerExit(Collider other)
    {
        // Check if the collider is in the MapBorder layer
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Calculate the direction pointing back IN toward the playable area
            ...
            // Get current forward direction before reflection
            ...
            // Add a bit of randomness to prevent predictable patterns and avoid getting stuck
            reflectionDirection = Quaternion.Euler(0, Random.Range(-20f, 20f), 0) * reflectionDirection;
            
            // Set new rotation immediately for responsive bouncing
            transform.rotation = Quaternion.LookRotation(reflectionDirection);
            
            // Set state for handling in Update method
            isHittingBoundary = true;
            boundaryRedirectionTime = Time.time + BOUNDARY_REDIRECT_DURATION;
            
            // If we have a wandering behavior, tell it to continue in this new direction
            if (wanderingBehavior != null)
            {
            ...
            // Temporarily stop any chase behavior if we were pursuing a target
