...
public class PlayerController : MonoBehaviour
{
    ...
    private void CreateCircularArrows(GameObject parent)
    {
        ...
        for (int i = 0; i < arrowCount; i++)
        {
            ...
            // Position arrows much lower than before, scaling with player size
            // Reduce Y position significantly
            Vector3 position = transform.position + new Vector3(
                Mathf.Cos(angle) * radius,
                -transform.localScale.y * 0.2f, // Lowered position - now below the player
            ...
            var heightField = typeof(PlayerFollowingArrowIndicator).GetField("floatHeight", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (heightField != null)
                // Make float height negative to position arrows below player
                heightField.SetValue(arrow, -transform.localScale.y * 0.1f); // Negative height to position below
            
            // Make sure arrows are visible even when below the player
            var bobField = typeof(PlayerFollowingArrowIndicator).GetField("bobAmount", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (bobField != null)
                bobField.SetValue(arrow, transform.localScale.y * 0.05f); // Increased bobbing for visibility
            
            // Make arrows point upward by adjusting rotation
            arrow.transform.Rotate(180, 0, 0); // Flip arrow to point upward
            
            // Make sure arrows are properly visible from the camera
            // Calculate position to ensure visibility
            if (!IsPointVisibleFromCamera(mainCamera, position))
            {
                // If not visible, move toward camera view
                Vector3 screenCenter = new Vector3(Screen.width/2, Screen.height/2, 0);
                Vector3 viewportPoint = mainCamera.WorldToScreenPoint(position);
                Vector3 dirToCenter = (screenCenter - viewportPoint).normalized;
                
                // Adjust position to be more visible
                Vector3 screenPos = mainCamera.WorldToScreenPoint(position);
                // Move 20% toward center of screen
                screenPos = Vector3.Lerp(screenPos, screenCenter, 0.2f);
                // Convert back to world space at same distance
                position = mainCamera.ScreenToWorldPoint(new Vector3(
    ...
    // Helper method to check if a point is visible from the camera
    private bool IsPointVisibleFromCamera(Camera camera, Vector3 worldPoint)
    {
    ...
    private void EnsureCorrectSizeAndCamera()
    {
        ...
        // Apply the correct scale based on the player's current level
        if (enableMetamorphosis)
        {
