...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPCounter(UIHelper uiHelper, int startXP, int targetXP, float duration)
    {
        ...
        {
            // Get the current level before potential level up
            int currentLevel = playerInventory.CurrentLevel;
            
            // Check if the current XP is enough for the next level
            int newLevel = playerInventory.GetLevelFromExperience(playerInventory._experience);
            
            // If the new level is higher, trigger the level up
            if (newLevel > currentLevel)
            {
                Debug.Log($"Level up detected after XP animation: {currentLevel} → {newLevel}");
                
                // Call the level up method or invoke the event directly
