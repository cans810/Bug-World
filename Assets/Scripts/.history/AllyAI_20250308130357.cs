...
public class AllyAI : MonoBehaviour
{
    ...
    private void CarryLootToBase()
    {
        ...
        // Calculate target position (only use base X and Z, keep current Y)
        ...
        // Calculate exact direction vector to the base (this will be constant throughout movement)
        Vector3 directionToBase = (targetPosition - transform.position).normalized;
        
        // Instantly face the direction (no gradual rotation)
        transform.forward = directionToBase;
        
        // Set animation to walking
        isMoving = true;
        UpdateAnimation(true);
        
        // Use consistent movement method
        // This avoids position jumps and ensures consistent speed
        Vector3 newPosition = transform.position + (directionToBase * livingEntity.moveSpeed * Time.deltaTime);
