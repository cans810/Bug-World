...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            // Enforce boundary even during waiting
            EnforceBoundaryLimits();
            ...
            // Use a more conservative radius for waypoint selection (80% of max)
            float effectiveRadius = maxWanderRadius * 0.8f;
            
            Vector3 targetWaypoint = GetSafeWaypoint(effectiveRadius);
            ...
            while (moveTimer < moveDuration && isWandering)
            {
                ...
                // Use a stricter threshold (90% of max radius)
                if (restrictToSpawnArea && distanceFromSpawn > maxWanderRadius * 0.9f)
                {
                    Debug.LogWarning($"{gameObject.name}: Approaching wander radius border - turning back");
    ...
    private IEnumerator TurnAndMoveInward()
    {
        ...
        // Pick a random angle variation (smaller range for more direct movement inward)
        float randomAngle = Random.Range(-40f, 40f);
        Vector3 finalDirection = Quaternion.Euler(0, randomAngle, 0) * directionToSpawn;
        
        // Get a new distance (between 20-50% of max radius - shorter to ensure we move inward)
        float turnBackDistance = maxWanderRadius * Random.Range(0.2f, 0.5f);
        Vector3 newWaypoint = transform.position + finalDirection * turnBackDistance;
        
        // Make sure the new waypoint is well within our wander radius
        float distanceFromSpawn = Vector3.Distance(newWaypoint, spawnPosition);
        if (distanceFromSpawn > maxWanderRadius * 0.7f)
        {
            // Adjust to ensure we stay well within radius
            newWaypoint = spawnPosition + (newWaypoint - spawnPosition).normalized * (maxWanderRadius * 0.6f);
        }
