...
public class PlayerController : MonoBehaviour
{
    ...
    // In PlayerController.cs - find the method that handles metamorphosis
    private void CheckForMetamorphosis(int newLevel)
    {
        // Return early if metamorphosis is disabled or already at max stage
        if (!enableMetamorphosis || currentMetamorphosisStage >= metamorphosisLevels.Length)
            return;
        
        // Check if the new level matches a metamorphosis level
        if (currentMetamorphosisStage < metamorphosisLevels.Length && 
            newLevel >= metamorphosisLevels[currentMetamorphosisStage])
        {
            // Trigger the metamorphosis
            StartCoroutine(PerformMetamorphosis());
        }
    }

    // Modify the metamorphosis process method to show the UI notification
    private IEnumerator PerformMetamorphosis()
    {
        // Show Size Up text through UIHelper
        if (uiHelper != null)
        {
        ...
        // Existing metamorphosis code...
        // Calculate the new scale
        Vector3 newScale = baseScale * Mathf.Pow(scaleIncreaseMultiplier, currentMetamorphosisStage + 1);
        ...
        while (elapsed < duration)
        {
            ...
            transform.localScale = Vector3.Lerp(startScale, newScale, smoothProgress);
        ...
        // Increment the metamorphosis stage
        currentMetamorphosisStage++;
        
        Debug.Log($"Metamorphosis complete! New stage: {currentMetamorphosisStage}, Scale: {newScale}");
    }
}