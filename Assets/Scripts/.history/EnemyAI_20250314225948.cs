...
public class EnemyAI : MonoBehaviour
{
    ...
    {
        // Define a rotation speed for smooth turning
        float combatRotationSpeed = 5.0f;
        ...
        while (isPlayerInHitbox && !livingEntity.IsDead && playerEntity != null && !playerEntity.IsDead)
        {
            ...
            if (directionToPlayer.magnitude > 0.1f)
            {
                ...
                // Calculate the shortest rotation path (handles the 0/360 degree wrap-around)
                float angleDifference = Mathf.DeltaAngle(currentEuler.y, targetEuler.y);
                
                // Apply rotation with consistent speed regardless of angle difference
                float rotationStep = combatRotationSpeed * Time.deltaTime * Mathf.Sign(angleDifference);
                
                // Limit rotation step to not exceed the remaining angle difference
                if (Mathf.Abs(rotationStep) > Mathf.Abs(angleDifference))
                    rotationStep = angleDifference;
                
                // Apply the rotation only on Y axis
                transform.rotation = Quaternion.Euler(
                    currentEuler.x,
                    currentEuler.y + rotationStep,
                    currentEuler.z
                );
                
                // Check if we're close enough to snap
                float newAngleDifference = Mathf.DeltaAngle(transform.rotation.eulerAngles.y, targetEuler.y);
                if (Mathf.Abs(newAngleDifference) < 0.5f)
                {
                    transform.rotation = Quaternion.Euler(
                        currentEuler.x,
                        targetEuler.y,
