...
public class PlayerController : MonoBehaviour
{
    ...
    // Strengthen the border collision detection and handling
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Calculate the normal direction pointing away from the border
            borderNormal = (transform.position - other.ClosestPoint(transform.position)).normalized;
            
            // Ensure the normal has a non-zero magnitude
            if (borderNormal.magnitude < 0.1f)
            {
                // Fallback if closest point calculation fails
                borderNormal = transform.position - other.transform.position;
                borderNormal.y = 0; // Keep it horizontal
                borderNormal = borderNormal.normalized;
            }
            
            // Log detailed info for debugging
            Debug.LogError($"Player hit map border: {other.gameObject.name}, Normal: {borderNormal}");
            ...
            // Immediately stop velocity in the border direction
            StopMovementTowardsBorder();
        }
    }

    // Add this helper method to forcefully stop movement toward the border
    private void StopMovementTowardsBorder()
    {
        Rigidbody rb = GetComponent<Rigidbody>();
        if (rb != null && !rb.isKinematic)
        {
            // Calculate component of velocity going into the border
            float movementThroughBorder = Vector3.Dot(rb.velocity, -borderNormal);
            
            // If moving toward border, cancel that component of velocity
            if (movementThroughBorder > 0)
            {
                Vector3 cancelVelocity = -borderNormal * movementThroughBorder;
                rb.velocity += cancelVelocity;
                
                // Add slight push away from border for more reliable collision avoidance
                rb.velocity += borderNormal * 0.2f;
                
                Debug.Log($"Stopping movement toward border. Before: {rb.velocity - cancelVelocity}, After: {rb.velocity}");
            }
        }
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            Vector3 newBorderNormal = (transform.position - other.ClosestPoint(transform.position)).normalized;
            
            // Only update if we got a valid normal
            if (newBorderNormal.magnitude > 0.1f)
            {
                borderNormal = newBorderNormal;
            }
            
            // Continue to restrict movement toward the border
            StopMovementTowardsBorder();
        }
    }
