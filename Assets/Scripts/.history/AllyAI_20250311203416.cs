...
public class AllyAI : MonoBehaviour
{
    ...
    public void SetMode(AIMode newMode)
    {
        ...
        // Handle wandering behavior based on the new mode
        if (wanderingBehavior != null)
        {
            ...
            {
                // Only disable wandering behavior for non-wander modes
                wanderingBehavior.SetWanderingEnabled(false);
                
                // Don't disable the component itself, just the wandering behavior
                // This ensures the component stays active and can be re-enabled later
            }
        }
        
        // Debug log the mode change
    ...
    {
        // If we lost the loot somehow, reset state
        if (carriedLoot == null)
        {
        ...
        // If we're close enough to the base
        if (distanceToBase <= lootDropDistance)
        {
            ...
            // Ensure wandering behavior is properly enabled if we're returning to wander mode
    ...
    private void DropLootAtBase()
    {
        ...
        // Set position at the base with the exact Y coordinate
        ...
        Debug.Log($"{gameObject.name} dropped loot at base");
    }
