...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask obstacleLayerMask; // Set this to include Ally layer and any other obstacles
    [SerializeField] private float pathDeviationAmount = 2.0f;
    ...
    private void CarryLootToBase()
    {
        ...
        // Calculate path to base (direct or with detour)
        Vector3 targetPosition = CalculatePathToBase();
        
        // Face and move towards the target
    ...
    // Add this method to calculate a path to the base
    private Vector3 CalculatePathToBase()
    {
        // If pathfinding is disabled or we're very close to the base, go direct
        float distanceToBase = Vector3.Distance(transform.position, baseTransform.position);
        if (!usePathfinding || distanceToBase < lootDropDistance * 1.5f)
        {
            isPathBlocked = false;
            return baseTransform.position;
        }
        
        // Only recalculate path periodically to avoid jitter
        if (Time.time < lastPathUpdateTime + pathUpdateInterval && currentPathTarget != Vector3.zero)
        {
        ...
        directionToBase.y = 0; // Keep on same Y level
        
        // Check if path is blocked
        bool wasBlocked = isPathBlocked;
        isPathBlocked = IsPathBlocked(directionToBase);
        
        // Debug visualization
        if (Debug.isDebugBuild)
        {
            Debug.DrawRay(transform.position, directionToBase * obstacleDetectionDistance, 
                isPathBlocked ? Color.red : Color.green, pathUpdateInterval);
        }
        
        // If path is clear, go direct to base
        if (!isPathBlocked)
        {
            currentPathTarget = baseTransform.position;
            return currentPathTarget;
        }
        
        // Path is blocked, calculate a detour
        Vector3 detourDirection;
        
        // If we just detected a blockage, choose a new random detour direction
        if (!wasBlocked || currentPathTarget == Vector3.zero)
        {
            // Try to find a clear path by checking multiple angles
            detourDirection = FindClearPath(directionToBase);
        }
        else
        {
            // Continue using the existing detour direction
            detourDirection = (currentPathTarget - transform.position).normalized;
            
            // If the current detour is also blocked, find a new one
            if (IsPathBlocked(detourDirection))
            {
                detourDirection = FindClearPath(directionToBase);
            }
        }
        
        // Calculate the detour target position
        Vector3 detourTarget = transform.position + detourDirection * pathDeviationAmount;
        
        // Ensure the detour doesn't take us too far from the base
        Vector3 baseToDetour = detourTarget - baseTransform.position;
        if (baseToDetour.magnitude > distanceToBase * 1.5f)
        {
            // Scale back the detour to not go too far away
            detourTarget = baseTransform.position + baseToDetour.normalized * distanceToBase;
        }
        
        // Make sure the detour point is within map boundaries
        if (mapBoundary != null && !mapBoundary.IsWithinBounds(detourTarget))
        {
            detourTarget = mapBoundary.GetNearestPointInBounds(detourTarget);
        }
        
        // Debug visualization of the detour
        if (Debug.isDebugBuild)
        {
            Debug.DrawLine(transform.position, detourTarget, Color.yellow, pathUpdateInterval);
        }
        
        currentPathTarget = detourTarget;
        return currentPathTarget;
    }

    // Check if the path in a given direction is blocked
    private bool IsPathBlocked(Vector3 direction)
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, obstacleDetectionDistance, obstacleLayerMask))
        {
            // Ignore self
            if (hit.collider.gameObject == gameObject)
                return false;
            
            // Check if it's another ally
            if (hit.collider.CompareTag("Ally"))
            {
                // If the ally is also carrying loot, consider the path blocked
                AllyAI otherAlly = hit.collider.GetComponent<AllyAI>();
                if (otherAlly != null && otherAlly.IsCarryingLoot())
                {
                    return true;
                }
                
                // If the ally is stationary, consider the path blocked
                if (otherAlly != null && !otherAlly.IsMoving())
                {
                    return true;
                }
            }
            
            // For other obstacles, always consider the path blocked
            return true;
        }
    ...
    // Find a clear path by checking multiple angles
    private Vector3 FindClearPath(Vector3 originalDirection)
    {
        ...
        {
            // Rotate the original direction by the angle
            ...
            if (!IsPathBlocked(rotatedDirection))
            {
                ...
                if (Debug.isDebugBuild)
                {
                    Debug.DrawRay(transform.position, rotatedDirection * obstacleDetectionDistance, Color.blue, pathUpdateInterval);
                }
        ...
        // If all directions are blocked, return a random direction as a last resort
        Vector3 randomDirection = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
        
        // Debug visualization
        if (Debug.isDebugBuild)
        {
            Debug.DrawRay(transform.position, randomDirection * obstacleDetectionDistance, Color.magenta, pathUpdateInterval);
        }
