...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator TurnAndMoveInward()
    {
        ...
        // Add a small threshold to prevent zero vectors
        if (directionToWaypoint.magnitude < 0.1f)
        {
            directionToWaypoint = transform.forward;
        }
        ...
        while (turnTimer < turnDuration)
        {
            ...
            // Use smoother step function for rotation
            t = t * t * (3f - 2f * t);
        ...
        // Remove the forced final rotation to prevent snapping
        // transform.rotation = targetRotation; // Commented out
    ...
    private void EnforceBoundaryLimits()
    {
        ...
        if (currentDistanceFromSpawn > maxWanderRadius * 1.05f)
        {
            ...
            // Use Lerp for smoother position correction
            transform.position = Vector3.Lerp(transform.position, correctedPosition, 0.1f);
