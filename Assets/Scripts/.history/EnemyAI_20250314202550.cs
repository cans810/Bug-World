...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if (isChasing)
        {
            ...
            else if (distanceToLastKnown >= detectionFollowRange)
            {
                // If we've lost the target beyond our follow range, give up and return to wandering
                ReturnToWandering();
            }
        }
    }
    ...
    {
        // Skip if we're dealing with boundary collision
        if (isHittingBoundary)
        {
            ...
            {
                // Continue turning during boundary redirection
                livingEntity.RotateTowards(transform.right, 3.0f);
                return;
            }
        }
        ...
        // Only check for nest avoidance now
        ...
        {
            // Move normally - no boundary checking
    ...
    // Modify ReturnToWandering to ensure proper rotation reset
    private void ReturnToWandering()
    {
        ...
        if (wanderingBehavior != null)
        {
            // Add a short delay to ensure the idle animation plays for a moment
            StartCoroutine(DelayedWanderingResume());
        }
    }
    
    // Modify DelayedWanderingResume to fix rotation issues
    private IEnumerator DelayedWanderingResume()
    {
        // Ensure AIWandering doesn't start moving immediately
        if (wanderingBehavior != null)
        {
            wanderingBehavior.enabled = false;
        }
        
        // Ensure we stay in idle animation for a short period
        yield return new WaitForSeconds(1.0f);
        
        // Fix rotation before resuming wandering - ensure enemy is facing forward
        // This prevents the sideways movement issue
        if (livingEntity != null)
        {
            // Get a random forward direction that's not the current one
            Vector3 newForward = Quaternion.Euler(0, Random.Range(30f, 330f), 0) * Vector3.forward;
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Update the last known position if we're chasing
            if (isChasing && currentTarget != null)
            {
                lastKnownPlayerPosition = currentTarget.transform.position;
            }
