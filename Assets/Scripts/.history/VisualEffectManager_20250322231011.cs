    ...
    private IEnumerator FlyCrumbToInventory(GameObject crumbSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)  // Using same duration as chitin for consistency
        {
            ...
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                ...
                if (uiHelper != null)
                {
                    // Simply pulse the crumb counter
                    // This will use the current values from the game state
                    if (uiHelper.crumbCountText != null)
                    {
                        // Just animate the UI element without trying to update the value
                        RectTransform crumbRect = uiHelper.crumbCountText.GetComponent<RectTransform>();
                        if (crumbRect != null)
                        {
                            StartCoroutine(PulseUIElement(crumbRect));
                        }
                    }
                }
                effectsTriggered = true;
