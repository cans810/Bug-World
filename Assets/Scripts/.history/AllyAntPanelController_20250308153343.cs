...
public class AllyAntPanelController : MonoBehaviour
{
    ...
    [SerializeField] private float buttonSpacing = 10f; // Spacing between buttons
    ...
    private GameObject[] FindAllyAnts()
    {
        ...
        {
            if (obj.name.Contains("AllyAnt(Clone)") || obj.name.Contains("AllyAnt"))
            {
                allyAnts.Add(obj);
            }
        }
    ...
    private void CreateButtonForAllyAnt(GameObject allyAnt, int index)
    {
        ...
        // Position the button (adjust based on your UI layout)
        RectTransform rectTransform = newButton.GetComponent<RectTransform>();
        if (rectTransform != null)
        {
            rectTransform.anchoredPosition = new Vector2(0, -index * (rectTransform.rect.height + buttonSpacing));
        }
