...
public class UIHelper : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Initialize the PlayerAttributes reference
        cachedPlayerAttributes = FindObjectOfType<PlayerAttributes>();
        if (cachedPlayerAttributes == null)
        {
            Debug.LogWarning("UIHelper: PlayerAttributes not found!");
        }
    ...
    public void UpdateChitinDisplay(int count)
    {
        if (chitinCountText != null)
        {
            PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
            int maxCapacity = playerInventory != null ? playerInventory.MaxChitinCapacity : 50;
            
            chitinCountText.text = string.Format(countFormat, count, maxCapacity);
            
            if (animateOnChange && countTextRect != null)
            {
                // Call the method directly instead of by name
                AnimateChitinCount();
            }
        }
    }
    
    public void UpdateCrumbDisplay(int count)
    {
        if (crumbCountText != null)
        {
            PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
            int maxCapacity = playerInventory != null ? playerInventory.MaxCrumbCapacity : 5;
            
            crumbCountText.text = string.Format(countFormat, count, maxCapacity);
    ...
    // Fix the experience counter animation to show the full XP amount
    public void UpdateExperienceDisplay(int experience, bool animate = true)
    {
        // Make sure we have the text component
        if (experienceCountText == null)
            return;
        
        // Check the player inventory for XP requirements if available
        PlayerInventory playerInv = FindObjectOfType<PlayerInventory>();
        int xpRequiredForNextLevel = 100; // Default fallback
        
        if (playerInv != null)
        {
            // Use the correct property name ExperienceForNextLevel instead of GetRequiredXPForNextLevel method
            xpRequiredForNextLevel = playerInv.ExperienceForNextLevel;
        }
        
        // If we're not animating or animation is disabled, just update immediately
        if (!animate || !animateXPCounter)
        {
            currentDisplayedXP = experience;
            targetXP = experience;
            experienceCountText.text = string.Format(xpFormat, experience, xpRequiredForNextLevel);
            
            // Also update any progress bar if you have one
            UpdateExperienceBar(experience, xpRequiredForNextLevel);
            return;
        }
        
        // Otherwise animate the counter
        targetXP = experience; // Set the target value
        ...
        // Start new animation
        xpAnimationCoroutine = StartCoroutine(AnimateXPCounter(xpRequiredForNextLevel));
        
        // Also animate the scale of the XP text
    ...
    public void UpdateEggDisplay(int count)
    {
        if (eggCountText != null)
        {
            ...
            if (animateOnChange && eggTextRect != null)
            {
                ...
                SafeStopAndStartCoroutine("AnimateEggCount");
                
                // Failsafe reset
                StartCoroutine(ResetScaleAfterDelay(eggTextRect, originalEggScale, animationDuration + 0.1f));
            }
        }
    }
    ...
    public void DebugAttributePoint()
    {
        ...
        {
            // Add the points
            cachedPlayerAttributes.availablePoints += 25;
            
            // Call AddAttributePoints if it exists (using reflection to be safe)
            try {
                var method = cachedPlayerAttributes.GetType().GetMethod("AddAttributePoints");
                if (method != null) {
                    method.Invoke(cachedPlayerAttributes, new object[] { 25 });
                    Debug.Log("UIHelper: Called AddAttributePoints method");
                }
            ...
            if (cachedPlayerAttributes.OnAttributePointsChanged != null)
            {
                try {
                    cachedPlayerAttributes.OnAttributePointsChanged.Invoke(cachedPlayerAttributes.availablePoints);
                    Debug.Log("UIHelper: Triggered OnAttributePointsChanged event");
                } catch (System.Exception) {
                    // Ignore errors if event doesn't exist
                }
            }
            
            Debug.Log($"UIHelper: Added 25 attribute points. New total: {cachedPlayerAttributes.availablePoints}");
            
            // Force UI refresh in multiple ways
            if (attributesPanel != null && attributesPanel.activeInHierarchy)
            {
                // Try updating through AttributeDisplay
                AttributeDisplay attributeDisplay = attributesPanel.GetComponentInChildren<AttributeDisplay>();
                if (attributeDisplay != null)
                {
                    attributeDisplay.UpdateDisplay();
                    Debug.Log("UIHelper: Called UpdateDisplay on AttributeDisplay");
                }
                
                // Try finding any UI Text components that might show the attribute points
                TMPro.TextMeshProUGUI[] texts = attributesPanel.GetComponentsInChildren<TMPro.TextMeshProUGUI>();
                foreach (var text in texts)
                {
                    if (text.name.Contains("Point") || text.text.Contains("Point"))
                    {
        ...
        {
            Debug.LogError("UIHelper: cachedPlayerAttributes is null! Make sure it's assigned in the inspector.");
        }
    }
