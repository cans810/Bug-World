...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    public void PlayChitinDepositEffect(int chitinAmount, int xpAmount, PlayerInventory inventory)
    {
        ...
        if (inventory != null)
        {
            ...
            // Make sure we don't have duplicate coroutines running
            StopAllChitinAnimations();
            
            Debug.Log($"Starting chitin deposit animation for {chitinAmount} chitin and {xpAmount} XP");
            
            // Start the coroutine for spawning symbols and keep a reference to it
            Coroutine spawnCoroutine = StartCoroutine(SpawnChitinSymbols(symbolCount, xpAmount));
            activeCoroutines.Add(spawnCoroutine);
            
            // Start a safety coroutine to ensure XP is added even if animations are interrupted
    ...
    private IEnumerator EnsureXPIsAdded()
    {
        ...
        while (elapsed < timeLimit && !xpAddedThisSession)
        {
            elapsed += Time.deltaTime;
            
            // If XP hasn't been added yet, check if we should add it now
            if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
            {
                ...
                {
                    Debug.Log("Animation may have been interrupted - ensuring XP is added via safety coroutine");
        ...
        // Final check - if XP still hasn't been added, add it now
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            Debug.Log("Final safety check - adding XP that wasn't added during animation");
            playerInventory.AddXP(pendingXpAmount);
    ...
    private IEnumerator SpawnChitinSymbols(int count, int xpAmount)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Start animation to line up above center
            StartCoroutine(AnimateChitinToLineup(chitinSymbol, screenCenter, i, count));
            
            // Wait before spawning next symbol
            yield return new WaitForSeconds(spawnDelay);
        }
        
        // Wait for all chitins to reach their positions
    ...
    private IEnumerator AnimateChitinToLineup(GameObject chitinSymbol, Vector2 screenCenter, int index, int totalCount)
    {
        RectTransform symbolRT = chitinSymbol.GetComponent<RectTransform>();
        ...
        // Calculate position in lineup - above center of screen
        // Space them horizontally based on total count
        float lineWidth = symbolSpacing * totalCount;
        float startX = screenCenter.x - (lineWidth / 2) + (symbolSpacing / 2);
        
        Vector2 targetPos = new Vector2(
            startX + (index * symbolSpacing),
            screenCenter.y + 150f  // Increased from 80f to 150f to position chitins higher
        );
    ...
    private IEnumerator TransformChitinsToXP(List<GameObject> chitinSymbols, int totalXPAmount)
    {
        ...
        // Transform each chitin to XP, one by one
        for (int i = 0; i < chitinSymbols.Count; i++)
        {
            ...
            // Wait before transforming the next chitin
    ...
    private IEnumerator TransformChitinToXPWithEffect(GameObject chitinSymbol, int xpToSpawn, Vector2 targetPosition)
    {
        ...
        // Spawn XP symbols in a small burst
        for (int i = 0; i < symbolsToSpawn; i++)
        {
            ...
            // Small delay between each XP symbol
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        // Add XP if XP hasn't been added yet (regardless of if this is first symbol)
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
    ...
    {
        // Wait 1 second for any potential state changes
        yield return new WaitForSeconds(1f);
        
        // Check if animations are still needed
        if (isAnimationInProgress && !xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            ...
            if (totalXpSymbols == 0 || xpSymbolsReachedTarget == 0)
            {
        ...
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            Debug.Log("Final safety check: animations didn't complete, adding XP directly");
            playerInventory.AddXP(pendingXpAmount);
            xpAddedThisSession = true;
        }
    }
