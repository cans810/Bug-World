...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        // Skip movement input if controls are disabled
    ...
    // Add or update this method to enable/disable player controls
    public void SetControlsEnabled(bool enabled)
    {
        ...
        if (!enabled)
        {
            // Stop any movement when disabling controls
            moveDirection = Vector3.zero;
            currentSpeed = 0f;
            
            // If we have an animator, set its parameters to stop movement animation
            if (animController != null)
            {
