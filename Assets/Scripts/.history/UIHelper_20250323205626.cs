...
public class UIHelper : MonoBehaviour
{
    ...
    {
        Debug.Log("UIHelper.ShowPanel called directly from button click");
        
        // Find the AttributeDisplay component
        AttributeDisplay attributeDisplay = FindObjectOfType<AttributeDisplay>();
        if (attributeDisplay != null)
        {
            // Tell it this is definitely player-initiated
            attributeDisplay.SetPlayerAction(true);
            
            // Show it using the most direct method 
            attributeDisplay.ForceShowAttributesPanel();
        }
        else
        {
            Debug.LogError("UIHelper.ShowPanel: Could not find AttributeDisplay component!");
            
            // Fallback: try to find panel directly
    ...
    // Add this ultra-direct method
    public void DirectShowAttributesPanel()
    {
        Debug.Log("UIHelper: DIRECT attribute panel activation attempt");
        
        // Try to find the panel directly
        GameObject attributesPanel = GameObject.Find("AttributesPanel");
        if (attributesPanel != null)
        {
            Debug.Log("UIHelper: Found AttributesPanel directly - activating");
            attributesPanel.SetActive(true);
            
            // Try to find and set the animator if available
            Animator panelAnimator = attributesPanel.GetComponent<Animator>();
            if (panelAnimator != null)
            {
                panelAnimator.SetBool("ShowUp", true);
        ...
        {
            Debug.LogError("UIHelper: Could not find AttributesPanel in the scene");
        }
    }

    // Add this method to show a notification instead of the panel
    public void NotifyAttributePointsAvailable(int points)
    {
        if (points > 0)
        {
            // Show an unobtrusive notification  
            ShowInformText($"You have {points} attribute points to spend!");
            
            // Optional: Add a visual indicator on the attribute button
            GameObject attributeButton = GameObject.Find("AttributeButton");
            if (attributeButton != null)
            {
                // Add a pulsing effect or color change to indicate points are available
                Image buttonImage = attributeButton.GetComponent<Image>();
                if (buttonImage != null)
                {
                    // Make it glow or change color
    ...
    // Modify or add this method to UIHelper - this will be called by buttons
    public void ShowAttributesPanel()
    {
        Debug.Log("UIHelper.ShowAttributesPanel called - this is definitely a player action");
        
        // Find the AttributeDisplay component if needed
        AttributeDisplay attributeDisplay = FindObjectOfType<AttributeDisplay>();
        if (attributeDisplay != null)
        {
            // Use the direct method that bypasses all safety checks
            attributeDisplay.ForceShowAttributesPanel();
        }
        else
        {
            ...
            // Direct fallback that doesn't require AttributeDisplay
            GameObject attributesPanel = GameObject.Find("AttributesPanel");
            if (attributesPanel != null)
            {
                attributesPanel.SetActive(true);
