...
public class UIHelper : MonoBehaviour
{
    ...
    private Transform FindPlayerNest()
    {
        ...
        if (nestObject == null)
        {
            ...
            return posToDropIcons; // Fallback to UI position
        }
        return nestObject.transform;
    }

    // Update the AnimateSingleChitinIcon method to target the 3D world nest
    private IEnumerator AnimateSingleChitinIcon(Vector3 startPos)
    {
        ...
        Debug.Log("Moving chitin icon to Player Nest 1");
        
        // Find the Player Nest 1 in the scene
        Transform nestTransform = FindPlayerNest();
        
        // Convert the 3D world position of the nest to screen space
        Vector3 nestScreenPos = Camera.main.WorldToScreenPoint(nestTransform.position);
        
        // Adjust the z-coordinate to ensure it's in front of the camera
        nestScreenPos.z = 0;
        
        // Move to the nest position
        float finalMoveTime = 0.7f;
        Vector3 finalPos = nestScreenPos;
        ...
        while (elapsedTime < finalMoveTime)
        {
            ...
            // Recalculate the screen position of the nest each frame to handle moving camera/nest
            Vector3 currentNestScreenPos = Camera.main.WorldToScreenPoint(nestTransform.position);
            currentNestScreenPos.z = 0;
            
            // Calculate position with arc
            Vector3 newPos = Vector3.Lerp(initialPos, currentNestScreenPos, t);
        ...
        Debug.Log("Reached Player Nest 1, destroying icon");
