...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if (isAtMapBorder && moveDirection.magnitude > 0.1f)
        {
            ...
            {
                // Only allow movement components that are perpendicular or toward center
                moveDirection = moveDirection - (borderNormal * movementWithBorder);
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Reset the joystick to stop ongoing movement
            if (joystick != null)
            {
                joystick.ResetJoystick();
            }
            
            // Immediately move the player back inside the boundary
            if (livingEntity != null)
            {
                // Push the player back toward the center
                livingEntity.MoveInDirection(borderNormal, 0.2f);
            }
            else
            {
                // Fallback direct position adjustment
