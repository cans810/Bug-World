...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Get starting position from chitin UI element
        Vector3 startPos;
        if (chitinCountText != null)
        {
        ...
        // Get the canvas for proper UI positioning
        Canvas parentCanvas = GetComponentInParent<Canvas>();
        if (parentCanvas == null)
        {
            Debug.LogError("[CRITICAL] No parent canvas found!");
            yield break;
        }
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            ...
            // Create icon
            GameObject icon = Instantiate(chitinIconPrefab, parentCanvas.transform);
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            while (timer < duration)
            {
                ...
                Vector3 newPos = Vector3.Lerp(iconStartPos, targetPos, t);
                // Add arc height (sine wave)
                newPos.y += 100f * Mathf.Sin(t * Mathf.PI);
    ...
    public void DebugAnimateChitins(int count)
    {
        ...
        StopCoroutine("SpawnAndAnimateChitinIcons");
