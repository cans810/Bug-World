...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        {
            // We've hit a boundary, mark as hitting boundary and start redirection timer
            isHittingBoundary = true;
            boundaryRedirectionTime = Time.time + BOUNDARY_REDIRECT_DURATION;
            
            // Get a perpendicular direction to move along the boundary
            Vector3 boundaryDirection = (mapBoundary.transform.position - transform.position).normalized;
            Vector3 newDirection = Vector3.Cross(Vector3.up, boundaryDirection);
            
            // Randomly choose direction
            if (Random.value < 0.5f)
                newDirection = -newDirection;
            
            // Turn to follow the new direction
            transform.forward = newDirection;
            
            // Set a new waypoint in this direction
            SetWaypointInDirection(newDirection);
        }
