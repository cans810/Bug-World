...
public class PlayerController : MonoBehaviour
{
    ...
    private void ShowMetamorphosisArrows()
    {
        ...
        // Position the center arrow lower in the view
        var distanceField = typeof(ScreenCenterArrowIndicator).GetField("distanceFromCamera", 
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
        if (distanceField != null)
            distanceField.SetValue(centerArrow, 0.6f); // Fixed close distance
        ...
        Debug.Log($"Created metamorphosis arrows at lower height for player size: {transform.localScale}");
    }

    // Improved helper method to create circular pattern of arrows at lower positions
    private void CreateCircularArrows(GameObject parent)
    {
        int arrowCount = 6;
        // Scale radius based on player size but keep it reasonable
        float radius = Mathf.Clamp(transform.localScale.x * 1.8f, 2f, 8f);
        ...
        for (int i = 0; i < arrowCount; i++)
        {
            ...
            Vector3 position = transform.position + new Vector3(
                ...
                // Position much lower - about 25% of player height
