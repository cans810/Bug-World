...
public class AttributeDisplay : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI recoveryValueText;  // New attribute
    ...
    [SerializeField] private TextMeshProUGUI recoveryBonusText;  // New attribute
    ...
    [SerializeField] private Button recoveryButton;  // New attribute
    ...
    {
        // Skip showing panel if we're loading data
        ...
        // The panel will be shown by the level-up sequence handler
        UpdateDisplay();
        
        // NOTE: We're removing the automatic panel show here, since it will be
        // handled by the level-up sequence in PlayerInventory
    }
    
    public void ShowPanel(bool show)
    {
        // Don't show the panel if a camera animation is in progress
        // (unless explicitly called from CameraAnimations.ShowAttributesPanelAfterAnimation)
        if (show && cameraAnimations != null && cameraAnimations.IsAnimationInProgress())
        {
            Debug.Log("Skipping attributes panel display during camera animation");
            return;
        }
        ...
        if (attributesPanel != null)
        {
            ...
            Debug.Log($"AttributeDisplay.ShowPanel called. Panel active: {attributesPanel.activeSelf}, Force: {show}");
            ...
            {
                // If no animator, make sure the panel is visible
                Debug.Log("No animator found on attributes panel");
            }
        }
        else
        {
    ...
    public void UpdateDisplayWithValues(int strength, int vitality, int agility, int incubation, int availablePoints)
    {
        // Call the new method with default recovery value of 0
        UpdateDisplayWithValues(strength, vitality, agility, incubation, 0, availablePoints);
        
        Debug.Log("Using backward compatibility method for UpdateDisplayWithValues - recovery set to 0");
    }
