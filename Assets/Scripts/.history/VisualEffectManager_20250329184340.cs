...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public GameObject safeAreaGameObject;
    ...
    // Complete rework of symbol animation system for consistent UI-like behavior
    public void PlayXPSymbolEffect(Vector3 startPosition, int amount, bool useWorldSpace = true)
    {
        ...
        // Get reference to canvas for UI symbols
        Canvas mainCanvas = FindObjectOfType<Canvas>();
        if (mainCanvas == null || mainCanvas.renderMode != RenderMode.ScreenSpaceOverlay)
        {
            Debug.LogError("Cannot spawn XP symbols - no ScreenSpaceOverlay canvas found");
            return;
        }
        
        // Convert the world start position to screen position
        Camera mainCamera = Camera.main;
        ...
        // Find the XP panel position in screen space
        RectTransform xpPanelRect = xpPanel?.GetComponent<RectTransform>();
        if (xpPanelRect == null)
        {
            Debug.LogWarning("XP Panel rect not found, using default position");
        }
        
        // Get screen position of XP panel
        Vector2 targetScreenPosition = xpPanelRect != null ? 
            RectTransformUtility.WorldToScreenPoint(mainCamera, xpPanelRect.position) : 
            new Vector2(Screen.width * 0.8f, Screen.height * 0.1f);
        
        // Start spawning XP symbols as UI elements
        StartCoroutine(SpawnUISymbols(screenStartPos, targetScreenPosition, amount, mainCanvas, "xp"));
    }

    // Rework for crumb symbols too - similar approach
    public void PlayCrumbSymbolEffect(Vector3 startPosition, int amount)
    {
        ...
        if (mainCanvas == null || mainCanvas.renderMode != RenderMode.ScreenSpaceOverlay)
        {
            Debug.LogError("Cannot spawn crumb symbols - no ScreenSpaceOverlay canvas found");
            return;
        }
        ...
        // Find the crumb panel position in screen space
        RectTransform crumbPanelRect = crumbPanel?.GetComponent<RectTransform>();
        if (crumbPanelRect == null)
        {
            Debug.LogWarning("Crumb Panel rect not found, using default position");
        }
        
        // Get screen position of crumb panel
        Vector2 targetScreenPosition = crumbPanelRect != null ? 
            RectTransformUtility.WorldToScreenPoint(mainCamera, crumbPanelRect.position) : 
            new Vector2(Screen.width * 0.2f, Screen.height * 0.1f);
        
        // Start spawning crumb symbols as UI elements
        StartCoroutine(SpawnUISymbols(screenStartPos, targetScreenPosition, amount, mainCanvas, "crumb"));
    }

    // Rework for chitin symbols too
    public void PlayChitinSymbolEffect(Vector3 startPosition, int amount)
    {
        ...
        if (mainCanvas == null || mainCanvas.renderMode != RenderMode.ScreenSpaceOverlay)
        {
            Debug.LogError("Cannot spawn chitin symbols - no ScreenSpaceOverlay canvas found");
            return;
        }
        ...
        // Find the chitin panel position in screen space
        RectTransform chitinPanelRect = chitinPanel?.GetComponent<RectTransform>();
        if (chitinPanelRect == null)
        {
            Debug.LogWarning("Chitin Panel rect not found, using default position");
        }
        
        // Get screen position of chitin panel
        Vector2 targetScreenPosition = chitinPanelRect != null ? 
            RectTransformUtility.WorldToScreenPoint(mainCamera, chitinPanelRect.position) : 
            new Vector2(Screen.width * 0.5f, Screen.height * 0.1f);
        
        // Start spawning chitin symbols as UI elements
        StartCoroutine(SpawnUISymbols(screenStartPos, targetScreenPosition, amount, mainCanvas, "chitin"));
    }

    // New method to spawn UI-based symbols that are guaranteed to be visible
    private IEnumerator SpawnUISymbols(Vector2 screenStartPos, Vector2 targetScreenPos, int count, Canvas canvas, string symbolType)
    {
        if (count <= 0 || canvas == null) yield break;
        ...
        {
            case "xp":
                prefab = xpSymbolPrefab;
                soundEffect = xpGainedSoundName;
                break;
            case "crumb":
                prefab = crumbSymbolPrefab;
                soundEffect = "CrumbCollect";
                break;
            case "chitin":
                prefab = chitinSymbolPrefab;
                soundEffect = "ResourceCollected";
                break;
            case "coin":
                prefab = coinSymbolPrefab;
                soundEffect = "CoinCollect";
                break;
            default:
                Debug.LogError($"Unknown symbol type: {symbolType}");
                yield break;
        }
        
        if (prefab == null)
        {
            Debug.LogError($"No prefab found for symbol type: {symbolType}");
            yield break;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            // Create a new UI Image for the symbol
            GameObject uiSymbol = new GameObject($"{symbolType}Symbol_{i}");
            uiSymbol.transform.SetParent(canvas.transform, false);
            
            // Add RectTransform and Image components
            RectTransform rectTransform = uiSymbol.AddComponent<RectTransform>();
            Image image = uiSymbol.AddComponent<Image>();
            
            // Get sprite from prefab if possible
            ...
            {
                Debug.LogWarning($"No sprite found on {symbolType} prefab, using default sprite");
                // You might want to have fallback sprites here
            }
            
            // Set initial position with small random offset
            float randomXOffset = Random.Range(-50f, 50f);
            rectTransform.position = new Vector3(
                screenStartPos.x + randomXOffset,
                screenStartPos.y,
                0
            );
            
            // Set size based on original scale
            rectTransform.sizeDelta = new Vector2(50f, 50f); // Adjust size as needed
            
            // Find the player position in screen space
            Camera mainCamera = Camera.main;
            if (mainCamera == null) break;
            
            GameObject player = GameObject.FindWithTag("Player");
            Vector2 playerScreenPos = player != null 
                ? mainCamera.WorldToScreenPoint(player.transform.position) 
                : new Vector2(Screen.width / 2, Screen.height / 2);
                
            // Start animation coroutine for this symbol
            StartCoroutine(AnimateUISymbol(uiSymbol, rectTransform.position, playerScreenPos, targetScreenPos, symbolType, soundEffect));
    ...
    // UI Symbol animation that's guaranteed to stay in front
    private IEnumerator AnimateUISymbol(GameObject symbol, Vector2 startPos, Vector2 midPos, Vector2 endPos, string symbolType, string soundEffectName)
    {
        ...
        while (elapsed < hoverDuration)
        {
            ...
            float yOffset = Mathf.Sin(t * Mathf.PI * 2) * 15f;
            rect.position = new Vector3(startPos.x, startPos.y + yOffset, 0);
            
            yield return null;
        }
        
        // Main animation - first half: to player
        float duration = (symbolType == "xp") ? xpFlyDuration : 1.0f;
        elapsed = 0f;
        ...
        while (elapsed < duration)
        {
            ...
            if (normalizedTime < 0.5f)
            {
                float t = normalizedTime * 2f; // 0-1 for first half
                
                // Arc movement
                float arcHeight = Mathf.Sin(t * Mathf.PI) * 40f; 
                
                // Lerp with arc
                Vector2 pos = Vector2.Lerp(startPos, midPos, t);
                pos.y += arcHeight;
            ...
            {
                float t = (normalizedTime - 0.5f) * 2f; // 0-1 for second half
                
                // Arc movement (smaller for second half)
                float arcHeight = Mathf.Sin(t * Mathf.PI) * 25f;
                
                // Lerp with arc
                Vector2 pos = Vector2.Lerp(midPos, endPos, t);
                pos.y += arcHeight;
        ...
        // Play sound effect at the end
        if (!string.IsNullOrEmpty(soundEffectName) && SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound(soundEffectName);
        }
    ...
    // Add this method to ensure any remaining world space symbols always face the camera
    private void Update()
    {
        Camera mainCamera = Camera.main;
        if (mainCamera == null) return;
        
        // Check all active symbol types
        foreach (var symbolType in new string[] { "xp", "chitin", "crumb", "coin" })
        {
            if (!symbolPools.ContainsKey(symbolType)) continue;
            
            // Find all active symbols of this type
            foreach (Transform child in transform)
            {
                ...
                if (child.name.Contains(symbolType))
                {
