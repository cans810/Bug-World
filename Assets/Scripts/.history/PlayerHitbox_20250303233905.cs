...
public class PlayerHitbox : MonoBehaviour
{
    ...
    [SerializeField] private string hitboxName = "Player Attack";
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (showDebugMessages)
        {
            Debug.Log($"[{hitboxName}] Hitbox collided with: {other.gameObject.name} (Layer: {LayerMask.LayerToName(other.gameObject.layer)})");
        }
        ...
        if (entity != null)
        {
            if (showDebugMessages)
            {
                Debug.Log($"[{hitboxName}] Dealing {damage} damage to {other.gameObject.name}. Current health: {entity.CurrentHealth}");
            }
            ...
            if (showDebugMessages)
            {
                Debug.Log($"[{hitboxName}] After damage health: {entity.CurrentHealth}");
            }
        }
        else
        {
            if (showDebugMessages)
            {
                Debug.Log($"[{hitboxName}] No LivingEntity component found on {other.gameObject.name}");
            }
        }
    }
