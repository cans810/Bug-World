...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Check if level up symbol prefab is assigned
        if (levelupSymbolPrefab == null)
        {
            Debug.LogError("Level Up Symbol prefab not assigned in VisualEffectManager!");
            return;
        }
        ...
        // Check if we have a pool for level up symbols
        if (!symbolPools.ContainsKey("levelup") || symbolPools["levelup"].Count == 0)
        {
            Debug.LogWarning("Level up symbol pool is missing or empty. Make sure levelupSymbolPrefab is assigned in the inspector.");
            return;
        }
