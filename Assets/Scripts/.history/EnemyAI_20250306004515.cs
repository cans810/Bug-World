...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Remove the animation update from here - we'll handle it only when state changes
        ...
        else if (isChasing)
        {
            ...
            {
                // Only update animation if we're transitioning from stopping to moving
                if (!isMoving)
                {
                    isMoving = true;
    ...
    private void ChaseAndAttackTarget()
    {
        ...
        // Check if we're ready to attack (cooldown expired)
        bool readyToAttack = Time.time >= lastAttackTime + attackInterval;
        
        // If we're close enough to attack AND the attack cooldown has expired
        if (distanceToTarget <= minAttackDistance && readyToAttack)
        {
            // Stop moving only when actually attacking
            if (isMoving)
            {
                isMoving = false;
                UpdateAnimation(false);
        ...
        // Otherwise, if we're in attack range but not ready to attack yet, keep moving
        else if (distanceToTarget <= minAttackDistance && !readyToAttack)
        {
            // Continue moving if we're not already
            if (!isMoving)
            {
                isMoving = true;
                UpdateAnimation(true);
            ...
            // Just face the target but don't move closer
            FaceTarget(currentTarget.transform.position);
        }
        // Otherwise, move towards the target
        else
        {
            // Only update animation state if we're transitioning from stopping to moving
            if (!isMoving)
            {
                isMoving = true;
                UpdateAnimation(true);
    ...
    private void ReturnToWandering()
    {
        ...
        // Only update animation if we're transitioning from moving to stopping
        if (isMoving)
        {
            isMoving = false;
            UpdateAnimation(false);
