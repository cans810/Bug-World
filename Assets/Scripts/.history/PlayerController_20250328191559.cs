...
public class PlayerController : MonoBehaviour
{
    ...
    // Update OnTriggerExit to prevent leaving without teleporting
    private void OnTriggerExit(Collider other)
    {
        // Check if player is trying to exit the BigBorder
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            Debug.LogError("Player attempted to exit BigBorder - PREVENTING EXIT");
            
            // Find closest point on border and move player there
            Vector3 closestPointOnBorder = other.ClosestPoint(transform.position);
            
            // Move player back to just inside the border (with small offset)
            Vector3 directionToInside = (other.bounds.center - transform.position).normalized;
            transform.position = closestPointOnBorder + directionToInside * 0.5f;
            
            // Zero out movement direction to prevent continued pushing
            moveDirection = Vector3.zero;
            
            // If we have a rigidbody, zero out velocity
            Rigidbody rb = GetComponent<Rigidbody>();
            if (rb != null && !rb.isKinematic)
            {
                rb.velocity = new Vector3(0, rb.velocity.y, 0);
            ...
            if (uiHelper != null)
            {
                uiHelper.ShowInformText("You cannot leave the playable area!");
                HapticFeedback.MediumFeedback();
            }
        }
    }
    ...
    // Update OnTriggerStay to handle all borders correctly
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Get player level (ensure we have reference)
            int playerLevel = 1;
            if (playerInventory == null)
            {
                playerInventory = GetComponent<PlayerInventory>();
            ...
            // If this is a level-restricted border and player doesn't meet level requirement
            if (requiredLevel > 0 && playerLevel < requiredLevel)
            {
                // Calculate border normal direction (away from border)
                Vector3 playerToColliderCenter = transform.position - other.bounds.center;
                playerToColliderCenter.y = 0; // Keep it horizontal
                borderNormal = playerToColliderCenter.normalized;
                
                // Set the border state to prevent movement
                isAtMapBorder = true;
                
                // Get the closest point on the border
                Vector3 closestPoint = other.ClosestPoint(transform.position + borderNormal * 2.0f);
                
                // Check if player is trying to move deeper into the restricted area
                if (Vector3.Dot(moveDirection, -borderNormal) > 0)
                {
                    // Cancel movement by zeroing moveDirection
                    moveDirection = Vector3.zero;
                    
                    // Only modify position if player is deep inside the border
                    float distanceToBorder = Vector3.Distance(transform.position, closestPoint);
                    if (distanceToBorder > 1.5f)
                    {
                        // Move player closer to border edge (but not fully to edge to avoid jittering)
                        transform.position = Vector3.Lerp(transform.position, closestPoint, 0.3f);
                    }
                    
                    // Zero velocity if using Rigidbody
                    Rigidbody rb = GetComponent<Rigidbody>();
                    if (rb != null && !rb.isKinematic)
                    {
                        rb.velocity = new Vector3(0, rb.velocity.y, 0);
                ...
                // Show level requirement message (with cooldown)
            ...
            {
                // Player either meets the level requirement OR this is not a level-restricted border
                
                // Update the player's border object name
                ...
                if (playerAttributes != null && playerAttributes.borderObjectName != borderName)
                {
                    ...
                    Debug.Log($"Updated player border to: {borderName}");
                }
                
                // Clear restriction state
                isAtMapBorder = false;
                borderNormal = Vector3.zero;
            }
        }
        ...
        {
            // Player is inside BigBorder - ensure we're not restricting movement
            isAtMapBorder = false;
            borderNormal = Vector3.zero;
        }
    }
