...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add these fields to PlayerInventory class
    ...
    // Modify the AddExperience method to handle multiple level-ups sequentially
    public void AddExperience(int amount)
    {
        // Skip if we're in loading mode
        if (isLoadingData) return;
        
        // Store previous level for comparison
        int previousLevel = CurrentLevel;
        
        // Add the experience to the underlying field
        _experience += amount;
        
        // Notify listeners of the XP change
        OnExperienceChanged?.Invoke(_experience);
        ...
        // If we gained at least one level
        if (levelsGained > 0)
        {
            Debug.Log($"Gained {levelsGained} levels! From {previousLevel} to {newLevel}");
            
            // Add to pending level-ups
            pendingLevelUps += levelsGained;
            
            // Start processing level-ups if not already doing so
            if (!isProcessingLevelUps)
            {
                StartCoroutine(ProcessLevelUpsSequentially());
            }
        }
    }

    // Add this coroutine to process level-ups one at a time
    private IEnumerator ProcessLevelUpsSequentially()
    {
        isProcessingLevelUps = true;
        
        // Check if OnLevelUp has multiple subscribers
        if (OnLevelUp != null)
        {
            Debug.Log($"OnLevelUp event has {OnLevelUp.GetInvocationList().Length} subscribers");
            foreach (var subscriber in OnLevelUp.GetInvocationList())
            {
                Debug.Log($"OnLevelUp subscriber: {subscriber.Method.DeclaringType}.{subscriber.Method.Name}");
            }
        }
        ...
        while (pendingLevelUps > 0)
        {
            ...
            {
                Debug.Log($"Reached max level ({maxLevel}). Stopping level-up sequence.");
                break;
            }
            
            Debug.Log($"Processing level-up: {CurrentLevel} → {nextLevel}");
            
            // Set the new level
            CurrentLevel = nextLevel;
            
            // IMPORTANT: Add attribute points for this level
            AddAttributePointsForLevelUp();
            ...
            // Handle level-up sequence (camera animations, etc.)
            HandleLevelUpSequence(nextLevel);
            
            // Decrement pending level-ups
            pendingLevelUps--;
            
            // Check if this level unlocks a new area
            bool hasNewArea = CheckForNewAreaAtLevel(nextLevel);
            
            // If this level unlocks a new area, wait for the camera animation to complete
            if (hasNewArea)
            {
                Debug.Log($"Level {nextLevel} unlocks a new area. Waiting for camera animation...");
            ...
            {
                // If no new area, just add a small delay between level-ups
    ...
    // Add this method to award attribute points for each level-up
    private void AddAttributePointsForLevelUp()
    {
        // Find the PlayerAttributes component
        PlayerAttributes playerAttributes = FindObjectOfType<PlayerAttributes>();
        if (playerAttributes != null)
        {
            // Add 2 attribute points per level
            int pointsToAdd = 2;
            
            // Log before adding points
            Debug.Log($"BEFORE adding points: Player has {playerAttributes.AvailablePoints} attribute points");
            
            playerAttributes.AddAttributePoints(pointsToAdd);
            
            // Log after adding points
            Debug.Log($"AFTER adding points: Player has {playerAttributes.AvailablePoints} attribute points");
            
            Debug.Log($"Added {pointsToAdd} attribute points for level-up to level {CurrentLevel}");
        }
        else
        {
            Debug.LogError("Could not find PlayerAttributes component to add attribute points!");
        }
    }

    // Helper method to check if a level unlocks a new area
    private bool CheckForNewAreaAtLevel(int level)
    {
