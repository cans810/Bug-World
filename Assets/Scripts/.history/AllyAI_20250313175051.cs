...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Only do major calculations periodically to reduce jitter
        bool isMajorUpdate = (Time.time - lastMajorUpdateTime) >= positionUpdateInterval;
        if (isMajorUpdate)
        {
            // Track if the player is stationary
            if (lastPlayerPosition != Vector3.zero)
            {
                float playerMoveDist = Vector3.Distance(playerTransform.position, lastPlayerPosition);
                isPlayerMoving = playerMoveDist > playerMovementThreshold;
            }
            lastPlayerPosition = playerTransform.position;
            lastMajorUpdateTime = Time.time;
            
            // Calculate the ideal position to move to - position offset from player
            float idealDistance = isPlayerMoving ? followDistance : playerStationaryStopDistance;
            
            // Find the direction from player to us
            Vector3 dirFromPlayer = (transform.position - playerTransform.position).normalized;
            
            // If we're too close to the player or at a weird angle, use a random offset direction
            if (dirFromPlayer.magnitude < 0.1f)
            {
                float randomAngle = Random.Range(0f, 360f) * Mathf.Deg2Rad;
                dirFromPlayer = new Vector3(Mathf.Cos(randomAngle), 0, Mathf.Sin(randomAngle));
            }
            
            // Calculate target position - where we want to end up
            targetPosition = playerTransform.position + dirFromPlayer * idealDistance;
        }
        ...
        float stopDistance = isPlayerMoving ? followDistance : playerStationaryStopDistance;
        bool shouldMove = distanceToPlayer > stopDistance + 0.5f;
        ...
        if (shouldMove)
        {
            ...
            // Apply very smooth rotation using our LivingEntity method
            if (dirToTarget.magnitude > 0.1f)
            {
                livingEntity.RotateTowards(dirToTarget, rotationSmoothFactor);
            }
            
            // Calculate velocity based on direction and current orientation
            Vector3 targetVelocity = transform.forward * livingEntity.moveSpeed * speedMultiplier;
            
            // Smooth velocity changes between frames
            Vector3 newVelocity = Vector3.Lerp(rb.velocity, targetVelocity, velocitySmoothFactor * Time.deltaTime);
            
            // Set velocity, preserving Y component
            rb.velocity = new Vector3(newVelocity.x, rb.velocity.y, newVelocity.z);
            
            // Mark that we've applied movement
            hasAppliedMovementThisFrame = true;
            
            // Update animation - only if it's not already in the right state
        ...
        {
            // Very gradually face the player when idle
            Vector3 lookDir = (playerTransform.position - transform.position).normalized;
            if (lookDir.magnitude > 0.1f)
            {
                livingEntity.RotateTowards(lookDir, rotationSmoothFactor * 0.5f);
            }
            
            // Slow to a stop
            if (rb.velocity.magnitude > 0.1f)
            {
                rb.velocity = Vector3.Lerp(rb.velocity, new Vector3(0, rb.velocity.y, 0), 5f * Time.deltaTime);
            }
