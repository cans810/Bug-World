...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        if (isTooCloseToNest)
        {
            ...
            // First rotate to face away from nest
        ...
        {
            // First ensure we're facing the direction we want to move
            livingEntity.RotateTowards(direction, targetRotationSpeed);
    ...
    private void ChaseAndAttackTarget()
    {
        ...
        else
        {
            ...
            // First face the target completely before moving
            FaceTarget(currentTarget.transform.position);
            
            // Only move forward after properly facing the target
            // This ensures we're always moving in the direction we're facing
            livingEntity.MoveInDirection(transform.forward);
        }
    }
    
    // Modify ReturnToWandering to include a pause after chase
    ...
    // New coroutine to handle the pause after chase
