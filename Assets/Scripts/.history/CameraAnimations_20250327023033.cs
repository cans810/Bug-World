...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator MetamorphosisCameraAnimation(Transform playerTransform, System.Action onCameraInPosition)
    {
        ...
        // UPDATED: Position camera in front of the player, scaled by size
        // Get player forward direction (we'll position opposite to this to see the front)
        Vector3 playerForward = playerTransform.forward;
        
        // Calculate scale factor based on player size - larger scale = greater distance
        float playerSize = (playerTransform.localScale.x + playerTransform.localScale.y + playerTransform.localScale.z) / 3f;
        
        // Scale distances based on player size
        float frontDistance = Mathf.Max(5.0f, playerSize * 3.5f); // Further for larger players
        float heightFactor = Mathf.Max(3.5f, playerSize * 1.5f);  // Higher for larger players
        
        // Position camera in front and above the player
        Vector3 horizontalOffset = -playerForward * frontDistance; // Negative forward = in front of player
        Vector3 targetPosition = playerTransform.position + horizontalOffset;
        targetPosition.y = playerTransform.position.y + heightFactor; // Elevate camera
        
        // Calculate rotation to look at the player from this position
