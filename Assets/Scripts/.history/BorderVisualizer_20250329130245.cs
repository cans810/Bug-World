...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Determine capsule direction and perpendicular vectors
        Vector3 direction;
        ...
        // Calculate half-length of the capsule's straight section
        ...
        // For perimeter rendering, we want a circle in the horizontal plane
        int segments = 36;
        perimeterLineRenderer.positionCount = segments;
        
        // Project both endpoints to the desired height
        Vector3 projectedPoint1 = new Vector3(point1.x, perimeterHeight, point1.z);
        Vector3 projectedPoint2 = new Vector3(point2.x, perimeterHeight, point2.z);
        
        // Calculate horizontal direction (for straight lines)
        Vector3 horizontalDirection = projectedPoint2 - projectedPoint1;
        horizontalDirection.y = 0; // Ensure it's perfectly horizontal
        
        if (horizontalDirection.magnitude < 0.001f) // Vertical capsule
        {
            // Just draw a circle for vertical capsules
            for (int i = 0; i < segments; i++)
            {
                float angle = (float)i / segments * 2f * Mathf.PI;
                float x = center.x + radius * Mathf.Cos(angle);
        ...
        {
            // For horizontal capsules, we need to draw a pill shape
            horizontalDirection = horizontalDirection.normalized;
            Vector3 perpendicular = new Vector3(-horizontalDirection.z, 0, horizontalDirection.x);
            
            // Points per section
            int halfCirclePoints = segments / 2;
            int pointsPerQuarter = halfCirclePoints / 2;
            
            // First quarter circle
            for (int i = 0; i <= pointsPerQuarter; i++)
            {
                float angle = Mathf.PI * 0.5f - Mathf.PI * 0.5f * i / pointsPerQuarter;
                Vector3 point = projectedPoint1 + perpendicular * radius * Mathf.Cos(angle) - 
                                horizontalDirection * radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(i, point);
            }
            
            // Straight line on one side
            perimeterLineRenderer.SetPosition(pointsPerQuarter + 1, 
                projectedPoint1 - perpendicular * radius);
            perimeterLineRenderer.SetPosition(pointsPerQuarter + 2, 
                projectedPoint2 - perpendicular * radius);
            
            // Second quarter circle
            for (int i = 0; i <= pointsPerQuarter; i++)
            {
                float angle = -Mathf.PI * 0.5f * i / pointsPerQuarter;
                Vector3 point = projectedPoint2 - horizontalDirection * radius * Mathf.Cos(angle) - 
                                perpendicular * radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(pointsPerQuarter + 3 + i, point);
            }
            
            // Straight line on the other side
            perimeterLineRenderer.SetPosition(pointsPerQuarter * 3 + 3, 
                projectedPoint2 + perpendicular * radius);
            perimeterLineRenderer.SetPosition(pointsPerQuarter * 3 + 4, 
                projectedPoint1 + perpendicular * radius);
            
            // Third quarter circle (back to start)
            for (int i = 1; i < pointsPerQuarter; i++)
            {
                float angle = Mathf.PI * 0.5f + Mathf.PI * 0.5f * i / pointsPerQuarter;
                Vector3 point = projectedPoint1 + perpendicular * radius * Mathf.Cos(angle) - 
                                horizontalDirection * radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(pointsPerQuarter * 3 + 4 + i, point);
            }
        }
    }
