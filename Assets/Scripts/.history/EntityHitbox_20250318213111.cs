...
public class EntityHitbox : MonoBehaviour
{
    ...
    public void NotifyEntityDeath(LivingEntity killedEntity)
    {
        if (killedEntity == null) return;
        
        // Check if the owner is the player
        if (owner != null && owner.gameObject.CompareTag("Player"))
        {
            // Attempt to discover the entity in the controller
            ...
            {
                string entityName = killedEntity.EntityType;
                if (string.IsNullOrEmpty(entityName))
                {
                    // Try to get entity name from the GameObject name
                    entityName = killedEntity.gameObject.name;
                    
                    // Remove common prefixes/suffixes like "Enemy_" or "(Clone)"
                    entityName = entityName.Replace("(Clone)", "").Replace("Enemy_", "");
                    
                    Debug.Log($"Using derived entity name: {entityName}");
                }
                
                // Log the attempt to discover
                Debug.Log($"Player killed entity: {entityName}, attempting discovery");
                
                // Try to discover both by the kill method and normal method for redundancy
                discoveryController.DiscoverEntityOnKill(entityName);
                discoveryController.CheckAndDiscoverEntity(entityName);
            }
        }
    }
} 