...
public class PlayerInventory : MonoBehaviour
{
    ...
    private int[] xpRequirements = new int[55] { 0, 20, 60, 110, 150, 210, 280, 350, 430, 510, 600, 700, 
    ...
    // Modify the AddExperience method to handle multi-level jumps better
    public bool AddExperience(int amount, bool allowLevelUp = true)
    {
        if (amount <= 0) return false;
        
        // If already at max level, don't add XP
        if (IsMaxLevel)
        {
            return false;
        }
        ...
        int oldLevel = GetLevelFromExperience(_experience);
        
        // Store the old experience value for UI animations
        int oldExperience = _experience;
        ...
        // Perform level up logic if allowed
        if (allowLevelUp && oldLevel != newLevel)
        {
            // Handle multi-level jumps properly
            HandleMultiLevelJump(oldLevel, newLevel);
        }
        
        // Invoke the experience changed event
        OnExperienceChanged?.Invoke(_experience);
        
        return true;
    }

    // Add this new method to handle multi-level jumps
    private void HandleMultiLevelJump(int oldLevel, int newLevel)
    {
        Debug.Log($"Processing level jump from {oldLevel} to {newLevel}");
        
        // Sanity check
        if (oldLevel >= newLevel) return;
        
        // Update the current level first
        CurrentLevel = oldLevel + 1; // Start with the first level gained
        
        // Check if we're loading - if so, just set the final level and return
        if (isLoadingData)
        {
        ...
        // Special handling for first level up with animation
        ProcessLevelUp(CurrentLevel);
        
        // Check if the first level up needs area animation
        if (CurrentLevel <= 55) // Only check for valid area unlock levels
        {
            bool hasAreaUnlock = CheckForAreaUnlock(CurrentLevel);
            if (hasAreaUnlock)
            {
                // Trigger area animation
                CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
                if (cameraAnimations != null)
                {
                    Debug.Log($"Triggering area animation for level {CurrentLevel}");
                    cameraAnimations.ShowAreaByLevel(CurrentLevel);
                    
                    // Schedule the remaining level-ups after animation
                    if (CurrentLevel < newLevel)
                    {
                        StartCoroutine(ContinueLevelUpsAfterAnimation(CurrentLevel + 1, newLevel));
                    }
                    return; // Exit now, let the coroutine handle the rest
                }
            }
        }
        
        // If we have more levels to go, process them all
        if (CurrentLevel < newLevel)
        {
            // Process each additional level up
            for (int level = CurrentLevel + 1; level <= newLevel; level++)
            {
                ...
                if (level <= 55) // Only check for valid area unlock levels
                {
                    ...
                    if (hasAreaUnlock)
                    {
                        ...
                        if (cameraAnimations != null)
                        {
                            Debug.Log($"Triggering area animation for level {level}");
                            cameraAnimations.ShowAreaByLevel(level);
                            
                            // Schedule any remaining level-ups after animation
                            if (level < newLevel)
                            {
                                StartCoroutine(ContinueLevelUpsAfterAnimation(level + 1, newLevel));
                            }
                            return; // Exit now, let the coroutine handle the rest
                        }
                    }
        ...
        // Final notification
        Debug.Log($"Level up sequence complete. Current level: {CurrentLevel}");
    }

    // Modify ProcessLevelUp to explicitly handle attribute points
    private void ProcessLevelUp(int level)
    {
        Debug.Log($"Processing level up for level {level}");
        
        // Add attribute points directly here as a backup
        // This ensures points are added even if the attribute system didn't catch the event
        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
        if (playerAttributes != null)
        {
            // Add the points for this level
            playerAttributes.AddAttributePoints(2);
            Debug.Log($"Added 2 attribute points directly for level {level}");
        }
        
        // Trigger level up event to notify other systems
        OnLevelUp?.Invoke(level);
        
        // Handle border unlocks
        DisableBordersForLevel(level);
    }

    // Add this method to ensure borders are properly disabled for each level
    private void DisableBordersForLevel(int level)
    {
        ...
        foreach (var border in borders)
        {
            ...
            if (requiredLevel > 0 && requiredLevel == level)
            {
                // This border should be disabled at this level
                border.SetVisualizationState(false);
                Debug.Log($"Disabled border {border.gameObject.name} at level {level}");
            }
        }
    }
    ...
    // Add this method to check if a level has an area unlock
    ...
    // Add this method to continue level-ups after camera animation
    private IEnumerator ContinueLevelUpsAfterAnimation(int startLevel, int endLevel)
    {
        // Wait for the camera animation to complete
        CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
        if (cameraAnimations != null)
        {
            // Wait until animation is no longer in progress
            while (cameraAnimations.IsAnimationInProgress())
            {
                yield return null;
            }
        ...
        Debug.Log($"Continuing level-ups from {startLevel} to {endLevel} after animation");
        ...
        for (int level = startLevel; level <= endLevel; level++)
        {
            ...
            // If this level has an area unlock, wait for its animation to complete
            if (hasAreaUnlock)
            {
                // Trigger area animation
                if (cameraAnimations != null)
                ...
                // Check if we're at the end level
                if (level < endLevel)
                {
                    // Start a new coroutine for the remaining levels
                    StartCoroutine(ContinueLevelUpsAfterAnimation(level + 1, endLevel));
        ...
        // Final notification
        Debug.Log($"Multi-level up sequence complete. Final level: {CurrentLevel}");
    }
} 