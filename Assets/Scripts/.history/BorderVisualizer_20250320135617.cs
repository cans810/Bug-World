...
public class BorderVisualizer : MonoBehaviour
{
    ...
    // Add this property to BorderVisualizer class
    public bool ShowInGame
    {
        get { return showInGame; }
        set 
        {
            if (showInGame != value)
            {
                ...
                // If we're turning visibility off, destroy the current visualization
                if (!showInGame && visualSphere != null)
                {
                    Destroy(visualSphere);
                    visualSphere = null;
                    radiusLine = null;
                }
                // If we're turning visibility on, create the visualization
                else if (showInGame && visualSphere == null)
                {
                    CreateVisualSphere();
                }
            }
