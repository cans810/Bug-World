...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private bool wanderWhenPlayerIdle = true;

    private bool isIdleWandering = false;
    
    // Add this field to track the maximum distance allies can wander from player
    [SerializeField] private float maxIdleWanderDistance = 4.0f; // Should be less than followDistance
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // If player is not moving and we're close enough, do idle wandering
        if (!isPlayerMoving && distanceToPlayer <= followDistance * 1.5f)
        {
            // If wandering when player is idle is enabled
            if (wanderWhenPlayerIdle && wanderingBehavior != null)
            {
                // Enable the wandering behavior with a smaller radius
                if (!isIdleWandering)
                {
                    isIdleWandering = true;
                    
                    // Set smaller wander distances for idle wandering
                    wanderingBehavior.SetWanderDistances(1.5f, 3.0f);
                    
                    // Enable the wandering behavior
                    wanderingBehavior.enabled = true;
                    wanderingBehavior.SetWanderingEnabled(true);
                    
                    // Start coroutine to check distance while wandering
                    StartCoroutine(MonitorIdleWanderingDistance());
                }
                
                // Let the AIWandering component handle the movement
            }
            else
            {
                // Original behavior - just stop
                isMoving = false;
                UpdateAnimation(false);
        ...
        {
            // Reset idle wandering state when player moves or we're too far
    ...
    // Modify the MonitorIdleWanderingDistance coroutine to make allies turn back when they reach the limit
    private IEnumerator MonitorIdleWanderingDistance()
    {
        while (isIdleWandering && playerTransform != null)
        {
            // Check distance to player
            float currentDistance = Vector3.Distance(transform.position, playerTransform.position);
            
            // If we've wandered too far (close to or beyond the follow distance)
            if (currentDistance > maxIdleWanderDistance)
            {
                // Temporarily disable wandering
                wanderingBehavior.SetWanderingEnabled(false);
                
                // Force the ally to move back toward the player
                isMoving = true;
                UpdateAnimation(true);
                ...
                // Move toward player for a short time
                float moveBackTime = 1.0f;
                float timer = 0f;
                
                while (timer < moveBackTime)
                {
                    // Move toward player
                    transform.position += transform.forward * livingEntity.moveSpeed * Time.deltaTime;
                    
                    timer += Time.deltaTime;
                    yield return null;
                }
                
                // After moving back toward player, resume wandering
                wanderingBehavior.SetWanderingEnabled(true);
            }
            
            yield return new WaitForSeconds(0.2f); // Check more frequently
        }
    }
}
