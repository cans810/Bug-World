...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private Mesh CreateStadiumMesh(float radius, float height, bool isXAxis)
    {
        ...
        {
            // Adjust angle to go from -90° to 90° (-π/2 to π/2)
            ...
            if (isXAxis)
                vertices[vertexIndex++] = semicircle1Center + new Vector3(x, 0, z);
            else
                vertices[vertexIndex++] = semicircle1Center + new Vector3(x, 0, z);
        }
        
        // Second semicircle (facing outward, 90° to 270°)
        ...
        for (int i = 0; i < circleSegments; i++)
        {
            // Adjust angle to go from 90° to 270° (π/2 to 3π/2)
            float angle = Mathf.PI * (0.5f + i / (float)(circleSegments - 1));
            float x = Mathf.Cos(angle) * radius;
            float z = Mathf.Sin(angle) * radius;
            ...
            // Adjust based on axis
            if (isXAxis)
                vertices[vertexIndex++] = semicircle2Center + new Vector3(x, 0, z);
            else
                vertices[vertexIndex++] = semicircle2Center + new Vector3(x, 0, z);
        }
        
        // Rectangle corner vertices to connect the semicircles
        if (isXAxis)
        {
