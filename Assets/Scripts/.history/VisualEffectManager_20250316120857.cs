...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private float xpSymbolSpawnOffsetX = 0f;
    [SerializeField] private float xpSymbolSpawnOffsetY = 100;
    [SerializeField] private float xpTargetOffsetX = -400f;
    [SerializeField] private float xpTargetOffsetY = 300f;
    [SerializeField] private float xpFlyDuration = 1.0f;
    [SerializeField] private float xpSpreadRadius = 50f;
    [SerializeField] private string xpGainedSoundName = "XPGained";

    [Header("XP Sound Settings")]
    [SerializeField] private AudioClip xpGainedSoundClip; // Direct reference to the sound clip
    [SerializeField] private float xpSoundVolume = 0.7f;  // Higher volume to ensure it's audible
    [SerializeField] private float xpSoundDelay = 0.05f;  // Small delay between sounds to prevent overlap
    ...
    public void PlayXPGainEffect(Vector3 nestPosition, int xpAmount)
    {
        ...
        StartCoroutine(SpawnXPSymbols(nestPosition, symbolCount));
    }

    private IEnumerator SpawnXPSymbols(Vector3 nestPosition, int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            StartCoroutine(AnimateXPSymbolToScreen(symbol, screenTopLeft));
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Create a curved path with midpoint above - use lower height like chitin animation
        Vector3 midPoint = Vector3.Lerp(startPos, targetPos, 0.5f);
        midPoint += Vector3.up * Random.Range(0.5f, 1.5f); // Lower arc to match chitin height
        ...
        while (elapsed < duration)
        {
            ...
            // Scale down as it approaches destination (instead of fading)
            if (normalizedTime > 0.5f)
            {
                float scaleT = (normalizedTime - 0.5f) / 0.5f; // Normalize to 0-1 range for second half
        ...
        // Play XP gained sound directly with AudioSource.PlayClipAtPoint for reliability
        if (xpGainedSoundClip != null)
        {
            // Use PlayClipAtPoint which creates a temporary AudioSource - more reliable
            AudioSource.PlayClipAtPoint(xpGainedSoundClip, Camera.main.transform.position, xpSoundVolume);
            Debug.Log("Playing XP sound directly with PlayClipAtPoint");
        }
        // Also try the SoundEffectManager as a backup
        else if (SoundEffectManager.Instance != null)
        {
            // Try to play through SoundEffectManager with increased priority
            AudioSource source = SoundEffectManager.Instance.PlaySound(xpGainedSoundName, Vector3.zero, false);
            
            // If we got a source back, increase its volume and priority
            if (source != null)
            {
                source.volume = 0.7f; // Increase volume
                source.priority = 0;  // Highest priority (0 is highest)
                Debug.Log($"Playing XP sound through SoundEffectManager: {xpGainedSoundName}");
            }
            else
            {
                Debug.LogWarning($"Failed to play sound {xpGainedSoundName} - sound not found or all audio sources busy");
            }
        }
        else
        {
            Debug.LogError("Cannot play XP sound - no sound clip assigned and SoundEffectManager not found");
        }
        
        // Add a small delay to prevent sound overlap
        yield return new WaitForSeconds(xpSoundDelay);
