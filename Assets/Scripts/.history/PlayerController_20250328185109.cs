...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            ...
            // Determine direction back into the BigBorder
            Vector3 borderCenter = other.bounds.center;
            Vector3 directionToCenter = (borderCenter - transform.position).normalized;
            
            // Apply a strong force back inside
            transform.position = other.ClosestPoint(transform.position) + directionToCenter * 2.0f;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            {
                Debug.LogError($"ENFORCING BORDER: Player (Level {playerLevel}) inside restricted area {borderName} (Level {requiredLevel})");
                
                // Calculate direction out of restricted area
                Vector3 playerToColliderCenter = transform.position - other.bounds.center;
                playerToColliderCenter.y = 0; // Keep it horizontal
                borderNormal = playerToColliderCenter.normalized;
                
                // Find the exit point and force-move player there
                Vector3 exitPoint = other.ClosestPoint(transform.position + borderNormal * 5.0f);
                transform.position = exitPoint + borderNormal * 2.0f;
                ...
                // Show level requirement message
                if (uiHelper != null && Time.time >= lastBorderMessageTime + 2.0f)
                {
                    uiHelper.ShowWarningText($"This area requires level {requiredLevel}!");
                    HapticFeedback.MediumFeedback();
        ...
        {
            // Player is inside BigBorder - just ensure we're not restricting movement
            isAtMapBorder = false;
            borderNormal = Vector3.zero;
        }
    }
