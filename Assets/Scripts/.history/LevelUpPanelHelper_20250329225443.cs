...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    // Add these fields to help track area unlock levels
    [Header("Area Unlock Settings")]
    [SerializeField] private List<AreaUnlockData> areaUnlocks = new List<AreaUnlockData>();

    // Add this class to store area unlock data
    [System.Serializable]
    public class AreaUnlockData
    {
        public string areaName;
        public int requiredLevel;
        public GameObject areaGameObject; // Reference to the area in the scene
    }
    ...
    public void InstantiateRewards()
    {
        ...
        foreach (Transform child in LowerPanelRewards.transform)
        {
        ...
        // Check if we should add attribute points reward
        if (attributePointRewardPrefab != null && currentLevel % 5 == 0)
        {
            attributePointReward = Instantiate(attributePointRewardPrefab, LowerPanelRewards.transform);
            RectTransform rectTransform = attributePointReward.GetComponent<RectTransform>();
            rectTransform.anchoredPosition = new Vector2(0, yOffset);
            yOffset -= rectTransform.rect.height;
            pendingRewards++;
        }

        // Check if we should add chitin capacity reward
        if (chitinCapacityRewardPrefab != null)
        {
            chitinCapacityReward = Instantiate(chitinCapacityRewardPrefab, LowerPanelRewards.transform);
            RectTransform rectTransform = chitinCapacityReward.GetComponent<RectTransform>();
            rectTransform.anchoredPosition = new Vector2(0, yOffset);
            yOffset -= rectTransform.rect.height;
            pendingRewards++;
        }

        // Check if we should add crumb capacity reward
        if (crumbCapacityRewardPrefab != null && currentLevel % 10 == 0)
        {
            crumbCapacityReward = Instantiate(crumbCapacityRewardPrefab, LowerPanelRewards.transform);
            RectTransform rectTransform = crumbCapacityReward.GetComponent<RectTransform>();
            rectTransform.anchoredPosition = new Vector2(0, yOffset);
            yOffset -= rectTransform.rect.height;
            pendingRewards++;
        }

        // Check if we unlock any new areas at this level
        foreach (AreaUnlockData areaData in areaUnlocks)
        {
            if (areaData.requiredLevel == currentLevel)
            {
                Debug.Log($"Found area unlock at level {currentLevel}: {areaData.areaName}");
                ...
                if (newAreaUnlockedRewardPrefab != null)
                {
                    ...
                    RectTransform rectTransform = newAreaUnlockedReward.GetComponent<RectTransform>();
                    rectTransform.anchoredPosition = new Vector2(0, yOffset);
                    yOffset -= rectTransform.rect.height;
                    
                    // Store the area data for when the claim button is clicked
                    areaTarget = areaData.areaGameObject;
                    areaLevel = areaData.requiredLevel;
                    areaName = areaData.areaName;
                    
                    // Update the text on the reward
                    TMPro.TextMeshProUGUI areaText = newAreaUnlockedReward.transform.Find("RewardText").GetComponent<TMPro.TextMeshProUGUI>();
                    if (areaText != null)
                    {
                        areaText.text = $"New Area: {areaData.areaName}";
                    }
                    
                    // Ensure the new area is active
                    if (areaData.areaGameObject != null)
                    {
                        areaData.areaGameObject.SetActive(true);
                    }
                    
                    pendingRewards++;
                    break; // Only show one area unlock per level
                }
            }
        }
    }
    ...
    // Add a method to handle "Claim" button click for new area
    public void ClaimNewAreaReward()
    {
        if (areaTarget != null)
        {
            Debug.Log($"Claiming new area: {areaName}");
            
            // Activate the area if it's not already active
            areaTarget.SetActive(true);
            
            // If we have a camera animations reference, pan to the new area
            if (cameraAnimations != null)
            {
                cameraAnimations.PanToTarget(areaTarget.transform, 2f);
            }
            
            // Add any other unlock effects you want
        }
