...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator AnimateCameraToArea(Transform areaTarget, int level = 0, string areaName = "")
    {
        ...
        {
            Debug.Log($"CameraAnimations: Firing animation completed event for area {areaName} (level {level})");
            OnCameraAnimationCompleted(areaTarget.gameObject, level, areaName);
            Debug.Log("CameraAnimations: Event fired");
        }
        else
        {
            Debug.LogWarning($"CameraAnimations: Could not fire event. Event has listeners: {OnCameraAnimationCompleted != null}, areaTarget: {areaTarget != null}");
        }
    }
