...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        if (isTooCloseToNest)
        {
            ...
            // If we're chasing, stop and return to wandering
            if (isChasing)
            {
    ...
    // Modify ChaseAndAttackTarget to respect nest boundaries and turn back
    private void ChaseAndAttackTarget()
    {
            ...
            !IsPositionTooCloseToNest(transform.position))
        {
            ...
            // After a short delay, give up and return to wandering
            StartCoroutine(DelayedReturnToWandering(2f));
            return;
        }
    ...
    // Add a coroutine to delay returning to wandering
    private IEnumerator DelayedReturnToWandering(float delay)
    {
        yield return new WaitForSeconds(delay);
        
        // Only return to wandering if we're still enabled and not already wandering
        if (enabled && isChasing)
        {
            ReturnToWandering();
        }
    }
