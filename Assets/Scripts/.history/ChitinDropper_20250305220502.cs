...
public class ChitinDropper : MonoBehaviour
{
    ...
    private IEnumerator SimpleChitinAnimation(GameObject chitinObject)
    {
        ...
        // Calculate a random landing position very close to the start
        float angle = Random.Range(0, 360) * Mathf.Deg2Rad;
        Vector3 offset = new Vector3(
            Mathf.Cos(angle) * Random.Range(0.05f, 0.2f),
            0,
            Mathf.Sin(angle) * Random.Range(0.05f, 0.2f)
        );
