...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning("Player hitbox not found. Auto-attack functionality will not work.");
        }
    }
    ...
    private void HandleEnemyDetected(LivingEntity enemy)
    {
        ...
        // Only attack enemies, not allies or other entities
    ...
    private void HandleEnemyLost(LivingEntity enemy)
    {
        ...
        if (livingEntity == null || livingEntity.GetTargetsInRange().Count == 0)
        {
            // Stop auto-attack coroutine if it's running
            if (autoAttackCoroutine != null)
            {
    ...
    private IEnumerator AutoAttackCoroutine()
    {
        while (enabled && livingEntity != null && !livingEntity.IsDead && autoAttackEnabled)
        {
            ...
            List<LivingEntity> targetsInRange = livingEntity.GetTargetsInRange();
            if (targetsInRange.Count > 0)
            {
                // Try to attack
                OnAttackButtonPressed();
                
                // Wait for the actual attack cooldown from the LivingEntity
                // This respects the player's agility attribute
                yield return new WaitForSeconds(livingEntity.attackCooldown);
            }
            else
            {
    ...
    public void ToggleAutoAttack(bool enabled)
    {
        ...
        // If enabled and there are enemies in range, start auto-attacking
                 ...
                 livingEntity.GetTargetsInRange().Count > 0 && 
                 autoAttackCoroutine == null)
        {
            autoAttackCoroutine = StartCoroutine(AutoAttackCoroutine());
        }
