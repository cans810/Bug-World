...
public class MissionManager : MonoBehaviour
{
    ...
    // Add these dictionaries for faster lookups
    private Dictionary<string, List<MissionData>> activeMissionsByEntityType = new Dictionary<string, List<MissionData>>();
    private Dictionary<string, List<MissionData>> activeMissionsByItemType = new Dictionary<string, List<MissionData>>();
    ...
    public void ActivateMission(string missionId)
    {
        ...
        // Call RebuildMissionCaches() whenever missions change:
        // 1. When activating a mission
        // 2. When completing a mission
        // 3. After loading mission progress
        RebuildMissionCaches();
    }
    ...
    {
        // Cache the RectTransform and Canvas lookup
        RectTransform rectTransform = panel.GetComponent<RectTransform>();
        Canvas canvas = panel.GetComponentInParent<Canvas>();
        
        if (rectTransform != null && canvas != null)
        {
            // Cache screen position calculation - only do this once
            Vector3 screenPosition = RectTransformUtility.WorldToScreenPoint(canvas.worldCamera, rectTransform.position);
            Ray ray = Camera.main.ScreenPointToRay(screenPosition);
            Vector3 worldPosition = ray.GetPoint(10f);
            
            // Wait before animations
            yield return new WaitForSeconds(0.5f);
            
            // Use a single check for both rewards to minimize code branching
    ...
    public void OnEntityKilled(string entityType)
    {
        ...
        string lowerEntityType = entityType.ToLower();
        bool foundMatchingMission = false;
        
        // Check if we have cached missions for this entity type
        if (activeMissionsByEntityType.TryGetValue(lowerEntityType, out List<MissionData> missions))
        {
            foreach (MissionData mission in missions)
            {
    ...
    public void SaveMissionProgress(GameData gameData)
    {
        ...
        foreach (var entry in allMissions)
        {
            ...
            if (mission.isActive || mission.isCompleted)
            {
                ...
                Debug.Log($"Saved mission: {mission.id}, Active: {mission.isActive}, Completed: {mission.isCompleted}, Progress: {mission.currentAmount}/{mission.targetAmount}");
            }
        }
    }
    ...
    // Add a method to rebuild the mission lookup caches
    private void RebuildMissionCaches()
    {
        activeMissionsByEntityType.Clear();
        activeMissionsByItemType.Clear();
        ...
        foreach (var entry in allMissions)
        {
            ...
            if (mission.isActive && !mission.isCompleted)
            {
                string targetType = mission.targetEntityType.ToLower();
                
                if (mission.missionType == MissionType.Kill)
                {
                    if (!activeMissionsByEntityType.ContainsKey(targetType))
                    {
                        activeMissionsByEntityType[targetType] = new List<MissionData>();
                    }
                    activeMissionsByEntityType[targetType].Add(mission);
                }
                else if (mission.missionType == MissionType.Collect)
                {
                    if (!activeMissionsByItemType.ContainsKey(targetType))
                    {
                        activeMissionsByItemType[targetType] = new List<MissionData>();
                    }
                    activeMissionsByItemType[targetType].Add(mission);
                }
            }
        }
    }
