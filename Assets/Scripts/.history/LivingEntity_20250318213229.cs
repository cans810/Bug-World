...
public class LivingEntity : MonoBehaviour
{
    ...
    [SerializeField] private string entityType = ""; // The type of entity (ant, fly, etc.)
    public string EntityType => string.IsNullOrEmpty(entityType) ? gameObject.name.Replace("(Clone)", "").Replace("Enemy_", "") : entityType;
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // Try to notify about discovery
        if (gameObject.layer == LayerMask.NameToLayer("Insects") && !hasBeenDiscovered)
        {
            // Find the discoveredEntitiesController
            DiscoveredEntitiesController discoveryController = FindObjectOfType<DiscoveredEntitiesController>();
            if (discoveryController != null)
            {
                // Try to discover this entity based on its type
                discoveryController.DiscoverEntityOnKill(EntityType);
                hasBeenDiscovered = true;
                Debug.Log($"Entity {EntityType} death triggered discovery check");
            }
        }
    ...
    // Optional - Add this to also expose this as a public property that other systems can read
