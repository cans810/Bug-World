...
public class AllyAI : MonoBehaviour
{
    ...
    {
        // Don't pick up loot if we're already carrying something, in combat,
        // chasing an enemy, or on cooldown
        if (carriedLoot != null || isInCombat || isChasing || Time.time < nextLootCollectionTime)
            return;
        
        // Check if we're close enough to pick it up
        float distance = Vector3.Distance(transform.position, loot.position);
        if (distance <= lootDetectionRadius)
        {
            // Pick up the loot
            PickUpLoot(loot);
        }
