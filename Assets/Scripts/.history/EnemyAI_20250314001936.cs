...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        else if (isOutsideBoundary || isOutsideEntityBoundary)
        {
            ...
            if (isOutsideEntityBoundary && hasBoundary)
            {
                ...
                // Move along the boundary circle
                Vector3 tangent = Vector3.Cross(Vector3.up, fromOrigin);
                Vector3 redirectedDirection = tangent;
    ...
    // Add this method to check if a position violates the entity generator boundary
    private bool CheckEntityBoundaryViolation(Vector3 position)
    {
        if (!hasBoundary)
            return false;
        
        // Check if the position is outside the circular boundary
