...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public void BuyProduct(string productId)
    {
        ...
        if (testMode)
        {
            // Simulate successful purchase directly
            if (logTestPurchases)
            {
                Debug.Log($"[TEST MODE] Simulating successful purchase for product: {productId}");
            }
            
            // Instead of starting the real purchase process, directly call the success handler
    ...
    // Add a method to handle successful purchases (whether real or simulated)
    private void HandleSuccessfulPurchase(string productId)
    {
        // Process the purchase and provide the content to the player
        
        // Log the purchase (real or simulated)
        ...
        if (uiHelper != null)
        {
            uiHelper.ShowInformText("Purchase successful!");
        }
        
        // Grant the purchased content based on product ID
        switch (productId)
        {
            case "coins_small":
                AddCoins(100);
    ...
    // Add these fields to the top of the MarketManager class
    [Header("Debug Settings")]
    [SerializeField] private bool testMode = false;
    [SerializeField] private bool logTestPurchases = true;
}
