...
public class EnemyAI : MonoBehaviour
{
    ...
    [SerializeField] private bool showDebugInfo = false; // Whether to show debug info in console
    ...
    private void HandleDeath()
    {
        ...
        // Wait for 2 seconds before sinking - allows death animation to play
        StartCoroutine(DelaySink(2.0f));
    }
    
    // New coroutine to delay the sink effect
    private IEnumerator DelaySink(float delay)
    {
        // Wait for the specified delay
        yield return new WaitForSeconds(delay);
        
        // Then start the sink effect
        SinkOnDeath();
    }
    ...
    protected virtual void AttackIfInRange()
    {
        ...
        if (currentTarget != null && !currentTarget.IsDead)
        {
            ...
            if (!inSameBorder)
            {
                // Debug log without conditional check
    ...
    // Modified method to handle player exit - preserve rotation
    private void HandlePlayerExitHitbox(LivingEntity playerEntity)
    {
        ...
        // Preserve current rotation by passing it to the wandering system
        Quaternion currentRotation = transform.rotation;
        
        // We'll wait a moment before returning to normal behavior
        StartCoroutine(ReturnToWanderingAfterDelay(0.5f, currentRotation));
    }

    // Modified coroutine to preserve rotation when returning to wandering
    private IEnumerator ReturnToWanderingAfterDelay(float delay, Quaternion preservedRotation)
    {
        ...
        if (!isPlayerInHitbox && !livingEntity.IsDead)
        {
            ...
            if (wanderingBehavior != null)
            {
                ...
                // Use the current forward direction based on preserved rotation
                wanderingBehavior.ForceStartWithDirection(preservedRotation * Vector3.forward);
            }
        }
    }

    // Modified method to handle player enter hitbox
    protected virtual void HandlePlayerEnterHitbox(LivingEntity playerEntity)
    {
        ...
        // Cancel ALL coroutines on this object EXCEPT rotation handling
        StopAllCoroutines();
        
        // IMPORTANT: Do NOT lock rotation - this allows smooth rotation
        if (livingEntity != null)
        {
            livingEntity.SetRotationLocked(false);
        }
    ...
    // Simplified coroutine to make enemy face the player smoothly
    public IEnumerator FacePlayerWhileInHitbox(LivingEntity playerEntity)
    {
        // Define rotation speed - higher value = faster rotation
        ...
        while (isPlayerInHitbox && !livingEntity.IsDead && playerEntity != null && !playerEntity.IsDead)
        {
            ...
            {
                // We'll use the LivingEntity's rotation method to ensure compatibility with other systems
                livingEntity.RotateTowards(directionToPlayer, rotationSpeed);
            }
    ...
    protected bool CheckSameBorderAs(GameObject other)
    {
        if (other == null) return false;
        
        // Check if the entity has a PlayerAttributes component (for player)
        PlayerAttributes playerAttrib = other.GetComponent<PlayerAttributes>();
    ...
    // Add this method to the EnemyAI class
    ...
    // Method to disable all outline components
    private void DisableAllOutlines()
    {
        // Find and disable EntityOutline component
        EntityOutline entityOutline = GetComponent<EntityOutline>();
    ...
    // Coroutine to sink the enemy into the ground
    private IEnumerator SinkIntoGround()
    {
        Vector3 startPosition = transform.position;
        float sinkDuration = 2.0f; // 2 seconds of sinking
        float sinkDistance = 1.5f; // How far to sink
        float elapsedTime = 0f;
        
        // Get all colliders and disable them to prevent collision during sinking
        ...
        // Sink gradually over time
        while (elapsedTime < sinkDuration)
        {
            elapsedTime += Time.deltaTime;
            float sinkProgress = elapsedTime / sinkDuration;
            
            // Use easeIn curve for smoother motion (start slow, accelerate)
            float easedProgress = Mathf.Pow(sinkProgress, 2);
            
            // Calculate new position - only changing Y coordinate
            Vector3 newPosition = startPosition;
            newPosition.y = startPosition.y - (sinkDistance * easedProgress);
        ...
        finalPosition.y = startPosition.y - sinkDistance;
        transform.position = finalPosition;
        
        Debug.Log($"{gameObject.name} has sunk into the ground");
    }
} 