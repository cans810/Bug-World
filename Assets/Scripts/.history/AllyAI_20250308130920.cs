...
public class AllyAI : MonoBehaviour
{
    ...
    private void CarryLootToBase()
    {
        ...
        // When carrying loot, use a special animation state if available
        // Or just update the walking animation without affecting movement
        if (animController != null)
        {
            // Consider using a special "carrying" animation state
            animController.SetWalking(true);
        }
        
        // Control movement directly without relying on other systems
        // Calculate exact distance to move this frame
        ...
        Vector3 targetPosition = originalPosition + moveDirection * moveAmount;
        
        // Apply boundary check if needed
        ...
        // Directly set position - override any other movement systems
        transform.position = targetPosition;
        
        // Since we directly manipulated position, ensure the animation doesn't add extra movement
        isMoving = false; // This might be used by other systems to determine if they should move the character
    }
