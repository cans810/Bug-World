...
public class InsectIncubator : MonoBehaviour
{
    ...
    public void CreateInsectEgg(string entityType = "ant")
    {
        if (insectEggPrefab == null)
        {
            Debug.LogError("Insect Egg prefab not assigned to InsectIncubator!");  
            return;
        }
        ...
        // Instantiate the egg
        GameObject newEgg = Instantiate(insectEggPrefab, eggPos.position, Quaternion.identity);
        
        // Set the entity type on the egg controller
        AllyEggController eggController = newEgg.GetComponent<AllyEggController>();
        if (eggController != null)
        {
            eggController.SetEntityTypeToHatch(entityType);
        }
        
        // Make it a child of this incubator
        ...
        Debug.Log($"Insect egg created in incubator for entity type: {entityType}");
    }
