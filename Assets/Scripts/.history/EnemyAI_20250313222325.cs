...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modify FaceTarget method to prevent rotation when standing after kill
    ...
    // Modify StandStillAfterKill method to completely freeze the entity
    private IEnumerator StandStillAfterKill()
    {
        ...
        // Disable wandering while standing still
        if (wanderingBehavior != null)
            wanderingBehavior.SetWanderingEnabled(false);
        ...
        // Save the current rotation to prevent any movement/spinning
        ...
        // Wait period - during this time, we'll continuously enforce the frozen rotation
        float timer = 0;
        float waitDuration = 3f;
        while (timer < waitDuration)
        {
            // Force the rotation to stay the same
