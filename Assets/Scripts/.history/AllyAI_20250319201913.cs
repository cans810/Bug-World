...
public class AllyAI : MonoBehaviour
{
    ...
    private void AttackTarget()
    {
        ...
        {
            // Target is gone, go back to following or idle
            currentState = wasPlayerMoving ? AIState.Following : AIState.Idle;
            return;
        }
        ...
        // If too far from player, stop attacking and return
        if (distanceToPlayer > maxDistanceFromPlayer)
        {
            currentState = AIState.Returning;
            if (showDebugInfo)
                Debug.Log($"Ally {gameObject.name}: Too far from player, returning");
            return;
        }
        ...
        if (distanceToTarget <= attackRange)
        {
            ...
            {
                // Face target
                Vector3 dirToTarget = currentTarget.transform.position - transform.position;
                dirToTarget.y = 0;
        ...
        else
        {
            ...
            // Move using LivingEntity
            if (livingEntity != null)
            {
                livingEntity.MoveInDirection(directionToTarget, 1.0f);
                hasAppliedMovementThisFrame = true;
            }
            else
            {
    ...
    private void ReturnToPlayer()
    {
        ...
        Vector3 returnPosition = CalculateFormationPosition();
        
        // Check if we've reached the player's vicinity
        float distanceToPlayer = Vector3.Distance(transform.position, playerTransform.position);
        
        if (distanceToPlayer <= followDistance * 1.2f)
        {
            // We're close enough to player, resume following or idle
            currentState = wasPlayerMoving ? AIState.Following : AIState.Idle;
            
            if (showDebugInfo)
                Debug.Log($"Ally {gameObject.name}: Returned to player");
                
            return;
        }
        
        // Move towards the return position
