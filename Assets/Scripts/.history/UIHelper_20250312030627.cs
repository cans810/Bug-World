...
public class UIHelper : MonoBehaviour
{
    ...
    // Completely rewritten animation method
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        if (mainCanvas == null)
        {
            Debug.LogError("[CRITICAL] Cannot find parent Canvas for UI animations!");
            yield break;
        }
        
        // Get starting position - use screen center if text is not available
        Vector3 startPos;
        if (chitinCountText != null)
        {
        ...
        else
        {
            ...
            startPos = new Vector3(Screen.width / 2, Screen.height / 2, 0);
            Debug.Log($"[CRITICAL] Using screen center position: {startPos}");
        }
        ...
        if (nestObject == null)
        {
            Debug.LogError("[CRITICAL] Cannot find Player Nest 1!");
            yield break;
        }
        
        // Get nest screen position for debugging
        Vector3 nestScreenPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
        Debug.Log($"[CRITICAL] Nest screen position: {nestScreenPos}");
        
        // Create a dedicated container for our icons to ensure proper hierarchy
        GameObject iconContainer = new GameObject("ChitinIconContainer");
        iconContainer.transform.SetParent(mainCanvas.transform, false);
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            ...
            try {
                // Create icon directly as a child of our container
                icon = Instantiate(chitinIconPrefab, iconContainer.transform);
                
                // Ensure the icon is visible
                Image iconImage = icon.GetComponent<Image>();
                if (iconImage != null)
                {
                    iconImage.color = Color.white; // Make sure it's fully opaque
                    iconImage.raycastTarget = false; // Disable raycast to prevent UI interaction issues
                    
                    // Make sure sprite is assigned
                    if (iconImage.sprite == null && chitinIconPrefab.GetComponent<Image>().sprite != null)
                    {
                        iconImage.sprite = chitinIconPrefab.GetComponent<Image>().sprite;
                    }
                }
                else
                {
                    // If no image component, add one
                    iconImage = icon.AddComponent<Image>();
                    iconImage.color = Color.white;
                    iconImage.raycastTarget = false;
                    
                    // Try to get a default sprite if needed
                    if (chitinIconPrefab.GetComponent<Image>() != null)
                    {
                        iconImage.sprite = chitinIconPrefab.GetComponent<Image>().sprite;
                    }
                }
                ...
                if (iconRect != null)
                {
                    iconRect.sizeDelta = new Vector2(50, 50); // Make it a good size
                }
                ...
                allIcons.Add(icon);
                Debug.Log($"[CRITICAL] Created icon {i+1} of {chitinCount} at position {icon.transform.position} on UI layer");
            }
            catch (System.Exception e) {
                Debug.LogError($"[CRITICAL] Error creating chitin icon: {e.Message}");
                if (icon != null) Destroy(icon);
            }
        }
        
        Debug.Log($"[CRITICAL] Created all {allIcons.Count} icons at once");
        
        // Wait a moment to ensure icons are rendered
        yield return new WaitForSeconds(0.5f);
        
        // Now animate each icon sequentially with longer duration
        for (int i = 0; i < allIcons.Count; i++)
        {
            GameObject icon = allIcons[i];
            ...
            // Move directly to nest with longer duration
            float duration = 1.0f;
            float timer = 0f;
            Vector3 iconStartPos = Vector3.zero;
            Vector3 originalScale = Vector3.one;
            Vector3 targetPos = Vector3.zero;
            
            try {
                iconStartPos = icon.transform.position;
                originalScale = icon.transform.localScale;
                
                Debug.Log($"[CRITICAL] Icon {i+1} starting position: {iconStartPos}");
                
                // Store the target position
                targetPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                targetPos.z = 0;
                
                Debug.Log($"[CRITICAL] Icon {i+1} target position: {targetPos}");
            }
            catch (System.Exception e) {
                Debug.LogError($"[CRITICAL] Error setting up animation: {e.Message}");
                if (icon != null) Destroy(icon);
                continue;
            }
            
            // Animation loop outside of try-catch
            while (timer < duration)
            {
                ...
                try {
                    ...
                    Vector3 currentNestPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                    currentNestPos.z = 0;
                    
                    // Calculate new position with arc
                    Vector3 newPos = Vector3.Lerp(iconStartPos, currentNestPos, easedT);
                    newPos.y += 100f * Mathf.Sin(t * Mathf.PI); // Higher arc
                    
                    // Update position
                    icon.transform.position = newPos;
                    
                    // Scale down gradually
                    icon.transform.localScale = Vector3.Lerp(originalScale, Vector3.zero, t * 0.8f);
                    
                    // Log position every few frames for debugging
                    if (Mathf.Approximately(t % 0.25f, 0))
                    {
                        Debug.Log($"[CRITICAL] Icon {i+1} current position: {newPos}, progress: {t:F2}");
                    }
                }
                catch (System.Exception e) {
                    Debug.LogError($"[CRITICAL] Error during animation: {e.Message}");
                    if (icon != null) Destroy(icon);
                    break;
                }
            ...
            try {
                if (icon != null)
                {
                    Debug.Log($"[CRITICAL] Completed animation for icon {i+1}");
                    Destroy(icon);
                }
            }
            catch (System.Exception e) {
                Debug.LogError($"[CRITICAL] Error destroying icon: {e.Message}");
            }
        }
        ...
        try {
            ...
            Debug.LogError($"[CRITICAL] Error destroying container: {e.Message}");
        }
        
        Debug.Log("[CRITICAL] Animation sequence completed for all icons");
    }

    // Add a public debug method to test the animation with a specific count
    public void DebugAnimateChitins(int count)
    {
        Debug.Log($"[DEBUG] Directly testing chitin animation with count: {count}");
        StartCoroutine(SpawnAndAnimateChitinIcons(count));
    }
} 
