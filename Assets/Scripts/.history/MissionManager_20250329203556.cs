...
public class MissionManager : MonoBehaviour
{
    ...
    [SerializeField] public Color progressFillColor = new Color(0.2f, 0.5f, 1f, 1f); // Solid blue by default
    [SerializeField] public Color completedFillColor = new Color(0.2f, 0.8f, 0.2f, 1f); // Solid green by default
    ...
    public void GiveRewardsForMission(string missionId, GameObject panel)
    {
        ...
        if (uiEffects != null)
        {
            ...
            if (coinReward > 0)
            {
                // Use our new method that takes the entire panel GameObject
                uiEffects.PlayCoinRewardEffect(panel, coinReward);
            }
            
            // XP reward (existing code)
            if (xpReward > 0 && SoundEffectManager.Instance != null)
            {
                SoundEffectManager.Instance.PlaySound("XPGained");
                Debug.Log($"Mission reward: {mission.xpReward} XP");
            }
        }
        ...
        // Now remove the panel with animation, but with a delay to allow coin animation to complete
    ...
    // Update the delayed rewards coroutine to wait longer for the coin animation
    private IEnumerator DelayedRewards(MissionData mission, int coinReward, int xpReward)
    {
        // Wait longer to let coin animations complete
    ...
    // Also increase the delay for panel removal
    private IEnumerator PlayHideAnimationAndDestroy(GameObject panel)
    {
        // Wait a bit longer to allow coin animations to finish
        yield return new WaitForSeconds(1.0f);
