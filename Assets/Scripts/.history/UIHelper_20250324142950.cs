...
public class UIHelper : MonoBehaviour
{
    ...
    public void WatchRewardedAdButtonForRevivalClicked()
    {
        ...
        // Find the RewardedAdExample component to trigger the ad
        RewardedAdExample adExample = FindObjectOfType<RewardedAdExample>();
        if (adExample != null)
        {
            // Load and show the ad
            adExample.LoadAd();
            
            // Set up a listener to handle successful ad completion
            // Subscribe to the ad completion callback
            StartCoroutine(WaitForAdCompletionAndRevive(adExample));
        }
        else
        {
            Debug.LogError("RewardedAdExample component not found!");
            ShowInformText("Error loading advertisement.");
        }
    }

    private IEnumerator WaitForAdCompletionAndRevive(RewardedAdExample adExample)
    {
        // Give some time for the ad to load
        yield return new WaitForSeconds(1f);
        
        // Show the ad
        adExample.ShowAd();
        
        // Wait for ad to finish (approximate time)
        ...
        if (playerController != null)
        {
            ...
            ShowInformText("You have been revived by watching an ad!");
        }
    }
        ...
        if (playerInventory != null)
        {
            ...
            if (playerInventory.CoinCount >= 150)
            {
                ...
                if (playerController != null)
                {
                    ...
                    // Show success message
                    ShowInformText("You have been revived by spending 150 coins!");
                }
            }
            else
            {
    ...
    public void CloseRevivalAndLooseChitinAndCrumbsClicked()
    {
        ...
        if (playerInventory != null)
        {
            ...
            if (playerController != null)
            {
                ...
                // Show message about resource loss
                ShowInformText($"You lost {chitinLoss} chitin and {crumbLoss} crumbs but have been revived!");
            }
        }
    }
