...
public class ChitinCollectible : MonoBehaviour
{
    ...
    private IEnumerator CollectionAnimation()
    {
        ...
        if (playerInventory != null)
        {
            ...
            // Play XP gain effect - now showing 2 XP symbols instead of 1
            VisualEffectManager visualEffectManager = FindObjectOfType<VisualEffectManager>();
            if (visualEffectManager != null)
            {
                // Show 2 XP symbols for each chitin (since each chitin is now worth 2 XP)
                visualEffectManager.PlayXPGainEffect(transform.position, 2);
            }
        }
        else
        {
