...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public void PlayXPSymbolEffect(Vector3 worldPosition, int xpAmount = 1)
    {
        Debug.Log($"Playing XP symbol effect at {worldPosition} with amount {xpAmount}");
        ...
        // Initialize XP pool if needed
        if (!symbolPools.ContainsKey("xp"))
        {
            InitializePool("xp", xpSymbolPrefab, 50); // Use larger pool for XP symbols
        }
        
        // Start spawning the XP symbols
        StartCoroutine(SpawnXPSymbols(worldPosition, xpAmount));
    }

    // Improved XP symbol spawning with better camera facing
    private IEnumerator SpawnXPSymbols(Vector3 worldPosition, int xpAmount)
    {
        // We need the camera, find it if not already assigned
        if (cameraTransform == null)
            cameraTransform = Camera.main?.transform;
        
        if (cameraTransform == null)
        {
            Debug.LogError("Cannot find camera for XP symbol orientation!");
            yield break;
        }
        
        // Find player for first half of animation
        GameObject player = GameObject.FindWithTag("Player");
        if (player == null)
        {
            Debug.LogWarning("Cannot find player for XP symbol animation!");
            // Continue anyway, will skip player step
        }
        
        // Find XP panel for target position
        Vector3 targetPos = Vector3.zero;
        if (xpPanel != null)
        {
            // Get XP panel position in screen space
            RectTransform xpRect = xpPanel.GetComponent<RectTransform>();
            if (xpRect != null)
            {
                // Convert the UI position to world space for symbol target
                Camera mainCamera = Camera.main;
                Vector3 screenPos = mainCamera.WorldToScreenPoint(xpRect.position);
                
                // Adjust Z distance for proper world position
                targetPos = mainCamera.ScreenToWorldPoint(new Vector3(screenPos.x, screenPos.y, 10f));
                Debug.Log($"XP panel world position: {targetPos}");
            }
        }
        else
        {
            // Fallback position - use top center screen position
            Camera mainCamera = Camera.main;
            targetPos = mainCamera.ViewportToWorldPoint(new Vector3(0.5f, 0.9f, 10f));
            Debug.LogWarning("XP panel not found! Using fallback position.");
        }
        
        // Spawn XP symbols with slight delay between each
        for (int i = 0; i < xpAmount; i++)
        {
            // Get a symbol from pool
            GameObject xpSymbol = GetSymbolFromPool("xp", xpSymbolPrefab);
            
            // Set initial properties
            xpSymbol.transform.SetParent(null); // Make sure it's in world space
            
            // Random position around spawn point
            Vector3 randomOffset = Random.insideUnitSphere * 0.5f;
            randomOffset.y = Mathf.Abs(randomOffset.y); // Keep offset above ground
            
            xpSymbol.transform.position = worldPosition + randomOffset;
            xpSymbol.transform.localScale = baseScale; // Use a consistent base scale
            
            // Always face the camera
            xpSymbol.transform.rotation = Quaternion.LookRotation(
                xpSymbol.transform.position - cameraTransform.position);
            
            // Make sure the renderer is enabled and visible
            SpriteRenderer renderer = xpSymbol.GetComponent<SpriteRenderer>();
            if (renderer != null)
            {
                renderer.enabled = true;
                renderer.sortingOrder = 100; // High sorting order to be visible
            }
            
            // Start the animation coroutine for this XP symbol
            StartCoroutine(AnimateXPSymbol(xpSymbol, player, targetPos));
    ...
    // Completely revamped XP symbol animation
    private IEnumerator AnimateXPSymbol(GameObject xpSymbol, GameObject player, Vector3 targetPos)
    {
        if (xpSymbol == null) yield break;
        
        Vector3 startPos = xpSymbol.transform.position;
        float totalDuration = xpFlyDuration;
        float elapsed = 0f;
        
        // First half of animation - fly toward player with arc
        Vector3 firstHalfTarget = player != null ? player.transform.position : (startPos + Vector3.up * 2f);
        
        while (elapsed < totalDuration * 0.5f)
        {
            if (xpSymbol == null) yield break;
            
            float normalizedT = elapsed / (totalDuration * 0.5f);
            
            // Calculate arc height
            float arcHeight = Mathf.Sin(normalizedT * Mathf.PI) * 1.5f;
            
            // Calculate position along curve
            Vector3 currentPos = Vector3.Lerp(startPos, firstHalfTarget, normalizedT);
            currentPos.y += arcHeight; // Add height for arc effect
        ...
        // Play XP sound at midpoint of animation
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound(xpGainedSoundName);
        }
        
        // Second half - fly to XP counter
        Vector3 secondHalfStart = xpSymbol.transform.position;
        float secondHalfElapsed = 0f;
        
        while (secondHalfElapsed < totalDuration * 0.5f)
        {
            if (xpSymbol == null) yield break;
            
            float normalizedT = secondHalfElapsed / (totalDuration * 0.5f);
            
            // Calculate arc height (smaller arc)
            float arcHeight = Mathf.Sin(normalizedT * Mathf.PI) * 0.8f;
            
            // Calculate position along curve
            Vector3 currentPos = Vector3.Lerp(secondHalfStart, targetPos, normalizedT);
            currentPos.y += arcHeight; // Add height for arc effect
            ...
            xpSymbol.transform.localScale = baseScale * scale;
            
            secondHalfElapsed += Time.deltaTime;
            yield return null;
        }
        
        // Ensure final position
        xpSymbol.transform.position = targetPos;
        ...
        if (uiHelper != null)
        {
            ...
            {
                // Add XP via the XP gain method of PlayerInventory
                playerInventory.GainExperiencePoints(xpPerSymbol);
            }
        }
