...
public class BaseInteraction : MonoBehaviour
{
    ...
    {
        // Find the UIHelper since it's not passed as parameter anymore
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper == null) yield break;
        
        // Define animation duration
        float duration = 0.7f; // Use the same duration as before
        
        float elapsed = 0f;
        int currentXP = startingXP;
        
        // Update the counter incrementally
        while (elapsed < duration && currentXP < startingXP + xpToAdd)
        {
            elapsed += Time.deltaTime;
            float progress = elapsed / duration;
            
            // Calculate how many XP points to add this frame
            int targetDisplayXP = startingXP + Mathf.FloorToInt(xpToAdd * progress);
            
            // Only update the UI if the value changed
            if (targetDisplayXP > currentXP)
            {
                // Update the display with the new value
                uiHelper.UpdateExperienceDisplay(targetDisplayXP, false);
                currentXP = targetDisplayXP;
        ...
        uiHelper.UpdateExperienceDisplay(startingXP + xpToAdd, false);
        
        // After the animation completes, use our new AddExperience method
        if (inventory != null)
        {
            // Call the sequential level-up method
