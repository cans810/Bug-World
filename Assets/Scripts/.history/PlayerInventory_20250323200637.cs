...
public class PlayerInventory : MonoBehaviour
{
    ...
    private void ProcessLevelUp(int level)
    {
        ...
        {
            playerAttributes.AddAttributePoints(playerAttributes.pointsPerLevel);
        }
        
        // NEW CODE: Increase crumb capacity by 2 on level up
        maxCrumbCapacity += 2;
    ...
    private void HandleLevelUpSequence(int level)
    {
        Debug.Log($"Starting level-up sequence for level {level}");
        
        // Instead, just show level up effects without showing the panel
        ...
        if (uiHelper != null)
        {
            uiHelper.ShowLevelUpText(level);
            // DO NOT call uiHelper.ShowAttributesPanel() here
        }
    }
    ...
    // Find the OnEarnExperience method or similar that handles leveling up
    private void HandleLevelUp()
    {
        // Other level up code...
        
        // IMPORTANT: Remove or comment out any lines that directly show the attribute panel
        // For example, if there's code like this:
        // attributeDisplay.ShowPanel(true);
        // OR
        // uiHelper.ShowAttributesPanel();
        // OR
        // FindObjectOfType<AttributeDisplay>().ShowPanel(true);
        
        // Just trigger the event without showing the panel
        if (OnLevelUp != null)
        {
            OnLevelUp(CurrentLevel);
        }
        
        // Do NOT call any UI show methods here
    }
} 