...
public class EntityHitbox : MonoBehaviour
{
    ...
    {
        // Check if owner is player and the other object is an insect
        if (playerController != null && ((1 << other.gameObject.layer) & LayerMask.GetMask("Insects")) != 0)
        {
            ...
            {
                // Trigger the player enter event for auto-attack
                OnPlayerEnterHitbox?.Invoke(insectEntity);
                
                if (showDebugMessages)
                {
                    Debug.Log($"Player hitbox detected insect: {insectEntity.name}");
                }
            }
            return;
        }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (playerController != null && ((1 << other.gameObject.layer) & LayerMask.GetMask("Insects")) != 0)
        {
            ...
            if (insectEntity != null)
            {
                // Trigger the player exit event to stop auto-attack
                OnPlayerExitHitbox?.Invoke(insectEntity);
                
                if (showDebugMessages)
                {
                    Debug.Log($"Player hitbox lost insect: {insectEntity.name}");
                }
            }
            return;
        }
        ...
        if (owner != null && IsInLootLayer(other.gameObject) && ownerAllyAI != null)
        {
            // Notify the AllyAI that loot is no longer in range
            ownerAllyAI.LootLeftRange(other.transform);
