...
public class ChitinDropper : MonoBehaviour
{
    ...
    [SerializeField] private bool showDropAreaGizmo = true;
    [SerializeField] private Color gizmoColor = new Color(0.5f, 0.8f, 1f, 0.5f); // Light blue with some transparency
    [SerializeField] private float gizmoRadius = 0.1f; // Size of the visualization
    ...
    // Update the OnDrawGizmos method to show the actual drop height
    private void OnDrawGizmos()
    {
        if (!showDropAreaGizmo)
            return;
            
        // Set the gizmo color
        Gizmos.color = gizmoColor;
        
        // Calculate the drop position with drop height applied
        Vector3 basePosition = transform.position;
        Vector3 dropPosition = new Vector3(basePosition.x, basePosition.y + dropHeight, basePosition.z);
        
        // Draw a sphere at the base position with different color
        Gizmos.color = new Color(0.3f, 0.3f, 0.3f, 0.3f); // Dark gray
        Gizmos.DrawSphere(basePosition, gizmoRadius * 0.5f);
        
        // Draw a line from base position to drop position
        Gizmos.color = Color.yellow;
        Gizmos.DrawLine(basePosition, dropPosition);
        
        // Draw a sphere at the actual drop position
        Gizmos.color = gizmoColor;
        Gizmos.DrawSphere(dropPosition, gizmoRadius);
        
        // Draw lines to show the potential chitin trajectories
        if (chitinPrefab != null)
        {
            // Draw the upward force range
            Gizmos.color = new Color(1f, 0.5f, 0f, 0.6f); // Orange for upward
            Gizmos.DrawLine(dropPosition, dropPosition + Vector3.up * upwardForceMax * 0.2f);
            
            // Draw the horizontal force range
            Gizmos.color = new Color(0f, 1f, 0.5f, 0.6f); // Green for horizontal
            float horizontalVisualization = horizontalForceMax * 0.2f;
            Vector3[] directions = new Vector3[] {
                new Vector3(horizontalVisualization, 0, 0),
                new Vector3(-horizontalVisualization, 0, 0),
                new Vector3(0, 0, horizontalVisualization),
                new Vector3(0, 0, -horizontalVisualization)
            };
            
            foreach (Vector3 dir in directions)
            {
                Gizmos.DrawLine(dropPosition, dropPosition + dir);
            }
        }
        ...
        UnityEditor.Handles.Label(dropPosition + Vector3.up * 0.5f, $"Chitin Drop Area (Height: {dropHeight})");
        #endif
    }
    ...
    {
        // Get the position with the drop height applied
        Vector3 basePosition = transform.position;
        Vector3 spawnPosition = new Vector3(basePosition.x, basePosition.y + dropHeight, basePosition.z);
