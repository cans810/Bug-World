...
public class UIHelper : MonoBehaviour
{
    ...
    {
        // Only show discovered entities panel if other panels are not active
        if (!allyAntsPanel.activeSelf && !DiscoveredEntitiesPanel.activeSelf)
        {
            DiscoveredEntitiesPanel.SetActive(true);
            
            // Find and show the actual catalog
            DiscoveredEntitiesController controller = DiscoveredEntitiesController.Instance;
            if (controller != null)
            {
                controller.ShowEntityCatalog();
                Debug.Log("Showing entity catalog");
            }
            else
            {
                Debug.LogWarning("DiscoveredEntitiesController not found");
            }
            
            //DiscoveredEntitiesPanel.GetComponent<Animator>().SetBool("ShowUp", true);
            //DiscoveredEntitiesPanel.GetComponent<Animator>().SetBool("Hide", false);
        }
    }
