...
public class AllyAI : MonoBehaviour
{
    ...
    // Add this field to track position changes for animation updates
    ...
    private void WanderBehavior()
    {
        ...
        // Check if we've moved since last frame to determine if we're actually wandering
        Vector3 currentPosition = transform.position;
        
        // If we've moved more than a small threshold since last frame, we're wandering
        if (lastFramePosition != Vector3.zero && Vector3.Distance(lastFramePosition, currentPosition) > 0.01f)
        {
            isMoving = true;
            UpdateAnimation(true);
        }
        else if (wanderingBehavior != null && wanderingBehavior.enabled)
        {
            // If wandering is enabled but we're not moving, check if we should be idle
            // This handles the case where the wandering behavior is pausing between movements
            ...
            {
                // Otherwise assume we're moving (wandering behavior is active)
                isMoving = true;
                UpdateAnimation(true);
            }
        }
