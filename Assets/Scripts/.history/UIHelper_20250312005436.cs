...
public class UIHelper : MonoBehaviour
{
    ...
    // Simplified chitin animation that works reliably
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        if (canvas == null)
        {
            Debug.LogError("Cannot find Canvas component in parents!");
            yield break;
        }
        
        // Get the starting position in screen space
        Vector2 startScreenPos = RectTransformUtility.WorldToScreenPoint(Camera.main, chitinCountText.transform.position);
        ...
        for (int i = 0; i < maxIconsToShow; i++)
        {
            ...
            if (iconRect == null)
            {
                Debug.LogError("Chitin icon prefab doesn't have a RectTransform component!");
                Destroy(chitinIcon);
                continue;
            }
            ...
            // Get the intermediate position in screen space
            Vector2 intermediateScreenPos = RectTransformUtility.WorldToScreenPoint(
                Camera.main, 
                posToMoveIconTransform.position
            );
            
            // Convert to local position
            Vector2 intermediateLocalPos;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(
                GetComponent<RectTransform>(),
                intermediateScreenPos,
                Camera.main,
                out intermediateLocalPos
            );
            
            // Add the random offset
            intermediateLocalPos += randomOffset;
            
            // First move to intermediate position
            StartCoroutine(MoveIconUI(
                iconRect,
                localPos,
                intermediateLocalPos,
                0.5f,
                () => {
                    // After reaching intermediate position, wait and then move to final position
                    StartCoroutine(WaitAndMoveToFinalUI(iconRect, intermediateLocalPos));
                }
            ));
    ...
    private IEnumerator WaitAndMoveToFinalUI(RectTransform iconRect, Vector2 currentPosition)
    {
        // Wait at the intermediate position
        yield return new WaitForSeconds(1f);
        
        // Get the final position in screen space
        Vector2 finalScreenPos = RectTransformUtility.WorldToScreenPoint(
            Camera.main,
            posToDropIcons.position
        );
        ...
        StartCoroutine(MoveIconUI(
            ...
            true // Add arc and scaling
        ));
    }

    private IEnumerator MoveIconUI(RectTransform iconRect, Vector2 startPos, Vector2 endPos, float duration, System.Action onComplete = null, bool addEffects = false)
    {
