    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            try {
                ...
                GameObject icon = Instantiate(chitinIconPrefab, iconContainer.transform);
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            try {
                ...
                float duration = 1.0f;
                float timer = 0f;
                Vector3 iconStartPos = icon.transform.position;
                Vector3 originalScale = icon.transform.localScale;
                
                Debug.Log($"[CRITICAL] Icon {i+1} starting position: {iconStartPos}");
                
                // Store the target position
                Vector3 targetPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                targetPos.z = 0;
                ...
                Debug.LogError($"[CRITICAL] Error animating chitin icon: {e.Message}");
