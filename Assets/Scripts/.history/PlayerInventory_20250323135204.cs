...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Improved camera animation for barrier removal
    private IEnumerator PlayBarrierRemovalAnimation(BarrierRocksController barrier)
    {
        Debug.Log("<color=cyan>Starting enhanced barrier removal camera animation</color>");
        
        // Check if barrier still exists (might have been destroyed by GameManager)
        if (barrier == null)
        {
            Debug.LogWarning("<color=orange>Barrier was already destroyed! Skipping animation sequence.</color>");
            
            // Still save the state - use a descriptive variable name to avoid conflict
            var gameManagerInstance = FindObjectOfType<GameManager>();
            if (gameManagerInstance != null)
            {
                gameManagerInstance.SetRockBarriersRemoved(true);
        ...
        // Calculate a cinematic viewing position
        // Position camera further back and higher for a wider view
        Vector3 directionToBarrier = (barrierPosition - playerTransform.position).normalized;
        Vector3 wideViewPosition = barrierPosition - directionToBarrier * 12f + Vector3.up * 8f;
        
        // Look at the barrier from a slight angle
        Vector3 lookDirection = (barrierPosition - wideViewPosition).normalized;
        Quaternion wideViewRotation = Quaternion.LookRotation(lookDirection);
        ...
        // PHASE 1: Move camera to viewing position
        Debug.Log("<color=cyan>PHASE 1: Moving camera to wide view position</color>");
        float moveToBarrierDuration = 2.0f;
        float elapsed = 0;
        
        while (elapsed < moveToBarrierDuration)
        {
            elapsed += Time.deltaTime;
            float t = Mathf.Clamp01(elapsed / moveToBarrierDuration);
            float smoothT = Mathf.SmoothStep(0, 1, t); // Smooth the movement
            
            mainCamera.transform.position = Vector3.Lerp(originalCameraPosition, wideViewPosition, smoothT);
            mainCamera.transform.rotation = Quaternion.Slerp(originalCameraRotation, wideViewRotation, smoothT);
            
            yield return null;
        }
        
        // Ensure camera is exactly at the intended position
        mainCamera.transform.position = wideViewPosition;
        mainCamera.transform.rotation = wideViewRotation;
        
        // PHASE 2: Wait before triggering animation
        Debug.Log("<color=cyan>PHASE 2: Short pause before animation</color>");
        yield return new WaitForSeconds(1.0f);
        
        // Check again if barrier still exists
        if (barrier == null)
        {
            Debug.LogWarning("<color=orange>Barrier was destroyed during the animation!</color>");
            // Skip to end
            yield break;
        }
        
        // PHASE 3: Trigger rock barrier removal
        Debug.Log("<color=cyan>PHASE 3: Triggering barrier removal</color>");
        ...
        try
        {
            ...
            if (SoundEffectManager.Instance != null && barrier != null)
            {
                barrier.PlayRockBreakSound();
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"<color=red>Error triggering barrier removal: {e.Message}</color>");
            // Continue with animation even if barrier operation fails
        }
        
        // PHASE 4: Wait to view the animation
        Debug.Log("<color=yellow>PHASE 4: Watching animation for 4 seconds</color>");
        
        // Use a loop to log the time remaining - this helps debug if it's actually waiting
        for (int i = 0; i < 8; i++)
        {
            yield return new WaitForSeconds(0.5f);
            Debug.Log($"<color=yellow>Still watching animation... ({i+1}/8 half-seconds)</color>");
        }
        
        // PHASE 5: Move camera back to original position
        Debug.Log("<color=cyan>PHASE 5: Returning camera to original position</color>");
        ...
        while (elapsed < returnDuration)
        {
            ...
            mainCamera.transform.position = Vector3.Lerp(wideViewPosition, originalCameraPosition, smoothT);
            mainCamera.transform.rotation = Quaternion.Slerp(wideViewRotation, originalCameraRotation, smoothT);
        ...
        // PHASE 6: Destroy the barrier after camera returns - with null check
        Debug.Log("<color=cyan>PHASE 6: Destroying barrier after camera return</color>");
        if (barrier != null)
        {
            try
            {
            ...
            {
                Debug.LogError($"<color=red>Error destroying barrier: {e.Message}</color>");
            }
        }
        else
        {
            Debug.Log("<color=orange>Barrier was already destroyed during animation</color>");
        }
        ...
        var gameSystemManager = FindObjectOfType<GameManager>();
        if (gameSystemManager != null)
        {
            // Set the barrier removal state
            gameSystemManager.SetRockBarriersRemoved(true);
            
            // Save the game
            gameSystemManager.SaveGame();
            
            Debug.Log("<color=green>Rock barrier removal state saved!</color>");
        }
    }
    ...
    // Add this method to check barriers after every level up
