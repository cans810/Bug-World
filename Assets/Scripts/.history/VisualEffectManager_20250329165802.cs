...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public void PlayCoinRewardEffect(Vector3 startPosition, int coinAmount)
    {
        ...
        // Number of coin symbols to spawn (scale based on reward amount)
        int symbolCount = Mathf.Clamp(coinAmount / 20, 3, 8);
        symbolCount = Mathf.Max(symbolCount, 3); // Always show at least 3 symbols
        
        Debug.Log($"Playing coin reward effect: {coinAmount} coins from {startPosition}, spawning {symbolCount} symbols");
        
        // Find the coin panel if it wasn't set in inspector
        if (coinPanel == null)
        {
            coinPanel = GameObject.Find("CoinPanel");
            if (coinPanel == null && canvasSafeArea != null)
            {
                // Try looking in the safe area
                coinPanel = canvasSafeArea.transform.Find("CoinPanel")?.gameObject;
            }
        }
        
        if (coinPanel == null)
        {
            Debug.LogError("Coin panel reference not found!");
            return;
        }
        
        // Spawn and animate coin symbols
        StartCoroutine(SpawnCoinSymbolsFromPosition(startPosition, symbolCount, coinAmount));
    }

    // Helper method to spawn coin symbols
    private IEnumerator SpawnCoinSymbolsFromPosition(Vector3 startPosition, int count, int totalCoins)
    {
        // Safety check for coinPanel
        if (coinPanel == null)
        {
            Debug.LogError("Cannot animate coins - coin panel is null");
            yield break;
        }
        
        // Get the coin panel's RectTransform
        RectTransform coinPanelRect = coinPanel.GetComponent<RectTransform>();
        if (coinPanelRect == null)
        {
            Debug.LogError("Coin panel doesn't have a RectTransform component");
            yield break;
        }
        
        // Calculate target position (center of coin panel)
        Vector3 targetPosition = coinPanel.transform.position;
        
        // Stagger the spawn of each coin symbol
        for (int i = 0; i < count; i++)
        {
            ...
            Vector2 randomOffset = Random.insideUnitCircle * 0.5f;
            symbol.transform.position = startPosition + new Vector3(randomOffset.x, randomOffset.y, 0f);
            ...
            if (spriteRenderer != null)
            {
                // Set a gold color for coins
                spriteRenderer.color = new Color(1f, 0.84f, 0f, 1f);
                // Ensure the sprite renders on top of everything
                spriteRenderer.sortingOrder = 1000;
            }
            
            // Animate the coin to the coin panel
            StartCoroutine(FlyCoinSymbolToCoinPanel(symbol, targetPosition));
            
            // Play coin sound for the first coin only to avoid sound spam
            if (i == 0 && SoundEffectManager.Instance != null)
            {
        ...
        // After all coins have animated, pulse the coin panel
        StartCoroutine(PulseUIElement(coinPanelRect));
    }

    // Method to animate a coin symbol to the coin panel
    private IEnumerator FlyCoinSymbolToCoinPanel(GameObject coinSymbol, Vector3 targetPosition)
    {
        SpriteRenderer spriteRenderer = coinSymbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;
        
        // Store original color and position
        Color originalColor = spriteRenderer.color;
        Vector3 startPosition = coinSymbol.transform.position;
        ...
        Vector3 originalScale = coinSymbol.transform.localScale;
        ...
        while (elapsed < duration)
        {
            ...
            // Use eased curves for nicer motion
            float easedProgress = EaseInOutCubic(normalizedTime);
            ...
            currentPos.y += arcFactor;
            
            // Update position
            coinSymbol.transform.position = currentPos;
            
            // Scale effect near the end (shrink as it approaches target)
            if (normalizedTime > 0.7f)
            {
                float scaleProgress = (normalizedTime - 0.7f) / 0.3f;
                coinSymbol.transform.localScale = Vector3.Lerp(originalScale, originalScale * 0.5f, scaleProgress);
            }
        ...
        // Return the symbol to the pool
        coinSymbol.SetActive(false);
        spriteRenderer.color = originalColor;
        coinSymbol.transform.localScale = originalScale;
    ...
    // Add this helper method to get a symbol from the pool (or create if needed)
    ...
    // Add this method to pulse a UI element when coins/XP reach it
    private IEnumerator PulseUIElement(RectTransform rectTransform)
    {
        ...
        while (elapsed < duration)
        {
            ...
            float easedT = Mathf.Sin(t * Mathf.PI * 0.5f); // Ease out sine
            
            rectTransform.localScale = Vector3.Lerp(originalScale, originalScale * 1.15f, easedT);
        ...
        while (elapsed < duration)
        {
            ...
            rectTransform.localScale = Vector3.Lerp(originalScale * 1.15f, originalScale, easedT);
    ...
    // Add this method to play XP reward animations from UI panels
    public void PlayXPRewardEffect(Vector3 panelPosition, int xpAmount)
    {
        ...
        // Determine number of symbols based on XP amount
        int symbolCount = Mathf.Clamp(xpAmount / 10, 3, 8);
        symbolCount = Mathf.Max(symbolCount, 3); // Always show at least 3 symbols
        
        Debug.Log($"Playing XP reward effect: {xpAmount} XP from {panelPosition}, spawning {symbolCount} symbols");
        
        // Find the XP panel if it wasn't set in inspector
        if (xpPanel == null)
        {
            xpPanel = GameObject.Find("XPPanel");
            if (xpPanel == null && canvasSafeArea != null)
            {
                // Try looking in the safe area
                xpPanel = canvasSafeArea.transform.Find("XPPanel")?.gameObject;
            }
        }
        
        if (xpPanel == null)
        {
            Debug.LogError("XP panel reference not found!");
            return;
        }
        
        // Spawn and animate XP symbols
        StartCoroutine(SpawnXPSymbolsFromPosition(panelPosition, symbolCount, xpAmount));
    }

    // Add this method to spawn XP symbols from a position (similar to coin method)
    private IEnumerator SpawnXPSymbolsFromPosition(Vector3 startPosition, int count, int totalXP)
    {
        // Safety check for xpPanel
        if (xpPanel == null)
        {
            Debug.LogError("Cannot animate XP - XP panel is null");
            yield break;
        }
        
        // Get the XP panel's RectTransform
        RectTransform xpPanelRect = xpPanel.GetComponent<RectTransform>();
        if (xpPanelRect == null)
        {
            Debug.LogError("XP panel doesn't have a RectTransform component");
            yield break;
        }
        
        // Calculate target position (center of XP panel)
        Vector3 targetPosition = xpPanel.transform.position;
        
        // Play XP sound at start of animation (only once)
        PlayXPSound();
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (spriteRenderer != null)
            {
                // Set a blue color for XP
                spriteRenderer.color = new Color(0.2f, 0.6f, 1f, 1f);
                // Ensure the sprite renders on top of everything
                spriteRenderer.sortingOrder = 1000;
            }
            
            // Animate the XP to the XP panel
            StartCoroutine(FlyXPSymbolToXPPanel(symbol, targetPosition));
            
            // Stagger the spawns
            yield return new WaitForSeconds(0.08f);
        }
        
        // After all XP symbols have animated, pulse the XP panel
        StartCoroutine(PulseUIElement(xpPanelRect));
        
        // Update player inventory with XP immediately if needed
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null)
        {
            Debug.Log($"Adding {totalXP} XP from reward");
            playerInventory.AddExperience(totalXP);
        }
    }

    // Method to animate an XP symbol to the XP panel
    private IEnumerator FlyXPSymbolToXPPanel(GameObject xpSymbol, Vector3 targetPosition)
    {
        SpriteRenderer spriteRenderer = xpSymbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;
        
        // Store original color and position
        Color originalColor = spriteRenderer.color;
        Vector3 startPosition = xpSymbol.transform.position;
        ...
        // Store original scale to restore later
        Vector3 originalScale = xpSymbol.transform.localScale;
        ...
        while (elapsed < duration)
        {
            ...
            // Add a slight arc to the path - higher arc for XP
            float arcHeight = 1.5f;
            float arcFactor = Mathf.Sin(easedProgress * Mathf.PI) * arcHeight;
            currentPos.y += arcFactor;
            
            // Update position
            xpSymbol.transform.position = currentPos;
            
            // Scale effect near the end (shrink as it approaches target)
            if (normalizedTime > 0.7f)
            {
                float scaleProgress = (normalizedTime - 0.7f) / 0.3f;
                xpSymbol.transform.localScale = Vector3.Lerp(originalScale, originalScale * 0.5f, scaleProgress);
            }
        ...
        // Return the symbol to the pool
        xpSymbol.SetActive(false);
        spriteRenderer.color = originalColor;
        xpSymbol.transform.localScale = originalScale;
        symbolPools["xp"].Enqueue(xpSymbol);
    }
} 