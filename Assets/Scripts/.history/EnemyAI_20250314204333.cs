...
public class EnemyAI : MonoBehaviour
{
    ...
    // Replace the existing boundary collision handlers with these smoother versions
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            transform.position += normal * 0.3f;
            
            // Start smooth turning coroutine
            StartCoroutine(SmoothTurnAtBoundary(normal));
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Move slightly away from boundary
            transform.position += directionToCenter * 0.3f;
            
            // Start smooth turning coroutine
            StartCoroutine(SmoothTurnAtBoundary(directionToCenter));
        }
    }

    // New coroutine for smooth turning at boundaries
    private IEnumerator SmoothTurnAtBoundary(Vector3 normal)
    {
        // Temporarily pause the wandering behavior
        bool wasWandering = false;
        if (wanderingBehavior != null)
        {
            wasWandering = wanderingBehavior.IsCurrentlyMoving();
            wanderingBehavior.SetWanderingEnabled(false);
        }
        
        // Calculate the new direction with some randomness
        ...
        float turnDuration = 0.5f; // Half a second to turn
