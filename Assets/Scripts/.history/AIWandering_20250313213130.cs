...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        {
            // DIRECT MOVEMENT APPROACH - bypassing potential frame rate issues
            
            // Calculate rotation with fixed smoothing factor
            float smoothRotationFactor = 3f; // Higher value = faster rotation
            Quaternion targetRotation = Quaternion.LookRotation(direction);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, smoothRotationFactor * Time.smoothDeltaTime);
            
            // Calculate movement distance this frame
            float moveSpeed = (livingEntity != null) ? livingEntity.moveSpeed : 2f;
            float moveDistance = moveSpeed * Time.smoothDeltaTime;
            
            // Apply movement directly to transform - most reliable method
            transform.position += transform.forward * moveDistance;
            
            // Notify rigidbody of position change if needed
