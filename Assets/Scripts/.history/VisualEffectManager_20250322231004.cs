...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator FlyChitinToNest(GameObject chitinSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                ...
                if (uiHelper != null)
                {
                    // Simply pulse the chitin counter
                    // This will use the current values from the game state
                    if (uiHelper.chitinCountText != null)
                    {
                        // Just animate the UI element without trying to update the value
                        RectTransform chitinRect = uiHelper.chitinCountText.GetComponent<RectTransform>();
                        if (chitinRect != null)
                        {
                            StartCoroutine(PulseUIElement(chitinRect));
                        }
                    }
                }
    ...
    private IEnumerator FlyCrumbToInventory(GameObject crumbSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)  // Using same duration as chitin for consistency
        {
            ...
            // Trigger effects near the end of the animation
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                // Get the UIHelper
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null)
                {
                    // Instead of directly accessing properties, just tell UIHelper to update its displays
                    // This avoids needing to know the exact property names
                    uiHelper.RefreshResourceDisplays();
                    
                    // Or alternatively, just animate the crumb display without passing values
                    uiHelper.AnimateCrumbCounter();
                }
                effectsTriggered = true;
            }
