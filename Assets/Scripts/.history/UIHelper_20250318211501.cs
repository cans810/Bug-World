...
public class UIHelper : MonoBehaviour
{
    ...
    public void UpdateChitinDisplay(int count)
    {
        if (chitinCountText != null)
        {
            chitinCountText.text = string.Format(countFormat, count, playerInventory?.MaxChitinCapacity ?? 100);
            
            if (animateOnChange && countTextRect != null)
            {
                // Use the safer method to handle the coroutine
                SafeStopAndStartCoroutine("AnimateChitinCount");
                
                // As a failsafe, force the scale to be correct after a delay
                StartCoroutine(ResetScaleAfterDelay(countTextRect, originalScale, animationDuration + 0.1f));
            }
        }
    }
    ...
    public void UpdateCrumbDisplay(int count)
    {
        if (crumbCountText != null)
        {
            ...
            {
                // Use the safer method to handle the coroutine
                SafeStopAndStartCoroutine("AnimateCrumbCount");
                
                // As a failsafe, force the scale to be correct after a delay
                StartCoroutine(ResetScaleAfterDelay(crumbTextRect, originalCrumbScale, animationDuration + 0.1f));
            }
        }
    }
    ...
    public void UpdateExperienceDisplay(int xpAmount, bool animate = true)
    {
        ...
        // As a failsafe, force the scale to be correct after a delay
        if (xpTextRect != null) {
            StartCoroutine(ResetScaleAfterDelay(xpTextRect, originalXpScale, 5f)); // Longer delay for XP counter animation
        }
    }
    ...
    public void ShowMarket()
    {
        ...
        {
            // If already initialized, update prices immediately
            marketManager.UpdateProductPricesInUI();
        }
    ...
    // Add this failsafe method
    private IEnumerator ResetScaleAfterDelay(RectTransform textRect, Vector3 originalScale, float delay)
    {
        yield return new WaitForSeconds(delay);
        if (textRect != null)
        {
    ...
    // Add a global method to reset all UI element scales - can be called periodically
    private void ResetAllTextScales()
    {
        ...
        if (xpTextRect != null) xpTextRect.localScale = originalXpScale;
        if (eggTextRect != null) eggTextRect.localScale = originalEggScale;
        if (coinTextRect != null) coinTextRect.localScale = originalCoinScale;
        
        Debug.Log("Reset all UI text scales to original values");
    }

    // Add this method to UIHelper.cs to update the market UI with Google Play prices
    public void UpdateMarketPrices()
    {
        // Get reference to MarketManager
        MarketManager marketManager = FindObjectOfType<MarketManager>();
        if (marketManager == null || !marketManager.IsInitialized())
        {
            Debug.LogError("MarketManager not found or not initialized!");
            return;
        }
        
        // Update UI with product information from Google Play
        marketManager.UpdateProductPricesInUI();
    }
} 
