...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    // Add this method to find all borders that match the current level
    private void CheckForUnlockedAreas(int newLevel)
    {
        ...
        foreach (BorderVisualizer border in allBorders)
        {
            if (border.GetRequiredLevel() == newLevel)
            {
                ...
                // Create the new area unlocked reward
                CreateNewAreaUnlockedReward(border.gameObject.name, newLevel);
                
                // Disable the border visualization
                border.DisableBorderVisualization();
    ...
    // Add this method to create the new area unlocked reward
    private void CreateNewAreaUnlockedReward(string areaName, int level)
    {
        ...
        // Clean up the area name (remove "Border" or other prefixes/suffixes)
        ...
        if (rewardText != null)
        {
            rewardText.text = $"New Area: {cleanAreaName}";
        }
