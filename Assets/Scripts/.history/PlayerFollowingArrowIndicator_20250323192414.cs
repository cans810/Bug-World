...
public class PlayerFollowingArrowIndicator : MonoBehaviour
{
    ...
    private void CreateArrowMesh()
    {
        ...
        meshFilter.mesh = CreateArrowMesh(0.3f, 0.6f, 0.4f); // width, length, head size
    ...
    private Mesh CreateArrowMesh(float width, float length, float headSize)
    {
        ...
        // Create vertices for a 3D arrow shape (improved design)
        Vector3[] vertices = new Vector3[]
        {
            // Arrow shaft (front)
            new Vector3(-width/3, 0, 0),                  // 0
            new Vector3(width/3, 0, 0),                   // 1
            new Vector3(width/3, 0, length - headSize),   // 2
            new Vector3(-width/3, 0, length - headSize),  // 3
            
            // Arrow head (front)
            new Vector3(-width/2, 0, length - headSize),    // 4
            new Vector3(width/2, 0, length - headSize),     // 5
            new Vector3(0, 0, length),                     // 6 (tip)
            
            // Arrow shaft (top/bottom edges for 3D)
            new Vector3(-width/3, width/3, 0),              // 7
            new Vector3(width/3, width/3, 0),               // 8
            new Vector3(width/3, width/3, length - headSize), // 9
            new Vector3(-width/3, width/3, length - headSize), // 10
            
            new Vector3(-width/3, -width/3, 0),              // 11
            new Vector3(width/3, -width/3, 0),               // 12
            new Vector3(width/3, -width/3, length - headSize), // 13
            new Vector3(-width/3, -width/3, length - headSize), // 14
            
            // Arrow head (sides for 3D)
            new Vector3(-width/2, width/3, length - headSize), // 15
            new Vector3(width/2, width/3, length - headSize),  // 16
            new Vector3(0, width/3, length),                  // 17
            
            new Vector3(-width/2, -width/3, length - headSize), // 18
            new Vector3(width/2, -width/3, length - headSize),  // 19
            new Vector3(0, -width/3, length)                   // 20
        };
