...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Improved method to check and remove barriers at level 30
    private void CheckForRockBarrierRemoval(int newLevel)
    {
        ...
        // Check if player has reached level 30 for rock barrier removal
        if (newLevel >= 30)
        {
            Debug.Log("Level 30+ detected - looking for barrier rocks to remove");
            
            // Find all BarrierRocksController instances
            BarrierRocksController[] rockBarriers = FindObjectsOfType<BarrierRocksController>();
            
            Debug.Log($"Found {rockBarriers.Length} barrier rock controller(s)");
            
            if (rockBarriers.Length > 0)
            {
                // Remove each barrier
                foreach (var barrier in rockBarriers)
                {
                    Debug.Log($"Removing barrier: {barrier.gameObject.name}");
                ...
                if (gameManager != null)
                {
                    ...
                    Debug.Log("Rock barriers removal state saved!");
                    
                    // Add visual feedback for the player
                    UIHelper uiHelper = FindObjectOfType<UIHelper>();
                    if (uiHelper != null)
                    {
                        uiHelper.ShowInformText("The rock barriers have been cleared!");
                    }
                }
                else
                {
                    Debug.LogError("GameManager not found - cannot save rock barrier state!");
                }
            }
            else
            {
                Debug.LogWarning("No barrier rocks found to remove, or they were already removed");
                
                // Still save the state even if no barriers found
                GameManager gameManager = FindObjectOfType<GameManager>();
                if (gameManager != null)
                {
                    gameManager.SetRockBarriersRemoved(true);
    ...
    // Modify the LevelUp method to fix the errors
    public void LevelUp()
    {
        CurrentLevel++;
        ...
        // Calculate attribute points (1 per level) - no need to create a local variable that isn't used
        
        // Invoke event
        OnLevelUp?.Invoke(CurrentLevel);
        
        // Reset XP count display to match level requirements (visual only)
        // Fix: ExperienceForNextLevel is read-only, use the correct update method
        // or update a backing field if there is one
        int nextLevelXP = GetExperienceRequiredForNextLevel(Experience);
        // If there's no method to directly set this, we can at least calculate it
        ...
        // Check if we've reached level 30 for rock barrier removal
        CheckForRockBarrierRemoval(CurrentLevel);
        
        // Save game on level up
        if (!isLoadingData)
        {
            // Fix: SavePlayerData doesn't exist, use SaveGame via GameManager instead
            GameManager gameManager = FindObjectOfType<GameManager>();
            if (gameManager != null)
            {
                gameManager.SaveGame();
