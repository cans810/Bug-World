...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float allyAvoidanceDistance = 1.5f;
    [SerializeField] private float allyDetectionRadius = 2.0f;
    [SerializeField] private LayerMask allyLayer;
    [SerializeField] private float avoidanceStrength = 2.0f;
    [SerializeField] private float personalSpaceRadius = 0.6f;
    [SerializeField] private float crowdAvoidanceMultiplier = 1.5f;
    ...
    // Replace the ApplyAllyAvoidance method with this improved version
    private Vector3 ApplyAllyAvoidance(Vector3 currentDirection)
    {
        ...
        int closeAlliesCount = 0; // Count of very close allies
        ...
        foreach (Collider allyCollider in nearbyAllies)
        {
            ...
            // Count very close allies for crowd detection
            if (distanceToAlly < personalSpaceRadius)
            {
                closeAlliesCount++;
            }
            ...
            if (distanceToAlly < allyAvoidanceDistance)
            {
                ...
                if (distanceToAlly < personalSpaceRadius)
                {
                    // Very strong repulsion in personal space
                    repulsionStrength = (1.0f - (distanceToAlly / personalSpaceRadius)) * avoidanceStrength * 2.0f;
                }
                else
                {
                    // Normal repulsion outside personal space
                    repulsionStrength = (1.0f - (distanceToAlly / allyAvoidanceDistance)) * avoidanceStrength;
                }
        ...
        if (avoidanceCount > 0)
        {
            // Apply crowd multiplier if there are multiple close allies
            if (closeAlliesCount >= 2)
            {
                // Increase avoidance strength in crowds
                float crowdFactor = 1.0f + (closeAlliesCount * 0.2f); // Increases with more allies
                avoidanceDirection *= Mathf.Min(crowdFactor, crowdAvoidanceMultiplier);
                
                // Debug log for crowd avoidance
                if (Debug.isDebugBuild && Time.frameCount % 60 == 0)
                {
                    Debug.Log($"{gameObject.name} applying crowd avoidance with {closeAlliesCount} close allies, factor: {crowdFactor}");
                }
            }
            ...
            float blendFactor = Mathf.Min(0.9f, 0.3f + (0.15f * closeAlliesCount)); // More avoidance in crowds
            Vector3 blendedDirection = Vector3.Lerp(currentDirection, avoidanceDirection, blendFactor);
            
            // Add a small random perturbation to help break symmetrical situations
            if (closeAlliesCount >= 3)
            {
                Vector3 randomDir = new Vector3(Random.Range(-0.2f, 0.2f), 0, Random.Range(-0.2f, 0.2f));
                blendedDirection += randomDir;
            }
            ...
            if (blendedDirection.magnitude < 0.1f)
            {
                ...
                blendedDirection += new Vector3(Random.Range(-0.1f, 0.1f), 0, Random.Range(-0.1f, 0.1f));
    ...
    // Replace the SeparateFromOverlappingAllies method
    private void SeparateFromOverlappingAllies()
    {
        ...
        Collider[] overlappingAllies = Physics.OverlapSphere(transform.position, personalSpaceRadius, allyLayer);
        
        // Count allies that are very close (for stuck detection)
        int nearbyAllyCount = 0;
        
        foreach (Collider ally in overlappingAllies)
        {
        ...
        if (nearbyAllyCount >= stuckThreshold)
        {
            if (!isStuck)
            {
                ...
                // Force idle animation immediately when stuck
                UpdateAnimation(false);
            }
        ...
        // If we have any overlapping allies, apply immediate separation
        if (nearbyAllyCount > 0)
        {
            ...
            foreach (Collider allyCollider in overlappingAllies)
            {
                ...
                else if (distance < personalSpaceRadius)
                {
                    ...
                    float separationStrength = 1.0f - (distance / personalSpaceRadius);
                    awayDirection = awayDirection.normalized * separationStrength;
                }
                else
                {
            ...
            {
                // Add a small random component to break symmetry in crowds
                if (nearbyAllyCount >= 3)
                {
                    separationMove += new Vector3(Random.Range(-0.1f, 0.1f), 0, Random.Range(-0.1f, 0.1f));
                }
                
                // Normalize and scale the movement
                separationMove = separationMove.normalized * 0.08f; // Slightly larger movement to ensure separation
