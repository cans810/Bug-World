...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Get camera controller reference once
        ...
        for (int i = metamorphosisLevels.Length - 1; i >= 0; i--)
        {
            if (level >= metamorphosisLevels[i])
            {
                ...
                // Update camera for new scale
                if (cameraController != null)
                {
                    cameraController.AdjustCameraForPlayerScale(newScale.x);
                }
        ...
        // Reset camera to base settings using the same cameraController reference
        if (cameraController != null)
        {
            cameraController.AdjustCameraForPlayerScale(baseScale.x);
        }
    }
