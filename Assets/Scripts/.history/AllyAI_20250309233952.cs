...
public class AllyAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        livingEntity.moveSpeed = playerTransform.gameObject.GetComponent<LivingEntity>().moveSpeed; 
    ...
    public void SetMode(AIMode newMode)
    {
        ...
        if (wanderingBehavior != null && newMode != AIMode.Wander)
        {
            wanderingBehavior.SetWanderingEnabled(false);
        }
        ...
        {
            case AIMode.Follow:
                // Already disabled wandering above
                break;
            ...
            case AIMode.Carrying:
                // Explicitly ensure wandering is disabled
                if (wanderingBehavior != null)
                    wanderingBehavior.SetWanderingEnabled(false);
                break;
    ...
    private void CarryLootToBase()
    {
        ...
        {
            // Use Rigidbody.MovePosition if available to avoid stacking with other movement systems
        ...
        // Debug log to track speed
        if (Time.frameCount % 60 == 0) // Log once per second at 60 FPS
        {
            Debug.Log($"{gameObject.name} carrying speed: {livingEntity.moveSpeed}, position delta: {(nextPosition - transform.position).magnitude / Time.deltaTime}");
        }
    }
