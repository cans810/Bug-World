...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        if (direction.magnitude > 0.1f)
        {
            ...
            if (livingEntity != null)
            {
                // Use smooth rotation similar to EnemyAI
                livingEntity.RotateTowards(direction, 1.5f); // Adjust rotation speed as needed
                
                // Then move forward using the entity's forward direction
                livingEntity.MoveInDirection(transform.forward, 1.0f); // Use consistent speed multiplier
            }
            else
            {
