...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (inventory != null)
        {
            ...
            // Also deposit crumbs - always deposit regardless of auto-deposit setting
            DepositPlayerCrumbs();
        }
        else
        {
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        // Allow deposits even when not at full capacity
        if (currentCrumbs > 0)
        {
            ...
            if (visualEffectManager != null)
            {
                // Use the correct method signature - just pass the count
                visualEffectManager.SpawnCrumbCollectEffect(currentCrumbs);
                
                // If we need to specify the equipped entity, we can add a separate 
                // call or property setting before calling the method
                if (hasEquippedEntity && entityController != null)
                {
                    // Optionally: Store the entity controller reference for the visual effect manager
                    // This might not be necessary if the visualEffectManager has its own way to check
                }
            }
            ...
            {
                // If entity is equipped, remove all crumbs from inventory
    ...
    // Add this coroutine to delay the counter update until after animation
    private IEnumerator UpdateEquippedEntityAfterAnimation(DiscoveredEntitiesController controller, int crumbCount)
    {
        ...
        // Update the entity controller with the deposited crumbs
