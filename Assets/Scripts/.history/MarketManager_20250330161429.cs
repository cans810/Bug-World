...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == "doubleThreat")
        {
            ...
            {
                // Create scorpion egg
                bool success = incubator.CreateMultipleEggs("scorpion", 1);
                ...
                if (success)
                {
                    ...
                    // Find the newly created egg for camera animation
                    StartCoroutine(ShowEggAnimation());
                    
                    // Show message to player
                    UIHelper uiHelper = FindObjectOfType<UIHelper>();
                    if (uiHelper != null)
                    {
                        uiHelper.ShowInformText("Double Threat purchased! Scorpion egg created.");
                    }
                }
                else
                {
                    // Show error message but don't refund (IAP was completed)
                    UIHelper uiHelper = FindObjectOfType<UIHelper>();
                    if (uiHelper != null)
                    {
                        uiHelper.ShowInformText("Purchase completed but not enough incubation space!");
                    }
    ...
    private IEnumerator ShowEggAnimation()
    {
        // Wait a moment for the egg to be fully created
        yield return new WaitForSeconds(0.5f);
        
        // Find the most recently created egg
        GameObject[] eggs = GameObject.FindGameObjectsWithTag("Egg");
        if (eggs != null && eggs.Length > 0)
        {
            // Get the last egg in the array (most likely the newest one)
            GameObject newestEgg = eggs[eggs.Length - 1];
            ...
            {
                // Create a special animation request for the egg
                cameraAnimations.AnimateToNewEgg(newestEgg.transform);
                Debug.Log("Started camera animation to show new scorpion egg");
            }
            else
            {
                Debug.LogError("Could not find CameraAnimations component for egg showcase");
            }
        }
        else
        {
            Debug.LogWarning("No eggs found for camera animation");
        }
    }
}
