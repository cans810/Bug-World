...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add this method to PlayerInventory.cs to handle sequential level ups
    private System.Collections.IEnumerator ProcessSequentialLevelUps(int oldLevel, int newLevel, int finalExperience)
    private IEnumerator ProcessSequentialLevelUps(int oldLevel, int newLevel, int finalExperience)
    {
        Debug.Log($"Processing sequential level ups from {oldLevel} to {newLevel}");
        
        // Store the final target experience
        int targetExperience = finalExperience;
        
        // Process each level one by one
        for (int level = oldLevel + 1; level <= newLevel; level++)
        {
            // Calculate the minimum experience needed for this level
            int experienceForThisLevel = xpRequirements[level - 1];
            
            // Temporarily set experience to exactly what's needed for this level
            _experience = experienceForThisLevel;
            ...
            // Log this intermediate level up
            Debug.Log($"Sequential level up: Now level {level}");
            
            // Handle the level-up sequence with proper flow
            if (!isLoadingData)
            {
                // This is the critical part - process each level's unique events
                HandleLevelUpSequence(level);
                
                // Wait for camera animations to complete if they're happening
                ...
                {
                    Debug.Log($"Waiting for camera animation to complete for level {level}");
                    
                    // Wait until the animation is no longer in progress
                    while (cameraAnimations.IsAnimationInProgress())
                    {
                        yield return null;
                    }
                    
                    // Additional delay to ensure UI elements settle
                    yield return new WaitForSeconds(0.5f);
                }
                else
                {
                    // If no camera animation, still add a short delay between levels
                    yield return new WaitForSeconds(0.2f);
                }
            }
        }
        
        // Finally, set the experience to the actual final value
        _experience = targetExperience;
        
        // Update UI one final time
        OnExperienceChanged?.Invoke(_experience);
        
        Debug.Log($"Sequential level up process complete. Final level: {newLevel}, Experience: {_experience}");
    }

    // Modify the AddExperience method to use the sequential level up process
    public void AddExperience(int amount)
    {
        if (amount <= 0) return;
        
        // Calculate the new total experience
        int newExperience = _experience + amount;
        
        // Determine levels before and after adding experience
        int oldLevel = GetLevelFromExperience(_experience);
        int newLevel = GetLevelFromExperience(newExperience);
        
        // Check if this will result in multiple level ups
        if (newLevel > oldLevel + 1)
        {
            Debug.Log($"Multiple level ups detected: {oldLevel} → {newLevel}. Processing sequentially.");
            StartCoroutine(ProcessSequentialLevelUps(oldLevel, newLevel, newExperience));
        }
        else
        {
            // Normal experience gain logic for single or no level up
