...
public class AttributeDisplay : MonoBehaviour
{
    ...
    [SerializeField] private bool autoHideWhenNoPoints = true;
    ...
    private void UpdateDisplay()
    {
        if (playerAttributes != null)
        {
            ...
            // Auto-hide panel when all points are spent
            if (autoHideWhenNoPoints && 
                lastPoints > 0 && 
                playerAttributes.AvailablePoints <= 0)
            {
                HidePanel();
            }
            
            // Update last known points
            lastPoints = playerAttributes.AvailablePoints;
        }
    }
    ...
    public void ToggleAttributesPanel()
    {
        if (attributesPanel != null)
        {
            // If panel is active, hide it; otherwise, show it
