...
public class PlayerInventory : MonoBehaviour
{
    ...
    public int Experience
    {
        ...
        private set
        {
            ...
            {
                // Add debug log to verify this block is reached
                Debug.Log($"<color=red>Level up detected: {oldLevel} -> {newLevel}</color>");
                
                // Level up occurred!
                Debug.Log($"PlayerInventory: Level up from {oldLevel} to {newLevel}");
                
                // Process the level up
                ProcessLevelUp(newLevel);  // This calls our ProcessLevelUp method
    ...
    public void AddExperience(int amount)
    {
        ...
        Debug.Log($"<color=blue>Adding {amount} experience. Current: {Experience}, Next Level: {ExperienceForNextLevel}</color>");
        Experience += amount;  // This should trigger the property setter above
    }
    ...
    private void ProcessLevelUp(int level)
    {
        ...
        // Add attribute points FIRST
        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
        if (playerAttributes != null)
        {
            // Force set the points to ensure it works
            int currentPoints = playerAttributes.AvailablePoints;
            int pointsToAdd = playerAttributes.pointsPerLevel;
            playerAttributes.SetAvailablePoints(currentPoints + pointsToAdd);
            
            Debug.Log($"<color=yellow>Level {level}: Adding {pointsToAdd} points. " +
                      $"Old: {currentPoints}, New: {currentPoints + pointsToAdd}</color>");
        }
        
        // Increase chitin and crumb capacity
        IncreaseChitinCapacity(15);
        IncreaseCrumbCapacity(2);
        Debug.Log($"<color=green>Increased chitin capacity by 15 and crumb capacity by 2 for level {level}</color>");
    ...
    private void OnLevelChanged(int newLevel)
    {
        ...
        if (newLevel >= 31)
        {
            ...
            // Start a coroutine to wait for any active animations before triggering barrier removal
