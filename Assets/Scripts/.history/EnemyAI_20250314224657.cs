...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modified method to prevent all movement when player enters hitbox
    private void HandlePlayerEnterHitbox(LivingEntity playerEntity)
    {
        ...
        if (rb != null)
        {
            ...
            // Completely zero out all movement and freeze position
            rb.velocity = Vector3.zero;
            rb.angularVelocity = Vector3.zero;
            rb.isKinematic = true;
            rb.constraints = RigidbodyConstraints.FreezeAll;
            
            // Store the original state for restoration when player leaves
            StartCoroutine(RestoreRigidbodyWhenPlayerLeaves(rb, wasKinematic));
        }
        
        // Force LivingEntity to lock rotation
        if (livingEntity != null)
        {
            livingEntity.SetRotationLocked(true);
        }
        
        // First set idle to ensure we're in a clean state
        if (animController != null)
        {
            animController.SetIdle();
        }
        
        // Store the current position to prevent any movement
        StartCoroutine(MaintainPositionDuringHitbox());
        StartCoroutine(MaintainPositionDuringCombat());
        
        // Small delay before attacking to ensure animations transition properly
        StartCoroutine(StartAttackAfterDelay(0.1f));
    }

    // Add this coroutine to maintain position during combat
    private IEnumerator MaintainPositionDuringCombat()
    {
        Vector3 combatPosition = transform.position;
    ...
    // Add this method to restore rigidbody when player leaves
    private IEnumerator RestoreRigidbodyWhenPlayerLeaves(Rigidbody rb, bool wasKinematic)
    {
        // Wait until player leaves hitbox
        while (isPlayerInHitbox)
        {
            yield return null;
        }
    ...
    private void HandlePlayerExitHitbox(LivingEntity playerEntity)
    {
        ...
        StartCoroutine(ReturnToWanderingAfterDelay(1.0f));
    }
