...
public class ArrowIndicator : MonoBehaviour
{
    ...
    private void UpdateArrowPosition()
    {
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            // Set rotation for the arrow to point in the right direction
            arrowObject.transform.rotation = Quaternion.Euler(0, 0, angle);
            
            // Make sure the sprite faces the camera without warping
            arrowObject.transform.forward = mainCamera.transform.forward;
            
            // Ensure the scale is uniform to prevent warping
            Vector3 currentScale = arrowObject.transform.localScale;
            float baseScale = arrowSize * (1f + pulseAmount * Mathf.Sin(Time.time * pulseSpeed));
    ...
    public void ShowArrow()
    {
        ...
        if (arrowObject == null)
        {
            ...
            // Make sure the sprite maintains its original proportions
            // Add a Billboard component if needed
            Billboard billboard = arrowObject.GetComponent<Billboard>();
            if (billboard == null)
            {
                billboard = arrowObject.AddComponent<Billboard>();
            }
            billboard.preserveAspect = true;
        }
