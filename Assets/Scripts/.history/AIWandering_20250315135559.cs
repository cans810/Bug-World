...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            while (isWandering)
            {
                ...
                // If we're at the border, stop and turn around
                if (restrictToSpawnArea && distanceFromSpawn >= maxWanderRadius * 0.95f)
                {
                    Debug.Log($"{gameObject.name}: Hit border, turning around");
    ...
    // Simplify the TurnAndMoveInward method to just turn and pick a new direction
    private IEnumerator TurnAndMoveInward()
    {
        // Stop moving first
        isWandering = false;
        
        // Calculate direction towards spawn center
        Vector3 directionToSpawn = (spawnPosition - transform.position).normalized;
        directionToSpawn.y = 0;
        
        // Turn to face roughly toward the center (with some randomness)
        ...
        // Visual feedback
        Debug.DrawRay(transform.position, newDirection * 2f, Color.red, 2f);
