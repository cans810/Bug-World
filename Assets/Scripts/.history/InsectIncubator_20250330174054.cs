...
public class InsectIncubator : MonoBehaviour
{
    ...
    public GameObject CreateInsectEgg(string entityType, Vector3 positionOffset = default, int index = -1)
    {
        ...
        // Ensure unique name for the egg
        if (index >= 0)
        {
            eggObj.name = $"{entityType}Egg_{index}_{Time.time.ToString("F2")}";
        }
        ...
        if (eggController != null)
        {
            ...
            // Set unique index to ensure hatched insect will have a unique name
            eggController.SetUniqueIndex(index);
    ...
    // Update CreateMultipleEggs to ensure unique names
    public bool CreateMultipleEggs(string entityType, int count)
    {
        ...
        // Calculate spacing between eggs
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create egg with offset and index for unique naming
            GameObject egg = CreateInsectEgg(entityType, offset, i);
