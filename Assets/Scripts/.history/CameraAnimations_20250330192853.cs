...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator AnimateCameraToArea(Transform areaTarget, int level = 0, string areaName = "", bool isEgg = false)
    {
        ...
        float useHeightOffset = isEgg ? 3.5f : heightOffset;  // Higher for better angled view (was 2f)
        float useLookDownAngle = isEgg ? 45f : lookDownAngle; // Less steep angle (was 90f - straight down)
        float useAnimationDuration = isEgg ? 1.0f : animationDuration; // Keep the faster animation
        float useAreaViewDuration = isEgg ? 3.0f : areaViewDuration; // Keep slightly longer viewing time
        
        // Calculate position above the target
        Vector3 areaPosition = areaTarget.position;
        Vector3 targetPosition;
        if (isEgg) {
            // For eggs: Position the camera at an offset angle, not directly above
            // This creates a more interesting perspective looking down at the egg
            Vector3 offsetDirection = new Vector3(0.8f, 0, -0.8f).normalized; // Diagonal offset
            targetPosition = new Vector3(
                areaPosition.x + offsetDirection.x * 4.0f, // Add horizontal offset
                areaPosition.y + useHeightOffset, 
                areaPosition.z + offsetDirection.z * 4.0f  // Add depth offset
            );
        } else {
            // For areas: Use the standard overhead view
            targetPosition = new Vector3(areaPosition.x, areaPosition.y + useHeightOffset, areaPosition.z);
        }
        
        // Calculate rotation to look at the target (not just down)
        Quaternion targetRotation;
        if (isEgg) {
            // For eggs: Look directly at the egg position from our offset position
            targetRotation = Quaternion.LookRotation(areaPosition - targetPosition);
        } else {
            // For areas: Use the standard look down angle
            targetRotation = Quaternion.Euler(useLookDownAngle, cameraTransform.rotation.eulerAngles.y, 0);
        }
    ...
    private IEnumerator AnimateCameraToNewInsect(Transform insectTransform)
    {
        ...
        // Calculate position to view the insect from the side
        Vector3 insectPosition = insectTransform.position;
        
        // Get a random angle to position the camera for more interesting views
        float randomAngle = Random.Range(30f, 60f);
        
        // Convert angle to direction vector (in XZ plane)
        float angleRad = randomAngle * Mathf.Deg2Rad;
        Vector3 viewDirection = new Vector3(Mathf.Cos(angleRad), 0, Mathf.Sin(angleRad)).normalized;
        
        // Position camera at an angle, further away, and higher up
        Vector3 viewPosition = insectPosition + 
                               viewDirection * 4.0f +     // Further away and at an angle (was 2.0f)
                               Vector3.up * 3.5f;         // Higher up (was 2.5f)
        
        // Calculate rotation to look at the insect from this position
