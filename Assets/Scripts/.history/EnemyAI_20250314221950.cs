...
public class EnemyAI : MonoBehaviour
{
    ...
    // Enhance the AttackIfInRange method to include more stability measures
    private void AttackIfInRange()
    {
        ...
        if (currentTarget != null && !currentTarget.IsDead)
        {
            ...
            if (distanceToTarget <= minAttackDistance)
            {
                ...
                // Stop wandering behavior completely
                if (wanderingBehavior != null)
                {
                    wanderingBehavior.SetWanderingEnabled(false);
                    wanderingBehavior.enabled = false; // Completely disable the wandering component
                    isWandering = false;
                }
                ...
                // Make sure there's no residual physics or forces causing rotation
            ...
            {
                // Re-enable wandering if target moves out of range
                if (wanderingBehavior != null && !wanderingBehavior.enabled)
                {
                    livingEntity.SetRotationLocked(false); // Ensure rotation is unlocked
                    wanderingBehavior.enabled = true;
                    wanderingBehavior.SetWanderingEnabled(true);
                    isWandering = true;
                }
    ...
    // Enhance FaceTarget method to ensure complete rotation stability during attacks
    private void FaceTarget(Vector3 target)
    {
        ...
        if (directionToTarget.magnitude > 0.1f)
        {
            ...
            // Calculate angle difference to see if we're already facing the target
            float angleDifference = Quaternion.Angle(transform.rotation, targetRotation);
            
            // Only rotate if we're not already closely facing the target (with a smaller threshold)
            if (angleDifference > 1.0f) // Reduced from 2.0f to 1.0f for more stability
            {
                // Store current rotation for potential reversion
                ...
                // Check if the rotation put us past the target direction and stabilize if needed
                Quaternion newRotation = transform.rotation;
                float newAngleDifference = Quaternion.Angle(newRotation, targetRotation);
                
                // If we're now facing very closely, snap precisely to the target rotation
                if (newAngleDifference < 0.5f)
                {
                    transform.rotation = targetRotation;
                    
                    // Tell the livingEntity to temporarily lock rotation
                    if (livingEntity != null)
                    {
                        livingEntity.SetRotationLocked(true);
                        
                        // Use Invoke to unlock rotation for the next frame
                        Invoke("UnlockRotation", Time.deltaTime);
                    }
                }
            }
            else
            {
                // We're already facing close enough - lock in the exact rotation
                transform.rotation = targetRotation;
                
                // Ensure any physics rotation is canceled
                Rigidbody rb = GetComponent<Rigidbody>();
                if (rb != null)
                {
                    rb.angularVelocity = Vector3.zero;
    ...
    // Helper method to unlock rotation after a frame
    private void UnlockRotation()
    {
        if (livingEntity != null)
        {
            // Only unlock if we're not in combat
            if (currentTarget == null || 
                Vector3.Distance(transform.position, currentTarget.transform.position) > minAttackDistance)
            {
