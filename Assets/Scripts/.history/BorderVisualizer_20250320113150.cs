    ...
    private void CreateVisualSphere()
    {
        if (showRadius && sphereCollider != null)
        {
            ...
            if (fillCircle)
            {
                ...
                // Create a foggy material for the circle
                Material circleMaterial;
                
                // Try to use a particle shader for foggy effect
                if (Shader.Find("Particles/Standard Unlit") != null)
                {
                    circleMaterial = new Material(Shader.Find("Particles/Standard Unlit"));
                    circleMaterial.SetFloat("_Mode", 2); // Fade transparency mode
                    ...
                    // Create a gradient effect for fog
                    Color foggyColor = new Color(radiusColor.r, radiusColor.g, radiusColor.b, 0.4f);
                    circleMaterial.SetColor("_TintColor", foggyColor);
                    circleMaterial.EnableKeyword("_FADING_ON");
                    
                    // Soft particles for foggy look
                    circleMaterial.SetFloat("_SoftParticlesNearFadeDistance", 0);
                    circleMaterial.SetFloat("_SoftParticlesFarFadeDistance", 1);
                }
                else
                {
                    // Fallback to a simpler shader if particle shader isn't available
                ...
                // Apply the material to the circle
        ...
        {
            // Original sphere visualization code but with foggy appearance
            ...
            // Create a foggy material for the visualization
            if (Shader.Find("Particles/Standard Unlit") != null)
            {
                // Use particle shader for better fog effect
                instanceMaterial = new Material(Shader.Find("Particles/Standard Unlit"));
                
                // Set up for foggy appearance
                instanceMaterial.SetFloat("_Mode", 2); // Fade transparency mode
                ...
                // Create a foggy color
                Color fogColor = new Color(sphereColor.r, sphereColor.g, sphereColor.b, 0.2f);
                instanceMaterial.SetColor("_TintColor", fogColor);
                
                // Enable fog-like soft particles
                instanceMaterial.EnableKeyword("_FADING_ON");
                instanceMaterial.SetFloat("_SoftParticlesNearFadeDistance", 0);
                instanceMaterial.SetFloat("_SoftParticlesFarFadeDistance", 1);
            }
            else if (visualizerMaterial == null)
            {
                // Fallback to standard shader with fog-like settings
                instanceMaterial = new Material(Shader.Find("Transparent/Diffuse"));
                
                // For mobile compatibility with foggy appearance
                ...
                // More transparent and foggy looking
                instanceMaterial.color = new Color(sphereColor.r, sphereColor.g, sphereColor.b, 0.15f);
            }
            else
            {
                // Use the provided material but make it foggier
                instanceMaterial = new Material(visualizerMaterial);
                // Adjust alpha for foggy appearance
                Color origColor = instanceMaterial.color;
                instanceMaterial.color = new Color(origColor.r, origColor.g, origColor.b, 0.15f);
            }
