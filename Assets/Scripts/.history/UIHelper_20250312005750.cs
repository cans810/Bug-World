...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        Vector2 startPos = chitinPanel != null ? 
        ...
        for (int i = 0; i < maxIconsToShow; i++)
        {
            ...
            // For Screen Space Overlay, we can use direct screen positions
            ...
            StartCoroutine(MoveIconScreenSpace(
                iconRect,
                startPos,
                (Vector2)posToMoveIconTransform.position + randomOffset,
                0.5f,
                () => {
                    // After reaching intermediate position, wait and then move to final position
                    StartCoroutine(WaitAndMoveToFinalScreenSpace(iconRect, (Vector2)posToMoveIconTransform.position + randomOffset));
                }
            ));
    ...
    private IEnumerator WaitAndMoveToFinalScreenSpace(RectTransform iconRect, Vector2 currentPosition)
    {
    ...
    private IEnumerator MoveIconScreenSpace(RectTransform iconRect, Vector2 startPos, Vector2 endPos, float duration, System.Action onComplete = null, bool addEffects = false)
    {
        ...
        while (Time.time < endTime)
        {
            ...
            // Set the new position directly (works for Screen Space Overlay)
            iconRect.position = newPos;
            
            yield return null;
        }
        
        // Ensure it reaches the exact end position
        iconRect.position = endPos;
