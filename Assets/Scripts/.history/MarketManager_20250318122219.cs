...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public void BuyProduct(string productId)
    {
        ...
        if (storeController != null && storeController.products.WithID(productId) != null)
        {
            ...
            // Add debug log to check if the product exists
            Product product = storeController.products.WithID(productId);
            if (product != null)
            {
                Debug.Log($"Product found: {product.definition.id}, {product.metadata.localizedTitle}, {product.metadata.localizedPriceString}");
            }
            else
            {
                Debug.LogError($"Product with ID {productId} not found in store controller");
            }
        }
        else
        {
