...
public class CameraAnimations : MonoBehaviour
{
    ...
    [SerializeField] private float minZoomHeight = 5f; // Closest zoom level
    [SerializeField] private float maxZoomHeight = 25f; // Furthest zoom level
    [SerializeField] private float zoomSpeed = 5f; // How fast zooming happens
    [SerializeField] private float mouseWheelZoomSpeed = 2f; // How sensitive the mouse wheel is
    ...
    private float targetZoomHeight; // Target height for smooth zooming
    private float lastTouchDistance; // Used for pinch detection
    private bool isPinching = false; // Track if user is pinching

    // Add this variable at the class level
    private Vector3 playerMapPosition; // Store player position for map reference
    ...
    private IEnumerator AnimateToMapViewCoroutine()
    {
        ...
        targetZoomHeight = mapViewHeight;
        
        // Store this for the boundary calculations
        originalMapPosition = new Vector3(playerPosition.x, playerPosition.y + targetZoomHeight, playerPosition.z);
    ...
    private void HandleMapViewDragging()
    {
        ...
        // Use the player position that's already stored at class level
        // We don't need to find it again since Update already did
    ...
    // Modified Update method to handle zooming
    private void Update()
    {
        // Only handle camera controls when in map view
        if (isInMapView && !isAnimating)
        {
            // Find player position once per frame instead of multiple times
            PlayerController player = FindObjectOfType<PlayerController>();
            playerMapPosition = player != null ? player.transform.position : Vector3.zero;
            
            // Handle dragging
            if (enableMapDragging)
            {
                HandleMapViewDragging();
            }
            
            // Handle mouse wheel zooming on PC
            float mouseScrollDelta = Input.mouseScrollDelta.y;
            if (Mathf.Abs(mouseScrollDelta) > 0.01f)
            {
                // Scroll up (positive) zooms in, scroll down (negative) zooms out
                float zoomDelta = mouseScrollDelta * mouseWheelZoomSpeed;
                targetZoomHeight = Mathf.Clamp(targetZoomHeight - zoomDelta, minZoomHeight, maxZoomHeight);
                
                // Debug
                Debug.Log($"Mouse wheel zoom: {mouseScrollDelta}, Target height: {targetZoomHeight}");
            }
            ...
            if (Input.touchCount == 2)
            {
                ...
                // If this is the first frame of the pinch
                if (!isPinching)
                {
                    isPinching = true;
                ...
                else
                {
                    ...
                    float pinchZoomAmount = deltaDistance * 0.01f * zoomSpeed; // Scale factor
                    targetZoomHeight = Mathf.Clamp(targetZoomHeight - pinchZoomAmount, minZoomHeight, maxZoomHeight);
                    ...
                    {
                        Debug.Log($"Pinch zoom: {deltaDistance:F1}, Target height: {targetZoomHeight:F1}");
                    }
                }
            }
            else
            {
                // Reset pinch state when not touching with two fingers
                isPinching = false;
            }
            
            // Apply zoom smoothly - FIX THE REFERENCES TO PLAYERPOSITION
            if (Mathf.Abs(cameraTransform.position.y - (playerMapPosition.y + targetZoomHeight)) > 0.01f)
            {
                // Get current position
                Vector3 currentPos = cameraTransform.position;
                
                // Calculate new height
                float newHeight = Mathf.Lerp(currentPos.y - playerMapPosition.y, targetZoomHeight, Time.deltaTime * 5f);
                
                // Keep x and z the same, only change height
                cameraTransform.position = new Vector3(
                    currentPos.x, 
                    playerMapPosition.y + newHeight, 
                    currentPos.z
                );
                
                // Adjust drag speed based on height (further zoom = faster panning)
                float zoomRatio = (newHeight - minZoomHeight) / (maxZoomHeight - minZoomHeight);
                float currentDragMultiplier = Mathf.Lerp(0.5f, 2f, zoomRatio);
                
                // Debug
                //Debug.Log($"Applying zoom: {newHeight:F1}, Drag multiplier: {currentDragMultiplier:F2}");
            }
        }
    }
} 