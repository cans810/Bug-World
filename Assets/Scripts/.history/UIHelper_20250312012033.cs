...
public class UIHelper : MonoBehaviour
{
    ...
    // Update the AnimateChitinDeposit method with more detailed logging
    public void AnimateChitinDeposit(int chitinCount)
    {
        // Make sure we have a positive count
        if (chitinCount <= 0)
        {
            Debug.LogError("Attempted to animate deposit with zero or negative chitin count: " + chitinCount);
            return;
        }
        
        Debug.Log($"[UIHelper] AnimateChitinDeposit called with count: {chitinCount}");
        
        // Force the count to be at least 1 for testing
        int countToUse = chitinCount;
        
        // Start the animation directly here instead of calling through AnimateChitinCollection
        StartCoroutine(SpawnAndAnimateChitinIcons(countToUse));
    }

    // Update the SpawnAndAnimateChitinIcons method with more detailed logging
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        Debug.Log($"[UIHelper] SpawnAndAnimateChitinIcons started with count: {chitinCount}");
        
        // Force a minimum of 1 icon for debugging
        if (chitinCount <= 0)
        {
            Debug.LogError($"[UIHelper] Invalid chitin count for animation: {chitinCount}");
            yield break;
        }
        
        // Limit the number of icons to prevent overwhelming visuals
        int maxIconsToShow = Mathf.Min(chitinCount, 10);
        Debug.Log($"[UIHelper] Will spawn {maxIconsToShow} chitin icons (from count: {chitinCount})");
        ...
        if (chitinPanel == null)
        {
            ...
            if (chitinPanel == null)
            {
                Debug.LogWarning("[UIHelper] Cannot find ChitinPanel - using chitinCountText position instead");
            }
        }
        ...
        Debug.Log($"[UIHelper] Starting position for icons: {startPos}");
        ...
        {
            Debug.Log($"[UIHelper] Creating chitin icon {i+1} of {maxIconsToShow}");
            
            // Instantiate the chitin icon directly here
            GameObject chitinIcon = Instantiate(chitinIconPrefab, transform);
            chitinIcon.transform.position = startPos;
            chitinIcon.name = $"ChitinIcon_{i+1}";
        ...
        Debug.Log($"[UIHelper] Spawned {spawnedIcons.Count} chitin icons in total");
    }
    ...
    // Add this method for direct testing from the Unity Editor
    public void TestChitinAnimation(int count)
    {
        Debug.Log($"[TEST] Directly testing chitin animation with count: {count}");
        StartCoroutine(DirectTestAnimation(count));
    }

    private IEnumerator DirectTestAnimation(int count)
    {
        // Wait a moment to ensure everything is initialized
        yield return new WaitForSeconds(0.5f);
        
        // Call the animation method directly
        AnimateChitinDeposit(count);
    }
} 
