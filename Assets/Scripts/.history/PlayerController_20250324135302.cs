...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandlePlayerDeath()
    {
        ...
        // Temporarily disable auto-attack functionality
        bool wasAutoAttackEnabled = autoAttackEnabled;
        autoAttackEnabled = false;
        
        // Cancel all pending actions
        StopAllCoroutines();
        
        // Only start revival coroutine
        StartCoroutine(ReviveAfterDelay(5f, wasAutoAttackEnabled));
    }

    private IEnumerator ReviveAfterDelay(float delay, bool restoreAutoAttack = true)
    {
        ...
        // Reset player position to spawn point if available
        if (spawnPoint != null)
        {
        ...
        // Reset BEFORE reviving to ensure clean state
        // Clear all enemies in range again (just to be sure)
        enemiesInRange.Clear();
        
        // Stop ALL coroutines to ensure no lingering attack routines
        StopAllCoroutines();
        
        // Reset auto-attack state
        autoAttackEnabled = false;
        if (autoAttackCoroutine != null)
        {
            StopCoroutine(autoAttackCoroutine);
        ...
        // Get the animation controller and reset ALL animation states
        if (animController != null && animController.Animator != null)
        {
            // Reset all animation parameters to default values
            foreach (AnimatorControllerParameter param in animController.Animator.parameters)
            {
                if (param.type == AnimatorControllerParameterType.Bool)
                    animController.Animator.SetBool(param.name, false);
                else if (param.type == AnimatorControllerParameterType.Trigger)
                    animController.Animator.ResetTrigger(param.name);
                else if (param.type == AnimatorControllerParameterType.Float)
                    animController.Animator.SetFloat(param.name, 0f);
                else if (param.type == AnimatorControllerParameterType.Int)
                    animController.Animator.SetInteger(param.name, 0);
            }
            
            // Force idle state
            animController.Animator.Play("Idle", 0, 0f);
        }
        
        // *** Add this section before reviving the entity ***
        
        // Force reset all input-related variables
        moveDirection = Vector3.zero;
        currentSpeed = 0f;
        isMoving = false;
        
        // Reset any joystick input if we have the OnScreenJoystick
        OnScreenJoystick[] joysticks = FindObjectsOfType<OnScreenJoystick>();
        foreach (OnScreenJoystick joystick in joysticks)
        {
            // Force reset the joystick
            joystick.ResetJoystick();
        }
        
        // Force release all keyboard inputs
        Input.ResetInputAxes();
        
        // Re-enable this component to ensure input processing works
        enabled = true;
        
        // IMPORTANT: Only AFTER all resets, revive the entity
        if (livingEntity != null)
        {
            livingEntity.Revive();
            
            // Reset attack state in livingEntity if applicable
            if (livingEntity.GetType().GetField("isAttacking", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance) != null)
            {
                System.Reflection.FieldInfo attackingField = livingEntity.GetType().GetField("isAttacking", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                attackingField.SetValue(livingEntity, false);
            }
        }
        ...
        // Ensure all attack animations are stopped
        StopAllAttacks();
        
        // Wait a short delay to ensure everything is reset
        yield return new WaitForSeconds(0.5f);
        
        // Re-enable player controls
        enabled = true;
        
        // Optionally restore auto-attack setting to what it was before death
        if (restoreAutoAttack)
        {
            yield return new WaitForSeconds(0.5f);  // Add a delay before re-enabling auto-attack
            autoAttackEnabled = restoreAutoAttack;
        }
    ...
    // New coroutine to update the cooldown timer text with improved formatting
    private IEnumerator UpdateCooldownTimerText(float initialCooldown)
    {
        ...
        {
            // Special formatting for values below 1 to prevent flickering
            string cooldownText;
            if (remainingCooldown < 1f)
            {
                // Fixed number of decimal places below 1 second
                cooldownText = remainingCooldown.ToString("0.0");
            }
            else
            {
                cooldownText = remainingCooldown.ToString("F1");
            }
            
            // Use a simpler format for the cooldown text
        ...
        // When cooldown is done, just provide haptic feedback without displaying text
        if (enabled && livingEntity != null && !livingEntity.IsDead)
        {
