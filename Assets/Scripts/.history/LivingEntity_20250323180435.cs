...
public class LivingEntity : MonoBehaviour
{
    ...
    [SerializeField] public float baseMoveSpeed = 2.0f;
    ...
    private float invulnerabilityEndTime = -1f;
    
    private bool hasBeenDiscovered = false;
    
    // Add this property to make recoveryTime accessible
    ...
    public virtual void TakeDamage(float amount, GameObject damageSource = null)
    {
        ...
        // Discovery code removed - entities are only discovered when killed
    }
    
    // Add this method to handle the death animation sequence
    private IEnumerator HandleDeathEffects()
    {
        // Wait briefly for death animation to start
        yield return new WaitForSeconds(0.2f);
        
        // First, find and disable the EntityOutline component
        EntityOutline outline = GetComponent<EntityOutline>();
        if (outline != null)
        {
            // Disable the outline first
            outline.enabled = false;
            
            // Also find any Outline component and disable it
            ...
            Debug.Log($"Disabled outline on dead entity: {gameObject.name}");
        }
        
        // Get the current position
        Vector3 startPosition = transform.position;
        Vector3 endPosition = startPosition - new Vector3(0, 1.0f, 0); // Sink 1 unit down
        
        // Slowly sink into the ground over 2 seconds
        ...
        while (elapsedTime < sinkDuration)
        {
            ...
            float percentComplete = elapsedTime / sinkDuration;
            
            // Use a smooth easing function
            float easedPercent = Mathf.SmoothStep(0, 1, percentComplete);
            transform.position = Vector3.Lerp(startPosition, endPosition, easedPercent);
            
            yield return null;
        }
        
        // Now that the entity has sunk, we can destroy the outline component
        if (outline != null)
        {
            Destroy(outline);
            Debug.Log($"Removed outline component from dead entity: {gameObject.name}");
        }
    }
    
    // Modify the Die method to correctly apply sinking effect only to enemies
    public void Die(GameObject killer = null)
    {
        ...
        // Check if this was killed by the player
        ...
        // Set the dead flag
        isDead = true;
        currentHealth = 0; // Make sure health is set to 0
        ...
        // Entity discovery and mission tracking if killed by player
        if (killedByPlayer)
        {
            // Get entity type for discovery
            string entityName = gameObject.name.ToLower();
            string entityType = GetEntityTypeFromName(entityName);
            
            // Trigger discovery system
            DiscoveredEntitiesController controller = FindObjectOfType<DiscoveredEntitiesController>();
            if (controller != null)
            {
                controller.DiscoverEntityOnKill(entityType);
                Debug.Log($"Entity discovery recorded for: {entityType}");
            }
            
            // Also notify mission manager
            if (MissionManager.Instance != null)
            {
                Debug.Log($"Notifying mission manager of {entityType} kill");
                MissionManager.Instance.OnEntityKilled(entityType);
            }
        }
        
        // IMPORTANT: Force death animation to play
        StartCoroutine(ForceDeathAnimationByReplacement());
        
        // Trigger the death event
        OnDeath.Invoke();
        
        // If health bar exists, destroy it
        if (healthBarInstance != null)
        {
            Destroy(healthBarInstance);
        }
        
        // Only do the sinking effect for non-player entities (ENEMIES)
        if (!isPlayer) // Changed from !killedByPlayer to !isPlayer
        {
            // Start our death effects coroutine
            StartCoroutine(HandleDeathEffects());
            Debug.Log($"Starting sink effect for non-player entity: {gameObject.name}");
        }
        else
        {
            Debug.Log("Not sinking player entity on death");
        }
    }
    ...
    // Add this method to LivingEntity to set temporary invulnerability
    public void SetTemporaryInvulnerability(float duration)
    {
        IsInvulnerable = true;
        invulnerabilityEndTime = Time.time + duration;
        
        // Visual feedback that player is invulnerable (optional)
        StartCoroutine(InvulnerabilityVisualFeedback(duration));
        
        // Debug log
        Debug.Log($"Player is invulnerable for {duration} seconds");
    }
    
    // Optional visual feedback coroutine
    private IEnumerator InvulnerabilityVisualFeedback(float duration)
    {
        ...
        {
            // Toggle between original color and a semi-transparent blue/white
            float pulse = Mathf.PingPong(Time.time * 5f, 1f);
            renderer.material.color = Color.Lerp(originalColor, new Color(0.7f, 0.7f, 1f, 0.7f), pulse);
            yield return null;
        }
    ...
    // When an entity dies, in the Die() method or wherever entity discovery happens
    private void RecordEntityDiscovery()
    {
        // Get the entity type
        string entityType = gameObject.name;
        
        // Remove any numbers or "(Clone)" from the name
        entityType = Regex.Replace(entityType, @"\d+|\(Clone\)", "").Trim();
        
        Debug.Log($"Attempting to record discovery for entity: {entityType}");
        
        // Find the DiscoveredEntitiesController and record the discovery
        DiscoveredEntitiesController entitiesController = FindObjectOfType<DiscoveredEntitiesController>();
        if (entitiesController != null)
        {
            // Special handling for Flea
            if (entityType.ToLower().Contains("flea"))
            {
                Debug.Log("DETECTED FLEA - Using special force discovery method");
                entitiesController.ForceDiscoverEntity("flea");
            }
            else
            {
                // Use regular discovery method for other entities
                entitiesController.DiscoverEntityOnKill(entityType);
            }
            
            Debug.Log($"Discovery recorded for: {entityType}");
        }
    }
    
    // Add this method to set the recovery time
    public void SetRecoveryTime(float newRecoveryTime)
    {
        recoveryTime = Mathf.Max(0.1f, newRecoveryTime); // Ensure recovery time doesn't go too low
        Debug.Log($"Recovery time set to {recoveryTime:F2} seconds");
    }

    // Add this method to set the move speed (called from PlayerAttributes)
    public void SetMoveSpeed(float newSpeed)
    {
        moveSpeed = Mathf.Max(0.5f, newSpeed); // Ensure speed doesn't go too low
        Debug.Log($"Move speed set to {moveSpeed:F2}");
    }

    // Add this helper method to clean up entity name
    private string GetEntityTypeFromName(string objectName)
    {
        ...
        // Handle common entity types - match partial names to standard types
        if (cleanName.Contains("ant"))
            return "ant";
        else if (cleanName.Contains("flea"))
            return "flea";
        else if (cleanName.Contains("beetle"))
            return "beetle";
        else if (cleanName.Contains("spider"))
            return "spider";
        
        // Return the cleaned name if no specific match
        return cleanName;
    }

    // Add this method to clear all targets in range
    public void ClearAllTargetsInRange()
    {
        if (targetsInRange != null)
        {
            ...
            // If this is the player, also stop any attacks in progress
            if (gameObject.CompareTag("Player"))
            {
                PlayerController playerController = GetComponent<PlayerController>();
                if (playerController != null)
                {
                    playerController.StopAllAttacks();
                }
            }
            
            Debug.Log($"Cleared all targets for {gameObject.name}");
        }
    }
}