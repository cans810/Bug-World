...
public class AllyAI : MonoBehaviour
{
    ...
    private void HandleDeath()
    {
        ...
        // Handle carried loot if the ally dies while carrying something
        if (carriedLoot != null)
        {
            // Detach the loot from the ally
            carriedLoot.SetParent(null);
            
            // Change the layer back to Loot from CarriedLoot
            carriedLoot.gameObject.layer = LayerMask.NameToLayer("Loot");
            
            // Position the loot on the ground near the ally
            carriedLoot.position = new Vector3(
                transform.position.x,
                0.02800143f, // Use the same ground height as when dropping
