...
public class AIWandering : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Move EVERY frame for smooth movement when wandering
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        // Skip if already at waypoint or invalid direction
        if (direction.magnitude < waypointReachedDistance || direction.magnitude < 0.1f)
            return;
        
        // Normalize the direction vector
        direction.Normalize();
        
        // DIRECT movement approach (no physics/no livingEntity)
        // First rotate toward the waypoint - DIRECT rotation
        Quaternion targetRotation = Quaternion.LookRotation(direction);
        transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, 4f * Time.deltaTime);
        
        // Then move forward - DIRECT movement
        float speed = livingEntity != null ? livingEntity.moveSpeed : 2.0f;
        transform.position += transform.forward * speed * Time.deltaTime;
        
        // No need to call animations - handled by UpdateAnimationState already
    }
