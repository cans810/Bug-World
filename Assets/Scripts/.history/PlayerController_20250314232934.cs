...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if (isAtMapBorder && moveDirection.magnitude > 0.1f)
        {
            ...
            {
                // If trying to move through border, cancel that component of movement
                // This prevents movement through the border
                Vector3 restrictedMove = moveDirection - (borderNormal * movementThroughBorder);
                moveDirection = restrictedMove;
                
                // Make sure movement remains normalized if needed
                if (moveDirection.magnitude > 1f)
                    moveDirection.Normalize();
                
                // Check if movement is now negligible after restriction
        ...
        if (currentSpeed > 0.01f)
        {
            if (livingEntity != null)
            {
                ...
                // Extra check: If at border, verify we aren't moving through it
                if (isAtMapBorder)
                {
                    float moveThroughBorder = Vector3.Dot(finalMoveDirection, borderNormal);
                    if (moveThroughBorder < 0)
                    {
                        // Add force against movement direction to push back from border
                        finalMoveDirection = finalMoveDirection - (borderNormal * moveThroughBorder);
                        
                        // Make sure movement remains normalized if needed
                        if (finalMoveDirection.magnitude > 1f)
            ...
            else
            {
                ...
                if (isAtMapBorder)
                {
                    float moveThroughBorder = Vector3.Dot(motion.normalized, borderNormal);
                    if (moveThroughBorder < 0)
                    {
                        // Nullify movement in the direction of border crossing
                        motion = motion - (borderNormal * moveThroughBorder * motion.magnitude);
                        
                        // If motion becomes negligible after restriction, stop
    ...
    {
        // Check if the collider is in the MapBorder layer (not PlayerHitbox)
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // We've hit a map border
            isAtMapBorder = true;
            
            // Calculate the normal direction pointing INWARD from the border
            // For a sphere with player inside, the normal points from closest point to player
            Vector3 closestPoint = other.ClosestPoint(transform.position);
            
            // For a sphere with player inside, we need to reverse the normal direction
            // Normal should point INWARD to keep player inside the sphere
            borderNormal = (closestPoint - transform.position).normalized;
            
            // If normal is zero (rare edge case), use a fallback
            if (borderNormal.magnitude < 0.001f)
            {
                // Use position relative to center of sphere if possible
                if (other is SphereCollider)
                {
                    borderNormal = (other.transform.position - transform.position).normalized;
                }
                else
                {
                    // Fallback to forward direction
                    borderNormal = transform.forward;
                }
            }
            ...
            if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
            {
                ...
                Debug.Log($"Player hit map border: {other.gameObject.name}");
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Update the normal vector as the player moves along the border
            Vector3 closestPoint = other.ClosestPoint(transform.position);
            
            // For a sphere with player inside, we need to reverse the normal direction
            // Normal should point INWARD to keep player inside the sphere
            Vector3 newNormal = (closestPoint - transform.position).normalized;
            
            // Only update if we have a meaningful normal
            if (newNormal.magnitude > 0.001f)
            {
                borderNormal = newNormal;
            }
            else if (other is SphereCollider)
            {
                // Fallback for sphere: use direction from center to player
                borderNormal = (other.transform.position - transform.position).normalized;
            }
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Player has somehow exited the map boundary - this shouldn't normally happen
            Debug.LogWarning("Player escaped map boundary! Forcing return to playable area.");
            
            // Reset border tracking variables
            isAtMapBorder = false;
            borderNormal = Vector3.zero;
            
            // Calculate direction back to center of the sphere
            if (other is SphereCollider sphereCollider)
            {
                // Calculate the center of the sphere in world space
                Vector3 sphereCenter = other.transform.position;
                ...
                // Calculate position just inside the sphere boundary (95% of radius)
                Vector3 safePosition = sphereCenter - (dirToCenter * (sphereRadius * 0.95f));
                
                // Teleport player back inside the sphere
                transform.position = safePosition;
                
                // Notify player with UI message
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText("You've been returned to the playable area!");
            ...
            {
                // For non-sphere colliders, teleport player to spawn point as fallback
                if (spawnPoint != null)
                {
                    transform.position = spawnPoint.position;
                    ...
                    if (uiHelper != null)
                    {
                        uiHelper.ShowInformText("You've been returned to the spawn point!");
                        HapticFeedback.MediumFeedback();
                    }
                }
            }
