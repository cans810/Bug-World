...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // MUCH longer duration to ensure it reaches target
        float duration = 2.0f;
        bool hasBumpedUI = false;
        
        // Debug marker at target position (helps visualize)
        GameObject marker = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        marker.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f);
        Renderer markerRenderer = marker.GetComponent<Renderer>();
        if (markerRenderer) markerRenderer.material.color = Color.red;
        
        // Initial target calc
        Ray ray = Camera.main.ScreenPointToRay(targetScreenPos);
        Vector3 targetWorldPos = ray.GetPoint(10f);
        marker.transform.position = targetWorldPos;
        
        Debug.Log($"Coin animation starting: {startPos} -> {targetWorldPos}");
        
        // Extremely simple direct movement
        float elapsed = 0f;
        while (elapsed < duration)
        {
            // Recalculate target each frame (for moving UI)
            ray = Camera.main.ScreenPointToRay(targetScreenPos);
            targetWorldPos = ray.GetPoint(10f);
            marker.transform.position = targetWorldPos;
            
            elapsed += Time.deltaTime;
            float t = Mathf.Clamp01(elapsed / duration);
            
            // Very simple direct movement - almost straight line with slight arc
            ...
            symbol.transform.rotation = Camera.main.transform.rotation;
            
            // Check proximity to target
            float distanceToTarget = Vector3.Distance(symbol.transform.position, targetWorldPos);
            
            // Bump UI when close to target
            if (!hasBumpedUI && t > 0.75f)
            {
                TriggerUICounterAnimation("coin");
                hasBumpedUI = true;
                Debug.Log("Coin UI bump triggered");
            }
            
            // Fade out only at end
            if (t > 0.9f)
            {
                float fade = (t - 0.9f) * 10f; // 0-1 in last 10%
                spriteRenderer.color = new Color(startColor.r, startColor.g, startColor.b, 1f - fade);
            }
        ...
        Debug.Log($"Coin reached target: {symbol.transform.position}");
        ...
        Debug.Log("Coin animation completed and cleaned up");
    }
