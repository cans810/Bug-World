...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Throttle position recalculation to reduce jitter
        bool shouldRecalculatePosition = Time.time >= lastPositionUpdateTime + positionRecalculationInterval;
        if (shouldRecalculatePosition)
        {
            lastPositionUpdateTime = Time.time;
            
            // Check if the player is moving
            if (Time.time >= lastPlayerVelocityCheckTime + playerVelocityCheckInterval)
            {
                lastPlayerVelocityCheckTime = Time.time;
                
                if (lastPlayerPosition != Vector3.zero)
                {
                    float playerMovementDistance = Vector3.Distance(playerTransform.position, lastPlayerPosition);
                    isPlayerMoving = playerMovementDistance > playerMovementThreshold;
                }
        ...
        // Determine effective follow distance based on player movement
        float effectiveFollowDistance;
        
        if (stopWhenPlayerStops && !isPlayerMoving)
        {
            // Use a larger distance when player is stationary
            effectiveFollowDistance = isMoving ? 
                playerStationaryStopDistance + 0.5f : playerStationaryStopDistance;
        }
        else
        {
            // Normal following distance when player is moving
            effectiveFollowDistance = isMoving ? followDistance + 0.5f : followDistance;
        }
        
        // Update logic for following
        if (distanceToPlayer > effectiveFollowDistance)
        {
            isMoving = true;
            
            // If player is stationary and we're using stop behavior, move slower as we approach
            float currentSpeedMultiplier = speedMultiplier;
            if (stopWhenPlayerStops && !isPlayerMoving)
            {
                // Reduce speed as we get closer to the stop distance
                float distanceFactor = Mathf.Clamp01((distanceToPlayer - effectiveFollowDistance) / 2.0f);
                currentSpeedMultiplier *= Mathf.Lerp(0.5f, 1.0f, distanceFactor);
            }
            
            // Only recalculate direction periodically
            if (shouldRecalculatePosition)
            {
                // Calculate raw direction to player
                Vector3 rawDirectionToPlayer = (playerTransform.position - transform.position).normalized;
                
                // Apply avoidance with other allies
                if (avoidOtherAllies)
                {
                    // Reduce the avoidance strength to avoid overreaction
                    float reducedDetectionRadius = allyDetectionRadius * 0.8f;
                    rawDirectionToPlayer = ApplyAllyAvoidance(rawDirectionToPlayer, reducedDetectionRadius);
                }
                ...
                {
                    smoothedMoveDirection = rawDirectionToPlayer;
                }
            }
            
            // Skip movement if we're too close to the destination or have a very small movement
            if (distanceToPlayer > minimumMoveDistance && smoothedMoveDirection.magnitude > 0.01f)
            {
                // Update rotation and movement using LivingEntity with smoothed direction
                livingEntity.RotateTowards(smoothedMoveDirection, rotationMultiplier * 0.8f);
                livingEntity.MoveInDirection(smoothedMoveDirection, currentSpeedMultiplier * 0.9f);
        ...
        else
        {
            ...
            // Only recalculate direction periodically when idle too
            if (shouldRecalculatePosition)
            {
                // Calculate direction to player for facing
                Vector3 directionToPlayer = (playerTransform.position - transform.position).normalized;
                livingEntity.RotateTowards(directionToPlayer, rotationMultiplier * 0.3f);
            }
    ...
    private Vector3 ApplyAllyAvoidance(Vector3 currentDirection, float detectionRadius)
    {
        ...
        foreach (Collider allyCollider in nearbyAllies)
        {
            ...
            {
                // Add a gentler repulsion vector (reduced strength)
                float repulsionStrength = (1.0f - (distanceToAlly / allyAvoidanceDistance)) * (avoidanceStrength * 0.7f);
                
                // Normalize direction before applying strength
                Vector3 normalizedDirection = directionToAlly.normalized;
                
                // Apply repulsion in opposite direction (more gently)
        ...
        // Blend the avoidance with the original direction (weighted more toward the original)
        Vector3 blendedDirection = (currentDirection * 0.7f) + (avoidanceDirection.normalized * 0.3f);
