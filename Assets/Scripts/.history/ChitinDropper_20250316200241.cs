...
public class ChitinDropper : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinPieces(int amount)
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            GameObject chitinObject = Instantiate(chitinPrefab, dropPosition, Quaternion.Euler(90, Random.Range(0, 360), 0));
            ...
            chitinObject.layer = LayerMask.NameToLayer("Loot");
    ...
    private IEnumerator SimpleChitinAnimation(GameObject chitinObject)
    {
        ...
        // Calculate a random landing position with REDUCED spread (smaller radius)
        float angle = Random.Range(0, 360) * Mathf.Deg2Rad;
        float spreadRadius = Random.Range(0.1f, 0.4f); // Reduced from 0.2f-0.8f to keep pieces closer
        ...
        Vector3 endPos = new Vector3(startPos.x + offset.x, 0.01f, startPos.z + offset.z);
        
        // Calculate peak position with HIGHER arc
        float peakHeight = Random.Range(0.6f, 1.0f); // Increased from 0.3f-0.6f for higher arc
        Vector3 peakPos = (startPos + endPos) * 0.5f + Vector3.up * peakHeight;
        
        // Animate over time
        float duration = 0.7f; // Slightly longer duration for more pronounced arc
        float elapsed = 0;
        ...
        while (elapsed < duration)
        {
            ...
            // Use cubic Bezier curve for a more pronounced arc
            float oneMinusT = 1f - t;
            Vector3 position = oneMinusT * oneMinusT * oneMinusT * startPos +
                              3f * oneMinusT * oneMinusT * t * (startPos + Vector3.up * peakHeight * 0.5f) +
                              3f * oneMinusT * t * t * peakPos +
                              t * t * t * endPos;
            
            chitinObject.transform.position = position;
            
            // Add a slight rotation for visual interest
            chitinObject.transform.Rotate(Vector3.up, 90f * Time.deltaTime);
