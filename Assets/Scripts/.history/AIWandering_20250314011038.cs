...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        {
            // We've hit a boundary - completely stop movement first
            ...
            // Mark as hitting boundary and set redirection timer
            isHittingBoundary = true;
            boundaryRedirectionTime = Time.time + BOUNDARY_REDIRECT_DURATION;
            
            // Get direction away from boundary center (opposite of boundary normal)
            Vector3 awayFromBoundary = (transform.position - mapBoundary.transform.position).normalized;
            
            // Set a new waypoint directly away from the boundary
            currentWaypoint = transform.position + awayFromBoundary * Random.Range(minWanderDistance, maxWanderDistance);
            
            // Make sure waypoint is within bounds
            if (mapBoundary != null && !mapBoundary.IsWithinBounds(currentWaypoint))
            {
                // If still outside, use a random perpendicular direction
                Vector3 perpDirection = Vector3.Cross(Vector3.up, awayFromBoundary);
                if (Random.value < 0.5f)
                    perpDirection = -perpDirection;
                    
                currentWaypoint = transform.position + perpDirection * Random.Range(minWanderDistance, maxWanderDistance);
            }
            
            // Face away from boundary
            transform.forward = awayFromBoundary;
            
            // Explicitly cancel wandering routine and restart
            StopAllCoroutines();
            StartCoroutine(WanderingRoutine());
        }
