...
public class ChitinCollectible : MonoBehaviour
{
    ...
    private void Update()
    {
        // Don't process collection logic if not collectable yet, already being collected,
        // or if we've already started the collection process
        if (!isCollectable || isBeingCollected || hasStartedCollection)
            return;
        ...
        if (distanceToPlayer <= collectRadius)
        {
            ...
            {
                // Mark that we've started collection to prevent multiple collections
    ...
    {
        Debug.Log("Force collect called on chitin");
        
        // Don't proceed if we've already started collection
        if (hasStartedCollection || isBeingCollected)
        {
            Debug.Log("Collection already in progress, ignoring ForceCollect");
            return;
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") && isCollectable && !isBeingCollected && !hasStartedCollection)
        {
