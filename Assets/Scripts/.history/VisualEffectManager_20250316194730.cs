...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public void PlayXPGainEffect(Vector3 nestPosition, int xpAmount)
    {
        ...
        // Calculate number of symbols based on XP amount
        // We want one symbol per XP point for better visual feedback
        int symbolCount = Mathf.Min(xpAmount, 20); // Cap at 20 symbols to avoid overwhelming effects
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpValue)
    {
        ...
        // Track if we've updated the XP counter yet
        bool hasUpdatedXP = false;
        
        // Find the player inventory once at the start
        ...
        while (elapsed < duration)
        {
            ...
            // Update XP counter when the symbol is almost at the destination
            // Only do this for the first symbol that reaches the destination
            if (!hasUpdatedXP && normalizedTime >= 0.9f && playerInventory != null)
            {
                // Add XP to the player's total
                playerInventory.AddExperience(xpValue);
                hasUpdatedXP = true;
                
                // Log for debugging
                Debug.Log($"XP symbol reached UI, adding {xpValue} XP");
            }
        ...
        // Add XP at the end if it hasn't been added yet (shouldn't happen, but just in case)
        if (!hasUpdatedXP && playerInventory != null)
        {
            playerInventory.AddExperience(xpValue);
        }
