...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Get the direction axis of the capsule
        Vector3 directionVector = Vector3.up; // Default Y axis (1)
        if (capsuleCollider.direction == 0) // X axis
            directionVector = Vector3.right;
        else if (capsuleCollider.direction == 2) // Z axis
            directionVector = Vector3.forward;
        
        // Calculate half length between sphere centers
        float halfHeight = (height - 2 * radius) * 0.5f;
        
        // Calculate the sphere centers
        Vector3 topSphereCenter = center + directionVector * halfHeight;
        Vector3 bottomSphereCenter = center - directionVector * halfHeight;
        
        // Create points for the perimeter
        int segments = 48; // More segments for smoother curve
        perimeterLineRenderer.positionCount = segments;
        ...
        // Generate the points around the perimeter
        for (int i = 0; i < segments; i++)
        {
            float angle = (2 * Mathf.PI * i) / segments;
            
            // Calculate the unit circle position
            Vector3 unitCirclePos = perpendicular1 * Mathf.Cos(angle) + perpendicular2 * Mathf.Sin(angle);
            
            // Calculate position on the capsule surface
            Vector3 pointOnCapsule;
            
            // If in the top half, use the top sphere
            if (angle <= Mathf.PI)
            {
                pointOnCapsule = topSphereCenter + unitCirclePos * radius;
            }
            // Otherwise use the bottom sphere
            else
            {
                pointOnCapsule = bottomSphereCenter + unitCirclePos * radius;
            }
            
            // Fix the height for flat rendering
            pointOnCapsule.y = perimeterHeight;
            
            perimeterLineRenderer.SetPosition(i, pointOnCapsule);
        }
    }
