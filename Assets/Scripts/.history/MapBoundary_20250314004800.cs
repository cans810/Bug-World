...
public class MapBoundary : MonoBehaviour
{
    ...
    private void CreateSphericalBoundary()
    {
        ...
        // Add a floor beneath the boundary to catch falling entities
        CreateFloorCollider(boundaryParent, boundaryRadius);
    ...
    // Add a floor collider to prevent entities from falling
    private void CreateFloorCollider(Transform parent, float radius)
    {
        GameObject floor = new GameObject("Floor_Collider");
        floor.transform.SetParent(parent);
        
        // Position the floor slightly below the lowest point of terrain
        // Find approximate lowest terrain point
        float floorY = transform.position.y - 10f; // Default 10 units below boundary center
        ...
        floorCollider.size = new Vector3(radius * 2.1f, 1f, radius * 2.1f); // Make it slightly wider than the boundary
        floorCollider.isTrigger = false; // Make it a solid collider
        
        // Tag it so we can identify it
        floor.tag = "BoundaryFloor";
    }
