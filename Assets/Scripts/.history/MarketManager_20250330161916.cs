...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == "doubleThreat")
        {
            ...
            if (incubator != null)
            {
                // Create one scorpion egg directly
                GameObject newEgg = incubator.CreateMultipleEggs("scorpion", 1);
                ...
                if (newEgg != null)
                {
                    ...
                    // Show the new egg after a short delay
                    StartCoroutine(ShowNewEggAfterDelay(newEgg, 0.5f));
                }
                else
                {
    ...
    // Add this coroutine to show the egg after a short delay
    private IEnumerator ShowNewEggAfterDelay(GameObject eggObject, float delay)
    {
        yield return new WaitForSeconds(delay);
        
        // Find camera animations
        CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
        if (cameraAnimations != null && eggObject != null)
        {
            Debug.Log($"Showing new egg: {eggObject.name}");
            
            // Animate to the egg
            cameraAnimations.AnimateToArea(eggObject.transform, 0, "New Scorpion Egg");
        }
        else
        {
            Debug.LogWarning("Could not show egg: " + 
                (cameraAnimations == null ? "CameraAnimations not found" : "Egg object is null"));
        }
    }
}
