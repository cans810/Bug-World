    ...
    private IEnumerator AnimateResourceCollection(GameObject symbol, string type)
    {
        ...
        if (type == "crumb" && animateToEquippedEntity)
        {
            ...
            if (entityController != null)
            {
                ...
                {
                    // Convert counter position to screen space to properly animate UI elements
                    Vector3 counterScreenPos = Camera.main.WorldToScreenPoint(counterTransform.position);
                    
                    // Create a new starting position at the nest
                    startPos = nestTransform.position;
                    
                    // Calculate a proper end position - this is key to fixing the animation
                    // Create a ray from the camera to the counter's screen position
                    Ray ray = Camera.main.ScreenPointToRay(counterScreenPos);
                    
                    // Get a point 10 units away from camera along this ray
                    // This ensures we're using world space that corresponds to the counter's screen position
                    float distanceToCamera = 10f;
                    endPos = ray.GetPoint(distanceToCamera);
                    
                    // Debug log to help track the positions
                    Debug.Log($"Counter screen pos: {counterScreenPos}, World end pos: {endPos}");
                    
                    // Reset elapsed time for second animation
                    elapsed = 0f;
                    float secondPhaseDuration = chitinFlyDuration * 0.6f; // Make this phase a bit faster
                    
                    // Create a new midpoint for second curve
                    midPoint = Vector3.Lerp(startPos, endPos, 0.5f);
                    midPoint += Vector3.up * Random.Range(0.5f, 1.5f);
                    
                    while (elapsed < secondPhaseDuration)
                    {
                        elapsed += Time.deltaTime;
                        float normalizedTime = elapsed / secondPhaseDuration;
                        float curveValue = chitinFlyCurve.Evaluate(normalizedTime);
