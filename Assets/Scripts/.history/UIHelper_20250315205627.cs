...
public class UIHelper : MonoBehaviour
{
    ...
    private void OnPlayerLevelUp(int level)
    {
        ...
        if (levelUpEffect != null)
        {
            ...
            StartCoroutine(HideLevelUpEffectAfterDelay(3f));
        }
        
        // Trigger camera animation
        if (cameraAnimations != null)
        {
            // Use the correct animation method - based on what's available in CameraAnimations
            // Most likely it's something like ShowArea or PlayAnimation
            cameraAnimations.ShowArea(null, level, "LevelUp");
            // Or alternatively:
            // cameraAnimations.PlayAnimation("LevelUp");
            
            shouldShowAttributesPanel = true;
        }
        else if (attributesPanel != null)
        {
            // If no camera animation, show the panel directly
