...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        else
        {
            ...
            // Calculate a blended rotation direction - blend toward player position and player's forward direction
            Vector3 playerForward = playerTransform.forward;
            playerForward.y = 0; // Keep on same Y level
            
            // Blend 70% direction to player, 30% player's forward direction
            Vector3 blendedDirection = (directionToPlayer * 0.7f + playerForward * 0.3f).normalized;
            
            // First rotate consistently using the consistent rotation method
            livingEntity.RotateTowards(blendedDirection, rotationMultiplier);
            
            // Then move using the consistent movement method
