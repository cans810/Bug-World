...
public class PlayerController : MonoBehaviour
{
    ...
    public void OnAttackButtonPressed()
    {
        ...
        if (livingEntity != null)
        {
            // Check if we're walking (currentSpeed > 0.1f) to determine which animation to play
            ...
            if (attackResult)
            {
                // If walking, play walkAttack animation, otherwise play regular attack
                if (isWalking && animController != null)
                {
                    animController.PlayAnimation("walkAttack");
                }
            ...
            else if (uiHelper != null)
            {
                ...
                // Only show the message if cooldown is the reason
                if (remainingCooldown > 0)
                {
                    // Start a coroutine to update the cooldown timer in the UI
