...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        while (elapsed < duration)
        {
            ...
            // Force end of animation if we're close enough to the end
            if (normalizedTime > 0.95f)
            {
                Debug.Log($"Forcing end of animation for symbol {symbolId}");
                break; // Exit the loop to ensure cleanup happens
            }
            ...
            // Enhanced fade out - start fading earlier and more aggressively
            if (normalizedTime > 0.75f)
            {
                float fadeProgress = (normalizedTime - 0.75f) / 0.25f;
