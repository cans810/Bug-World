...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Further increase animation duration to ensure completion
        float duration = 1.8f; // Increased from 1.5f to 1.8f
        ...
        // Create a visual indicator at the target position (DEBUG ONLY)
        GameObject debugMarker = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        debugMarker.transform.position = targetWorldPos;
        debugMarker.transform.localScale = new Vector3(0.2f, 0.2f, 0.2f);
        Destroy(debugMarker, duration + 0.5f);
        
        // Calculate a FIXED midpoint for the arc (not random per frame)
        // Lower the arc height for a more direct path
        float arcHeight = Random.Range(0.7f, 1.5f);
        Vector3 fixedMidPoint = Vector3.Lerp(startPos, targetWorldPos, 0.5f) + Vector3.up * arcHeight;
        
        // Create a unique ID for debugging
        string symbolId = System.Guid.NewGuid().ToString().Substring(0, 8);
        Debug.Log($"Starting coin animation for symbol {symbolId} to target {targetWorldPos}");
        
        try
        {
            // Error checking before starting the loop
            if (symbol == null || !symbol.activeInHierarchy)
            {
                Debug.LogWarning($"Coin symbol {symbolId} is null or inactive before animation");
                throw new System.NullReferenceException("Symbol object is null or inactive");
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"Error initializing coin animation for {symbolId}: {e.Message}");
            // Early cleanup
            CleanupCoinSymbol(symbol, spriteRenderer, startColor, initialScale);
            yield break;
        }
        ...
        while (elapsed < duration)
        {
            // Safety check in case something was destroyed during animation
            if (symbol == null || !symbol.activeInHierarchy || spriteRenderer == null)
            {
                Debug.LogWarning($"Coin symbol {symbolId} became null or inactive during animation");
                yield break;
            }
            ...
            // Simplified path calculation for last 20% of animation
            if (normalizedTime > 0.8f)
            {
                // Direct linear interpolation to target in final stage
                float finalProgress = (normalizedTime - 0.8f) / 0.2f;
                
                // Use exponential easing to accelerate toward target
                float easedProgress = 1f - Mathf.Pow(1f - finalProgress, 3); // Cubic ease-out
                
                // Move directly to target position
                symbol.transform.position = Vector3.Lerp(
                    symbol.transform.position, 
                    targetWorldPos, 
                    easedProgress * 0.2f); // Smooth approach with 20% step
                
                // Keep visible until very end
                if (normalizedTime > 0.95f)
                {
                    // Only fade in last 5% of animation
                    float fadeAmount = (normalizedTime - 0.95f) / 0.05f;
                    spriteRenderer.color = new Color(startColor.r, startColor.g, startColor.b, 
                        Mathf.Lerp(1f, 0f, fadeAmount));
                }
            }
            else
            {
                // Normal bezier curve for first 80% of animation
                float curveValue = chitinFlyCurve.Evaluate(normalizedTime);
            ...
            if (!hasBumpedUI && normalizedTime > 0.65f) // Trigger earlier
            {
                TriggerUICounterAnimation("coin");
                hasBumpedUI = true;
            }
        ...
        // Force position to exact target at the end
        if (symbol != null && symbol.activeInHierarchy)
        {
            symbol.transform.position = targetWorldPos;
            // Wait one frame at the destination before cleanup
            yield return null;
        }
        
        // Final cleanup with debug log
        Debug.Log($"Animation complete for symbol {symbolId}, cleaning up");
        CleanupCoinSymbol(symbol, spriteRenderer, startColor, initialScale);
    }

    // Improved cleanup method with additional logging
    private void CleanupCoinSymbol(GameObject symbol, SpriteRenderer spriteRenderer, Color startColor, Vector3 initialScale)
    {
        if (symbol == null) 
        {
            Debug.Log("Attempted cleanup on null symbol");
            return;
        }
        ...
        {
            Debug.Log($"Deactivating coin symbol in cleanup");
            symbol.SetActive(false);
        }
        
        // Make sure the symbol is properly returned to the pool
        if (symbolPools.ContainsKey("coin"))
        {
            Debug.Log($"Returning coin symbol to pool");
            symbolPools["coin"].Enqueue(symbol);
        }
        else
        {
            Debug.LogWarning("Coin pool doesn't exist during cleanup!");
        }
    }
    ...
    private IEnumerator FlyChitinToNest(GameObject chitinSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            {
                TriggerUICounterAnimation("chitin");
    ...
    private IEnumerator FlyCrumbToInventory(GameObject crumbSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)  // Using same duration as chitin for consistency
        {
            ...
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                TriggerUICounterAnimation("crumb");
                effectsTriggered = true;
            }
    ...
    // Method to ensure all resource counter text elements are consistently animated
    private void TriggerUICounterAnimation(string resourceType)
    {
        // Get the UIHelper
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper == null) return;
        
        // Simplify the animation handling - use safer methods that don't require us to 
        // directly access properties from PlayerInventory
        switch (resourceType.ToLower())
        {
            case "coin":
                // We know this method exists and works
                uiHelper.UpdateCoinDisplay(FindObjectOfType<PlayerCoins>().GetCoins());
                break;
            ...
            case "crumb":
            case "egg":
                // For other resources, use a method to pulse the appropriate UI element
                PulseResourceCounter(uiHelper, resourceType);
                break;
    ...
    // Helper method to pulse the appropriate counter based on resource type
    private void PulseResourceCounter(UIHelper uiHelper, string resourceType)
    {
        // Get the appropriate panel for the resource type
        ...
        switch (resourceType.ToLower())
        {
            ...
            case "crumb":
                ...
                break;
            default:
                Debug.LogWarning($"No panel assigned for resource type: {resourceType}");
                return;
        }
        
        if (panel != null)
        {
            // Find the Text component in the panel's children
            ...
            if (textComponent != null)
            {
                ...
                if (textRect != null)
                {
                    StartCoroutine(PulseUIElement(textRect));
                }
            }
            else
            {
                // Fallback to pulsing the panel itself
                RectTransform panelRect = panel.GetComponent<RectTransform>();
                if (panelRect != null)
                {
