...
public class BorderVisualizer : MonoBehaviour
{
    ...
    {
        // Special case for OuterBase1
        if (gameObject.name.Contains("OuterBase1"))
        {
            if (entityGenerator != null)
            {
                entityGenerator.EnableSpawning(true); // Always enable spawning for OuterBase1
                spawningEnabled = true;
                Debug.Log($"Special case: Spawning for {gameObject.name} always enabled");
            }
            return;
        }

        // Normal case for all other areas
        if (controlsEntitySpawning && entityGenerator != null)
        {
