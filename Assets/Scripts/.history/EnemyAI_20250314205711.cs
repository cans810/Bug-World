...
public class EnemyAI : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        {
            // Calculate direction AWAY from the border center and toward the playable area
            Vector3 directionToCenter = (other.bounds.center - transform.position).normalized;
            
            // We want to move TOWARD the center of the playable area
            Vector3 pushDirection = -directionToCenter;
            pushDirection.y = 0;
            pushDirection.Normalize();
            
            // Move back inside the boundary
            transform.position += pushDirection * 1.0f;
            
            // Get current forward direction
            Vector3 currentDirection = transform.forward;
            
            // Calculate the reflection direction
            Vector3 reflectionDirection = Vector3.Reflect(currentDirection, pushDirection);
            ...
            // Tell the wandering behavior to continue in this new direction
            if (wanderingBehavior != null)
            {
