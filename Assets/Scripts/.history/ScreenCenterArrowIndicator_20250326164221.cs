...
public class ScreenCenterArrowIndicator : MonoBehaviour
{
    ...
    [SerializeField] private float arrowSize = 0.05f;
    ...
    [SerializeField] private float verticalPositionOffset = -0.5f; // More negative value places the arrow lower
    [SerializeField] private bool useScreenRelativePosition = true;
    [SerializeField] [Range(0.0f, 1.0f)] private float screenHeightPosition = 0.3f; // 0 = bottom, 1 = top
    
    // Add a new option to position relative to player
    [SerializeField] private bool positionRelativeToPlayer = true;
    [SerializeField] private float heightAbovePlayer = 0.1f; // Height offset above player
    ...
    private void PositionArrowInView()
    {
        ...
        if (positionRelativeToPlayer && playerTransform != null)
        {
            // Get player position in screen space
            Vector3 playerScreenPos = mainCamera.WorldToScreenPoint(playerTransform.position);
            
            // Add the height offset (in world units)
            Vector3 playerWorldPos = playerTransform.position;
            playerWorldPos.y += heightAbovePlayer;
            
            // Project to screen space to find X
            Vector3 playerPosWithHeight = mainCamera.WorldToScreenPoint(playerWorldPos);
            
            // Create final screen position (using player's X position but at specified screen height)
            Vector3 screenPosition = new Vector3(
                playerScreenPos.x, 
                playerPosWithHeight.y,
                playerScreenPos.z);
        ...
        else if (useScreenRelativePosition)
        {
            // Position arrow at a specific height percentage of the screen
            Vector3 screenPosition = new Vector3(Screen.width * 0.5f, Screen.height * screenHeightPosition, distanceFromCamera);
            Vector3 worldPosition = mainCamera.ScreenToWorldPoint(screenPosition);
        ...
        {
            // Use the camera-relative approach with the vertical offset
            Vector3 forward = mainCamera.transform.forward;
            Vector3 up = mainCamera.transform.up;
            
            // Calculate position with vertical offset
            Vector3 offsetPosition = mainCamera.transform.position + 
                                    forward * distanceFromCamera + 
                                    up * verticalPositionOffset;
