...
public class CameraAnimations : MonoBehaviour
{
    ...
    {
        // Subscribe to the LevelAreaArrowManager when enabled
        LevelAreaArrowManager arrowManager = FindObjectOfType<LevelAreaArrowManager>();
        if (arrowManager != null)
        {
            // Unsubscribe first to avoid multiple subscriptions
            ...
            // Subscribe again
            arrowManager.OnAreaUnlocked += OnAreaUnlocked;
            Debug.Log("CameraAnimations: Successfully subscribed to LevelAreaArrowManager events (OnEnable)");
        }
    }
    ...
    {
        Debug.Log($"CameraAnimations.OnAreaUnlocked called: area={areaName}, level={level}, isLoading={isLoadingData}");
        
        // Skip animations if we're loading data
        if (isLoadingData)
        {
            Debug.Log("CameraAnimations: Skipping animation because isLoadingData=true");
            return;
        }
        
        if (areaTarget != null)
        {
            Debug.Log($"CameraAnimations: Starting animation coroutine for {areaName}");
            StartCoroutine(AnimateCameraToArea(areaTarget.transform, level, areaName));
        }
        else
        {
            Debug.LogError("CameraAnimations: Cannot animate to null areaTarget!");
        }
    }
    ...
    {
        Debug.Log($"CameraAnimations: Attempting to show area for level {level}");
            ...
            if (areasField != null)
            {
                ...
                if (areas != null)
                {
                    foreach (var area in areas)
                    {
                        ...
                        {
                            Debug.Log($"CameraAnimations: Found area to show for level {level}: {area.areaName}");
                            StartCoroutine(AnimateCameraToArea(area.areaTarget.transform, level, area.areaName));
                            return;
                        }
                    }
                    Debug.Log($"CameraAnimations: No area found with required level {level}");
                }
                else
                {
                    Debug.LogWarning("CameraAnimations: Could not access levelAreas array");
                }
            }
            else
            {
                Debug.LogWarning("CameraAnimations: Could not find levelAreas field via reflection");
            }
        }
        else
        {
    ...
    public void TestAnimation(int level)
    {
        Debug.Log($"CameraAnimations: Testing animation for level {level}");
        ShowAreaByLevel(level);
    }
} 