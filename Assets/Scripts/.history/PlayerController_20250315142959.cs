...
public class PlayerController : MonoBehaviour
{
    ...
    // Modify the OnAttackButtonPressed method to stop movement and just attack
    public bool OnAttackButtonPressed()
    {
        ...
        if (livingEntity != null)
        {
            ...
            // If attack was successful, play the attack animation
            if (attackResult)
            {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // We're back inside the playable area
            isAtMapBorder = false;
    ...
    // Now let's fix the auto-attack detection by making it more strict
    private IEnumerator CheckForEnemiesInRange()
    {
        while (enabled)
        {
            ...
            // Only run this check if we don't already have enemies in range
            if (enemiesInRange.Count == 0)
            {
                // Use a very small radius for precise detection
                float attackRange = 2f; // Reduce to 2 meters for more precise detection
                ...
                {
                    // Check if this is an enemy - be very strict about the layer check
                    if (col.gameObject.layer == LayerMask.NameToLayer("Insects"))
                    {
                        LivingEntity enemy = col.GetComponent<LivingEntity>();
                        if (enemy != null && !enemy.IsDead)
                        {
                            // Check if the enemy is actually in front of the player and close enough
                            Vector3 directionToEnemy = (enemy.transform.position - transform.position).normalized;
                            float dotProduct = Vector3.Dot(transform.forward, directionToEnemy);
                            float distance = Vector3.Distance(transform.position, enemy.transform.position);
                            
                            // Only consider enemies that are directly in front of the player (within ~60 degrees)
                            // and within a very close range
                            if (dotProduct > 0.5f && distance < 2f)
                            {
                                foundEnemies = true;
                                ...
                                if (!enemiesInRange.Contains(enemy))
                                {
                                    ...
                                    if (showDebugMessages)
                                    {
                                        Debug.Log($"Found enemy in range: {enemy.name}, distance: {distance}, angle: {Mathf.Acos(dotProduct) * Mathf.Rad2Deg}");
                                    }
                                }
                            }
                        }
                ...
                // If we found enemies and auto-attack isn't running, start it
                if (foundEnemies && autoAttackEnabled && autoAttackCoroutine == null)
                {
                    if (showDebugMessages)
                    {
                        Debug.Log("Starting auto-attack from periodic check");
                    }
            ...
            for (int i = enemiesInRange.Count - 1; i >= 0; i--)
            {
                ...
                if (enemy == null || enemy.IsDead || 
                    Vector3.Distance(transform.position, enemy.transform.position) > 3f)
                {
    ...
    // Improved auto-attack coroutine
    private IEnumerator AutoAttackCoroutine()
    {
        ...
        while (enabled && livingEntity != null && !livingEntity.IsDead && autoAttackEnabled)
        {
            ...
            if (enemiesInRange.Count > 0)
            {
                // Find the closest enemy to face
                ...
                // Face the closest enemy before attacking
                if (closestEnemy != null)
                {
                    Vector3 directionToEnemy = (closestEnemy.transform.position - transform.position).normalized;
                    livingEntity.RotateTowards(directionToEnemy, 10f); // Faster rotation for auto-attack
                    
                    // Wait a small amount of time to allow rotation
                    yield return new WaitForSeconds(0.1f);
                }
                
                if (showDebugMessages)
                {
                    Debug.Log($"Attempting auto-attack with {enemiesInRange.Count} enemies in range");
                }
                
                // Try to attack
                bool attackSuccessful = OnAttackButtonPressed();
                
                if (showDebugMessages)
                {
                    Debug.Log($"Attack attempt result: {(attackSuccessful ? "Success" : "Failed")}");
                }
                
                // If attack was successful, wait for the cooldown to finish
                if (attackSuccessful)
                {
                    // Wait until the attack cooldown is complete
