...
public class PlayerController : MonoBehaviour
{
    ...
    private void ApplyAntMovementEffects()
    {
        ...
        {
            // Update wobble time
            wobbleTime += Time.deltaTime * bodyWobbleSpeed * (currentSpeed / livingEntity.moveSpeed);
            
            // Calculate wobble offset - ants have a slight side-to-side motion when walking
            float xOffset = Mathf.Sin(wobbleTime * 2f) * bodyWobbleAmount * (currentSpeed / livingEntity.moveSpeed);
            float yOffset = Mathf.Abs(Mathf.Sin(wobbleTime)) * bodyWobbleAmount * 0.5f * (currentSpeed / livingEntity.moveSpeed);
