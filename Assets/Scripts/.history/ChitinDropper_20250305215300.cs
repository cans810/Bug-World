...
public class ChitinDropper : MonoBehaviour
{
    ...
    [SerializeField] private float scatterRadius = 0.0001f;
    [SerializeField] private float dropHeight = 0.08f;
    
    [Header("Physics Forces")]
    [SerializeField] private float upwardForceMin = 0.5f; // Minimum upward force
    [SerializeField] private float upwardForceMax = 1.0f; // Maximum upward force
    [SerializeField] private float horizontalForce = 0.05f; // Very low horizontal force
    ...
    private IEnumerator SpawnChitinPieces(int amount)
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            if (rb != null)
            {
                // Apply stronger upward force with minimal horizontal scatter
                float upForce = Random.Range(upwardForceMin, upwardForceMax);
                Vector3 force = new Vector3(
                    Random.Range(-horizontalForce, horizontalForce), // Very small horizontal X force
                    upForce, // Stronger upward force
                    Random.Range(-horizontalForce, horizontalForce)  // Very small horizontal Z force
                );
