...
public class AIWandering : MonoBehaviour
{
    ...
    // Revised border handling to stop at boundary like the player
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (other is SphereCollider sphereCollider)
            {
                ...
                // Calculate direction from entity to sphere center (inward)
                inwardDirection = (sphereCenter - transform.position).normalized;
            }
            else
            {
                // For non-sphere colliders, use closest point
                Vector3 closestPoint = other.ClosestPoint(transform.position);
                // Direction from entity to closest point (inward)
                inwardDirection = (closestPoint - transform.position).normalized;
            }
            
            // Keep on same Y level and ensure normalized
            inwardDirection.y = 0;
            inwardDirection.Normalize();
            
            // Move entity back inside boundary with stronger push
            transform.position += inwardDirection * 0.5f;
            ...
            // Set rotation to face inward
            transform.rotation = Quaternion.LookRotation(inwardDirection);
            
            // Start a coroutine to set a new safe waypoint after a delay
            StartCoroutine(SetSafeWanderingAfterDelay(inwardDirection));
        }
    }

    // New coroutine to resume wandering in a safe direction after a delay
    private IEnumerator SetSafeWanderingAfterDelay(Vector3 safeDirection)
    {
        // Wait a short time before starting to wander again
        yield return new WaitForSeconds(1.0f);
        
        // Add some randomness to the safe direction
        Vector3 randomizedDirection = Quaternion.Euler(0, Random.Range(-45f, 45f), 0) * safeDirection;
        
        // Set a waypoint at a greater distance to ensure it's well inside the boundary
        float safeDistance = maxWanderDistance;
        Vector3 newWaypoint = transform.position + randomizedDirection * safeDistance;
        
        // Set the new waypoint
        currentWaypoint = newWaypoint;
        
        // Reset state to start moving
        isWaiting = false;
        isWandering = true;
        ...
        // Start a new wandering routine in the safe direction
        StartCoroutine(SafeMovement(randomizedDirection, safeDistance));
    }

    // New coroutine to move in a safe direction after hitting a boundary
    private IEnumerator SafeMovement(Vector3 direction, float distance)
    {
        ...
        float moveDuration = distance / livingEntity.moveSpeed;
        float moveTimer = 0;
        float distanceMoved = 0;
        
        // Move in straight line in the safe direction
        while (moveTimer < moveDuration && distanceMoved < distance)
        {
            if (!isWandering)
                break;
        ...
        // After safe movement, resume normal wandering behavior
        StartCoroutine(WanderingRoutine());
    }
