...
public class ChitinDropper : MonoBehaviour
{
    ...
    // Always draw the gizmo, not just when selected
    private void OnDrawGizmos()
    {
        DrawChitinDropGizmo();
    }

    // Also draw when selected (will make it more visible when selected)
    private void OnDrawGizmosSelected()
    {
        DrawChitinDropGizmo();
    }

    // Helper method to draw the gizmo
    private void DrawChitinDropGizmo()
    {
        // Make sure to use the current transform position
        Vector3 spawnPosition = transform.position;
        ...
        // Draw a vertical line down to help visualize height
        Gizmos.DrawLine(spawnPosition, new Vector3(spawnPosition.x, spawnPosition.y - 1f, spawnPosition.z));
