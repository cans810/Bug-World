...
public class EntityHitbox : MonoBehaviour
{
    ...
    private void Update()
    {
        // Add a periodic check to validate targets are still in actual range
    ...
    {
        // Get the list of targets from the owner
        List<LivingEntity> targetsToRemove = new List<LivingEntity>();
        
        foreach (var target in owner.GetTargetsInRange())
        {
            // Skip null targets
            if (target == null) continue;
            
            // Check if target is too far away (outside realistic range)
            float distanceToTarget = Vector3.Distance(owner.transform.position, target.transform.position);
            if (distanceToTarget > 10f) // 10 units is a reasonable max auto-attack range
            {
                targetsToRemove.Add(target);
                
                if (showDebugMessages)
                {
                    Debug.Log($"Removing distant target {target.name} at distance {distanceToTarget}");
                }
                
                // Trigger exit event to stop auto-attack
                OnPlayerExitHitbox?.Invoke(target);
            }
        }
        ...
        {
            owner.RemoveTargetInRange(target);
        }
    }
} 