...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Get the starting position (use chitinCountText if available)
        Vector3 startPosition = chitinCountText != null ? 
            chitinCountText.transform.position : 
            new Vector3(Screen.width * 0.1f, Screen.height * 0.9f, 0);
        ...
        for (int i = 0; i < maxIconsToShow; i++)
        {
            ...
            StartCoroutine(MoveIcon(
                chitinIcon, 
                startPosition, 
                posToMoveIconTransform.position + randomOffset, 
                0.5f, 
                () => {
                    // After reaching intermediate position, wait and then move to final position
                    StartCoroutine(WaitAndMoveToFinal(chitinIcon, posToMoveIconTransform.position + randomOffset));
                }
            ));
    ...
    private IEnumerator WaitAndMoveToFinal(GameObject icon, Vector3 currentPosition)
    {
    ...
    private IEnumerator MoveIcon(GameObject icon, Vector3 startPos, Vector3 endPos, float duration, System.Action onComplete = null, bool addEffects = false)
    {
