...
public class EntityOutline : MonoBehaviour
{
    ...
    private void RemoveOutlinesFromExcludedObjects()
    {
        ...
        foreach (Transform child in allChildren)
        {
            ...
            {
                // Remove any existing Outline component
                Outline childOutline = child.GetComponent<Outline>();
                if (childOutline != null)
                {
                    Debug.Log($"Removing outline from excluded object: {child.name} on layer {LayerMask.LayerToName(child.gameObject.layer)}");
                    Destroy(childOutline);
                }
                
                // Also check for renderers with outline materials and fix them
                ...
                if (renderer != null)
                {
                    ...
                    // Create a new array without outline materials
                    List<Material> cleanedMaterials = new List<Material>();
                    
                    foreach (Material mat in materials)
                    {
                        if (mat != null && !mat.name.Contains("Outline"))
                        {
                            cleanedMaterials.Add(mat);
                        }
                        else if (mat != null && mat.name.Contains("Outline"))
                        {
                            materialChanged = true;
                            Debug.Log($"Removed outline material from {child.name}");
                        }
                    }
                    
                    // Apply the cleaned materials if we removed any outline materials
                    if (materialChanged)
                    {
                        renderer.sharedMaterials = cleanedMaterials.ToArray();
                    }
                }
            }
        }
