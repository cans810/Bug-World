...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinSymbols(int count, int xpAmount)
    {
        ...
        // Spawn chitin symbols with minimal delay
        for (int i = 0; i < count; i++)
        {
            ...
            // Start animation to line up above center immediately
            StartCoroutine(AnimateChitinToLineup(chitinSymbol, screenCenter, i, count));
            
            // Very minimal delay between spawning symbols
            yield return new WaitForSeconds(0.05f);
        }
        
        // Wait briefly for chitins to get closer to their positions, but don't wait for full completion
