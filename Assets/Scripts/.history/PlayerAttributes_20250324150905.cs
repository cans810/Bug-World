...
public class PlayerAttributes : MonoBehaviour
{
    ...
    public void AddAttributePoints(int pointsToAdd)
    {
        ...
        // Add a more detailed debug message with stack trace info
        System.Diagnostics.StackTrace stackTrace = new System.Diagnostics.StackTrace();
        string callerMethod = stackTrace.GetFrame(1)?.GetMethod()?.Name ?? "unknown";
        string callerClass = stackTrace.GetFrame(1)?.GetMethod()?.DeclaringType?.Name ?? "unknown";
        
        Debug.Log($"<color=yellow>ATTRIBUTE POINTS ADDED</color>: {pointsToAdd} points " +
                  $"(Old: {oldPoints}, New: {availablePoints}) " +
    ...
    // Check for any attack-related messages
    public void UpdateAttackStats()
    {
        // ... other code ...

        // If showing cooldown information, use the correct text component
        if (uiHelper != null && baseAttackCooldown > 0)
        {
            uiHelper.ShowAttackCooldownText($"Cooldown: {baseAttackCooldown:F1}s");
        }
