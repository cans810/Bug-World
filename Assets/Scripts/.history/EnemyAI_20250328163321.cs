...
public class EnemyAI : MonoBehaviour
{
    ...
    // Updated coroutine to make enemy face the player with a quick but smooth rotation
    public IEnumerator FacePlayerWhileInHitbox(LivingEntity playerEntity)
    {
        while (isPlayerInHitbox && !livingEntity.IsDead && playerEntity != null && !playerEntity.IsDead)
        {
            ...
            if (directionToPlayer.magnitude > 0.1f)
            {
                ...
                // Perform a quick but smooth rotation (over 0.1 seconds)
                float duration = 0.1f;
                float elapsed = 0f;
                ...
                // Wait for a short period before checking player position again
