...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float idleWanderRadius = 2.0f;
    [SerializeField] private float idleWanderSpeed = 0.7f;
    [SerializeField] private float idleWanderChangeDirectionTime = 2.0f;
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (!isPlayerMoving && distanceToPlayer <= followDistance * 1.5f)
        {
            ...
            if (wanderWhenPlayerIdle)
            {
                // Start idle wandering if not already
                if (!isIdleWandering)
                {
                    isIdleWandering = true;
                    SetNewIdleWanderTarget();
                }
    ...
    private void SetNewIdleWanderTarget()
    {
        // Reset timer
        idleWanderTimer = idleWanderChangeDirectionTime;
        
        // Get a random point within the wander radius around the player
        Vector2 randomCircle = Random.insideUnitCircle * idleWanderRadius;
        Vector3 targetPosition = playerTransform.position + new Vector3(randomCircle.x, 0, randomCircle.y);
        
        // Make sure the target is within map boundaries
        if (mapBoundary != null)
        {
            targetPosition = mapBoundary.GetNearestPointInBounds(targetPosition);
        }
        
        idleWanderTarget = targetPosition;
    }

    private void HandleIdleWandering()
    {
        // Update timer
        idleWanderTimer -= Time.deltaTime;
        
        // Check if we need a new target
        if (idleWanderTimer <= 0 || Vector3.Distance(transform.position, idleWanderTarget) < 0.3f)
        {
            SetNewIdleWanderTarget();
        }
        
        // Calculate direction to wander target
        Vector3 directionToTarget = (idleWanderTarget - transform.position).normalized;
        
        // Apply ally avoidance
        Vector3 moveDirection = ApplyAllyAvoidance(directionToTarget);
        ...
        // Move at a slower pace for idle wandering
        Vector3 nextPosition = transform.position + moveDirection * (livingEntity.moveSpeed * idleWanderSpeed) * Time.deltaTime;
        
        // Apply movement with boundary check
        if (mapBoundary != null && !mapBoundary.IsWithinBounds(nextPosition))
        {
            // Get a new target if we hit a boundary
            SetNewIdleWanderTarget();
        }
        else
        {
