...
public class PlayerController : MonoBehaviour
{
    ...
    // Update the attack method to properly register the player as the killer
    public void PerformAttack()
    {
        ...
        // Try to get target from enemies in range (which seems to be how the game tracks enemies)
        if (enemiesInRange != null && enemiesInRange.Count > 0)
        {
            // Get the first non-dead enemy in range
