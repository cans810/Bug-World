...
public class InsectIncubator : MonoBehaviour
{
    ...
    public void OnInsectHatched(GameObject newInsect)
    {
        ...
        // Make sure AllyAI is enabled
        AllyAI allyAI = newInsect.GetComponent<AllyAI>();
        if (allyAI != null)
        {
            allyAI.enabled = true;
            allyAI.ForceEnableMovement();
            Debug.Log($"Ensuring AllyAI is enabled on newly hatched {newInsect.name}");
        }
        
        // Ensure AIWandering is properly configured
        AIWandering wandering = newInsect.GetComponent<AIWandering>();
        if (wandering != null)
        {
            wandering.SetWanderingEnabled(true);
            Debug.Log($"Ensuring wandering is enabled on newly hatched {newInsect.name}");
        }
