...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    // Add these fields at the top of the MarketManager class
    private bool isProcessingPurchase = false;
    private float purchaseProcessCooldown = 1.0f;
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == "doubleThreat")
        {
            ...
            if (incubator != null)
            {
                ...
                if (success)
                {
                    ...
                    GameObject eggToShow = eggs.Length > 0 ? eggs[0] : GameObject.Find("AntEggPile");
        ...
        StartCoroutine(ResetPurchaseState(0.5f));
        return PurchaseProcessingResult.Complete;
    }
    ...
    public void BuyProduct(string productId)
    {
        ...
        // Don't allow purchase if we're already processing one
        if (isProcessingPurchase) {
            Debug.Log("Already processing a purchase. Please wait.");
            return;
        }
        ...
        if (productId == "doubleThreat")
        {
            ...
            if (incubator != null)
            {
                ...
                if (!hasSpace)
                {
                    ...
                    {
                        string message = eggExists ?
                            "An egg is already incubating! Cannot purchase." :
                            "Not enough incubation space! Cannot purchase.";
                    ...
                    // Start a coroutine to reset after a short time
                    StartCoroutine(ResetPurchaseState(0.5f));
        ...
        // Mark that we're now processing a purchase
        ...
        else
        {
            ...
            isProcessingPurchase = false; // Reset state
        }
    }
    ...
    public void HideMarketPanel()
    {
        ...
        Debug.Log("Market panel hidden - will be reopenable after short cooldown");
    }

    public void ShowMarketPanel()
    {
        // Check if we're currently processing a purchase
        if (isProcessingPurchase) {
            Debug.Log("Cannot show market panel - still processing last purchase");
            return;
        }
        
        // Check for cooldown after a purchase failure
        if (Time.time - lastPurchaseTime < purchaseProcessCooldown) {
            Debug.Log($"Market panel on cooldown. Try again in {purchaseProcessCooldown - (Time.time - lastPurchaseTime):F1} seconds");
            return;
        }
        
        // If we pass the checks, show the panel
    ...
    // Add this method to reset purchase state
    private IEnumerator ResetPurchaseState(float delay) {
        yield return new WaitForSeconds(delay);
        isProcessingPurchase = false;
        Debug.Log("Purchase state reset - market can be reopened");
    }
}
