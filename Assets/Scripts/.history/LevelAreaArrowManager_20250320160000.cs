...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    private void OnPlayerLevelUp(int newLevel)
    {
        ...
        foreach (LevelAreaTarget area in levelAreas)
        {
            ...
            {
                foundUnlockedArea = true;
                Debug.Log($"LevelAreaArrowManager: Found newly unlocked area: {area.areaName}");
                ...
                {
                    Debug.Log($"LevelAreaArrowManager: Firing OnAreaUnlocked event for {area.areaName}");
                    OnAreaUnlocked(area.areaTarget, newLevel, area.areaName);
                }
                else
                {
                    Debug.LogError($"LevelAreaArrowManager: OnAreaUnlocked event is null or area target is null");
                    if (OnAreaUnlocked == null) Debug.LogError("Event is null");
                    if (area.areaTarget == null) Debug.LogError("Area target is null");
                }
        ...
        if (!foundUnlockedArea)
        {
            Debug.Log($"LevelAreaArrowManager: No newly unlocked areas found for level {newLevel}");
        }
    }
