...
public class AttributeDisplay : MonoBehaviour
{
    ...
    {
        Debug.Log("AttributeDisplay: FORCE SHOWING PANEL - bypassing ALL safety checks");
        ...
        // Temporarily disable OnEnable's behavior by setting a flag
        // that will be checked in OnEnable
        forceShowingPanel = true;
        
        // Direct activation - no checks, no conditions
        attributesPanel.SetActive(true);
        
        // Reset the flag after activation
        forceShowingPanel = false;
        
        // If animator exists, set parameters
        if (panelAnimator != null)
        {
            panelAnimator.SetBool("ShowUp", true);
            panelAnimator.SetBool("Hide", false);
        ...
        // Optional: Print panel's hierarchy path to debug
        string path = GetGameObjectPath(attributesPanel);
        Debug.Log($"Panel GameObject path: {path}");
    }

    // Helper method to print GameObject hierarchy path
    ...
    // Add this to catch any automatic showing
    private void LateUpdate()
    {
        // If panel is active but we didn't explicitly show it via player action,
        // and we're not in the middle of showing it with forceShowingPanel,
        // then something is automatically showing it - hide it
        if (attributesPanel != null && attributesPanel.activeSelf && 
            !isPlayerInitiatedAction && !forceShowingPanel)
        {
            Debug.LogWarning("AttributeDisplay: Panel activated automatically! Hiding it.");
            attributesPanel.SetActive(false);
        }
    }
} 