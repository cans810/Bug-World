...
public class UIHelper : MonoBehaviour
{
    ...
    // Update the egg display method to use the same format as other resources
    public void UpdateEggDisplay(int count)
    {
        if (eggCountText != null)
        {
            // Use the same format as other resources (current/max)
            // For eggs, max is always 1 since we only allow one egg at a time
            eggCountText.text = string.Format(countFormat, count, 1);
