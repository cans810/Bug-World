...
public class FlyingEntityAI : MonoBehaviour
{
    ...
    private void EnforceBoundaryLimits()
    {
        ...
        // If beyond or very close to boundary (95% of max radius), handle it smoothly
        if (distanceFromSpawn > maxWanderRadius * 0.95f)
        {
            // Stop current wandering routine to prevent conflicts
            if (wanderingCoroutine != null)
            {
                StopCoroutine(wanderingCoroutine);
                wanderingCoroutine = null;
            ...
            // Start boundary correction routine
            StartCoroutine(HandleBoundaryCorrection());
        }
    }
    
    private IEnumerator HandleBoundaryCorrection()
    {
        Debug.Log("Entity reached boundary, starting correction");
        
        // Calculate direction back toward spawn point
        Vector2 currentPosFlat = new Vector2(transform.position.x, transform.position.z);
        Vector2 spawnPosFlat = new Vector2(spawnPosition.x, spawnPosition.z);
        Vector2 directionToSpawn = (spawnPosFlat - currentPosFlat).normalized;
        
        // Convert to 3D direction while maintaining fixed height
        Vector3 lookDirection = new Vector3(directionToSpawn.x, 0, directionToSpawn.y);
        
        // First, smoothly rotate to face toward the spawn point
        ...
        // Ensure we have the exact target rotation
        transform.rotation = targetRotation;
        
        // Move inward for a short time
        float moveInwardDuration = 2.0f;
        float moveTimer = 0f;
        
        while (moveTimer < moveInwardDuration)
        {
            // Move toward spawn point (inward)
            if (livingEntity != null)
            {
        ...
        // Now we're safely inside the boundary, resume normal wandering
        Debug.Log("Boundary correction complete, resuming normal wandering");
