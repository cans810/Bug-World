...
public class MapBoundary : MonoBehaviour
{
    ...
    private void CreateSphericalBoundary()
    {
        ...
        // Create a game object for the boundary
        GameObject boundaryObject = new GameObject("Boundary_Trigger");
        boundaryObject.transform.SetParent(boundaryParent);
        boundaryObject.transform.position = transform.position;
        
        // Add a sphere collider set as a trigger
        SphereCollider collider = boundaryObject.AddComponent<SphereCollider>();
        collider.radius = boundaryRadius;
        collider.isTrigger = true; // Make it a trigger, not a solid collider
        
        // Add a rigidbody to make trigger detection work properly
        Rigidbody rb = boundaryObject.AddComponent<Rigidbody>();
        rb.isKinematic = true; // Make it non-physical
        
        // Add the boundary enforcer component
        BoundaryEnforcer enforcer = boundaryObject.AddComponent<BoundaryEnforcer>();
