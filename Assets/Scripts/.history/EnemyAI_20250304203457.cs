...
public class EnemyAI : MonoBehaviour
{
    ...
    private IEnumerator PlayDeathAnimationWithPriority()
    {
        ...
        if (animController != null)
        {
            ...
            if (animator != null)
            {
                // DIRECT ANIMATION CONTROL - Forces death animation to play immediately
                // 1. First reset all parameters
                animator.SetBool("Walk", false);
                animator.SetBool("Attack", false);
                animator.SetBool("Idle", false);
                animator.SetBool("Death", true);
                
                // 2. OPTION A: Force exit from attack state first by playing idle briefly
                animator.SetBool("Idle", 0, 0);
                yield return new WaitForSeconds(0.05f);
                
                // 3. Then directly play death animation by name (bypassing transitions)
                animator.SetBool("Death", 0, 0);
                
                // 4. Or alternatively, cross-fade with zero blend time:
                // animator.CrossFade("Death", 0.0f, 0, 0);
            }
