...
public class PlayerAttributes : MonoBehaviour
{
    ...
    private string _borderObjectName = "MapBorder";
    public string borderObjectName 
    {
        get { return _borderObjectName; }
        set 
        {
            if (_borderObjectName != value)
            {
                Debug.Log($"<color=yellow>Border changing from {_borderObjectName} to {value}</color>");
                
                // Add call stack for debugging
                System.Diagnostics.StackTrace stackTrace = new System.Diagnostics.StackTrace();
                string caller = stackTrace.GetFrame(1)?.GetMethod()?.Name ?? "unknown";
                Debug.Log($"<color=yellow>Border changed by: {caller}</color>");
                
                _borderObjectName = value;
            }
        }
    }
    ...
    private void HandleLevelUp(int newLevel)
    {
        ...
        Debug.Log($"PlayerAttributes.HandleLevelUp called for level {newLevel}");
        
        // IMPORTANT: Remove any code that adds attribute points here!
        // *** Do not add attribute points in this method ***
        
        // It's okay to recalculate stats or apply effects, but not add points
        // ApplyAttributeEffects();
        
        // For debugging, show current points status after level up
        DebugAttributePoints();
    }
    ...
    // Method to increase speed (called from UI)
    public bool IncreaseSpeed()
    {
        if (availablePoints <= 0 || speed.points >= maxSpeedPoints)
            return false;
        ...
        Debug.Log($"Increased Speed to {speed.points}/{maxSpeedPoints}. Speed multiplier: {speed.GetMultiplier():F2}x");
        return true;
    }
    ...
    public void ApplyAttributeEffects()
    {
        ...
        if (livingEntity != null)
        {
            ...
            // Apply movement speed from speed attribute
            livingEntity.SetMoveSpeed(livingEntity.baseMoveSpeed * speed.GetMultiplier());
            
            Debug.Log($"Applied attribute effects: Health={newMaxHealth}, " +
                      $"Damage={newAttackDamage}, Cooldown={newAttackCooldown}, " +
                      $"Recovery Time={newRecoveryTime}, Move Speed={livingEntity.moveSpeed}");
        }
    ...
    {
        Debug.Log("<color=cyan>RecalculateAvailablePoints called</color>");
        ...
        Debug.Log($"Called by: {callerClass}.{callerMethod}");
        
        PlayerInventory inventory = FindObjectOfType<PlayerInventory>();
        if (inventory == null) return;
        
        int levelBasedPoints = (inventory.CurrentLevel - 1) * pointsPerLevel;
        int spentPoints = StrengthPoints + VitalityPoints + AgilityPoints + 
                         IncubationPoints + RecoveryPoints + SpeedPoints;
        
        // Calculate how many points the player should have total
        int expectedTotal = levelBasedPoints;
        
        // IMPORTANT: Add a special mode for initial game load vs. normal gameplay
        // During normal gameplay, don't add points here - they're added in ProcessLevelUp
        bool isNormalLevelUp = callerClass == "PlayerInventory" && 
                              (callerMethod == "ProcessLevelUp" || callerMethod == "AddExperienceGradually");
        
        if (isNormalLevelUp)
        {
            Debug.Log("<color=orange>Skipping point addition in RecalculateAvailablePoints - already handled in ProcessLevelUp</color>");
        }
        // Only add points if we're genuinely missing some (e.g., from a game load)
        else if (availablePoints + spentPoints < expectedTotal)
        {
            int pointsToAdd = expectedTotal - (availablePoints + spentPoints);
            availablePoints += pointsToAdd;
            
            Debug.Log($"<color=orange>RecalculateAvailablePoints: Adding {pointsToAdd} missing points.</color> " +
                      $"ExpectedTotal: {expectedTotal}, Current: {availablePoints + spentPoints - pointsToAdd}");
        }
    ...
    public void SetAttributeValues(int strength, int vitality, int agility, int incubation, int recovery, int speed, int availablePoints)
    {
    ...
    public void SetAttributeValues(int strength, int vitality, int agility, int incubation, int recovery, int availablePoints)
    {
        ...
        SetAttributeValues(strength, vitality, agility, incubation, recovery, 0, availablePoints);
        
        Debug.Log("Using backward compatibility method for SetAttributeValues - speed set to 0");
    }
    ...
    public void SavePlayerAttributes(GameData saveData)
    {
        ...
        saveData.currentSpeed = speed.points;
    ...
    public void LoadPlayerAttributes(GameData saveData)
    {
        ...
        try
        {
            ...
            speed.points = saveData.currentSpeed;
            ...
            if (livingEntity != null)
            {
                ...
                float newAttackCooldown = baseAttackCooldown * AttackCooldownMultiplier;
                float newRecoveryTime = baseRecoveryTime / recovery.GetMultiplier();
                float newMoveSpeed = livingEntity.baseMoveSpeed * speed.GetMultiplier();
                
                livingEntity.SetMaxHealth(newMaxHealth);
                livingEntity.SetAttackDamage(newAttackDamage);
                livingEntity.SetAttackCooldown(newAttackCooldown);
                livingEntity.SetRecoveryTime(newRecoveryTime);
                livingEntity.SetMoveSpeed(newMoveSpeed);
                
                Debug.Log($"Loaded player attributes: Health={newMaxHealth}, " +
                          $"Damage={newAttackDamage}, Cooldown={newAttackCooldown}, " +
                          $"Recovery Time={newRecoveryTime}, Move Speed={newMoveSpeed}");
            }
            else
            {
    ...
    // Modify the AddAttributePoints method to show call stack
    public void AddAttributePoints(int pointsToAdd)
    {
        if (pointsToAdd <= 0) return;
        
        int oldPoints = availablePoints;
        availablePoints += pointsToAdd;
        
        // Add a more detailed debug message
        Debug.Log($"<color=yellow>ATTRIBUTE POINTS ADDED: {pointsToAdd} points " +
                  $"(Old: {oldPoints}, New: {availablePoints})</color>");
        
        // Notify listeners that attributes have changed
        OnAttributesChanged?.Invoke();
    }

    // Add this method to PlayerAttributes to debug point calculations
    public void DebugAttributePoints()
    {
        ...
        // Get current level
        PlayerInventory inventory = FindObjectOfType<PlayerInventory>();
        int currentLevel = inventory != null ? inventory.CurrentLevel : 0;
        
        // Calculate expected total points
        int expectedTotal = (currentLevel - 1) * pointsPerLevel;
        
        Debug.Log($"ATTRIBUTE POINTS DEBUG:\n" +
                  $"Current Level: {currentLevel}\n" +
                  $"Points Per Level: {pointsPerLevel}\n" +
                  $"Expected Total Points: {expectedTotal}\n" +
                  $"Spent Points: {spentPoints}\n" +
                  $"Available Points: {availablePoints}\n" +
                  $"Total Points (Spent + Available): {spentPoints + availablePoints}");
    }

    // Add this method to allow updating the UI from outside
    public void UpdateUI()
    {
        // Manually invoke the events to update UI
        ...
        Debug.Log("PlayerAttributes UI updated manually");
    }

    // Add this method to directly set available points (for testing)
    public void SetAvailablePoints(int points)
    {
        availablePoints = points;
        OnAttributesChanged?.Invoke();  // Make sure to trigger the event
        Debug.Log($"<color=cyan>Directly set available points to: {points}</color>");
    }

    // Add this method to PlayerAttributes.cs
    public string GetCurrentBorderName()
    {
        return borderObjectName;
    }

    // Add this method to check if player is in a specific border
    public bool IsInBorder(string borderToCheck)
    {
        return borderObjectName == borderToCheck;
    }
} 