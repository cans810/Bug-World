...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // For Y-axis capsule, projection is a circle
        if (direction == 1)
        {
            for (int i = 0; i < segments; i++)
            {
        ...
        // For X or Z axis capsules
        float halfHeightNoRadius = (height - 2 * radius) * 0.5f;
        
        // Calculate end centers of the capsule
        ...
        // Draw stadium shape (rectangle with semicircular ends)
        for (int i = 0; i < segments; i++)
        {
            float angle = (float)i / segments * 2f * Mathf.PI;
            Vector3 point;
            
            // Define which part of the stadium shape we're in
            if (angle <= Mathf.PI / 2 || angle > 3 * Mathf.PI / 2)
            {
                // Top semicircle (end1)
                if (direction == 0) // X-axis
                {
                    float localAngle = (angle <= Mathf.PI / 2) ? angle : angle - 3 * Mathf.PI / 2 + Mathf.PI / 2;
                    point = new Vector3(
                        end1.x,
                        perimeterHeight,
                        center.z + radius * Mathf.Sin(localAngle)
                    );
                }
                else // Z-axis
                {
                    float localAngle = (angle <= Mathf.PI / 2) ? angle : angle - 3 * Mathf.PI / 2 + Mathf.PI / 2;
                    point = new Vector3(
                        center.x + radius * Mathf.Sin(localAngle),
                        perimeterHeight,
                        end1.z
                    );
                }
            ...
            {
                // Bottom semicircle (end2)
                if (direction == 0) // X-axis
                {
                    float localAngle = angle - Mathf.PI / 2;
                    point = new Vector3(
                        end2.x,
                        perimeterHeight,
                        center.z + radius * Mathf.Sin(localAngle)
            ...
            else
            {
                ...
                point = new Vector3(center.x, perimeterHeight, center.z);
            }
