...
public class CameraAnimations : MonoBehaviour
{
    ...
    private void OnAreaUnlocked(GameObject areaTarget, int level, string areaName)
    {
        ...
        if (isLoadingData)
        {
            Debug.Log("Skipping camera animation due to loading data state");
            return;
        }
        
        if (areaTarget != null)
        {
            Debug.Log($"CameraAnimations: OnAreaUnlocked called for area: {areaName}, level: {level}");
            StartCoroutine(AnimateCameraToArea(areaTarget.transform, level, areaName));
        }
        else
        {
            Debug.LogError("CameraAnimations: OnAreaUnlocked called with null area target");
        }
    }
    ...
    private IEnumerator AnimateCameraToArea(Transform areaTarget, int level = 0, string areaName = "")
    {
        ...
        // Find and fade out the border visualizer for this area
        FadeOutBorderForArea(areaTarget.gameObject);
            ...
            if (areasField != null)
            {
                ...
                if (areas != null)
                {
                    foreach (var area in areas)
                    {
                        ...
                        {
                            Debug.Log($"CameraAnimations: Found area target for level {level}: {area.areaTarget.name}");
                            StartCoroutine(AnimateCameraToArea(area.areaTarget.transform, level, area.areaName));
                            return;
                        }
                    }
                    Debug.LogWarning($"CameraAnimations: No area found for level {level}");
                }
                else
                {
                    Debug.LogError("CameraAnimations: Could not access level areas array");
                }
            }
            else
            {
                Debug.LogError("CameraAnimations: Could not access levelAreas field");
            }
        }
        else
        {
    ...
    // Add this new method to find and fade out the border visualizer
    private void FadeOutBorderForArea(GameObject areaTarget)
    {
        // Try to find the border by name pattern matching
        string areaName = areaTarget.name;
        string borderNamePattern = "";
        
        // Extract numbers from area name (if any)
        System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(areaName, @"\d+");
        if (match.Success)
        {
            string areaNumber = match.Value;
            borderNamePattern = "MapBorder" + areaNumber;
        }
        
        // Find all border visualizers in the scene
        BorderVisualizer[] allBorders = FindObjectsOfType<BorderVisualizer>();
        Debug.Log($"Found {allBorders.Length} border visualizers, looking for match to area: {areaName}");
        
        foreach (BorderVisualizer border in allBorders)
        {
            // Check if this border matches our target area
            bool isMatch = false;
            
            // Check if border name contains our pattern
            if (!string.IsNullOrEmpty(borderNamePattern) && border.gameObject.name.Contains(borderNamePattern))
            {
                isMatch = true;
            }
            // Also check if border is placed near our area (distance-based check)
            else if (Vector3.Distance(border.transform.position, areaTarget.transform.position) < 30f)
            {
                isMatch = true;
            }
            ...
            if (isMatch)
            {
                Debug.Log($"Found matching border: {border.gameObject.name} for area: {areaName}, fading it out");
