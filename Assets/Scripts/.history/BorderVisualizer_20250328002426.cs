...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        int halfCirclePoints = 30; // Half for the two semicircles (15 points per semicircle)
        ...
        // Calculate the exact number of points we'll be generating
        int topSemicirclePoints = halfCirclePoints / 2;
        int rightStraightPoints = pointsPerStraight;
        int bottomSemicirclePoints = halfCirclePoints / 2;
        int leftStraightPoints = pointsPerStraight;
        int totalPoints = topSemicirclePoints + rightStraightPoints + bottomSemicirclePoints + leftStraightPoints;
        
        // Set the exact position count
        perimeterLineRenderer.positionCount = totalPoints;
        
        // Track point index
        int pointIndex = 0;
        
        // Generate points for the first semicircle (top)
        for (int i = 0; i < topSemicirclePoints; i++)
        {
            float angle = Mathf.PI * i / (topSemicirclePoints - 1);
        ...
        for (int i = 0; i < rightStraightPoints; i++)
        {
            float t = (float)i / (rightStraightPoints - 1);
        ...
        for (int i = 0; i < bottomSemicirclePoints; i++)
        {
            float angle = Mathf.PI + Mathf.PI * i / (bottomSemicirclePoints - 1);
            float x = radius * Mathf.Cos(angle);
            float z = radius * Mathf.Sin(angle);
        ...
        for (int i = 0; i < leftStraightPoints; i++)
        {
            float t = (float)i / (leftStraightPoints - 1);
    ...
    // Add this public method to BorderVisualizer.cs
    public void DisableBorderVisualization()
    {
        ...
        // Log the border disabling
        Debug.Log($"Border visualization disabled for {gameObject.name}");
    }
} 