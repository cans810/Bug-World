...
public class EntityGenerator : MonoBehaviour
{
    ...
    public class EntityType
    {
        ...
        public int chitinDropAmount = 1; // Default chitin to drop
        public bool randomizeDrops = false;
        public Vector2Int chitinDropRange = new Vector2Int(1, 3);
    ...
    [SerializeField] private float spawnRadius = 30f; // How far from center to spawn
    ...
    private void TrySpawnEntity()
    {
        ...
        // More detailed debugging before spawn
        Debug.Log($"About to spawn {selectedType.entityName} with chitin amount: {selectedType.chitinDropAmount}");
        ...
        if (TryGetSpawnPosition(out spawnPosition))
        {
            ...
            // Set the chitin amount based on entity type
            LivingEntity livingEntity = newEntity.GetComponent<LivingEntity>();
            if (livingEntity != null)
            {
                // Store the original value for comparison
                int originalChitinAmount = livingEntity.chitinAmount;
                
                // Set the new value
                livingEntity.chitinAmount = selectedType.chitinDropAmount;
                
                // Enhanced debug
                Debug.Log($"Entity {selectedType.entityName}: Changed chitin from {originalChitinAmount} to {selectedType.chitinDropAmount}. Current value: {livingEntity.chitinAmount}");
            }
