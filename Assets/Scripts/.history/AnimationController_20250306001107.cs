...
public class AnimationController : MonoBehaviour
{
    ...
    /// <summary>
    /// Sets the AttackWalking animation state - DIRECT LAYER CONTROL
    /// </summary>
    public void SetAttackWalking(bool attackWalking)
    {
        if (animator != null)
        {
            // Log detailed info about all animation layers
            ...
            // 1. Completely reset the animator's state
            // 2. Try to find an "AttackWalk" animation clip and play it directly
            
            animator.StopPlayback(); // Stop any current animation playback
            
            // Try setting the boolean but also use direct play approach
            animator.SetBool("AttackWalking", attackWalking);
            
            // Track internal state
            isAttackWalking = attackWalking;
            
            // Try to forcefully disable other animations
            if (attackWalking)
            {
                // Set ALL other booleans to false
                animator.SetBool("Idle", false);
                animator.SetBool("Walk", false);
                animator.SetBool("Attack", false);
                
                // Try to force the animation directly by name with variations
                string[] possibleClipNames = new string[] {
                    "AttackWalking", "Attack_Walking", "AttackWalk", 
                    "Attack_Walk", "Attack Walking"
                };
                
                foreach (string clipName in possibleClipNames)
                {
                    try {
                        // Try to play the animation directly
                        animator.Play(clipName, 0, 0);
                        Debug.Log($"Directly playing animation clip: {clipName}");
                    } catch {
