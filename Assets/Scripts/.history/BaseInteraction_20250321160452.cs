    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        if (visualEffectManager != null)
        {
            ...
            if (uiHelper != null)
            {
                ...
                StartCoroutine(AnimateXPCounter(playerInventory, startXP, xpAmount));
            }
        }
    }
    
    // Modify the AnimateXPCounter method to use our new AddExperience method instead
    private IEnumerator AnimateXPCounter(PlayerInventory inventory, int startingXP, int xpToAdd)
    {
        if (uiHelper == null) yield break;
        
        float elapsed = 0f;
        int xpDifference = xpToAdd - startingXP;
        int currentDisplayXP = startingXP;
        
        // Update the counter incrementally
        while (elapsed < duration && currentDisplayXP < xpToAdd)
        {
            elapsed += Time.deltaTime;
            float progress = elapsed / duration;
            
            // Calculate how many XP points to add this frame
            int targetDisplayXP = startingXP + Mathf.FloorToInt(xpDifference * progress);
        ...
        uiHelper.UpdateExperienceDisplay(xpToAdd, false);
        
        // After the animation completes, don't use CheckAndTriggerLevelUp
        // Instead, use our new AddExperience method
        if (inventory != null)
        {
            // Replace the direct level trigger with our sequential level up method
            inventory.AddExperience(xpToAdd);
            
            // Note: We're not adding the XP to _experience directly anymore
            // since AddExperience will handle that properly
        }
