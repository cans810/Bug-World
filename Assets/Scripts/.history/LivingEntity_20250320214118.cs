...
public class LivingEntity : MonoBehaviour
{
    ...
    [SerializeField] private bool enableSinkingAnimation = true;
    [SerializeField] private float sinkingDuration = 3.5f;   // Keep this slow sinking duration
    [SerializeField] private float sinkingDelay = 1.0f;      // Changed to 1.0 second initial delay
    [SerializeField] private float sinkingDepth = 1.0f;      // Changed to 1.0 unit depth
    [SerializeField] private float finalPauseDelay = 0.3f;   // Keep the final pause
    [SerializeField] private AnimationCurve sinkingCurve = AnimationCurve.EaseInOut(0f, 0f, 1f, 1f);
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // Record entity discovery on death
        RecordEntityDiscovery();
        ...
        // If it's an enemy (not the player), perform the sinking animation
        if (destroyOnDeath && enableSinkingAnimation && gameObject.tag != "Player")
        {
            StartCoroutine(SinkAndDestroy());
        }
        else if (destroyOnDeath)
        {
            // If sinking is disabled but we still need to destroy
            Destroy(gameObject, destroyDelay);
        }
        
        // If using animation controller, trigger death animation
        if (animationController != null)
        {
    ...
    private IEnumerator SinkAndDestroy()
    {
        ...
        Vector3 endPosition = startPosition - new Vector3(0, sinkingDepth, 0);
        
        // If there's an animation controller, make it play death animation  
        if (animationController != null)
        {
            animationController.SetDead();
        }
        
        // Add initial delay before sinking starts
        if (sinkingDelay > 0)
        {
        ...
        // Continuously sink until complete
        while (elapsed < sinkingDuration)
        {
            elapsed += Time.deltaTime;
            float progress = Mathf.Clamp01(elapsed / sinkingDuration);
            
            // Use animation curve for smoother motion
            float curveValue = sinkingCurve.Evaluate(progress);
            
            // Calculate new position
            Vector3 newPosition = Vector3.Lerp(startPosition, endPosition, curveValue);
        ...
        // Wait a moment at the end for dramatic effect
        yield return new WaitForSeconds(finalPauseDelay);
