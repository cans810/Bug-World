...
public class UIHelper : MonoBehaviour
{
    ...
    private void OnPlayerLevelUp(int newLevel)
    {
        ...
        // Show level up effect that was already in place
        if (levelUpEffect != null)
        {
        ...
        if (playerObj != null)
        {
            ...
            {
                // Play level up effect and show attributes panel when complete
                visualEffectManager.PlayLevelUpEffect(playerObj.transform.position, () => {
                    // Show attributes panel after animation completes
        ...
        ShowInformText($"Level Up! You are now level {newLevel}!");
    }
    ...
    private void Update()
    {
        ...
        // Check for keyboard attack input (space or left mouse click)
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0))
        {
            // Make sure we're not clicking on UI elements
            bool isOverUI = EventSystem.current != null && EventSystem.current.IsPointerOverGameObject();
            if (!isOverUI)
            {
                // Find player controller and trigger attack
