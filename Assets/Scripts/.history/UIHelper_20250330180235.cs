...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this method to properly update both egg text and fill bar
    public void UpdateEggDisplay(int currentEggCount)
    {
        // Find InsectIncubator to get max capacity
        InsectIncubator incubator = FindObjectOfType<InsectIncubator>();
        int maxCapacity = incubator != null ? incubator.GetMaxEggCapacity() : 0;
        
        // Update egg count text
        if (eggCountText != null)
        {
            eggCountText.text = string.Format(countFormat, currentEggCount, maxCapacity);
            Debug.Log($"Updated egg count text: {currentEggCount}/{maxCapacity}");
            
            // Animate text scale if enabled
            if (animateOnChange && eggTextRect != null)
            {
                StartCoroutine(AnimateScalePunch(eggTextRect, originalEggScale));
            }
        }
