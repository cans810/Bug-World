...
public class ChitinBehavior : MonoBehaviour
{
    ...
    [SerializeField] private float minArcHeight = 0.5f;  // Height at which we've reached the arc peak
    [SerializeField] private float stickHeight = 0.05f;  // Height at which to stick
    [SerializeField] private float initialUpForce = 4f;  // Upward force - increased from 3
    [SerializeField] private float horizontalForce = 1.2f; // Horizontal scatter - increased from 0.8
    ...
    private void Awake()
    {
        ...
        // Ensure the chitin's position is set as our start position
    ...
    private void ApplyInitialForce()
    {
        if (rb != null && !hasAppliedForce)
        {
            ...
            // Apply solid upward force
            Vector3 upForce = Vector3.up * initialUpForce;
            
            // Apply horizontal scatter
