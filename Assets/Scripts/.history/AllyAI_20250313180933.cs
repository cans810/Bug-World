...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Check if player is moving by comparing their current position to last recorded position
        ...
        // Use larger follow distance when player is stationary
        float currentFollowDistance = followDistance;
        if (stopWhenPlayerStops && !isPlayerMoving)
        {
            currentFollowDistance = playerStationaryStopDistance;
        }
        
        // If we're close enough to the player, stop moving
        if (distanceToPlayer <= currentFollowDistance)
        {
            // Stop moving
            isMoving = false;
            
            // Explicitly transition to idle animation
            if (animController != null)
            {
        ...
        // If we're too far, move towards the player
        else if (isPlayerMoving || distanceToPlayer > playerStationaryStopDistance)
        {
            isMoving = true;
            
            // Explicitly transition to walk animation
            if (animController != null)
            {
            ...
            // Move forward - exactly like the enemy movement
            MoveWithBoundaryCheck(transform.forward);
        }
        else
        {
            // Player is not moving and we're at a reasonable distance, so stop
            isMoving = false;
    ...
    {
        // If we have a wandering behavior component, let it handle movement
        if (wanderingBehavior != null)
        {
            // Make sure wandering is enabled
            wanderingBehavior.SetWanderingEnabled(true);
            
            // Let AIWandering handle the movement and animations
            isMoving = wanderingBehavior.IsCurrentlyMoving();
            
            // Update animation based on the wandering state
            if (animController != null)
            {
                if (isMoving)
                {
            ...
            // Mark that movement has been handled this frame
            hasAppliedMovementThisFrame = isMoving;
        }
        else
        {
            // Fallback if no wandering behavior exists
            Debug.LogWarning($"{gameObject.name} is in Wander mode but has no AIWandering component");
            
            // Switch to follow mode instead
    ...
    public void SetMode(AIMode newMode)
    {
        ...
        // Handle wandering behavior based on mode
        if (wanderingBehavior != null)
        {
            if (newMode == AIMode.Wander)
            {
                wanderingBehavior.enabled = true;
                wanderingBehavior.SetWanderingEnabled(true);
                // Force a new waypoint when starting to wander
            ...
            else
            {
                ...
                // Completely disable wandering component for carrying mode
    ...
    // Simplified MoveWithBoundaryCheck to match enemy implementation
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        // Check if next position is outside boundary
        ...
        if (isOutsideBoundary)
        {
            ...
            FaceTarget(transform.position + redirectedDirection);
            
            // Apply movement using LivingEntity
            livingEntity.MoveInDirection(transform.forward, speedMultiplier);
        }
        else
        {
            // Apply movement using LivingEntity
    ...
    // Simplified FaceTarget matching enemy implementation
    private void FaceTarget(Vector3 target)
    {
        ...
        {
            // Use the same rotation approach as enemies
