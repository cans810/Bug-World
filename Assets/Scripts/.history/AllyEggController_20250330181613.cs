...
public class AllyEggController : MonoBehaviour
{
    ...
    // Add a delegate and event for countdown completion
    public delegate void CountdownCompleteDelegate(AllyEggController eggController);
    public static event CountdownCompleteDelegate OnCountdownComplete;
    ...
    private IEnumerator HatchEgg()
    {
        ...
        // Before destroying the egg, notify InsectIncubator
        if (OnCountdownComplete != null)
        {
            OnCountdownComplete(this);
        }
        
        // Manually call the OnEggHatched method for the InsectIncubator
        InsectIncubator incubator = FindObjectOfType<InsectIncubator>();
        if (incubator != null)
        {
            incubator.OnEggHatched();
            Debug.Log("MANUALLY notified InsectIncubator about egg hatching");
        }
    ...
    // Update this method to handle camera animation completion message
    public void OnCameraAnimationComplete(GameObject insect)
    {
        ...
        // Enable movement for the focused insect
        AllyAI allyAI = insect.GetComponent<AllyAI>();
        if (allyAI != null)
        {
            allyAI.enabled = true;
        ...
        // Also find and enable all other recently hatched insects that might be disabled
        ...
        foreach (var ally in allAllies)
        {
            ...
            {
                ally.enabled = true;
                ally.ForceEnableMovement(); // Use the force enable method for reliability
                Debug.Log($"Also re-enabled AllyAI for {ally.gameObject.name} that wasn't directly shown");
            }
        }
    }

    // Modify the OnEggHatched method to properly notify InsectIncubator
    private void OnEggHatched(GameObject insectPrefab)
    {
        // Get insect incubator reference
        InsectIncubator incubator = FindObjectOfType<InsectIncubator>();
        if (incubator != null)
        {
            // Notify incubator that an egg has hatched
            incubator.OnEggHatched();
            Debug.Log("Notified InsectIncubator that egg has hatched");
        }
        else
        {
            Debug.LogWarning("Could not find InsectIncubator to notify about egg hatching");
        }
        
        // Rest of hatching code...
