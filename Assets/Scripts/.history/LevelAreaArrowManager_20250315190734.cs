...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    public void CheckAreaEntry(string borderName)
    {
        ...
        foreach (LevelAreaTarget area in levelAreas)
        {
            // Check if this border name matches the area target name
            // The naming convention should be consistent (e.g., MapBorder2 → Area2)
            if (area.areaTarget != null && 
                area.areaTarget.name == borderName || // Exact match
                (borderName.Contains(area.areaName) || area.areaName.Contains(borderName))) // Partial match
            {
