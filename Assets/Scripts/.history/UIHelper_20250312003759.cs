...
public class UIHelper : MonoBehaviour
{
    ...
    // Update the AnimateChitinCollection method to use UI transforms
    public void AnimateChitinCollection(int chitinCount, Vector3 startPosition)
    {
        if (chitinIconPrefab == null || posToMoveIconTransform == null || nestTransform == null || chitinCountText == null)
        {
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        {
            // Instantiate the chitin icon as a child of the chitinCountText
            GameObject chitinIcon = Instantiate(chitinIconPrefab, chitinCountText.transform);
            
            // Set its initial position to match the chitinCountText
            chitinIcon.transform.localPosition = Vector3.zero;
    ...
    private IEnumerator MoveChitinIconUI(GameObject icon, Transform startTransform, Transform endTransform, Vector3 offset, float duration)
    {
        ...
        Vector3 startPos = RectTransformUtility.WorldToScreenPoint(Camera.main, startTransform.position);
        
        // Get the ending position in screen space
        Vector3 endPos = RectTransformUtility.WorldToScreenPoint(Camera.main, endTransform.position) + offset;
        ...
        while (Time.time < endTime)
        {
            ...
            Vector3 currentScreenPos = Vector3.Lerp(startPos, endPos, t);
        ...
        icon.transform.position = endTransform.position + offset;
    ...
    private IEnumerator MoveToNestAndDestroyUI(GameObject icon, Transform startTransform, Transform endTransform, float duration)
    {
        ...
        // Get the ending position in screen space
        Vector3 endPos = RectTransformUtility.WorldToScreenPoint(Camera.main, endTransform.position);
        ...
        while (Time.time < endTime)
        {
            ...
            Vector3 currentScreenPos = Vector3.Lerp(startPos, endPos, easedT);
