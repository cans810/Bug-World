...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    public void OnPlayerLevelUp(int newLevel)
    {
        ...
        if (foundAreaToUnlock && areaToUnlock != null && areaToUnlock.areaTarget != null)
        {
            ...
            // IMPORTANT: First check if the camera animation component exists before showing the message
            bool canPlayAnimation = (cameraAnimationsRef != null || 
                                    OnAreaUnlocked != null || 
                                    onAreaUnlockedEvent != null);
                                    
            if (canPlayAnimation)
            {
                // Now show the message because we're about to animate
                if (uiHelper != null)
                {
                ...
                if (cameraAnimationsRef != null)
                {
                    Debug.Log($"Directly calling CameraAnimations.OnAreaUnlocked for {areaToUnlock.areaName}");
                    cameraAnimationsRef.OnAreaUnlocked(areaToUnlock.areaTarget, newLevel, areaToUnlock.areaName);
                }
            ...
            {
                Debug.LogWarning($"Found area to unlock ({areaToUnlock.areaName}) but no animation handlers are available!");
            }
        }
        else
        {
