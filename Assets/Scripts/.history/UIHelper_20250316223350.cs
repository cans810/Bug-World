...
public class UIHelper : MonoBehaviour
{
    ...
    // Show the level-up rewards panel with appropriate rewards
    public void ShowLevelUpRewardsPanel(int newLevel)
    {
        // Skip showing rewards if we're loading data
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null && playerInventory.IsLoadingData)
        {
            return;
        ...
        if (onLevelUpThingsGainedPanel != null && awardsPrefabs.Count >= 3)
        {
            // Clear any existing rewards
            ClearRewardsPanelChildren();
            
            // Activate the panel
            onLevelUpThingsGainedPanel.SetActive(true);
            
            // Create the standard level-up rewards
            CreateRewardPrefab(0); // Chitin capacity
            CreateRewardPrefab(1); // Crumb capacity
            CreateRewardPrefab(2); // Attribute point
            
            // Check if a new area is unlocked at this level
            bool newAreaUnlocked = CheckIfNewAreaUnlocked(newLevel);
            if (newAreaUnlocked && awardsPrefabs.Count >= 4)
            {
                CreateRewardPrefab(3); // Area unlocked
            }
            
            // Start a coroutine to hide the panel after a delay
            StartCoroutine(HideLevelUpRewardsPanelAfterDelay(6.0f));
        }
        else
        {
            Debug.LogWarning("Level-up rewards panel or prefabs are not properly set up!");
        }
    }

    // Helper method to create a reward prefab inside the panel
    private void CreateRewardPrefab(int prefabIndex)
    {
        if (prefabIndex < 0 || prefabIndex >= awardsPrefabs.Count || awardsPrefabs[prefabIndex] == null)
        {
            Debug.LogWarning($"Cannot create reward prefab at index {prefabIndex}: invalid index or null prefab");
            return;
        }
        
        // Instantiate the prefab as a child of the panel
        GameObject rewardInstance = Instantiate(awardsPrefabs[prefabIndex], onLevelUpThingsGainedPanel.transform);
        
        // Optional: Set position, scale or other properties
        // For example, you might want to position them vertically
        RectTransform rect = rewardInstance.GetComponent<RectTransform>();
        if (rect != null)
        {
            // You can adjust positioning here if needed
            // For now, let the panel's layout component handle it
        }
    }

    // Helper method to clear existing rewards
    private void ClearRewardsPanelChildren()
    {
        if (onLevelUpThingsGainedPanel == null) return;
        
        // Destroy all children of the panel
    ...
    // Helper method to check if a new area is unlocked at this level
    private bool CheckIfNewAreaUnlocked(int level)
    {
        // This would be your logic to determine if a new area unlocks at this level
        // For example:
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null)
        {
            // Since GameManager.IsNewAreaUnlockedAtLevel doesn't exist yet,
            // use our fallback logic instead
            /*return gameManager.IsNewAreaUnlockedAtLevel(level);*/
        }
        
        // Fallback to a simple level check
        // Assuming areas unlock at these levels:
        return level == 2 || level == 4 || level == 6 || level == 8;
    }

    private IEnumerator HideLevelUpRewardsPanelAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
