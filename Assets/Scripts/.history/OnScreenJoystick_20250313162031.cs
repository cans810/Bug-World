...
public class OnScreenJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [SerializeField] private string touchableUILayer = "TouchableUI"; // UI elements on this layer can be touched through
            ...
            if (UnityEngine.Input.touchCount > 0)
            {
                ...
                if (ignoreUIElements)
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        if (result.gameObject.layer == LayerMask.NameToLayer(touchableUILayer))
                        {
                            if (debugLayers)
                            {
