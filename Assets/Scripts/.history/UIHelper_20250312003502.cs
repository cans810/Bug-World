...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this method to create and animate chitin icons
    public void AnimateChitinCollection(int chitinCount, Vector3 startPosition)
    {
        if (chitinIconPrefab == null || posToMoveIconTransform == null || nestTransform == null)
        {
        ...
        StartCoroutine(SpawnAndAnimateChitinIcons(chitinCount, startPosition));
    }

    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount, Vector3 startPosition)
    {
        ...
        {
            // Instantiate the chitin icon as a child of this GameObject
            GameObject chitinIcon = Instantiate(chitinIconPrefab, transform);
            
            // Set its initial position in world space
            chitinIcon.transform.position = startPosition;
            ...
            StartCoroutine(MoveChitinIcon(chitinIcon, startPosition, 
                posToMoveIconTransform.position + randomOffset, 0.5f));
    ...
    private IEnumerator MoveChitinIcon(GameObject icon, Vector3 startPos, Vector3 endPos, float duration)
    {
        ...
        StartCoroutine(MoveToNestAndDestroy(icon, endPos, nestTransform.position, 0.7f));
    }

    private IEnumerator MoveToNestAndDestroy(GameObject icon, Vector3 startPos, Vector3 nestPos, float duration)
    {
        float startTime = Time.time;
        float endTime = startTime + duration;
        ...
        while (Time.time < endTime)
        {
            ...
            Vector3 currentPos = Vector3.Lerp(startPos, nestPos, t);
    ...
    // Call this method from PlayerInventory when chitins are deposited
    public void AnimateChitinDeposit(int chitinCount)
    {
        // Find the player position to use as the starting point
        GameObject player = GameObject.FindGameObjectWithTag("Player");
        if (player != null && chitinCount > 0)
        {
            AnimateChitinCollection(chitinCount, player.transform.position);
        }
    }
} 
