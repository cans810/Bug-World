...
public class UIHelper : MonoBehaviour
{
    ...
    {
        if (chitinIconPrefab == null || posToMoveIconTransform == null)
        {
    ...
    {
        // Ensure we have a positive count
        if (chitinCount <= 0)
        {
            Debug.LogWarning("Attempted to spawn chitin icons with zero or negative count: " + chitinCount);
            yield break;
        }
        
        // Limit the number of icons to prevent overwhelming visuals
        int maxIconsToShow = Mathf.Min(chitinCount, 10);
        Debug.Log($"Will spawn {maxIconsToShow} chitin icons");
    ...
    // Fix the AnimateSingleChitinIcon method to ensure proper movement path
