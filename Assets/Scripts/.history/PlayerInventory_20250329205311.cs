...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        // Add XP to total
        int previousXP = TotalExperience;
        ...
        // Debug log
        Debug.Log($"Added {amount} XP. Total XP: {TotalExperience}, Level: {PlayerLevel}");
        
        // Explicitly check if level changed and show UI if needed
        if (PlayerLevel > previousLevel)
        {
            Debug.Log($"Player leveled up from {previousLevel} to {PlayerLevel}!");
            
            // Ensure level up panel shows
            LevelUpPanelHelper levelUpPanel = FindObjectOfType<LevelUpPanelHelper>();
            if (levelUpPanel != null)
            {
                levelUpPanel.ShowLevelUpPanel(PlayerLevel);
            }
