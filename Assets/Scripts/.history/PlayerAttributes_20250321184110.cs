...
public class PlayerAttributes : MonoBehaviour
{
    ...
    // Add at the class level with other attributes
    private AttributeStat speedStat = new AttributeStat { name = "Speed", points = 0, percentagePerPoint = 1f };
    public int SpeedPoints => speedStat.points;
    public float SpeedMultiplier => speedStat.GetMultiplier();
    public int MaxSpeedPoints => 25; // Maximum 25 points allowed for speed

    // Add this property to return the raw speed multiplier (e.g., 1.25 for 25 points)
    public float MovementSpeedMultiplier
    {
        get { return speedStat.GetMultiplier(); }
    }
    ...
    {
        // Only allow if we have points and haven't maxed out
        if (availablePoints > 0 && speedStat.points < MaxSpeedPoints)
        {
            speedStat.points++;
            availablePoints--;
            ...
            Debug.Log($"Increased Speed to {speedStat.points}. Speed multiplier: {speedStat.GetMultiplier():F2}x");
            return true;
        }
    ...
    public void RecalculateAvailablePoints(bool forceRecalculate = false)
    {
        ...
        int spentPoints = StrengthPoints + VitalityPoints + AgilityPoints + IncubationPoints + RecoveryPoints + SpeedPoints;
    ...
    // Update this method to include the speed parameter
    public void SetAttributeValues(int strength, int vitality, int agility, int incubation, int recovery, int speed, int availablePoints)
    {
        ...
        this.speedStat.points = speed; // Set speed value
        ...
        Debug.Log($"Directly set attribute values: Strength={strength}, " +
                  $"Vitality={vitality}, Agility={agility}, " +
                  $"Incubation={incubation}, Recovery={recovery}, " +
                  $"Speed={speed}, AvailablePoints={availablePoints}");
    }

    // Update this method overload to maintain backward compatibility
    ...
    // Add another overload for even older save files without recovery
    public void SetAttributeValues(int strength, int vitality, int agility, int incubation, int availablePoints)
    {
        // Call the new method with default recovery and speed values of 0
        SetAttributeValues(strength, vitality, agility, incubation, 0, 0, availablePoints);
        
        Debug.Log("Using oldest backward compatibility method for SetAttributeValues - recovery and speed set to 0");
    }
    ...
    public void SavePlayerAttributes(GameData saveData)
    {
        ...
        saveData.currentSpeed = speedStat.points;
    ...
    public void LoadPlayerAttributes(GameData saveData)
    {
        ...
        try
        {
            ...
            speedStat.points = saveData.currentSpeed;
    ...
    // Update the ResetAttributes method to include speed
    public void ResetAttributes()
    {
        ...
        incubationPoints = 0;
        recovery.points = 0;
        speedStat.points = 0; // Reset speed
        availablePoints = 0;
        
        // Notify listeners
        OnAttributesChanged?.Invoke();
        
        Debug.Log("All attributes have been reset to 0");
    }
} 