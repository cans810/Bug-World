...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Always check for nestTransform1, since this is our target destination
        if (nestTransform1 == null) 
        {
            Debug.LogWarning("nestTransform1 is null - cannot animate chitin collection!");
            yield break;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            symbol.SetActive(true);
            // Pass the first nest transform explicitly
            StartCoroutine(AnimateResourceCollection(symbol, "chitin", nestTransform1));
    ...
    public IEnumerator SpawnCrumbCollectEffect(int count)
    {
        ...
        if (nestTransform1 == null)
        {
            Debug.LogWarning("nestTransform1 is null - cannot animate crumb collection!");
            yield break;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            symbol.SetActive(true);
            // Pass the first nest transform explicitly
            StartCoroutine(AnimateResourceCollection(symbol, "crumb", nestTransform1));

            yield return new WaitForSeconds(chitinSpawnDelay);
        }
    }

    private IEnumerator AnimateResourceCollection(GameObject symbol, string resourceType, Transform targetNest)
    {
        // Safety check
        if (targetNest == null)
        {
            Debug.LogWarning($"Target nest is null for {resourceType} animation. Using camera position as fallback.");
            // Use a fallback position if the target nest is null
            targetNest = Camera.main.transform;
        }

        SpriteRenderer spriteRenderer = symbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;

        // Rest of the method remains the same, but will use the specific targetNest passed in
        Color startColor = spriteRenderer.color;
        float elapsed = 0f;
        
        // Set scale based on resource type
        if (resourceType == "chitin")
            symbol.transform.localScale = chitinScale;
        else if (resourceType == "crumb")
            symbol.transform.localScale = crumbScale;
        
        // Apply slight random scaling
        float randomScale = Random.Range(scaleMultiplierRange.x, scaleMultiplierRange.y);
        symbol.transform.localScale *= randomScale;
        
        // Get the starting position
        Vector3 startPos = symbol.transform.position;
        // Use the explicitly provided target nest
        Vector3 targetPos = targetNest.position;
        
        // Add slight height to ensure visible flight path
        targetPos.y += 0.5f;
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            {
                SoundEffectManager.Instance.PlaySound("SymbolToNest", nestTransform1.position, true);
            }
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpValue)
    {
        ...
        // IMPORTANT: Remove this code that adds additional XP
