...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add ally point settings
    [Header("Ally Points Settings")]
    [SerializeField] private int crumbsRequiredForAllyPoint = 5;
    ...
    // Add a new event for when ally points are granted
    public event Action<int> OnAllyPointsAdded;
    ...
    public void DepositCrumbsAtBase(int amount)
    {
        ...
        // Calculate ally points (each 5 crumbs = 1 ally point)
        int allyPoints = amount / crumbsRequiredForAllyPoint;
        
        // If any ally points were earned
        if (allyPoints > 0)
        {
            // Find PlayerAttributes and add ally points
            PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
            if (playerAttributes != null)
            {
                playerAttributes.AddAllyPoints(allyPoints);
                
                // Notify any listeners
                OnAllyPointsAdded?.Invoke(allyPoints);
                
                if (showDebugMessages)
                {
                    Debug.Log($"Earned {allyPoints} ally points from depositing {amount} crumbs");
                }
            }
        }
    }
