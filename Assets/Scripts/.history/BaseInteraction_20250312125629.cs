...
public class BaseInteraction : MonoBehaviour
{
    ...
    [SerializeField] private GameObject healEffectPrefab; // Prefab with the + sprite/3D model
    [SerializeField] private int effectsPerHeal = 3; // How many effects to spawn per heal
    [SerializeField] private float effectSpawnRadius = 1f; // How far from the entity to spawn effects
    [SerializeField] private float effectFloatSpeed = 2f; // How fast effects float up
    [SerializeField] private float effectLifetime = 1f; // How long effects last before destroying
    ...
    private void ApplyIntervalHealing(LivingEntity entity)
    {
        ...
        if (healedThisInterval > 0)
        {
            ...
            {
                SpawnHealingEffects(entity.transform.position);
            }
    ...
    private void SpawnHealingEffects(Vector3 position)
    {
        for (int i = 0; i < effectsPerHeal; i++)
        {
            // Calculate random position within radius
            Vector2 randomCircle = Random.insideUnitCircle * effectSpawnRadius;
            Vector3 spawnPos = position + new Vector3(randomCircle.x, 1f, randomCircle.y);
            
            // Spawn the effect
            GameObject effect = Instantiate(healEffectPrefab, spawnPos, Quaternion.identity);
            
            // Set the effect properties
            if (effect.TryGetComponent<HealingEffect>(out var healingEffect))
            {
                healingEffect.floatSpeed = effectFloatSpeed;
                healingEffect.lifetime = effectLifetime;
            }
        }
    }
} 