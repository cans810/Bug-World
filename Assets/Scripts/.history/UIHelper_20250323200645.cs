...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this method to UIHelper to allow manually showing the attributes panel
    public void ShowAttributesPanel()
    {
        if (attributesPanel != null)
        {
            ...
            {
                soundManager.PlaySound("ui_panel_open");
            }
        }
        else
        {
            Debug.LogWarning("UIHelper: attributes panel reference is null!");
        }
    }

    // Add a method to manually show attributes panel from UI button
    public void ShowAttributesPanelFromButton()
    {
        // This is for manual triggering only
        Debug.Log("Attribute panel manually opened by player");
        ShowAttributesPanel();
    }

    // Check if there are any methods in UIHelper that automatically show the panel
    // When the player levels up or completes a camera animation
    public void OnLevelUpCompleted(int level)
    {
        // REMOVE any code that shows attributes panel automatically
        // For example, look for and remove:
        // ShowAttributesPanel();
        
        // Instead just show the level text without showing the panel
