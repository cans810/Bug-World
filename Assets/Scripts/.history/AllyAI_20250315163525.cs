...
public class AllyAI : MonoBehaviour
{
    ...
    // Update the AttemptAttack method to be more consistent with the Player's attack system
    private bool AttemptAttack(LivingEntity target)
    {
        ...
        // Play attack animation through AnimationController
        if (animController != null)
        {
            ...
            // The actual damage is handled by the animation event
            return true;
        }
        else
        {
            // Fallback if animation controller isn't available
            if (livingEntity.TryAttack())
            {
                if (showDebugInfo)
                    Debug.Log($"Ally {gameObject.name}: Direct attack on {target.name}");
                return true;
            }
        }
    ...
    // Update the AutoAttackCoroutine to be more accurate with cooldowns
                ...
                if (closestEnemy != null)
                {
                    ...
                    if (distanceToEnemy <= attackRange)
                    {
                        ...
                        if (attackStarted)
                        {
                            ...
                            while (timer < cooldownTime && livingEntity.RemainingAttackCooldown > 0)
                            {
