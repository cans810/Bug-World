...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        {
            // First rotate towards the waypoint (same method for enemies and allies)
            if (livingEntity != null)
            {
                // Use the same rotation method from LivingEntity that enemies use
                livingEntity.RotateTowards(direction, 1.5f);
                
                // Use the same movement method from LivingEntity that enemies use
                livingEntity.MoveInDirection(transform.forward, 1.0f);
            }
            else
            {
                // Fallback rotation and movement if no LivingEntity
