...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Check if the player is providing any input
        if (playerInputDirection.magnitude > 0.1f)
        {
            // Player is moving, so we should move too
            isMoving = true;
            ...
            // Ensure the direction is normalized and has no Y component
            playerInputDirection.y = 0;
            if (playerInputDirection.magnitude > 0)
                playerInputDirection.Normalize();
            
            // Rotate to face the direction we're moving
            livingEntity.RotateTowards(playerInputDirection, rotationMultiplier);
            
            // Move in the same direction as the player's input
            livingEntity.MoveInDirection(playerInputDirection, speedMultiplier);
        ...
        {
            // Player is not providing input, so stop moving
            isMoving = false;
        ...
        // Optional: Add a safety check to prevent allies from getting too far from player
        float distanceToPlayer = Vector3.Distance(transform.position, playerTransform.position);
        if (distanceToPlayer > followDistance * 3) // Use a larger threshold than normal follow
        {
            // If too far, move directly toward player
            Vector3 directionToPlayer = (playerTransform.position - transform.position).normalized;
            directionToPlayer.y = 0;
            
            // Override previous movement with a move toward player
            livingEntity.RotateTowards(directionToPlayer, rotationMultiplier * 2); // Faster rotation
            livingEntity.MoveInDirection(directionToPlayer, speedMultiplier * 1.5f); // Faster movement
            hasAppliedMovementThisFrame = true;
        }
    }
