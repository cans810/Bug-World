...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modify FaceTarget to use even smoother turning with lower rotation speeds
    private void FaceTarget(Vector3 target)
    {
        ...
        if (directionToTarget.magnitude > 0.1f)
        {
            ...
            // Use significantly reduced rotation speeds for smoother turns
            // Even in combat, we want smooth rotation now
            float rotationSpeed = inCombatStance ? targetRotationSpeed * 0.8f : targetRotationSpeed * 0.5f;
            
            // Apply very smooth rotation using Slerp with lower speed value
    ...
    // Updated coroutine to make enemy face the player with even smoother rotation
    public IEnumerator FacePlayerWhileInHitbox(LivingEntity playerEntity)
    {
        while (isPlayerInHitbox && !livingEntity.IsDead && playerEntity != null && !playerEntity.IsDead)
        {
            ...
            {
                // Use very smooth rotation with lower rotation speed
                Quaternion targetRotation = Quaternion.LookRotation(directionToPlayer);
                
                // Use a reduced rotation speed for very smooth turning
                float smoothRotationSpeed = targetRotationSpeed * 0.5f;
