...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        {
            // We've hit the map boundary - stop, start redirecting
            isHittingBoundary = true;
            boundaryRedirectionTime = Time.time + BOUNDARY_REDIRECT_DURATION;
            
            // Choose a random perpendicular direction (left or right along boundary)
            Vector3 boundaryDirection = (mapBoundary.transform.position - transform.position).normalized;
            Vector3 redirectDirection = Vector3.Cross(Vector3.up, boundaryDirection);
            ...
            // Start turning in the new direction
            livingEntity.RotateTowards(redirectDirection, 3.0f);
