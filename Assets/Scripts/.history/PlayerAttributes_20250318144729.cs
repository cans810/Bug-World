...
public class PlayerAttributes : MonoBehaviour
{
    ...
    // Modify your existing SetAttributeValues method to accept a force parameter
    public void SetAttributeValues(int strength, int vitality, int agility, int incubation, int availablePoints, bool forceApply = false)
    {
        // Set force flag to bypass any checks
        if (forceApply)
        {
            // Directly set values bypassing any validation
            strength.points = strength;
            vitality.points = vitality;
            agility.points = agility;
            incubationPoints = incubation;
            availablePoints = availablePoints;
            
            // Apply effects
            ApplyAttributeEffects();
            
            Debug.Log($"FORCED attribute values: Str={strength}, Vit={vitality}, Agi={agility}, Inc={incubation}");
            return;
        }
        ...
        Debug.Log($"Directly set attribute values: Strength={strength.points}, Vitality={vitality.points}, " +
                  $"Agility={agility.points}, Incubation={incubationPoints}, AvailablePoints={availablePoints}");
    }
    ...
    public void SavePlayerAttributes(GameData saveData)
    {
        ...
        Debug.Log($"Saving player attributes: Strength={strength.points}, " +
                  $"Vitality={vitality.points}, Agility={agility.points}, " +
                  $"Incubation={incubationPoints}, AvailablePoints={availablePoints}");
    }
    ...
    public void LoadPlayerAttributes(GameData saveData)
    {
        if (saveData == null)
        {
            Debug.LogWarning("No saved player attributes found. Using defaults.");
            return;
        }
        
        try
        {
            // Set loading flag to true before applying data
            isLoadingData = true;
            ...
            Debug.Log($"Loaded player attributes: Strength={strength.points}, " +
                      $"Vitality={vitality.points}, Agility={agility.points}, " +
                      $"Incubation={incubationPoints}, AvailablePoints={availablePoints}");
            ...
            // Reset loading flag after all data is applied
            isLoadingData = false;
        }
    ...
    // Add this method to debug attribute values
    public void DebugAttributeValues()
    {
        Debug.Log("=== PLAYER ATTRIBUTES DEBUG ===");
        Debug.Log($"Strength: {strength.points} (Multiplier: {strength.GetMultiplier():F2}x)");
        Debug.Log($"Vitality: {vitality.points} (Multiplier: {vitality.GetMultiplier():F2}x)");
        Debug.Log($"Agility: {agility.points} (Multiplier: {agility.GetMultiplier():F2}x)");
        Debug.Log($"Incubation: {incubationPoints} (Max Eggs: {MaxEggCapacity})");
