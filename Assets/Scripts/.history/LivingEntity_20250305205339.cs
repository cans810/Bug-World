...
public class LivingEntity : MonoBehaviour
{
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // Instead of adding directly to player, drop chitin at death location
        if (chitinAmount > 0)
        {
            // Check if we have the drop manager
            ChitinDropManager dropManager = ChitinDropManager.Instance;
            if (dropManager != null)
            {
                dropManager.DropChitin(transform.position, chitinAmount);
                Debug.Log($"Dropping {chitinAmount} chitin pieces at {transform.position}");
            }
            else
            {
                // Fallback to direct collection if no drop manager
                ...
                if (player != null)
                {
                    ...
                    {
                        playerInventory.AddChitin(chitinAmount);
                        Debug.Log($"Direct collection of {chitinAmount} chitin (no drop manager found)");
                    }
                }
            }
        }
