...
public class AllyAI : MonoBehaviour
{
    ...
    private void CarryLootToBase()
    {
        ...
        {
            // No base defined or no loot, drop the loot and return to previous mode
            Debug.LogWarning($"{gameObject.name}: Base transform is null or loot is null. Dropping loot and returning to previous mode.");
            if (carriedLoot != null)
            {
        ...
        // Debug to see if we're calculating the correct distance
        if (Time.frameCount % 60 == 0) // Only log every 60 frames to avoid spam
        {
            Debug.Log($"{gameObject.name}: Distance to base: {distanceToBase}, Base position: {baseTransform.position}");
        }
        ...
        // Get a position with the base's X and Z, but keep our Y
        Vector3 targetPosition = new Vector3(
            baseTransform.position.x,
            transform.position.y, // Use the ally's Y position
        ...
        // Face and move towards the base (using the adjusted Y position)
        FaceTarget(targetPosition);
        
        // Direct movement toward base (more reliable than using transform.forward)
        Vector3 direction = (targetPosition - transform.position).normalized;
        MoveWithBoundaryCheck(direction);
    }
