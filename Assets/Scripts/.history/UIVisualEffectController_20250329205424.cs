    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            ...
            if (uiHelper != null)
            {
                ...
                int currentXP = playerInventory.GetExperience();
            ...
            // Explicitly check for level up and show panel if needed
            if (playerInventory.CurrentLevel > previousLevel)
            {
                ...
                {
                    // Use the correct method to show level up panel
                    levelUpPanel.ShowLevelUp(playerInventory.CurrentLevel);
