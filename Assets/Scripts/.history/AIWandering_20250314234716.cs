...
public class AIWandering : MonoBehaviour
{
    ...
    // Enhanced border detection system with better bounce mechanics
    private void OnTriggerExit(Collider other)
    {
        // Check if the collider is in the MapBorder layer
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Calculate the inward reflection normal based on collider type
            ...
            if (other is SphereCollider sphereCollider)
            {
                ...
                // Calculate direction from enemy to sphere center (inward)
                reflectionNormal = (sphereCenter - transform.position).normalized;
            }
            else
            {
                // For non-sphere colliders, use closest point
                Vector3 closestPoint = other.ClosestPoint(transform.position);
                // Direction from enemy to closest point (inward)
                reflectionNormal = (closestPoint - transform.position).normalized;
            }
            
            // Keep on same Y level and ensure normalized
            reflectionNormal.y = 0;
            reflectionNormal.Normalize();
            
            // Get current forward direction
            Vector3 currentDirection = transform.forward;
            
            // Calculate reflection direction (bounce) using physics reflection
            ...
            transform.position += reflectionNormal * 0.5f;
            
            // Add randomness to the bounce direction (more variation for wandering AI)
            float randomVariation = Random.Range(-45f, 45f);
            Vector3 newDirection = Quaternion.Euler(0, randomVariation, 0) * reflectionDirection;
            
            // Immediately apply the new rotation for responsive bounce
            transform.rotation = Quaternion.LookRotation(newDirection);
            
            // Interrupt current movement and force a new movement pattern
            StopAllCoroutines();
            
            // Start bounce movement with the new direction
            StartCoroutine(BounceMovement(newDirection));
        }
    }

    // Enhanced bounce movement for more natural behavior
    private IEnumerator BounceMovement(Vector3 direction)
    {
        ...
        // Use a more substantial distance for the bounce movement
        float moveDistance = Random.Range(minWanderDistance, maxWanderDistance);
        float moveDuration = moveDistance / livingEntity.moveSpeed;
        float moveTimer = 0;
        float distanceMoved = 0;
        ...
        // Apply initial impulse by moving a bit faster at first
        float initialBoostFactor = 1.5f;
        float initialBoostDuration = 0.3f;
        
        // Boost phase - move faster initially
        while (moveTimer < initialBoostDuration && distanceMoved < moveDistance)
        {
            // Use boosted speed for initial movement
            float stepDistance = livingEntity.moveSpeed * initialBoostFactor * Time.deltaTime;
            
            if (livingEntity != null)
            {
                Quaternion previousRotation = transform.rotation;
                livingEntity.MoveInDirection(moveDirection, initialBoostFactor);
        ...
        // Normal phase - continue at regular speed
        while (moveTimer < moveDuration && distanceMoved < moveDistance)
        {
            if (!isWandering)
                break;
