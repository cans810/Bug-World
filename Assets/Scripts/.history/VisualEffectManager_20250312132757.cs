...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private RectTransform chitinPanel;
    ...
    private IEnumerator SpawnChitinCollectEffect(int count)
    {
        ...
        Vector3 screenPosition = RectTransformUtility.WorldToScreenPoint(Camera.main, chitinPanel.position);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create random offset in screen space
            Vector2 randomOffset = Random.insideUnitCircle * spreadRadius;
            Vector3 spawnScreenPos = screenPosition + new Vector3(randomOffset.x, randomOffset.y, 0);

            // Convert screen position to world position
            Ray ray = Camera.main.ScreenPointToRay(spawnScreenPos);
            float distanceToCamera = 10f; // Adjust this value based on your needs
    ...
    {
        // Get the screen position of the start and end points
        Vector3 startScreenPos = Camera.main.WorldToScreenPoint(symbol.transform.position);
        Vector3 endScreenPos = Camera.main.WorldToScreenPoint(nestTransform.position);
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            Vector3 currentScreenPos = Vector3.Lerp(startScreenPos, endScreenPos, curveValue);
            
            // Convert back to world space
            Ray ray = Camera.main.ScreenPointToRay(currentScreenPos);
            float distanceToCamera = 10f;
            symbol.transform.position = ray.GetPoint(distanceToCamera);
