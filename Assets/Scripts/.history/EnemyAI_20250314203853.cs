...
public class EnemyAI : MonoBehaviour
{
    ...
    // Add this method to detect both trigger and non-trigger collisions
    private void OnCollisionEnter(Collision collision)
    {
        // Check if the collider is in the MapBorder layer
        if (collision.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            HandleBoundaryCollision(collision.contacts[0].normal);
        }
    }

    // Keep the existing OnTriggerExit for compatibility
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            HandleBoundaryCollision(directionToCenter);
        }
    }

    // New method to handle boundary collision with a given normal vector
    private void HandleBoundaryCollision(Vector3 collisionNormal)
    {
        // Ensure the normal is on the horizontal plane
        collisionNormal.y = 0;
        ...
        Vector3 reflectionDirection = Vector3.Reflect(currentDirection, collisionNormal);
        
        // Move enemy back slightly inside boundary
        transform.position += collisionNormal * 0.5f;
        ...
        // Disable wandering behavior temporarily
        if (wanderingBehavior != null)
        {
        ...
        // Force immediate movement in the new direction
        StartCoroutine(BounceMovement(newDirection));
    }

    // Add this coroutine to force movement after a bounce
    private IEnumerator BounceMovement(Vector3 direction)
    {
        // Wait a frame to ensure everything is set up
        yield return null;
        
        // Force movement in the bounce direction
        float bounceDistance = 2.0f;
        float bounceSpeed = livingEntity.moveSpeed * 1.5f; // Slightly faster bounce
        float distanceMoved = 0;
        
        while (distanceMoved < bounceDistance)
        {
            // Skip if we're no longer in boundary collision state
            if (!isHittingBoundary)
                break;
        ...
        // After bouncing, return to wandering
        ReturnToWandering();
    }
} 