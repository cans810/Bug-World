...
public class EnemyAI : MonoBehaviour
{
    ...
    private void ChaseAndAttackTarget()
    {
        ...
        {
            // Stop moving - make sure animations transition cleanly
            if (isMoving)
            {
                isMoving = false;
                UpdateAnimation(false); // This will set idle to true and walking to false
            }
        ...
        else
        {
            ...
            // Only update animation state if it's changing
            if (!isMoving)
            {
                isMoving = true;
                UpdateAnimation(true); // This will set walking to true and idle to false
            }
    ...
    // Helper method to update animations - updated with proper state management
    private void UpdateAnimation(bool isWalking)
    {
        if (animController != null)
        {
            // Ensure we're setting exclusive animation states
            if (isWalking)
            {
                // Set walking and ensure idle is off
                animController.SetWalking(true);
                
                // Some animation controllers might need explicit idle state handling
            ...
            {
                // Set idle and ensure walking is off
                animController.SetWalking(false);
                
                // Some animation controllers might need explicit idle state setting
                if (!animController.IsIdle())
                {
