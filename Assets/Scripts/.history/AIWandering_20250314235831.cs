...
public class AIWandering : MonoBehaviour
{
    ...
    // Enhanced border detection with multiple rays
    private bool IsBorderInDirection(Vector3 direction, float distance)
    {
        // Cast multiple rays in a cone pattern for better detection
        Vector3 rayOrigin = transform.position + Vector3.up * 0.5f;
        
        // Cast the main ray in the primary direction
        RaycastHit hit;
        Debug.DrawRay(rayOrigin, direction * distance, Color.red, 0.5f);
        if (Physics.Raycast(rayOrigin, direction, out hit, distance, borderLayer))
        {
            return true; // Border detected in primary direction
        }
        
        // Cast additional rays to cover a wider angle (30 degrees left and right)
        Vector3 leftDirection = Quaternion.Euler(0, -30, 0) * direction;
        Vector3 rightDirection = Quaternion.Euler(0, 30, 0) * direction;
        
        // Draw debug rays for visualization
        Debug.DrawRay(rayOrigin, leftDirection * distance, Color.yellow, 0.5f);
        Debug.DrawRay(rayOrigin, rightDirection * distance, Color.yellow, 0.5f);
        
        // Check the additional rays
        if (Physics.Raycast(rayOrigin, leftDirection, out hit, distance, borderLayer) ||
            Physics.Raycast(rayOrigin, rightDirection, out hit, distance, borderLayer))
        {
            return true; // Border detected in side directions
        }
        
        // No border detected in any of the rays
        return false;
    }

    // Fixed GetRandomDirection method to avoid recursion
    private Vector3 GetRandomDirection()
    {
        // For border avoidance, we need to be more careful to avoid infinite recursion
        int attempts = 0;
        int maxAttempts = 10;
        ...
        while (attempts < maxAttempts)
        {
            ...
            if (!avoidBorders || !IsBorderInDirection(direction, borderDetectionDistance))
            {
                return direction; // Found safe direction
            }
            
            attempts++;
        }
        
        // If all attempts failed, return direction toward center (if we have a reference point)
        if (playerNestTransform != null)
        {
            return (playerNestTransform.position - transform.position).normalized;
        }
        
        // Absolute fallback - pick a random direction but warn in console
        Debug.LogWarning($"Entity {gameObject.name} couldn't find a safe direction away from borders");
        float fallbackAngle = Random.Range(0f, 360f);
        return Quaternion.Euler(0, fallbackAngle, 0) * Vector3.forward;
    }
    ...
    // Add this method to force a new waypoint in a specific direction
    public void ForceNewWaypointInDirection(Vector3 direction)
    {
        ...
        if (avoidBorders && IsBorderInDirection(direction, borderDetectionDistance))
        {
            // Find a safer direction that's somewhat close to the original
            Vector3 saferDirection = FindSafeDirection();
            
            // Blend with original direction to maintain some of the intent
            direction = Vector3.Lerp(direction, saferDirection, 0.7f).normalized;
        }
        
        // Set a waypoint at a random distance in the given direction
        float randomDistance = Random.Range(minWanderDistance, maxWanderDistance);
        
        // Limit distance if border is detected
        if (avoidBorders)
        {
            RaycastHit hit;
            if (Physics.Raycast(transform.position + Vector3.up * 0.5f, direction, out hit, randomDistance, borderLayer))
            {
                // Border detected - set distance to 80% of the distance to the border
                randomDistance = hit.distance * 0.8f;
            }
        }
        
        Vector3 newWaypoint = transform.position + direction * randomDistance;
        
        // For enemies, check nest proximity
        if (!IsAlly())
        {
            // Make sure it's not too close to nest
            if (IsPositionTooCloseToNest(newWaypoint) && playerNestTransform != null)
            {
                // Try to find a point that's away from the nest but still in the desired direction
                Vector3 awayFromNest = GetDirectionAwayFromNest(transform.position);
