...
public class GameManager : MonoBehaviour
{
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (playerAttributes != null)
        {
            ...
            // Use the saved available points instead of calculating them
            // Only use calculation as a fallback if availableAttributePoints doesn't exist in saved data
            if (gameData.availableAttributePoints >= 0) // Check if field exists in saved data
            {
            ...
            else
            {
                ...
                int calculatedPoints = (gameData.currentLevel - 1) - 
                    (gameData.currentStrength + gameData.currentVitality + 
                     gameData.currentAgility + gameData.currentIncubation);
                SetPrivateField(playerAttributes, "availablePoints", Mathf.Max(0, calculatedPoints));
            }
            
            // Recalculate available points based on level and already spent points (for UI updates)
            playerAttributes.RecalculateAvailablePoints();
        }
