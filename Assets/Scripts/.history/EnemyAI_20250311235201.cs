...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        if (isTooCloseToNest)
        {
            ...
            // If we're also outside boundary, blend the directions
            if (isOutsideBoundary)
            {
                Vector3 safePosition = mapBoundary.GetNearestPointInBounds(nextPosition);
                Vector3 towardsBoundary = (safePosition - transform.position).normalized;
                
                // Blend the two directions (50% away from nest, 50% towards boundary)
                Vector3 blendedDirection = (awayFromNest + towardsBoundary).normalized;
                
                // Update facing direction
                transform.forward = blendedDirection;
                
                // Move in the blended direction
                transform.position += blendedDirection * livingEntity.moveSpeed * Time.deltaTime;
            }
            else
            {
    ...
    // Modify ChaseAndAttackTarget to respect nest boundaries
    private void ChaseAndAttackTarget()
    {
            ...
            !IsPositionTooCloseToNest(transform.position))
        {
            ...
            // Face the target but don't move closer
        ...
        else
        {
            ...
            // Move with boundary and nest check
            MoveWithBoundaryCheck(transform.forward);
        }
    }
