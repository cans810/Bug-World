...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Modify the AddExperience method to handle level skipping properly
    public void AddExperience(int amount)
    {
        ...
        _experience += amount; // Use the field directly instead of TotalExperience property
        
        // Check if we've leveled up
        int newLevel = GetLevelFromExperience(_experience); // Use existing method
        
        // If we've leveled up
        if (newLevel > previousLevel)
        {
            // Important: Check each level we passed through for area unlocks
            for (int level = previousLevel + 1; level <= newLevel; level++)
            {
                ...
                // If this level has an area unlock, stop here and let the camera animation play
                if (hasAreaUnlock && level < newLevel)
                {
                    // Set the current level to this level
                    CurrentLevel = level;
                    
                    // Schedule the remaining level-ups to happen after the animation
                    StartCoroutine(ContinueLevelUpsAfterAnimation(level + 1, newLevel));
                    
                    // Exit the method to prevent further level-ups until animation completes
                    return;
                }
            }
            
            // If we get here, either there were no area unlocks or we've processed them all
            CurrentLevel = newLevel;
        }
        
        // Trigger the experience changed event
        OnExperienceChanged?.Invoke(_experience);
    }

    // Add this method to process a single level-up
    private void ProcessLevelUp(int level)
    {
        Debug.Log($"Processing level up to level {level}");
        
        // Add attribute points (use a default value of 1 if pointsPerLevel doesn't exist)
        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
        if (playerAttributes != null)
        {
            playerAttributes.AddAttributePoints(1); // Default to 1 point per level
        }
    ...
    // Add this coroutine to continue level-ups after animation
    private IEnumerator ContinueLevelUpsAfterAnimation(int startLevel, int endLevel)
    {
        ...
        // Finally set the level to the end level
        CurrentLevel = endLevel;
        
        // Trigger the experience changed event to update UI
        OnExperienceChanged?.Invoke(_experience);
    }
