...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private Vector2 chitinPanelCenter;
    private Vector2 xpPanelCenter;
    ...
    private void InitializePanelPositions()
    {
        ...
        // Calculate and cache the chitin panel position
        if (chitinPanel != null)
        {
        ...
        // Calculate and cache the XP panel position
        if (xpPanel != null)
        {
    ...
    // Called on first layout
    private void Start()
    {
        // This is a backup to make sure panel positions are initialized after layout
        if (!panelPositionsInitialized)
        {
            InitializePanelPositions();
        }
    }
    ...
    public void PlayChitinDepositEffect(int chitinAmount, int xpAmount)
    {
        ...
        // Make sure panel positions are initialized
        if (!panelPositionsInitialized)
        {
            InitializePanelPositions();
        }
        ...
        // Start the coroutine for spawning symbols - use a faster method
        StartCoroutine(SpawnAllChitinSymbolsImmediately(symbolCount, xpAmount));
    }
    
    // New faster method to spawn all symbols immediately
    private IEnumerator SpawnAllChitinSymbolsImmediately(int count, int xpAmount)
    {
        ...
        // First, create all chitin symbols at once
        GameObject[] chitinSymbols = new GameObject[count];
        for (int i = 0; i < count; i++)
        {
            // Create chitin symbol
            chitinSymbols[i] = Instantiate(chitinSymbolPrefab, transform);
            RectTransform symbolRT = chitinSymbols[i].GetComponent<RectTransform>();
            
            // Apply the smaller scale
            symbolRT.localScale = chitinSymbolScale;
            
            // Position near chitin panel with slight randomization
            symbolRT.position = chitinPanelCenter + new Vector2(
                Random.Range(-20f, 20f),
                Random.Range(-20f, 20f)
            );
        }
        
        // Very short delay to ensure all are instantiated
        yield return null;
        
        // Now start animating them with staggered timing
        for (int i = 0; i < count; i++)
        {
            ...
            StartCoroutine(AnimateChitinToCenter(chitinSymbols[i], screenCenter, i, xpForThisChitin));
            
            // Much shorter delay between animation starts
            if (i < count - 1) // Don't delay after the last one
            {
                yield return new WaitForSeconds(spawnDelay * 0.2f); // 80% reduction in delay
            }
        }
    }
    ...
    {
        // Use cached XP panel position
        if (xpPanelCenter == Vector2.zero)
        {
        ...
        // Pre-instantiate all XP symbols first
        GameObject[] xpSymbols = new GameObject[count];
        for (int i = 0; i < count; i++)
        {
            // Create XP symbol
            xpSymbols[i] = Instantiate(xpSymbolPrefab, transform);
            RectTransform symbolRT = xpSymbols[i].GetComponent<RectTransform>();
        ...
        // Now animate them with staggered timing
        for (int i = 0; i < count; i++)
        {
            // Start animation
            StartCoroutine(AnimateXPToPanel(xpSymbols[i], xpPanelCenter));
            
            // Very short delay between animation starts
            if (i < count - 1) // Don't delay after the last one
            {
                yield return new WaitForSeconds(spawnDelay * 0.2f); // 80% reduction in delay
            }
