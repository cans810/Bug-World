...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject attributeSymbolPrefab;
    ...
    private void Awake()
    {
        ...
        InitializePool("attribute", attributeSymbolPrefab);
    ...
    public void PlayLevelUpEffects(Vector3 playerPosition)
    {
        // Spawn attribute symbols
        StartCoroutine(SpawnLevelUpSymbols(playerPosition, "attribute", 2, attributesPanel));
        
        // Spawn chitin symbols
        StartCoroutine(SpawnLevelUpSymbols(playerPosition, "chitin", 5, chitinPanel));
        
        // Spawn crumb symbols
        StartCoroutine(SpawnLevelUpSymbols(playerPosition, "crumb", 2, crumbPanel));
    }

    private IEnumerator SpawnLevelUpSymbols(Vector3 playerPosition, string symbolType, int count, GameObject targetPanel)
    {
        // Get the appropriate prefab based on symbol type
        GameObject prefab = null;
        switch (symbolType)
        {
            case "attribute":
                prefab = attributeSymbolPrefab;
                break;
            case "chitin":
        ...
        if (prefab == null || targetPanel == null) yield break;
        ...
        for (int i = 0; i < count; i++)
        {
            if (!symbolPools.ContainsKey(symbolType) || symbolPools[symbolType].Count == 0)
            {
                // Create a new symbol if pool is empty
                ...
                {
                    symbolPools[symbolType] = new Queue<GameObject>();
                }
                symbolPools[symbolType].Enqueue(newSymbol);
            }

            GameObject symbol = symbolPools[symbolType].Dequeue();
            
            // Start at player position with a small random offset
            Vector3 startPos = playerPosition + new Vector3(
            ...
            // Start the animation to the UI position
            StartCoroutine(AnimateLevelUpSymbol(symbol, startPos, targetScreenPos, symbolType));

            yield return new WaitForSeconds(0.1f);
        }
    }

    private IEnumerator AnimateLevelUpSymbol(GameObject symbol, Vector3 startWorldPos, Vector2 targetScreenPos, string symbolType)
    {
        SpriteRenderer spriteRenderer = symbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;
        ...
        Vector3 endWorldPos = ray.GetPoint(10f); // 10 units from camera
        ...
        while (elapsed < duration)
        {
            ...
            // Move symbol along a curve
            Vector3 a = Vector3.Lerp(startWorldPos, endWorldPos, t);
            Vector3 b = a + Vector3.up * Mathf.Sin(t * Mathf.PI) * 2f;
            symbol.transform.position = Vector3.Lerp(a, b, t);

            // Fade out at the end
            if (t > 0.8f)
            {
                float alpha = 1f - ((t - 0.8f) / 0.2f);
                spriteRenderer.color = new Color(startColor.r, startColor.g, startColor.b, alpha);
            }
        ...
        symbolPools[symbolType].Enqueue(symbol);
    }
} 