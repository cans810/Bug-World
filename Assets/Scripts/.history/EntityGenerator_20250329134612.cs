...
public class EntityGenerator : MonoBehaviour
{
    ...
    [SerializeField] private float spawnRadius = 30f; // Controls overall spawn area size
    [SerializeField] private Vector3 boxDimensions = new Vector3(60f, 1f, 60f); // Dimensions for box (can be proportional to radius)
    [SerializeField] private bool linkRadiusToBox = false; // When true, spawnRadius affects box size
    ...
    private bool TryGetSpawnPosition(out Vector3 position)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            {
                // Box shape spawning
                Vector3 dimensions = boxDimensions;
                
                // If linking radius to box, scale the box dimensions
                if (linkRadiusToBox)
                {
                    // Calculate a squared box with the same area as the circle
                    // Area of circle = π * r² 
                    // Area of square = side²
                    // side = √(π * r²) = r * √π
                    float side = spawnRadius * Mathf.Sqrt(Mathf.PI);
                    
                    // Use the radius to set box dimensions while maintaining aspect ratio
                    float aspectRatioX = boxDimensions.x / Mathf.Max(boxDimensions.x, boxDimensions.z);
                    float aspectRatioZ = boxDimensions.z / Mathf.Max(boxDimensions.x, boxDimensions.z);
                    
                    dimensions.x = side * aspectRatioX;
                    dimensions.z = side * aspectRatioZ;
                }
                
                randomPoint = center + new Vector3(
                    Random.Range(-dimensions.x * 0.5f, dimensions.x * 0.5f),
                    0,
                    Random.Range(-dimensions.z * 0.5f, dimensions.z * 0.5f)
                );
            }
            else
            {
