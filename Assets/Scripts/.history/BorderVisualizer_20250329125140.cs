...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Calculate the half-length of the capsule's central line
        float halfLength = (height - 2 * radius) * 0.5f;
        
        // We need enough points to make each section look smooth
        int pointsPerSection = segments / 4; // 4 sections: 2 semicircles + 2 straight lines
        int index = 0;
        
        // For each of the four sections:
        // 1. Right semicircle (top half)
        // 2. Right to left straight line (bottom)
        // 3. Left semicircle (bottom half)
        // 4. Left to right straight line (top)
        
        // 1. Right semicircle (first quarter of the perimeter)
        for (int i = 0; i <= pointsPerSection; i++)
        {
            float t = (float)i / pointsPerSection;
            float angle = -Mathf.PI/2 + t * Mathf.PI; // -90° to +90°
            
            // Calculate positions for the right semicircle
            if (direction == 0) // X-axis
            {
                float x = center.x + halfLength;
                float z = center.z + radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
            else if (direction == 2) // Z-axis
            {
                float x = center.x + radius * Mathf.Sin(angle);
                float z = center.z + halfLength;
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
            else // Y-axis
            {
                float angle2 = t * 2 * Mathf.PI / 4; // First quarter of circle
                float x = center.x + radius * Mathf.Cos(angle2);
                float z = center.z + radius * Mathf.Sin(angle2);
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
        }
        
        // 2. Bottom straight line (second quarter)
        for (int i = 0; i <= pointsPerSection; i++)
        {
            float t = (float)i / pointsPerSection;
            ...
            if (direction == 0) // X-axis
            {
                float x = center.x + halfLength - t * 2 * halfLength;
            ...
            else if (direction == 2) // Z-axis
            {
                ...
                float z = center.z + halfLength - t * 2 * halfLength;
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
            else // Y-axis
            {
                float angle = Mathf.PI/2 + t * 2 * Mathf.PI / 4; // Second quarter of circle
                float x = center.x + radius * Mathf.Cos(angle);
                float z = center.z + radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
        ...
        // 3. Left semicircle (third quarter)
        for (int i = 0; i <= pointsPerSection; i++)
        {
            float t = (float)i / pointsPerSection;
            ...
            {
                float angle2 = Mathf.PI + t * 2 * Mathf.PI / 4; // Third quarter of circle
                float x = center.x + radius * Mathf.Cos(angle2);
                float z = center.z + radius * Mathf.Sin(angle2);
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
        ...
        // 4. Top straight line (fourth quarter) - completes the loop
        for (int i = 0; i < pointsPerSection; i++) // Note the < instead of <= to avoid duplicating the first point
        {
            float t = (float)i / pointsPerSection;
            ...
            {
                float angle = 3*Mathf.PI/2 + t * 2 * Mathf.PI / 4; // Fourth quarter of circle
                float x = center.x + radius * Mathf.Cos(angle);
                float z = center.z + radius * Mathf.Sin(angle);
                perimeterLineRenderer.SetPosition(index++, new Vector3(x, perimeterHeight, z));
            }
