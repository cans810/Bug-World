...
public class OnScreenJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [SerializeField] private string touchableSortingLayer = "TouchableUI"; // UI elements with this sorting layer can be touched through
    [SerializeField] private bool debugSortingLayers = false; // Set to true to log sorting layer info
    ...
    private void Update()
    {
        ...
        if (useDirectInput)
        {
            ...
            if (UnityEngine.Input.touchCount > 0)
            {
                ...
                // First check if we're touching another UI element
                ...
                if (ignoreUIElements)
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        // Check if this element has a SpriteRenderer with the TouchableUI sorting layer
                        SpriteRenderer spriteRenderer = result.gameObject.GetComponent<SpriteRenderer>();
                        if (spriteRenderer != null && spriteRenderer.sortingLayerName == touchableSortingLayer)
                        {
                            if (debugSortingLayers)
                            {
                                Debug.Log($"UI element {result.gameObject.name} has TouchableUI sorting layer, allowing joystick activation");
                            }
                        ...
                        if (((1 << result.gameObject.layer) & uiLayerMask) != 0)
                        {
                            if (showDebugInfo)
                            {
                                Debug.Log($"Touch hit UI element: {result.gameObject.name}, ignoring joystick activation");
                                if (debugSortingLayers && spriteRenderer != null)
                                {
                                    Debug.Log($"Sorting layer: {spriteRenderer.sortingLayerName}, Order in layer: {spriteRenderer.sortingOrder}");
                                }
                            }
                ...
                // Only proceed if we're not touching another UI element
                if (!touchingOtherUI)
                {
                    // Handle touch begin
            ...
            {
                // First check if we're clicking another UI element
                ...
                if (ignoreUIElements)
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        if (((1 << result.gameObject.layer) & uiLayerMask) != 0)
                        {
                            if (showDebugInfo)
                            {
                                Debug.Log($"Click hit UI element: {result.gameObject.name}, ignoring joystick activation");
                            }
                ...
                // Only proceed if we're not clicking another UI element
                if (!clickingOtherUI)
                {
    ...
    private void HandleDirectInput(Vector2 inputPosition)
    {
        ...
        if (showDebugInfo && Time.frameCount % 30 == 0)
        {
            ...
            Debug.Log($"Handle distance: {Vector2.Distance(joystickCenter, joystickHandle.position)}, Max radius: {maxRadius}");
        }
    }
