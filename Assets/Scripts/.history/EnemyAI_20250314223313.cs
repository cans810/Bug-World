...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // If player is in hitbox, don't do any regular AI behavior
        if (isPlayerInHitbox)
            return;
    ...
    // New method to handle player entering hitbox with stronger immobilization
    private void HandlePlayerEnterHitbox(LivingEntity playerEntity)
    {
        ...
        // Stop all movement - be very explicit
        isMoving = false;
        isWandering = false;
        inCombatStance = false; // Make sure we're not in combat stance which has its own movement logic
        
        // Completely disable the wandering behavior
        if (wanderingBehavior != null)
        {
            wanderingBehavior.SetWanderingEnabled(false);
            wanderingBehavior.enabled = false;
        ...
        // Cancel any movement coroutines
        StopAllCoroutines();
        
        // Freeze rigidbody if one exists
        Rigidbody rb = GetComponent<Rigidbody>();
        if (rb != null)
        {
            rb.velocity = Vector3.zero;
            rb.angularVelocity = Vector3.zero;
            rb.isKinematic = true; // Make kinematic to prevent physics affecting position
        }
        
        // Store position and rotation to enforce remaining still
        Vector3 fixedPosition = transform.position;
        Quaternion fixedRotation = transform.rotation;
        
        // Apply idle animation immediately
        if (animController != null)
        {
            animController.SetIdle();
        }
        
        // Start attacking in place without moving
        StartCoroutine(AttackInPlaceWithFixedPosition(fixedPosition, fixedRotation));
    }

    // Improved coroutine that maintains exact position and rotation while attacking
    private IEnumerator AttackInPlaceWithFixedPosition(Vector3 fixedPosition, Quaternion fixedRotation)
    {
        while (isPlayerInHitbox && !livingEntity.IsDead)
        {
            // Force position and rotation to remain constant
    ...
    // New method to handle player exiting hitbox
    private void HandlePlayerExitHitbox(LivingEntity playerEntity)
    {
        ...
        // Return to wandering after a short delay
        StartCoroutine(ReturnToWanderingAfterDelay(1.5f));
    }

    // New coroutine to return to wandering after delay
    private IEnumerator ReturnToWanderingAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        
        // Only return to wandering if player is still not in hitbox
