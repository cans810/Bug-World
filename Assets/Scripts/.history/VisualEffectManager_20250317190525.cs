...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpValue)
    {
        ...
        // Calculate speed multiplier based on total XP symbols
        int totalXPSymbols = symbolPools["xp"].Count;
        float speedMultiplier = Mathf.Clamp(1 + (totalXPSymbols * 0.1f), 1, 3); // 1x to 3x speed
