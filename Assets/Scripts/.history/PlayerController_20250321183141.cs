...
public class PlayerController : MonoBehaviour
{
    ...
    // Add a reference to PlayerAttributes
    [SerializeField] private PlayerAttributes playerAttributes;

    // Current border tracking
    private string currentBorderName = "";
    private int currentBorderLevel = 0;
    private bool currentBorderAccessible = true;
    ...
    private void Start()
    {
        ...
        // In Start or Awake method, find it if not assigned
    ...
    private void HandleMovement()
    {
        ...
        if (currentSpeed > 0.01f)
        {
            ...
            if (shouldMove)
            {
                // Get the speed multiplier from player attributes
                float speedMultiplier = 1f;
                if (playerAttributes != null)
                {
                    speedMultiplier = playerAttributes.SpeedMultiplier;
                }
                
                if (livingEntity != null)
                {
                    // Apply the final movement with current speed and speed multiplier
                    livingEntity.MoveInDirection(finalMoveDirection, (currentSpeed * speedMultiplier) / livingEntity.moveSpeed);
                }
                else
                {
                    // Fallback if living entity not available
                    Vector3 motion = transform.forward * currentSpeed * speedMultiplier * Time.deltaTime;
    ...
    // Method to set the current border when player enters a border trigger
    public void SetCurrentBorder(string borderName, int requiredLevel, Vector3 borderPosition)
    {
        // Only update the current border name if it's different from the current one
        if (currentBorderName != borderName)
        {
            currentBorderName = borderName;
            currentBorderLevel = requiredLevel;
            currentBorderAccessible = true;
            
            // Inform UI or other systems about the border change
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null && showBorderMessages)
            {
                if (Time.time - lastBorderMessageTime > borderMessageCooldown)
                {
                    // Format a nice border name for display
                    string displayName = FormatBorderName(borderName);
                    uiHelper.ShowInformText($"Entering {displayName}");
                    lastBorderMessageTime = Time.time;
                }
            }
            
            Debug.Log($"Border changed to: {borderName} (Level: {requiredLevel})");
        }
    }

    // Method to mark a border as inaccessible due to level requirements
    public void SetBorderInaccessible(string borderName, int requiredLevel)
    {
        // IMPORTANT: Don't change the current border name
        // Just set the flag that indicates the attempted border is inaccessible
        currentBorderAccessible = false;
        
        // Show a message about the required level
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null && showBorderMessages)
        {
            if (Time.time - lastBorderMessageTime > borderMessageCooldown)
            {
                uiHelper.ShowInformText($"Level {requiredLevel} required to enter this area");
                lastBorderMessageTime = Time.time;
            }
        }
        ...
        if (showDebugMessages)
        {
            Debug.Log($"Border {borderName} is inaccessible - requires level {requiredLevel}");
        }
    }

    // Helper method to format border names nicely
    private string FormatBorderName(string rawName)
    {
        // Remove "MapBorder" prefix if present
        string displayName = rawName.Replace("MapBorder", "Area ");
        
        // Remove numeric border markers if needed
        displayName = System.Text.RegularExpressions.Regex.Replace(displayName, @"Border\d+", "Area");
