...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        if (uiEffects != null)
        {
            // Pass the deposited chitin amount for visual effects
            uiEffects.PlayChitinDepositEffect(depositedChitinAmount, xpAmount);
            Debug.Log("Deposited chitin count: " + depositedChitinAmount);
            
            // IMPORTANT: Add the XP to player inventory BEFORE waiting for animations
            // This ensures XP is registered even if animations are interrupted
            if (playerInventory != null)
            {
                // Add XP to player inventory
                playerInventory.AddXP(xpAmount);
                Debug.Log($"*** Added {xpAmount} XP to player inventory ***");
            }
        ...
        else
        {
            ...
            Debug.Log($"XP Gained: {xpAmount} (No UI effects)");
            
            // Add XP directly if no animations
            if (playerInventory != null)
            {
                playerInventory.AddXP(xpAmount);
        ...
        {
            Debug.LogWarning("Cannot update UI - playerInventory is null: " + (playerInventory == null));
        }
    }
