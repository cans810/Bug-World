...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Get the prefab based on symbol type
        ...
        // Create a pool for the symbols if it doesn't exist
        if (!symbolPools.ContainsKey(symbolType))
        {
            InitializePool(symbolType, prefab);
        }

        // Get the target position in screen space
        Vector2 targetScreenPos = GetPanelScreenPosition(targetPanel);

        for (int i = 0; i < count; i++)
        {
            if (symbolPools[symbolType].Count == 0) yield break;

            GameObject symbol = symbolPools[symbolType].Dequeue();
            
            // Start at player position with a small random offset
            Vector2 randomOffset = Random.insideUnitCircle * 0.5f;
            symbol.transform.position = playerPosition + new Vector3(randomOffset.x, 0.5f, randomOffset.y);
            ...
            // Start the animation to the UI position
            StartCoroutine(AnimateSymbolToScreen(symbol, targetScreenPos, symbolType));
    ...
    private Vector2 GetPanelScreenPosition(GameObject panel)
    {
        ...
        // Get the center position of the panel in screen space
        Vector2 screenPoint = RectTransformUtility.WorldToScreenPoint(Camera.main, rectTransform.position);
        return screenPoint;
    }

    private IEnumerator AnimateSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, string symbolType)
    {
        // Get the starting position (at the player)
        Vector3 startPos = symbol.transform.position;
        ...
        while (elapsed < duration)
        {
            ...
            Vector3 endPos = ray.GetPoint(10f); // 10 units from camera
            
            // Move symbol towards target
            symbol.transform.position = Vector3.Lerp(startPos, endPos, normalizedTime);
            
            // Scale down over time
            symbol.transform.localScale = Vector3.Lerp(initialScale, finalScale, normalizedTime);
