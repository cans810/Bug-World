...
public class PlayerController : MonoBehaviour
{
    ...
    private void HideBorderVisualization(GameObject borderObject)
    {
        ...
        if (visualizer != null)
        {
            ...
            // Use a unique identifier for the player that doesn't rely on playerName
            string playerIdentifier = "Player1"; // Default fallback
            
            // Try to get a unique ID if available
            PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
            if (playerAttributes != null)
            {
                // Use player's current level as part of the identifier
                playerIdentifier = "Player_" + GetComponent<PlayerInventory>()?.CurrentLevel.ToString() ?? "1";
            }
