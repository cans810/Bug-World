...
public class MissionManager : MonoBehaviour
{
    ...
    public void CompleteMission(string missionId)
    {
        ...
        // Play the mission complete sound effect
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("MissionComplete");
        }
        ...
        // Hide the mission panel with animation
        HideMissionPanel(missionId, true);
        
        // Activate next missions
        ActivateNextMissions(mission);
        
        // After mission is completed, save progress
        if (!isLoadingData)
        {
            if (gameManager != null)
            {
