...
public class AllyAI : MonoBehaviour
{
    ...
    private void MoveWithJoystick(Vector2 joystickInput)
    {
        ...
        if (isMoving)
        {
            // Use speed relative to distance from target position
            float speedFactor = 1.0f;
            if (distanceToTarget > followDistance * 2f)
            {
                speedFactor = 1.5f; // Move faster to catch up if very far
            }
            else if (distanceToTarget < followDistance * 0.5f)
            {
                speedFactor = 0.7f; // Move slower when close to target position
            }
    ...
    {
        // When player stops, move to proper follow position
        ...
        // Calculate target position behind player based on player's facing direction
        Vector3 targetPosition = playerTransform.position + (offsetDirection * followDistance);
        ...
        if (distanceToTarget > followDistance * 0.3f)
        {
            ...
            livingEntity.MoveInDirection(moveDirection, 0.7f); // Move slower when approaching idle position
            UpdateAnimation(true);
            hasAppliedMovementThisFrame = true;
        }
        else
        {
            // We've reached the target position, just face toward player
