...
public class AllyAntPanelController : MonoBehaviour
{
    ...
    private void CreateButtonForAllyAnt(GameObject allyAnt, int index)
    {
        ...
        // Set the button text
        string buttonLabel = isDead ? $"Ally Ant {index + 1} (Dead)" : $"Ally Ant {index + 1}";
    ...
    private void UpdateDeadAntButtons()
    {
        foreach (GameObject buttonObj in createdButtons)
        {
            ...
            if (buttonData != null && buttonData.LinkedAnt != null && button != null)
            {
                ...
                // Update button text if dead
                if (isDead)
                {
                    // Get the index from the current text
                    string currentText = "";
                    
                    // Try to get text from Text component
                    ...
                    // Extract the ant number and add (Dead) if not already there
                    if (!currentText.Contains("(Dead)"))
                    {
                        string newText = currentText + " (Dead)";
