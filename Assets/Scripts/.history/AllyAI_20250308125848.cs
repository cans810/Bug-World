...
public class AllyAI : MonoBehaviour
{
    ...
    private void CarryLootToBase()
    {
        ...
        // Calculate distance to base (using only X and Z coordinates)
        Vector3 basePositionFlat = new Vector3(baseTransform.position.x, transform.position.y, baseTransform.position.z);
        float distanceToBase = Vector3.Distance(transform.position, basePositionFlat);
        ...
        // Face directly toward base (flat Y)
        FaceTarget(basePositionFlat);
        
        // SIMPLIFIED DIRECT MOVEMENT:
        // Calculate direction vector on X/Z plane only
        Vector3 direction = basePositionFlat - transform.position;
        direction.y = 0; // Ensure no vertical movement
        direction.Normalize();
        
        // Move directly toward the base at the normal movement speed
        transform.position += direction * livingEntity.moveSpeed * Time.deltaTime;
    }
