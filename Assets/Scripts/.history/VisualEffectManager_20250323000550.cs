...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // COMPLETE REWRITE - direct linear path, no fancy animations
        SpriteRenderer spriteRenderer = symbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;
        ...
        // EXTREMELY SHORT DURATION - move quickly!
        float duration = 0.4f;
        
        // Define uiBumped variable that was missing
        bool uiBumped = false;
        ...
        if (coinPanel != null)
        {
            // Get target position directly from the EXACT CENTER of the coin panel
            RectTransform panelRect = coinPanel.GetComponent<RectTransform>();
            Canvas canvas = coinPanel.GetComponentInParent<Canvas>();
            ...
            Vector3 worldPos = (corners[0] + corners[1] + corners[2] + corners[3]) / 4f;
            
            // Convert world space to screen space
            Vector3 screenPos = cam.WorldToScreenPoint(worldPos);
            
            // Convert screen space to world space at proper distance
            targetPos = cam.ScreenToWorldPoint(new Vector3(screenPos.x, screenPos.y, 10f));
        ...
        {
            // Fallback - use provided screen position
            Ray ray = Camera.main.ScreenPointToRay(targetScreenPos);
            targetPos = ray.GetPoint(10f);
        }
        
        // Move directly in a straight line - NO ARCS
        float elapsed = 0f;
        while (elapsed < duration)
        {
            if (symbol == null || !symbol.activeInHierarchy) break;
            
            elapsed += Time.deltaTime;
            float t = Mathf.Clamp01(elapsed / duration);
            
            // DIRECT LINEAR MOVEMENT with no curves or arcs
            symbol.transform.position = Vector3.Lerp(startPos, targetPos, t);
            
            // Face camera
            symbol.transform.rotation = Camera.main.transform.rotation;
            
            // Trigger UI when 70% there
            if (t >= 0.7f && !uiBumped)
            {
        ...
        // FORCE to exact position at the end
