...
public class PlayerAttributes : MonoBehaviour
{
    ...
    public void SetMaxEggCapacity(int capacity)
    {
        maxEggCapacity = capacity;
        ...
        Debug.Log($"Max egg capacity set to: {maxEggCapacity}");
    }

    // Add this method to PlayerAttributes to handle loading the egg capacity
    public void LoadEggCapacityFromGameData(GameData data)
    {
        if (data != null && data.maxEggCapacity > 0)
        {
            // Set the incubation points based on the max egg capacity
            // Since MaxEggCapacity = 1 + incubationPoints, we need to subtract 1
            incubationPoints = data.maxEggCapacity - 1;
            
            Debug.Log($"Loaded egg capacity: {data.maxEggCapacity}, set incubation points to {incubationPoints}");
