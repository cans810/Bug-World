...
public class PlayerController : MonoBehaviour
{
    ...
    private IEnumerator ReviveAfterDelay(float delay)
    {
        ...
        // Stop any auto-attack coroutine that might be running
        if (autoAttackCoroutine != null)
        {
            StopCoroutine(autoAttackCoroutine);
            autoAttackCoroutine = null;
        ...
        if (livingEntity != null)
        {
            ...
            livingEntity.SetTemporaryInvulnerability(1f); // 1 second of invulnerability
        }
    }
    ...
    private void HandleMovement()
    {
        ...
        if (animController != null)
        {
            ...
            // If we're attacking and start moving, switch to walk attack
            if (animController.IsAnimationPlaying("attack") && shouldBeWalking)
            {
                animController.SetWalkAttacking(true);
            }
            // If we're walk attacking and stop moving, switch to regular attack
            else if (animController.IsAnimationPlaying("walkattack") && !shouldBeWalking)
            {
                animController.SetAttacking(true);
            }
            // Normal walking state
            else if (shouldBeWalking != animController.IsAnimationPlaying("walk"))
            {
        ...
        while (enabled && livingEntity != null && !livingEntity.IsDead && autoAttackEnabled)
        {
            ...
            if (enemiesInRange.Count > 0)
            {
                ...
                if (closestEnemy != null)
                {
                    ...
                    // Check if we're moving to determine attack type
