...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float legAnimationSpeed = 1.5f; // Controls how fast legs move relative to body speed
    ...
    private void HandleMovement()
    {
        ...
        if (animController != null)
        {
            ...
            // Adjust animation speed based on movement speed and leg animation multiplier
            if (currentSpeed > 0.1f)
            {
                float speedRatio = currentSpeed / moveSpeed; // 0-1 value
                animController.SetAnimationSpeed(speedRatio * legAnimationSpeed);
            }
            else
            {
                animController.SetAnimationSpeed(1f);
            }
        }
