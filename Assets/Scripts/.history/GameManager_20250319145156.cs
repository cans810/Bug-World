...
public class GameManager : MonoBehaviour
{
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (playerInventory != null)
        {
            ...
            // IMPORTANT: Explicitly set the level to ensure UI updates
    ...
    // Update LoadSavedEggs to make eggs children of the incubator
    private void LoadSavedEggs()
    {
        ...
        // Spawn eggs as children of the incubator
        foreach (var eggData in gameData.activeEggs)
        {
            // Instantiate egg as a child of the incubator at the egg position
            GameObject eggObj = Instantiate(
                incubator.GetEggPrefab(), 
                incubator.transform.position, // Use incubator position
                Quaternion.identity,
                incubator.transform // Set as child
            );
            
            // Position the egg at the egg spawn position
            if (incubator.GetEggSpawnPosition() != null)
            {
                eggObj.transform.position = incubator.GetEggSpawnPosition().position;
            }
