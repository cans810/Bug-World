...
public class UIHelper : MonoBehaviour
{
    ...
    // Simplified method that directly forces the panel to show
    public void ShowAttributesPanel()
    {
        Debug.Log("UIHelper: ShowAttributesPanel called");
        
        // Find the AttributeDisplay component
        AttributeDisplay attributeDisplay = FindObjectOfType<AttributeDisplay>();
        if (attributeDisplay != null)
        {
            // Use the force method instead of the safety-checked methods
            attributeDisplay.ForceShowPanel();
        }
        else
        {
            Debug.LogError("UIHelper: Cannot find AttributeDisplay component!");
        }
        
        // Also directly activate the panel if we have a reference to it
        if (attributesPanel != null)
        {
            Debug.Log("UIHelper: Directly activating attributesPanel GameObject");
            attributesPanel.SetActive(true);
        ...
        {
            Debug.LogWarning("UIHelper: No direct reference to attributesPanel");
        }
    }

    // Use this for UI button click events
    ...
    // Add this method that does NOTHING but show the panel directly
    public void DEBUG_DIRECT_SHOW_PANEL()
    {
        Debug.Log("======= DIRECT PANEL SHOW ATTEMPT =======");
        
        // Try multiple approaches to force the panel visible
        
        // First, check if we have a direct reference
        if (attributesPanel != null)
        {
            Debug.Log("UIHelper has direct panel reference: " + attributesPanel.name);
            
            // Force panel to be active regardless of any other state
            attributesPanel.SetActive(true);
            Debug.Log("Panel SetActive(true) called directly");
            
            // Try to find animator
            Animator panelAnim = attributesPanel.GetComponent<Animator>();
            if (panelAnim != null)
            {
                Debug.Log("Found animator component on panel");
                
                // Force all animation states that might show the panel
                panelAnim.SetBool("ShowUp", true);
                panelAnim.SetBool("Hide", false);
                panelAnim.SetBool("IsHidden", false);
                panelAnim.SetBool("IsVisible", true);
                
                Debug.Log("Set all possible animation states to show panel");
            }
            else
            {
                Debug.Log("No animator found on panel");
            }
            
            // Check if it has a CanvasGroup that might be affecting visibility
            CanvasGroup canvasGroup = attributesPanel.GetComponent<CanvasGroup>();
            if (canvasGroup != null)
            {
                Debug.Log("Found CanvasGroup component");
                canvasGroup.alpha = 1f;
                canvasGroup.interactable = true;
                canvasGroup.blocksRaycasts = true;
                Debug.Log("Set CanvasGroup to fully visible and interactive");
            }
        }
        else
        {
            Debug.Log("No direct panel reference, trying to find it");
            
            // Try to find by name (assuming it's named "AttributesPanel" or similar)
            GameObject foundPanel = GameObject.Find("AttributesPanel");
            if (foundPanel == null) foundPanel = GameObject.Find("AttributePanel");
            if (foundPanel == null) foundPanel = GameObject.Find("StatsPanel");
            
            if (foundPanel != null)
            {
                Debug.Log("Found panel by name: " + foundPanel.name);
                foundPanel.SetActive(true);
                Debug.Log("Panel SetActive(true) called directly on found panel");
            }
            else
            {
                Debug.Log("Could not find panel by name, trying direct AttributeDisplay reference");
            }
        }
        
        // Find and use AttributeDisplay regardless of our setup
        AttributeDisplay[] allAttributeDisplays = FindObjectsOfType<AttributeDisplay>(true); // Include inactive objects
        
        Debug.Log($"Found {allAttributeDisplays.Length} AttributeDisplay components");
        
        foreach (AttributeDisplay display in allAttributeDisplays)
        {
            Debug.Log($"Calling ForceShowPanel on AttributeDisplay on GameObject: {display.gameObject.name}");
            display.ForceShowPanel();
        }
        
        Debug.Log("======= DIRECT PANEL SHOW COMPLETE =======");
    }
} 
