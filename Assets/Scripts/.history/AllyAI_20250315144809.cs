...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (Time.time > lastPlayerVelocityCheckTime + playerVelocityCheckInterval)
        {
            ...
            {
                float playerMovementAmount = Vector3.Distance(playerTransform.position, lastPlayerPosition);
                playerIsMoving = playerMovementAmount > playerMovementThreshold;
            }
        ...
        // If player is not moving OR we're close enough, stop moving
        if (!playerIsMoving || distanceToPlayer <= followDistance)
        {
            // Stop moving
            isMoving = false;
            ...
            // No rotation when stopped - just maintain current orientation
        }
        // If player is moving and we're too far, move towards the player
        else
        {
            isMoving = true;
            ...
            // Calculate direction to player - this is the only direction we care about
            Vector3 directionToPlayer = (playerTransform.position - transform.position).normalized;
            directionToPlayer.y = 0; // Keep on same Y level
            
            // Rotate to face the player's position (not their forward direction)
