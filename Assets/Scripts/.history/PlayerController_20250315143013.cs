...
public class PlayerController : MonoBehaviour
{
    ...
    private IEnumerator CheckForEnemiesInRange()
    {
        while (enabled)
        {
            ...
            if (enemiesInRange.Count == 0 && autoAttackEnabled && livingEntity != null && !livingEntity.IsDead)
            {
                // Use a small radius for precise detection
                float attackRange = 2.5f;
                
                // Find all enemies in a radius around the player
                Collider[] colliders = Physics.OverlapSphere(transform.position, attackRange, 
                                                            LayerMask.GetMask("Insects"));
                ...
                foreach (Collider col in colliders)
                {
                    ...
                    if (enemy != null && !enemy.IsDead)
                    {
                        ...
                        if (!enemiesInRange.Contains(enemy))
                        {
                            ...
                            if (showDebugMessages)
                            {
                                Debug.Log($"Found enemy in range: {enemy.name}");
                            }
                        }
                    }
                }
