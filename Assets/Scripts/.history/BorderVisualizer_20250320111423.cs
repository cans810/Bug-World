...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void CreateVisualSphere()
    {
        if (showRadius && sphereCollider != null)
        {
            ...
            if (fillCircle)
            {
                // Create a disc to represent the filled circle
                // We'll use a quad and scale it to the radius
                GameObject disc = GameObject.CreatePrimitive(PrimitiveType.Quad);
                disc.name = "FilledDisc";
                ...
                // Scale the quad to match the radius
                float diameter = sphereCollider.radius * 2;
                disc.transform.localScale = new Vector3(diameter, diameter, 1f);
                
                // Create a material for the disc
                Material discMaterial = new Material(Shader.Find("Sprites/Default"));
                discMaterial.color = radiusColor;
                ...
                // Create additional discs for different viewing angles
                CreateOrthogonalDisc(radiusObj.transform, diameter, discMaterial, "FilledDiscX", Vector3.right, Vector3.up);
                CreateOrthogonalDisc(radiusObj.transform, diameter, discMaterial, "FilledDiscY", Vector3.up, Vector3.forward);
    ...
    // Helper method to create discs in orthogonal planes
    private void CreateOrthogonalDisc(Transform parent, float diameter, Material material, string name, Vector3 right, Vector3 up)
    {
        GameObject disc = GameObject.CreatePrimitive(PrimitiveType.Quad);
        disc.name = name;
        ...
        // Position and orient the disc
        disc.transform.localRotation = Quaternion.LookRotation(Vector3.Cross(right, up), up);
        disc.transform.localScale = new Vector3(diameter, diameter, 1f);
        ...
        if (visualSphere != null && sphereCollider != null)
        {
            if (showRadius)
            {
                if (fillCircle)
                {
                    ...
                    {
                        if (child.localScale.x != diameter)
                        {
                            child.localScale = new Vector3(diameter, diameter, 1f);
                        }
                    }
