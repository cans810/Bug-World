...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float[] metamorphosisLevels = new float[] { 11f, 22f, 33f, 44f, 55f };
    [SerializeField] private float[] scaleMultipliers = new float[] { 1.2f, 1.4f, 1.6f, 1.8f, 2.0f };
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (other is SphereCollider sphereCollider)
            {
                ...
                // Calculate direction from player to sphere center (INWARD direction)
                borderNormal = (sphereCenter - transform.position).normalized;
            }
            else if (other is CapsuleCollider capsuleCollider)
            {
                // For capsule, calculate closest point on collider surface
                Vector3 closestPoint = other.ClosestPoint(transform.position);
                // Direction from player to closest point (INWARD direction)
                borderNormal = (closestPoint - transform.position).normalized;
            }
            else
            {
                // For all other collider types
                Vector3 closestPoint = other.ClosestPoint(transform.position);
                borderNormal = (closestPoint - transform.position).normalized;
            }
    ...
    private void ApplyMetamorphosisScale(int level)
    {
        ...
        for (int i = metamorphosisLevels.Length - 1; i >= 0; i--)
        {
            ...
            {
                Vector3 newScale = baseScale * scaleMultipliers[i];
                transform.localScale = newScale;
                
                Debug.Log($"Applied metamorphosis scale {scaleMultipliers[i]}x for level {level}");
                return;
            }
        }
        
        // If no metamorphosis level reached, use base scale
        transform.localScale = baseScale;
    }

    // Add this method to check for metamorphosis on level up
    public void CheckMetamorphosis(int newLevel)
    {
        if (!enableMetamorphosis) return;
        ...
        for (int i = 0; i < metamorphosisLevels.Length; i++)
        {
            if (newLevel == metamorphosisLevels[i])
            {
                ...
                {
                    cameraController.UpdateCameraSettingsForLevel(newLevel);
                }
                
                Debug.Log($"<color=yellow>METAMORPHOSIS: Level {newLevel} reached! New scale: {scaleMultipliers[i]}x</color>");
                return;
            }
        }
    }
    ...
    // Add this debugging method to visualize border normals
    void OnDrawGizmos()
    {
        if (isAtMapBorder && borderNormal != Vector3.zero)
        {
            // Draw a line showing the border normal direction
            Gizmos.color = Color.red;
            Gizmos.DrawLine(transform.position, transform.position + borderNormal * 2f);
            
            // Draw a sphere at the end point
            Gizmos.DrawSphere(transform.position + borderNormal * 2f, 0.2f);
        }
    }
}