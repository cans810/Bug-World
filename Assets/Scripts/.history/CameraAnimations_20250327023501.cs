...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator MetamorphosisCameraAnimation(Transform playerTransform, System.Action onCameraInPosition)
    {
        ...
        // UPDATED: Position camera in front and above the player
        // Use player's forward direction (but negative to go to front)
        Vector3 playerForward = playerTransform.;
        Vector3 playerUp = Vector3.up;
        
        // Calculate positioning values based on player scale, but larger to be further away
        float forwardOffset = playerTransform.localScale.z * 5.0f; // Increased distance for front view
        float heightOffset = playerTransform.localScale.y * 4.0f;  // Height for looking down
        
        // Position camera in front of the player (negative forward direction)
        ...
        // Calculate rotation to look down at the player from this front position
