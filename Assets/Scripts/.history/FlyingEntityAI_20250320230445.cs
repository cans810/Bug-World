...
public class FlyingEntityAI : MonoBehaviour
{
    ...
    [SerializeField] private float boundaryRotationDuration = 0.5f; // How long it takes to rotate at boundary
    ...
    private void EnforceBoundaryLimits()
    {
        ...
        // If beyond boundary, move back inside and reverse direction
        if (distanceFromSpawn > maxWanderRadius * 0.95f) // Use 95% of max radius to catch it early
        {
            ...
            // Start the boundary handling behavior (rotate away from boundary)
            if (!isHandlingBoundary && wanderingCoroutine != null)
            {
                StopCoroutine(wanderingCoroutine);
                StartCoroutine(HandleBoundaryCollision(directionFromSpawn));
            }
        }
    }
    
    // New method to handle boundary collision with rotation
    private IEnumerator HandleBoundaryCollision(Vector2 outwardDirection)
    {
        ...
        // Calculate target rotation to face away from the boundary (180 degrees away from current direction)
        Quaternion targetRotation = Quaternion.LookRotation(inwardDirection);
        
        // Rotate to face away from boundary
        float elapsed = 0f;
        while (elapsed < boundaryRotationDuration)
        {
            ...
            // Maintain height during rotation
            MaintainFixedHeight();
        ...
        Debug.DrawLine(transform.position, currentWaypoint, Color.red, 3f);
