...
public class PlayerController : MonoBehaviour
{
    ...
    // Add these fields to track previously reached metamorphosis stages
    [Header("Metamorphosis Tracking")]
    [SerializeField] private bool isLoadingGame = false;
    [SerializeField] private List<int> completedMetamorphosisStages = new List<int>();
    ...
    // Modify the CheckMetamorphosis method to avoid duplicate animations
    public void CheckMetamorphosis(int level)
    {
        if (enableMetamorphosis)
        {
            // Calculate the metamorphosis stage based on player level
            int stage = 0;
            for (int i = 0; i < metamorphosisLevels.Length; i++)
            {
                if (level >= metamorphosisLevels[i])
            ...
            // Only trigger metamorphosis if we're reaching a new stage
            // AND we're not just loading a game
            if (stage > currentMetamorphosisStage && !isLoadingGame)
            {
                // Store the previous stage for comparison
                int previousStage = currentMetamorphosisStage;
                
                // Update current stage
                currentMetamorphosisStage = stage;
                
                // Record this stage as completed
                if (!completedMetamorphosisStages.Contains(stage))
                {
                    completedMetamorphosisStages.Add(stage);
                }
                
                // Trigger the metamorphosis animation
                TriggerMetamorphosis();
                
                Debug.Log($"<color=green>Metamorphosis triggered! Level {level}, Stage {stage}</color>");
            }
            else if (stage > currentMetamorphosisStage && isLoadingGame)
            {
                // When loading a game, just update the stage and scale without animation
                currentMetamorphosisStage = stage;
                UpdatePlayerScale();
                
                // Record all stages up to current as completed
                for (int i = 1; i <= stage; i++)
                {
                    if (!completedMetamorphosisStages.Contains(i))
                    {
                        completedMetamorphosisStages.Add(i);
                    }
                }
                
                Debug.Log($"<color=blue>Loaded at metamorphosis stage {stage} - skipping animation</color>");
            }
        }
    }

    // Add this method to set the loading state
    public void SetLoadingGame(bool loading)
    {
        isLoadingGame = loading;
        if (!loading)
        {
            Debug.Log("Loading complete, metamorphosis animations enabled for future level-ups");
        }
    }
