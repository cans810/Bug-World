...
public class AttributeDisplay : MonoBehaviour
{
    ...
    public void LoadGameData(GameData data)
    {
        ...
        if (availablePointsText != null && playerAttributes != null)
        {
            availablePointsText.text = $"Attribute Points: {playerAttributes.AvailablePoints}";
        }
        ...
        if (strengthValueText != null)
        {
            strengthValueText.text = $"{data.currentStrength}/100";
        }
        
        if (strengthBonusText != null)
        {
            float strengthMultiplier = 1f + (data.currentStrength * 0.01f); // Assuming 1% per point
            strengthBonusText.text = $"+{(strengthMultiplier - 1) * 100:F0}% Damage";
        }
        
        if (vitalityValueText != null)
        {
            vitalityValueText.text = $"{data.currentVitality}/100";
        }
        
        if (vitalityBonusText != null)
        {
            float vitalityMultiplier = 1f + (data.currentVitality * 0.01f); // Assuming 1% per point
            vitalityBonusText.text = $"+{(vitalityMultiplier - 1) * 100:F0}% Health";
        }
        
        if (incubationValueText != null)
        {
            incubationValueText.text = $"{data.currentIncubation}/20";
        }
        
        if (incubationBonusText != null)
        {
            incubationBonusText.text = $"+{data.currentIncubation} Egg Capacity";
        }
        
        if (agilityValueText != null)
        {
            agilityValueText.text = $"{data.currentAgility}/100";
        }
        
        if (agilityBonusText != null)
        {
            float agilityMultiplier = 1f + (data.currentAgility * 0.01f); // Assuming 1% per point
            agilityBonusText.text = $"-{(agilityMultiplier - 1) * 100:F0}% Attack Cooldown";
        }
        
        // Update button interactability
        UpdateButtonInteractability();
        
        Debug.Log("Attribute display updated with loaded game data.");
    }
