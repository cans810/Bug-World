...
public class UIHelper : MonoBehaviour
{
    ...
    public void ShowInformText(string message, float duration = 5f)
    {
        ...
        // Start timed hide
        Coroutine newCoroutine = StartCoroutine(HideTextAfterDelay("inform", duration));
        activeTextCoroutines["inform"] = newCoroutine;
        
        Debug.Log($"<color=yellow>ShowInformText: '{message}' for {duration}s</color>");
    }
    ...
    public void ShowWarningText(string message, float duration = 6f)
    {
        ...
        // Start timed hide
        Coroutine newCoroutine = StartCoroutine(HideTextAfterDelay("warning", duration));
        activeTextCoroutines["warning"] = newCoroutine;
        
        Debug.Log($"<color=red>ShowWarningText: '{message}' for {duration}s</color>");
    }
    ...
    private IEnumerator HideTextAfterDelay(string id, float duration)
    {
        ...
        if (!isMessageLocked && informPlayerText != null)
        {
            // Reset text state
            informPlayerText.gameObject.SetActive(false);
            isShowingInformText = false;
            ...
            Debug.Log($"<color=cyan>Text '{id}' hidden after {actualDuration:F2}s (requested: {duration:F2}s)</color>");
        }
