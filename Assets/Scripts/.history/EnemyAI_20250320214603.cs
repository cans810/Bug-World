...
public class EnemyAI : MonoBehaviour
{
    ...
    private void HandleDeath()
    {
        ...
        // Start the sinking coroutine instead of being immediately destroyed
    ...
    // Add this new coroutine for the sinking effect
    private IEnumerator SinkAndDestroy()
    {
        // Wait for death animation to play (adjust time as needed for your animation)
        float deathAnimDuration = 1.5f;
        yield return new WaitForSeconds(deathAnimDuration);
        
        // Get initial position
        Vector3 startPosition = transform.position;
        Vector3 endPosition = new Vector3(startPosition.x, startPosition.y - 2.0f, startPosition.z); // Sink 2 units down
        
        // Sinking parameters
        float sinkDuration = 1.5f;
        float elapsedTime = 0;
        
        // Gradually sink the enemy
        while (elapsedTime < sinkDuration)
        {
            // Calculate the new position with smooth easing
            float t = elapsedTime / sinkDuration;
            float smoothT = Mathf.SmoothStep(0, 1, t); // Smooth easing function
        ...
        // Final pause before destruction
        yield return new WaitForSeconds(0.2f);
        
        // If this enemy is managed by LivingEntity's destroy mechanism, we don't need to destroy it here
        // The LivingEntity component will handle the destruction
        
        // But if you want to ensure it's destroyed, you can uncomment this:
        // Destroy(gameObject);
    }
} 