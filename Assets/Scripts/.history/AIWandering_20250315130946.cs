...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // Find the map border by layer
        GameObject[] borderObjects = GameObject.FindGameObjectsWithTag("MapBorder");
        if (borderObjects.Length > 0)
        {
            foreach (GameObject borderObj in borderObjects)
            {
                if (borderObj.layer == LayerMask.NameToLayer("MapBorder"))
                {
                    mapBorderTransform = borderObj.transform;
                    
                    // Get the sphere collider to determine radius
                    SphereCollider borderCollider = borderObj.GetComponent<SphereCollider>();
                    if (borderCollider != null)
                    {
                        mapBorderRadius = borderCollider.radius * Mathf.Max(
                            borderObj.transform.lossyScale.x,
                            borderObj.transform.lossyScale.y,
                            borderObj.transform.lossyScale.z
                        );
            ...
            {
                Debug.LogWarning("Map border objects found but none have a SphereCollider");
            }
        }
        else
        {
            Debug.LogWarning("No map border objects found in scene");
        }
    }
    ...
    // New method to check if waypoint is safe from map borders
    private bool IsWaypointSafeFromBorders(Vector3 waypoint)
    {
        ...
        // The waypoint is safe if it's within the safe radius
    ...
    // New method to get a direction away from both borders and nest
    private Vector3 GetSafeDirection()
    {
        Vector3 directionFromBorder = Vector3.zero;
        Vector3 directionFromNest = Vector3.zero;
        
        // Get direction from border (if applicable)
        if (mapBorderTransform != null)
        {
            // For a sphere border, direction is from border center to current position
            directionFromBorder = (transform.position - mapBorderTransform.position).normalized;
        }
