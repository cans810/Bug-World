...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Ensure we have a valid count and prefab
        if (chitinCount <= 0 || chitinIconPrefab == null)
        {
            Debug.LogError("[CRITICAL] Invalid chitin count or missing prefab: " + chitinCount);
            yield break;
        }
        
        // Ensure we have a valid UI canvas to parent the icons to
        ...
        // Get starting position - use chitin panel if available
        Vector3 startPos;
        if (chitinPanel != null)
        {
            startPos = chitinPanel.position;
            Debug.Log($"[CRITICAL] Using chitinPanel position: {startPos}");
        }
        else if (chitinCountText != null)
        {
        ...
        nestScreenPos.z = 0; // Ensure z is 0 for UI positioning
        ...
        {
            // Create icon
            GameObject icon = Instantiate(chitinIconPrefab, mainCanvas.transform);
            ...
            if (iconRect != null)
            {
                ...
                // Position at start with random offset
                ...
                icon.transform.position = startPos + randomOffset;
            }
            ...
            allIcons.Add(icon);
            Debug.Log($"[CRITICAL] Created icon {i+1} of {chitinCount} at position {icon.transform.position}");
        }
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            // Store the target position - update it each frame to follow the nest
