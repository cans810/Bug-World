...
public class AttributeDisplay : MonoBehaviour
{
    ...
    private bool isPlayerInitiatedAction = false;
    private bool forceShowingPanel = false;
    private float lastShowAttemptTime = 0f;
    ...
    {
        // Just update the display values, but don't show the panel
        UpdateDisplay();
        
        // Log the level up but don't show panel
        Debug.Log($"AttributeDisplay: Player leveled up to level {newLevel}. Panel will NOT be shown automatically.");
        
        // The panel will only be shown when explicitly requested by the player
    }
    
    public void ShowPanel(bool show)
    {
        // ONLY show the panel if explicitly called from a player action
        if (show && !isPlayerInitiatedAction)
        {
            Debug.Log("AttributeDisplay: Blocked automatic attempt to show panel. Only showing on player action.");
            return;
        }
    ...
    {
        Debug.Log($"AttributeDisplay: Camera animation completed for area {areaName}");
        
        // Don't show the panel automatically
        // The player will need to click the attribute button to see the panel
    }

    // Update method - completely disable auto-showing behavior
    private void Update()
    {
        // DISABLE automatic panel showing completely
        ...
        */
        ...
        // Reset the flag without showing the panel
        if (shouldShowAttributesPanel)
        {
            Debug.Log("AttributeDisplay: Auto-show behavior disabled. Panel will not show automatically.");
            shouldShowAttributesPanel = false;
        }
    }
    ...
    // New method to be called specifically from UI actions
    public void ShowPanelFromPlayerAction(bool show)
    {
        Debug.Log("AttributeDisplay: ShowPanelFromPlayerAction called - this is a player-initiated action");
        isPlayerInitiatedAction = true;
        ShowPanel(show);
        isPlayerInitiatedAction = false; // Reset immediately after
    }
    ...
    public void NotifyCameraAnimationEnded()
    {
        ...
        // Don't show panel automatically after camera animation
        // Even if we have points to spend, wait for player to click the button
        // Remove the code that automatically shows the panel
    }
    ...
    // Add this method to stop any pending attempts to show the panel
    public void CancelPendingPanelDisplay()
    {
        shouldShowAttributesPanel = false;
        
        // Also ensure the panel is hidden
        if (attributesPanel != null && attributesPanel.activeSelf)
        {
            Debug.Log("AttributeDisplay: Force hiding panel");
    ...
    // Fix the OnEnable method to not always hide the panel
    private void OnEnable()
    {
        // DON'T hide the panel on enable - this causes a cycle
        // where showing the panel triggers OnEnable which hides it again
        
        // Just clear the flag but don't force hide
        shouldShowAttributesPanel = false;
        
        Debug.Log("AttributeDisplay.OnEnable called - panel will remain visible if explicitly shown");
    }

    // Make the existing OnAttributeButtonClicked method call this new method
    public void OnAttributeButtonClicked()
    {
        Debug.Log("Attribute button clicked!");
        ShowPanelFromPlayerAction(true);
    }

    // Modify the ForceShowAttributesPanel method to be even more direct
    public void ForceShowAttributesPanel()
    {
        Debug.Log("AttributeDisplay: ForceShowAttributesPanel called - forcing panel to show");
        
        // Mark this as player initiated so LateUpdate doesn't hide it
        isPlayerInitiatedAction = true;
        forceShowingPanel = true;
        ...
        try
        {
            ...
            if (attributesPanel == null)
            {
                Debug.LogError("AttributesPanel reference is NULL - cannot show panel!");
                return;
            }
            ...
            Debug.Log("Attributes panel successfully activated");
        }
        finally
        {
            // Reset flags after 0.5 seconds to prevent immediate hiding
            StartCoroutine(ResetFlagsAfterDelay(0.5f));
        }
    }

    // Add this coroutine to reset flags safely
    private IEnumerator ResetFlagsAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        isPlayerInitiatedAction = false;
        forceShowingPanel = false;
        Debug.Log("AttributeDisplay: Reset player action flags after delay");
    }

    // Modify LateUpdate to be less aggressive
    private void LateUpdate()
    {
        // Don't hide the panel for a short time after explicit show requests
        if (Time.time - lastShowAttemptTime < 1.0f)
            return;
        
        // Only hide if we're sure it wasn't explicitly shown
        if (attributesPanel != null && attributesPanel.activeSelf && 
            !isPlayerInitiatedAction && !forceShowingPanel)
        {
            Debug.LogWarning("AttributeDisplay: Panel likely activated automatically - hiding it");
            attributesPanel.SetActive(false);
        }
    }

    public void SetPlayerAction(bool isPlayerAction)
    {
        isPlayerInitiatedAction = isPlayerAction;
        forceShowingPanel = isPlayerAction;
        
        // Set the timestamp to prevent LateUpdate from hiding panel
        lastShowAttemptTime = Time.time;
        
        Debug.Log($"AttributeDisplay: Player action flag set to {isPlayerAction}");
    }
} 