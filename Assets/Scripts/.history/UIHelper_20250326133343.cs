...
public class UIHelper : MonoBehaviour
{
    ...
    [Header("Revival Panel")]
    [SerializeField] public GameObject arrowToShowAttributeButton;
    ...
    private void HandleRewardedAdCompleted()
    {
        ...
        if (playerController != null)
        {
            ...
            // Manually call the revival method
            RevivePlayer(playerController, "You have been revived by watching an ad!");
        }
    }

    // Spend 150 coins for revival
    public void Give150CoinsForRevivalClicked()
    {
        SoundEffectManager.Instance.PlaySound("ButtonClicked");
        ...
        if (playerInventory != null)
        {
            ...
            if (playerInventory.CoinCount >= 150)
            {
                // Try to find a suitable method to reduce coins
                bool coinsRemoved = false;
                
                // Try SpendCoins if it exists (from PlayerCoins)
                PlayerCoins playerCoins = FindObjectOfType<PlayerCoins>();
                if (playerCoins != null && playerCoins.SpendCoins(150))
                {
                    coinsRemoved = true;
                }
                // If not, try using reflection to find a method in PlayerInventory
                else if (playerInventory != null)
                {
                    System.Type invType = playerInventory.GetType();
                    
                    // Try RemoveCoins
                    System.Reflection.MethodInfo removeMethod = invType.GetMethod("RemoveCoins", 
                        System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                    
                    if (removeMethod != null)
                    {
                        removeMethod.Invoke(playerInventory, new object[] { 150 });
                        coinsRemoved = true;
                    }
                    // Try SpendCoins if RemoveCoins doesn't exist
                    else
                    {
                        System.Reflection.MethodInfo spendMethod = invType.GetMethod("SpendCoins", 
                            System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                        
                        if (spendMethod != null)
                        {
                            bool result = (bool)spendMethod.Invoke(playerInventory, new object[] { 150 });
                            coinsRemoved = result;
                        }
                    }
                }
                
                // Continue with revival if coins were successfully removed
                if (coinsRemoved)
                {
                    // Find the player controller to revive
                    PlayerController playerController = FindObjectOfType<PlayerController>();
                    if (playerController != null)
                    {
                        ...
                        // Manually call the revival method
                        RevivePlayer(playerController, "You have been revived by spending 150 coins!");
                    }
                }
                else
                {
                    ShowInformText("Error processing coin payment. Please try another option.");
                }
            }
            else
            {
                // Not enough coins
                ShowInformText("Not enough coins! You need 150 coins to revive.");
    ...
    // Lose resources but revive anyway
    public void CloseRevivalAndLooseChitinAndCrumbsClicked()
    {
        SoundEffectManager.Instance.PlaySound("ButtonClicked");
        
        // Find player inventory to reduce resources
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null)
        {
            // Calculate resource loss (50% of current resources)
            int chitinLoss = Mathf.CeilToInt(playerInventory.ChitinCount * 0.5f);
            int crumbLoss = Mathf.CeilToInt(playerInventory.CrumbCount * 0.5f);
            
            // Apply the resource reduction - use public methods
            playerInventory.SetChitinCount(playerInventory.ChitinCount - chitinLoss);
            playerInventory.SetCrumbCount(playerInventory.CrumbCount - crumbLoss);
            
            // Use the appropriate method to update the UI
            // The events can't be invoked directly
            try
            {
                // Try different approaches for updating the UI
                Debug.Log($"Reduced resources: -{chitinLoss} chitin, -{crumbLoss} crumbs");
                
                // Option 1: Try using direct property setters (they should trigger UI updates)
                // (We already called SetChitinCount and SetCrumbCount above)
                
                // Option 2: Call any public method to refresh UI
                System.Type invType = playerInventory.GetType();
                System.Reflection.MethodInfo updateMethod = invType.GetMethod("ForceUpdateUI", 
                    System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                
                if (updateMethod != null)
                {
                    updateMethod.Invoke(playerInventory, null);
                    Debug.Log("Called ForceUpdateUI method");
                }
            }
            catch (System.Exception e)
            {
                Debug.LogWarning($"Error updating inventory UI: {e.Message}");
            }
            ...
            if (playerController != null)
            {
                ...
                // Manually call the revival method
                RevivePlayer(playerController, $"You lost {chitinLoss} chitin and {crumbLoss} crumbs but have been revived!");
            }
        }
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        if (pauseStatus)
        {
            Debug.Log("Game paused - saving game...");
            SaveGame();
        }
    }
    ...
    {
        Debug.Log("Game quitting - saving game...");
        SaveGame();
    }

    // Add this method to UIHelper.cs to make the attribute button arrow pulse
    public void ShowPulsingAttributeArrow()
    {
        if (arrowToShowAttributeButton != null)
        {
            // Stop any existing animation if it's already running
            StopCoroutine("PulseAttributeArrow");
            
            // Start the pulsing animation
            StartCoroutine(PulseAttributeArrow(3.0f));
        }
        else
        {
            Debug.LogWarning("Arrow to show attribute button is not assigned in UIHelper");
        }
    }

    // Improved coroutine to create a pulsing/beating effect on the arrow with fade effects
    private IEnumerator PulseAttributeArrow(float duration)
    {
        // Make sure the arrow is active
        arrowToShowAttributeButton.SetActive(true);
        
        // Get the original scale to return to
        Vector3 originalScale = arrowToShowAttributeButton.transform.localScale;
        Vector3 maxScale = originalScale * 1.2f; // Scale up by 20%
        
        // Get the image component for fading
        Image arrowImage = arrowToShowAttributeButton.GetComponent<Image>();
        if (arrowImage == null)
        {
            Debug.LogWarning("No Image component found on arrow - cannot fade");
            arrowImage = arrowToShowAttributeButton.GetComponentInChildren<Image>();
        }
        
        // If we found the image component, start with alpha at 0 for fade-in
        ...
        while (fadeTimer < fadeInTime)
        {
            ...
            float fadeProgress = fadeTimer / fadeInTime;
            
            // Apply fade
            if (arrowImage != null)
            {
                Color newColor = arrowImage.color;
                newColor.a = Mathf.Lerp(0f, originalColor.a, fadeProgress);
        ...
        if (arrowImage != null)
        {
            Color fullyVisibleColor = originalColor;
            arrowImage.color = fullyVisibleColor;
        }
        
        // Pulse phase
        float pulseTime = duration - 1.0f; // subtract fade time (0.5s in, 0.5s out)
        float elapsedTime = 0f;
        
        // Continue pulsing for the specified duration
        while (elapsedTime < pulseTime)
        {
            // Calculate the pulse phase (0 to 1 and back to 0) using a sine wave
            float pulseFactor = 0.5f + 0.5f * Mathf.Sin(elapsedTime * 5f); // 5f controls pulse speed
            
            // Set the scale based on the pulse factor
            arrowToShowAttributeButton.transform.localScale = Vector3.Lerp(originalScale, maxScale, pulseFactor);
        ...
        // Fade out phase - 0.5 seconds
        fadeTimer = 0f;
        
        while (fadeTimer < fadeInTime) // Reuse fadeInTime for symmetry
        {
            fadeTimer += Time.deltaTime;
            float fadeProgress = fadeTimer / fadeInTime;
        ...
        // Ensure the arrow returns to its original scale
        arrowToShowAttributeButton.transform.localScale = originalScale;
        
        // Reset color to original with full alpha for next time
        if (arrowImage != null)
        {
            arrowImage.color = originalColor;
        }
        
        // Hide the arrow when the animation completes
        arrowToShowAttributeButton.SetActive(false);
    }

    // Add this method to UIHelper.cs if it doesn't exist
    public void HideAttackCooldownText()
    {
        if (attackCoolDownText != null)
        {
            attackCoolDownText.gameObject.SetActive(false);
    ...
    // Make sure the ShowAttackCooldownText method activates the text
    public void ShowAttackCooldownText(string text)
    {
        if (attackCoolDownText != null)
        {
            attackCoolDownText.gameObject.SetActive(true);
            attackCoolDownText.text = text;
        }
    }
} 
