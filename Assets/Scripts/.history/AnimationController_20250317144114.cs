...
public class AnimationController : MonoBehaviour
{
    ...
    private void UpdateAnimatorParameters()
    {
        ...
        // Handle walk attack logic for Player only
        if (isWalkAttacking && gameObject.CompareTag("Player"))
        {
            // When walk attacking, ONLY set walkAttack parameter to true
            // and turn off all other animations
            animator.SetBool(walkAttackParam, true);
            animator.SetBool(attackingParam, false);
            animator.SetBool(eatingParam, false);
            animator.SetBool(deadParam, false);
            
            // Keep movement blend tree values at 0 to prevent walk/idle animations
    ...
    public void SetWalking(bool walking)
    {
        ...
        {
            // If we're attacking and start walking, switch to walk attack (Player only)
            if (isAttacking && walking && gameObject.CompareTag("Player"))
            {
            ...
            else if (!isWalkAttacking) // Only change walk state if not in walk attack
            {
                ResetStates();
                isWalking = walking;
                isIdle = !walking;
    ...
    public void SetAttacking(bool attacking)
    {
        ...
        // Handle walk attack state for Player only
        ...
        {
            // Non-player entities just use regular attack
            isAttacking = attacking;
            isWalkAttacking = false;
        }
    ...
    public void SetWalkAttacking(bool attacking)
    {
        ...
        // Also set the basic attack param if we're beginning an attack
        if (attacking)
        {
            isAttacking = true;
            animator.SetBool(attackingParam, true);
        }
    }
