...
public class AllyEggController : MonoBehaviour
{
    ...
    private void SpawnAlly()
    {
        if (insectPrefabs[0] != null)
        {
            ...
            {
                SoundEffectManager.Instance.PlaySound("InsectHatched", transform.position);
                Debug.Log("Playing insect hatched sound via SoundEffectManager");
            }
            
            // Notify incubator that ant has hatched (if needed)
            if (parentIncubator != null)
            {
                parentIncubator.OnAntHatched(newAnt);
            }
