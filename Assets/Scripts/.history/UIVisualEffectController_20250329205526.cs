...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            ...
            int previousLevel = playerInventory.CurrentLevel;
            ...
            // Update UI to reflect changes - Use the gradual update method to show fill animation
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null)
            {
                // Get current XP total AFTER adding the new XP
                int currentXP = playerInventory._experience;
                
                // Animate the XP fill image from previous value to new value
                int previousXP = currentXP - pendingXpAmount;
                StartCoroutine(AnimateXPFillBar(uiHelper, previousXP, currentXP));
            }
            
            // Check if level changed and show UI if needed
            if (playerInventory.CurrentLevel > previousLevel)
            {
                // The player has leveled up, make sure the panel shows
                LevelUpPanelHelper levelUpPanel = FindObjectOfType<LevelUpPanelHelper>();
                if (levelUpPanel != null)
                {
                    // Use ShowLevelUpPanel method (or whatever the actual method is)
                    levelUpPanel.OnLevelUp(playerInventory.CurrentLevel);
                    Debug.Log("Showing level up panel for level " + playerInventory.CurrentLevel);
                ...
                {
                    Debug.LogWarning("LevelUpPanelHelper not found, cannot show level up panel");
                }
            }
    ...
    // Add this method to handle XP fill bar animation
    private IEnumerator AnimateXPFillBar(UIHelper uiHelper, int startXP, int targetXP)
    {
        ...
        while (elapsed < duration && currentDisplayXP < targetXP)
        {
            ...
            {
                // Update the display with the new value - but DON'T check for level up again
