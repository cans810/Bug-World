...
public class UIHelper : MonoBehaviour
{
    ...
    // Helper method to animate a single icon with error handling and dynamic target position
    private IEnumerator AnimateIconSafe(GameObject icon, Vector3 startPosition, Vector3 targetPos, Vector3 originalScale, float duration, int iconIndex)
    {
        ...
        while (!animationComplete && icon != null)
        {
            try
            {
                ...
                // Get the current nest position - update every frame
                Vector3 currentTargetPos;
                GameObject nestObject = GameObject.Find("Player Nest 1");
                if (nestObject != null)
                {
                    currentTargetPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                    currentTargetPos.z = 0;
                }
                else
                {
                    // Use the original target position as fallback
                    currentTargetPos = targetPos;
                }
                
                // Use a simple arc motion with the updated target position
                float arcHeight = 200f * Mathf.Sin(normalizedTime * Mathf.PI);
                Vector3 currentPos = Vector3.Lerp(startPosition, currentTargetPos, normalizedTime);
                ...
                {
                    Debug.Log($"[CRITICAL] Icon {iconIndex} at position {currentPos}, target: {currentTargetPos}, progress: {normalizedTime:F2}");
                }
            ...
            catch (System.Exception e)
            {
                ...
                animationComplete = true; // End the animation if there's an error
            }
            
            // This yield is outside the try/catch
            yield return null;
        }
        
        // Final cleanup outside of try/catch
        try
        {
            if (icon != null)
            {
