...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        // Update animation state only when needed
        if (animController != null)
        {
        ...
        // If we have movement input, update rotation directly
        if (moveDirection.magnitude > 0.1f)
        {
            // Direct rotation approach for more responsive turning
            float rotationSpeed = livingEntity.rotationSpeed * 2f; // Double rotation speed for responsiveness
            ...
            transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, step * 60f);
        }
