...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        // BORDER CHECK: Do one final check for level-restricted borders in our path
        if (currentSpeed > 0.01f)
        {
            // Do one final check for level-restricted borders in our path
            Vector3 movePosition = transform.position + (transform.forward * currentSpeed * Time.deltaTime);
            Collider[] hitColliders = Physics.OverlapSphere(movePosition, 0.5f, LayerMask.GetMask("MapBorder"));
            
            foreach (Collider col in hitColliders)
            {
                string borderName = col.gameObject.name;
                int requiredLevel = GetRequiredLevelForBorder(borderName);
                
                if (requiredLevel > 0 && playerInventory.CurrentLevel < requiredLevel)
                {
                    // We're about to enter a restricted area - block movement
                    Debug.Log($"Movement blocked: About to enter {borderName} (requires level {requiredLevel})");
                    currentSpeed = 0;
                    isAtMapBorder = true;
                    borderNormal = (transform.position - col.bounds.center).normalized;
                    break;
                }
            }
        }
        ...
        if (currentSpeed > 0.01f)
        {
            ...
            if (shouldMove)
            {
                ...
                {
                    // Use rigidbody movement for better physics interaction
                    rb.MovePosition(rb.position + finalMoveDirection * currentSpeed * Time.deltaTime);
                }
                else
                {
                    // Use transform-based movement for non-physics objects
    ...
    // Modify the OnTriggerStay method to better enforce level restrictions
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // BigBorder handling remains the same...
            // ... existing BigBorder code ...
        }
        else if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (borderName.StartsWith("MapBorder"))
            {
                ...
                // Skip if this border has no level requirement
                ...
                {
                    // Debug log the block
                    Debug.Log($"BLOCKING PLAYER: Level {playerLevel} trying to cross {borderName} (requires level {requiredLevel})");
                    
                    // Calculate the normal pointing away from the border
                    // Use raycast for more precise normal calculation
                    Vector3 rayDirection = transform.position - other.bounds.center;
                    rayDirection.Normalize();
                    
                    // Cast a ray from the border center toward player
                    RaycastHit hit;
                    if (Physics.Raycast(other.bounds.center, rayDirection, out hit, 10f))
                    {
                    ...
                    else
                    {
                        ...
                        borderNormal = (transform.position - other.bounds.center).normalized;
                    }
                    
                    // Set flag to prevent movement into this area
                    isAtMapBorder = true;
                    
                    // Apply immediate blocking force to prevent glitching through
                    Vector3 pushDirection = borderNormal * 0.2f;
    ...
    // Add this helper method to consistently determine required levels
    private int GetRequiredLevelForBorder(string borderName)
    {
        switch (borderName)
        {
            case "MapBorder2": return 5;
            case "MapBorder3": return 10;
            case "MapBorder4": return 15;
            case "MapBorder5": return 20;
            case "MapBorder6": return 25;
            case "MapBorder7": return 30;
            case "MapBorder9": return 35;
            case "MapBorder10": return 40;
            case "MapBorder11": return 45;
            case "MapBorder12": return 50;
            case "MapBorder13": return 55;
            default: return 0; // No level requirement
        }
    }
    ...
    // Add this check to HandleMovement to act as a backup system
    private void HandleMovement()
    {
        // ... existing code ...
        
        // Add this BEFORE applying the final movement
        if (currentSpeed > 0.01f)
        {
            // Do one final check for level-restricted borders in our path
            Vector3 movePosition = transform.position + (transform.forward * currentSpeed * Time.deltaTime);
    ...
    // Add this to the Awake or Start method
    private void InitializePlayerPosition()
    {
        // Optional safety measure - if player spawns in a restricted area, move them out
        Collider[] nearbyBorders = Physics.OverlapSphere(transform.position, 5f, LayerMask.GetMask("MapBorder"));
        foreach (Collider border in nearbyBorders)
        {
            string borderName = border.gameObject.name;
            int requiredLevel = GetRequiredLevelForBorder(borderName);
            ...
            if (requiredLevel > 0 && playerInventory.CurrentLevel < requiredLevel)
            {
                // Player is in or near a restricted area - find nearest spawn point
                Debug.LogWarning($"Player in restricted area {borderName} at startup - moving to spawn point");
