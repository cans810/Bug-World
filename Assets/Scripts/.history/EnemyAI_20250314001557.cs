        ...
        if (livingEntity.HasTargetsInRange())
        {
            ...
            if (potentialTarget != null && !potentialTarget.IsDead)
            {
                ...
                if (shouldChase)
                {
                    ...
                    // When chasing the player, immediately check if we're approaching a boundary
                    Vector3 directionToTarget = currentTarget.transform.position - transform.position;
                    directionToTarget.y = 0; // Keep on ground plane
                    directionToTarget.Normalize();
                    
                    // Before moving, check if this would take us over a territory boundary
                    if (CheckAndRespectTerritory(directionToTarget))
                    {
                        // Safe to proceed with movement toward target
                        FaceTarget(currentTarget.transform.position);
                        MoveWithBoundaryCheck(transform.forward);
                    }
                    else
                    {
                        // Movement would cross boundary, already handled in CheckAndRespectTerritory
                        // Territory boundary would prevent further pursuit if target is outside territory
                    }
                }
                else
                {
