...
public class AllyAntPanelController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (allAntsOrdersPanel != null && !allAntsOrdersPanel.activeSelf)
        {
            ...
            allAntsOrdersPanel.GetComponent<Animator>().SetBool("ShowUp", true);
            allAntsOrdersPanel.GetComponent<Animator>().SetBool("Hide", false);
        }
        ...
        UpdateAllAntsButtonsInteractability();
        
        // Check for dead ants and update their buttons
        UpdateDeadAntButtons();
    }
    ...
    private void CreateButtonForAllyAnt(GameObject allyAnt, int index)
    {
        ...
        bool isDead = IsAntDead(allyAnt);
        
        if (buttonComponent != null)
        {
            // Disable button if ant is dead
            buttonComponent.interactable = !isDead;
        ...
        // Set the button text
        string buttonLabel = isDead ? "Dead" : $"Ally Ant {index + 1}";
        
        // Set the button text using TextMeshProUGUI component
        ...
        {
            Debug.LogWarning($"Could not find TextMeshProUGUI component on button for ant {index + 1}");
        }
    }
    ...
    private void OnDisable()
    {
        ...
        if (allAntsOrdersPanel != null)
        {
            allAntsOrdersPanel.GetComponent<Animator>().SetBool("ShowUp", false);
            allAntsOrdersPanel.GetComponent<Animator>().SetBool("Hide", true);
        }
    ...
    private void CommandAllAntsFollow()
    {
        ...
        allAntsfollowModeToggle.interactable = false;
        allAntswanderModeToggle.interactable = true;
    ...
    private void CommandAllAntsWander()
    {
        ...
        allAntswanderModeToggle.interactable = false;
        allAntsfollowModeToggle.interactable = true;
    ...
    private void UpdateAllAntsModeText()
    {
        ...
        if (lootEnabledCount == 0)
            ...
            allAntsLootModeOrderText.text = "All Looting";
        else
            allAntsLootModeOrderText.text = "Custom";
    }

    // Add this method to update button interactability based on all ants state
    private void UpdateAllAntsButtonsInteractability()
    {
        // Find all ally ants
        GameObject[] allyAnts = FindAllyAnts();
        ...
        if (allyAnts.Length == 0)
        {
            // No ants, disable all buttons
            if (allAntsfollowModeToggle != null) allAntsfollowModeToggle.interactable = false;
            if (allAntswanderModeToggle != null) allAntswanderModeToggle.interactable = false;
            if (allAntslootModeToggle != null) allAntslootModeToggle.interactable = false;
            return;
        }
        ...
        int totalAnts = allyAnts.Length;
        
        // Update follow button interactability
        if (allAntsfollowModeToggle != null)
            allAntsfollowModeToggle.interactable = followingCount < totalAnts;
        
        // Update wander button interactability
        if (allAntswanderModeToggle != null)
            allAntswanderModeToggle.interactable = wanderingCount < totalAnts;
        
        // Loot toggle is always interactable if there are ants
        if (allAntslootModeToggle != null)
            allAntslootModeToggle.interactable = true;
    }

    // Add this method to check if an ant is dead
    private bool IsAntDead(GameObject ant)
    {
        if (ant == null)
            return true;
        
        // Check if the ant has a LivingEntity component and if it's dead
        LivingEntity livingEntity = ant.GetComponent<LivingEntity>();
        if (livingEntity != null)
        {
            return livingEntity.CurrentHealth <= 0;
        }
        
        return false;
    }

    // Add this method to update button states for dead ants
    private void UpdateDeadAntButtons()
    {
        foreach (GameObject buttonObj in createdButtons)
        {
            AntButtonData buttonData = buttonObj.GetComponent<AntButtonData>();
            ...
            if (buttonData != null && buttonData.LinkedAnt != null && button != null)
            {
                // Check if the ant is dead
                bool isDead = IsAntDead(buttonData.LinkedAnt);
                
                // Debug log the death status
                if (isDead)
                {
                    Debug.Log($"Ant {buttonData.LinkedAnt.name} is dead!");
                }
                
                // Update button interactability
                button.interactable = !isDead;
                
                // Get text component - try different approaches
                TextMeshProUGUI buttonText = null;
                
                // First try direct child
                buttonText = buttonObj.GetComponentInChildren<TextMeshProUGUI>(true);
                
                if (buttonText == null)
                {
                    Debug.LogWarning($"Could not find TextMeshProUGUI component on button {buttonObj.name}");
                    
                    // Try to find by name if there's a specific naming convention
                ...
                // Update button text based on dead status
                if (isDead && buttonText != null)
                {
                    Debug.Log($"Setting button text to 'Dead' (was: {buttonText.text})");
                    buttonText.text = "Dead";
                    
                    // If this was the selected ant, deselect it
                    if (currentlySelectedAnt == buttonData.LinkedAnt)
                    {
                        CloseOrdersPanel();
                    }
                }
                ...
                {
                    Debug.LogError($"Could not update text for dead ant - buttonText is null");
                }
            }
        }
    }
