...
public class UIHelper : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LivingEntity playerHealth = FindObjectOfType<LivingEntity>();
        if (playerHealth != null)
        {
            playerHealth.OnHealthChanged.AddListener(delegate { UpdateHPDisplay(); });
        }
        ...
        // Add this to initialize HP display
    ...
    private void OnDestroy()
    {
        ...
        if (playerHealth != null)
        {
            playerHealth.OnHealthChanged.RemoveListener(delegate { UpdateHPDisplay(); });
        }
    }
    ...
    // Add this new method to update HP display
    public void UpdateHPDisplay()
    {
        if (currentHPText != null)
        {
            // Find player health component - adjust this to match your actual player health component
            LivingEntity playerHealth = FindObjectOfType<LivingEntity>();
            
            if (playerHealth != null)
            {
                int currentHP = Mathf.RoundToInt(playerHealth.CurrentHealth);
                int maxHP = Mathf.RoundToInt(playerHealth.MaxHealth);
                currentHPText.text = string.Format(hpFormat, currentHP, maxHP);
            }
        }
    }
} 
