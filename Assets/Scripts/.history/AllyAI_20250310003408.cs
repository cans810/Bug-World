...
public class AllyAI : MonoBehaviour
{
    ...
    // Add these fields to store original wandering settings
    private float originalMinWanderDistance;
    private float originalMaxWanderDistance;
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (!isPlayerMoving && distanceToPlayer <= followDistance * 1.5f)
        {
            ...
            if (wanderWhenPlayerIdle && wanderingBehavior != null)
            {
                ...
                if (!isIdleWandering)
                {
                    ...
                    // Configure the wandering behavior for idle wandering
                    // Store original values to restore later
                    originalMinWanderDistance = wanderingBehavior.minWanderDistance;
                    originalMaxWanderDistance = wanderingBehavior.maxWanderDistance;
                    
                    // Set smaller wander distances for idle wandering
                    wanderingBehavior.minWanderDistance = idleWanderRadius * 0.5f;
                    wanderingBehavior.maxWanderDistance = idleWanderRadius;
        ...
        else if (isIdleWandering)
        {
            ...
            // Restore original wandering settings
            if (wanderingBehavior != null)
            {
                wanderingBehavior.minWanderDistance = originalMinWanderDistance;
                wanderingBehavior.maxWanderDistance = originalMaxWanderDistance;
                wanderingBehavior.SetWanderingEnabled(false);
            }
        }
