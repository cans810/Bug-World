...
public class AIWandering : MonoBehaviour
{
    ...
    // Add this method to handle collisions with the PlayerNest1 layer
    private void OnTriggerEnter(Collider other)
    {
        // Check if this is the player nest
        if (other.gameObject.layer == LayerMask.NameToLayer("PlayerNest1"))
        {
            // Skip for allies
            if (IsAlly())
                return;
            ...
            if (showAvoidanceDebug)
            {
                Debug.LogWarning($"{gameObject.name}: Hit player nest boundary - redirecting");
            }
            ...
            // Calculate direction away from nest
            Vector3 directionAwayFromNest = (transform.position - other.transform.position).normalized;
            directionAwayFromNest.y = 0; // Keep on horizontal plane
            
            // Move slightly away from the boundary to prevent getting stuck
            transform.position += directionAwayFromNest * 0.5f;
            
            // Start a new wandering routine with a direction away from the nest
            StartCoroutine(RedirectAfterNestCollision(directionAwayFromNest));
        }
    }

    // New coroutine to handle redirection after hitting nest boundary
    private IEnumerator RedirectAfterNestCollision(Vector3 directionAwayFromNest)
    {
        // Wait a short moment
        yield return new WaitForSeconds(0.5f);
        ...
        float turnTimer = 0;
        Quaternion startRotation = transform.rotation;
        Quaternion targetRotation = Quaternion.LookRotation(directionAwayFromNest);
        ...
        // Get a safe waypoint in the direction away from nest
        float safeDistance = Random.Range(minWanderDistance, maxWanderDistance);
        Vector3 targetWaypoint = GetSafeWaypointUsingDirection(directionAwayFromNest, safeDistance);
        ...
        while (moveTimer < moveDuration && isWandering)
        {
            // Check for borders ahead
            if (IsBorderAhead(Vector3.Distance(transform.position, targetWaypoint)))
            {
                break;
            }
    ...
    // Helper method to get a safe waypoint in a specific direction
    private Vector3 GetSafeWaypointUsingDirection(Vector3 direction, float distance)
    {
        // Try the direct direction first
        Vector3 targetPosition = transform.position + direction.normalized * distance;
        
        // Check if it's safe
        if (IsWaypointSafeFromBorders(targetPosition) && IsWaypointSafeFromNest(targetPosition))
        {
            return targetPosition;
        }
        
        // If not safe, try variations of the direction
        for (int i = 0; i < 8; i++)
        {
            // Try directions at 45-degree intervals
            float angle = i * 45f;
            Vector3 rotatedDirection = Quaternion.Euler(0, angle, 0) * direction;
            Vector3 alternatePosition = transform.position + rotatedDirection.normalized * distance;
            
            if (IsWaypointSafeFromBorders(alternatePosition) && IsWaypointSafeFromNest(alternatePosition))
            {
                return alternatePosition;
            }
        }
        
        // If all else fails, just move a short distance in the original direction
        return transform.position + direction.normalized * (minWanderDistance * 0.5f);
    }
}