...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Add these fields to VisualEffectManager
    [Header("Canvas References")]
    [SerializeField] private GameObject canvasSafeArea; // Reference to the SafeArea GameObject in Canvas
    [SerializeField] private RectTransform canvasRectTransform; // Main canvas rect transform
    ...
    private void InitializeUIPool(string poolName, GameObject prefab, int count = 10)
    {
        ...
        {
            canvasSafeArea = FindObjectOfType<UIHelper>()?.SafeArea;
            if (canvasSafeArea == null)
            {
                Debug.LogError("Cannot find Canvas SafeArea! UI symbols won't work properly.");
                return;
            }
        }
        ...
        if (!symbolPools.ContainsKey(poolName))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                GameObject symbol = Instantiate(prefab, canvasSafeArea.transform);
                
                // Convert to UI Image if it's a sprite renderer
                ...
                if (rectTransform != null)
                {
                    ...
                    rectTransform.sizeDelta = new Vector2(50, 50); // Reasonable size for UI
                }
            ...
            Debug.Log($"Created UI symbol pool: {poolName} with {count} symbols");
        }
    }
    ...
    {
        // Similar implementation as PlayXPSymbolEffect but with chitin pools and targets
        // ...
    }

    // Similar implementations for crumb symbols, etc.
} 