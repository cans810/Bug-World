...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // Just use the same method for consistency
        ForceContinuousWandering();
    }
    ...
    // Add a method for continuous wandering without stopping
    public void ForceContinuousWandering()
    {
        // First, stop all coroutines to prevent conflicts
        StopAllCoroutines();
        ...
        // Choose a random direction for immediate movement
        Vector3 randomDirection = GetRandomDirection();
        
        // Ensure we're not heading toward the nest
        ...
        // Set rotation immediately without any transition
        transform.rotation = Quaternion.LookRotation(randomDirection);
        
        // Skip straight to the movement phase of wandering routine
        StartCoroutine(SimpleWanderingMovement());
    }

    // Simplified wandering movement
    private IEnumerator SimpleWanderingMovement()
    {
        while (true)
        {
            // First, determine the direction to move
            Vector3 moveDirection = transform.forward;
            ...
            // Calculate a random distance to move
            float moveDistance = Random.Range(minWanderDistance, maxWanderDistance);
            float distanceMoved = 0;
            
            // Move in a straight line until we reach the desired distance
            while (distanceMoved < moveDistance)
            {
                // Exit conditions
                if (!isWandering || !enabled)
                    yield break;
                ...
                // Calculate step distance this frame
                float stepDistance = livingEntity.moveSpeed * Time.deltaTime;
                
                // Move forward in a perfectly straight line
                if (livingEntity != null)
                {
                    // Lock rotation and move
                    Quaternion currentRotation = transform.rotation;
                    livingEntity.MoveInDirection(moveDirection, 1.0f);
                    transform.rotation = currentRotation; // Prevent any rotation changes
                }
                else
                {
                    transform.position += moveDirection * stepDistance;
            ...
            {
                Vector3 dirToNest = (playerNestTransform.position - transform.position).normalized;
                float dotProduct = Vector3.Dot(newDirection, dirToNest);
                
                if (dotProduct > 0.5f)
                {
                    newDirection = -dirToNest;
                    newDirection = Quaternion.Euler(0, Random.Range(-45f, 45f), 0) * newDirection;
                }
            }
    ...
    // Override and simplify start method
    public void OnEnable()
    {
        // Only start wandering if not already started
