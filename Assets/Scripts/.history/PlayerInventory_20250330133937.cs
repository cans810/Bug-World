...
public class PlayerInventory : MonoBehaviour
{
    ...
    public void CollectChitin(GameObject chitinObject)
    {
        ...
        // IMPROVED CAPACITY CHECK: Check capacity BEFORE attempting to collect
        // This will block any collection attempts when already at max capacity
        if (_chitinCount >= maxChitinCapacity)
        {
        ...
        // Additional capacity check - belt and suspenders approach
        if (_chitinCount < maxChitinCapacity)
        {
        ...
        {
            // We somehow got here despite being at capacity, so trigger the max capacity event
            OnChitinMaxed?.Invoke();
        }
    }
    ...
    public bool AddChitin(int amount = 1, bool playSound = true)
    {
        ...
        // IMPROVED CHECK: Don't allow adding more if we're at or would exceed max capacity
        if (_chitinCount >= maxChitinCapacity)
        {
            // Trigger the maxed event to notify UI/audio systems
            OnChitinMaxed?.Invoke();
        ...
        // Calculate how much we can actually add without exceeding max
        int amountToAdd = Mathf.Min(amount, maxChitinCapacity - _chitinCount);
