...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (currentCrumbs > 0)
        {
            ...
            // If player has an equipped entity, handle that case
            ...
            // No equipped entity, check incubator
            else if (antIncubator != null)
            {
                // First check if incubator is full
                if (antIncubator.IsIncubatorFull())
                {
                    // Show a message that the incubator is full
                    UIHelper uiHelper = FindObjectOfType<UIHelper>();
                    if (uiHelper != null)
                    {
                        string message = antIncubator.GetIncubatorFullMessage();
                        uiHelper.ShowInformText(message);
                    }
                    
                    if (showDebugMessages)
                    {
                        Debug.Log("Cannot deposit crumbs: Incubator is full or egg already exists");
                    }
                    
                    return; // Prevent deposit if incubator is full
                }
                
                // Spawn visual effects for the crumbs being deposited only if incubator has capacity
                if (visualEffectManager != null)
                {
                    visualEffectManager.SpawnCrumbCollectEffect(currentCrumbs);
                }
                
                // Use crumbs for egg creation
