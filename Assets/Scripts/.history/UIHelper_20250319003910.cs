...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator AnimateTextScale(RectTransform targetRect, Vector3 originalScale, float duration)
    {
        ...
        {
            Debug.LogError($"Error during text animation: {e.Message}");
        }
        finally
        {
            // Always reset the scale in the finally block
            if (targetRect != null)
            {
                targetRect.localScale = originalScale;
            }
    ...
    // Replace existing animation methods with this safer version
    public void AnimateChitinCount()
    {
        if (countTextRect == null) return;
        
        // Stop any existing animation
        StopCoroutine("AnimateTextScale");
        
        // Start the animation
        StartCoroutine(AnimateTextScale(countTextRect, originalScale, animationDuration));
        
        // Always schedule a reset as a failsafe
        StartCoroutine(ResetScaleAfterDelay(countTextRect, originalScale, animationDuration + 0.1f));
    }

    public void AnimateCrumbCount()
    {
        if (crumbTextRect == null) return;
