...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    // Add this variable near the top with your other private variables
    private bool isPlayingXPSound = false;
    ...
    // Optimized version that starts spawning immediately
    private IEnumerator SpawnChitinSymbolsImmediate(int count, int xpAmount)
    {
        ...
        // Prepare lineup positions first - avoid calculations during spawning loop
        float lineWidth = symbolSpacing * count;
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        // Start the XP sound effect coroutine if it's not already running
        if (!isPlayingXPSound && SoundEffectManager.Instance != null)
        {
            StartCoroutine(PlayXPSoundRepeating());
        }
    ...
    // New coroutine to play the XP sound at consistent intervals
    private IEnumerator PlayXPSoundRepeating()
    {
        // Set flag to prevent multiple sound coroutines
        isPlayingXPSound = true;
        
        // Keep playing sound at intervals as long as XP symbols are flying
        while (isAnimationInProgress && xpSymbolsReachedTarget < totalXpSymbols)
        {
            // Play sound effect
            if (SoundEffectManager.Instance != null)
            {
            ...
            // Wait 1 second before playing again
            yield return new WaitForSeconds(0.6f);
        }
        
        // Play one final sound for good measure
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("XPGained");
        }
