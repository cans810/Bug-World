...
public class ChitinDropManager : MonoBehaviour
{
    ...
    {
        Debug.Log($"DropChitin called with amount: {amount} at position {position}");
    ...
    private IEnumerator SpawnChitinPieces(Vector3 position, int amount)
    {
        ...
        Debug.Log($"About to spawn {amount} chitin pieces");
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            // Make sure the chitin object is active and visible
            chitinObject.SetActive(true);
            ...
            if (rb != null)
            {
                ...
                // Make sure the rigidbody isn't kinematic or sleeping
                rb.isKinematic = false;
                rb.Sleep();
                rb.WakeUp();
            }
            
            Debug.Log($"Spawned chitin piece {i+1}/{amount} at {dropPosition}");
