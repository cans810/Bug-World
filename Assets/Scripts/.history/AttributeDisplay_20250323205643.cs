...
public class AttributeDisplay : MonoBehaviour
{
    ...
    {
        // Don't do any checks for 2 seconds after a show attempt
        if (Time.time - lastShowAttemptTime < 2.0f)
            return;
        
        // Only hide if we're 100% sure it wasn't explicitly shown
        if (attributesPanel != null && attributesPanel.activeSelf && 
            !isPlayerInitiatedAction && !forceShowingPanel)
        {
            Debug.LogWarning("AttributeDisplay: Panel may have been activated automatically - hiding it");
            attributesPanel.SetActive(false);
        }
    }
