...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        if (visualEffectManager != null)
        {
            ...
            if (uiEffects != null)
            {
                Debug.Log($"Found UIVisualEffectController, calling PlayChitinDepositEffect with {playerInventory.ChitinCount} chitin and {xpAmount} XP");
                
                // Trigger the UI-based chitin deposit effect
                uiEffects.PlayChitinDepositEffect(playerInventory.ChitinCount, xpAmount);
                
                // Let the animation play out before adding XP
                yield return new WaitForSeconds(1.5f);
            }
            else
            {
                Debug.LogError("UIVisualEffectController not found in scene! Please add it to your Canvas.");
            }
            ...
            if (playerInventory != null)
            {
                ...
                // Debug log to verify
                Debug.Log($"Added {xpAmount} XP from {playerInventory.ChitinCount} chitin (1 XP per chitin)");
            }
