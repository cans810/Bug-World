...
public class UIHelper : MonoBehaviour
{
    ...
    // Add a method to create a dedicated animation canvas
    private Canvas CreateAnimationCanvas()
    {
        // Create a new GameObject for the animation canvas
        GameObject canvasObj = new GameObject("ChitinAnimationCanvas");
        ...
        // Add a GraphicRaycaster (required for Canvas)
        canvasObj.AddComponent<GraphicRaycaster>();
        
        Debug.Log("[CRITICAL] Created dedicated animation canvas");
        return canvas;
    }

    // Update the SpawnAndAnimateChitinIcons method to use the dedicated canvas
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Create a dedicated canvas for the animation
        Canvas animCanvas = CreateAnimationCanvas();
        
        // Get starting position in screen coordinates
        Vector2 startScreenPos;
        if (chitinCountText != null)
        {
            // Convert the chitinCountText position to screen coordinates
            RectTransform rectTransform = chitinCountText.GetComponent<RectTransform>();
            Vector3[] corners = new Vector3[4];
            rectTransform.GetWorldCorners(corners);
            startScreenPos = RectTransformUtility.WorldToScreenPoint(Camera.main, corners[0]);
            Debug.Log($"[CRITICAL] Using chitinCountText screen position: {startScreenPos}");
        }
        else
        {
            // Use screen center as fallback
            startScreenPos = new Vector2(Screen.width / 2, Screen.height / 2);
            Debug.Log($"[CRITICAL] Using screen center position: {startScreenPos}");
        }
        ...
        if (nestObject == null)
        {
            ...
            Destroy(animCanvas.gameObject);
            yield break;
        }
        
        // Get nest screen position
        Vector2 nestScreenPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
        ...
        {
            // Create icon as a child of the animation canvas
            GameObject icon = Instantiate(chitinIconPrefab, animCanvas.transform);
            ...
            if (iconRect != null)
            {
                ...
                RectTransformUtility.ScreenPointToLocalPointInRectangle(
                    animCanvas.GetComponent<RectTransform>(),
                    startScreenPos,
                    null, // No camera needed for ScreenSpaceOverlay
                    out anchoredPos
                );
            ...
            if (iconImage != null)
            {
                iconImage.color = new Color(1f, 1f, 1f, 1f); // Fully opaque white
                iconImage.raycastTarget = false;
            }
            
            icon.name = $"ChitinIcon_{i+1}";
            allIcons.Add(icon);
            
            Debug.Log($"[CRITICAL] Created icon {i+1} of {chitinCount} at anchored position {iconRect.anchoredPosition}");
        }
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            // Convert nest screen position to canvas position
            Vector2 endPos;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(
                animCanvas.GetComponent<RectTransform>(),
                nestScreenPos,
            ...
            Debug.Log($"[CRITICAL] Icon {i+1} animation from {startPos} to {endPos}");
            ...
            while (timer < duration)
            {
                ...
                // Update nest position in case it moved
                Vector2 currentNestScreenPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                Vector2 currentEndPos;
                RectTransformUtility.ScreenPointToLocalPointInRectangle(
                    animCanvas.GetComponent<RectTransform>(),
                    currentNestScreenPos,
                    null,
                    out currentEndPos
                );
                
                // Calculate new position with arc
                Vector2 newPos = Vector2.Lerp(startPos, currentEndPos, easedT);
                newPos.y += 100f * Mathf.Sin(t * Mathf.PI); // Add arc
                
                // Update position and scale
                iconRect.anchoredPosition = newPos;
                iconRect.localScale = Vector3.Lerp(originalScale, Vector3.zero, t * 0.8f);
