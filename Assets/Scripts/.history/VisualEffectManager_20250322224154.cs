...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Define the target screen position (top right for coin counter)
        Vector2 screenTopRight = new Vector2(Screen.width - 100f, Screen.height - 50f);
        
        // Debug log to confirm the method is being called
        Debug.Log($"Spawning {count} coin symbols from position {panelPosition}");
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Set scale for coin symbols - doubled size to 0.06
            Vector3 coinScale = new Vector3(0.06f, 0.06f, 0.06f);
            ...
            Debug.Log($"Spawned coin symbol {i} at position {symbol.transform.position}");
            
            // Start the animation to the UI position
            StartCoroutine(AnimateCoinSymbolToScreen(symbol, screenTopRight));
            
            // Stagger the spawns
            yield return new WaitForSeconds(0.08f); // Slightly longer delay between coins
        }
        ...
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("CoinCollect", Vector3.zero, false);
        }
    }
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Animation duration - increased for slower movement
        float duration = 1.8f; // Increased from 1.0f to 1.8f for slower movement
        ...
        while (elapsed < duration)
        {
            ...
            // Calculate current target screen position
            Vector2 currentTargetScreenPos = new Vector2(Screen.width - 100f, Screen.height - 50f);
            Ray ray = Camera.main.ScreenPointToRay(currentTargetScreenPos);
            float distanceToCamera = 10f;
            Vector3 targetPos = ray.GetPoint(distanceToCamera);
            ...
            if (!hasBumpedUI && normalizedTime > 0.75f)
            {
                ...
                if (uiHelper != null)
                {
                    ...
                    if (playerInventory != null)
                    {
                        // Trigger the coin text animation
                        uiHelper.UpdateCoinDisplay(playerInventory.CoinAmount);
                    }
            yield return null;
        }
