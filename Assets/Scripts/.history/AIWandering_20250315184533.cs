...
public class AIWandering : MonoBehaviour
{
    ...
    [SerializeField] private bool enforceFixedYPosition = true;
    ...
    private void Update()
    {
        ...
        // Only enforce Y position occasionally to avoid disrupting movement
        if (enforceFixedYPosition && livingEntity != null && !livingEntity.IsDead && Time.frameCount % 30 == 0)
        {
            EnforceYPosition();
        }
    }
    ...
    private IEnumerator WanderingRoutine()
    {
        ...
        while (true)
        {
            // Only enforce Y at start of major state change, not every iteration
            if (enforceFixedYPosition && Mathf.Abs(transform.position.y - fixedYPosition) > 0.1f)
            {
                EnforceYPosition();
            }
            ...
            // During movement phase, only check Y occasionally
            int frameCount = 0;
            while (isWandering)
            {
                ...
                // Only enforce Y every 10 frames during movement
                frameCount++;
                if (enforceFixedYPosition && frameCount % 10 == 0)
                {
                    EnforceYPosition();
                }
    ...
    public void ForceRestartWandering()
    {
        ...
        // Ensure Y position is correct when restarting
        if (enforceFixedYPosition)
        {
            EnforceYPosition();
        }
    ...
    private IEnumerator MoveToWaypointRoutine(Vector3 waypoint)
    {
        ...
        while (moveTimer < moveDuration && isWandering)
        {
            ...
            // Always enforce Y position after movement
            if (enforceFixedYPosition)
            {
                EnforceYPosition();
            }
    ...
    // Modify the EnforceYPosition method to be less aggressive
    private void EnforceYPosition()
    {
        // Only apply correction if the difference is significant
        Vector3 position = transform.position;
        if (Mathf.Abs(position.y - spawnPosition.y) > 0.05f) // Increased tolerance
        {
            position.y = spawnPosition.y;
            transform.position = position;
            
            // Only affect velocity if we're far off the expected Y
            Rigidbody rb = GetComponent<Rigidbody>();
