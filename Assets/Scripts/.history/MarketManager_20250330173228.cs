...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == "doubleThreat")
        {
            ...
            if (incubator != null)
            {
                ...
                if (success)
                {
                    // Calculate the center position of all eggs
                    Vector3 eggsCenter = CalculateEggsCenter();
                    ...
                    // Show the eggs at their center position after a short delay
                    StartCoroutine(ShowEggAfterDelay(eggsCenter, 0.5f));
                }
                else
                {
                    // Show error message
    ...
    // Update ShowEggAfterDelay to take a position rather than a single egg
    private IEnumerator ShowEggAfterDelay(Vector3 position, float delay)
    {
        ...
        {
            // Use position-based animation method instead of transform-based
            cameraAnimations.AnimateToPosition(position);
            Debug.Log($"Camera animating to show eggs at position: {position}");
        }
    ...
    // Add a method to find all eggs and calculate their center position
    private Vector3 CalculateEggsCenter()
    {
        GameObject[] eggs = GameObject.FindGameObjectsWithTag("AllyEgg");
        
        if (eggs.Length == 0)
        {
            // Fallback to AntEggPile if no tagged eggs found
            GameObject eggPile = GameObject.Find("AntEggPile(Clone)");
            return eggPile != null ? eggPile.transform.position : Vector3.zero;
        }
        ...
        {
            center += egg.transform.position;
        }
        center /= eggs.Length;
        
        Debug.Log($"Calculated center position of {eggs.Length} eggs: {center}");
        return center;
    }
