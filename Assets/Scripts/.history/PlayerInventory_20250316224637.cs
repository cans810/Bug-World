...
public class PlayerInventory : MonoBehaviour
{
    ...
    private void CheckLevelUp()
    {
        ...
        if (newLevel > oldLevel)
        {
            ...
            // Check if a new area is unlocked at this level (example implementation)
            bool newAreaUnlocked = CheckForNewAreaUnlock(newLevel);
            ...
            // Show level up UI message and rewards panel
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null)
            {
                uiHelper.ShowInformText($"Level Up! You are now level {newLevel}");
                
                // Show the rewards panel with appropriate prefabs
                uiHelper.ShowLevelUpRewardsText(newAreaUnlocked);
            }
        }
    }

    // Add this method to check if a new area is unlocked at this level
    private bool CheckForNewAreaUnlock(int newLevel)
    {
        // Example implementation for which levels unlock new areas
        bool unlocksNewArea = (newLevel == 2 || newLevel == 5 || newLevel == 8 || newLevel == 10);
        
        // Notify the UI to show the level-up rewards panel with appropriate prefabs
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            // Show the rewards panel with the area unlock indicator if applicable
            uiHelper.ShowLevelUpRewardsText(unlocksNewArea);
        }
        
        // Return the result for other usages
        return unlocksNewArea;
    }
