...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (hasTerritory && respectTerritory && !isReturningToTerritory)
        {
            ...
            bool outsideTerritory = false;
            
            if (isRingTerritory)
            {
                // For ring territories, check both inner and outer boundaries
                outsideTerritory = distanceToCenter < innerTerritoryRadius - territoryReturnDistance || 
                                  distanceToCenter > territoryRadius + territoryReturnDistance;
            }
            else
            {
                // For regular territories, just check outer boundary
                outsideTerritory = distanceToCenter > territoryRadius + territoryReturnDistance;
            }
