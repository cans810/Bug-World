...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void CreateVisualSphere()
    {
        if (showRadius && sphereCollider != null)
        {
            ...
            if (fillCircle)
            {
                ...
                // Orient the circle to lie on the X-Y plane (vertical)
    ...
    // Helper method to create a circular mesh for X-Y plane
    private Mesh CreateCircleMesh(float radius, int segments)
    {
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            vertices[i + 1] = new Vector3(x, y, 0); // Note: Using x,y instead of x,z for X-Y plane
        }
