...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add this coroutine to handle the remaining level-ups after camera animation
    private IEnumerator ContinueLevelUpsAfterAnimation(int startLevel, int endLevel)
    {
        // Wait for camera animation to complete (approximately)
        ...
        for (int level = startLevel; level <= endLevel; level++)
        {
            ...
            // Check for additional area unlocks
            bool hasAreaUnlock = CheckForAreaUnlock(level);
            
            // If this level has an area unlock, wait for that animation to complete
            if (hasAreaUnlock)
            {
