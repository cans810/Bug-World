...
public class UIHelper : MonoBehaviour
{
    ...
    // Add this method to show the Size Up text
    public void ShowSizeUpText()
    {
        // Make sure we have a reference to the text component
        if (sizeUpText != null)
        {
            // Set the text and activate it
            sizeUpText.text = "Size Up!";
            sizeUpText.gameObject.SetActive(true);
            
            // Start coroutine to hide it after 3 seconds
            StartCoroutine(HideSizeUpTextAfterDelay(3f));
            
            // Also add a visual effect - pulsing animation
            StartCoroutine(AnimateSizeUpText());
            
            // Play a sound effect if available
            if (SoundEffectManager.Instance != null)
            {
                SoundEffectManager.Instance.PlaySound("SizeUp", transform.position, false);
            }
            
            Debug.Log("Size Up text displayed");
        }
        else
        {
            Debug.LogWarning("Size Up text reference is missing in UIHelper");
        }
    }

    // Coroutine to hide the Size Up text after a delay
    ...
    // Coroutine to animate the Size Up text with a pulsing effect
    private IEnumerator AnimateSizeUpText()
    {
        if (sizeUpText == null) yield break;
        
        // Get the rect transform
        RectTransform textRect = sizeUpText.rectTransform;
        Vector3 originalScale = textRect.localScale;
        
        // Animation parameters
        float duration = 3f; // Same as display time
        float pulseCount = 3f; // Number of pulses
        float maxScale = 1.3f; // Maximum size
        ...
        while (elapsed < duration)
        {
            ...
            float pulse = Mathf.Sin(progress * pulseCount * 2f * Mathf.PI) * 0.5f + 0.5f;
            
            // Apply scale
            float currentScale = Mathf.Lerp(1f, maxScale, pulse);
