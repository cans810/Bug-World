...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (distanceToPlayer > followDistance)
        {
            ...
            // Force animation update with explicit walking state
            if (animController != null)
            {
                animController.SetWalking(true);
                
                // Debug animation state
                Debug.Log($"Ally moving to maintain distance ({distanceToPlayer:F2} > {followDistance}), setting walk animation");
            }
            else
            {
                Debug.LogWarning("AnimationController is null on AllyAI!");
            }
        ...
        // If we're close enough to player, stop moving
        else
        {
            isMoving = false;
            
            // Force animation update with explicit idle state
            if (animController != null)
            {
