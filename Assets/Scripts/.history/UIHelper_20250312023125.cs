...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Get the canvas - since UIHelper is attached to a canvas, we can use this directly
        Canvas canvas = GetComponent<Canvas>();
        if (canvas == null)
        {
            Debug.LogError("[CRITICAL] UIHelper must be attached to a Canvas!");
            yield break;
        }
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            ...
            if (iconImage != null)
            {
                ...
                {
                    Debug.LogError("[CRITICAL] Icon sprite is null!");
                    // Try to find a default sprite
                    Sprite defaultSprite = Resources.FindObjectsOfTypeAll<Sprite>().FirstOrDefault();
                    if (defaultSprite != null)
                    {
                        iconImage.sprite = defaultSprite;
                        Debug.Log("[CRITICAL] Applied a default sprite to the icon");
                    }
                }
            }
            ...
            // Make sure it's active and in front
            icon.SetActive(true);
        ...
        // Now animate each icon sequentially with a longer duration for testing
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            // Animate with a longer duration for testing
            float duration = 2.0f;
            float timer = 0f;
            ...
            while (timer < duration)
            {
                ...
                // Calculate new position with a higher arc
                Vector3 newPos = Vector3.Lerp(iconStartPos, targetPos, t);
                newPos.y += 200f * Mathf.Sin(t * Mathf.PI); // Higher arc for visibility
                
                // Update position
                icon.transform.position = newPos;
                
                // Scale down gradually but not completely until the end
                icon.transform.localScale = Vector3.Lerp(originalScale, originalScale * 0.5f, t);
                
                // Log position periodically
                if (Mathf.Approximately(t % 0.25f, 0))
                {
                    Debug.Log($"[CRITICAL] Icon {i+1} at position {newPos}, progress {t:F2}");
                }
                
                yield return null;
            }
            
            // Final scale down and destroy
            float finalTimer = 0f;
            ...
            while (finalTimer < finalDuration)
            {
                ...
                float t = Mathf.Clamp01(finalTimer / finalDuration);
                
                // Scale down to zero
                icon.transform.localScale = Vector3.Lerp(originalScale * 0.5f, Vector3.zero, t);
        ...
        Debug.Log("[CRITICAL] Animation sequence completed");
    }
