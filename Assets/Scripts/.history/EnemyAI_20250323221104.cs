...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modify FaceTarget for better balanced rotation speed
    private void FaceTarget(Vector3 target)
    {
        ...
        if (directionToTarget.magnitude > 0.1f)
        {
            ...
            // Calculate angle difference to determine rotation speed
            float angleDifference = Quaternion.Angle(transform.rotation, targetRotation);
            
            // Adjust rotation speed based on how far we need to turn
            // Higher base speed but still smooth
            float baseSpeed = inCombatStance ? targetRotationSpeed * 2.5f : targetRotationSpeed * 2.0f;
            
            // Increase rotation speed when the angle is larger
            float adaptiveSpeed = baseSpeed * (1.0f + (angleDifference / 180.0f));
            
            // Apply smooth rotation using Slerp with adaptive speed
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, adaptiveSpeed * Time.deltaTime);
            
            // Extra precaution to clear any angular velocity when in combat
            if (inCombatStance)
            {
                Rigidbody rb = GetComponent<Rigidbody>();
                if (rb != null)
    ...
    // Updated coroutine to make enemy face the player with adaptive rotation speed
    public IEnumerator FacePlayerWhileInHitbox(LivingEntity playerEntity)
    {
        while (isPlayerInHitbox && !livingEntity.IsDead && playerEntity != null && !playerEntity.IsDead)
        {
            ...
            if (directionToPlayer.magnitude > 0.1f)
            {
                ...
                // Use a higher base rotation speed but still maintain smoothness
                float baseSpeed = targetRotationSpeed * 2.0f;
