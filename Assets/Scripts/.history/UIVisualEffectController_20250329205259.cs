...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        if (!xpAddedThisSession && playerInventory != null && pendingXpAmount > 0)
        {
            ...
            int previousLevel = playerInventory.PlayerLevel;
            ...
            // Explicitly check for level up and show panel if needed - in case the playerInventory AddXP didn't do it
            if (playerInventory.PlayerLevel > previousLevel)
            {
                ...
                {
                    levelUpPanel.ShowLevelUpPanel(playerInventory.PlayerLevel);
                    Debug.Log("Showing level up panel for level " + playerInventory.PlayerLevel);
