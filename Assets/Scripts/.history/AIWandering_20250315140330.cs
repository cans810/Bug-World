...
public class AIWandering : MonoBehaviour
{
    ...
    private Vector3 GetSafeWaypoint(float customRadius = 0f)
    {
        ...
        // If we're already near the center, move in a random direction but stay within safe radius
        if (Vector3.Distance(transform.position, spawnPosition) < minWanderDistance)
        {
            Vector3 randomDir = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
            safeWaypoint = transform.position + randomDir * (minWanderDistance * 0.8f);
            
            // Ensure this random direction doesn't take us too close to the border
            float distToSpawn = Vector2.Distance(
                new Vector2(safeWaypoint.x, safeWaypoint.z),
                new Vector2(spawnPosition.x, spawnPosition.z)
            );
            
            if (distToSpawn > effectiveRadius * 0.9f)
            {
                // If too close to border, just move toward center
                safeWaypoint = transform.position + towardsCenter * (minWanderDistance * 0.8f);
            }
        }
