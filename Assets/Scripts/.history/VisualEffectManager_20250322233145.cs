...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        {
            Debug.LogError($"Error in coin animation: {e.Message}");
        }
        finally
        {
            // Ensure cleanup happens even if there was an error in the animation
            // Reset and return to pool
            if (spriteRenderer != null)
                spriteRenderer.color = startColor;
            symbol.transform.localScale = initialScale;
            ...
            if (symbolPools.ContainsKey("coin") && symbol != null)
            {
