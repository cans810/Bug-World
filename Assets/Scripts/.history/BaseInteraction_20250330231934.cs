...
public class BaseInteraction : MonoBehaviour
{
    ...
    {
        // Early return if no player inventory
        if (playerInventory == null) return;
        
        // Get current crumb count
        int crumbCount = playerInventory.CrumbCount;
        
        // Continue only if player has crumbs to deposit
        if (crumbCount <= 0) return;
        
        // This is where we'll add our first-time check
        // If player has at least one crumb and is attempting to deposit
        bool isFirstCrumbDeposit = playerInventory.HasYetToCollectCrumb();
        
        if (isFirstCrumbDeposit)
        {
            // Mark that player has attempted to deposit crumbs
            playerInventory.SetHasCollectedCrumb(true);
            
            // Activate the discovered entities button indicator
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null && uiHelper.discoveredEntitiesButtonPointingPulsatingImage != null)
            {
                ...
                Debug.Log("First crumb deposit - Activated discovered entities panel indicator");
            }
        }
        ...
        if (showDebugMessages)
        {
            Debug.Log($"Depositing {crumbCount} crumbs into incubator");
        }
        
        // Add crumbs to incubator
        if (insectIncubator != null)
        {
            insectIncubator.AddCrumbs(crumbCount);
            ...
            if (visualEffectManager != null)
            {
                ...
                visualEffectManager.SpawnEffect("shine", effectPosition, crumbCount);
            }
            
            // Remove crumbs from player inventory
            playerInventory.RemoveCrumbs(crumbCount);
        }
        else
        {
            Debug.LogWarning("Incubator reference is null, cannot deposit crumbs");
        }
    }
