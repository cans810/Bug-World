...
public class UIHelper : MonoBehaviour
{
    ...
    public void ShowInformText(string text)
    {
        ...
        // Trigger animation
        textAnimator.SetBool("Dissappear", false);  // Fixed spelling
    ...
    private System.Collections.IEnumerator HideInformTextAfterDelay(float delay)
    {
        ...
        Debug.Log("3 seconds passed, hiding inform text now");
        
        // Get animator reference
        Animator textAnimator = informPlayerText.GetComponent<Animator>();
        if (textAnimator != null)
        {
            // Directly set animation parameters
            textAnimator.SetBool("Appear", false);
            textAnimator.SetBool("Dissappear", true);
            
            // Force animator update to ensure it processes the parameter changes immediately
            textAnimator.Update(0f);
            
            Debug.Log("Animation parameters set: Appear=false, Dissappear=true");
        }
        else
        {
            Debug.LogWarning("Animator is null when trying to hide text");
        }
    ...
    public void HideInformText()
    {
        if (informPlayerText == null)
        {
            Debug.LogWarning("Cannot hide inform text - text component is null");
            return;
        }
