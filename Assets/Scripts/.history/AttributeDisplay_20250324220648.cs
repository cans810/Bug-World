...
public class AttributeDisplay : MonoBehaviour
{
    ...
    public void UpdateDisplay()
    {
        if (playerAttributes != null)
        {
            // Update available points - Change the format to be more clear
            if (availablePointsText != null)
            {
                int points = playerAttributes.AvailablePoints;
                availablePointsText.text = $"{points}";
                
                // Add detailed debug log
                Debug.Log($"<color=magenta>AttributeDisplay updated: Available Points = {points}</color>");
            }
        ...
        {
            Debug.LogError("PlayerAttributes reference is missing in AttributeDisplay!");
        }
    }
