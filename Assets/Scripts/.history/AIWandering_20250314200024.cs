...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // Stop all coroutines to interrupt any current patterns
        StopAllCoroutines();
        
        // Set state to wandering (not waiting)
        isWaiting = false;
        isWandering = true;
        ...
        // Avoid the nest if needed
        if (!IsAlly() && playerNestTransform != null && avoidPlayerNest)
        {
            Vector3 dirToNest = (playerNestTransform.position - transform.position).normalized;
            float dotProduct = Vector3.Dot(randomDirection, dirToNest);
            
            // If pointing toward nest, pick a different direction
            if (dotProduct > 0.5f)
            {
                randomDirection = -dirToNest;
                randomDirection = Quaternion.Euler(0, Random.Range(-45f, 45f), 0) * randomDirection;
        ...
        StartCoroutine(ContinuousStraightMovement(randomDirection));
    }

    // Coroutine to immediately start moving in a straight line
    private IEnumerator ContinuousStraightMovement(Vector3 direction)
    {
        // Use the same movement logic as our main routine
        Vector3 startPosition = transform.position;
        Vector3 moveDirection = direction.normalized;
        ...
        // Move in straight line without waiting
        while (moveTimer < moveDuration && distanceMoved < moveDistance)
        {
            if (!isWandering)
                break;
        ...
        // After completing this movement, resume normal wandering pattern
        StartCoroutine(WanderingRoutine());
    }
}