...
public class EnemyAI : MonoBehaviour
{
    ...
    // Add these to support boundary handling
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Force move back inside and reflect
            ForceInsideBoundary(other, normal);
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // This is the case where they've actually left the boundary
            // Calculate direction from center of border to enemy
            Vector3 normal = (transform.position - other.bounds.center).normalized;
            normal.y = 0;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // Force move back inside and reflect
            ForceInsideBoundary(collision.collider, normal);
        }
    }

    private void ForceInsideBoundary(Collider boundary, Vector3 normal)
    {
        // Stop wandering temporarily to take control
        if (wanderingBehavior != null)
        {
        ...
        // Move significantly back inside the boundary
        transform.position += normal * 1.0f; // Increased push distance
        ...
        reflectionDirection = Quaternion.Euler(0, Random.Range(-15f, 15f), 0) * reflectionDirection;
        
        // Rotate to face the reflection direction
        transform.rotation = Quaternion.LookRotation(reflectionDirection);
        
        // Force the wandering behavior to use this new direction
        if (wanderingBehavior != null)
        {
            StartCoroutine(ResumeWanderingAfterBounce(reflectionDirection));
        }
    }

    private IEnumerator ResumeWanderingAfterBounce(Vector3 direction)
    {
        // Wait a short time before resuming wandering
        yield return new WaitForSeconds(0.2f);
        
        // Re-enable wandering with the new direction
        if (wanderingBehavior != null)
        {
            wanderingBehavior.SetWanderingEnabled(true);
            isWandering = true;
            wanderingBehavior.ForceNewWaypointInDirection(direction);
        }
    }
} 