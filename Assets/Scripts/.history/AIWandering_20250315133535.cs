...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // Try up to 10 random directions (increased from 8)
    ...
    private Vector3 GetSafeWaypointUsingVisualizer(Vector3 direction, float distance)
    {
        ...
        if (!borderVisualizerExists)
        {
            ...
            Vector3 fallback = transform.position + direction.normalized * distance * 0.5f;
            fallback.y = spawnPosition.y; // Force Y position
            return fallback;
        }

        // Calculate raw target position - with forced Y coordinate
        Vector3 targetPosition = transform.position + direction.normalized * distance;
        targetPosition.y = spawnPosition.y; // Force Y position
        
        // Apply stricter spawn area restrictions
        if (restrictToSpawnArea)
        {
            // Use horizontal distance for better accuracy
            float distanceFromSpawn = Vector2.Distance(
                new Vector2(targetPosition.x, targetPosition.z),
                new Vector2(spawnPosition.x, spawnPosition.z)
            );
            
            if (distanceFromSpawn > maxWanderRadius * 0.85f) // Use 85% of radius as safety margin
            {
                ...
                directionFromSpawn.y = 0; // Force horizontal direction
                directionFromSpawn.Normalize();
                
                targetPosition = spawnPosition + directionFromSpawn * (maxWanderRadius * 0.85f);
                targetPosition.y = spawnPosition.y; // Force Y position
            }
        }
        ...
        else
        {
            ...
            safePosition.y = spawnPosition.y; // Force Y position
            
            // Apply spawn area restriction to the safe position
            if (restrictToSpawnArea)
            {
                float distanceFromSpawn = Vector2.Distance(
                    new Vector2(safePosition.x, safePosition.z),
                    new Vector2(spawnPosition.x, spawnPosition.z)
                );
                ...
                if (distanceFromSpawn > maxWanderRadius * 0.85f)
                {
                    ...
                    safePosition = spawnPosition + directionFromSpawn * (maxWanderRadius * 0.85f);
                    safePosition.y = spawnPosition.y; // Force Y position
                }
            }
            ...
            {
                // If safe position is too close, move toward center of spawn area
                if (restrictToSpawnArea)
                {
                    Vector3 towardsCenter = (spawnPosition - transform.position).normalized;
                    towardsCenter.y = 0;
                    safePosition = transform.position + towardsCenter * (distance * 0.3f);
                    safePosition.y = spawnPosition.y; // Force Y position
                }
                else
                {
                    // Random direction if not restricted
                    Vector3 randomDir = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
                    safePosition = transform.position + randomDir * (distance * 0.3f);
                    safePosition.y = spawnPosition.y; // Force Y position
                }
            }
