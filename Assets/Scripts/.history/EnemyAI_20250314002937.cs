...
public class EnemyAI : MonoBehaviour
{
    ...
    // Add these properties near the other private variables
    private Vector3 boundaryOrigin;
    private float boundaryRadius = 0f;
    private bool hasBoundary = false;
    
    // Add these properties near the other boundary variables
    private float innerBoundaryRadius = 0f;
    private bool useRingBoundary = false;
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        bool isOutsideEntityBoundary = CheckEntityBoundaryViolation(nextPosition);
        ...
        else if (isOutsideBoundary || isOutsideEntityBoundary)
        {
            // Handle boundary violation
            Vector3 safePosition;
            
            if (isOutsideEntityBoundary && hasBoundary)
            {
                ...
                Vector3 redirectedDirection;
                
                // If using ring boundary, redirect differently based on which boundary was violated
                if (useRingBoundary)
                {
                    if (distanceToOrigin < innerBoundaryRadius)
                    {
                        // Too close to center, move outward
                        redirectedDirection = fromOrigin;
                    }
                    else if (distanceToOrigin > boundaryRadius)
                    {
                    ...
                    {
                        // Move along the ring (tangent)
                        redirectedDirection = Vector3.Cross(Vector3.up, fromOrigin);
                    }
                }
                else
                {
                    // For regular circular boundary, just move along the edge
                    redirectedDirection = Vector3.Cross(Vector3.up, fromOrigin);
                }
                
                // Rotate towards redirected direction
            ...
            else if (isOutsideBoundary && mapBoundary != null)
            {
                // Get the nearest safe position
                safePosition = mapBoundary.GetNearestPointInBounds(nextPosition);
    ...
    // Modify CheckEntityBoundaryViolation to handle ring boundaries
    private bool CheckEntityBoundaryViolation(Vector3 position)
    {
        if (!hasBoundary)
            return false;
        
        // Calculate distance to boundary origin (ignoring Y)
        float distanceToOrigin = Vector3.Distance(
            new Vector3(position.x, 0, position.z),
            new Vector3(boundaryOrigin.x, 0, boundaryOrigin.z)
        );
        
        // For ring boundaries, we need to stay between inner and outer radius
        if (useRingBoundary)
        {
            return distanceToOrigin > boundaryRadius || distanceToOrigin < innerBoundaryRadius;
        }
        
        // For regular circular boundaries, just check outer radius
        return distanceToOrigin > boundaryRadius;
    }
    
    // Add these public methods at the bottom of the class
    ...
    // Add a method to set inner boundary radius
    public void SetInnerBoundaryRadius(float radius)
    {
        innerBoundaryRadius = radius;
        useRingBoundary = radius > 0f;
    }
} 