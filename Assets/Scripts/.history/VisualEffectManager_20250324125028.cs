...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Updated animation method with faster, spiral movement
    private IEnumerator AnimateLevelUpSymbolRisingUpward(GameObject symbol, Transform playerTransform, Vector3 initialOffset, float delay, System.Action onComplete)
    {
        ...
        // Animation parameters - faster animation
        float animDuration = 1.5f;  // Reduced from 2.5f for faster movement
        float maxHeight = 8.0f;     // Higher maximum height for more dramatic effect
        float spiralRadius = 2.0f;  // Maximum radius of the spiral
        float spiralSpeed = 5.0f;   // How fast the spiral rotates
        float elapsed = 0f;
        
        // Random direction for this symbol's spiral
        float randomOffset = Random.Range(0f, Mathf.PI * 2);
        ...
        while (elapsed < animDuration && playerTransform != null)
        {
            ...
            float angle = normalizedTime * spiralSpeed * Mathf.PI * 2 + randomOffset;
            float currentRadius = spiralRadius * normalizedTime; // Radius increases as it rises
            ...
            float scaleMultiplier = normalizedTime < 0.2f ? 
                Mathf.Lerp(0.7f, 1.5f, normalizedTime / 0.2f) : // Larger scale peak (1.5x)
                Mathf.Lerp(1.5f, 0.5f, (normalizedTime - 0.2f) / 0.8f);
