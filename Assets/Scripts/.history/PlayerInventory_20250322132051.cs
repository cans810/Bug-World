...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add a more reliable method to process a single level-up
    private void ProcessLevelUp(int level)
    {
        Debug.Log($"Processing level up to level {level}");
        
        // Add attribute points - use the configured points per level from PlayerAttributes
        PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
        if (playerAttributes != null)
        {
            // Use the appropriate pointsPerLevel value (default to 2 if not found)
            int pointsToAdd = 2; // Default
