...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinSymbolsImmediate(int count, int xpAmount)
    {
        ...
        float screenWidth = Screen.width - 300f; // 300px margin on each side
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            Vector2 targetPos = new Vector2(
                ...
                screenCenter.y + 750f - (row * rowSpacing) // Positioned higher on screen (changed from 400f)
            );
    ...
    private IEnumerator AnimateChitinToLineup(GameObject chitinSymbol, Vector2 targetPos, int index, Vector2 startPos)
    {
        ...
        // Start with a smaller scale (40% of normal size)
        symbolRT.localScale = baseScale * 0.4f;
        ...
        // Animation
        float elapsed = 0f;
        bool soundPlayed = false;  // Track if we've played the sound yet
        ...
        while (elapsed < flyDuration)
        {
            ...
            // Gradually increase scale as it approaches the target
            // Start at 40% size and grow to full size with slight pulsing
            float growFactor = Mathf.Lerp(0.4f, 1.0f, curvedT);
            float pulseFactor = 1.0f + Mathf.Sin(t * Mathf.PI * 2) * 0.1f; // Slight pulse
            symbolRT.localScale = baseScale * growFactor * pulseFactor;
            
            // Play Pickup1 sound when the symbol is about halfway to its destination
            // Only play for some chitin symbols to avoid sound overload
            if (!soundPlayed && t > 0.4f && t < 0.6f && index % 3 == 0)
            {
                soundPlayed = true;
                if (SoundEffectManager.Instance != null)
                {
                    // Use random pitch for variety
                    float randomPitch = Random.Range(0.9f, 1.1f);
                    SoundEffectManager.Instance.PlaySoundWithPitch("Pickup2", randomPitch);
                }
            }
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        while (elapsed < xpFlyDuration)
        {
            ...
            // Gradually decrease scale as it approaches the target
            // Start at 100% size and shrink to 60% of original size with slight pulse
            float shrinkFactor = Mathf.Lerp(1.0f, 0.6f, t);
            float pulseFactor = 1.0f + Mathf.Sin(t * Mathf.PI * 2f) * 0.1f; // Subtle pulse
            symbolRT.localScale = baseScale * shrinkFactor * pulseFactor;
            
            yield return null;
        }
        
        // Ensure we hit the target with final scale
        symbolRT.position = targetPosition;
        symbolRT.localScale = baseScale * 0.6f; // Final smaller size
        ...
        // Play sound with varied volume based on how many XP symbols have reached the target
        if (SoundEffectManager.Instance != null && Random.value < 0.3f) // Only play for 30% of symbols
        {
            // Small delay for audio timing
            yield return new WaitForSeconds(Random.Range(0.01f, 0.05f));
            
            // Play with randomized pitch and varied volume
            float randomPitch = Random.Range(0.9f, 1.1f);
            float volume = Mathf.Lerp(0.5f, 1.0f, xpSymbolsReachedTarget / (float)totalXpSymbols);
            SoundEffectManager.Instance.PlaySoundWithPitchAndVolume("XPGained", randomPitch, volume);
        }
