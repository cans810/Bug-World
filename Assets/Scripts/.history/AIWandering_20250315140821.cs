...
public class AIWandering : MonoBehaviour
{
    ...
    private Vector3 GetSafeWaypoint(float customRadius = 0f)
    {
        ...
        // Subtract 0.1f to ensure we stay away from the border
        float effectiveRadius = (customRadius > 0 ? customRadius : maxWanderRadius) - 0.1f;
        ...
        for (int attempt = 0; attempt < 10; attempt++)
        {
            ...
            // Spawn area check - ensure waypoint is well within the safe radius
            if (restrictToSpawnArea)
            {
                ...
                // If waypoint is too close to border, adjust it inward
                if (waypointDistanceFromSpawn > effectiveRadius * 0.95f)
                {
                    ...
                    // Use 90% of effective radius to ensure we're safely away from border
                    potentialWaypoint = spawnPosition + directionFromSpawn * (effectiveRadius * 0.9f);
                    potentialWaypoint.y = spawnPosition.y;
            ...
            // Final safety check - ensure waypoint is within safe radius
            float finalDistanceFromSpawn = Vector2.Distance(
                new Vector2(potentialWaypoint.x, potentialWaypoint.z),
                new Vector2(spawnPosition.x, spawnPosition.z)
            );
            
            if (finalDistanceFromSpawn <= effectiveRadius * 0.95f)
                return potentialWaypoint;
        }
        
        // IMPROVED FALLBACK: Always move toward center with a safe distance
        // Calculate how far we are from spawn
        ...
        Vector3 safeWaypoint;
        
        if (distanceToSpawn > effectiveRadius * 0.8f)
        {
            // Near border - move directly toward center with a medium distance
            Vector3 towardsCenter = (spawnPosition - transform.position).normalized;
            towardsCenter.y = 0;
            
            // Use a distance that's proportional to how far we are from center
            float safeDistance = Mathf.Min(distanceToSpawn * 0.5f, minWanderDistance * 2f);
            safeWaypoint = transform.position + towardsCenter * safeDistance;
        }
        else if (distanceToSpawn < minWanderDistance)
        {
            // Very close to center - move in a random direction but stay well within bounds
            Vector3 randomDir = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
            float safeDistance = effectiveRadius * 0.4f; // Stay in the inner 40% of the radius
            safeWaypoint = transform.position + randomDir * safeDistance;
        }
        else
        {
            // Somewhere in the middle - move in a semi-random direction with inward bias
            Vector3 towardsCenter = (spawnPosition - transform.position).normalized;
            Vector3 randomDir = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
            
            // Blend between random and center direction
            Vector3 blendedDir = Vector3.Slerp(randomDir, towardsCenter, 0.3f);
            float safeDistance = effectiveRadius * 0.3f;
            safeWaypoint = transform.position + blendedDir * safeDistance;
        }
        
        // Final safety check - ensure the fallback waypoint is within bounds
        float fallbackDistanceFromSpawn = Vector2.Distance(
            new Vector2(safeWaypoint.x, safeWaypoint.z),
            new Vector2(spawnPosition.x, spawnPosition.z)
        );
        
        if (fallbackDistanceFromSpawn > effectiveRadius * 0.9f)
        {
            // If still too close to border, force a position at 50% of radius from center
            Vector3 directionFromSpawn = (safeWaypoint - spawnPosition).normalized;
            safeWaypoint = spawnPosition + directionFromSpawn * (effectiveRadius * 0.5f);
        }
        
        safeWaypoint.y = spawnPosition.y;
        
        // Use green for fallback waypoints too, since yellow looks confusing
