...
public class AllyEggController : MonoBehaviour
{
    ...
    // Improved method to find and enable all AllyAI components in the Ally layer
    private void EnableAllAllyAIComponents()
    {
        ...
        // Create a layer mask for the Ally layer
        ...
        // Filter objects by layer and enable their AllyAI components
        foreach (GameObject obj in allObjects)
        {
            if (obj.layer == allyLayer)
            {
                // Get AllyAI component if it exists
                AllyAI allyAI = obj.GetComponent<AllyAI>();
                ...
                if (allyAI != null)
                {
                    ...
                    enabledCount++;
                    Debug.Log($"Re-enabled AllyAI for {obj.name} in Ally layer");
                }
            }
        }
        
        Debug.Log($"Enabled {enabledCount} AllyAI components on {layerMask} GameObjects in Ally layer");
    }
}
