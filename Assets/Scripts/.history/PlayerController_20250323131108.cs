...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isAtMapBorder && borderNormal != Vector3.zero)
        {
            ...
            {
                // Cancel the portion of movement going into the border
                Vector3 tangentialMovement = moveDirection - (-borderNormal * movementIntoBorder);
                moveDirection = tangentialMovement.normalized * moveDirection.magnitude * 0.8f; // Reduce speed along boundary
                
                // Apply a small push away from the border
                if (livingEntity != null)
                {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (requiredLevel > 0 && playerLevel < requiredLevel)
            {
                ...
                if (livingEntity != null)
                {
                    livingEntity.MoveInDirection(borderNormal, 1f);
                }
                else
                {
                    transform.position += borderNormal * 1f;
                }
                ...
                borderName == "MapBorder13"))
            {
                ...
                if (livingEntity != null)
                {
                    livingEntity.MoveInDirection(borderNormal, 0.1f);
                }
                else
                {
                    transform.position += borderNormal * 0.1f;
                }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        else if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // If player isn't high enough level, enforce the border restriction
            if (playerLevel < requiredLevel)
            {
                ...
                Vector3 centerOrClosestPoint;
                if (other is SphereCollider)
                {
                    // For sphere colliders, push toward center
                    centerOrClosestPoint = other.transform.position;
                }
                else
                {
                    // For other colliders, get closest point
                    centerOrClosestPoint = other.ClosestPoint(transform.position + (other.transform.position - transform.position).normalized * 0.5f);
                }
                
                // Update border normal to point away from border
                borderNormal = (transform.position - centerOrClosestPoint).normalized;
                
                // Apply immediate force to push player back
                if (livingEntity != null)
                {
                    livingEntity.MoveInDirection(borderNormal, 0.4f);
                }
                else
                {
                    transform.position += borderNormal * 0.4f;
                }
