...
public class UIHelper : MonoBehaviour
{
    ...
    // Method to show a message that stays until cleared
    public void ShowPersistentInformText(string message)
    {
        // Stop any existing persistent message routine
        ...
        Debug.Log($"Showing persistent message: {message}");
    }

    // Method to clear the persistent message
    public void ClearPersistentInformText()
    {
        if (hasPersistentMessage && informPlayerText != null)
        {
            informPlayerText.gameObject.SetActive(false);
            isShowingInformText = false;
            hasPersistentMessage = false;
            
            Debug.Log("Cleared persistent inform text");
        }
    }

    // Override the regular ShowInformText to not clear persistent messages
    public void ShowInformText(string message, float duration = 5f)
    {
        // If we have a persistent message, don't override it
        if (hasPersistentMessage) return;
        ...
        if (informPlayerText != null)
        {
            ...
            persistentMessageCoroutine = StartCoroutine(ShowInformTextCoroutine(message, duration));
        }
    }
    ...
    // Add this coroutine for showing temporary inform text
    private IEnumerator ShowInformTextCoroutine(string message, float duration)
    {
        if (informPlayerText != null)
        {
            // Set the text and enable the object
            informPlayerText.text = message;
            informPlayerText.gameObject.SetActive(true);
            isShowingInformText = true;
            ...
            // Only hide if we're not showing a persistent message
    ...
    // Add this method to show warning text in red with specified duration
    public void ShowWarningText(string message, float duration = 6f)
    {
        ...
        if (informPlayerText != null)
        {
            ...
            // Save original color
            Color originalColor = informPlayerText.color;
            ...
            // Start coroutine to hide message and restore color
            persistentMessageCoroutine = StartCoroutine(ShowWarningTextCoroutine(duration, originalColor));
        }
    }

    // Coroutine to handle warning text timing and color restoration
    private IEnumerator ShowWarningTextCoroutine(float duration, Color originalColor)
    {
        // Wait for the specified duration
        yield return new WaitForSeconds(duration);
        
        // Hide the text
        if (informPlayerText != null && !hasPersistentMessage)
        {
            informPlayerText.gameObject.SetActive(false);
            isShowingInformText = false;
            
            // Restore the original color
            informPlayerText.color = originalColor;
        }
    }
} 
