...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float attackRangeBuffer = 0.5f; // Add a small buffer to the attack range
    [SerializeField] private float minAttackInterval = 0.2f; // Minimum time between attack attempts
    ...
    private IEnumerator AutoAttackCoroutine()
    {
        ...
        while (enabled && livingEntity != null && !livingEntity.IsDead && enableAutoAttack)
        {
            ...
            if (enemiesInRange.Count > 0)
            {
                ...
                {
                    // Calculate attack range based on living entity's attack range + buffer
                    ...
                    if (closestDistance <= attackRange)
                    {
                        ...
                        livingEntity.RotateTowards(directionToEnemy, 10f); // Faster rotation for combat
                        
                        // Small delay for rotation
                        yield return new WaitForSeconds(0.1f);
                        
                        // Only attack if enough time has passed since last attempt
                        if (Time.time - lastAttackAttemptTime >= minAttackInterval)
                        {
                            // Try to attack
                            isAttacking = true;
                            ...
                            if (attackStarted)
                            {
                                // Wait for attack animation to complete or cooldown to finish
                            ...
                            {
                                // If attack wasn't successful, wait a short time
                                yield return new WaitForSeconds(0.2f);
                            }
                        }
                        else
                        {
                            // Still on attack attempt cooldown
                            yield return new WaitForSeconds(0.1f);
                        }
                    }
                    else
                    {
                        // Too far to attack, move closer if configured to do so
                        isAttacking = false;
                        yield return new WaitForSeconds(0.2f);
                    }
                }
            ...
            {
                // No enemies in range, exit coroutine
                if (showDebugInfo)
                    Debug.Log($"Ally {gameObject.name}: No more enemies in range, stopping auto-attack");
        ...
        // Reset coroutine reference and attack state
        autoAttackCoroutine = null;
        isAttacking = false;
    ...
    private bool AttemptAttack(LivingEntity target)
    {
        ...
        // Set target in living entity (if method available)
        ...
        if (attackTriggered && animController != null)
        {
            ...
            if (showDebugInfo)
                Debug.Log($"Ally {gameObject.name}: Attacking {target.name}");
        }
