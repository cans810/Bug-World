...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleActions()
    {
        ...
        // Attack on left mouse button or touch - delegate to LivingEntity instead
        if (Input.GetMouseButtonDown(0) && !animController.IsAnimationPlaying("attack"))
        {
            // Check if we're clicking/touching on a UI element
            ...
            // Even if we're over a UI element, we need to ensure the click is processed by the UI system
            // This helps with first-click issues on mobile
            else
            {
                // This is intentionally empty - we're just ensuring the click is registered by the UI system
                // and not consumed by our attack logic
            }
        }
    ...
    private bool IsPointerOverUIElement()
    {
        ...
        if (Application.isMobilePlatform)
        {
            if (Input.touchCount > 0)
            {
                ...
                // Make sure we're checking for UI elements correctly
                if (EventSystem.current.IsPointerOverGameObject(id))
                {
                    // Debug to verify UI detection is working
                    Debug.Log("Touch detected over UI element");
                    return true;
                }
            }
            return false;
        ...
        else
        {
            ...
            {
                // Debug to verify UI detection is working
                Debug.Log("Mouse detected over UI element");
            }
            return isOverUI;
        }
    }
