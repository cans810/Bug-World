...
public class PlayerAttributes : MonoBehaviour
{
    ...
    // Add these fields to PlayerAttributes class
    [SerializeField] private AttributeStat speedStat = new AttributeStat { name = "Speed", points = 0, percentagePerPoint = 0.01f }; // 1% per point
    private const int MAX_SPEED_POINTS = 25;

    // Add these properties to access speed attribute
    public int SpeedPoints => speedStat.points;
    public int MaxSpeedPoints => MAX_SPEED_POINTS;
    public float SpeedMultiplier => 1f + (speedStat.points * speedStat.percentagePerPoint);
    ...
    {
        // Check if we have available points and haven't reached the max
        if (availablePoints > 0 && speedStat.points < MAX_SPEED_POINTS)
        {
            speedStat.points++;
            availablePoints--;
            ...
            // Log the update
            Debug.Log($"Increased Speed to {speedStat.points}. Speed multiplier: {SpeedMultiplier:F2}x");
