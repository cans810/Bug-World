...
public class ChitinDropper : MonoBehaviour
{
    ...
    // This method draws gizmos when the object is selected in the Scene view
    private void OnDrawGizmosSelected()
    {
        ...
        // Draw a sphere at the spawn position
        Gizmos.color = gizmoColor;
        Gizmos.DrawSphere(spawnPosition, 0.2f);
        
        // Draw potential chitin trajectories
        DrawPotentialTrajectories(spawnPosition);
    ...
    // This method visualizes possible chitin trajectories
    private void DrawPotentialTrajectories(Vector3 startPos)
    {
        // Get component for custom amount if available
        LivingEntity entity = GetComponent<LivingEntity>();
        int amount = (entity != null) ? entity.chitinAmount : Random.Range(minChitinToDrop, maxChitinToDrop + 1);
        
        // Draw arcs for sample chitin pieces
        for (int i = 0; i < gizmoSampleCount; i++)
        {
            // Create a random direction with minimal horizontal force
            Vector2 randomDirection = Random.insideUnitCircle.normalized;
            Vector3 horizontalForce = new Vector3(
                randomDirection.x,
                0,
            ...
            float upwardForce = Random.Range(1.5f, 2.5f);
            ...
            // Draw trajectory arc
            DrawTrajectoryArc(startPos, initialVelocity, 30);
        }
        
        // Display how many pieces will drop
        Vector3 labelPos = startPos + Vector3.up * 0.8f;
        #if UNITY_EDITOR
        UnityEditor.Handles.BeginGUI();
        UnityEditor.Handles.Label(labelPos, $"Will drop {amount} chitin pieces");
        UnityEditor.Handles.EndGUI();
        #endif
    }

    // Draws a predicted trajectory arc
    private void DrawTrajectoryArc(Vector3 start, Vector3 initialVelocity, int steps)
    {
        Vector3 previousPoint = start;
        float timestep = 0.1f;
        
        Color arcColor = new Color(gizmoColor.r, gizmoColor.g, gizmoColor.b, 0.4f);
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            // Position formula: p = p0 + v0*t + 0.5*g*t^2
            Vector3 position = start + initialVelocity * t + 0.5f * Physics.gravity * t * t;
            
            // Draw line segment
            Gizmos.DrawLine(previousPoint, position);
            
            // Also draw small sphere at each point 
            if (i % 5 == 0) // Every 5 steps
            {
                float alpha = 1.0f - ((float)i / steps); // Fade out with distance
                Gizmos.color = new Color(arcColor.r, arcColor.g, arcColor.b, alpha * 0.4f);
