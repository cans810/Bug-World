...
public class UIHelper : MonoBehaviour
{
    ...
    private readonly string disappearParam = "Dissappear";
    ...
    {
        // Use a more reliable way to wait for animation completion
        // Wait until the animator is actually in the Dissappear state
        while (!informTextAnimator.GetCurrentAnimatorStateInfo(0).IsName("Dissappear"))
        {
            yield return null;
        }
        
        // Then wait until the normalized time is past 0.95 (95% complete)
        while (informTextAnimator.GetCurrentAnimatorStateInfo(0).normalizedTime < 0.95f)
        {
            yield return null;
        }
        
        // Now the animation is definitely complete
    ...
    {
        // Animation has completed, now in fully visible state
        // No need to do anything special here
    }

    public void OnDisappearAnimationComplete()
    {
        // Animation has completed, now fully hidden
        isInformTextShowing = false;
        // You could also disable the GameObject here if needed
    }

    void OnAnimatorStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        // Check which state we entered
        if (stateInfo.IsName("Appear"))
        {
            // Started appear animation
        }
        else if (stateInfo.IsName("Dissappear"))
        {
            // Started disappear animation
        }
    }

    void OnAnimatorStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        // Check which state we exited
        if (stateInfo.IsName("Appear"))
        {
            // Finished appear animation
        }
        else if (stateInfo.IsName("Dissappear"))
        {
            // Finished disappear animation
            isInformTextShowing = false;
        }
    }
} 
