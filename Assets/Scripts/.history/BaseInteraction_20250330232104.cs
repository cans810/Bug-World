...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (playerInventory != null && playerInventory.CrumbCount > 0)
        {
            // Check if player has an equipped entity
            string equippedEntity = playerInventory.GetEquippedEntityType();
            
            // If no entity equipped, show guidance
            if (string.IsNullOrEmpty(equippedEntity))
            {
                // Find the UI Helper
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null && uiHelper.discoveredEntitiesButtonPointingPulsatingImage != null)
                {
                    ...
                    Debug.Log("Player attempted to deposit crumbs without an entity selected - showing guidance indicator");
                }
                
                // Since no entity is equipped, exit without depositing
                return;
            }
            
            // Rest of the existing crumb deposit code continues here...
            int crumbsToDeposit = playerInventory.CrumbCount;
            bool depositSuccess = HandleCrumbDeposit(crumbsToDeposit);
            
            if (depositSuccess)
            {
                // Play deposit sound via SoundEffectManager
                if (useSoundEffectManager && SoundEffectManager.Instance != null)
                {
                ...
                if (visualEffectManager != null)
                {
                    ...
                    ShowCrumbCollectionEffect(effectPosition, crumbsToDeposit);
                    
                    Debug.Log($"Showed crumb collection effect at {effectPosition}");
                }
            }
        }
    }
