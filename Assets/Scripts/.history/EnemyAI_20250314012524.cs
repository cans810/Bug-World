...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        // Only check for map boundary and nest avoidance
        ...
        {
            // Calculate direction from boundary center to entity (normal vector)
            Vector3 normal = (transform.position - mapBoundary.transform.position).normalized;
            
            // Calculate reflection vector (bounce direction) - classic reflection formula
            Vector3 reflectedDirection = Vector3.Reflect(direction, normal);
            
            // Apply a slight random variation to prevent strict back-and-forth bouncing
            reflectedDirection = Quaternion.Euler(0, Random.Range(-15f, 15f), 0) * reflectedDirection;
            
            // Instantly rotate to face the reflection direction
            transform.forward = reflectedDirection;
            
            // Apply the new direction at full speed (bounce effect)
            livingEntity.MoveInDirection(reflectedDirection);
            
            // If we're chasing, update the target position to be in the new direction
            if (isChasing)
            {
                lastKnownPlayerPosition = transform.position + reflectedDirection * 5f;
            }
        }
        else
        {
    ...
    private void ReturnToWandering()
    {
        ...
        if (wanderingBehavior != null)
        {
            ...
            wanderingBehavior.SetWanderingEnabled(true);
            // Force the wandering behavior to pick a new direction and start moving
    ...
    // Add this new method
    public void RedirectAfterBounce(Vector3 newDirection)
    {
        // If chasing player, update the target position
        if (isChasing)
        {
            // Create a new chase position in the new direction
            lastKnownPlayerPosition = transform.position + newDirection * 5f;
        }
        
        // Update the movement direction
        transform.forward = newDirection;
        
        // If using wandering behavior, force a new waypoint
        if (wanderingBehavior != null)
        {
            wanderingBehavior.ForceNewWaypoint();
        }
