...
public class EnemyAI : MonoBehaviour
{
    ...
    // Modify ReturnToWandering to immediately resume wandering without stopping
    private void ReturnToWandering()
    {
        ...
        // Skip the isMoving = false line to maintain movement
        // We want to transition directly to wandering
        
        // Cancel any pending coroutines that might re-enable chasing
        StopAllCoroutines();
        
        // Make sure wandering behavior is enabled and properly reset, immediately
        if (wanderingBehavior != null)
        {
            wanderingBehavior.enabled = true;
            wanderingBehavior.SetWanderingEnabled(true);
            
            // Force a new waypoint but maintain movement
            wanderingBehavior.ForceContinuousWandering();
        }
    }
