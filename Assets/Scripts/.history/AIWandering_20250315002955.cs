...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            // Death check after waiting
            if (livingEntity != null && livingEntity.IsDead) continue;

            // BORDER-CENTRIC MOVEMENT LOGIC
            // First, check if we're already too close to any border
            Vector3 moveDirection = Vector3.zero;
            float moveDistance = minWanderDistance;
            
            // Check proximity to borders with a 360° scan
            bool tooCloseToAnyBorder = false;
            Vector3 escapeDirection = Vector3.zero;

            // Do a full 360° scan with 16 rays
            for (int i = 0; i < 8; i++)
            {
                float angle = i * 45f; // 45 degrees between rays instead of 22.5
                Vector3 rayDir = Quaternion.Euler(0, angle, 0) * Vector3.forward;
                
                // Use a shorter distance for the "too close" check - make this MUCH shorter
                if (IsBorderInDirection(rayDir, borderDetectionDistance * 0.5f)) // 0.5 instead of 0.75
                {
                    tooCloseToAnyBorder = true;
                    // Accumulate the escape direction (away from borders)
                    escapeDirection -= rayDir;
                    
                    // Draw a warning ray showing the border is too close
                    Debug.DrawRay(transform.position, rayDir * borderDetectionDistance * 0.5f, Color.red, 3f);
                }
            }

            // CRITICAL: If too close to any border, move AWAY from borders with higher priority
            if (tooCloseToAnyBorder && escapeDirection.magnitude > 0.1f)
            {
                // Normalize and use the escape direction
                escapeDirection.y = 0;
                escapeDirection.Normalize();
                moveDirection = escapeDirection;
                
                // Use a shorter but guaranteed distance to move away from border
                moveDistance = minWanderDistance * 0.8f;
                
                Debug.LogWarning($"{gameObject.name}: TOO CLOSE TO BORDER! Moving away urgently!");
                Debug.DrawRay(transform.position, escapeDirection * 3f, Color.magenta, 5f);
            }
            // Normal movement with border checking
            else
            {
                // Try to find a completely safe direction using BorderVisualizer
                bool foundSafeDirection = false;
                
                // Try 12 random angles
                for (int attempt = 0; attempt < 12; attempt++)
                {
                    // Generate a random direction
                    float randomAngle = Random.Range(0, 360);
                    Vector3 testDirection = Quaternion.Euler(0, randomAngle, 0) * Vector3.forward;
                    
                    // Test if this direction with full distance would be valid
                    Vector3 testTarget = transform.position + testDirection * maxWanderDistance;
                    
                    if (BorderVisualizer.IsWaypointValid(transform.position, testTarget))
                    {
                        // Found a completely safe direction
                        moveDirection = testDirection;
                        moveDistance = Random.Range(minWanderDistance, maxWanderDistance);
                        foundSafeDirection = true;
                        
                        Debug.Log($"{gameObject.name}: Found safe direction on attempt {attempt}");
                        Debug.DrawRay(transform.position, moveDirection * moveDistance, Color.green, 3f);
                        break;
                    }
                }
                
                // If no completely safe direction found, use center-oriented movement
                if (!foundSafeDirection)
                {
                    Debug.LogWarning($"{gameObject.name}: No safe direction found, moving toward center");
                    
                    // Move toward center if we have a reference point
                    if (playerNestTransform != null)
                    {
                        // Direction toward center but with variation
                        Vector3 centerDir = (playerNestTransform.position - transform.position).normalized;
                        moveDirection = Quaternion.Euler(0, Random.Range(-45f, 45f), 0) * centerDir;
                    }
                    else
                    {
                        // Absolute fallback - try a random direction with very short distance
                        moveDirection = Random.onUnitSphere;
                        moveDirection.y = 0;
                        moveDirection.Normalize();
                    }
                    
                    // Use a shorter distance when unsure about safety
                    moveDistance = minWanderDistance * 0.7f;
                }
                
                // After all the if/else branches for center movement, make sure moveDirection is assigned
                if (moveDirection == Vector3.zero)
                {
                    moveDirection = Random.onUnitSphere;
                    moveDirection.y = 0;
                    moveDirection.Normalize();
                    moveDistance = minWanderDistance * 0.5f;
                    Debug.LogError($"{gameObject.name}: FALLBACK - Using completely random direction");
                }
            }
            ...
            // Calculate final waypoint position - use BorderVisualizer for final safety check
            Vector3 targetWaypoint = GetSafeWaypointUsingVisualizer(moveDirection, moveDistance);
            
            // GUARANTEED MOVEMENT FALLBACK - ensure entities always move somewhere
            if (Vector3.Distance(targetWaypoint, transform.position) < 0.5f)
            {
                Debug.LogWarning($"{gameObject.name}: Border system preventing all movement! Using emergency fallback movement.");
                
                // Emergency fallback - move a short distance in a random direction
                Vector3 emergencyDirection = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
                targetWaypoint = transform.position + emergencyDirection * minWanderDistance * 0.5f;
                
                // Visualize emergency movement
                Debug.DrawLine(transform.position, targetWaypoint, Color.blue, 5f);
            }
            ...
            {
                // CRITICAL: Check for borders EVERY FRAME during movement
                float remainingDistance = Vector3.Distance(transform.position, targetWaypoint);
                
                if (IsBorderInDirection(transform.forward, remainingDistance * 0.5f))
                {
                    Debug.LogWarning($"{gameObject.name}: Border detected while moving - stopping immediately!");
                    break;
                }
    ...
    // Complete rewrite of the movement logic to prioritize border avoidance
    private bool IsBorderInDirection(Vector3 direction, float distance)
    {
        ...
        Debug.DrawRay(rayOrigin, direction * distance, Color.yellow, 0.5f);
        
        // Cast 3 rays in a spread to detect borders more reliably
        // Center ray
        if (Physics.Raycast(rayOrigin, direction, distance, borderLayer))
            return true;
        
        // Left and right rays at 20 degree spread
        Vector3 leftDir = Quaternion.Euler(0, -20, 0) * direction;
        Vector3 rightDir = Quaternion.Euler(0, 20, 0) * direction;
        
        if (Physics.Raycast(rayOrigin, leftDir, distance, borderLayer) ||
            Physics.Raycast(rayOrigin, rightDir, distance, borderLayer))
            return true;
    ...
    public void ForceNewWaypointInDirection(Vector3 direction)
    {
        ...
        // IMPORTANT CHANGE: Always use the BorderVisualizer for safety
        float randomDistance = Random.Range(minWanderDistance, maxWanderDistance);
        Vector3 safeWaypoint = GetSafeWaypointUsingVisualizer(direction, randomDistance);
        ...
        // Start movement directly to the safe waypoint
        StartCoroutine(MoveDirectlyToWaypoint(safeWaypoint));
    }

    // Add this helper method for direct movement to a waypoint
    private IEnumerator MoveDirectlyToWaypoint(Vector3 waypoint)
    {
        ...
        // Movement logic with continuous border checks
        ...
        while (moveTimer < moveDuration && isWandering)
        {
            // Perform frequent border checks during movement
            if (moveTimer % 0.2f < Time.deltaTime)
            {
                // Check if we're still on a safe path
                Vector3 remainingPath = waypoint - transform.position;
                if (!BorderVisualizer.IsWaypointValid(transform.position, waypoint))
                {
                    Debug.LogWarning($"{gameObject.name}: Path became unsafe during movement - stopping");
                    break;
                }
            }
