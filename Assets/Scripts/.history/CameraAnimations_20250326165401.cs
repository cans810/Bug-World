...
public class CameraAnimations : MonoBehaviour
{
    ...
    // Update the coroutine to use the callback at the right moment
    private IEnumerator MetamorphosisCameraAnimation(Transform playerTransform, System.Action onCameraInPosition)
    {
        ...
        // Position in front of the player but at a shorter distance (closer)
        float distanceMultiplier = playerTransform.localScale.z * 1.2f + 1.5f; // Even closer
        
        // Calculate horizontal position (we'll adjust height separately)
        Vector3 horizontalOffset = playerForward * distanceMultiplier;
        
        // Position camera much higher to look down at a steeper angle
        float heightOffset = playerTransform.localScale.y * 3.0f; // Much higher position
        
        // Final target position combines horizontal offset with increased height
        Vector3 targetPosition = playerTransform.position + horizontalOffset;
        targetPosition.y = playerTransform.position.y + heightOffset;
        
        // Calculate rotation to look more steeply down at the player
        Quaternion targetRotation = Quaternion.LookRotation(
            playerTransform.position - targetPosition, // Direction to look
