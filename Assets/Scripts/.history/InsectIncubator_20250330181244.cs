...
public class InsectIncubator : MonoBehaviour
{
    ...
    // Update the OnEggHatched method to handle multiple eggs
    public void OnEggHatched(GameObject eggObject)
    {
        // Decrement the count
        currentEggCount = Mathf.Max(0, currentEggCount - 1);
        
        // Check if there are still eggs in the scene
        GameObject[] remainingEggs = GameObject.FindGameObjectsWithTag("AllyEgg");
        bool hasRemainingEggs = remainingEggs.Length > 0;
        
        // Only reset EggExists flag if there are no more eggs
        if (!hasRemainingEggs)
        {
            EggExists = false;
            Debug.Log("All eggs have hatched, resetting EggExists flag");
        }
        else
        {
            Debug.Log($"Egg hatched but {remainingEggs.Length} eggs still remain");
        }
        ...
        {
            uiHelper.UpdateEggDisplay(currentEggCount);
            Debug.Log($"Updated UI after egg hatched. New count: {currentEggCount}");
        }
        else
        {
            // Try to find UIHelper if not assigned
            ...
            {
                helper.UpdateEggDisplay(currentEggCount);
                Debug.Log($"Found and updated UIHelper after egg hatched. New count: {currentEggCount}");
            }
        }
