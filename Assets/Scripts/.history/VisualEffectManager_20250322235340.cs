...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        while (elapsed < duration)
        {
            ...
            // DESTROY EARLIER - only animate to 85% of the way
            if (t > 0.85f)
            {
                break;
            }
            
            // Very simple direct movement - linear path with slight upward curve
            float yOffset = Mathf.Sin(t * Mathf.PI) * 0.3f; // Much smaller arc
            ...
            if (t > 0.7f)
            {
                float fade = (t - 0.7f) / 0.15f; // Fade over 15% of animation
        ...
        // Make sure UI gets bumped if it hasn't already
        if (!hasBumpedUI)
        {
            TriggerUICounterAnimation("coin");
        }
