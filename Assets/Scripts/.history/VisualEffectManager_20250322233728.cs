...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateCoinSymbolToScreen(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Animation duration - INCREASED slightly to ensure they reach destination
        float duration = 1.5f; // Increased from 1.2f to 1.5f to ensure full travel
        ...
        Debug.Log($"Starting coin animation for symbol {symbolId}");
        ...
        while (elapsed < duration)
        {
            ...
            // Force completion when very close to the end, but don't exit loop early
            bool nearlyFinished = normalizedTime > 0.98f;
            
            // Use a modified curve to ensure coins reach their destination
            // This curve accelerates faster at the end
            float curveValue = nearlyFinished ? 1f : chitinFlyCurve.Evaluate(normalizedTime);
            ...
            // Use a modified position calculation to ensure they reach the destination
            Vector3 newPosition;
            if (nearlyFinished) {
                // In the last 2% of time, move directly to target
                newPosition = Vector3.Lerp(symbol.transform.position, targetWorldPos, 0.5f);
            } else {
                newPosition = Vector3.Lerp(a, b, curveValue);
            }
            ...
            // IMPORTANT: Only fade out AFTER reaching destination (>90%)
            if (normalizedTime > 0.9f)
            {
                float fadeProgress = (normalizedTime - 0.9f) / 0.1f;
        ...
        if (symbol != null && symbol.activeInHierarchy)
        {
            symbol.SetActive(false); // Force deactivate here
        }
        CleanupCoinSymbol(symbol, spriteRenderer, startColor, initialScale);
    }
