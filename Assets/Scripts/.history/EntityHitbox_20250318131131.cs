...
public class EntityHitbox : MonoBehaviour
{
    ...
    // Add this delegate and event
    public delegate void EnemyDeathDelegate(LivingEntity enemy);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (ownerIsPlayerOrAlly && otherIsInsect)
        {
            ...
            if (insectEntity != null && !insectEntity.IsDead)
            {
                ...
                if (!enemiesInRange.Contains(insectEntity))
                {
                    enemiesInRange.Add(insectEntity);
                }
                
                // Subscribe to death event
                insectEntity.OnDeath.AddListener(HandleEnemyDeath);
                
                // Trigger the enter event for auto-attack
                OnPlayerEnterHitbox?.Invoke(insectEntity);
                
                // DISCOVERY SYSTEM - Add this block
                if (playerController != null) // Only trigger discovery for player
                {
                    Debug.Log($"ENTITY HITBOX: Player detected insect {insectEntity.name}, triggering discovery");
                    DiscoveredEntitiesController controller = DiscoveredEntitiesController.Instance;
                    if (controller != null)
                    {
                        controller.CheckAndDiscoverEntity(insectEntity.name);
                    }
                    else
                    {
                        Debug.LogWarning("DiscoveredEntitiesController not found for discovery!");
                    }
                }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (ownerIsPlayerOrAlly && otherIsInsect)
        {
            ...
            if (insectEntity != null)
            {
                ...
                if (enemiesInRange.Contains(insectEntity))
                {
                    enemiesInRange.Remove(insectEntity);
                }
                
                // Unsubscribe from death event
                insectEntity.OnDeath.RemoveListener(HandleEnemyDeath);
    ...
    // Modify the method that handles enemy deaths
    private void HandleEnemyDeath(LivingEntity enemy)
    {
