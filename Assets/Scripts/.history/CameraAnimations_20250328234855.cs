...
public class CameraAnimations : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isInMapView && !isAnimating)
        {
            ...
            // Apply zoom smoothly
            if (Mathf.Abs(cameraTransform.position.y - (playerPosition.y + targetZoomHeight)) > 0.01f)
            {
                // Get current position
                Vector3 currentPos = cameraTransform.position;
                
                // Calculate new height
                float newHeight = Mathf.Lerp(currentPos.y - playerPosition.y, targetZoomHeight, Time.deltaTime * 5f);
