...
public class AttributeDisplay : MonoBehaviour
{
    ...
    private void Start()
    {
        // Add this line at the beginning of Start to ensure auto-show is disabled
        autoShowOnLevelUp = false;
    ...
    private void OnPlayerLevelUp(int newLevel)
    {
        ...
        if (autoShowOnLevelUp && !isInCameraAnimation)
        {
            ShowPanel(true);
        }
    }
    ...
    private void ShowPanelAfterCameraAnimation(GameObject areaTarget, int level, string areaName)
    {
        ...
        Debug.Log("AttributeDisplay: Camera animation completed, but NOT showing panel automatically");
        
        // DO NOT show panel automatically - remove any auto-show code
        // shouldShowAttributesPanel = false; // Reset flag but don't show panel
    }

    // Update method to check if we should show the panel
    private void Update()
    {
        // MODIFY THIS - only show if we've explicitly asked for it AND auto-show is enabled
        if (shouldShowAttributesPanel && autoShowOnLevelUp && cameraAnimations != null && !cameraAnimations.IsAnimationInProgress())
        {
            Debug.Log("Animation ended, checking if we should show attributes panel");
            shouldShowAttributesPanel = false;
            
            // We don't want to show automatically, so remove this code or add a check
            // Only show if autoShowOnLevelUp is true
            if (autoShowOnLevelUp)
            {
                // Then show the panel...
                if (attributesPanel != null)
                {
    ...
    public void NotifyCameraAnimationEnded()
    {
        ...
        // Instead, just log that we're not showing the panel
        Debug.Log("AttributeDisplay: Camera animation ended but not showing attribute panel automatically");
    }
