...
public class PlayerInventory : MonoBehaviour
{
    ...
    [SerializeField] private int _coinCount = 0;  // Use a single field for coins
    ...
    public int CoinCount
    {
        ...
        private set
        {
            _coinCount = Mathf.Max(0, value);  // Ensure coins don't go below 0
    ...
    {
        Debug.Log("ForceUpdateUI called - updating all UI elements");
        
        // Trigger all UI update events
        if (OnChitinCountChanged != null) OnChitinCountChanged.Invoke(_chitinCount);
        if (OnCrumbCountChanged != null) OnCrumbCountChanged.Invoke(_crumbCount);
        if (OnExperienceChanged != null) OnExperienceChanged.Invoke(_experience);
        if (OnLevelDisplayUpdate != null) OnLevelDisplayUpdate.Invoke(CurrentLevel);
        if (OnCoinCountChanged != null) OnCoinCountChanged.Invoke(_coinCount);
        
        // Update UI directly if needed
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            uiHelper.UpdateCoinDisplay(_coinCount);
            uiHelper.UpdateEggDisplay(currentEggCount);
            // Update other UI elements as needed
        }
        
        Debug.Log($"Forced UI update - Level: {CurrentLevel}, XP: {_experience}, " +
                  $"Chitin: {_chitinCount}, Crumb: {_crumbCount}, Egg: {currentEggCount}, " +
                  $"Coin: {_coinCount}");
    }
    ...
    // Add this method to the DepositChitin method to check for double calls
    public void DepositChitin(int amount)
    {
        if (amount <= 0 || ChitinCount < amount) return;
        
        // Calculate XP gain using the experiencePerChitin field
        int xpGain = amount * experiencePerChitin;
        
        // Add debug log with stack trace
        Debug.Log($"DepositChitin called with {amount} chitin for {xpGain} XP (using experiencePerChitin={experiencePerChitin})");
        
        // Reset xpGain to exactly what we want (force it to be 2 per chitin)
        xpGain = amount * 2;
        ...
        // Trigger XP gain visual effect
        VisualEffectManager visualEffectManager = FindObjectOfType<VisualEffectManager>();
        if (visualEffectManager != null)
        {
            // FORCE the visual effect to use exactly the right amount
            visualEffectManager.PlayXPGainEffect(transform.position, xpGain);
        }
    ...
    public void SaveEggData(GameData gameData)
    {
        if (gameData == null) return;
        
        // Save egg data
        gameData.currentEgg = currentEggCount;
        gameData.maxEggCapacity = maxEggCapacity;
        
        Debug.Log($"Saved egg data: Count={currentEggCount}, MaxCapacity={maxEggCapacity}");
    }
    ...
    public bool AddCoins(int amount)
    {
        ...
        Debug.Log($"Added {amount} coins. New total: {_coinCount}");
        return true;
    }
    ...
    public void SaveInventoryData(GameData gameData)
    {
        if (gameData == null) return;
        
        // Save all inventory data directly
        gameData.currentLevel = CurrentLevel;
        gameData.currentXP = _experience;
        gameData.currentChitin = _chitinCount;
        gameData.currentCrumb = _crumbCount;
        gameData.currentEgg = currentEggCount;
        gameData.maxEggCapacity = maxEggCapacity;
        gameData.hasYetToCollectChitin = hasYetToCollectChitin;
        gameData.hasYetToCollectCrumb = hasYetToCollectCrumb;
        gameData.currentCoin = _coinCount;
        
        Debug.Log($"PlayerInventory directly saved data: Level={CurrentLevel}, XP={_experience}, " +
                  $"Chitin={_chitinCount}, Crumb={_crumbCount}, Egg={currentEggCount}, " +
                  $"MaxEggCapacity={maxEggCapacity}, Coin={_coinCount}");
    }

    // Add this method to PlayerInventory
    public void LoadInventoryData(GameData gameData)
    {
        if (gameData == null) return;
        ...
        // Load all inventory data directly - USING PROPERTIES where possible
        // This is crucial to trigger UI updates
        
        // Use properties for values that have UI events
        ChitinCount = gameData.currentChitin;  // Use property instead of field
        CrumbCount = gameData.currentCrumb;    // Use property instead of field
        
        // For experience, we need to set it and potentially update level
        _experience = gameData.currentXP;
        
        // Set fields that don't have property setters
        currentEggCount = gameData.currentEgg;
        maxEggCapacity = gameData.maxEggCapacity;
        hasYetToCollectChitin = gameData.hasYetToCollectChitin;
        hasYetToCollectCrumb = gameData.hasYetToCollectCrumb;
        CoinCount = gameData.currentCoin;  // Use property instead of field
        ...
        Debug.Log($"PlayerInventory directly loaded data: Level={CurrentLevel}, XP={_experience}, " +
                  $"Chitin={_chitinCount}, Crumb={_crumbCount}, Egg={currentEggCount}, " +
                  $"MaxEggCapacity={maxEggCapacity}, Coin={_coinCount}");
    }

    // Add this comprehensive save method to PlayerInventory
    public void SaveAllInventoryData(GameData gameData)
    {
        if (gameData == null) return;
        ...
        gameData.hasYetToCollectChitin = hasYetToCollectChitin;
        gameData.hasYetToCollectCrumb = hasYetToCollectCrumb;
        gameData.currentCoin = _coinCount;  // Use _coinCount instead of currentCoinCount
        
        Debug.Log($"PlayerInventory directly saved ALL data: Level={CurrentLevel}, XP={_experience}, " +
                  $"Chitin={_chitinCount}, Crumb={_crumbCount}, Egg={currentEggCount}, " +
                  $"MaxEggCapacity={maxEggCapacity}, Coin={_coinCount}");
    }

    // Add this to PlayerInventory.cs
    private void OnValidate()
    {
        // Prevent accidental resets in editor
        if (Application.isPlaying && GameManager.Instance != null && GameManager.Instance.HasCompletedDelayedLoad)
        {
            Debug.LogWarning("Prevented OnValidate from potentially resetting values during play");
            return;
        }
    }
} 