...
public class AllyAI : MonoBehaviour
{
    ...
    private float lootApproachStartTime = 0f;
    private float requiredApproachTimeBeforePickup = 0.5f; // Minimum time in seconds before allowing pickup
    ...
    public void RegisterLootInRange(Transform loot)
    {
        // Don't register loot if we're already carrying something, in combat,
        // chasing an enemy, or on cooldown
        if (carriedLoot != null || isInCombat || isChasing || 
            Time.time < nextLootCollectionTime || detectedLoot != null || 
            currentMode == AIMode.GoingToLoot)
            return;
        
        // Add the loot to a potential targets list instead of immediately processing it
        // The Update method will handle approaching and picking up
        detectedLoot = loot;
        
        // Store the approach start time
        lootApproachStartTime = Time.time;
        
        // Store the previous mode before changing to going-to-loot mode
        previousMode = currentMode;
        SetMode(AIMode.GoingToLoot);
        
        // Enable debugging so we can see what's happening
        Debug.Log($"{gameObject.name} registered loot {loot.name} in range, will approach it");
    }

    private void GoToLootBehavior()
    {
        if (detectedLoot == null)
        {
            Debug.Log($"{gameObject.name} has no detected loot, returning to {previousMode}");
            SetMode(previousMode);
            return;
        }
        
        float distanceToLoot = Vector3.Distance(transform.position, detectedLoot.position);
        bool timeRequirementMet = (Time.time - lootApproachStartTime) >= requiredApproachTimeBeforePickup;
        bool distanceRequirementMet = distanceToLoot <= lootPickupDistance;
        
        Debug.Log($"{gameObject.name} approaching loot: Distance={distanceToLoot:F2}, TimeReq={timeRequirementMet}, DistReq={distanceRequirementMet}");
        
        if (timeRequirementMet && distanceRequirementMet)
        {
            Debug.Log($"{gameObject.name} reached loot and picking up");
            isMoving = false;
            UpdateAnimation(false);
    ...
    public void LootLeftRange(Transform loot)
    {
        ...
        {
            Debug.Log($"{gameObject.name} lost sight of loot {loot.name}");
            detectedLoot = null;
