...
public class AllyAI : MonoBehaviour
{
    ...
    private void HandleEnemyDetected(LivingEntity enemy)
    {
        ...
        if (enemy == null || enemy.gameObject.CompareTag("Player") || enemy.gameObject.CompareTag("Ally"))
            return;
        ...
        if (!enemiesInRange.Contains(enemy))
        {
            ...
            if (showDebugInfo)
                Debug.Log($"Ally {gameObject.name}: Enemy detected - {enemy.name}");
            
            // Start auto-attack if we haven't already
            if (enableAutoAttack && autoAttackCoroutine == null && !isAttacking)
            {
    ...
    private void HandleEnemyLost(LivingEntity enemy)
    {
        ...
        if (enemiesInRange.Contains(enemy))
        {
            ...
            if (showDebugInfo)
                Debug.Log($"Ally {gameObject.name}: Enemy lost - {enemy?.name ?? "null"}");
        }
    }
    ...
    {
        if (showDebugInfo)
            Debug.Log($"Ally {gameObject.name}: Starting auto-attack coroutine");
        
        int failedAttemptsCount = 0;
        
        while (enabled && livingEntity != null && !livingEntity.IsDead)
        {
            // If we have enemies to attack
            if (enemiesInRange.Count > 0)
            {
                ...
                LivingEntity target = FindClosestEnemy();
                ...
                if (target != null)
                {
                    ...
                    if (distanceToEnemy <= attackRange)
                    {
                        ...
                        livingEntity.RotateTowards(directionToEnemy, livingEntity.rotationSpeed * 2f);
                        
                        // Try to attack if cooldown allows
                        if (Time.time - lastAttackAttemptTime > 0.5f && 
                            livingEntity.RemainingAttackCooldown < 0.1f)
                        {
                            // Set flag and record attempt time
                            isAttacking = true;
                            lastAttackAttemptTime = Time.time;
                            
                            // Perform attack
                            bool attackStarted = livingEntity.AttemptAttack();
                            
                            if (attackStarted)
                            {
                                // Attack was successful, wait for cooldown
                                if (showDebugInfo)
                                    Debug.Log($"Ally {gameObject.name}: Attack started successfully");
                            ...
                            {
                                // Attack failed to start
                                failedAttemptsCount++;
                                if (showDebugInfo)
                                    Debug.Log($"Ally {gameObject.name}: Attack failed to start. Attempt #{failedAttemptsCount}");
                        ...
                        {
                            // Still on cooldown, wait a bit
                            yield return new WaitForSeconds(0.2f);
                        }
                    }
                    else
                    {
                        // Too far to attack, reset state
                        isAttacking = false;
                        yield return new WaitForSeconds(0.2f);
                    }
                }
