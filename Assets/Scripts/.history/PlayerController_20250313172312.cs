...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Don't move if dead or in special animation
        ...
        // Update animation state - only change if needed to avoid animation glitches
        if (animController != null)
        {
            bool shouldBeWalking = currentSpeed > 0.1f;
            if (animController.IsAnimationPlaying("walk") != shouldBeWalking)
            {
        ...
        {
            // Calculate rotation speed based on frame time
            float rotationStep = livingEntity.rotationSpeed * Time.deltaTime;
            
            // Calculate target rotation - ensure y-axis alignment
