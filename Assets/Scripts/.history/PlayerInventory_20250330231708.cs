...
public class PlayerInventory : MonoBehaviour
{
    ...
    [SerializeField] public int maxCrumbCapacity = 2;
    ...
    [SerializeField] private int experiencePerChitin = 1;
    ...
    public int Experience
    {
        ...
        private set
        {
            ...
            // Check for level up - skip if we're in revival mode
            if (!IsReviving)
            {
    ...
    { 
        ...
        set 
        {
            int oldValue = currentEggCount;
            currentEggCount = Mathf.Clamp(value, 0, maxEggCapacity);
            
            // If the value changed, update the UI
            if (oldValue != currentEggCount)
                UpdateEggBar(currentEggCount);
        }
    }
    ...
    // Add this property to PlayerInventory.cs
    ...
    // Add this field to cache the UIHelper reference
    private UIHelper cachedUIHelper;
    ...
    private void Start()
    {
        ...
        // Add listeners to update resource bars
        OnChitinCountChanged += UpdateChitinBar;
        OnCrumbCountChanged += UpdateCrumbBar;
        
        // Find and cache UIHelper
        cachedUIHelper = FindObjectOfType<UIHelper>();
        if (cachedUIHelper != null)
        {
            // Initialize bars with current values
            UpdateChitinBar(_chitinCount);
            UpdateCrumbBar(_crumbCount);
            UpdateEggBar(currentEggCount);
        }
    ...
    public bool AddCrumb(int amount = 1, bool playSound = true)
    {
        // Check if this is the first time collection
        if (hasYetToCollectCrumb && amount > 0)
        {
            Debug.Log("Player has yet to collect crumb! Showing info panel...");
            
            // IMPORTANT: Set the flag to false immediately to fix the collection issue
        ...
        // Play pickup sound - restore original sound method
        PlayPickupSound();
    ...
    public bool AddChitin(int amount = 1, bool playSound = true)
    {
        // Check if this is the first time collecting chitin
        if (hasYetToCollectChitin && amount > 0)
        {
            ...
            if (uiHelper != null && uiHelper.ItemInformPanel != null)
            {
                ...
                if (itemInfo != null)
                {
                    ...
                    if (arrowManager != null)
                    {
                        ...
                        // Show persistent inform text about depositing
                        if (uiHelper != null)
                        {
                            uiHelper.ShowInformText("Go to your nest to deposit your chitin.", 6f);
                        }
                    }
                }
            }
        ...
        if (ChitinCount >= maxChitinCapacity)
        {
            ...
            // Show the arrow to the nest when capacity is reached
            ChitinDepositArrowManager arrowManager = FindObjectOfType<ChitinDepositArrowManager>();
            if (arrowManager != null)
            {
                arrowManager.ForceShowArrow();
                Debug.Log("Showing arrow to nest - chitin capacity full");
            }
            ...
            if (uiHelper != null)
            {
                uiHelper.ShowWarningText("Chitin Capacity Reached, Go To Your Nest To deposit.", 6f);
            }
    ...
    public void DepositChitinAtBase(int amount)
    {
        ...
        // Hide the arrow now that player has reached the nest
        ChitinDepositArrowManager arrowManager = FindObjectOfType<ChitinDepositArrowManager>();
        if (arrowManager != null)
        {
        ...
        // IMPORTANT: Clear the persistent nest message after depositing
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            uiHelper.ClearPersistentInformText();
            
            // Show a confirmation message
            uiHelper.ShowInformText($"Deposited {amount} chitin at your nest!");
        }
    ...
    // Modify the PlayLevelUpSound method to ensure it works properly
    private void PlayLevelUpSound()
    {
        Debug.Log("<color=red>PlayLevelUpSound called - attempting to play level up sound effect</color>");
        
        // First try using SoundEffectManager
        if (useSoundEffectManager && SoundEffectManager.Instance != null)
        {
            // Debug if sound exists in manager
            bool hasSoundEffect = SoundEffectManager.Instance.HasSoundEffect(levelUpSoundEffectName);
            Debug.Log($"SoundEffectManager has '{levelUpSoundEffectName}' sound: {hasSoundEffect}");
            
            if (hasSoundEffect)
            {
                // Force higher volume and clear pitch
                SoundEffectManager.Instance.PlaySoundWithPitch(levelUpSoundEffectName, 1.0f);
                Debug.Log($"<color=green>Level up sound '{levelUpSoundEffectName}' played with SoundEffectManager</color>");
                return;
            }
            else
            {
                Debug.LogError($"<color=red>Level up sound '{levelUpSoundEffectName}' NOT FOUND in SoundEffectManager!</color>");
            }
        }
        ...
        if (audioSource != null && levelUpSound != null)
        {
            ...
            Debug.Log("<color=green>Level up sound played using direct AudioSource</color>");
        }
        else
        {
            Debug.LogError("<color=red>Failed to play level up sound: AudioSource or levelUpSound clip is null</color>");
        }
    }
    ...
    public void ForceUpdateUIUpdateCrumbCapacity()
    {
        if (OnCrumbCountChanged != null)
            OnCrumbCountChanged(CrumbCount);
    }

    public void ForceUpdateUIUpdateChitinCapacity()
    {
        // Trigger UI update events
        if (OnChitinCountChanged != null)
            OnChitinCountChanged(ChitinCount);
    ...
    public bool RemoveCoins(int amount)
    {
        ...
        {
            Debug.LogWarning($"Not enough coins to remove {amount}. Current: {currentCoinCount}");
            return false;
        }
        ...
        Debug.Log($"Removed {amount} coins. New total: {currentCoinCount}");
        return true;
    }
    ...
    private void HandleLevelUpSequence(int newLevel)
    {
        ...
        // Show level up panel with rewards if not loading saved data
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null && uiHelper.LevelUpPanel != null)
        {
            // Special handling for level 2 - show attributes panel indicator
            if (newLevel == 2)
            {
                // Activate the attribute panel button pointing pulsating image
                if (uiHelper.attributePanelButtonPointingPulsatingImage != null)
                {
                    uiHelper.attributePanelButtonPointingPulsatingImage.SetActive(true);
                    Debug.Log("Player reached level 2 - Activated attribute panel indicator");
                }
                else
                {
                    Debug.LogWarning("attributePanelButtonPointingPulsatingImage is null in UIHelper");
                }
            }
            
            // Continue with existing level up reward logic
            // (existing code)
        }
    ...
    private void OnLevelChanged(int newLevel)
    {
        ...
        // Add level 2 check for attribute panel indicator
        if (newLevel == 2 && !isLoadingData)
        {
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null && uiHelper.attributePanelButtonPointingPulsatingImage != null)
            {
                uiHelper.attributePanelButtonPointingPulsatingImage.SetActive(true);
                Debug.Log("Player reached level 2 - Activated attribute panel indicator");
            }
    ...
    // Add these methods to update the UI when inventory changes
    private void UpdateChitinBar(int chitinCount)
    {
        if (cachedUIHelper == null)
            cachedUIHelper = FindObjectOfType<UIHelper>();
        
        if (cachedUIHelper != null)
            cachedUIHelper.UpdateChitinBar(chitinCount, maxChitinCapacity);
    }

    private void UpdateCrumbBar(int crumbCount)
    {
        if (cachedUIHelper == null)
            cachedUIHelper = FindObjectOfType<UIHelper>();
        
        if (cachedUIHelper != null)
            cachedUIHelper.UpdateCrumbBar(crumbCount, maxCrumbCapacity);
    }

    private void UpdateEggBar(int eggCount)
    {
        if (cachedUIHelper == null)
            cachedUIHelper = FindObjectOfType<UIHelper>();
        
        if (cachedUIHelper != null)
            cachedUIHelper.UpdateEggBar(eggCount, MaxEggCapacity);
    }

    // Add these public methods to access the first-time collection flags
    public bool HasYetToCollectCrumb()
    {
        return hasYetToCollectCrumb;
    }

    public void SetHasCollectedCrumb(bool hasCollected)
    {
        hasYetToCollectCrumb = !hasCollected;
        
        // Save this state change
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null && autoSaveEnabled)
        {
            gameManager.SaveGame();
        }
    }
