...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private RectTransform canvasRectTransform;  // Reference to the main canvas rect
    [SerializeField] private GameObject uiChitinIconPrefab;      // 2D UI version of chitin for canvas
    [SerializeField] private GameObject uiXPIconPrefab;          // 2D UI version of XP for canvas
    ...
    public GameObject canvasSafeArea;
    ...
    private void InitializeUIElementPools()
    {
        // Clear any existing UI element pools to ensure a fresh start
        uiElementPools = new Dictionary<string, Queue<GameObject>>();
        
        // Ensure we have a canvas safe area
        if (canvasSafeArea == null)
        {
            Debug.LogError("Canvas Safe Area is missing! UI animations will not work.", this);
            return;
        }
        
        // Make sure any existing pools are deleted
        Transform existingChitinPool = canvasSafeArea.transform.Find("UIChitinPool");
        if (existingChitinPool != null)
        {
            DestroyImmediate(existingChitinPool.gameObject);
        }
        
        Transform existingXPPool = canvasSafeArea.transform.Find("UIXPPool");
        if (existingXPPool != null) 
        {
            DestroyImmediate(existingXPPool.gameObject);
        }
        
        // Create Chitin UI pool
        if (uiChitinIconPrefab != null)
        {
            // Create container directly under canvasSafeArea
            GameObject container = new GameObject("UIChitinPool");
            container.transform.SetParent(canvasSafeArea.transform, false);
            
            // Ensure container has RectTransform
            if (!container.GetComponent<RectTransform>())
            {
                container.AddComponent<RectTransform>();
            }
            
            Queue<GameObject> pool = new Queue<GameObject>();
            uiElementPools["uiChitin"] = pool;
            
            // Create pool items
            for (int i = 0; i < 10; i++)
            {
                GameObject icon = Instantiate(uiChitinIconPrefab);
                icon.transform.SetParent(container.transform, false);
                
                // Ensure icon has proper RectTransform settings
            ...
            Debug.Log($"Created UIChitinPool with {pool.Count} items as child of {canvasSafeArea.name}");
        }
        
        // Create XP UI pool
        if (uiXPIconPrefab != null)
        {
            // Create container directly under canvasSafeArea
            GameObject container = new GameObject("UIXPPool");
            container.transform.SetParent(canvasSafeArea.transform, false);
            ...
            Queue<GameObject> pool = new Queue<GameObject>();
            uiElementPools["uiXP"] = pool;
            
            // Create pool items
            for (int i = 0; i < 10; i++)
            {
                GameObject icon = Instantiate(uiXPIconPrefab);
                icon.transform.SetParent(container.transform, false);
            ...
            Debug.Log($"Created UIXPPool with {pool.Count} items as child of {canvasSafeArea.name}");
        }
    }
    ...
    // Add this method to handle the new chitin collection animation
    public void PlayNewChitinCollectionAnimation(int chitinAmount, int xpAmount)
    {
        if (chitinPanel == null || xpPanel == null || canvasSafeArea == null)
        {
            Debug.LogError("Missing panel references for chitin animation. Check inspector.");
            return;
        }

        StartCoroutine(AnimateChitinToXPSequence(chitinAmount, xpAmount));
    }

    // Coroutine to handle the animation sequence
    private IEnumerator AnimateChitinToXPSequence(int chitinAmount, int xpAmount)
    {
        ...
        RectTransform safeAreaRect = canvasSafeArea.GetComponent<RectTransform>();
        
        if (chitinPanelRect == null || xpPanelRect == null || safeAreaRect == null)
        {
            Debug.LogError("Missing RectTransform components on panels");
            yield break;
        }
        
        // Get screen positions
        Vector3 chitinPanelPos = GetWorldPositionFromUI(chitinPanelRect);
        Vector3 xpPanelPos = GetWorldPositionFromUI(xpPanelRect);
        
        // Calculate screen center position
        Vector3 screenCenterPos = Camera.main.ViewportToWorldPoint(new Vector3(0.5f, 0.5f, 10f));
        screenCenterPos.z = -1f; // Ensure it's visible
        
        // Calculate number of symbols to spawn
        int chitinSymbolCount = Mathf.Min(5, chitinAmount); // Cap at 5 symbols regardless of amount
        int xpSymbolCount = Mathf.Min(5, xpAmount);         // Cap at 5 symbols for XP too
        
        // First phase: Spawn chitin symbols and animate to center
        List<GameObject> chitinSymbols = new List<GameObject>();
        
        // Play collection sound
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("ChitinCollected");
        }
        
        // Animate chitin symbols from panel to center
        for (int i = 0; i < chitinSymbolCount; i++)
        {
            // Get chitin symbol from pool
            GameObject chitinSymbol = GetSymbolFromPool("chitin", chitinSymbolPrefab);
            chitinSymbols.Add(chitinSymbol);
            
            // Set initial position with small random offset
            Vector3 startPos = chitinPanelPos + new Vector3(
                Random.Range(-0.5f, 0.5f),
                Random.Range(-0.5f, 0.5f),
                0f
            );
            
            chitinSymbol.transform.position = startPos;
            chitinSymbol.transform.localScale = chitinScale;
            chitinSymbol.SetActive(true);
            
            // Start animation to center
            StartCoroutine(AnimateSymbolToPosition(
                chitinSymbol, 
                startPos, 
                screenCenterPos, 
                1.0f, // Duration
        ...
        // Wait for all chitin symbols to reach center
        yield return new WaitForSeconds(1.0f);
        
        // Return chitin symbols to the pool
        foreach (GameObject symbol in chitinSymbols)
        {
            symbol.SetActive(false);
            symbolPools["chitin"].Enqueue(symbol);
        }
        
        // Second phase: Spawn XP symbols at center and animate to XP panel
        ...
        // Animate XP symbols from center to XP panel
        for (int i = 0; i < xpSymbolCount; i++)
        {
            // Get XP symbol from pool
            GameObject xpSymbol = GetSymbolFromPool("xp", xpSymbolPrefab);
            
            // Set initial position with small random offset
            Vector3 startPos = screenCenterPos + new Vector3(
                Random.Range(-0.5f, 0.5f),
                Random.Range(-0.5f, 0.5f),
                0f
            );
            
            xpSymbol.transform.position = startPos;
            xpSymbol.transform.localScale = baseScale * 0.4f; // Bigger scale for XP
            xpSymbol.SetActive(true);
            
            // Start animation to XP panel
            StartCoroutine(AnimateSymbolToPosition(
                xpSymbol, 
                startPos, 
                xpPanelPos, 
    ...
    // Helper method to animate a symbol from one position to another
    private IEnumerator AnimateSymbolToPosition(
        GameObject symbol, 
        Vector3 startPos, 
        Vector3 endPos, 
        float duration, 
        ...
        bool fadeOut = false)
    {
        ...
        if (renderer == null)
        {
            Debug.LogError("Symbol doesn't have a SpriteRenderer component");
            yield break;
        }
        ...
        while (elapsed < duration)
        {
            ...
            // Optional: Add some bobbing
            symbol.transform.position += new Vector3(0, Mathf.Sin(normalizedTime * Mathf.PI * 2) * 0.3f, 0);
            
            // Fade out at the end if requested
            if (fadeOut && normalizedTime > 0.7f)
            {
                float alpha = 1f - ((normalizedTime - 0.7f) / 0.3f);
        ...
        symbol.transform.position = endPos;
        
        // If we're supposed to fade out, make sure we're fully faded
        if (fadeOut)
        {
            renderer.color = new Color(originalColor.r, originalColor.g, originalColor.b, 0);
            
            // Small delay before deactivating
            yield return new WaitForSeconds(0.1f);
            ...
            if (symbol.name.Contains("XP"))
            {
                symbolPools["xp"].Enqueue(symbol);
            }
            else if (symbol.name.Contains("Chitin"))
            {
                symbolPools["chitin"].Enqueue(symbol);
            }
        }
    ...
    // Helper method to get UI element from pool
    private GameObject GetUIElementFromPool(string type)
    {
        // Validate inputs
        if (canvasSafeArea == null)
        {
            Debug.LogError("Canvas Safe Area is null!", this);
            return null;
        }
        
        if (!uiElementPools.ContainsKey(type) || uiElementPools[type].Count == 0)
        {
            // We need to create a new element - get the proper prefab
            GameObject prefab = type == "uiChitin" ? uiChitinIconPrefab : uiXPIconPrefab;
            if (prefab == null)
            {
                Debug.LogError($"No prefab for UI element type: {type}", this);
                return null;
            }
            
            // Find or create the container
            string containerName = type == "uiChitin" ? "UIChitinPool" : "UIXPPool";
            Transform container = canvasSafeArea.transform.Find(containerName);
            ...
            if (container == null)
            {
                ...
                containerObj.transform.SetParent(canvasSafeArea.transform, false);
                container = containerObj.transform;
                
                Debug.Log($"Created new container: {containerName} under {canvasSafeArea.name}");
            }
            ...
            Debug.Log($"Created new UI element for pool {type} under {container.name}");
            return newElement;
        }
        
        // Get from pool
        return uiElementPools[type].Dequeue();
    }

    // Return UI element to pool
    private void ReturnUIElementToPool(GameObject element, string type)
    {
        if (element != null)
        {
            element.SetActive(false);
            if (uiElementPools.ContainsKey(type))
            {
                uiElementPools[type].Enqueue(element);
            }
        }
    }

    // New method for canvas-based animation
    public void PlayCanvasChitinAnimation(int chitinAmount, int xpAmount)
    {
        if (chitinPanel == null || xpPanel == null || canvasSafeArea == null)
        {
            Debug.LogError("Missing panel references for chitin animation");
            return;
        }
        
        StartCoroutine(AnimateCanvasChitinToXP(chitinAmount, xpAmount));
    }

    // Canvas-based animation coroutine
    private IEnumerator AnimateCanvasChitinToXP(int chitinAmount, int xpAmount)
    {
        ...
        {
            Debug.LogError("Missing RectTransform components");
            yield break;
        }
        
        // Get screen center position in canvas space
        Vector2 screenCenter = new Vector2(Screen.width / 2f, Screen.height / 2f);
        
        // Calculate number of symbols (cap at 5)
        int chitinSymbolCount = Mathf.Min(5, chitinAmount);
        int xpSymbolCount = Mathf.Min(5, xpAmount);
        
        // First phase: Animate chitin symbols to center
        List<GameObject> chitinIcons = new List<GameObject>();
        ...
        // Get world-space position of chitin panel in canvas
        Vector3[] corners = new Vector3[4];
        chitinPanelRect.GetWorldCorners(corners);
        Vector3 chitinPanelPos = (corners[0] + corners[2]) / 2f; // Center of the panel
        
        // Spawn and animate chitin icons
        for (int i = 0; i < chitinSymbolCount; i++)
        {
            GameObject chitinIcon = GetUIElementFromPool("uiChitin");
            if (chitinIcon != null)
            {
                chitinIcons.Add(chitinIcon);
                
                // Set up RectTransform
                RectTransform iconRect = chitinIcon.GetComponent<RectTransform>();
                if (iconRect != null)
                {
                    // Position near chitin panel with random offset
                    Vector2 startPos = new Vector2(
                        chitinPanelRect.position.x + Random.Range(-20f, 20f),
                        chitinPanelRect.position.y + Random.Range(-20f, 20f)
                    );
                    
                    iconRect.position = startPos;
                    chitinIcon.SetActive(true);
        ...
        // Return chitin icons to pool
        foreach (GameObject icon in chitinIcons)
        {
            ReturnUIElementToPool(icon, "uiChitin");
        }
        
        // Second phase: Spawn XP icons at center and animate to XP panel
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("XPGained");
        }
        
        // Get world-space position of XP panel
        xpPanelRect.GetWorldCorners(corners);
        Vector3 xpPanelPos = (corners[0] + corners[2]) / 2f; // Center of the panel
        
        // Spawn and animate XP icons
        for (int i = 0; i < xpSymbolCount; i++)
        {
            GameObject xpIcon = GetUIElementFromPool("uiXP");
            if (xpIcon != null)
            {
                // Set up RectTransform
                RectTransform iconRect = xpIcon.GetComponent<RectTransform>();
                if (iconRect != null)
                {
                    // Position at center with random offset
                    Vector2 startPos = new Vector2(
                        screenCenter.x + Random.Range(-30f, 30f),
                        screenCenter.y + Random.Range(-30f, 30f)
                    );
                    
                    iconRect.position = startPos;
                    xpIcon.SetActive(true);
                    
                    // Get image component for fading
                    Image iconImage = xpIcon.GetComponent<Image>();
                    
                    // Animate to XP panel
                    StartCoroutine(AnimateRectTransformWithFade(
                        iconRect,
                        iconImage,
                        startPos,
                        new Vector2(xpPanelPos.x, xpPanelPos.y),
                        1.0f,
                        i * 0.1f,
                        "uiXP"
                    ));
    ...
    // Animate RectTransform from one position to another
    private IEnumerator AnimateRectTransform(
        ...
        float delay = 0f)
    {
        ...
        while (elapsed < duration)
        {
            ...
            // Update position
            rectTransform.position = Vector2.Lerp(startPos, endPos, easedT);
            
            // Add slight bobbing
            float bob = Mathf.Sin(t * Mathf.PI * 2) * 10f;
            rectTransform.position += new Vector3(0, bob, 0);
    ...
    // Animate RectTransform with fade out
    private IEnumerator AnimateRectTransformWithFade(
        RectTransform rectTransform,
        Image image,
        Vector2 startPos,
        Vector2 endPos,
        ...
        string poolType = "")
    {
        ...
        while (elapsed < duration)
        {
            ...
            if (t > 0.7f)
            {
                float fadeAlpha = Mathf.Lerp(originalAlpha, 0f, (t - 0.7f) / 0.3f);
                image.color = new Color(originalColor.r, originalColor.g, originalColor.b, fadeAlpha);
            }
            
            yield return null;
        }
        
        // Return to pool if specified
        if (!string.IsNullOrEmpty(poolType))
        {
            // Reset alpha for reuse
            image.color = originalColor;
            ReturnUIElementToPool(rectTransform.gameObject, poolType);
        }
    }
} 