    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            else
            {
                ...
                // Direction from enemy to closest point (back toward inside)
            ...
            // Move back inside by a stronger amount to ensure we're fully inside the boundary
            ...
            // Set rotation to face back inward
            transform.rotation = Quaternion.LookRotation(inwardDirection);
            
            // If we have a wandering behavior, tell it to stop and wait
            if (wanderingBehavior != null)
            {
                ...
                // Schedule a new waypoint that's definitely inside the boundary
                StartCoroutine(SetNewSafeWaypointAfterDelay(inwardDirection));
            }
            
            // Temporarily stop any chase behavior
            if (currentTarget != null)
            {
                // Store the target but temporarily stop pursuing
                StartCoroutine(ResumeChasingAfterRedirection());
            }
        }
    }

    // New method to set a safer waypoint after a delay
    private IEnumerator SetNewSafeWaypointAfterDelay(Vector3 safeDirection)
    {
        // Wait a short time before setting the new waypoint
        yield return new WaitForSeconds(1.0f);
        
        // Only set new waypoint if we're still in boundary redirection mode
        if (isHittingBoundary && wanderingBehavior != null)
        {
            ...
            // Set a new waypoint in the safe direction with a sufficient distance
            wanderingBehavior.ForceNewWaypointInDirection(randomizedDirection);
