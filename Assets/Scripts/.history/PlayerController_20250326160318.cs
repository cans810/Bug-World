...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Setup player scale and camera settings based on level
        StartCoroutine(SetupPlayerAndCameraBasedOnLevel());
    }
    ...
    {
        // Reset border state when exiting barrier rocks
        if (other.gameObject.layer == LayerMask.NameToLayer("BarrierRocks"))
        {
            isAtMapBorder = false;
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (borderName == "MapBorder1" || borderName == "MapBorder8" || borderName == "MapBorder14")
            {
                isAtMapBorder = false;
                borderNormal = Vector3.zero;
                return;
            ...
            if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
            {
                ...
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText("Playable area reached!");
                }
        ...
        else if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // Reset border state when exiting the BigBorder
            isAtMapBorder = false;
            borderNormal = Vector3.zero;
        }
    }
    ...
    {
        // First, check if this is a barrier rock
        if (other.gameObject.layer == LayerMask.NameToLayer("BarrierRocks"))
        {
            // Set flags to prevent movement into the barrier
            isAtMapBorder = true;

            // Calculate the normal pointing away from the barrier
            Vector3 playerToColliderCenter = transform.position - other.bounds.center;
            borderNormal = playerToColliderCenter.normalized;
            
            // Apply a strong force to push player away from barrier
            if (livingEntity != null)
            {
                livingEntity.MoveInDirection(borderNormal, 0.3f);
            }
            else
            {
                transform.position += borderNormal * 0.3f;
            }
            
            // Optionally show a message about the barrier
            if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
            {
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText("This path is blocked by rocks! You need to reach level 31 to pass.");
                }
        ...
        // Original code for MapBorder layer check
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            string borderName = other.gameObject.name;
    ...
    // Modify the OnTriggerStay method to maintain level restrictions
    private void OnTriggerStay(Collider other)
    {
        // Check for barrier rocks first
        if (other.gameObject.layer == LayerMask.NameToLayer("BarrierRocks"))
        {
            // Maintain the border state
            isAtMapBorder = true;
            
            // Keep calculating the normal to always point away from the barrier
            Vector3 playerToColliderCenter = transform.position - other.bounds.center;
            borderNormal = playerToColliderCenter.normalized;
            
            // Apply a constant small force to push player away from barrier
            if (livingEntity != null && moveDirection.magnitude > 0.1f)
            {
                // Check if player is trying to move into the barrier
                float movementIntoBorder = Vector3.Dot(moveDirection, -borderNormal);
                if (movementIntoBorder > 0.2f)
                {
                    // Apply a counterforce
                    livingEntity.MoveInDirection(borderNormal, 0.2f);
                }
            }
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // If player is trying to move away from the border
            if (moveDirection.magnitude > 0.1f)
            {
                // Check if player is trying to move back toward playable area
                float movementWithBorder = Vector3.Dot(moveDirection, borderNormal);
                
                // If moving more into the playable area (positive dot product with borderNormal)
                if (movementWithBorder > 0.2f)
                {
                    // Allow movement back into the playable area
                    isAtMapBorder = false;
                }
                else
                {
                    // Update the borderNormal to ensure it points inward
                    Vector3 closestPoint = other.ClosestPoint(transform.position);
                    borderNormal = (closestPoint - transform.position).normalized;
                    isAtMapBorder = true;
                }
        ...
        else if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Check if this is a level-restricted border
            string borderName = other.gameObject.name;
            if (borderName == "MapBorder2" || borderName == "MapBorder3" || 
                borderName == "MapBorder4" || borderName == "MapBorder5" ||
                borderName == "MapBorder6" || borderName == "MapBorder7" ||
                borderName == "MapBorder9" || borderName == "MapBorder10" ||
                borderName == "MapBorder11" || borderName == "MapBorder12" ||
                borderName == "MapBorder13")
            {
                ...
                // If player isn't high enough level, enforce border restriction
                if (playerLevel < requiredLevel)
                {
                    // Calculate the normal pointing away from the border
                    Vector3 playerToColliderCenter = transform.position - other.bounds.center;
                    borderNormal = playerToColliderCenter.normalized;
                    
                    // Set flag to prevent movement into this area
                    isAtMapBorder = true;
    ...
    public void CheckMetamorphosis(int newLevel)
    {
        ...
        Debug.Log($"Checking metamorphosis for level {newLevel}. Current stage: {currentMetamorphosisStage}");
        
        // Check if this level is a metamorphosis level
        bool foundMatch = false;
        for (int i = 0; i < metamorphosisLevels.Length; i++)
        {
            if (newLevel == metamorphosisLevels[i])
            {
                foundMatch = true;
                ...
                Debug.Log($"<color=yellow>METAMORPHOSIS TRIGGERED: Level {newLevel} reached! New scale: {cumulativeScale}x (Stage {currentMetamorphosisStage})</color>");
                
                // Update camera settings
                UpdateCameraSettings(newLevel);
                break;
            }
        }
        ...
        if (!foundMatch)
        {
            Debug.Log($"Level {newLevel} is not a metamorphosis level. Available levels: {string.Join(", ", metamorphosisLevels)}");
        }
    }
    ...
    // Update the PlayMetamorphosisEffect method to include visible arrows
    private void PlayMetamorphosisEffect()
    {
        // Add visual effects for metamorphosis here
        VisualEffectManager vfxManager = FindObjectOfType<VisualEffectManager>();
        if (vfxManager != null)
        {
            // You can add a specific metamorphosis effect in VisualEffectManager
            // vfxManager.PlayMetamorphosisEffect(transform.position);
        }
        ...
        // Show metamorphosis arrows to indicate growth
        ShowMetamorphosisArrows();
        
        // Trigger a camera shake effect
        CameraController cameraController = FindObjectOfType<CameraController>();
        if (cameraController != null)
        {
            // Add a small camera shake if the controller has the method
        ...
        // Also potentially trigger a UI notification about metamorphosis
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            uiHelper.ShowInformText("Metamorphosis: Your ant has grown stronger!");
        }
    }

    // New method to show arrows during metamorphosis
    private void ShowMetamorphosisArrows()
    {
        // Create a parent for all the arrows
        GameObject arrowsParent = new GameObject("MetamorphosisArrows");
        
        // Create a screen center arrow
        ScreenCenterArrowIndicator centerArrow = arrowsParent.AddComponent<ScreenCenterArrowIndicator>();
        centerArrow.SetTarget(transform); // Point to the player
        centerArrow.SetArrowColor(Color.green); // Green for growth
        
        // Modify the center arrow properties for better visibility based on player scale
        var sizeField = typeof(ScreenCenterArrowIndicator).GetField("arrowSize", 
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
        if (sizeField != null)
            sizeField.SetValue(centerArrow, 0.05f + (transform.localScale.x * 0.015f)); // Size based on player scale
        
        var distanceField = typeof(ScreenCenterArrowIndicator).GetField("distanceFromCamera", 
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
        if (distanceField != null)
            distanceField.SetValue(centerArrow, 0.5f + (transform.localScale.x * 0.05f)); // Distance from camera based on player scale
        
        // Create multiple arrows in circle pattern around player
        CreateCircularArrows(arrowsParent);
        
        // Show the center arrow
        centerArrow.ShowArrow();
        
        // Destroy the arrows after 3 seconds
        Destroy(arrowsParent, 3f);
        
        // Debug to confirm arrow creation
        Debug.Log($"Created metamorphosis arrows scaled to player size: {transform.localScale}");
    }

    // Helper method to create circular pattern of arrows that scale with the player
    private void CreateCircularArrows(GameObject parent)
    {
        int arrowCount = 6;
        // Scale radius based on player size
        ...
        for (int i = 0; i < arrowCount; i++)
        {
            ...
            Vector3 position = transform.position + new Vector3(
                ...
                transform.localScale.y * 0.4f, // Height proportional to player size
                Mathf.Sin(angle) * radius
            );
            
            // Create a game object for this arrow
            GameObject arrowObj = new GameObject($"MetamorphosisArrow_{i}");
            arrowObj.transform.parent = parent.transform;
            
            // Add a player following arrow indicator
            PlayerFollowingArrowIndicator arrow = arrowObj.AddComponent<PlayerFollowingArrowIndicator>();
            arrow.SetTarget(transform); // Point to the player
            
            // Set arrow properties
            arrow.SetArrowColor(new Color(0.3f, 0.8f, 0.3f)); // Light green
            
            // Use reflection to set private serialized fields for better visibility
            // Scale arrow size with player size
            var sizeField = typeof(PlayerFollowingArrowIndicator).GetField("arrowSize", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (sizeField != null)
                sizeField.SetValue(arrow, transform.localScale.x * 0.5f); // Size relative to player
                
            var heightField = typeof(PlayerFollowingArrowIndicator).GetField("floatHeight", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (heightField != null)
                heightField.SetValue(arrow, transform.localScale.y * 1.2f); // Height relative to player
            
            // Make sure arrows are properly visible from the camera by adjusting their positions
            // This will keep them in view regardless of camera angle
            
            // Get the angle to camera
            Vector3 dirToCamera = (mainCamera.transform.position - position).normalized;
            // Move the arrow position slightly toward the camera
            position += dirToCamera * (transform.localScale.x * 0.5f);
        ...
        // Also adjust the center arrow in the same method
        ScreenCenterArrowIndicator centerArrow = parent.GetComponent<ScreenCenterArrowIndicator>();
        if (centerArrow != null)
        {
            var centerSizeField = typeof(ScreenCenterArrowIndicator).GetField("arrowSize", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (centerSizeField != null)
                centerSizeField.SetValue(centerArrow, 0.05f + (transform.localScale.x * 0.015f)); // Size relative to player
        }
    }

    // Add this coroutine to ensure player scale and camera are set correctly
    private IEnumerator SetupPlayerAndCameraBasedOnLevel()
    {
        // Wait a frame to ensure all components are initialized
        yield return null;
        
        // Try to find playerInventory again if it's still null
        if (playerInventory == null)
        {
            playerInventory = GetComponent<PlayerInventory>();
            if (playerInventory == null)
        ...
        // If we found the inventory, set up scale and camera
        if (playerInventory != null)
        {
            int currentLevel = playerInventory.CurrentLevel;
            Debug.Log($"Setting up player scale and camera for level: {currentLevel}");
            
            // Apply metamorphosis scale if enabled
            if (enableMetamorphosis)
            {
                ApplyMetamorphosisScale(currentLevel);
            }
            else
            {
                // Even if metamorphosis is disabled, we should still set the camera
                Debug.Log("Metamorphosis disabled, but still updating camera settings");
                UpdateCameraSettings(currentLevel);
            }
            
            // Unsubscribe first to avoid double subscription
            playerInventory.OnLevelChanged -= OnPlayerLevelChanged;
            
            // Subscribe to level changed event
            playerInventory.OnLevelChanged += OnPlayerLevelChanged;
            Debug.Log("Subscribed to OnLevelChanged event");
        }
        else
        {
            Debug.LogError("Failed to find PlayerInventory after retry. Camera and player scale may not be set correctly.");
        }
        
        // Final verification of camera settings
        yield return new WaitForSeconds(0.2f); // Short delay to ensure camera controller is ready
        ...
        if (playerInventory != null)
        {
            ...
            if (cameraController != null)
            {
                Debug.Log($"Verifying camera settings for level: {playerInventory.CurrentLevel}");
                // Force update camera settings one more time to be absolutely sure
                UpdateCameraSettings(playerInventory.CurrentLevel);
            }
        }
    }

    // Add a new handler for the OnLevelChanged event
    private void OnPlayerLevelChanged(int newLevel)
    {
        Debug.Log($"OnPlayerLevelChanged triggered with level {newLevel}");
    ...
    // Add a public method to force update the player scale and camera settings
    public void ForceUpdatePlayerAndCameraSettings()
    {
        if (playerInventory != null)
        {
            int currentLevel = playerInventory.CurrentLevel;
            Debug.Log($"Force updating player scale and camera for level: {currentLevel}");
            ...
            {
                // Even if metamorphosis is disabled, still update camera
                UpdateCameraSettings(currentLevel);
            }
        }
        else
        {
            Debug.LogError("Cannot force update settings - PlayerInventory is null");
        }
    }
}