...
public class PlayerController : MonoBehaviour
{
    ...
    // Add this dictionary to store border level requirements
    private Dictionary<string, int> borderLevelRequirements = new Dictionary<string, int>
    {
        { "MapBorder1", 0 },  // Always allow passage
        { "MapBorder2", 3 },  // Requires level 3
        { "MapBorder3", 4 },  // Requires level 4
        { "MapBorder4", 5 },  // Requires level 5
        { "MapBorder5", 6 }   // Requires level 6
    };
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            int playerLevel = 1; // Default to level 1
            PlayerInventory inventory = GetComponent<PlayerInventory>();
            if (inventory != null)
            {
                playerLevel = inventory.CurrentLevel;
            }
            
            // Check if this border has level requirements
            if (borderLevelRequirements.TryGetValue(borderName, out int requiredLevel))
            {
                // For MapBorder1 or if player meets the level requirement, allow passage
                if (borderName == "MapBorder1" || playerLevel >= requiredLevel)
                {
                    // Allow player to pass
                    isAtMapBorder = false;
                    borderNormal = Vector3.zero;
                    
                    // Show passage message if needed
                    if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
                    {
                        if (uiHelper != null && borderName != "MapBorder1")
                        {
                            uiHelper.ShowInformText($"Entering level {requiredLevel} area!");
                        }
                ...
                // Level requirement not met, block player
                isAtMapBorder = true;
                ...
                if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
                {
                    // Display UI message about level requirement
                    if (uiHelper != null)
                    {
            ...
            {
                // Default behavior for unrecognized borders (fallback to original behavior)
                isAtMapBorder = true;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (borderLevelRequirements.TryGetValue(borderName, out int requiredLevel))
            {
                ...
                if (borderName == "MapBorder1" || playerLevel >= requiredLevel)
                {
                    // Allow player to enter the area
                    isAtMapBorder = false;
                    borderNormal = Vector3.zero;
                    
                    // Show entry message if needed
                    if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
                    {
                        if (uiHelper != null && borderName != "MapBorder1")
                        {
                ...
                {
                    // Level requirement not met, prevent entry
                    isAtMapBorder = true;
                    
                    // Calculate repulsion direction - away from the border center
                    if (other is SphereCollider sphereCollider)
                    {
                        // Get the center of the sphere in world space
                        Vector3 sphereCenter = other.transform.TransformPoint(sphereCollider.center);
                        // Calculate direction from sphere center to player (outward)
                    ...
                    else
                    {
                        // For non-sphere colliders, try to get closest point on the collider
                        Vector3 closestPoint = other.ClosestPoint(transform.position);
                        // Direction from closest point to player (outward)
                        borderNormal = (transform.position - closestPoint).normalized;
                    }
                    
                    // Immediately push the player away from this boundary
                    if (livingEntity != null)
                    {
                        livingEntity.MoveInDirection(borderNormal, 0.5f);
                    }
            ...
            {
                // Default behavior for unrecognized borders
                isAtMapBorder = false;
                borderNormal = Vector3.zero;
            }
        }
