...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnXPSymbols(Vector3 nestPosition, int count, int totalXPAmount)
    {
        ...
        // Track if this is the first symbol (to start XP increment)
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Pass the isFirstSymbol flag to the animation coroutine
            StartCoroutine(AnimateXPSymbolToScreen(symbol, screenTopLeft, isFirstSymbol ? xpPerSymbol : 0));
            
            // After the first symbol, all others will have 0 XP value for increment
            // This ensures only the first symbol triggers the XP increment
            isFirstSymbol = false;
        ...
        // After all symbols are spawned, add the remaining XP at once
        // (but only if we're not doing one-by-one increments)
        if (!incrementXPOneByOne)
        {
            // Find the player inventory and add XP all at once
            PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
            if (playerInventory != null)
            {
                // Add all XP at once after a delay to let animations finish
                StartCoroutine(AddRemainingXPAfterDelay(playerInventory, totalXPAmount, xpFlyDuration + 0.2f));
            }
        }
    }

    // New helper method to add remaining XP after a delay
    private IEnumerator AddRemainingXPAfterDelay(PlayerInventory inventory, int xpAmount, float delay)
    {
        yield return new WaitForSeconds(delay);
        inventory.AddXPIncremental(xpAmount);
    }
