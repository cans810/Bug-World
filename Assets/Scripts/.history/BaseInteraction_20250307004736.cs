...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (currentCrumbs > 0)
        {
            // Try to create an egg with the crumbs
            if (antIncubator != null)
            {
                // Check if there are enough crumbs to create an egg
                if (currentCrumbs >= antIncubator.GetRequiredCrumbs())
                {
                    bool eggCreated = antIncubator.ProcessCrumbDeposit(currentCrumbs);
                    
                    if (eggCreated)
                    {
                        // Only deposit the exact number of crumbs required
                        ...
                        if (showDebugMessages)
                        {
                            Debug.Log($"Used {antIncubator.GetRequiredCrumbs()} crumbs to create an ant egg!");
                        }
                    }
                    else
                    {
                        // Egg creation failed for some other reason (like max eggs reached)
                        // Don't take the crumbs from the player
                        if (showDebugMessages)
                        {
                            Debug.Log("Couldn't create an egg, keeping the crumbs in inventory.");
                        }
                    }
                }
                else
                {
                    // Not enough crumbs, inform the player but don't remove anything
                    antIncubator.ProcessCrumbDeposit(currentCrumbs); // This will show the "need more crumbs" message
                    
                    if (showDebugMessages)
                    {
                        Debug.Log($"Not enough crumbs ({currentCrumbs}/{antIncubator.GetRequiredCrumbs()}) to create an egg.");
                    }
                }
            }
        }
