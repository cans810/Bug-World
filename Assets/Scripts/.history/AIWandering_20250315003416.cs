...
public class AIWandering : MonoBehaviour
{
    ...
    // Force start wandering but maintain current rotation
    public void ForceStartWithCurrentRotation()
    {
        // Stop any current routines
        StopAllCoroutines();
        ...
        StartCoroutine(WanderingRoutineWithCurrentRotation());
        
        Debug.Log($"{gameObject.name}: Forced to start wandering with current rotation");
    }

    // Special wandering routine that keeps current rotation initially
    private IEnumerator WanderingRoutineWithCurrentRotation()
    {
        // First, generate a waypoint in the direction we're currently facing
        Vector3 initialDirection = transform.forward;
        float initialDistance = Random.Range(minWanderDistance, maxWanderDistance);
        Vector3 initialWaypoint = GetSafeWaypointUsingVisualizer(initialDirection, initialDistance);
