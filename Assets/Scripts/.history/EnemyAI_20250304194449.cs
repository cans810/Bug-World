...
public class EnemyAI : MonoBehaviour
{
    ...
    private void CheckForTarget()
    {
        ...
        // Check if we have any targets in range (using the EntityHitbox system)
        if (livingEntity.targetsInRange.Count > 0)
        {
            // Get the closest target
            targetEntity = livingEntity.GetClosestTarget();
            
            if (targetEntity != null && !targetEntity.IsDead)
            {
                // We have a valid target, decide whether to chase or attack
                ...
                // Stop wandering when we have a target
                isWandering = false;
            }
            else
            {
        ...
        {
            // No targets in range, go back to idle if we were chasing or attacking
            targetEntity = null;
            if (currentState == AIState.Chase || currentState == AIState.Attack)
            {
                currentState = AIState.Idle;
