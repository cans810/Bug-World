...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        // Setup similar to other working resource animations
        float elapsed = 0;
        SpriteRenderer spriteRenderer = symbol.GetComponent<SpriteRenderer>();
        if (spriteRenderer == null) yield break;
        ...
        // Use EXACT SAME approach as chitin/crumb animations
        float duration = 0.8f;  // Even faster
        ...
        // Create a fixed midpoint for the arc - LOWER arc for more direct path
        float arcHeight = Random.Range(0.5f, 1.0f);
        ...
        while (elapsed < duration)
        {
            ...
            // Use bezier curve with fixed midpoint - EXACTLY like chitin/crumb
            ...
            // Trigger UI animation earlier
            if (!effectsTriggered && normalizedTime > 0.5f)
            {
                // Trigger UI earlier in the animation
                TriggerUICounterAnimation("coin");
                effectsTriggered = true;
            }
            
            // Fade out at the end - match the pattern of other resources
        ...
        // CLEANUP EXACTLY like other resources
        symbol.SetActive(false);
