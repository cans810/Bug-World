...
public class LivingEntity : MonoBehaviour
{
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // IMPORTANT: Register kill for discovery system if killer is player
        if (killer != null && killer.CompareTag("Player"))
        {
            // Get entity name/type for discovery
            string entityName = gameObject.name.ToLower();
            
            // Extract the entity type from the name (handle names like "Ant(Clone)")
            string entityType = entityName;
            if (entityName.Contains("ant"))
                entityType = "ant";
            else if (entityName.Contains("fly"))
                entityType = "fly";
            else if (entityName.Contains("ladybug"))
                entityType = "ladybug";
            else if (entityName.Contains("flea"))
                entityType = "flea";
            
            // Log the kill for discovery
            Debug.Log($"Player killed entity: {entityType} ({entityName})");
            
            // Trigger discovery system
            DiscoveredEntitiesController controller = DiscoveredEntitiesController.Instance;
            if (controller != null)
            {
        ...
        // Notify MissionManager about this entity's death
        if (MissionManager.Instance != null)
        {
            // Clean up the entity name by removing numbers and "(Clone)" suffix
            string cleanName = System.Text.RegularExpressions.Regex.Replace(entityName, @"\d+|\(Clone\)", "").Trim();
            
            MissionManager.Instance.OnEntityKilled(cleanName);
            Debug.Log($"Mission system notified of {cleanName} death");
        }
    }
