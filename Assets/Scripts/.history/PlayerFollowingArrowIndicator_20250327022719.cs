...
public class PlayerFollowingArrowIndicator : MonoBehaviour
{
    ...
    [SerializeField] private float floatHeight = 4.0f;
    ...
    private void UpdateArrowPosition()
    {
        ...
        // Get the player's height from their transform scale
        float playerHeight = playerTransform.localScale.y;
        
        // Use player's height + small offset instead of fixed float height
        Vector3 targetPosition;
        
        // Calculate position based on player's current height
        // Just slightly above the player
        float heightPosition = playerHeight + 0.1f;
        
        if (floatHeight < 0)
        {
            // Position below player if negative float height specified
            // This is the case for the metamorphosis arrows
            heightPosition = -playerHeight + floatHeight; // Maintain negative offset if specified
        }
        
        // Calculate the final position relative to player
        targetPosition = playerTransform.position + Vector3.up * heightPosition;
        
        // Add offset in direction based on floatHeight sign
        if (heightPosition < 0)
        {
            // Add offset behind the player if below
            targetPosition += -playerTransform.forward * followOffset;
        }
        else
        {
            // Add offset in front of player if above
            targetPosition += playerTransform.forward * followOffset;
        }
