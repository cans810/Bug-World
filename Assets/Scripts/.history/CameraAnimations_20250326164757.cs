...
public class CameraAnimations : MonoBehaviour
{
    ...
    // New coroutine to handle the metamorphosis camera effect
    private IEnumerator MetamorphosisCameraAnimation(Transform playerTransform)
    {
        ...
        // We want the camera to look at the player from the front
        Vector3 playerForward = playerTransform.forward;
        // Calculate a position that's in front of the player (opposite of player's forward)
        Vector3 targetPosition = playerTransform.position - playerForward * (playerTransform.localScale.z * 2f + 3f);
        
        // Keep the camera at a good height (slightly above player)
        targetPosition.y = playerTransform.position.y + playerTransform.localScale.y * 1.2f;
