...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        {
            // Instantiate the chitin icon as a child of the ChitinPanel
            GameObject chitinIcon = Instantiate(chitinIconPrefab, chitinPanel);
            
            // Set its initial position to match the chitinCountText position
            // We'll use screen space to ensure it appears at the chitin counter
            Vector2 chitinCountScreenPos = RectTransformUtility.WorldToScreenPoint(Camera.main, chitinCountText.transform.position);
            
            // Convert screen position to local position in the ChitinPanel
            RectTransformUtility.ScreenPointToLocalPointInRectangle(
                chitinPanel.GetComponent<RectTransform>(),
                chitinCountScreenPos,
                Camera.main,
                out Vector2 localPos);
            
            // Set the local position
            chitinIcon.GetComponent<RectTransform>().anchoredPosition = localPos;
