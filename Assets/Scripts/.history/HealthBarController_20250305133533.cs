...
public class HealthBarController : MonoBehaviour
{
    ...
    private void OnEntityDamaged(float damageAmount)
    {
        ...
        // If damage numbers are enabled and we have the prefab
        if (showDamageNumbers && damageNumberPrefab != null && damageAmount > 0)
        {
    ...
    private void OnEntityHealed(float healAmount)
    {
        ...
        // If heal numbers are enabled and we have the prefab
        if (showHealNumbers && healNumberPrefab != null && healAmount > 0)
        {
            SpawnHealNumber(healAmount);
        }
    }
