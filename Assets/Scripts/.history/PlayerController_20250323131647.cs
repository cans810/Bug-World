...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Border Enforcement")]
    [SerializeField] private float teleportBackDistance = 1.5f; // How far to teleport back when crossing a border
    [SerializeField] private float borderPushStrength = 2.5f; // Increased force for harder border enforcement
    private Vector3 lastSafePosition; // Last position inside a valid area
    private float positionUpdateInterval = 0.2f;
    ...
    private void Update()
    {
        // Update last safe position when not at a border
        if (!isAtMapBorder)
        {
        ...
        // First, handle any boundary restrictions before allowing movement
        if (isAtMapBorder && borderNormal != Vector3.zero)
        {
            // If player is trying to move further into the restricted area
            float movementIntoBorder = Vector3.Dot(moveDirection, -borderNormal);
            if (movementIntoBorder > 0.05f) // If trying to move into the border
            {
                // Apply much stronger push force to prevent squeezing through
                if (livingEntity != null)
                {
                    livingEntity.MoveInDirection(borderNormal, borderPushStrength * Time.deltaTime * 60f);
                    
                    // Cancel all movement in the border direction
                    Vector3 tangentialMovement = moveDirection - (-borderNormal * movementIntoBorder);
                    moveDirection = tangentialMovement.normalized * moveDirection.magnitude * 0.5f; // Reduce speed significantly
                }
                else
                {
                    transform.position += borderNormal * borderPushStrength * Time.deltaTime * 60f;
                }
                
                // Emergency teleport back if player has somehow gotten too far past the border
                if (lastSafePosition != Vector3.zero && Vector3.Dot(transform.position - lastSafePosition, -borderNormal) > teleportBackDistance)
                {
                    // Player has gone too far past the border - teleport back to safety
                    transform.position = lastSafePosition + borderNormal * 0.5f;
                    Debug.Log("Emergency border teleport activated - player returned to safe area");
                    
                    if (uiHelper != null)
                    {
                        uiHelper.ShowInformText("You cannot go that way!");
                    }
                }
            }
        }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (livingEntity != null)
            {
                ...
                livingEntity.MoveInDirection(borderNormal, 0.2f); // Increased from 0.2f
            }
            else
            {
                // Fallback direct position adjustment with stronger push
                transform.position += borderNormal * 0.2f; // Increased from 0.2f
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("BarrierRocks"))
        {
            ...
            {
                transform.position += borderNormal * 0.2f;
            }
            ...
            if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
            {
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText("This path is blocked by rocks!");
                }
    ...
    // Modify the OnTriggerStay method to use stronger enforcement
    private void OnTriggerStay(Collider other)
    {
        // Check for level-restricted borders
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Get the border name and required level
            string borderName = other.gameObject.name;
            
            // Skip borders that don't need level restrictions
            if (borderName == "MapBorder1" || borderName == "MapBorder8" || borderName == "MapBorder14") 
                return;
            ...
            // Get player level
            int playerLevel = playerInventory != null ? playerInventory.CurrentLevel : 1;
            
            // If player isn't high enough level, enforce the border restriction with stronger measures
            if (playerLevel < requiredLevel)
            {
                // Set border restriction flags
                isAtMapBorder = true;
                
                // Calculate direction for pushing the player back
                Vector3 borderCenter = other.bounds.center;
                Vector3 pushDirection = (transform.position - borderCenter).normalized;
                borderNormal = pushDirection;
                
                // Apply a much stronger force to push player back
                if (livingEntity != null)
                {
                    livingEntity.MoveInDirection(borderNormal, borderPushStrength * Time.deltaTime * 60f);
                }
                ...
                // If player somehow got past border, teleport them back
                if (lastSafePosition != Vector3.zero)
                {
                    float distancePastBorder = Vector3.Dot(transform.position - lastSafePosition, -borderNormal);
                    if (distancePastBorder > 0.75f) // If player has moved significantly past border
                    {
                        transform.position = lastSafePosition;
                        Debug.Log($"Border teleport activated for {borderName} - returned to safe position");
        ...
        // Rest of your OnTriggerStay implementation...
    }
