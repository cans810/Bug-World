...
public class PlayerController : MonoBehaviour
{
    ...
    // Modify CheckForMetamorphosis method to show Size Up! text
    private void CheckForMetamorphosis()
    {
        // Skip if metamorphosis is disabled
        if (!enableMetamorphosis) return;
        
        // Skip if we're initializing
        if (isInitializing || isInitializingScale) return;
        
        // Get current level
        int currentLevel = playerInventory != null ? playerInventory.CurrentLevel : 1;
        
        // Check if we've reached a new metamorphosis level
        bool shouldMetamorphose = false;
        
        for (int i = 0; i < metamorphosisLevels.Length; i++)
        {
            if (currentLevel == metamorphosisLevels[i] && i > currentMetamorphosisStage)
            {
                currentMetamorphosisStage = i;
                shouldMetamorphose = true;
                break;
            }
        }
        
        if (shouldMetamorphose)
        {
            // Calculate new scale
            Vector3 newScale = baseScale * Mathf.Pow(scaleIncreaseMultiplier, currentMetamorphosisStage + 1);
            
            // Apply scale
            ApplyNewScale(newScale);
            
            // Adjust camera for the new size
            UpdateCameraForMetamorphosis(currentLevel);
            
            // Show Size Up! message for 3 seconds
            if (uiHelper != null)
            {
                // Display the Size Up! message with highlight color
                uiHelper.ShowInformText("<color=#FFD700>Size Up!</color>", 3.0f);
                
                // Add haptic feedback
                CandyCoded.HapticFeedback.HapticFeedback.MediumFeedback();
            }
            
            Debug.Log($"Player metamorphosed! New stage: {currentMetamorphosisStage+1}, Level: {currentLevel}, Scale: {newScale}");
        }
    }
}