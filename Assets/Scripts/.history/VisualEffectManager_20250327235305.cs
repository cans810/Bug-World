...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator AnimateXPSymbolToScreen(GameObject symbol, Vector2 targetScreenPos, int xpValue, int currentXP)
    {
        ...
        while (elapsed < duration)
        {
            ...
            // IMPORTANT: This might be adding XP twice - comment it out if it is
            // When the symbol is 90% of the way to the destination, add XP
            if (!hasAddedXP && normalizedTime >= 0.9f)
            {
                // This should ONLY update UI, not actually add XP again
                // playerInventory.AddXP(xpValue); // Comment this line if it's duplicating XP
                
                // Just update UI
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null)
                {
                    uiHelper.UpdateExperienceDisplay(currentXP + xpValue, false);
                }
