...
public class EntityGenerator : MonoBehaviour
{
    ...
    [SerializeField] private float spawnHeight = 0.1f; // Height above ground to spawn (reduced from 0.5f)
    ...
    private bool TryGetSpawnPosition(out Vector3 position)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            // Raycast to find ground - start from a lower height (20 instead of 50)
            RaycastHit hit;
            if (Physics.Raycast(randomPoint + Vector3.up * 20f, Vector3.down, out hit, 40f, spawnableLayers))
            {
