...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    // Add this field at the class level, with the private variables
    private float lastXPSoundTime = 0f;
    ...
    public void PlayChitinDepositEffect(int chitinAmount, int xpAmount, PlayerInventory inventory)
    {
        ...
        if (inventory != null)
        {
            ...
            // Stop all existing animations immediately to prevent conflicts
            ...
            // Start the coroutine for spawning symbols immediately and keep a reference to it
            ...
            // Delay this slightly to prevent it from interfering with the main animation
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        // Play sound once per second instead of for every symbol
        if (SoundEffectManager.Instance != null && Time.time - lastXPSoundTime >= 1.0f)
        {
            SoundEffectManager.Instance.PlaySound("XPGained");
            lastXPSoundTime = Time.time;
        }
    ...
    private IEnumerator DelayedDepositCompletionCheck()
    {
        ...
        if (isAnimationInProgress && pendingXpAmount > 0 && playerInventory != null)
        {
