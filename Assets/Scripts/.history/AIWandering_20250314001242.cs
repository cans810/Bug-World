...
public class AIWandering : MonoBehaviour
{
        ...
        while (!validWaypointFound && attempts < maxAttempts)
        {
            ...
            else
            {
                ...
                if (hasTerritory && respectTerritory)
                {
                    ...
                    if (isRingTerritory)
                    {
                        // For ring territories, check if position is within the ring
                        withinTerritory = distanceToCenter >= (innerTerritoryRadius + territoryRespectDistance) && 
                                          distanceToCenter <= (territoryRadius - territoryRespectDistance);
                    }
                    else
                    {
                        // For regular territories, just check if within radius
    ...
    private void EnforceTerritory()
    {
        ...
        if (isRingTerritory)
        {
            // For ring territories, we need to stay between inner and outer radius
            if (distanceToCenter < innerTerritoryRadius + territoryRespectDistance)
            {
                // Too close to inner boundary, move away from center
                needsAdjustment = true;
                directionToMove = (flatPosition - flatCenter).normalized;
            }
            else if (distanceToCenter > territoryRadius - territoryRespectDistance)
            {
                // Too close to outer boundary, move toward center
                needsAdjustment = true;
                directionToMove = (flatCenter - flatPosition).normalized;
            }
        }
        else
        {
            // For regular territories, just check outer boundary
            if (distanceToCenter > territoryRadius - territoryRespectDistance)
            {
                // Too close to boundary, move toward center
                needsAdjustment = true;
                directionToMove = (flatCenter - flatPosition).normalized;
            }
        }
        
        // If we need to adjust, set a new waypoint
        if (needsAdjustment)
        {
            float turnDistance = Random.Range(minWanderDistance, maxWanderDistance);
            currentWaypoint = transform.position + directionToMove * turnDistance;
