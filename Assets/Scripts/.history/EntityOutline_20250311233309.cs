...
public class EntityOutline : MonoBehaviour
{
    ...
    private void SetupOutline()
    {
        ...
        // Exclude loot objects from outline
        ExcludeLootFromOutline();
    ...
    // Add a new method to exclude loot objects from the outline
    private void ExcludeLootFromOutline()
    {
        ...
        // Create a list to store renderers to exclude
        List<Renderer> renderersToExclude = new List<Renderer>();
        
        // Check each renderer
        foreach (Renderer renderer in allRenderers)
        {
            // Skip if this is the main entity's renderer
            if (renderer.gameObject == entity) continue;
            
            // Check if this renderer is on a loot layer
            if (renderer.gameObject.layer == LayerMask.NameToLayer("CarriedLoot") || 
                renderer.gameObject.layer == LayerMask.NameToLayer("Loot"))
            {
                renderersToExclude.Add(renderer);
                Debug.Log($"Excluding loot object from outline: {renderer.gameObject.name}");
            }
        }
        
        // If we found any loot renderers, exclude them from the outline
        if (renderersToExclude.Count > 0)
        {
            // Get the current list of renderers the outline is using
            Renderer[] currentRenderers = outline.GetComponentsInChildren<Renderer>();
            
            // Create a new list without the excluded renderers
            List<Renderer> filteredRenderers = new List<Renderer>();
            foreach (Renderer renderer in currentRenderers)
            {
                if (!renderersToExclude.Contains(renderer))
                {
                    filteredRenderers.Add(renderer);
                }
            }
            
            // Set the filtered renderers on the outline
            outline.Renderers = filteredRenderers.ToArray();
        }
    }
    ...
    // Update the ForceUpdate method to also exclude loot
