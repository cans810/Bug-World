...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        if (isEntityBoundaryViolation)
        {
            ...
            // If using ring boundary, handle different boundary violations
            if (useRingBoundary)
            {
            ...
            {
                // For regular circular boundary, move tangent to the boundary
                redirectedDirection = Vector3.Cross(Vector3.up, fromOrigin);
            }
            
            // Turn to follow the redirected direction
            transform.forward = redirectedDirection;
            
            // Move along the redirected direction
            transform.position += redirectedDirection * livingEntity.moveSpeed * Time.deltaTime;
            
            // Set a new waypoint in the redirected direction
            SetWaypointInDirection(redirectedDirection);
        }
    ...
    private bool IsWithinEntityBoundary(Vector3 position)
    {
        ...
        // For ring boundaries, check both inner and outer radius
        if (useRingBoundary)
        {
