...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Calculate half of the straight section length
        float halfLineLength = (height - 2 * radius) * 0.5f;
        float totalLength = height - 2 * radius; // Total length of straight sections
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            // Stadium shape consists of:
            // - Two semi-circles (at 0-90 & 270-360 degrees and at 90-270 degrees)
            // - Two straight lines connecting the semi-circles
            
            if (direction == 0) // X-axis capsule
            {
                if (angle < Mathf.PI / 2 || angle > 3 * Mathf.PI / 2)
                {
                    // Right semicircle
                    float circleAngle = (angle < Mathf.PI / 2) ? angle : (angle - 3 * Mathf.PI / 2 + Mathf.PI / 2);
                    point.x = center.x + halfLineLength;
                    point.z = center.z + radius * Mathf.Cos(circleAngle);
                }
                else if (angle >= Mathf.PI / 2 && angle <= 3 * Mathf.PI / 2)
                {
                    // Left semicircle
            ...
            else if (direction == 2) // Z-axis capsule
            {
                if (angle < Mathf.PI / 2 || angle > 3 * Mathf.PI / 2)
                {
                    // Front semicircle
                    float circleAngle = (angle < Mathf.PI / 2) ? angle : (angle - 3 * Mathf.PI / 2 + Mathf.PI / 2);
                    point.x = center.x + radius * Mathf.Cos(circleAngle);
                    point.z = center.z + halfLineLength;
                }
                else if (angle >= Mathf.PI / 2 && angle <= 3 * Mathf.PI / 2)
                {
                    // Back semicircle
