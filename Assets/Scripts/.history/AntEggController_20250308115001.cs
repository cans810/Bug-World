...
public class AntEggController : MonoBehaviour
{
    ...
    [Header("Testing")]
    [SerializeField] private bool useDebugTime = false; // For testing
    [SerializeField] private float debugHatchTime = 30f; // 30 seconds for testing
    ...
    private void Awake()
    {
        // Apply debug time if in testing mode
        if (useDebugTime && Application.isEditor)
        {
            hatchTime = debugHatchTime;
            Debug.Log($"[DEBUG] Using shorter hatch time: {hatchTime} seconds");
        }
    }
    ...
    private void UpdatePlayerMessage()
    {
        if (uiHelper != null && uiHelper.informPlayerText != null)
        {
            ...
            uiHelper.informPlayerText.text = $"Ant egg hatching in {FormatTime(remainingTime)}";
        }
    }
