...
public class CameraAnimations : MonoBehaviour
{
    ...
    public void OnAreaUnlocked(GameObject areaTarget, int level, string areaName)
    {
        ...
        if (areaTarget != null)
        {
            // NEW CODE: Find and disable border for this area first
    ...
    private IEnumerator ProcessAnimationQueue()
    {
        ...
        while (animationQueue.Count > 0)
        {
            ...
            // Process animation with the isEgg flag
    ...
    private IEnumerator AnimateCameraToArea(Transform areaTarget, int level = 0, string areaName = "", bool isEgg = false)
    {
        ...
        float useHeightOffset = isEgg ? 3f : heightOffset;  // Much lower for eggs
        float useLookDownAngle = isEgg ? 30f : lookDownAngle; // Less steep angle for eggs
        float useAnimationDuration = isEgg ? 1.5f : animationDuration; // Faster for eggs
        float useAreaViewDuration = isEgg ? 2.5f : areaViewDuration; // Shorter viewing time
        ...
        float panDuration = useAreaViewDuration * 0.8f;
    ...
    // This method is called by the event system after camera animation
    private void ShowAttributesPanelAfterAnimation(GameObject areaTarget, int level, string areaName)
    {
        ...
        if (isAnimating)
        {
            Debug.Log("Not showing attributes panel yet - animation still in progress");
            return;
        }

        // Find the attribute display and show the panel
        AttributeDisplay attributeDisplay = FindObjectOfType<AttributeDisplay>();
        if (attributeDisplay != null)
        {
    ...
    private void RemoveBorderForArea(GameObject areaTarget, int level)
    {
        ...
        foreach (BorderVisualizer border in allBorders)
        {
            ...
            if (isMatch)
            {
                ...
                Debug.Log($"Fading out border: {borderName}");
                border.FadeOutAndDisable();
                
                // Also set it as permanently disabled for save/load
