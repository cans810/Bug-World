...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawBoxPerimeter()
    {
        ...
        // We need more points to represent a rotated rectangle properly
        // Using 5 points (the fifth point closes the loop back to the first)
        perimeterLineRenderer.positionCount = 5;
        
        // Calculate the four corners of the box at the specified height
        float halfWidth = size.x / 2;
        float halfLength = size.z / 2;
        
        // Create local space corners relative to the center
        ...
        // Set the positions in local space with proper height and center offset
        for (int i = 0; i < corners.Length; i++)
        {
            // Apply the center offset and set the correct height
            ...
            perimeterLineRenderer.SetPosition(i, corners[i]);
        }
        
        // Since LineRenderer is a child of this GameObject, it will automatically
        // inherit the parent's rotation, so we don't need to apply rotation explicitly
    }
