...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayer()
    {
        ...
        if (distanceToPlayer > followDistance)
        {
            ...
            float moveDistance = Mathf.Min(livingEntity.moveSpeed * Time.deltaTime, distanceToPlayer - followDistance);
            
            // Normalize direction
            Vector3 normalizedDirection = directionToPlayer.normalized;
            
            // Calculate movement
            Vector3 movement = normalizedDirection * moveDistance;
        ...
        else
        {
            ...
            if (animController != null)
            {
                if (!animController.IsAnimationPlaying("idle"))
                {
