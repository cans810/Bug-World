...
public class UIHelper : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 cooldownTextOffset = new Vector2(0, 30); // Reduced Y offset from 100 to 30
    ...
    // Add this new method to display attack cooldown text
    ...
    // Improved position method to be even closer to player and stop flickering
    private void UpdateCooldownTextPosition()
    {
        ...
        // Get position much closer to player's head (reduced Y offset further from 1.2f to 0.7f)
        Vector3 playerPos = player.transform.position + new Vector3(0, 0.7f, 0);
        ...
        if (textRect != null)
        {
            ...
            // Use much stronger smoothing for the position to prevent flickering
            textRect.position = Vector3.Lerp(textRect.position, new Vector3(screenPos.x, screenPos.y, 0), Time.deltaTime * 25f);
        }
    }

    // Fix the cooldown text hiding coroutine to make it more reliable
    private IEnumerator HideAttackCooldownText(string originalMessage)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(numericPart))
            {
                ...
                // Add a small buffer to ensure we don't hide prematurely
                cooldownTime += 0.2f;
            }
            else
            {
    ...
    // Update the cooldown text display in PlayerController.cs to prevent flickering when below 1 second
