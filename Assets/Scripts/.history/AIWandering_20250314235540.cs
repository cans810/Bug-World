...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            // 2. PICK A NEW RANDOM DIRECTION that avoids borders
            ...
            // 3. Check ahead for borders before committing to this direction
            ...
            while (moveTimer < moveDuration && distanceMoved < moveDistance)
            {
                ...
                // NEW: Check for borders ahead while moving
                if (avoidBorders && IsBorderInDirection(transform.forward, Mathf.Min(borderDetectionDistance, moveDistance - distanceMoved)))
                {
                    // Border detected ahead while moving - break out of movement loop
                    break;
                }
    ...
    {
        // Generate basic random direction
    ...
    // Add this method to check if a direction leads toward a border
    private bool IsBorderInDirection(Vector3 direction, float distance)
    {
        // Cast a ray in the given direction
        RaycastHit hit;
        Vector3 rayOrigin = transform.position + Vector3.up * 0.5f; // Slightly above ground to avoid terrain
        ...
        {
            // Border detected in this direction
            return true;
        }
    ...
    // Add this method to find a safe direction away from borders
    private Vector3 FindSafeDirection()
    {
        // Try several random directions until we find one that doesn't lead to a border
        for (int i = 0; i < 8; i++) // Try up to 8 different directions
        {
            // Get a random direction
            Vector3 randomDirection = GetRandomDirection();
            
            // Check if this direction leads to a border
            if (!IsBorderInDirection(randomDirection, borderDetectionDistance))
            {
        ...
        // If no safe direction found, return direction toward center of play area
        if (playerNestTransform != null)
        {
            // Use the nest as a reference point to the center of the play area
            return (playerNestTransform.position - transform.position).normalized;
        }
        
        // Fallback: Return a random direction with downward bias (away from any potential borders)
