...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void CreateVisualSphere()
    {
        if (showRadius && sphereCollider != null)
        {
            ...
            if (fillCircle)
            {
                // Create a proper circle visualization using a cylinder
                GameObject disc = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
                disc.name = "FilledCircle";
                ...
                // Scale the cylinder to match the radius and make it very thin to appear as a disc
                float diameter = sphereCollider.radius * 2;
                disc.transform.localScale = new Vector3(diameter, 0.01f, diameter);
                
                // Orient the disc to lie flat on the X-Z plane (ground)
                // Note: Cylinders in Unity are already oriented with their circular faces on the X-Z plane
                
                // Create a material for the disc
                Material discMaterial = new Material(Shader.Find("Transparent/Diffuse"));
                
                // Set the color with semi-transparency
                Color discColor = radiusColor;
                discColor.a = 0.5f; // Make it semi-transparent
                discMaterial.color = discColor;
                
                // For mobile compatibility, use simpler transparency settings
                discMaterial.SetFloat("_Mode", 3); // Set to transparent mode
                discMaterial.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
                discMaterial.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
                discMaterial.SetInt("_ZWrite", 0);
                discMaterial.DisableKeyword("_ALPHATEST_ON");
                discMaterial.EnableKeyword("_ALPHABLEND_ON");
                discMaterial.DisableKeyword("_ALPHAPREMULTIPLY_ON");
                discMaterial.renderQueue = 3000;
