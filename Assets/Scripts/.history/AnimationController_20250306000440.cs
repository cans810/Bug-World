...
public class AnimationController : MonoBehaviour
{
    ...
    /// <summary>
    /// Sets the AttackWalking animation state
    /// </summary>
    public void SetAttackWalking(bool attackWalking)
    {
        if (animator != null)
        {
            ...
            Debug.Log($"[DETAILED] Setting AttackWalking to {attackWalking}");
            Debug.Log($"[DETAILED] Current animator state: {animator.GetCurrentAnimatorStateInfo(0).fullPathHash}");
            Debug.Log($"[DETAILED] Current animator state name: {animator.GetCurrentAnimatorStateInfo(0).IsName("AttackWalking") ? "AttackWalking" : animator.GetCurrentAnimatorStateInfo(0).ToString()}");
            
            // Disable everything else first
            foreach (AnimatorControllerParameter param in animator.parameters)
            {
                if (param.name != "AttackWalking" && param.type == AnimatorControllerParameterType.Bool)
                {
                    animator.SetBool(param.name, false);
                    Debug.Log($"[DETAILED] Setting {param.name} to FALSE");
                }
            }
            
            // Force update to clear states
            animator.Update(0.0f);
            
            // Now set AttackWalking
            animator.SetBool("AttackWalking", attackWalking);
            Debug.Log($"[DETAILED] Final setting of AttackWalking to {attackWalking}");
            
            // Try to force transition to this state by name
            if (attackWalking)
            {
                try {
                    animator.Play("AttackWalking", 0, 0f);
                    Debug.Log("[DETAILED] Directly playing AttackWalking state");
                }
                catch (System.Exception e) {
                    Debug.LogError($"[DETAILED] Error playing AttackWalking state: {e.Message}");
                }
            }
            ...
            // Update all parameters based on internal state
            UpdateAnimatorParameters();
        }
    }
