...
public class BaseInteraction : MonoBehaviour
{
    ...
    [SerializeField] private bool hasEnteredNestForFirstTime = false;

    [Header("First-Time Deposits")]
    [SerializeField] private bool hasDepositedCrumbForFirstTime = false;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (inventory != null)
        {
            ...
            // Add this code for first-time nest introduction
            if (!hasEnteredNestForFirstTime)
            {
                // Show the introductory panel
                UIHelper uiHelper = FindObjectOfType<UIHelper>();
                if (uiHelper != null && uiHelper.thisIsYourNestPanel != null)
                {
                    uiHelper.thisIsYourNestPanel.GetComponent<EnteredInNestFirstTimeHelper>().OpenThisIsYourNestPanel();
                    uiHelper.thisIsYourNestPanel.GetComponent<Animator>().SetBool("ShowUp", true);
                    uiHelper.thisIsYourNestPanel.GetComponent<Animator>().SetBool("Hide", false);
                    Debug.Log("First time in nest! Showing introductory panel.");
                }
                
                // Mark as visited and save
                hasEnteredNestForFirstTime = true;
                
                // Save the game to record this change
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            // Reset the circle color when player leaves
            if (CircleImage != null)
            {
                CircleImage.color = baseCircleColor;
                Debug.Log($"Reset base circle color to: {baseCircleColor}");
            }
        ...
        else if (other.CompareTag("Ally") || other.GetComponent<AllyAI>() != null)
        {
            // Get the ally's LivingEntity component
            LivingEntity allyEntity = other.GetComponent<LivingEntity>();
            if (allyEntity != null && alliesInBase.Contains(allyEntity))
            {
                if (showDebugMessages)
                {
                    Debug.Log($"Ally {allyEntity.gameObject.name} left the base.");
                }
                
                // Restore the original outline color if we have a reference to it
                if (allyOutlines.ContainsKey(allyEntity) && originalAllyColors.ContainsKey(allyEntity))
                {
                    EntityOutline allyOutline = allyOutlines[allyEntity];
                    Color originalColor = originalAllyColors[allyEntity];
                    
                    if (allyOutline != null)
                    {
                        allyOutline.SetOutlineColor(originalColor);
                        
                        if (showDebugMessages)
                        {
                            Debug.Log($"Restored ally {allyEntity.gameObject.name} outline color to {originalColor}");
                        }
                    }
                    
                    // Remove from dictionaries
                    allyOutlines.Remove(allyEntity);
                    originalAllyColors.Remove(allyEntity);
                }
                
                // Remove from list of allies in base
                alliesInBase.Remove(allyEntity);
            }
        }
    }
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        // Wait a minimal time for other effects to start (reduced from 1.5s)
        if (visualEffectManager != null)
        {
            yield return new WaitForSeconds(0.1f);  // Reduced to 0.1s for faster response
        }
        ...
        if (uiEffects != null && playerInventory != null)
        {
            ...
            // Let the animation play out (but don't need as much time)
            yield return new WaitForSeconds(1.0f);  // Reduced from 1.5s
        }
        else
        {
    ...
    {
        if (playerInventory == null || !autoDepositCrumbs) return;
        
        // Check if player has crumbs to deposit
        int playerCrumbs = playerInventory.CrumbCount;
        if (playerCrumbs <= 0) return;
        
        // Get the incubator reference if needed
        if (insectIncubator == null)
        {
            insectIncubator = FindObjectOfType<InsectIncubator>();
            if (insectIncubator == null)
            {
                Debug.LogError("No InsectIncubator found in scene! Cannot deposit crumbs.");
                return;
            }
        }
        
        // Now attempt deposit - using the correct method name for InsectIncubator
        int oldCrumbCount = playerCrumbs;
        
        // Method could be named differently - here are common alternatives
        
        // Try DepositCrumbs instead of AddCrumbs
        insectIncubator.DepositCrumbs(playerCrumbs);
        
        // OR - If it has an AddCrumbsToIncubation method
        // insectIncubator.AddCrumbsToIncubation(playerCrumbs);
        
        // Clear all crumbs from player inventory
        playerInventory.RemoveCrumb(playerCrumbs);
        
        // Play collection sound via manager if available
        if (useSoundEffectManager && SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound(depositSoundEffectName);
        }
        else if (audioSource != null && depositSound != null)
        {
            audioSource.clip = depositSound;
            audioSource.Play();
        }
        
        // Visual effect for crumb collection
        if (visualEffectManager != null)
        {
            // Use updated spawn effect method with "shine" effect
            visualEffectManager.SpawnEffect("shine", transform.position + Vector3.up * 1.5f, 5);
        }
        
        // First-time crumb deposit notification
        if (!hasDepositedCrumbForFirstTime)
        {
            Debug.Log("First crumb deposit detected - showing Discovered Entities button indicator");
            hasDepositedCrumbForFirstTime = true;
            
            // Find UIHelper and activate the indicator
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null && uiHelper.discoveredEntitiesButtonPointingPulsatingImage != null)
            {
                uiHelper.discoveredEntitiesButtonPointingPulsatingImage.SetActive(true);
                Debug.Log("Activated Discovered Entities button indicator for first-time crumb deposit");
            }
        ...
        Debug.Log($"Deposited {oldCrumbCount} crumbs. Player now has {playerInventory.CrumbCount} crumbs left.");
    }
    ...
    {
        // Wait for a reasonable time for all animations to complete
        float animationDelay = chitinFlyDuration + 0.5f;
        ...
        Debug.Log($"Chitin deposit completed and references cleaned up after {animationDelay}s");
    }
    ...
    // Add this method to save the first visit state
    public void SaveFirstVisitState(GameData data)
    {
        if (data != null)
        {
            data.hasEnteredNestForFirstTime = hasEnteredNestForFirstTime;
            Debug.Log($"Saved nest first visit state: {hasEnteredNestForFirstTime}");
        }
    }

    // Add this method to load the first visit state
    public void LoadFirstVisitState(GameData data)
    {
        if (data != null)
        {
            hasEnteredNestForFirstTime = data.hasEnteredNestForFirstTime;
            Debug.Log($"Loaded nest first visit state: {hasEnteredNestForFirstTime}");
        }
    }

    // Add a new method to save and load this state
    public void SaveFirstCrumbDepositState(GameData data)
    {
        if (data != null)
        {
            data.hasDepositedCrumbForFirstTime = hasDepositedCrumbForFirstTime;
            Debug.Log($"Saved first crumb deposit state: {hasDepositedCrumbForFirstTime}");
        }
    }

    public void LoadFirstCrumbDepositState(GameData data)
    {
        if (data != null)
        {
            hasDepositedCrumbForFirstTime = data.hasDepositedCrumbForFirstTime;
            Debug.Log($"Loaded first crumb deposit state: {hasDepositedCrumbForFirstTime}");
        }
    }
} 