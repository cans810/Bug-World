...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if (isAtMapBorder && moveDirection.magnitude > 0.1f)
        {
            ...
            // If trying to move away from the center (negative dot product with normal)
            if (movementWithBorder < 0)
            {
                // Calculate the component of movement that's going out of the border
                Vector3 outwardComponent = -borderNormal * movementWithBorder;
                
                // Remove the outward component, keeping only the tangential movement
                Vector3 correctedDirection = moveDirection - outwardComponent;
                ...
                // If movement becomes negligible after restriction, allow some movement parallel to border
                if (moveDirection.magnitude < 0.1f)
                {
                    // Create a movement parallel to the border to prevent getting stuck
                    Vector3 tangent = Vector3.Cross(Vector3.up, borderNormal).normalized;
                    if (tangent.magnitude < 0.1f) // If tangent is too small, try a different axis
                        tangent = Vector3.Cross(Vector3.right, borderNormal).normalized;
                        
                    // Add a small amount of tangential movement to prevent sticking
                    moveDirection = tangent * 0.1f;
                }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        // Add this at the end of OnTriggerEnter method, before the closing brace
        else if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // We entered the BigBorder - reset the border status since we're inside now
            isAtMapBorder = false;
            borderNormal = Vector3.zero;
            
            if (showDebugMessages)
            {
                Debug.Log("Player entered BigBorder area - movement unrestricted inside the border");
            }
        }
    }

    // Modify the OnTriggerStay method to be smarter about allowing movement away from the border
    private void OnTriggerStay(Collider other)
    {
        // Only handle BigBorder in OnTriggerStay
        if (other.gameObject.layer == LayerMask.NameToLayer("BigBorder"))
        {
            // Get the closest point on the collider boundary to the player
            Vector3 closestPoint = other.ClosestPoint(transform.position);
            
            // Direction from closest point to player (pointing inward)
            ...
            // Check if player is trying to move outward (against the border)
            if (moveDirection.magnitude > 0.1f)
            {
                float movementAgainstBorder = Vector3.Dot(moveDirection, -borderNormal);
                
                // If player is close to the edge AND trying to move outward
                if (distanceToEdge < 0.5f && movementAgainstBorder > 0.1f)
                {
                    isAtMapBorder = true;
                    
                    // Only show the message if not on cooldown
                    if (showBorderMessages && Time.time >= lastBorderMessageTime + borderMessageCooldown)
                    {
                        // Display UI message if we have a UI helper
                        if (uiHelper != null)
                ...
                {
                    // Player is moving away from the border or not close enough
                    isAtMapBorder = false;
                }
            }
            else
            {
                // Player is not moving, so don't restrict
                isAtMapBorder = false;
            }
        }
    }
