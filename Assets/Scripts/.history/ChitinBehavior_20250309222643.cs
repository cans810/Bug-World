...
public class ChitinBehavior : MonoBehaviour
{
    ...
    private void StickInPlace()
    {
        ...
        if (collectible != null)
        {
            ...
            // Check if player is nearby and force collection if needed
            Transform playerTransform = GameObject.FindGameObjectWithTag("Player")?.transform;
            if (playerTransform != null)
            {
                float distanceToPlayer = Vector3.Distance(transform.position, playerTransform.position);
                float collectRadius = 1.5f; // Use default value since GetCollectRadius() is gone
                ...
                if (distanceToPlayer <= collectRadius)
                {
                    ...
                    // Use TryCollect instead of ForceCollect
