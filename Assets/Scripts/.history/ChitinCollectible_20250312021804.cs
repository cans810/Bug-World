...
public class ChitinCollectible : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            // Check if this is a carried loot item at the base
            if (gameObject.layer == LayerMask.NameToLayer("CarriedLoot"))
            {
                // For carried loot, only collect if there's room in the inventory
                if (playerInventory != null && !playerInventory.IsChitinFull)
                {
                    TryCollect();
                }
                else
                {
                    // Play the full inventory sound if inventory is full
                    if (fullInventorySound != null)
                    {
                        AudioSource.PlayClipAtPoint(fullInventorySound, transform.position);
                    }
                    Debug.Log($"Cannot collect carried chitin: inventory full");
                }
            }
            else
            {
                // For normal loot, use the standard collection logic
                TryCollect();
            }
        }
    }
