...
public class UIHelper : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (playerInventory != null)
        {
            currentDisplayedXP = playerInventory._experience;
        }
    ...
    public void CloseRevivalAndLooseChitinAndCrumbsClicked()
    {
        ...
        // Find player inventory to reset resources
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null)
        {
            // Set reviving flag BEFORE changing resources
            playerInventory.IsReviving = true;
            
            // Get current values for the message
            int chitinLost = playerInventory.ChitinCount;
            int crumbLost = playerInventory.CrumbCount;
            ...
            if (playerController != null)
            {
                ...
                // Important: Now we call RevivePlayer with our own message
                // and let it handle resetting the reviving flag
                RevivePlayer(playerController, $"You lost all your chitin and crumbs but have been revived!");
            }
            else
            {
                // Reset the flag if we couldn't find the player controller
    ...
    // Add this method to your UIHelper class to ensure resource fill images are updated
    public void UpdateAllResourceDisplays()
    {
        // Find player inventory
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory == null) return;
        
        // Update all resource displays with correct method signatures
        UpdateChitinDisplay(playerInventory.ChitinCount);
        UpdateCrumbDisplay(playerInventory.CrumbCount);
        
        // Try different ways to access egg count - use a direct safe fallback
        int eggCount = 0;
        try {
            // Try several properties that might exist
            if (playerInventory.GetType().GetProperty("EggCount") != null)
                eggCount = playerInventory.EggCount;
            else
                eggCount = playerInventory.GetEggs(); // Or try the method
        }
        catch {
            Debug.LogWarning("Could not get egg count - using 0");
        }
        UpdateEggDisplay(eggCount);
        
        UpdateCoinDisplay(playerInventory.CoinCount);
        UpdateExperienceDisplay(playerInventory._experience, false);
        
        Debug.Log("Updated all resource displays");
    }
} 
