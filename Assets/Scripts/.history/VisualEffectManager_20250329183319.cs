...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Updated XP symbol spawning coroutine to ensure better visibility
    private IEnumerator SpawnXPSymbols(Vector3 startPosition, Vector3 targetPosition, int count, GameObject player, bool useWorldSpace)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Set initial position either in world space or screen space
            if (useWorldSpace)
            {
                // Position in world space with random offset
                xpSymbol.transform.position = startPosition + randomOffset;
            }
            else
            {
                // Start at a position in front of the camera
                Vector3 viewportStartPos = mainCamera.WorldToViewportPoint(startPosition);
                xpSymbol.transform.position = mainCamera.ViewportToWorldPoint(
                    new Vector3(viewportStartPos.x, viewportStartPos.y, 1f));
            }
            ...
            StartCoroutine(AnimateXPSymbol(xpSymbol, xpSymbol.transform.position, targetPosition, player, mainCamera));
    ...
    // Updated XP symbol animation to ensure visibility
    private IEnumerator AnimateXPSymbol(GameObject symbol, Vector3 startPos, Vector3 endPos, GameObject player, Camera mainCamera)
    {
        ...
        Vector3 initialPosition = symbol.transform.position;
        
        // Initial hover animation - stay in place but bob slightly
        while (elapsed < hoverTime)
        {
            ...
            float yOffset = Mathf.Sin(normalizedTime * Mathf.PI * 2) * 0.2f;
            symbol.transform.position = initialPosition + new Vector3(0, yOffset, 0);
        ...
        // Calculate an intermediate point - first go to player, then to XP counter
        ...
        while (elapsed < duration)
        {
            ...
            if (normalizedTime < 0.5f)
            {
                ...
                // Calculate arc height
                float arcHeight = Mathf.Sin(t * Mathf.PI) * 1f;
                Vector3 arcOffset = Vector3.up * arcHeight;
                
                Vector3 currentPos = Vector3.Lerp(startPos, playerPosition, t) + arcOffset;
            ...
            else
            {
                ...
                // Convert target position to ensure it's in front of the camera
                Vector3 screenPos = mainCamera.WorldToScreenPoint(endPos);
                Vector3 safeEndPos = mainCamera.ScreenToWorldPoint(new Vector3(screenPos.x, screenPos.y, 5f));
                
                // Calculate arc height
                float arcHeight = Mathf.Sin(t * Mathf.PI) * 0.5f;
                Vector3 arcOffset = Vector3.up * arcHeight;
                
                Vector3 currentPos = Vector3.Lerp(playerPosition, safeEndPos, t) + arcOffset;
                symbol.transform.position = currentPos;
            }
        ...
        // Update the XP counter if needed
        PlayerInventory playerInventory = player.GetComponent<PlayerInventory>();
        if (playerInventory != null && incrementXPOneByOne)
        {
            // This effect is purely visual - the actual XP addition is handled elsewhere
        }
    }
} 