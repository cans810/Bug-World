...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        Debug.Log($"[PlayerInventory] DepositChitinAtBase called with amount: {amount}, current count: {ChitinCount}");
        
        if (amount <= 0 || ChitinCount < amount)
        {
            Debug.LogWarning($"[PlayerInventory] Cannot deposit chitins: amount={amount}, current count={ChitinCount}");
            return;
        }
        ...
        Debug.Log($"[PlayerInventory] Depositing {amount} chitins at base (current count: {ChitinCount})");
        
        // Animate the chitin deposit before removing them from inventory
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            Debug.Log($"[PlayerInventory] Calling AnimateChitinDeposit with amount: {amount}");
            uiHelper.AnimateChitinDeposit(amount);
        }
        else
        {
            Debug.LogWarning("[PlayerInventory] Could not find UIHelper for chitin deposit animation");
        }
        ...
        // Log the deposit
        Debug.Log($"[PlayerInventory] Deposited {amount} chitins, gained {xpGained} XP");
    }
