...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinCollectEffect(int count)
    {
        ...
        if (mainCanvas == null)
        {
            Debug.LogError("Canvas not found for UI animations!");
            yield break;
        }

        // Find chitin panel position in screen space
        Vector2 screenTopRight = GetScreenPositionFromPanel(chitinPanel);

        // Find nest panel position in screen space
        Vector2 nestScreenPos = GetScreenPositionFromPanel(chitinPanel);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create random offset around chitin panel
            Vector2 randomOffset = Random.insideUnitCircle * spreadRadius;
            Vector3 spawnScreenPos = screenTopRight + randomOffset;

            // Create a UI version of the symbol if needed
            if (symbol.GetComponent<RectTransform>() == null)
            {
                // Convert the symbol to a UI element
                GameObject uiSymbol = new GameObject("UI_ChitinSymbol");
                RectTransform rectTransform = uiSymbol.AddComponent<RectTransform>();
                Image image = uiSymbol.AddComponent<Image>();
                
                // Get the sprite from the original symbol
                SpriteRenderer spriteRenderer = symbol.GetComponent<SpriteRenderer>();
                if (spriteRenderer != null)
                {
                ...
                rectTransform.localScale = Vector3.one * 50f; // Adjust scale for UI
                
                // Return the original to the pool and use the UI version
                symbol.SetActive(false);
            ...
            {
                // Use existing UI chitin symbol
                RectTransform rectTransform = symbol.GetComponent<RectTransform>();
                rectTransform.position = spawnScreenPos;
            }
            
            symbol.SetActive(true);
            StartCoroutine(AnimateChitinInUISpace(symbol, nestScreenPos));

            yield return new WaitForSeconds(chitinSpawnDelay);
        }
    }

    private IEnumerator AnimateChitinInUISpace(GameObject symbol, Vector2 targetScreenPos)
    {
        RectTransform rectTransform = symbol.GetComponent<RectTransform>();
        if (rectTransform == null)
        {
            Debug.LogError("Symbol doesn't have a RectTransform for UI animation!");
            yield break;
        }
        ...
        if (image == null)
        {
            Debug.LogError("Symbol doesn't have an Image component for UI animation!");
            yield break;
        }
        
        Vector2 startPos = rectTransform.position;
        Vector2 endPos = targetScreenPos;
        Color startColor = image.color;
        float elapsed = 0f;

        // Create midpoint for arc
        Vector2 midPoint = Vector2.Lerp(startPos, endPos, 0.5f);
        midPoint.y += Random.Range(100f, 200f); // Arc height in UI space
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            // Bezier curve movement in UI space
            Vector2 a = Vector2.Lerp(startPos, midPoint, curveValue);
            Vector2 b = Vector2.Lerp(midPoint, endPos, curveValue);
            rectTransform.position = Vector2.Lerp(a, b, curveValue);
            ...
            {
                // Play sound
                SoundEffectManager.Instance.PlaySound("SymbolToNest", Vector3.zero, true);
        ...
        // Return to pool or destroy if it's a UI-specific version
        if (symbolPools["chitin"].Contains(symbol))
        {
            symbolPools["chitin"].Enqueue(symbol);
        }
        else
