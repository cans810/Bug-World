...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        Debug.Log($"[CRITICAL] DepositChitinAtBase called with amount: {amount}, current count: {ChitinCount}");
        
        if (amount <= 0 || ChitinCount < amount)
        {
            Debug.LogWarning($"[CRITICAL] Cannot deposit chitins: amount={amount}, current count={ChitinCount}");
            return;
        }
        
        // Store the amount for animation
        int amountToAnimate = amount;
        ...
        if (uiHelper != null)
        {
            Debug.Log($"[CRITICAL] Calling DebugAnimateChitins with amount: {amountToAnimate}");
            
            // Start animation and wait for it to complete before updating inventory
            StartCoroutine(AnimateAndThenUpdateInventory(uiHelper, amountToAnimate, amount));
        }
        else
        {
            Debug.LogError("[CRITICAL] Could not find UIHelper!");
            
            // If no UIHelper, just update inventory immediately
            UpdateInventoryAfterDeposit(amount);
        }
        
        Debug.Log($"[CRITICAL] Deposit initiated. Animating {amountToAnimate} chitins.");
    }

    // New coroutine to handle the animation and inventory update sequence
    private IEnumerator AnimateAndThenUpdateInventory(UIHelper uiHelper, int animationAmount, int depositAmount)
    {
        // Start the animation
        uiHelper.DebugAnimateChitins(animationAmount);
        
        // Wait for animation to complete (approximately)
        // The animation takes about 0.2s setup + 0.1s per icon + 1.0s for movement
        float animationDuration = 0.2f + (0.1f * animationAmount) + 1.0f;
        yield return new WaitForSeconds(animationDuration);
        
        // Now update the inventory
        UpdateInventoryAfterDeposit(depositAmount);
        
        Debug.Log($"[CRITICAL] Deposit completed after animation. Updated inventory for {depositAmount} chitins.");
    }

    // Helper method to update inventory after deposit
    private void UpdateInventoryAfterDeposit(int amount)
    {
        // Remove chitin from inventory
        ChitinCount -= amount;
    ...
    public void DepositAllChitinAtBase()
    {
        ...
        Debug.Log($"Depositing ALL chitins: {chitinToDeposit}");
        
        if (chitinToDeposit <= 0)
        {
            Debug.LogWarning("No chitins to deposit");
            return;
        }
