    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (currentCrumbs > 0)
        {
            ...
            if (antIncubator != null)
            {
                ...
                // If egg creation was successful or we couldn't create an egg,
                // deposit the crumbs anyway to get XP and reset the count
                ...
                if (showDebugMessages)
                {
                    Debug.Log($"Deposited {currentCrumbs} crumbs! XP gained: {currentCrumbs * 5}!");
                    if (eggCreated) 
                    {
                        Debug.Log("Created an ant egg!");
                    }
                }
            }
            else
            {
                // If no incubator found, just deposit the crumbs for XP
                playerInventory.DepositAllCrumbsAtBase();
                
                if (showDebugMessages)
                {
                    Debug.Log($"Deposited {currentCrumbs} crumbs, but no incubator found to create eggs.");
                }
            }
        }
    }
