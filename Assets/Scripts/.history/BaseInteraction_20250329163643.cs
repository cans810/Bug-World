...
public class BaseInteraction : MonoBehaviour
{
    ...
    // Add this field to the class (at the top with other fields)
    [SerializeField] private int experiencePerChitin = 1; // XP gained per chitin deposited
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        {
            // Change circle image color back to default
            if (CircleImage != null)
            {
            ...
            // Stop healing if no allies need healing
            bool anyAllyNeedsHealing = false;
            foreach (LivingEntity ally in alliesInBase)
            {
                if (ally != null && !ally.IsDead && ally.CurrentHealth < ally.MaxHealth)
                {
                    anyAllyNeedsHealing = true;
                    break;
                }
            }
            
            if (!anyAllyNeedsHealing)
            {
                StopHealing();
            }
        ...
        if (allyAI != null)
        {
            ...
            {
                // Restore original outline color
                if (allyOutlines.ContainsKey(allyEntity))
                {
                    EntityOutline allyOutline = allyOutlines[allyEntity];
                    if (allyOutline != null && originalAllyColors.ContainsKey(allyEntity))
                    {
                        allyOutline.SetOutlineColor(originalAllyColors[allyEntity]);
                    }
                ...
                // Stop healing if no entities need healing
                if (playerEntity == null || playerEntity.CurrentHealth >= playerEntity.MaxHealth)
                {
                    bool anyAllyNeedsHealing = false;
                    foreach (LivingEntity ally in alliesInBase)
                    {
    ...
    {
        // Use the new unified chitin-to-XP animation system if available
        if (visualEffectManager != null)
        {
            // Get the chitin amount from XP amount (they should be proportional)
            int chitinAmount = xpAmount / experiencePerChitin;
            if (chitinAmount <= 0) chitinAmount = 1; // Ensure at least 1 chitin is shown
            
            // Play the new combined animation sequence
            visualEffectManager.PlayNewChitinCollectionAnimation(chitinAmount, xpAmount);
            
            // Wait for animation to complete (estimate based on total duration)
            float animationDuration = 2.5f; // Total time for chitin->center->XP->XPpanel animation
            yield return new WaitForSeconds(animationDuration);
            
            // THIS IS WHERE WE NEED TO ADD THE XP (only once!)
            // Add the XP directly to the player inventory
            if (playerInventory != null)
            {
                // Use AddXP instead of AddExperienceGradually
                ...
                if (uiHelper != null)
                {
                    uiHelper.UpdateExperienceDisplay(playerInventory.Experience, false);
                }
            }
        }
        else
        {
            // Fallback to original behavior if visual effect manager isn't available
            // Wait for chitins to reach the nest (based on the animation duration)
            yield return new WaitForSeconds(1.5f); // Default duration
            
            // Add the XP directly without animation
            if (playerInventory != null)
            {
                playerInventory.AddXP(xpAmount);
    ...
    // Add this method to play deposit sound
    private void PlayDepositSound()
    {
        // Play sound effect using SoundEffectManager if available
        if (useSoundEffectManager && SoundEffectManager.Instance != null)
        {
    ...
    // Fix the ProcessChitinDeposit method
    private void ProcessChitinDeposit(int chitinCount)
    {
        if (playerInventory == null) return;
        
        // Calculate XP amount
        int xpAmount = chitinCount * experiencePerChitin;
        
        // Store initial XP value
        int startXP = playerInventory.Experience;
        int targetXP = startXP + xpAmount;
        
        // Trigger event for other systems
        OnChitinDeposited?.Invoke(chitinCount);
        
        // Play sound effect for deposit
        PlayDepositSound();
        
        // Instead of spawning and animating chitin symbols, use the new animation system
        StartCoroutine(PlayXPGainAfterChitinDeposit(xpAmount, startXP, targetXP));
        
        // Directly update UI to show zero chitin
        UIHelper uiHelper = FindObjectOfType<UIHelper>();
        if (uiHelper != null)
        {
            // Fix the UpdateChitinDisplay call to match its signature
            uiHelper.UpdateChitinDisplay(0);
        }
    }
} 