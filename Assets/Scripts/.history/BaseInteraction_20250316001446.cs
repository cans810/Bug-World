...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount)
    {
        ...
        if (visualEffectManager != null)
        {
            ...
            // Use direct PlayXPGainEffect with the known nest position instead of trying to find by tag
            if (nestTransform != null)
            {
                visualEffectManager.PlayXPGainEffect(nestTransform.position, xpAmount);
            }
            else if (actualNestGameObject != null)
            {
                visualEffectManager.PlayXPGainEffect(actualNestGameObject.transform.position, xpAmount);
            }
            else
            {
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (currentCrumbs > 0)
        {
            if (antIncubator != null)
            {
                ...
                {
                    // Spawn visual effects for the crumbs being used
                    if (visualEffectManager != null)
                    {
                        visualEffectManager.SpawnEffect("crumb", playerInventory.transform.position, requiredCrumbs);
                    }
