...
public class ChitinDropper : MonoBehaviour
{
    ...
    [SerializeField] private float upwardForceMin = 0.2f;  // DRAMATICALLY reduced
    [SerializeField] private float upwardForceMax = 0.5f;  // DRAMATICALLY reduced
    [SerializeField] private float horizontalForce = 0.3f; // DRAMATICALLY reduced
    [SerializeField] private float explosionRadius = 0.2f; // DRAMATICALLY reduced
    ...
    private IEnumerator SimpleChitinAnimation(GameObject chitinObject)
    {
        ...
        // Calculate a random landing position with MORE SPREAD
        float angle = Random.Range(0, 360) * Mathf.Deg2Rad;
        Vector3 offset = new Vector3(
            Mathf.Cos(angle) * Random.Range(0.1f, 0.4f), // INCREASED from (0.05f, 0.2f)
            0,
            Mathf.Sin(angle) * Random.Range(0.1f, 0.4f)  // INCREASED from (0.05f, 0.2f)
        );
        
        // End position - on the ground with small random offset
        Vector3 endPos = new Vector3(startPos.x + offset.x, 0.05f, startPos.z + offset.z);
        
        // Calculate peak position with HIGHER arc
        float peakHeight = Random.Range(0.2f, 0.35f); // INCREASED from (0.1f, 0.2f)
        Vector3 peakPos = (startPos + endPos) * 0.5f + Vector3.up * peakHeight;
        
        // Animate over time (slightly longer duration)
        float duration = 0.4f; // INCREASED from 0.3f
        float elapsed = 0;
        ...
        while (elapsed < duration)
        {
            ...
            // Simple rotation
            chitinObject.transform.Rotate(Vector3.up, 90 * Time.deltaTime);
