...
public class AIWandering : MonoBehaviour
{
    ...
    [SerializeField] private float borderDetectionDistance = 3f; // How far ahead to check for borders
    [SerializeField] private float borderAvoidanceAngle = 45f; // Angle to check for borders
    [SerializeField] private LayerMask borderLayer; // Layer for border colliders
    ...
    private IEnumerator WanderingRoutine()
    {
        ...
        while (true)
        {
            ...
            targetWaypoint.y = fixedYPosition;

            // Check if path to waypoint would cross a border
            Vector3 waypointDirection = (targetWaypoint - transform.position).normalized;
            if (CheckForBorders(waypointDirection))
            {
                // If border detected, get a new waypoint in the opposite direction
                Vector3 awayDirection = -waypointDirection;
                targetWaypoint = GetSafeWaypointInDirection(awayDirection);
                targetWaypoint.y = fixedYPosition;
            }
    ...
    private IEnumerator MoveToWaypointRoutine(Vector3 waypoint)
    {
        ...
        {
            // Check for borders in current movement direction
            if (CheckForBorders(transform.forward))
            {
                // If border detected, stop and get a new waypoint
                Debug.Log($"{gameObject.name}: Detected border, changing direction");
                StartCoroutine(TurnAndMoveInward());
                yield break;
            }
    ...
    // Add this method to check for borders in front of the enemy
    private bool CheckForBorders(Vector3 direction)
    {
        // Main forward ray
        if (Physics.Raycast(transform.position, direction, borderDetectionDistance, borderLayer))
        {
            return true;
        }

        // Check left and right angles
        Vector3 leftDir = Quaternion.Euler(0, -borderAvoidanceAngle, 0) * direction;
        if (Physics.Raycast(transform.position, leftDir, borderDetectionDistance, borderLayer))
        {
            return true;
        }

        Vector3 rightDir = Quaternion.Euler(0, borderAvoidanceAngle, 0) * direction;
        if (Physics.Raycast(transform.position, rightDir, borderDetectionDistance, borderLayer))
        {
            return true;
        }
    ...
    // Add this new method to get a safe waypoint in a specific direction
    private Vector3 GetSafeWaypointInDirection(Vector3 direction)
    {
        direction.y = 0;
        direction.Normalize();

        // Calculate a safe distance that's less than the border detection distance
        float safeDistance = borderDetectionDistance * 0.8f;

        // Try up to 5 different distances
        for (int i = 0; i < 5; i++)
        {
            float distance = Random.Range(safeDistance * 0.5f, safeDistance);
            Vector3 potentialWaypoint = transform.position + direction * distance;

            // Check if this waypoint is safe from borders
            if (!CheckForBorders((potentialWaypoint - transform.position).normalized))
            {
                return potentialWaypoint;
            }
        }

        // If no safe waypoint found, return current position
