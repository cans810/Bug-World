...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Check if player is moving every frame for more responsive stopping
        if (Time.time >= lastPlayerVelocityCheckTime + playerVelocityCheckInterval)
        {
            ...
            if (lastPlayerPosition != Vector3.zero)
            {
                ...
                // Debug player movement status
                Debug.Log($"Player movement check: distance={playerMovementDistance:F3}, isMoving={isPlayerMoving}");
            }
        ...
        // IMMEDIATE STOP if player is not moving and we're close enough
        if (!isPlayerMoving && distanceToPlayer <= playerStationaryStopDistance)
        {
        ...
        // Only follow if player is moving or we're too far away
    ...
    {
        // SIMPLIFIED APPROACH: Just enable the wandering component and let it handle everything
        ...
        if (wanderingBehavior != null)
        {
            ...
            if (!wanderingBehavior.enabled)
            {
                ...
                // Force a new waypoint when first enabling
                wanderingBehavior.ForceNewWaypoint();
            }
            
            // Enable wandering - this is the critical line
            wanderingBehavior.SetWanderingEnabled(true);
            
            // Track state but don't interfere with animation control
            isMoving = wanderingBehavior.IsCurrentlyMoving();
            hasAppliedMovementThisFrame = isMoving;
        }
        else
        {
            // If no wandering component, switch to follow mode
    ...
    // Override the SetMode method to properly handle transitions
    public void SetMode(AIMode newMode)
    {
        ...
        Debug.Log($"{gameObject.name} changing mode from {currentMode} to {newMode}");
        ...
        {
            // For wandering, make sure the component is enabled and reset
            if (wanderingBehavior != null)
            {
                wanderingBehavior.enabled = true;
                wanderingBehavior.SetWanderingEnabled(true);
                ...
                // Let AIWandering handle animations now
                Debug.Log($"Wander mode: AIWandering enabled for {gameObject.name}");
            }
        }
        else
        {
            // For non-wandering modes, disable wandering
            if (wanderingBehavior != null)
            {
                wanderingBehavior.SetWanderingEnabled(false);
                
                // Special case for carrying
