...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // For X or Z axis aligned capsules, we need a stadium shape (rectangle with semicircles at ends)
        else
        {
            float halfHeight = (height - 2 * radius) * 0.5f;
            Vector3 direction1 = direction == 0 ? Vector3.right : Vector3.forward;
            Vector3 end1 = center + direction1 * halfHeight;
            Vector3 end2 = center - direction1 * halfHeight;
            
            int semicircleSegments = segments / 2;
            int pointIndex = 0;
            
            // First semicircle
            for (int i = 0; i <= semicircleSegments / 2; i++)
            {
                float angle = i * Mathf.PI / (semicircleSegments / 2);
                float x, z;
                
                if (direction == 0) // X axis
                {
                    x = end1.x;
                    z = end1.z + radius * Mathf.Sin(angle);
                }
                else // Z axis
                {
                    x = end1.x + radius * Mathf.Sin(angle);
                    z = end1.z;
                }
            ...
            for (int i = 0; i <= semicircleSegments / 2; i++)
            {
                float angle = Mathf.PI + i * Mathf.PI / (semicircleSegments / 2);
                float x, z;
                
                if (direction == 0) // X axis
                {
                    x = end2.x;
                    z = end2.z + radius * Mathf.Sin(angle);
                }
                else // Z axis
                {
                    x = end2.x + radius * Mathf.Sin(angle);
                    z = end2.z;
                }
            ...
            // Complete the loop by filling remaining points
            for (int i = pointIndex; i < segments; i++)
            {
                float t = (float)(i - pointIndex) / (segments - pointIndex);
                Vector3 position;
                
                if (direction == 0) // X axis
                {
                    float x = Mathf.Lerp(end2.x, end1.x, t);
                    float z = end1.z - radius;
                    position = new Vector3(x, perimeterHeight, z);
                }
                else // Z axis
                {
                    float x = end1.x - radius;
                    float z = Mathf.Lerp(end2.z, end1.z, t);
                    position = new Vector3(x, perimeterHeight, z);
                }
