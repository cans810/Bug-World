...
public class ChitinCollectible : MonoBehaviour
{
    ...
    // Add static counter to track chitins in collection process
    private static int chitinsBeingCollected = 0;
    ...
    public bool TryCollect()
    {
        ...
        // Thread-safe check for collection capacity
        bool canCollect = false;
        lock (collectionLock)
        {
            // Check both current inventory and chitins being collected
            int currentChitin = playerInventory.ChitinCount;
            int projectedTotal = currentChitin + chitinsBeingCollected + 1; // +1 for this one
            
            if (projectedTotal <= playerInventory.MaxChitinCapacity)
            {
                // We're within capacity limits, mark as being collected
                chitinsBeingCollected++; // Increment counter
                canCollect = true;
                
                // Debug info
                Debug.Log($"Chitin collection check: Current={currentChitin}, BeingCollected={chitinsBeingCollected}, " +
                          $"ProjectedTotal={projectedTotal}, MaxCapacity={playerInventory.MaxChitinCapacity}");
            }
            else
            {
                Debug.Log($"Cannot collect chitin: would exceed capacity. Current={currentChitin}, " +
                          $"BeingCollected={chitinsBeingCollected}, MaxCapacity={playerInventory.MaxChitinCapacity}");
            }
        }
        
        if (!canCollect)
        {
            // Player inventory will be full after all current collections
            ...
            // Trigger the "maxed" event to show UI feedback
            if (playerInventory.IsChitinFull || (playerInventory.ChitinCount + chitinsBeingCollected >= playerInventory.MaxChitinCapacity))
            {
                // Using reflection to access private event
                var onChitinMaxedField = typeof(PlayerInventory).GetField("OnChitinMaxed", 
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                
                if (onChitinMaxedField != null)
                {
                    var onChitinMaxed = onChitinMaxedField.GetValue(playerInventory) as System.Action;
    ...
    private IEnumerator CollectionAnimation()
    {
        ...
        // Add to player inventory with safety check
        if (playerInventory != null)
        {
            // Final check that we're still under capacity
            ...
            {
                Debug.LogWarning($"Chitin not added: inventory full ({playerInventory.ChitinCount}/{playerInventory.MaxChitinCapacity})");
            }
        }
        else
        {
        ...
        lock (collectionLock)
        {
            chitinsBeingCollected--;
            // Ensure counter never goes below zero (defensive programming)
            chitinsBeingCollected = Mathf.Max(0, chitinsBeingCollected);
            Debug.Log($"Chitin collection completed. Remaining in process: {chitinsBeingCollected}");
        }
    ...
    {
        // If this object is being destroyed while still in the collection process,
        // ensure we decrement the counter to prevent capacity calculation errors
        if (isBeingCollected && !isAnimationRunning)
        {
            lock (collectionLock)
            {
                chitinsBeingCollected--;
                chitinsBeingCollected = Mathf.Max(0, chitinsBeingCollected);
                Debug.Log($"Chitin being collected was destroyed early. Remaining in process: {chitinsBeingCollected}");
            }
        }
