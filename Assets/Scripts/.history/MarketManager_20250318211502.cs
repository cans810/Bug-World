using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using TMPro;
using UnityEngine.UI;
using UnityEngine.Purchasing;
using UnityEngine.SceneManagement;

public class MarketManager : MonoBehaviour, IStoreListener
{
    [System.Serializable]
    public class MarketItem
    {
        public string productId;          // Product ID in Google Play
        public TextMeshProUGUI priceText; // UI text to display price
        public string defaultPrice = "$0.99"; // Default price to show before initialization
    }
    
    [SerializeField] private List<MarketItem> marketItems = new List<MarketItem>();
    
    private IStoreController storeController;
    private IExtensionProvider extensionProvider;

    public PlayerInventory playerInventory;

    public TextMeshProUGUI coinsText;

    public GameObject MarketPanel;

    public TextMeshProUGUI Point250PriceText;
    public TextMeshProUGUI Point250CurrencyText;

    public TextMeshProUGUI Point500PriceText;
    public TextMeshProUGUI Point500CurrencyText;

    public TextMeshProUGUI Point750PriceText;
    public TextMeshProUGUI Point750CurrencyText;

    public TextMeshProUGUI Point1000PriceText;
    public TextMeshProUGUI Point1000CurrencyText;


    // Example point packages
    public readonly Dictionary<string, int> coinPackages = new Dictionary<string, int>
    {
        { "coin250", 250 },
        { "coin500", 500 },
        { "coin750", 750 },
        { "coin1000", 1000 },
    };

    // Add this field to track initialization status
    private bool isInitialized = false;

    // Start is called before the first frame update
    private void Start()
    {
        InitializePurchasing();
    }

    public void InitializePurchasing()
    {
        if (isInitialized) return;
        
        var builder = ConfigurationBuilder.Instance(StandardPurchasingModule.Instance());
        
        // Add products to the builder
        foreach (var item in marketItems)
        {
            builder.AddProduct(item.productId, ProductType.Consumable);
        }
        
        UnityPurchasing.Initialize(this, builder);
    }

    public bool IsInitialized()
    {
        return isInitialized && storeController != null;
    }

    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        storeController = controller;
        extensionProvider = extensions;
        isInitialized = true;
        
        Debug.Log("IAP Initialization successful!");
        
        // Update UI with the correct prices immediately
        UpdateProductPricesInUI();
    }

    public void OnInitializeFailed(InitializationFailureReason error)
    {
        Debug.LogError($"IAP Initialization failed: {error}");
    }

    public void OnInitializeFailed(InitializationFailureReason error, string message)
    {
        Debug.LogError($"IAP Initialization failed: {error}. Message: {message}");
    }

    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        string productId = args.purchasedProduct.definition.id;
        Debug.Log($"Processing purchase: {productId}");

        if (coinPackages.ContainsKey(productId))
        {
            Debug.Log($"Processing coins purchase for {productId}");
            int coinsToAdd = coinPackages[productId];
            
            // Get PlayerInventory and add coins
            if (playerInventory != null)
            {
                bool success = playerInventory.AddCoins(coinsToAdd);
                if (success)
                {
                    Debug.Log($"Coins purchase completed. New total: {playerInventory.CoinCount}");
                    UpdateCoinsDisplay(); // Update the UI immediately
                }
                else
                {
                    Debug.LogError("Failed to add coins to player inventory");
                }
            }
            else
            {
                Debug.LogError("PlayerInventory reference is null!");
            }
        }
        else
        {
            Debug.LogWarning($"Unknown product ID: {productId}");
        }

        return PurchaseProcessingResult.Complete;
    }

    public void OnPurchaseFailed(Product product, PurchaseFailureReason failureReason)
    {
        Debug.LogError($"Purchase failed: {product.definition.id}, reason: {failureReason}");
    }

    public void AddCoins(int coins)
    {
        if (playerInventory == null)
        {
            Debug.LogError("PlayerInventory reference is null!");
            return;
        }

        Debug.Log($"=== Adding Coins from Market ===");
        Debug.Log($"Current coins before adding: {playerInventory.CoinCount}");
        Debug.Log($"Coins to add: {coins}");

        bool success = playerInventory.AddCoins(coins);
        
        if (success)
        {
            Debug.Log($"New total coins: {playerInventory.CoinCount}");
            UpdateCoinsDisplay();
        }
        else
        {
            Debug.LogError("Failed to add coins to player inventory");
        }
    }

    public void UpdateCoinsDisplay()
    {
        if (coinsText != null)
        {
            coinsText.text = playerInventory.CoinCount.ToString();
        }
    }

    // Method to update UI with localized prices from Google Play
    public void UpdateProductPricesInUI()
    {
        if (!IsInitialized())
        {
            Debug.LogWarning("Cannot update prices - IAP not initialized");
            return;
        }
        
        foreach (var item in marketItems)
        {
            Product product = storeController.products.WithID(item.productId);
            
            if (product != null && product.availableToPurchase)
            {
                // Set the price text with the localized price from Google Play
                // This automatically includes the correct currency symbol
                if (item.priceText != null)
                {
                    item.priceText.text = product.metadata.localizedPriceString;
                    Debug.Log($"Updated price for {product.definition.id}: {product.metadata.localizedPriceString}");
                }
            }
            else
            {
                // If product info isn't available yet, use the default price
                if (item.priceText != null)
                {
                    item.priceText.text = item.defaultPrice;
                    Debug.Log($"Using default price for {item.productId}: {item.defaultPrice}");
                }
            }
        }
    }

    // Method to make a purchase
    public void BuyProduct(string productId)
    {
        if (!IsInitialized())
        {
            Debug.LogError("Cannot purchase - IAP not initialized");
            return;
        }
        
        Product product = storeController.products.WithID(productId);
        
        if (product != null && product.availableToPurchase)
        {
            Debug.Log($"Purchasing product: {product.definition.id}");
            storeController.InitiatePurchase(product);
        }
        else
        {
            Debug.LogError($"Cannot purchase product: {productId}. Not available for purchase.");
        }
    }

    public void HideMarketPanel(){
        MarketPanel.SetActive(false);
    }

    public void ShowMarketPanel(){
        MarketPanel.SetActive(true);
        UpdateCoinsDisplay();
    }
}
