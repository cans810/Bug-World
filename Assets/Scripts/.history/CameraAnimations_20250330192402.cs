...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator AnimateCameraToArea(Transform areaTarget, int level = 0, string areaName = "", bool isEgg = false)
    {
        ...
        float useHeightOffset = isEgg ? 4.0f : heightOffset;  // Higher for eggs to get better angle
        float useLookDownAngle = isEgg ? 45f : lookDownAngle; // Less steep angle for better egg viewing
        ...
        if (isEgg)
        {
            ...
            // For eggs, add a rotating camera movement during the view duration
            float rotationTimer = 0f;
            float startingYRotation = cameraTransform.eulerAngles.y;
            
            // Calculate a full 180-degree rotation around the egg
            while (rotationTimer < useAreaViewDuration)
            {
                rotationTimer += Time.deltaTime;
                float rotationProgress = rotationTimer / useAreaViewDuration;
                
                // Calculate new Y rotation angle - rotate 180 degrees around the egg
                float newYAngle = startingYRotation + (180f * rotationProgress);
                
                // Apply the rotation around the egg while maintaining height and distance
                float radius = Vector3.Distance(new Vector3(areaPosition.x, 0, areaPosition.z), 
                                               new Vector3(cameraTransform.position.x, 0, cameraTransform.position.z));
                
                // Calculate new position based on angle
                float newX = areaPosition.x + (Mathf.Sin(newYAngle * Mathf.Deg2Rad) * radius);
                float newZ = areaPosition.z + (Mathf.Cos(newYAngle * Mathf.Deg2Rad) * radius);
                
                // Update camera position
                cameraTransform.position = new Vector3(newX, cameraTransform.position.y, newZ);
                
                // Always look at the egg
                cameraTransform.LookAt(areaPosition);
    ...
    // Coroutine to animate camera to newly hatched insect - with dynamic camera movement
    private IEnumerator AnimateCameraToNewInsect(Transform insectTransform)
    {
        ...
        // Starting position - to the right and slightly behind
        Vector3 startViewPosition = insectPosition + 
                               Vector3.right * 2.0f +          // To the right
                               Vector3.up * 2.5f +             // Higher up
                               insectTransform.forward * 1.0f; // Slightly behind
        
        // Ending position - to the left and slightly in front
        Vector3 endViewPosition = insectPosition + 
                               Vector3.left * 2.0f +           // To the left
                               Vector3.up * 2.5f +             // Same height
                               insectTransform.forward * -1.0f; // Slightly in front
        
        // Calculate rotation to look at the insect from starting position
        Quaternion startViewRotation = Quaternion.LookRotation(insectPosition - startViewPosition);
        ...
        // Animate to the starting viewing position
        ...
        while (elapsedTime < animationDuration)
        {
            ...
            cameraTransform.position = Vector3.Lerp(originalPosition, startViewPosition, t);
            cameraTransform.rotation = Quaternion.Slerp(originalRotation, startViewRotation, t);
            
            yield return null;
        }
        
        // Play the Pickup3 sound effect
        SoundEffectManager.Instance?.PlaySound("Pickup3");
        
        // Now move camera from start to end position over the hold duration
        float orbitalDuration = 3.0f;
        elapsedTime = 0f;
        
        // Save starting position/rotation since we're now fully at start position
        Vector3 currentPos = cameraTransform.position;
        Quaternion currentRot = cameraTransform.rotation;
        
        while (elapsedTime < orbitalDuration)
        {
            elapsedTime += Time.deltaTime;
            float t = Mathf.SmoothStep(0, 1, elapsedTime / orbitalDuration);
            
            // Move camera in arc around the insect rather than straight line
            Vector3 newPosition = Vector3.Lerp(startViewPosition, endViewPosition, t);
            
            // Add slight arc to camera path by raising the camera in the middle of the path
            float arcHeight = 1.0f; // Height of the arc
            float arcOffset = Mathf.Sin(t * Mathf.PI) * arcHeight; // Peaks in the middle
            newPosition.y += arcOffset; // Apply the arc offset to the Y position
            
            // Update camera position
            cameraTransform.position = newPosition;
            
            // Always look at the insect
            cameraTransform.LookAt(insectPosition);
        ...
        while (elapsedTime < returnDuration)
        {
            ...
            cameraTransform.position = Vector3.Lerp(currentPos, originalPosition, t);
            cameraTransform.rotation = Quaternion.Slerp(currentRot, originalRotation, t);
        ...
        // Force enable ALL insect AI components in the scene
        ForceEnableAllInsectAI();
