...
public class AllyAI : MonoBehaviour
{
    ...
    // Add this field to AllyAI
    private AIWanderingExtension wanderingExtension;
    ...
    private void Start()
    {
        ...
        // Get wandering extension
        wanderingExtension = GetComponent<AIWanderingExtension>();
    }
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // If player is not moving and we're close enough, just idle
        if (!isPlayerMoving && distanceToPlayer <= followDistance * 1.5f)
        {
            ...
            // Make sure wandering is disabled
            if (wanderingBehavior != null)
                wanderingBehavior.SetWanderingEnabled(false);
    ...
    private void DropLoot(Vector3 dropPosition)
    {
        ...
        // Ensure the layer is set to CarriedLoot
        ...
        // Re-enable collider to allow player collection
        ...
        // Re-enable the ChitinCollectible component if it exists
        ChitinCollectible chitinCollectible = carriedLoot.GetComponent<ChitinCollectible>();
        if (chitinCollectible != null)
        {
            chitinCollectible.enabled = true;
        }
        
        // Store a reference to the dropped loot to avoid immediately re-targeting it
        Transform droppedLoot = carriedLoot;
        ...
        if (originalMode == AIMode.Wander && wanderingBehavior != null)
        {
            ...
            // Instead of calling ForceNewWanderTarget, use this:
            if (wanderingExtension != null)
            {
                wanderingExtension.ForceNewWanderTarget();
            }
            else
            {
                ...
                if (baseTransform != null)
                {
                    // Try to move away from the base
                    Vector3 directionFromBase = (transform.position - baseTransform.position).normalized;
                    if (directionFromBase.magnitude > 0.1f)
                    {
                        // Blend random direction with direction away from base
                        randomDirection = (randomDirection + directionFromBase * 2f).normalized;
                    }
                }
                
                // Apply the movement
                transform.position += randomDirection * 0.75f;
                Debug.Log($"{gameObject.name} forced to move in direction {randomDirection}");
                
                // Reset wandering state if possible
                if (wanderingBehavior != null)
                {
                    wanderingBehavior.SetWanderingEnabled(true);
                }
        ...
        // Move the ally slightly away from the drop position to avoid getting stuck
        Vector3 moveAwayDirection = (transform.position - dropPosition).normalized;
        if (moveAwayDirection.magnitude < 0.1f)
        {
            // If we're exactly at the drop position, pick a random direction
            moveAwayDirection = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
        }
        
        // Apply a small movement away from the drop point
        transform.position += moveAwayDirection * 0.5f;
        ...
        // Start a short coroutine to ensure the ally moves away from the base
        StartCoroutine(EnsureMoveAwayFromBase(droppedLoot));
    }

    // Add this new coroutine to ensure the ally moves away from the base
    private IEnumerator EnsureMoveAwayFromBase(Transform droppedLoot)
    {
        // Wait a short time to let the physics and AI update
        yield return new WaitForSeconds(0.2f);
        
        // If we're still near the base and in Wander mode, force a move away
        if (baseTransform != null && Vector3.Distance(transform.position, baseTransform.position) < 2.0f 
            && currentMode == AIMode.Wander && wanderingBehavior != null)
        {
            // Force the wandering behavior to pick a target away from the base
            wanderingBehavior.SetWanderingEnabled(true);
            
            // Apply a small movement away from the base
            Vector3 moveAwayDirection = (transform.position - baseTransform.position).normalized;
            if (moveAwayDirection.magnitude < 0.1f)
            {
                // If we're exactly at the base position, pick a random direction
                moveAwayDirection = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
            }
            ...
            Debug.Log($"{gameObject.name} forced to move away from base after dropping loot");
        }
        
        // Ensure we don't immediately target the loot we just dropped
        yield return new WaitForSeconds(1.0f);
        
        // Reset any stuck state
        if (!isMoving && currentMode == AIMode.Wander && wanderingBehavior != null)
        {
            wanderingBehavior.SetWanderingEnabled(true);
            Debug.Log($"{gameObject.name} was potentially stuck, forcing new wander target");
        }
    }
