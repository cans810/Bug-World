...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    // Modify the Start method to get a reference to PlayerController
    private void Start()
    {
    ...
    // Modify the ShowPanel method to disable player movement
    public void ShowPanel()
    {
        // Disable player movement when showing reward panel
        if (playerController != null)
        {
            playerController.SetCanMove(false);
            Debug.Log("Disabled player movement for rewards panel");
        }
        
        // Existing code for showing the panel...
        gameObject.SetActive(true);
        
        // Set initial panel state
        SetPanelState(PanelState.Initial);
        
        // Play panel appear sound
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("PanelAppear");
        }
    }

    // Modify the ClosePanel method to re-enable player movement
    public void ClosePanel()
    {
        // Re-enable player movement when closing the panel
        if (playerController != null)
        {
            playerController.SetCanMove(true);
            Debug.Log("Re-enabled player movement after rewards claimed");
        }
        
        // Existing code for closing the panel...
        gameObject.SetActive(false);
        
        // Notify that panel is closed
