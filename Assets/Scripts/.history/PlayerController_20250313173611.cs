...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        // If we have movement input, update rotation using the living entity's method
        if (moveDirection.magnitude > 0.1f)
        {
            // Use the centralized rotation method for consistency
            if (livingEntity != null)
            {
                livingEntity.RotateTowards(moveDirection, 2.0f); // Double rotation speed for responsiveness
            }
            else
            {
