...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        if (visualEffectManager != null)
        {
            ...
            if (uiEffects != null)
            {
                // Trigger the UI-based chitin deposit effect (pass both chitin and XP amounts)
                uiEffects.PlayChitinDepositEffect(playerInventory.ChitinCount, xpAmount);
                Debug.Debug.LogError();("chitin count: " + playerInventory.ChitinCount);
            ...
            {
                // Fallback if no UI effects controller
                Debug.Log($"XP Gained: {xpAmount}");
