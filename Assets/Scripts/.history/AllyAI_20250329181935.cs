...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float maxDistanceFromPlayer = 10f; // Maximum distance before ally rushes back to player
    [SerializeField] private float returnSpeedMultiplier = 1.5f; // Speed multiplier when returning to player
    private bool isReturningToPlayer = false; // Track if ally is currently returning to player
    ...
    private void Update()
    {
        ...
        isReturningToPlayer = distanceToPlayer > maxDistanceFromPlayer;
        
        // If too far from player, prioritize returning to player
    ...
    private void MoveToFollowPosition()
    {
        ...
        // Rotate and move to get to the ideal position
    ...
    {
        // Skip if auto-attack is disabled, enemy is null/dead, or we're returning to player
        if (!enableAutoAttack || enemy == null || enemy.IsDead || isReturningToPlayer) 
            return;
    ...
    private IEnumerator AutoAttackCoroutine()
    {
        ...
        {
            // If we're returning to player, stop attacking
            if (isReturningToPlayer)
            {
                if (showDebugInfo)
                    Debug.Log($"Ally {gameObject.name}: Pausing attacks while returning to player");
    ...
    // New method to handle returning to player when too far away
    private void ReturnToPlayer()
    {
        ...
        if (useMovementSmoothing)
        {
            smoothedMoveDirection = Vector3.SmoothDamp(
                ...
                directionToPlayer, 
                ref directionVelocity, 
                movementSmoothTime * 0.5f); // Faster smoothing when returning
                
            directionToPlayer = smoothedMoveDirection;
        }
        ...
        livingEntity.RotateTowards(directionToPlayer, 5.0f);
        
        // Move faster when returning to player
        livingEntity.MoveInDirection(directionToPlayer, returnSpeedMultiplier);
        ...
        if (showDebugInfo)
        {
            Debug.Log($"Ally {gameObject.name}: Returning to player, distance: {Vector3.Distance(transform.position, playerTransform.position):F2}");
            Debug.DrawLine(transform.position, playerTransform.position, Color.red);
        }
    }

    // Add this method to force enable AI movement
    public void ForceEnableMovement()
    {
        ...
        // Log the status
        Debug.Log($"Force enabled movement for {gameObject.name}");
