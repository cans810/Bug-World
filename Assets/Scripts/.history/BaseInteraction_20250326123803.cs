...
public class BaseInteraction : MonoBehaviour
{
    ...
    private IEnumerator PlayXPGainAfterChitinDeposit(int xpAmount, int startXP, int targetXP)
    {
        ...
        if (visualEffectManager != null)
        {
            ...
            // No need to animate the XP counter, since AddExperienceGradually will handle it
            // Just make sure UI has latest values
            UIHelper uiHelper = FindObjectOfType<UIHelper>();
            if (uiHelper != null)
            {
        ...
        if (currentCrumbs > 0)
        {
            ...
            if (!hasEquippedEntity)
            {
                ...
                if (UIHelper.Instance != null)
                {
                    ...
                    // Add this line to show the pulsing attribute arrow
                    UIHelper.Instance.ShowPulsingAttributeArrow();
                }
                return;
            }
