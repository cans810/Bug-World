...
public class EnemyAI : MonoBehaviour
{
    ...
    // Add this field to track the center of the play area
    private Vector3 playAreaCenter = Vector3.zero;
    ...
    private void Start()
    {
        ...
        // Set the play area center - assuming it's at the origin or using the nest as reference
        if (playerNestTransform != null)
        {
            // Use the nest position as a reference point for the center
            playAreaCenter = new Vector3(playerNestTransform.position.x, 0, playerNestTransform.position.z);
        }
    ...
    // Modify the OnTriggerExit method to use stronger centering behavior
    private void OnTriggerExit(Collider other)
    {
        // Check if the collider is in the MapBorder layer
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            // Vector pointing from the enemy toward the center of the play area
            Vector3 directionToCenter = (playAreaCenter - transform.position).normalized;
            directionToCenter.y = 0; // Keep on horizontal plane
            
            // Use a stronger push toward the center
            transform.position += directionToCenter * 1.0f;
            
            // For reflection, blend between standard reflection and center direction
            Vector3 currentDirection = transform.forward;
            
            // Get the standard reflection normal from the boundary
            Vector3 boundaryNormal = (other.bounds.center - transform.position).normalized;
            boundaryNormal.y = 0;
            
            // Calculate standard reflection direction
            Vector3 standardReflection = Vector3.Reflect(currentDirection, boundaryNormal);
            
            // Blend between reflection and center direction (70% reflection, 30% center bias)
            Vector3 reflectionDirection = Vector3.Lerp(standardReflection, directionToCenter, 0.3f);
            
            // Normalize the direction
            reflectionDirection.Normalize();
            
            // Add slight randomness
            reflectionDirection = Quaternion.Euler(0, Random.Range(-5f, 5f), 0) * reflectionDirection;
            
            // Set new rotation
            transform.rotation = Quaternion.LookRotation(reflectionDirection);
            
            // Tell wandering behavior to use this new direction with center bias
            if (wanderingBehavior != null)
            {
                wanderingBehavior.SetWanderingEnabled(true);
                wanderingBehavior.ForceNewWaypointInDirection(reflectionDirection);
                
                // Also suggest using smaller wander distances when near the boundary
                float distanceFromCenter = Vector3.Distance(transform.position, playAreaCenter);
                float maxDistance = 6f; // Assuming this is a reasonable max distance in your map
                
                // The closer to the edge, the smaller the wander distance should be
                float centralityFactor = Mathf.Clamp01(1 - (distanceFromCenter / maxDistance));
                wanderingBehavior.SetWanderDistances(2f, 3f + (centralityFactor * 4f));
            }
        }
    }
} 