...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    private ArrowIndicator arrowIndicator; // Reference to the arrow indicator
    ...
    private void Start()
    {
        ...
        // Extra debug to verify subscription
        Debug.Log("LevelAreaArrowManager: Subscribing to player level up event");
        ...
        // Create the arrow indicator component
    ...
    {
        Debug.Log($"LevelAreaArrowManager: Player leveled up to {newLevel}!");
        ...
        foreach (LevelAreaTarget area in levelAreas)
        {
            ...
            if (area.requiredLevel == newLevel && !area.hasBeenVisited && area.areaTarget != null)
            {
                ...
                Debug.Log($"LevelAreaArrowManager: Triggering OnAreaUnlocked for {area.areaName}");
                
                // Trigger the area unlocked event (which will start the camera animation)
                OnAreaUnlocked?.Invoke(area.areaTarget, newLevel, area.areaName);
        ...
        if (!foundArea)
        {
            Debug.Log($"LevelAreaArrowManager: No areas found for level {newLevel}");
        }
    }
    
    private void ShowArrowToArea(LevelAreaTarget area)
    {
        if (area == null || area.areaTarget == null)
        {
            Debug.LogError("Cannot show arrow: area or target is null!");
            return;
        }
        
        Debug.Log($"ShowArrowToArea called for {area.areaName}, target: {area.areaTarget.name}");
        ...
        // Set the new target and show the arrow
        currentTarget = area.areaTarget;
        arrowIndicator.SetTarget(currentTarget);
        arrowIndicator.ShowArrow();
        isArrowActive = true;
        
        // Start coroutine to hide the arrow after duration
    ...
    public void CheckForUnlockedAreas(int playerLevel)
    {
        Debug.Log($"LevelAreaArrowManager: Checking for areas unlocked at level {playerLevel}");
        
        bool foundArea = false;
        // Loop through all level areas
        foreach (var levelArea in levelAreas)
        {
            if (levelArea.requiredLevel == playerLevel && levelArea.areaTarget != null)
            {
                foundArea = true;
                Debug.Log($"Area unlocked: {levelArea.areaName} (Level {levelArea.requiredLevel})");
                
                // Directly trigger the event
                if (OnAreaUnlocked != null)
                {
                    Debug.Log($"LevelAreaArrowManager: Directly invoking OnAreaUnlocked for {levelArea.areaName}");
                    OnAreaUnlocked.Invoke(levelArea.areaTarget, playerLevel, levelArea.areaName);
                }
                else
                {
                    Debug.LogError("LevelAreaArrowManager: OnAreaUnlocked event is null!");
                }
                break;
            }
        }
        ...
        if (!foundArea)
        {
            Debug.Log($"LevelAreaArrowManager: No areas found for level {playerLevel} in CheckForUnlockedAreas");
        }
    }
    ...
    public void TestAreaUnlocked(int level)
    {
        Debug.Log($"LevelAreaArrowManager: Testing area unlocked for level {level}");
        CheckForUnlockedAreas(level);
    }
} 