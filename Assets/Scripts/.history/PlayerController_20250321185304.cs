...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Check if the collider is a map border
        if (other.gameObject.name.Contains("MapBorder"))
        {
            string borderName = other.gameObject.name;
            
            // Get the BorderVisualizer component to check required level
            BorderVisualizer borderVisualizer = other.GetComponent<BorderVisualizer>();
            
            // Check if player has the required level to enter this area before setting the border name
            if (borderVisualizer != null)
            {
                int requiredLevel = borderVisualizer.GetRequiredLevel();
                
                // Only set the border name if player has sufficient level or if it's the default border
                if (playerInventory != null && 
                    (playerInventory.CurrentLevel >= requiredLevel || requiredLevel == 0 || borderName == "MapBorder"))
                {
                    // Update player's border object name - player is allowed to enter this area
                    PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
                    if (playerAttributes != null)
                    {
                        playerAttributes.borderObjectName = borderName;
                        Debug.Log($"Player entered border: {borderName}, border name updated");
                    }
                }
                else
                {
                    // Player doesn't have required level - don't update border name
                    Debug.Log($"Player attempted to enter {borderName} but lacks required level {requiredLevel} (current level: {playerInventory?.CurrentLevel ?? 0})");
                    
                    // Show message about required level if not on cooldown
                    if (uiHelper != null && Time.time > lastBorderMessageTime + borderMessageCooldown)
                    {
                        lastBorderMessageTime = Time.time;
                        uiHelper.ShowInformText($"Requires level {requiredLevel} to enter this area");
                        
                        // Optional: Play haptic feedback for boundary hit
                        HapticFeedback.LightFeedback();
                    }
                    
                    // Set border normal to help with collision resolution
                    borderNormal = -other.transform.forward;
                    isAtMapBorder = true;
                }
            }
            else
            {
                // If no BorderVisualizer component (fallback), still set the border name
                // This handles older/default borders without level requirements
                PlayerAttributes playerAttributes = GetComponent<PlayerAttributes>();
                if (playerAttributes != null)
                {
                    playerAttributes.borderObjectName = borderName;
                    Debug.Log($"Player entered border without visualizer: {borderName}, border name updated");
                }
            }
        }
        
        // ... rest of the OnTriggerEnter code ...
    }
