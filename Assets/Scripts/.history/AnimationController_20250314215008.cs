...
public class AnimationController : MonoBehaviour
{
    ...
    private readonly string walkAttackParam = "WalkAttack";
    ...
    private void UpdateAnimatorParameters()
    {
        ...
        // Only set walkAttack if the parameter exists
        // This is a Player-specific parameter that may not exist on other entities
        if (HasAnimatorParameter(walkAttackParam))
        {
            animator.SetBool(walkAttackParam, isWalkAttacking);
        }
    }
    ...
    public bool IsAnimationPlaying(string stateName)
    {
        switch (stateName.ToLower())
        {
            ...
            case "walkattack": return isWalkAttacking;
    ...
    public void OnAttackComplete()
    {
        ...
        if (animator != null)
        {
            ...
            animator.SetBool(walkAttackParam, false);
        }
    ...
    public void SetWalkAttacking(bool attacking)
    {
        // Skip if dead
        if (isDead) return;
        
        // Only allow the "Player" GameObject to use WalkAttack animation
        if (gameObject.name != "Player" && transform.root.name != "Player")
        {
            // For non-Player objects, just use the regular attack animation instead
            if (attacking)
            {
                SetAttacking(true);
            }
        ...
        // Continue with normal WalkAttack logic for Player only
        isWalkAttacking = attacking;
        
        if (attacking)
        {
            // When starting a walk-attack, properly manage states:
            // 1. Set attack state on
            isAttacking = true;
            // 2. Keep track of walking state but don't display it during walk-attack
            // This is intentionally not resetting isWalking, as we want to resume walking
            // after the attack is done, but we manage the animation differently
        }
        else
        {
            // When finishing walk-attack, just turn off attack states
            isAttacking = false;
        }
        
        // Update animator parameters correctly
        if (animator != null)
        {
            animator.SetBool(walkAttackParam, attacking);
            animator.SetBool(attackingParam, attacking);
            
            // During walk-attack, explicitly turn off the walking animation
            // but preserve the internal walking state
            if (attacking)
            {
            ...
            {
                // Resume walking animation if we were walking before
                animator.SetBool(walkingParam, true);
            }
        }
        
        // If we've stopped walk-attacking, and we were walking before,
        // make sure the walking animation resumes
        if (!attacking && isWalking)
        {
            UpdateAnimatorParameters();
        }
    }
