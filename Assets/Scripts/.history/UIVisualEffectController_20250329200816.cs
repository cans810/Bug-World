...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    {
        Debug.Log($"PlayChitinDepositEffect called with {chitinAmount} chitin and {xpAmount} XP");
        
        if (chitinSymbolPrefab == null)
        {
            Debug.LogError("Chitin symbol prefab not assigned!");
            return;
        }
    ...
    private IEnumerator SpawnChitinSymbols(int count, int xpAmount)
    {
        ...
        Debug.Log($"Beginning to spawn {count} chitin symbols");
        ...
        Debug.Log($"Chitin panel center: {panelCenter}, Screen center: {screenCenter}");

        // Spawn chitin symbols with delay
        for (int i = 0; i < count; i++)
        {
            // Create chitin symbol - BASIC DIRECT CREATION FOR TESTING
            ...
            // ENSURE THE SYMBOL HAS A PROPER SIZE
            symbolRT.sizeDelta = new Vector2(50, 50); // Set explicit size
            
            // Position near chitin panel with slight randomization
            Vector2 spawnPosition = panelCenter + new Vector2(
                Random.Range(-20f, 20f),
                Random.Range(-20f, 20f)
            );
            
            symbolRT.position = spawnPosition;
            
            // Make sure it's visible!
            Image symbolImage = chitinSymbol.GetComponent<Image>();
            if (symbolImage != null)
            {
                symbolImage.color = new Color(1f, 1f, 1f, 1f); // Fully opaque
            }
            
            Debug.Log($"Spawned chitin symbol {i+1}/{count} at position {spawnPosition}");
        ...
        Debug.Log($"Waiting {totalDelay}s before spawning XP symbols");
        yield return new WaitForSeconds(totalDelay);
        
        // Play XP gain effect
        if (xpAmount > 0)
        {
            Debug.Log($"Now spawning {xpAmount} XP symbols");
    ...
    private IEnumerator AnimateChitinToCenter(GameObject chitinSymbol, Vector2 screenCenter, int index)
    {
        ...
        if (symbolRT == null)
        {
            Debug.LogError("Chitin symbol has no RectTransform!");
            yield break;
        }
        ...
        Debug.Log($"Animating chitin symbol {index} from {startPos} to {targetPos}");
        ...
        Debug.Log($"Chitin symbol {index} reached target position");
