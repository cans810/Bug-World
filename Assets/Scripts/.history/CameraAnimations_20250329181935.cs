...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator AnimateCameraToNewInsect(Transform insectTransform)
    {
        ...
        // Calculate position to view the insect from the OPPOSITE side
        // (so we look at it from behind, seeing its front)
        Vector3 insectPosition = insectTransform.position;
        Vector3 insectForward = insectTransform.forward;
        
        // Position camera BEHIND the insect (same as insect's forward direction)
        // Changed from negative to positive to view from the opposite side
        Vector3 viewPosition = insectPosition + insectForward * 2.5f;
        viewPosition.y = insectPosition.y + 1.0f; // Increased height for better view
        
        // Calculate rotation to look at the insect from behind
