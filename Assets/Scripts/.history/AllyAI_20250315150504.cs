...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Distance to player (for safety check)
        float distanceToPlayer = Vector3.Distance(transform.position, playerTransform.position);
        
        // CRITICAL CASE: If too far from player, always move toward player
        if (distanceToPlayer > followDistance * 3)
        {
        ...
        // MAIN BEHAVIOR: If player is moving, follow with same input
        if (playerMoving)
        {
            // Calculate ideal position for reference (even if we don't use it directly)
            ...
            // If we're very close to ideal position or somewhat close and in line with player's movement,
            // just use player's input direction
            Vector3 moveDirection;
            
            if (distanceFromIdealPos < 1.5f || 
                (distanceFromIdealPos < 3.0f && Vector3.Dot(playerInputDirection.normalized, 
                                                          (idealPosition - transform.position).normalized) > 0.7f))
            {
        ...
        {
            // Player isn't moving, just stop in place
            isMoving = false;
            UpdateAnimation(false);
