...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // Skip if we're too close to the waypoint
        ...
        // For smooth rotation, use direct quaternion interpolation
        Quaternion targetRotation = Quaternion.LookRotation(direction);
        transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, 1.5f * Time.deltaTime);
        
        // For movement, use a consistent approach for all entities
        if (livingEntity != null)
        {
            // Calculate the movement vector
            Vector3 moveVector = transform.forward;
            
            // Apply movement directly using transform for maximum consistency
            transform.position += moveVector * livingEntity.moveSpeed * Time.deltaTime;
        }
        else
        {
            // Fallback if no living entity
