...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private bool hasRequiredLevel = false;
    ...
    private void Awake()
    {
        ...
        if (playerInventory != null)
        {
            ...
            playerInventory.OnLevelUp += CheckPlayerLevel;
        }
        else
        {
            Debug.LogWarning("PlayerInventory not found. Border visibility won't update with player level.");
        }
    }
    
    private void Start()
    {
        // Check if player has the required level already
        CheckPlayerLevel();
        
        // Only show the perimeter if the player doesn't have the required level
        if (showRuntimePerimeter && !hasRequiredLevel && requiredPlayerLevel > 0)
        {
            CreatePerimeterVisual();
        }
    }
    
    private void CheckPlayerLevel(int newLevel = 0)
    {
        if (playerInventory == null) return;
        
        // If newLevel is 0, get the current level from player inventory
        int playerLevel = newLevel > 0 ? newLevel : playerInventory.CurrentLevel;
        
        // Check if player has the required level
        hasRequiredLevel = playerLevel >= requiredPlayerLevel;
        
        // Update perimeter visibility if it exists
        if (perimeterLineRenderer != null && requiredPlayerLevel > 0)
        {
            perimeterLineRenderer.enabled = !hasRequiredLevel;
            
            if (hasRequiredLevel)
            {
                Debug.Log($"Player level {playerLevel} is sufficient for area {gameObject.name} (required: {requiredPlayerLevel}). Hiding perimeter.");
            }
        }
    }
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // For a capsule, we need to create a 2D projection that looks like:
        //    ____
        //   /    \
        //  |      |
        //  |      |
        ...
        int segments = 60; // Total points in the perimeter
        perimeterLineRenderer.positionCount = segments;
        ...
        int halfCirclePoints = segments / 2; // Half for the two semicircles
        int straightPoints = segments - halfCirclePoints; // The rest for the straight sections
        int pointsPerStraight = straightPoints / 2; // Points per straight section
        
        // Track point index
        int pointIndex = 0;
        
        // Generate points for the first semicircle (top)
        for (int i = 0; i <= halfCirclePoints / 2; i++)
        {
            float angle = Mathf.PI * i / (halfCirclePoints / 2);
            float x = radius * Mathf.Cos(angle);
            float z = radius * Mathf.Sin(angle);
            
            // Position at the top of the capsule
            Vector3 point = new Vector3(center.x + x, perimeterHeight, center.z + straightLength/2 + z);
            perimeterLineRenderer.SetPosition(pointIndex++, point);
        }
        
        // Generate points for the right straight section
        for (int i = 0; i <= pointsPerStraight; i++)
        {
            float t = (float)i / pointsPerStraight;
            float z = straightLength/2 * (1 - t) - straightLength/2 * t; // From top to bottom
            
            Vector3 point = new Vector3(center.x + radius, perimeterHeight, center.z + z);
            perimeterLineRenderer.SetPosition(pointIndex++, point);
        }
        
        // Generate points for the second semicircle (bottom)
        for (int i = 0; i <= halfCirclePoints / 2; i++)
        {
            float angle = Mathf.PI + Mathf.PI * i / (halfCirclePoints / 2);
            float x = radius * Mathf.Cos(angle);
            float z = radius * Mathf.Sin(angle);
            
            // Position at the bottom of the capsule
            Vector3 point = new Vector3(center.x + x, perimeterHeight, center.z - straightLength/2 + z);
            perimeterLineRenderer.SetPosition(pointIndex++, point);
        }
        
        // Generate points for the left straight section
        for (int i = 0; i <= pointsPerStraight; i++)
        {
            float t = (float)i / pointsPerStraight;
            float z = -straightLength/2 * (1 - t) + straightLength/2 * t; // From bottom to top
            
            Vector3 point = new Vector3(center.x - radius, perimeterHeight, center.z + z);
            perimeterLineRenderer.SetPosition(pointIndex++, point);
        }
        
        // Handle case where we might have extra points due to rounding
        while (pointIndex < segments)
        {
            // Connect back to the start
            Vector3 firstPoint = perimeterLineRenderer.GetPosition(0);
            perimeterLineRenderer.SetPosition(pointIndex++, firstPoint);
        }
    }
