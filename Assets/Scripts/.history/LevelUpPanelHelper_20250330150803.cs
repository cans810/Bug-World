...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    // Add this flag near the top of the class with other fields
    ...
    {
        // Find player inventory if not already assigned
        if (playerInventory == null)
            playerInventory = FindObjectOfType<PlayerInventory>();
        
        if (playerInventory != null)
        {
            // IMPORTANT: Set the initial last level seen value to current level BEFORE subscribing
        ...
        // Mark as initializing - wait for game to fully load before showing level up panel
        isInitializing = true;
    ...
    // Replace the HandleRevivalComplete method with one that can be called from outside
    ...
    {
        pendingRewards--;
        Debug.Log($"Decremented pending rewards. {pendingRewards} remaining.");
        
        if (pendingRewards <= 0)
        {
            // Close the panel when all rewards are claimed
    ...
    {
        // Skip if panel is already active, player is reviving, or we're initializing the game
        if (isPanelActive || IsPlayerReviving() || isInitializing)
        {
            Debug.Log($"Skipping level up panel for level {newLevel} - " +
                     $"Panel active: {isPanelActive}, Reviving: {IsPlayerReviving()}, Initializing: {isInitializing}");
            
            // Always update last seen level
    ...
    // Modify the ShowLevelUpPanelDelayed coroutine to disable joystick
    private IEnumerator ShowLevelUpPanelDelayed(float delay, int newLevel)
    {
        ...
        // Disable joystick and reset its state to stop player movement
        if (joystick != null)
        {
            joystick.gameObject.SetActive(false);
            joystick.ResetJoystick();
            Debug.Log("Disabled joystick for level up panel");
        }
        
        // Also disable player movement via PlayerController
        PlayerController playerController = FindObjectOfType<PlayerController>();
        if (playerController != null)
        {
            playerController.SetControlsEnabled(false);
            Debug.Log("Disabled player controls for level up panel");
        }
    ...
    // Add this method to delay when we consider initialization complete
    private IEnumerator CompleteInitialization()
    {
        // Wait for the game to fully load (adjust time as needed)
        yield return new WaitForSeconds(2.0f);
        isInitializing = false;
        Debug.Log("Level up panel helper initialization completed");
    }

    // Add this coroutine to delay closing
    private IEnumerator CloseAfterDelay(float delay)
    {
        ...
        // Wait a bit for the closing animation
        yield return new WaitForSeconds(0.5f);
        
        // Then re-enable joystick and player controls
        if (joystick != null)
        {
            joystick.gameObject.SetActive(true);
            Debug.Log("Re-enabled joystick after level up rewards");
        }
        ...
        if (playerController != null)
        {
            ...
            Debug.Log("Re-enabled player controls after level up rewards");
        }
    }
}
