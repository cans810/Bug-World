...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        {
            // Create icon with a visible color and size
            GameObject icon = Instantiate(chitinIconPrefab, transform);
            
            // Make sure the icon is in the UI canvas hierarchy
            RectTransform canvasRect = GetComponentInParent<Canvas>().GetComponent<RectTransform>();
            if (canvasRect != null)
            {
                icon.transform.SetParent(canvasRect, false);
            }
            ...
            // Position at start - convert to local position in canvas space
            icon.transform.position = startPos;
            
            // Try setting the sorting order higher to ensure visibility
            Canvas iconCanvas = icon.GetComponent<Canvas>();
            if (iconCanvas != null)
            {
                iconCanvas.sortingOrder = 100; // High sorting order
            }
            else
            {
                // Add a canvas component if it doesn't exist
                Canvas newCanvas = icon.AddComponent<Canvas>();
                newCanvas.overrideSorting = true;
                newCanvas.sortingOrder = 100;
                icon.AddComponent<CanvasRenderer>();
            }
            
            // Set the layer to UI layer instead of custom layer
