...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == "doubleThreat")
        {
            ...
            if (incubator != null)
            {
                // Create one scorpion egg directly and get the reference
                GameObject eggCreated = incubator.CreateScorpionEgg();

                // Create two scorpion eggs side by side and get a reference to the first one
                bool success = incubator.CreateMultipleEggs("scorpion", 2);
                GameObject eggCreated = GameObject.FindGameObjectWithTag("AllyEgg"); // Get any egg to show in animation
                ...
                if (eggCreated != null)
                {
                    ...
                    StartCoroutine(ShowEggAfterDelay(eggCreated, 0.5f));
                }
                else
                {
