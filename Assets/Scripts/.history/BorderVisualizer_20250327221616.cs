...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Determine the direction of the capsule (x, y, or z axis)
        ...
        // Create points for the capsule perimeter
        int segments = 40; // Total segments (more = smoother)
        perimeterLineRenderer.positionCount = segments;
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            // Convert the angle to a position on the capsule perimeter
            if (direction == 0) // X-axis
            {
                // For X-axis capsule, perimeter is in the Y-Z plane
                if (angle < Mathf.PI) // First half
                {
                    // Map to first half of capsule
                ...
                {
                    if (angle < Mathf.PI*3/2 && angle > Mathf.PI/2) 
                    {
                        // Semi-circles at the ends
                        point = new Vector3(
                            center.x - halfHeight,
            ...
            {
                // For Y-axis capsule, perimeter is in the X-Z plane
                float x = radius * Mathf.Cos(angle);
                float z = radius * Mathf.Sin(angle);
                point = new Vector3(center.x + x, center.y, center.z + z);
            }
            else // Z-axis
            {
                // For Z-axis capsule, perimeter is in the X-Y plane
                if (angle < Mathf.PI) // First half
                {
                    // Map to first half of capsule
                    if (angle < Mathf.PI/2 || angle > Mathf.PI*3/2) 
            ...
            // Set to perimeter height (overriding Y to create a flat visual)
