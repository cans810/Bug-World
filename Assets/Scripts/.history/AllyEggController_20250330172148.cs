...
public class AllyEggController : MonoBehaviour
{
    ...
    public void OnCameraAnimationComplete(GameObject insect)
    {
        ...
        {
            wandering.SetWanderingEnabled(true);
            Debug.Log($"Re-enabled wandering for {insect.name} after camera animation");
        }
    ...
    // Fix the OnHatchingComplete method to remove the GlobalEventManager reference
    public void OnHatchingComplete(GameObject insect)
    {
        Debug.Log($"Hatching complete for {insect.name}, enabling AI components");
        
        // Enable AllyAI for following behavior
        AllyAI allyAI = insect.GetComponent<AllyAI>();
        if (allyAI != null)
        {
            allyAI.enabled = true;
            allyAI.ForceEnableMovement();
            Debug.Log($"Enabled AllyAI for {insect.name}");
        }
        
        // Enable AIWandering for wandering behavior
        AIWandering wandering = insect.GetComponent<AIWandering>();
        if (wandering != null)
        {
            wandering.enabled = true;
            wandering.SetWanderingEnabled(true);
            Debug.Log($"Enabled AIWandering for {insect.name}");
        }
        
        // Ensure animation controller is properly initialized
        AnimationController animController = insect.GetComponent<AnimationController>();
        if (animController != null)
        {
            animController.SetIdle();
            Debug.Log($"Reset animations for {insect.name}");
        }
        
        // Force physics components to be active
        Rigidbody rb = insect.GetComponent<Rigidbody>();
        if (rb != null)
        {
            rb.velocity = Vector3.zero;
            rb.isKinematic = false;
            Debug.Log($"Reset physics for {insect.name}");
        }
        
        // Broadcast a message to notify any components that need to know about the new ally
        // This replaces the GlobalEventManager reference that doesn't exist
        insect.SendMessage("OnAllyActivated", SendMessageOptions.DontRequireReceiver);
        PlayerController player = FindObjectOfType<PlayerController>();
        if (player != null)
        {
            player.SendMessage("OnNewAllyCreated", insect, SendMessageOptions.DontRequireReceiver);
        }
    }
}
