...
public class ItemInformController : MonoBehaviour
{
    ...
    public void SetResourceType(ResourceType type)
    {
        ...
        Debug.Log($"SetResourceType called with type: {type}. HasYetToCollectChitin: {playerInventory?.HasYetToCollectChitin}");
        
        // Check if this is chitin and it's the first time collection
        if (type == ResourceType.Chitin && playerInventory != null && playerInventory.HasYetToCollectChitin)
        {
            Debug.Log("FIRST TIME CHITIN COLLECTION DETECTED - Attempting to show arrow");
            
            // Show a guide arrow pointing to the nest
            ChitinDepositArrowManager arrowManager = FindObjectOfType<ChitinDepositArrowManager>();
            if (arrowManager != null)
            {
                // Show arrow immediately for first collection
                Debug.Log("ChitinDepositArrowManager found, calling ForceShowArrow");
                arrowManager.ForceShowArrow();
            }
            else
            {
                Debug.LogError("ChitinDepositArrowManager not found! Cannot show nest arrow.");
                
                // As a fallback, try to create the arrow manager if it doesn't exist
                GameObject arrowManagerObj = new GameObject("ChitinDepositArrowManager");
                arrowManager = arrowManagerObj.AddComponent<ChitinDepositArrowManager>();
                arrowManager.ForceShowArrow();
            }
            ...
            if (uiHelper != null)
            {
                uiHelper.ShowInformText("Chitin collected! Go to your nest to deposit it.");
            }
        }
    }
