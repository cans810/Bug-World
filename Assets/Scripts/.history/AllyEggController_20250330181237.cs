...
public class AllyEggController : MonoBehaviour
{
    ...
    private bool hasStartedHatching = false;
    public event System.Action OnEggHatched;
    ...
    {
        if (hasStartedHatching) return;
        hasStartedHatching = true;
        
        Debug.Log($"Starting egg hatching for {entityTypeToHatch}");
        
        // Notify all listeners about the hatch event BEFORE spawning the insect
        OnEggHatched?.Invoke();
        
        // Also find and notify the InsectIncubator about the hatch
        InsectIncubator incubator = FindObjectOfType<InsectIncubator>();
        if (incubator != null)
        {
            incubator.OnEggHatched(this.gameObject);
            Debug.Log("Notified InsectIncubator about egg hatching");
        }
        else
        {
        ...
        // Play hatching animation and sound
        StartCoroutine(HatchingProcess());
    }
}
