...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (distanceToPlayer <= followDistance)
        {
            ...
            // When stopped, gradually rotate to match player's forward direction
            if (playerTransform != null)
            {
                Vector3 playerForward = playerTransform.forward;
                playerForward.y = 0; // Keep on same Y level
                
                // Only rotate if player's forward direction is significant
                if (playerForward.magnitude > 0.1f)
                {
                    // Use livingEntity.RotateTowards for consistent rotation
                    livingEntity.RotateTowards(playerForward, rotationMultiplier * 0.5f);
                }
            }
        }
        ...
        else
        {
            ...
            // Calculate direction to player - this is the primary movement direction
            Vector3 directionToPlayer = (playerTransform.position - transform.position).normalized;
            directionToPlayer.y = 0; // Keep on same Y level
            
            // We'll use a simpler approach that works better:
            // 1. Always MOVE directly toward the player's position
            // 2. ROTATE more toward player's facing direction as we get closer
            
            // Calculate how close we are to the target follow distance (0 = at follow distance, 1 = very far)
            float distanceFactor = Mathf.Clamp01((distanceToPlayer - followDistance) / (followDistance * 2));
            
            // When far: rotate purely toward player position
            // When getting closer: start blending in the player's forward direction
            Vector3 playerForward = playerTransform.forward;
            playerForward.y = 0;
            
            // The closer we get to the player, the more we match their rotation
            // Far away = 100% toward player, At follow distance = 50% toward player, 50% player's direction
            float playerDirWeight = Mathf.Lerp(0.5f, 0f, distanceFactor);
            Vector3 rotateDirection = Vector3.Lerp(directionToPlayer, playerForward, playerDirWeight).normalized;
            
            // Rotate toward the calculated direction
            livingEntity.RotateTowards(rotateDirection, rotationMultiplier * 0.7f);
            
            // But ALWAYS move directly toward the player for more natural following
