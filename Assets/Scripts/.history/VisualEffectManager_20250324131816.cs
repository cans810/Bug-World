...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Modify the animation to make symbols move much slower
    private IEnumerator AnimateLevelUpSymbolInRay(GameObject symbol, Transform playerTransform, Vector3 rayDirection, float delay, System.Action onComplete)
    {
        ...
        // Animation parameters - SIGNIFICANTLY slower animation
        float animDuration = 4.0f;  // Increased from 2.5f to 4.0f for much slower movement
        float maxHeight = 9.0f;     // Reduced height (from 12.0f) so they don't travel as far vertically
        float elapsed = 0f;
        
        // Keep the curved path setup
        ...
        // Reduce velocity multiplier to slow down later symbols too
        float rayVelocityMultiplier = 1.0f + (delay * 1.0f); // Further reduced from 2.0f to 1.0f
        ...
        while (elapsed < animDuration && playerTransform != null)
        {
            ...
            // Use a more gradual curve for slower initial movement
            float verticalProgress = Mathf.Pow(normalizedTime, 0.9f); // Even less acceleration (0.9 vs 0.8)
            float currentHeight = Mathf.Lerp(0, maxHeight * rayVelocityMultiplier, verticalProgress);
            
            // Keep the ray very straight
            float rayIntegrity = 0.98f;
            ...
            Vector3 newPosition = startPosition + new Vector3(
                horizontalDrift + rayOffset.x * currentHeight * 0.05f,
                currentHeight,
                rayOffset.z * currentHeight * 0.05f
            );
            ...
            // Even more gradual growth
            float baseGrowth = normalizedTime < 0.5f ? // Increased from 0.4f for even slower growth
                Mathf.Lerp(0.1f, 1.0f, normalizedTime / 0.5f) : 
                1.0f;
            ...
            // Keep symbols visible longer before fading
            float alpha;
            if (normalizedTime < 0.1f) {
                alpha = Mathf.Lerp(0f, 1f, normalizedTime / 0.1f);
            }
            else if (normalizedTime > 0.85f) { // Increased from 0.8f for even longer visibility
                alpha = Mathf.Lerp(1f, 0f, (normalizedTime - 0.85f) / 0.15f);
            }
            else {
                alpha = 1f;
            }
