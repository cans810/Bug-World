...
public class AIWandering : MonoBehaviour
{
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            // If we got back our current position, wait longer and try again
            if (Vector3.Distance(targetWaypoint, transform.position) < 0.1f)
            {
            ...
            // Log movement start
            Debug.Log($"{gameObject.name}: Moving toward waypoint at {targetWaypoint}, distance: {distanceToWaypoint}");
            ...
            while (moveTimer < moveDuration && isWandering)
            {
                ...
                if (moveTimer % 0.5f < Time.deltaTime)
                {
                    ...
                    if (IsBorderInDirection(currentDirection, remainingDistance + 2f))
                    {
                        Debug.LogWarning($"{gameObject.name}: Border detected during movement - stopping");
                        break;
                    }
                }
                ...
                {
                    Debug.Log($"{gameObject.name}: Reached waypoint");
                    break;
                }
