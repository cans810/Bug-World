...
public class PlayerInventory : MonoBehaviour
{
    ...
    private void ProcessLevelUp(int level)
    {
        ...
        {
            // Add 2 points per level and log it
            int pointsToAdd = playerAttributes.pointsPerLevel;  // This should be 2
            playerAttributes.AddAttributePoints(pointsToAdd);
            Debug.Log($"<color=yellow>Added {pointsToAdd} attribute points on level up to level {level}</color>");
        }
