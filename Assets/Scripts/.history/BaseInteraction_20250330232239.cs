...
public class BaseInteraction : MonoBehaviour
{
    ...
    // Add this field to track first crumb deposit
    [Header("First Crumb Deposit")]
    [SerializeField] private bool hasEnteredWithCrumbsForFirstTime = false;
    ...
    {
        // Only continue if player has crumbs to deposit
        if (playerInventory == null || playerInventory.CrumbCount <= 0) return;
        
        // Check if this is the first time player has entered with crumbs
        if (!hasEnteredWithCrumbsForFirstTime)
        {
            // Mark that player has entered with crumbs
            hasEnteredWithCrumbsForFirstTime = true;
            ...
            if (uiHelper != null && uiHelper.discoveredEntitiesButtonPointingPulsatingImage != null)
            {
                ...
                // Show a message to guide the player
                if (uiHelper.informPlayerText != null)
                {
                    uiHelper.ShowInformText("Select an entity in your collection to deposit crumbs!", 5f);
                }
                
                Debug.Log("First time with crumbs - showing Discovered Entities guidance indicator");
            }
        ...
        // Now continue with the actual crumb deposit logic
        ...
        if (!hasEquippedEntity)
        {
            ...
            if (uiHelper != null)
            {
                uiHelper.ShowInformText("Select an entity first before depositing crumbs!");
            }
            return;
        }
    ...
    // Add this method to save the first crumb deposit state
    public void SaveFirstCrumbDepositState(GameData data)
    {
        if (data != null)
        {
            data.hasEnteredWithCrumbsForFirstTime = hasEnteredWithCrumbsForFirstTime;
            Debug.Log($"Saved first crumb deposit state: {hasEnteredWithCrumbsForFirstTime}");
        }
    }

    // Add this method to load the first crumb deposit state
    public void LoadFirstCrumbDepositState(GameData data)
    {
        if (data != null)
        {
            hasEnteredWithCrumbsForFirstTime = data.hasEnteredWithCrumbsForFirstTime;
            Debug.Log($"Loaded first crumb deposit state: {hasEnteredWithCrumbsForFirstTime}");
        }
    }
} 