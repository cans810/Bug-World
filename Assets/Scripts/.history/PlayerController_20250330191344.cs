...
public class PlayerController : MonoBehaviour
{
    ...
    // Add this as a class field at the top of the PlayerController class
    private bool wasShowingBorderMessage = false;
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            if (isAtMapBorder)
            {
                ...
                // Hide any border message if it was showing
                if (wasShowingBorderMessage && uiHelper != null)
                {
                    uiHelper.HideInformText();
                    wasShowingBorderMessage = false;
                }
            }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            else if (playerLevel < requiredLevel) 
            {
                ...
                // Check if player is trying to move into the border (dot product with normal is negative)
                bool isMovingThroughBorder = Vector3.Dot(moveDirection, borderNormal) < 0;
                
                // If player is trying to move into the border
                if (isMovingThroughBorder)
                {
                    // Apply movement restrictions as before
                    moveDirection = Vector3.zero;
                    ...
                    // Only show message if player is actively trying to move through the border
                    if (Time.time >= lastBorderMessageTime + borderMessageCooldown)
                    {
                        if (uiHelper != null)
                        {
                            ...
                            wasShowingBorderMessage = true; // Track that we're showing message
                        }
                    }
                }
                else if (wasShowingBorderMessage) // Player was showing message but stopped crossing
                {
                    // Player stopped trying to cross the border, hide the message immediately
                    if (uiHelper != null)
                    {
                        uiHelper.HideInformText();
                        wasShowingBorderMessage = false;
    ...
    private void UpdateCameraSettingsImmediate(int level)
    {
        ...
        float zoomLevel = 2.04f;
        float heightOffset = 2.62f;
