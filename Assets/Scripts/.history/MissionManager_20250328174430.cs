...
public class MissionManager : MonoBehaviour
{
    ...
    [SerializeField] private string completeButtonPath = "CompleteButton"; // Path to the button component
    ...
    private void ShowMissionPanel(MissionData mission)
    {
        ...
        // Find the complete button and set up its event
        Button completeButton = missionPanelInstance.transform.Find(completeButtonPath)?.GetComponent<Button>();
        if (completeButton != null)
        {
            // Initially disable the button (will be enabled when mission completes)
            completeButton.gameObject.SetActive(false);
            
            // Add click listener to give rewards
            completeButton.onClick.AddListener(() => GiveRewardsForMission(mission.id, missionPanelInstance));
        }
        else
        {
            Debug.LogWarning($"Complete button not found at path: {completeButtonPath}");
        }
    ...
    private void UpdateMissionPanelProgress(string missionId)
    {
        ...
        // If mission is completed, activate the complete button
        if (mission.CheckIsCompleted() && panel != null)
        {
            Button completeButton = panel.transform.Find(completeButtonPath)?.GetComponent<Button>();
            if (completeButton != null)
            {
                completeButton.gameObject.SetActive(true);
                
                // Optional: Add animation or highlight to the button
                // For example, you could add a pulsing animation to draw attention
            }
        }
    }
