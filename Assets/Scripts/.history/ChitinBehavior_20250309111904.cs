...
public class ChitinBehavior : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // ! This doesnt work, instead I used layers
    ...
    private void StickInPlace()
    {
        ...
        if (rb != null)
        {
            ...
            rb.collisionDetectionMode = CollisionDetectionMode.Discrete;  // Set to discrete for animation
            
            if (debugMode)
                Debug.Log($"Rigidbody set to: isKinematic={rb.isKinematic}, collisionMode={rb.collisionDetectionMode}, constraints={rb.constraints}");
        }
        ...
        if (collectible != null)
        {
            ...
            // IMPORTANT: Directly call ForceCollect if player is nearby
            // This ensures the collection animation works even if the Update method hasn't run yet
            Transform playerTransform = GameObject.FindGameObjectWithTag("Player")?.transform;
            if (playerTransform != null)
            {
                float distanceToPlayer = Vector3.Distance(transform.position, playerTransform.position);
                if (distanceToPlayer <= 1.5f) // Use a reasonable collection radius
                {
