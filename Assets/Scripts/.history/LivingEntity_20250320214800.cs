...
public class LivingEntity : MonoBehaviour
{
    ...
    [SerializeField] private bool enableSinkOnDeath = true;
    [SerializeField] private float sinkDuration = 1.5f;
    [SerializeField] private float sinkDelay = 0.5f;  // Delay after death animation before sinking starts
    [SerializeField] private float sinkDistance = 1.0f;  // How far to sink into the ground
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // Handle the body with sinking and destruction
        if (destroyOnDeath)
        {
            if (enableSinkOnDeath)
            {
                // Start sinking coroutine with delay for death animation
                StartCoroutine(SinkAndDestroy());
            }
            else
            {
                // Just destroy with delay as before
    ...
    private IEnumerator SinkAndDestroy()
    {
        ...
        Vector3 endPosition = new Vector3(startPosition.x, startPosition.y - sinkDistance, startPosition.z);
        ...
        // Final delay before destruction (optional, for any effects)
