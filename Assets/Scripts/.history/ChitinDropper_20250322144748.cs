...
public class ChitinDropper : MonoBehaviour
{
    ...
    // This will always draw in Scene view
    private void OnDrawGizmos()
    {
        // Always use the raw transform.position to ensure we get the most current position
        Vector3 currentPosition = transform.position;
        
        // Create a distinctive marker that will be visible in any scene
        // First draw a thick vertical line
        Gizmos.color = Color.red;
        Gizmos.DrawLine(currentPosition + Vector3.up * 3f, currentPosition);
        
        // Draw a large cross at the drop point
        float crossSize = 0.5f;
        Gizmos.DrawLine(currentPosition + Vector3.right * crossSize, currentPosition + Vector3.left * crossSize);
        Gizmos.DrawLine(currentPosition + Vector3.forward * crossSize, currentPosition + Vector3.back * crossSize);
        
        // Draw a sphere at the center
        Gizmos.DrawSphere(currentPosition, 0.25f);
        
        // For extra visibility, add a wireframe cube
        ...
        UnityEditor.Handles.Label(currentPosition + Vector3.up * 0.7f, 
            $"Chitin: {minChitinToDrop}-{maxChitinToDrop}");
        UnityEditor.Handles.EndGUI();
        #endif

        // Force Scene view repaint in editor mode
        #if UNITY_EDITOR
