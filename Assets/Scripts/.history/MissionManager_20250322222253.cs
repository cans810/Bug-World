...
public class MissionManager : MonoBehaviour
{
    ...
    private IEnumerator PlayCoinAnimationThenHide(GameObject panel, int coinAmount)
    {
        ...
        if (visualEffectManager != null && visualEffectManager.CoinPrefab != null)
        {
            coinPrefab = visualEffectManager.CoinPrefab;
        }
        
        // If we couldn't get a coin prefab, just hide the panel
        if (coinPrefab == null)
        {
            Debug.LogWarning("Could not get coin prefab - no animation will play");
        ...
        {
            SoundEffectManager.Instance.PlaySound("Coin", Vector3.zero, false);
        }
        
        // Create and animate the coin symbols
        List<GameObject> coinInstances = new List<GameObject>();
        RectTransform panelRect = panel.GetComponent<RectTransform>();
        
        // Target position for coins (top right coin counter)
        Vector2 targetScreenPos = new Vector2(Screen.width - 150f, Screen.height - 75f);
        
        for (int i = 0; i < numberOfCoins; i++)
        {
            // Create coin directly on the panel
            GameObject coin = Instantiate(coinPrefab, panel.transform);
            coinInstances.Add(coin);
            
            // Position randomly within the panel
            RectTransform coinRect = coin.GetComponent<RectTransform>();
            if (coinRect != null && panelRect != null)
            {
                // Random position within panel bounds
                float offsetX = UnityEngine.Random.Range(-panelRect.rect.width/2 + 20, panelRect.rect.width/2 - 20);
                float offsetY = UnityEngine.Random.Range(-panelRect.rect.height/2 + 20, panelRect.rect.height/2 - 20);
                
                coinRect.anchoredPosition = new Vector2(offsetX, offsetY);
                coin.transform.localScale = new Vector3(30f, 30f, 30f); // Make coins visible in UI
            }
        ...
        // Short pause before coins fly away
        yield return new WaitForSeconds(0.2f);
        
        // Animate coins to target position
        StartCoroutine(AnimateCoinsToTarget(coinInstances, targetScreenPos));
        
        // Wait for coins to finish moving
    ...
    /// <summary>
    /// Animates coins from the panel to the target position
    /// </summary>
    private IEnumerator AnimateCoinsToTarget(List<GameObject> coins, Vector2 targetScreenPos)
    {
        float duration = 0.8f;
        float elapsed = 0f;
        ...
        while (elapsed < duration)
        {
            ...
            foreach (GameObject coin in coins)
            {
                ...
                // Create a midpoint above the direct path for an arc
                Vector3 midPoint = Vector3.Lerp(startPos, targetScreenPos, 0.5f);
                midPoint.y += 50f;
                
                // Bezier curve movement
                Vector3 a = Vector3.Lerp(startPos, midPoint, t);
                Vector3 b = Vector3.Lerp(midPoint, targetScreenPos, t);
                Vector3 currentScreenPos = Vector3.Lerp(a, b, t);
                
                // Convert back to world space
                coin.transform.position = Camera.main.ScreenToWorldPoint(new Vector3(
                    currentScreenPos.x, 
                    currentScreenPos.y, 
                    10f));
                ...
                {
                    // Grow in first half
                    coin.transform.localScale = Vector3.Lerp(
