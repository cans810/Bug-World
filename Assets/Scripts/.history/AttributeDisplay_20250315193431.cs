...
public class AttributeDisplay : MonoBehaviour
{
    ...
    {
        Debug.Log("AttributeDisplay.ShowPanel() called");
        
        if (attributesPanel != null)
        {
            attributesPanel.SetActive(true);
            Debug.Log($"AttributeDisplay: Panel set active = true");
            ...
            if (panelAnimator != null)
            {
                ...
                Debug.Log("AttributeDisplay: Panel animation triggered");
            }
        }
        else
        {
    ...
    {
        Debug.Log($"AttributeDisplay: Camera animation completed to area {areaName} (level {level})");
        
        // Only show panel if we have points to spend
        if (playerAttributes != null)
        {
            Debug.Log($"AttributeDisplay: Player has {playerAttributes.AvailablePoints} attribute points available");
            
            if (playerAttributes.AvailablePoints > 0)
            {
                Debug.Log("AttributeDisplay: Showing panel with delay");
                // Slight delay to give the arrow time to appear first
                StartCoroutine(ShowPanelWithDelay(0.5f));
            }
            else
            {
                Debug.Log("AttributeDisplay: Not showing panel (no points available)");
            }
        }
        else
        {
            Debug.LogError("AttributeDisplay: playerAttributes is null!");
        }
    }

    private IEnumerator ShowPanelWithDelay(float delay)
    {
        Debug.Log($"AttributeDisplay: Waiting {delay} seconds before showing panel");
        yield return new WaitForSeconds(delay);
        Debug.Log("AttributeDisplay: Now showing panel");
        ShowPanel();
    }
