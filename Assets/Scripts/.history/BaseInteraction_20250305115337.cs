...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void StopHealing()
    {
        ...
        // Final healing debug message
        if (showDebugMessages && totalHealedAmount > 0)
        {
            Debug.Log($"Healing stopped. Total healed: {totalHealedAmount:F1} health");
        }
    }
    ...
    private void ApplySmoothHealing()
    {
        ...
        if (playerEntity.CurrentHealth < playerEntity.MaxHealth)
        {
            ...
            // Show feedback periodically rather than every frame
            if (showDebugMessages && debugMessageTimer <= 0f)
            {
                Debug.Log($"Healing at {healPercentPerSecond}% per second. Current health: {playerEntity.CurrentHealth:F1}/{playerEntity.MaxHealth}");
