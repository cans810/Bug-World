...
public class AllyAntPanelController : MonoBehaviour
{
    ...
    [SerializeField] private Toggle allAntsfollowModeToggle;
    [SerializeField] private Toggle allAntswanderModeToggle;
    [SerializeField] private Toggle allAntslootModeToggle;
    ...
    void Start()
    {
        ...
        // Set up all ants command toggles
        if (allAntsfollowModeToggle != null)
            allAntsfollowModeToggle.onValueChanged.AddListener(CommandAllAntsFollow);
            
        if (allAntswanderModeToggle != null)
            allAntswanderModeToggle.onValueChanged.AddListener(CommandAllAntsWander);
            
        if (allAntslootModeToggle != null)
            allAntslootModeToggle.onValueChanged.AddListener(ToggleAllAntsLootMode);
    ...
    // Method to update the All Ants Orders Panel toggles
    private void UpdateAllAntsOrdersPanel()
    {
        ...
        // Get the current loot mode status across all ants
        bool allHaveLootEnabled = IsAllAntsLootEnabled();
        
        // Update the loot toggle without triggering event
        if (allAntslootModeToggle != null)
            allAntslootModeToggle.SetIsOnWithoutNotify(allHaveLootEnabled);
    }
    
    // Command all ants to follow player
    private void CommandAllAntsFollow(bool isOn)
    {
        if (!isOn)
            return; // Only respond to toggle on, not toggle off
        ...
        // Reset the wander toggle
        if (allAntswanderModeToggle != null)
            allAntswanderModeToggle.SetIsOnWithoutNotify(false);
    ...
    private void CommandAllAntsWander(bool isOn)
    {
        if (!isOn)
            return; // Only respond to toggle on, not toggle off
        ...
        // Reset the follow toggle
        if (allAntsfollowModeToggle != null)
            allAntsfollowModeToggle.SetIsOnWithoutNotify(false);
    ...
    private void ToggleAllAntsLootMode(bool isOn)
    {
        // Find all ally ants
        GameObject[] allyAnts = FindAllyAnts();
        ...
        Debug.Log($"Set all {allyAnts.Length} ants loot collection to: {(isOn ? "ENABLED" : "DISABLED")}");
