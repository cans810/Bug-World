    ...
    private void DrawCapsulePerimeter()
    {
        ...
        switch (capsuleCollider.direction)
        {
            case 0: // X axis
                ...
                perpendicularA = Vector3.forward;
        ...
        float halfStraightLength = straightLength * 0.5f;
        
        // Calculate number of points
        int segments = 60; // Total points in the perimeter
        int halfCirclePoints = 30; // Points for the two semicircles
        int straightPoints = segments - halfCirclePoints; // Points for straight sections
        ...
        // Calculate end points of capsule
        Vector3 topEnd = center + direction * halfStraightLength;
        Vector3 bottomEnd = center - direction * halfStraightLength;
        ...
        for (int i = 0; i < topSemicirclePoints; i++)
        {
            ...
            Vector3 pointOffset = perpendicularA * (x * radius) + perpendicularB * (y * radius);
            Vector3 point = topEnd + pointOffset;
            
            // If Y is not up, we need to maintain perimeter height
            if (capsuleCollider.direction != 1) // Not Y-axis
            {
        ...
        for (int i = 0; i < rightStraightPoints; i++)
        {
            ...
            Vector3 point = topEnd + perpendicularA * radius - direction * straightLength * t;
            
            // Maintain perimeter height if not Y-axis
        ...
        for (int i = 0; i < bottomSemicirclePoints; i++)
        {
            ...
            Vector3 pointOffset = perpendicularA * (x * radius) + perpendicularB * (y * radius);
            Vector3 point = bottomEnd + pointOffset;
        ...
        for (int i = 0; i < leftStraightPoints; i++)
        {
            ...
            Vector3 point = bottomEnd - perpendicularA * radius + direction * straightLength * t;
