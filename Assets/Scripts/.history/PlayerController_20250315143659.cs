        ...
        while (enabled && livingEntity != null && !livingEntity.IsDead && autoAttackEnabled)
        {
            ...
            if (enemiesInRange.Count > 0)
            {
                ...
                if (closestEnemy != null)
                {
                    ...
                    // Force the attack hit frame to apply damage directly
                    if (OnAttackButtonPressed())
                    {
                        // Wait for the attack animation to reach the hit frame
                        yield return new WaitForSeconds(0.3f); // Adjust timing based on your animation
                        
                        // Directly apply damage to the enemy
                        closestEnemy.TakeDamage(livingEntity.AttackDamage, gameObject);
