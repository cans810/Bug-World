...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    public void HideMarketPanel() {
        ...
        Animator panelAnimator = MarketPanel.GetComponent<Animator>();
        
        if (panelAnimator != null) {
            // Use animator to hide panel
            panelAnimator.SetBool("ShowUp", false);
            panelAnimator.SetBool("Hide", true);
            
            Debug.Log("Market panel hiding animation triggered");
        } else {
            // Fallback to direct deactivation if no animator
            MarketPanel.SetActive(false);
            Debug.LogWarning("No animator found on MarketPanel, using direct deactivation");
        }
    }

    public void ShowMarketPanel() {
        // First ensure the panel is active before accessing the animator
        ...
        if (panelAnimator != null) {
            // Reset animation state and trigger show animation
            panelAnimator.SetBool("Hide", false);
            panelAnimator.SetBool("ShowUp", true);
            
            Debug.Log("Market panel showing animation triggered");
        }
        
        // Update coin display and prices
        UpdateCoinsDisplay();
    ...
    // Add this method to be called from buttons or other UI elements
    public void OnMarketButtonPressed() {
        // No matter what, force the panel to be active
        MarketPanel.SetActive(true);
        
        // Then show the panel properly
        ShowMarketPanel();
        ...
        Debug.Log("Market button pressed - forcing MarketPanel active");
    }
}
