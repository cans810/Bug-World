...
public class AllyEggController : MonoBehaviour
{
    ...
    // Add this method to AllyEggController to make sure AI is properly enabled
    public void OnEggHatched(GameObject insect)
    {
        // Make sure the insect has its AI components enabled
        if (insect != null)
        {
            // Enable AllyAI component
            AllyAI allyAI = insect.GetComponent<AllyAI>();
            if (allyAI != null)
            {
                allyAI.enabled = true;
                allyAI.ForceEnableMovement();
                Debug.Log($"Egg hatched: Enabled AllyAI for {insect.name}");
            }
            
            // Enable AIWandering component
            AIWandering wandering = insect.GetComponent<AIWandering>();
            if (wandering != null)
            {
                wandering.SetWanderingEnabled(true);
                Debug.Log($"Egg hatched: Enabled AIWandering for {insect.name}");
            }
        }
        
        // Perform any other egg hatching cleanup...
    }

    // Modify the existing Hatch method to call this new method
    public void Hatch()
    {
    ...
    // Update the HatchCoroutine to make sure all allies have their AI enabled
    private IEnumerator HatchCoroutine()
    {
        // Existing hatch code...
        
        // When the insect spawns
        GameObject spawnedInsect = Instantiate(insectPrefabs[insectIndexToSpawn], transform.position, Quaternion.identity);
        
        // Call new method to ensure AI is enabled
        OnEggHatched(spawnedInsect);
