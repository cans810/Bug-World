...
public class LivingEntity : MonoBehaviour
{
    ...
    {
        Debug.Log($"Entity {gameObject.name} taking damage, tag: {gameObject.tag}, hasBeenDiscovered: {hasBeenDiscovered}");
        ...
        // Check if this is an entity that should be discovered
        if (!hasBeenDiscovered && gameObject.layer == LayerMask.NameToLayer("Insects"))
        {
            Debug.Log("*** DISCOVERY CHECK PASSED in TakeDamage ***");
            hasBeenDiscovered = true;
            
            // Notify the discovered entities controller
            DiscoveredEntitiesController controller = DiscoveredEntitiesController.Instance;
            Debug.Log($"Controller found: {controller != null}");
            if (controller != null)
            {
                Debug.Log($"Sending discovery: {gameObject.name}");
                controller.CheckAndDiscoverEntity(gameObject.name);
            }
        }
    }
    ...
    public virtual void Die(GameObject killer = null)
    {
        ...
        // Register kill for discovery system
        RegisterKill(killer);
    ...
    {
        // Check for discovery
        if (other.CompareTag("Player") && gameObject.layer == LayerMask.NameToLayer("Insects"))
        {
            LivingEntity ownerEntity = transform.parent?.GetComponent<LivingEntity>();
            if (ownerEntity != null && !ownerEntity.hasBeenDiscovered)
            {
                ownerEntity.hasBeenDiscovered = true;
                
                // Notify the discovered entities controller
                DiscoveredEntitiesController controller = DiscoveredEntitiesController.Instance;
                if (controller != null)
                {
                    Debug.Log($"ONTRIGGER: Discovering entity: {ownerEntity.gameObject.name}");
                    controller.CheckAndDiscoverEntity(ownerEntity.gameObject.name);
                }
            }
        }
    }
    ...
    // Add this method to the LivingEntity class
    public void RegisterKill(LivingEntity killer)
    {
        if (killer == null) return;
        
        // Check if the killer is the player
        if (killer.gameObject.CompareTag("Player"))
        {
