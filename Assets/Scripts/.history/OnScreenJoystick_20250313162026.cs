            ...
            if (UnityEngine.Input.touchCount > 0)
            {
                ...
                if (ignoreUIElements)
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        // Check if this element is on the TouchableUI layer
                        if (result.gameObject.layer == LayerMask.NameToLayer(touchableSortingLayer))
                        {
                            if (debugSortingLayers)
                            {
                                Debug.Log($"UI element {result.gameObject.name} is on TouchableUI layer, allowing joystick activation");
                            }
