...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private float coinSpreadRadius = 30f;
    ...
    // Add a public property to expose the coin prefab
    public GameObject CoinPrefab => coinSymbolPrefab;
    ...
    /// <summary>
    /// Spawns a coin symbol from a UI element and animates it to the target position
    /// </summary>
    public void SpawnCoinSymbolFromUI(Vector3 startScreenPos, Vector2 targetScreenPos, float delay = 0f)
    {
        StartCoroutine(SpawnCoinSymbolCoroutine(startScreenPos, targetScreenPos, delay));
    }

    private IEnumerator SpawnCoinSymbolCoroutine(Vector3 startScreenPos, Vector2 targetScreenPos, float delay)
    {
        // Wait for the delay
        if (delay > 0)
            yield return new WaitForSeconds(delay);
        
        // Check if we have the coin prefab
        ...
        if (symbolPools["coin"].Count == 0) yield break;
        
        // Get a coin symbol from the pool
        GameObject symbol = symbolPools["coin"].Dequeue();
        
        // Convert start screen position to world position
        Ray startRay = Camera.main.ScreenPointToRay(startScreenPos);
        float distanceToCamera = 10f;
        Vector3 startWorldPos = startRay.GetPoint(distanceToCamera);
        
        // Set position and make active
        symbol.transform.position = startWorldPos;
        symbol.transform.rotation = cameraTransform.rotation;
        symbol.transform.localScale = baseScale;
        symbol.SetActive(true);
        
        // Animate to target position
        StartCoroutine(AnimateCoinSymbol(symbol, targetScreenPos));
    }

    private IEnumerator AnimateCoinSymbol(GameObject symbol, Vector2 targetScreenPos)
    {
        ...
        // Store initial scale
        Vector3 initialScale = symbol.transform.localScale;
        Vector3 peakScale = initialScale * 1.5f; // Peak at 150% of original size
        Vector3 finalScale = initialScale * 0.8f; // End at 80% of original size
        ...
        while (elapsed < duration)
        {
            ...
            // Convert current target screen position to world position
            Ray ray = Camera.main.ScreenPointToRay(targetScreenPos);
            Vector3 targetPos = ray.GetPoint(10f);
            
            // Use bezier curve for a nice arc
            Vector3 midPoint = Vector3.Lerp(startPos, targetPos, 0.5f);
            // Add more height to the arc
            midPoint += Vector3.up * 2f;
            
            // Calculate current position along the curve
            float t = Mathf.SmoothStep(0, 1, normalizedTime); // Use SmoothStep for easing
            Vector3 a = Vector3.Lerp(startPos, midPoint, t);
            Vector3 b = Vector3.Lerp(midPoint, targetPos, t);
            symbol.transform.position = Vector3.Lerp(a, b, t);
            
            // Scale animation - grow then shrink
            if (normalizedTime < 0.5f)
            {
                // First half - grow to peak
                float scaleT = normalizedTime * 2f; // 0 to 1 in first half
                symbol.transform.localScale = Vector3.Lerp(initialScale, peakScale, scaleT);
            }
            else
            {
                // Second half - shrink to final
                float scaleT = (normalizedTime - 0.5f) * 2f; // 0 to 1 in second half
                symbol.transform.localScale = Vector3.Lerp(peakScale, finalScale, scaleT);
            }
    ...
    // Add this method to match the pattern of other effect spawning methods
    public void SpawnCoinEffect(Vector3 position, int count = 7, bool fromUI = false)
    {
        StartCoroutine(SpawnCoinEffectCoroutine(position, count, fromUI));
    }

    private IEnumerator SpawnCoinEffectCoroutine(Vector3 position, int count, bool fromUI)
    {
        // Initialize coin pool if necessary
        if (!symbolPools.ContainsKey("coin"))
        {
            InitializePool("coin", coinSymbolPrefab);
        }
        ...
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound(coinSoundName, Camera.main.transform.position);
        }
        
        // Target position is top right of screen (coin counter)
        Vector2 targetScreenPos = new Vector2(Screen.width - 150f, Screen.height - 75f);
        
        // Spawn multiple coins with slight delays
        for (int i = 0; i < count; i++)
        {
            if (symbolPools["coin"].Count == 0) break;
            
            GameObject symbol = symbolPools["coin"].Dequeue();
            
            // Set starting position with random offset
            Vector3 startPos;
            if (fromUI)
            {
                // For UI elements, convert screen position to world space
                Vector2 randomOffset = UnityEngine.Random.insideUnitCircle * coinSpreadRadius;
                Vector3 screenPos = position + new Vector3(randomOffset.x, randomOffset.y, 0);
                Ray ray = Camera.main.ScreenPointToRay(screenPos);
                startPos = ray.GetPoint(10f);
            }
            else
            {
                // For world objects, use direct world position
            ...
            symbol.transform.position = startPos;
            symbol.transform.rotation = cameraTransform.rotation;
            symbol.transform.localScale = baseScale;
            symbol.SetActive(true);
            
            // Start animation coroutine
            StartCoroutine(AnimateCoinEffect(symbol, targetScreenPos));
    ...
    private IEnumerator AnimateCoinEffect(GameObject symbol, Vector2 targetScreenPos)
    {
        // Get the starting position
        Vector3 startPos = symbol.transform.position;
        ...
        // Store initial scale
        Vector3 initialScale = symbol.transform.localScale;
        Vector3 peakScale = initialScale * 1.5f;    // Peak at 150% size
        Vector3 finalScale = initialScale * 0.8f;    // End at 80% size
        
        while (elapsed < coinFlyDuration)
        {
            elapsed += Time.deltaTime;
            float normalizedTime = elapsed / coinFlyDuration;
            
            // Convert target screen position to world position
            Ray ray = Camera.main.ScreenPointToRay(targetScreenPos);
            Vector3 targetPos = ray.GetPoint(10f);
            
            // Create curved path
            Vector3 midPoint = Vector3.Lerp(startPos, targetPos, 0.5f);
            midPoint.y += 1.5f; // Make arc higher
            
            // Use bezier curve for nice arc movement
            ...
            {
                // First half - grow to peak
                float scaleT = normalizedTime * 2f;
                symbol.transform.localScale = Vector3.Lerp(initialScale, peakScale, scaleT);
            }
            else
            {
                // Second half - shrink to final
                float scaleT = (normalizedTime - 0.5f) * 2f;
                symbol.transform.localScale = Vector3.Lerp(peakScale, finalScale, scaleT);
            }
