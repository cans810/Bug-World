...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void DepositPlayerChitin()
    {
        ...
        if (currentChitin > 0)
        {
            ...
            if (showDebugMessages)
            {
                ...
                UIHelper.Instance.ShowInformText($"Deposited {currentChitin} chitin! XP gained: {currentChitin * 10}!");
            }
        }
    }
                ...
                if (currentCrumbs >= requiredCrumbs)
                {
                    ...
                    if (eggCreated)
                    {
                        ...
                        if (showDebugMessages)
                        {
                            ...
                            UIHelper.Instance.ShowInformText($"Used {requiredCrumbs} crumbs to create an egg!");
                        }
                    }
                }
                else
    ...
    private void ApplyIntervalHealing()
    {
        ...
        if (showDebugMessages && debugMessageTimer <= 0f)
        {
            ...
            UIHelper.Instance.ShowInformText($"Healed for {healedThisInterval} HP");
