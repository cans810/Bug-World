...
public class OnScreenJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
                    ...
                    for (int i = 0; i < UnityEngine.Input.touchCount; i++)
                    {
                        ...
                        if (touch.fingerId == activeTouchId)
                        {
                            ...
                            else if (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled)
                            {
                                if (showDebugInfo)
                                {
                                    Debug.Log("Direct input: Active touch ended, resetting joystick");
                                }
    ...
    // Add this public method to allow external scripts to reset the joystick
    public void ResetJoystick()
    {
        if (showDebugInfo)
        {
            Debug.Log("Joystick reset triggered externally");
        }
