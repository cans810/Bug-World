...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        // Check if we have a valid nest reference
        if (cachedNestObject == null)
        {
            Debug.LogError("[CRITICAL] No nest object assigned! Please assign it in the inspector.");
            isAnimatingChitin = false;
            currentChitinAnimation = null;
            yield break;
        }
        ...
        // Get target position from the cached nest object
        ...
        try
        {
            for (int i = 0; i < chitinCount; i++)
            {
                ...
                icon.tag = "ChitinIcon"; // Set tag explicitly
        ...
        catch (System.Exception e)
        {
            ...
            // Clean up any icons that were created
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            // Wait a moment before starting each icon's animation - outside try/catch
            ...
            // Run animation in a separate coroutine
            StartCoroutine(AnimateIconSafe(icon, startPosition, targetPos, originalScale, duration, i+1));
        }
        
        // Wait for all animations to complete (approximate)
    ...
    // Modified helper method with optimized nest position updates
    private IEnumerator AnimateIconSafe(GameObject icon, Vector3 startPosition, Vector3 targetPos, Vector3 originalScale, float duration, int iconIndex)
    {
        ...
        Debug.Log($"[CRITICAL] Starting animation for icon {iconIndex} from {startPosition} to {targetPos}");
        ...
        while (!animationComplete && icon != null)
        {
            try
            {
                ...
                // Update target position every frame using the cached nest object
                if (cachedNestObject != null && frameCount % 2 == 0)
                {
                    ...
                    if (frameCount % 30 == 0)
                    {
                        Debug.Log($"[CRITICAL] Updated target position to {targetPos}");
                    }
                }
                
                // Use a simple arc motion with the updated target position
                ...
                if (frameCount % 30 == 0)
                {
                    Debug.Log($"[CRITICAL] Icon {iconIndex} at position {currentPos}, progress: {normalizedTime:F2}");
                }
            ...
            {
                Debug.LogError($"[CRITICAL] Error animating icon {iconIndex}: {e.Message}");
                animationComplete = true;
            }
        ...
        try
        {
            if (icon != null)
            {
                Debug.Log($"[CRITICAL] Destroying icon {iconIndex}");
                Destroy(icon);
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"[CRITICAL] Error destroying icon {iconIndex}: {e.Message}");
        }
    }
