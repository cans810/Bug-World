...
public class LevelAreaArrowManager : MonoBehaviour
{
    ...
    [HideInInspector] public List<LevelAreaTarget> pendingAreaUnlocks = new List<LevelAreaTarget>();
    ...
    private void Start()
    {
        ...
        // Subscribe to camera animation completion for arrow showing only
        CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
        if (cameraAnimations != null)
        {
            // Subscribe to the completion event to show arrow after animation
            cameraAnimations.OnCameraAnimationCompleted += ShowArrowAfterAnimation;
            
            Debug.Log("Subscribed to camera animation completion event");
        }
        else
        {
    ...
    private void ShowArrowToArea(LevelAreaTarget area)
    {
        ...
        Debug.Log($"Showing arrow to area: {area.areaName} - Arrow will remain until player reaches the area");
    }
    ...
    public void CheckAreaEntry(string borderName)
    {
        ...
        foreach (LevelAreaTarget area in levelAreas)
        {
            ...
            bool targetNameMatch = area.areaTarget == currentTarget; // Additional check to match with current target
            
            Debug.Log($"Name comparison: ExactMatch={exactMatch}, PartialMatch={partialMatch}, TargetMatch={targetNameMatch}");
            
            if (isMatch || exactMatch || partialMatch || targetNameMatch)
            {
                foundMatch = true;
    ...
    // Add this method to adjust the arrow material
    public void SetArrowMaterial(bool isMatte)
    {
        // Get references to arrow renderers
        Renderer[] arrowRenderers = GetComponentsInChildren<Renderer>();
        ...
        foreach (Renderer renderer in arrowRenderers)
        {
            ...
            foreach (Material material in materials)
            {
                if (isMatte)
                {
                    ...
                    if (material.HasProperty("_Glossiness")) // Standard shader property
                    ...
                    if (material.HasProperty("_Specular"))
                        material.SetColor("_Specular", new Color(0.1f, 0.1f, 0.1f));
                        
                    // For emissive materials
                    if (material.HasProperty("_EmissionColor"))
                    {
                        Color currentEmission = material.GetColor("_EmissionColor");
                        // Keep emission but reduce intensity
                        material.SetColor("_EmissionColor", currentEmission * 0.7f);
                    }
                }
                else
                {
                    // Reset to original shiny values if needed
                    if (material.HasProperty("_Smoothness"))
        ...
        Debug.Log($"Arrow material set to {(isMatte ? "matte" : "shiny")} appearance");
    }
    
    private void CheckForAreaUnlocks(int newLevel)
    {
        ...
        foreach (var area in levelAreas)
        {
            ...
            {
                Debug.Log($"Area {area.areaName} unlockable at level {newLevel}");
                
                // Add to pending unlocks instead of immediately triggering
                if (!pendingAreaUnlocks.Contains(area))
                {
                    pendingAreaUnlocks.Add(area);
                    foundNewArea = true;
                }
            }
        }
        
        // Let other systems know areas are unlockable but not yet shown
        if (foundNewArea)
        {
            Debug.Log("New areas are available to unlock");
        }
    }
    
    // Add this public method to get all pending area unlocks for the current level
    public LevelAreaTarget GetPendingAreaUnlockForLevel(int level)
    {
        foreach (var area in pendingAreaUnlocks)
        {
            if (area.requiredLevel == level)
            {
                return area;
            }
        }
    ...
    // Add this to remove area from pending unlocks after it's been shown
    public void RemoveFromPendingUnlocks(LevelAreaTarget area)
    {
        if (pendingAreaUnlocks.Contains(area))
        {
            pendingAreaUnlocks.Remove(area);
            Debug.Log($"Removed {area.areaName} from pending area unlocks");
        }
    }
    
    // Add this new method to the LevelAreaArrowManager class
    public void ShowArrowToNest()
    {
        Debug.Log("Showing arrow to player's nest");
        
        // Try to find the nest GameObject in the scene
        GameObject nest = GameObject.FindGameObjectWithTag("Nest");
        
        // If no object with "Nest" tag, try finding by name
        if (nest == null)
        {
            nest = GameObject.Find("Nest");
        }
        
        // If still not found, try finding NestController
        if (nest == null)
        {
            NestController nestController = FindObjectOfType<NestController>();
            if (nestController != null)
            {
                nest = nestController.gameObject;
            }
        }
        
        // If we found the nest, show the arrow
        if (nest != null)
        {
            // Set current target
            currentTarget = nest;
            ...
            // Update arrow target
            arrowIndicator.SetTarget(nest.transform);
            
            // Show the arrow
            arrowIndicator.ShowArrow();
            isArrowActive = true;
            
            Debug.Log($"Arrow pointing to nest: {nest.name}");
        }
        else
        {
            Debug.LogError("Cannot show arrow to nest: Nest object not found!");
        }
    }
} 