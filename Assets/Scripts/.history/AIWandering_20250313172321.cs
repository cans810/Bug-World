...
public class AIWandering : MonoBehaviour
{
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        {
            // We've reached the waypoint, set a new one
            SetRandomWaypoint();
            return;
        }
    ...
    // Update the animation state in a more reliable way
    private void UpdateAnimationState(bool isWalking)
    {
        // Use AnimationController if available
        if (animController != null)
        {
            // Only update if the state is actually changing to avoid animation glitches
