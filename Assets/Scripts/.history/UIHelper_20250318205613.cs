...
public class UIHelper : MonoBehaviour
{
    ...
    // Add these variables to track active animations
    private Coroutine chitinTextAnimation = null;
    private Coroutine crumbTextAnimation = null;
    private Coroutine eggTextAnimation = null;
    private Coroutine experienceTextAnimation = null;
    private Coroutine coinTextAnimation = null;

    private float originalChitinTextFontSize;
    private float originalCrumbTextFontSize;
    private float originalEggTextFontSize;
    private float originalExperienceTextFontSize;
    private float originalCoinTextFontSize;

    private float textBumpScale = 1.4f;
    ...
    private void Start()
    {
        ...
        if (chitinCountText != null)
        {
            ...
            originalChitinTextFontSize = chitinCountText.fontSize;
        }
        ...
        if (crumbCountText != null)
        {
            ...
            originalCrumbTextFontSize = crumbCountText.fontSize;
        }
        ...
        if (experienceCountText != null)
        {
            ...
            originalExperienceTextFontSize = experienceCountText.fontSize;
        }
        ...
        if (eggCountText != null)
        {
            ...
            originalEggTextFontSize = eggCountText.fontSize;
        }
        ...
        if (coinCountText != null)
        {
            ...
            originalCoinTextFontSize = coinCountText.fontSize;
        }
    ...
    public void UpdateChitinDisplay(int count, bool animate = true)
    {
        if (chitinCountText != null)
        {
            int maxCapacity = 999; // Default if inventory not found
            PlayerInventory inventory = FindObjectOfType<PlayerInventory>();
            if (inventory != null)
            {
                maxCapacity = inventory.MaxChitinCapacity;
            }
            
            // Stop any existing animation
            if (chitinTextAnimation != null)
            {
                StopCoroutine(chitinTextAnimation);
                // Reset to original font size
                chitinCountText.fontSize = originalChitinTextFontSize;
            }
            ...
            {
                chitinTextAnimation = StartCoroutine(AnimateTextBump(chitinCountText, originalChitinTextFontSize * textBumpScale, originalChitinTextFontSize));
            }
        }
    }
    ...
    public void UpdateCrumbDisplay(int count)
    {
        if (crumbCountText != null)
        {
            crumbCountText.text = string.Format(countFormat, count, playerInventory?.MaxCrumbCapacity ?? 100);
    ...
    // Update the AnimateTextBump method to properly handle interruptions
    private IEnumerator AnimateTextBump(TextMeshProUGUI textComponent, float targetSize, float originalSize)
    {
        if (textComponent == null) yield break;
        
        // Animation in progress tracking
        Coroutine* animationRef = GetAnimationReference(textComponent);
        ...
        {
            timer += Time.deltaTime;
            float progress = Mathf.Min(1, timer / (animationDuration / 2));
            float currentSize = Mathf.Lerp(originalSize, targetSize, progress);
            textComponent.fontSize = currentSize;
            yield return null;
        }
        
        // Ensure we hit the target size
        ...
        while (timer < animationDuration / 2)
        {
            timer += Time.deltaTime;
            float progress = Mathf.Min(1, timer / (animationDuration / 2));
            float currentSize = Mathf.Lerp(targetSize, originalSize, progress);
            textComponent.fontSize = currentSize;
            yield return null;
        }
        
        // IMPORTANT: Always ensure we end at the original size
        textComponent.fontSize = originalSize;
        
        // Clear the reference to this animation
        if (animationRef != null)
            *animationRef = null;
    }

    // Helper method to get the appropriate animation reference
    private Coroutine* GetAnimationReference(TextMeshProUGUI textComponent)
    {
        if (textComponent == chitinCountText) return &chitinTextAnimation;
        if (textComponent == crumbCountText) return &crumbTextAnimation;
        if (textComponent == eggCountText) return &eggTextAnimation;
        if (textComponent == experienceCountText) return &experienceTextAnimation;
        if (textComponent == coinCountText) return &coinTextAnimation;
        return null;
    }
} 
