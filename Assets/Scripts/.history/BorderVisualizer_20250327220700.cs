...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // Create a more accurate capsule perimeter
        int segments = 40; // Total number of segments for the perimeter
        perimeterLineRenderer.positionCount = segments;
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            // Get perpendicular vectors to create the circle
            Vector3 perpendicular1 = Vector3.Cross(direction, Vector3.up);
            if (perpendicular1.magnitude < 0.01f)
                perpendicular1 = Vector3.Cross(direction, Vector3.right);
            perpendicular1.Normalize();
            
            Vector3 perpendicular2 = Vector3.Cross(perpendicular1, direction);
            perpendicular2.Normalize();
            
            // First half of the circle (0 to π) - use one end of the capsule
            if (t < Mathf.PI)
            {
                point = center + halfHeight * direction + 
                        radius * (perpendicular1 * Mathf.Cos(t) + perpendicular2 * Mathf.Sin(t));
            }
            // Second half of the circle (π to 2π) - use the other end of the capsule
            else
            {
                point = center - halfHeight * direction + 
                        radius * (perpendicular1 * Mathf.Cos(t) + perpendicular2 * Mathf.Sin(t));
