...
public class EnemyAI : MonoBehaviour
{
    ...
    private void DisableAllOutlines()
    {
        ...
        if (entityOutline != null)
        {
            ...
            Debug.Log($"Disabled EntityOutline on {gameObject.name}");
        }
        
        // Find and disable Outline component
        Outline outline = GetComponent<Outline>();
        if (outline != null)
        {
            outline.enabled = false;
            Debug.Log($"Disabled Outline on {gameObject.name}");
        }
        
        // Also check for outline components in children
        Outline[] childOutlines = GetComponentsInChildren<Outline>();
        foreach (Outline childOutline in childOutlines)
        {
            if (childOutline != null && childOutline.gameObject != gameObject)
            {
                childOutline.enabled = false;
                Debug.Log($"Disabled child Outline on {childOutline.gameObject.name}");
            }
        }
    }
