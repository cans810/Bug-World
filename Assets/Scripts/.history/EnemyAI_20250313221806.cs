...
public class EnemyAI : MonoBehaviour
{
    ...
    // Add these new fields to track player kill state
    private bool hasKilledPlayer = false;
    private float playerKillTime = 0f;
    private float postKillPauseTime = 3f; // Stand still for 3 seconds after killing player
    ...
    // Modify ChaseAndAttackTarget to handle player death correctly
    private void ChaseAndAttackTarget()
    {
        // Check if we just killed the player
        if (currentTarget != null && currentTarget.CompareTag("Player") && currentTarget.IsDead)
        {
            // If we haven't recorded this kill yet
            if (!hasKilledPlayer)
            {
                hasKilledPlayer = true;
                playerKillTime = Time.time;
                
                // Stop moving immediately and face the player's body
                isMoving = false;
                UpdateAnimation(false);
                FaceTarget(currentTarget.transform.position);
                
                // Disable wandering during the pause
                if (wanderingBehavior != null)
                    wanderingBehavior.SetWanderingEnabled(false);
                
                return;
            }
            // If we're within the pause time after killing the player
            else if (Time.time < playerKillTime + postKillPauseTime)
            {
                // Keep standing still
                isMoving = false;
                UpdateAnimation(false);
            ...
            // If we've waited long enough, return to wandering
            else
            {
                ReturnToWandering();
                hasKilledPlayer = false; // Reset kill flag
                return;
            }
        }
        
        // Reset kill tracking if the target is not a dead player
        if (currentTarget == null || !currentTarget.CompareTag("Player") || !currentTarget.IsDead)
        {
    ...
    {
        // Don't return to wandering if we're in the post-kill pause period
        if (hasKilledPlayer && Time.time < playerKillTime + postKillPauseTime)
        {
            return;
        }
