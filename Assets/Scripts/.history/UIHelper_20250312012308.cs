...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        Vector3 startPos = chitinCountText != null ? 
            chitinCountText.transform.position : 
            transform.position;
        
        // Get intermediate position
        Vector3 intermediatePos = posToMoveIconTransform != null ?
        ...
        for (int i = 0; i < chitinCount; i++)
        {
            ...
            Debug.Log($"[CRITICAL] Created icon {i+1} of {chitinCount}");
        }
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            while (timer1 < duration1)
            {
                if (icon == null) break;
                
                timer1 += Time.deltaTime;
                float t = timer1 / duration1;
                icon.transform.position = Vector3.Lerp(iconStartPos, intermediatePos, t);
                yield return null;
            }
            
            if (icon == null) continue;
            
            // Wait at intermediate position
            ...
            while (timer2 < duration2)
            {
                ...
                float t = timer2 / duration2;
                
                // Recalculate nest position each frame
                Vector3 currentNestPos = Camera.main.WorldToScreenPoint(nestObject.transform.position);
                currentNestPos.z = 0;
                
                // Move with arc
                Vector3 newPos = Vector3.Lerp(iconStartPos, currentNestPos, t);
                newPos.y += 50f * Mathf.Sin(t * Mathf.PI);
                
                // Scale down
                icon.transform.localScale = Vector3.Lerp(Vector3.one, Vector3.zero, t);
