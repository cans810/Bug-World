...
public class PlayerController : MonoBehaviour
{
    ...
    // Add these variables to PlayerController
    private bool movementDisabledByUI = false;
    private bool jumpDisabledByUI = false;
    ...
    private void Update()
    {
        // If movement is disabled by UI, skip input processing
        if (movementDisabledByUI)
        {
            // Still process animations to ensure player appears stationary
    ...
    // Fix the SetMovementEnabled method
    public void SetMovementEnabled(bool enabled)
    {
        movementDisabledByUI = !enabled;
        jumpDisabledByUI = !enabled;
        
        // If disabling movement, also stop current movement
        if (!enabled)
        {
            // Stop all movement using the variables that actually exist in your class
            moveDirection = Vector3.zero;
            
            // For a CharacterController component
            if (GetComponent<CharacterController>() != null)
            {
                GetComponent<CharacterController>().Move(Vector3.zero);
            }
            
            // For a Rigidbody component
            Rigidbody rb = GetComponent<Rigidbody>();
            if (rb != null)
            {
        ...
        Debug.Log($"Player movement {(enabled ? "enabled" : "disabled")} by UI");
    }

    // Also modify the Jump method
    public void Jump()
    {
        // Don't allow jumping if disabled by UI
        if (jumpDisabledByUI)
            return;
        
        // Rest of your existing Jump code...
    }
}