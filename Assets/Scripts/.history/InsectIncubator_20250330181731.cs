...
public class InsectIncubator : MonoBehaviour
{
    ...
    public GameObject CreateInsectEgg(string entityType, Vector3 positionOffset = default)
    {
        ...
        {
            uiHelper.UpdateEggDisplay(currentEggCount);
            Debug.Log($"Updated UI for new egg. Current count: {currentEggCount}");
        }
        else
        {
            // Try to find UIHelper if not assigned
            UIHelper helper = FindObjectOfType<UIHelper>();
            if (helper != null)
            {
                helper.UpdateEggDisplay(currentEggCount);
                Debug.Log($"Found and updated UIHelper for new egg. Current count: {currentEggCount}");
            }
            else
            {
                Debug.LogWarning("Could not find UIHelper to update egg display");
            }
        }
        
        // Update internal UI elements
        UpdateEggCapacityUI();
    ...
    // Improve the OnEggHatched method to handle multiple eggs
    public void OnEggHatched()
    {
        // Decrement egg count
        currentEggCount = Mathf.Max(0, currentEggCount - 1);
        
        // If we have no more eggs, reset the EggExists flag
        ...
        {
            // We still have eggs, so keep EggExists true
            EggExists = true;
        }
        
        Debug.Log($"Egg hatched notification received. Updated egg count: {currentEggCount}");
        
        // Update UI through all available paths to ensure it gets updated
        // 1. Direct UIHelper update
        if (uiHelper != null)
        {
            uiHelper.UpdateEggDisplay(currentEggCount);
            Debug.Log($"Updated UIHelper egg display after hatching. Count: {currentEggCount}");
        }
        else
        {
            // Try to find UIHelper if not assigned
            ...
            {
                helper.UpdateEggDisplay(currentEggCount);
                Debug.Log($"Found and updated UIHelper after hatching. Count: {currentEggCount}");
            }
        }
        
        // 2. Update internal UI elements
        UpdateEggCapacityUI();
        
        // 3. Force a full UI update
        UpdateAllUI();
    }
    ...
    // Update the GetMaxEggCapacity method to use PlayerInventory directly
    public int GetMaxEggCapacity()
    {
        // Get the max capacity directly from the PlayerInventory
        if (playerInventory != null && playerInventory.MaxEggCapacity > 0)
        {
            Debug.Log($"Using max egg capacity from PlayerInventory: {playerInventory.MaxEggCapacity}");
            return playerInventory.MaxEggCapacity;
        }
        
        Debug.LogWarning("PlayerInventory not found or invalid capacity, using default egg capacity");
    ...
    // Add this getter method for the current egg count
    ...
    // Update CreateMultipleEggs to ensure proper counting and UI update
    public bool CreateMultipleEggs(string entityType, int count)
    {
        ...
        float eggSpacing = 0.5f;
        
        // Store created eggs
        List<GameObject> createdEggs = new List<GameObject>();
        
        // Store the original egg count to calculate the correct final count
        int startingEggCount = currentEggCount;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create egg with offset - but don't rely on its internal count update
            GameObject egg = CreateInsectEggWithoutCountUpdate(entityType, offset);
            ...
            if (egg == null)
            {
                ...
                // Force update egg count based on what we actually created
                currentEggCount = startingEggCount + createdEggs.Count;
                
                // Update UI with the current egg count
                UpdateAllUI();
        ...
        // Set egg count explicitly based on what we created
        currentEggCount = startingEggCount + count;
        
        // Force update the UI with the correct count
        UpdateAllUI();
        
        Debug.Log($"Successfully created {count} {entityType} eggs in a row. New egg count: {currentEggCount}");
        return true;
    }

    // Add this method to create an egg without updating the count
    private GameObject CreateInsectEggWithoutCountUpdate(string entityType, Vector3 positionOffset = default)
    {
        // No eggs allowed if at capacity
        if (currentEggCount >= maxEggs)
        {
        ...
        // Don't update the egg count here - we'll do it manually after all eggs are created
    ...
    {
        // Ensure UI is fully updated with the correct count
        if (uiHelper != null)
        {
            uiHelper.UpdateEggDisplay(currentEggCount);
            Debug.Log($"Updated UI with final egg count: {currentEggCount}");
        }
        else
        {
            // Try to find UIHelper if not assigned
            ...
            {
                helper.UpdateEggDisplay(currentEggCount);
                Debug.Log($"Found and updated UIHelper with final egg count: {currentEggCount}");
            }
        }
    ...
    void OnEnable()
    {
        // Subscribe to the egg countdown complete event
        AllyEggController.OnCountdownComplete += HandleEggHatched;
    }
    ...
    // Add a handler for the countdown complete event
    private void HandleEggHatched(AllyEggController eggController)
    {
        Debug.Log("InsectIncubator received egg hatched event from AllyEggController");
        
        // Call our existing method to update the UI
        OnEggHatched();
    }
}
