...
public class CameraAnimations : MonoBehaviour
{
    ...
    [SerializeField] private float horizontalBoundary = 50f; // Horizontal movement limit (sides)
    [SerializeField] private float verticalBoundary = 100f; // Forward/backward movement limit
    ...
    private void HandleMapViewDragging()
    {
        ...
        else if (Input.GetMouseButton(0) && isDragging)
        {
            ...
            // Apply different limits for horizontal and vertical movement
            ...
            Vector3 offsetXZ = newPositionXZ - mapCenterXZ;
            
            // Clamp X movement (left/right)
            if (Mathf.Abs(offsetXZ.x) > horizontalBoundary)
            {
                float sign = Mathf.Sign(offsetXZ.x);
                newPosition.x = mapCenterXZ.x + sign * horizontalBoundary;
            }
            
            // Clamp Z movement (forward/backward)
            if (Mathf.Abs(offsetXZ.z) > verticalBoundary)
            {
                float sign = Mathf.Sign(offsetXZ.z);
                newPosition.z = mapCenterXZ.z + sign * verticalBoundary;
            }
