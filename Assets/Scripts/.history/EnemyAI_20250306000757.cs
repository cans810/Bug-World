...
public class EnemyAI : MonoBehaviour
{
    ...
    // Updated helper method to update animations - simplified direct approach
    private void UpdateAnimation(bool isWalking, bool isAttacking)
    {
        ...
        // SIMPLIFIED APPROACH: Handle AttackWalking state separately
        if (isWalking && isAttacking && hasAttackWalkingAnimation)
        {
            // Try direct state name approach - no fancy parameter setting
            Animator animator = animController.Animator;
            if (animator != null)
            {
                // Find the exact name of the animation state - first log all available states
                var stateInfo = animator.GetCurrentAnimatorStateInfo(0);
                Debug.Log($"Current state hash: {stateInfo.shortNameHash}, full: {stateInfo.fullPathHash}");
                
                // Try direct crossfade to target state - try several common naming patterns
                try {
                    // Set parameters to a known state to help transitions
                    animator.SetBool("Idle", false);
                    animator.SetBool("Walk", false);
                    animator.SetBool("Attack", false);
                    animator.SetBool("AttackWalking", true);
                    
                    // Force immediate transitions using CrossFade
                    animator.CrossFade("AttackWalking", 0.1f);
                    
                    Debug.Log("Applied DIRECT AttackWalking transition!");
                }
                catch (System.Exception e) {
                    Debug.LogError($"Error directly setting animation state: {e.Message}");
                }
            }
        }
        else
        {
            // Reset the attack walking state since we're not using it
            if (hasAttackWalkingAnimation && animController.Animator != null)
            {
                animController.Animator.SetBool("AttackWalking", false);
            }
            
            // Handle normal walking/attacking states
