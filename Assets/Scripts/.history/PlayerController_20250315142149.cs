...
public class PlayerController : MonoBehaviour
{
    ...
    // Add this method to periodically check for enemies in range
    private IEnumerator CheckForEnemiesInRange()
    {
        while (enabled)
        {
            ...
            if (enemiesInRange.Count == 0)
            {
                ...
                float attackRange = 3f; // Reduce from 5f to 3f for more precise detection
                ...
                foreach (Collider col in colliders)
                {
                    ...
                    if (col.gameObject.layer == LayerMask.NameToLayer("Insects"))
                    {
                        ...
                        if (enemy != null && !enemy.IsDead)
                        {
                            ...
                            // Only consider enemies that are somewhat in front of the player (within ~90 degrees)
                            if (dotProduct > 0)
                            {
                                foundEnemies = true;
