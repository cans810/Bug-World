...
public class EntityOutline : MonoBehaviour
{
    ...
    // Add this method to exclude certain layers from the outline effect
    private void ExcludeLayersFromOutline()
    {
        ...
        foreach (Renderer renderer in allRenderers)
        {
            ...
            {
                // Add this renderer to the outline's ignored renderers list
                outline.AddIgnoredRenderer(renderer);
                Debug.Log($"Excluded renderer on {renderer.gameObject.name} from outline (layer: {LayerMask.LayerToName(renderer.gameObject.layer)})");
            }
        }
    }
    ...
    // Modify the ForceUpdate method to also exclude layers
    public void ForceUpdate()
    {
        if (outline != null)
        {
            ...
            // Clear any ignored renderers before re-enabling
            outline.ClearIgnoredRenderers();
            
            outline.enabled = true;
            
            // Re-exclude certain layers
            if (excludeCarriedLoot)
            {
                ExcludeLayersFromOutline();
            }
