...
public class PlayerInventory : MonoBehaviour
{
    ...
    private void ProcessLevelUp(int level)
    {
        ...
        if (uiHelper != null)
        {
            // Update the message to include the crumb capacity increase
            string rewardsMessage = $"+{playerAttributes.pointsPerLevel} Attribute Points\n+2 Crumb Capacity";
            uiHelper.ShowLevelUpRewardsText(rewardsMessage);
            
            // Make sure UI is refreshed with new capacity
            uiHelper.UpdateCrumbDisplay(CrumbCount);
        }
