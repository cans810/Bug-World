...
public class EnemyAI : MonoBehaviour
{
    ...
    [SerializeField] private bool matchPlayerSpeed = false; // Changed to false to prevent matching player speed
    ...
    private void MoveWithBoundaryCheck(Vector3 direction, bool isChasing = false)
    {
        ...
        if (isChasing) 
        {
            // Apply chase multiplier
            currentSpeed *= chaseSpeedMultiplier;
            
            // If we should match player speed and we have a valid target
            if (matchPlayerSpeed && currentTarget != null)
            {
                // Get player's move speed (either from LivingEntity or direct from PlayerController)
                float playerSpeed = 0f;
                
                // Try to get from LivingEntity first
                LivingEntity playerEntity = currentTarget.GetComponent<LivingEntity>();
                if (playerEntity != null)
                {
                    playerSpeed = playerEntity.moveSpeed;
                }
                // If player has no LivingEntity or speed is 0, try PlayerController
                else
                {
                    // Since we don't know the exact property name in PlayerController,
                    // we'll try to get the player's speed in a more generic way
                    PlayerController playerController = currentTarget.GetComponent<PlayerController>();
                    if (playerController != null)
                    {
                        // Since PlayerController doesn't have a moveSpeed property, 
                        // we need to use another approach. Let's use a fixed value 
                        // that matches what the player is expected to move at
                        playerSpeed = 5.0f; // Set this to match your player's typical movement speed
                        
                        // Alternatively, you can add a public field to this EnemyAI class to set the player speed:
                        // [SerializeField] private float defaultPlayerSpeed = 5.0f;
                    }
                }
                
                // If player's speed is valid and our speed is higher, match it
                if (playerSpeed > 0 && currentSpeed > playerSpeed)
                {
