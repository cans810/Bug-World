...
public class GameManager : MonoBehaviour
{
    ...
    {
        Debug.Log("=== STARTING GAME DATA LOAD PROCESS ===");
        ...
        if (loadedData != null)
        {
            Debug.Log("Successfully loaded game data from file");
        ...
        {
            Debug.LogWarning("No saved game data found or data is invalid. Using default values.");
        ...
        Debug.Log("=== GAME DATA LOAD PROCESS COMPLETE ===");
    }
    ...
    public void ApplyLoadedDataToGameState()
    {
        ...
        if (playerInventory != null)
        {
            Debug.Log("Applying inventory data to PlayerInventory");
            
            // Set loading state to prevent triggering level-up events
            playerInventory.SetLoadingDataState(true);
            
            // Apply inventory data directly
            playerInventory.LoadInventoryData(gameData);
            
            Debug.Log($"Applied inventory data: Level={gameData.currentLevel}, XP={gameData.currentXP}, " +
                      $"Chitin={gameData.currentChitin}, Crumb={gameData.currentCrumb}, Egg={gameData.currentEgg}, " +
                      $"Coin={gameData.currentCoin}");
        }
        else
        {
            Debug.LogError("PlayerInventory not found! Cannot apply inventory data.");
        }
        
        // Apply player attributes
        if (playerAttributes != null)
        {
            Debug.Log("Applying attribute data to PlayerAttributes");
            
            // Load attributes directly
            playerAttributes.LoadPlayerAttributes(gameData);
        ...
        {
            Debug.LogError("PlayerAttributes not found! Cannot apply attribute data.");
        }
        
        // Apply player health
        if (playerEntity != null)
        {
            Debug.Log("Applying health data to player entity");
        ...
        {
            Debug.LogError("Player entity not found! Cannot apply health data.");
        }
        
        // Load discovered entities
        if (entitiesController != null)
        {
            Debug.Log("Applying entity data to DiscoveredEntitiesController");
            entitiesController.LoadEntityPurchaseStatus(gameData);
        }
        else
        {
            Debug.LogWarning("DiscoveredEntitiesController not found! Cannot apply entity data.");
        }
        
        // Reset loading state in PlayerInventory
        if (playerInventory != null)
        {
