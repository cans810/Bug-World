...
public class UIVisualEffectController : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinSymbolsImmediate(int count, int xpAmount)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            Vector2 targetPos = new Vector2(
                ...
                screenCenter.y + 200f - (row * rowSpacing) // Lower rows are positioned below
            );
    ...
    private IEnumerator AnimateXPToPanel(GameObject xpSymbol, Vector2 targetPosition)
    {
        ...
        // Play sound for EACH XP symbol that reaches the target, but with slight variations
        if (SoundEffectManager.Instance != null)
        {
            // Add slight random delay to avoid perfect synchronization of sounds
            yield return new WaitForSeconds(Random.Range(0.01f, 0.05f));
            
            // Play the sound with randomized pitch to create distinction between overlapping sounds
            float randomPitch = Random.Range(0.9f, 1.1f);
            SoundEffectManager.Instance.PlaySoundWithPitch("XPGained", randomPitch);
        }
