...
public class EnemyAI : MonoBehaviour
{
    ...
    private void MoveWithBoundaryCheck(Vector3 direction)
    {
        ...
        {
            // We've hit the map boundary - completely stop all movement
            ...
            // Mark as hitting boundary
            isHittingBoundary = true;
            boundaryRedirectionTime = Time.time + BOUNDARY_REDIRECT_DURATION;
            
            // Get direction directly away from boundary center
            Vector3 awayFromBoundary = (transform.position - mapBoundary.transform.position).normalized;
            
            // Turn away from boundary
            livingEntity.RotateTowards(awayFromBoundary, 5.0f);
            
            // Reset target if we're chasing
            if (isChasing)
            {
                lastKnownPlayerPosition = transform.position + awayFromBoundary * 5f;
            }
        }
        else
        {
