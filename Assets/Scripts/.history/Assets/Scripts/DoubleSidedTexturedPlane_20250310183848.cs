...
public class DoubleSidedTexturedPlane : MonoBehaviour
{
    ...
    public void CreateDoubleSidedPlane()
    {
        ...
        if (createSides)
        {
            ...
            if (texturedSides)
            {
                // Use the same textured material for sides
                sideMaterial = textureMaterial;
            }
            else
            {
                // Create material for sides with solid color
            ...
            // Create four sides with custom UV mapping for textured sides
                           ...
                           Vector2 uvStart, Vector2 uvEnd)
    {
        ...
        // If using textured sides, adjust the UVs to match the edge of the texture
        if (texturedSides)
        {
            Mesh mesh = side.GetComponent<MeshFilter>().mesh;
            Vector2[] uvs = mesh.uv;
            
            // Remap UVs to only use the edge of the texture
            for (int i = 0; i < uvs.Length; i++)
            {
                float u = uvs[i].x;
                float v = uvs[i].y;
                
                // Map u from 0-1 to uvStart.x-uvEnd.x
                // Map v from 0-1 to uvStart.y-uvEnd.y
                uvs[i].x = Mathf.Lerp(uvStart.x, uvEnd.x, u);
                uvs[i].y = Mathf.Lerp(uvStart.y, uvEnd.y, v);
            }
