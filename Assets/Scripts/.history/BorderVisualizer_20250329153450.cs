...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Debug.Log($"EntityGenerator {gameObject.name} Awake - active: {gameObject.activeSelf}");
    }
    ...
    private void Start()
    {
        ...
        Debug.Log($"EntityGenerator {gameObject.name} Start - enableSpawning: {spawningEnabled}");
    }
    ...
    void OnEnable()
    {
        Debug.Log($"EntityGenerator {gameObject.name} OnEnable");
    }

    void OnDisable()
    {
        Debug.Log($"EntityGenerator {gameObject.name} OnDisable - caused by: {new System.Diagnostics.StackTrace().ToString()}");
    }
} 