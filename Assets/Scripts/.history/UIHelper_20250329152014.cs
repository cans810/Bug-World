...
public class UIHelper : MonoBehaviour
{
    ...
    public void Give50CoinsForRevivalClicked()
    {
        ...
        if (playerInventory != null)
        {
            ...
            if (playerInventory.CoinCount >= 50)
            {
                ...
                if (playerCoins != null && playerCoins.SpendCoins(50))
                {
                    coinsRemoved = true;
                }
                ...
                else if (playerInventory != null)
                {
                    ...
                    {
                        removeMethod.Invoke(playerInventory, new object[] { 50 });
                        coinsRemoved = true;
                    }
                    ...
                    else
                    {
                        ...
                        if (spendMethod != null)
                        {
                            bool result = (bool)spendMethod.Invoke(playerInventory, new object[] { 50 });
                            coinsRemoved = result;
                        }
                    }
                }
    ...
    private void ToggleAllEntityIcons(bool showIcons)
    {
        ...
        Debug.Log($"ToggleAllEntityIcons: Found {allBorders.Length} borders, showIcons={showIcons}");
        
        foreach (var border in allBorders)
        {
            string borderName = border.gameObject.name;
            bool isUnlocked = border.IsSpawningEnabled() || border.IsDisabled();
            bool hasIcons = border.HasEntityIcons();
            
            Debug.Log($"Border: {borderName}, IsUnlocked: {isUnlocked}, HasIcons: {hasIcons}");
            
            if (showIcons)
            {
                // Only show icons for borders that haven't been unlocked yet
                if (!isUnlocked && hasIcons)
                {
                    border.ActivateEntityIcons();
                    Debug.Log($"<color=green>Activated</color> icons for {borderName}");
                }
                else
                {
                    Debug.Log($"<color=yellow>Not showing</color> icons for {borderName}: unlocked={isUnlocked}, hasIcons={hasIcons}");
                }
            }
            else
            {
