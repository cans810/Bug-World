...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnCoinSymbols(Vector3 panelPosition, int count, int totalCoinAmount)
    {
        ...
        if (coinPanel == null)
        {
            ...
            StartCoroutine(SpawnCoinSymbolsWithTarget(panelPosition, count, totalCoinAmount, screenTopRight));
        }
        else
        {
            ...
            {
                // Convert the panel position to screen space
                Canvas canvas = coinPanel.GetComponentInParent<Canvas>();
                Camera uiCamera = canvas != null ? canvas.worldCamera : Camera.main;
                
                // Get the center position of the coin panel in screen space
                Vector3[] corners = new Vector3[4];
                coinPanelRect.GetWorldCorners(corners);
                Vector3 center = (corners[0] + corners[2]) / 2f; // Average of bottom-left and top-right
                
                Debug.Log($"Targeting coins to panel at world position: {center}");
                
                // Start the actual spawn coroutine with the panel's position
                StartCoroutine(SpawnCoinSymbolsWithTarget(panelPosition, count, totalCoinAmount, uiCamera.WorldToScreenPoint(center)));
            }
            else
            {
