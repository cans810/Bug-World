...
public class CameraAnimations : MonoBehaviour
{
    ...
    // Modify the OnEggAnimationComplete method to ensure AllyAI components are enabled
    private void OnEggAnimationComplete(GameObject insectObject)
    {
        if (insectObject == null) return;
        
        // Get all components that need to be re-enabled
        AllyAI allyAI = insectObject.GetComponent<AllyAI>();
        AIWandering wandering = insectObject.GetComponent<AIWandering>();
        
        // Enable AllyAI with extra error checking
        if (allyAI != null)
        {
            allyAI.enabled = true;
            allyAI.ForceEnableMovement(); // Use our new method for extra reliability
            Debug.Log($"Directly re-enabled AllyAI for {insectObject.name}");
        }
        else
        {
            // Try to find AllyAI in children if not on the main object
            AllyAI[] childAIs = insectObject.GetComponentsInChildren<AllyAI>(true);
            foreach (var childAI in childAIs)
            {
                childAI.enabled = true;
                childAI.ForceEnableMovement();
                Debug.Log($"Re-enabled child AllyAI component on {childAI.gameObject.name}");
            }
        }
        ...
        {
            wandering.SetWanderingEnabled(true);
            Debug.Log($"Directly re-enabled wandering for {insectObject.name}");
        }
        else
        {
            // Try to find AIWandering in children
            AIWandering[] childWanderings = insectObject.GetComponentsInChildren<AIWandering>(true);
            foreach (var childWandering in childWanderings)
            {
                childWandering.SetWanderingEnabled(true);
                Debug.Log($"Re-enabled child AIWandering component on {childWandering.gameObject.name}");
            }
        }
        ...
        foreach (var controller in eggControllers)
        {
            controller.OnCameraAnimationComplete(insectObject);
        }
        
        // Also keep the SendMessage as a fallback
        SendMessageOptions options = SendMessageOptions.DontRequireReceiver;
        GameObject.FindObjectOfType<AllyEggController>()?.gameObject.SendMessage(
            "OnCameraAnimationComplete", insectObject, options);
    }
} 