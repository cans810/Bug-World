...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private void InitializePool(string type, GameObject prefab)
    {
        ...
        poolContainer.transform.SetParent(null); // First create it without a parent
        poolContainer.transform.localScale = Vector3.one;
        poolContainer.transform.SetParent(transform, true); // Then parent it while preserving world scale
        ...
        for (int i = 0; i < poolSizePerType; i++)
        {
            ...
            // Force world scale to be exactly what we want
            Vector3 targetScale = type == "chitin" ? chitinScale : baseScale;
            symbol.transform.localScale = targetScale;
            Debug.Log($"Symbol {i} scale after setting: {symbol.transform.localScale}");

            // Parent while preserving world scale
