...
public class EntityGenerator : MonoBehaviour
{
    ...
    [SerializeField] private bool restrictEntitiesToBoundary = true; // Whether entities should stay within boundary
    ...
    private void TrySpawnEntity()
    {
        ...
        if (TryGetSpawnPosition(out spawnPosition))
        {
            ...
            // Set boundary for the entity if restrictEntitiesToBoundary is enabled
            if (restrictEntitiesToBoundary)
            {
                // Apply boundary to AI scripts
                EnemyAI enemyAI = newEntity.GetComponent<EnemyAI>();
                if (enemyAI != null)
                {
                    enemyAI.SetBoundaryOrigin(transform.position);
                    enemyAI.SetBoundaryRadius(spawnRadius);
                    
                    // Add ring boundary support
                    if (useRingSpawning)
                    {
                        enemyAI.SetInnerBoundaryRadius(innerRadius);
                    }
                }
                ...
                if (wandering != null)
                {
                    wandering.SetBoundaryOrigin(transform.position);
                    wandering.SetBoundaryRadius(spawnRadius);
                    
                    // Add ring boundary support
                    if (useRingSpawning)
                    {
                        wandering.SetInnerBoundaryRadius(innerRadius);
                    }
                }
            }
