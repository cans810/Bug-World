...
public class VisualEffectManager : MonoBehaviour
{
    ...
    {
        int symbolCount = 12; // Increased from 8 to create a denser ray
        ...
        // Create a central ray direction with slight randomness
        ...
        for (int i = 0; i < symbolCount; i++)
        {
            ...
            // Much tighter clustering around the central ray
            Vector3 randomOffset = new Vector3(
                Random.Range(-0.2f, 0.2f),  // Tighter horizontal spread
                0.1f * i,                    // Stagger height slightly based on index
                Random.Range(-0.2f, 0.2f)    // Tighter depth spread
            );
            ...
            // Almost no delay between symbols for a continuous ray effect
            StartCoroutine(AnimateLevelUpSymbolInRay(symbol, playerObj.transform, rayDirection, i * 0.02f, () => {
                completedAnimations++;
    ...
    // New animation method specifically for ray-like movement
    private IEnumerator AnimateLevelUpSymbolInRay(GameObject symbol, Transform playerTransform, Vector3 rayDirection, float delay, System.Action onComplete)
    {
        ...
        float animDuration = 1.2f;  // Even faster for ray effect
        float maxHeight = 10.0f;    // Higher maximum height for longer ray
        float elapsed = 0f;
        
        // Create a slightly curved ray path
        Vector3 rayOffset = new Vector3(
            Random.Range(-0.1f, 0.1f), // Very small random offset
            0,
            Random.Range(-0.1f, 0.1f)
        ).normalized * 0.3f;
        
        // Ray velocity increases with symbol index for a spreading effect
        float rayVelocityMultiplier = 1.0f + (delay * 5.0f);
        ...
        while (elapsed < animDuration && playerTransform != null)
        {
            ...
            // Accelerating upward movement for ray effect
            float verticalProgress = Mathf.Pow(normalizedTime, 0.7f); // Accelerating curve
            float currentHeight = Mathf.Lerp(0, maxHeight * rayVelocityMultiplier, verticalProgress);
            
            // Almost no horizontal drift for a straight ray
            float rayIntegrity = 0.95f; // How straight the ray is (0-1)
            float horizontalDrift = Mathf.Sin(normalizedTime * 3f) * 0.1f * (1f - rayIntegrity);
            
            // Calculate new position - ray-like trajectory
            Vector3 newPosition = startPosition + new Vector3(
                horizontalDrift + rayOffset.x * currentHeight * 0.1f,
                currentHeight,
                rayOffset.z * currentHeight * 0.1f
            );
            ...
            // Scale growth with slight pulse
            float baseGrowth = normalizedTime < 0.3f ? 
                Mathf.Lerp(0.1f, 1.0f, normalizedTime / 0.3f) : 
                1.0f;
                
            // Add subtle pulse
            float pulse = 1.0f + 0.1f * Mathf.Sin(normalizedTime * 15f);
            ...
            if (normalizedTime < 0.1f) {
                ...
                alpha = Mathf.Lerp(1f, 0f, (normalizedTime - 0.7f) / 0.3f);
            }
            else {
                alpha = 1f;
            }
