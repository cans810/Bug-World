...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [SerializeField] private RectTransform safeAreaTransform; // Reference to UIHelper.SafeArea
    ...
    private void Start()
    {
        ...
        // Find SafeArea reference if not assigned
        if (safeAreaTransform == null)
        {
            ...
            {
                Debug.LogWarning("SafeArea reference not found! Chitin animations may not work correctly.");
            }
        }
    }
    ...
    private IEnumerator SpawnChitinCollectEffect(int count)
    {
        ...
        if (safeAreaTransform == null)
        {
            Debug.LogError("SafeArea reference is missing! Cannot spawn chitin effects.");
            yield break;
        }

        // Get the screen position of the chitin panel
        Vector2 chitinPanelPos = new Vector2(Screen.width + chitinSpawnOffsetX, Screen.height - spawnOffsetY);
        
        // Get the screen position of the active nest
        Vector3 nestScreenPos = Camera.main.WorldToScreenPoint(activeNestTransform.position);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Create random offset around panel position
            Vector2 randomOffset = Random.insideUnitCircle * spreadRadius;
            Vector2 spawnScreenPos = chitinPanelPos + randomOffset;
            
            // Create a new UI version of the symbol in the SafeArea
            GameObject uiSymbol = Instantiate(chitinSymbolPrefab, safeAreaTransform);
            RectTransform uiSymbolRect = uiSymbol.GetComponent<RectTransform>();
            
            // Set the anchored position based on screen coordinates
            if (uiSymbolRect != null)
            {
                // Convert screen position to anchored position in SafeArea
                Vector2 anchoredPos;
                ...
                uiSymbolRect.anchoredPosition = anchoredPos;
                
                // Start UI-based animation
                StartCoroutine(AnimateChitinInUI(uiSymbol, uiSymbolRect, nestScreenPos));
            }
            
            // Return the original symbol to the pool immediately (we're not using it)
            symbol.SetActive(false);
    ...
    // New method to animate chitin symbols in UI space
    private IEnumerator AnimateChitinInUI(GameObject uiSymbol, RectTransform uiSymbolRect, Vector3 nestScreenPos)
    {
        // Make sure UI symbol is active and visible
        uiSymbol.SetActive(true);
        
        // Get starting position
        Vector2 startPos = uiSymbolRect.anchoredPosition;
        
        // Convert nest screen position to anchored position
        Vector2 endAnchoredPos;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(
            safeAreaTransform,
            new Vector2(nestScreenPos.x, nestScreenPos.y),
            null, // No camera for Screen Space - Overlay
            out endAnchoredPos
        );
        
        // Calculate a midpoint for the arc (in anchored position space)
        Vector2 midPoint = Vector2.Lerp(startPos, endAnchoredPos, 0.5f);
        midPoint.y += Random.Range(100f, 200f); // Add some height for the arc
        
        float elapsed = 0f;
        
        // Get image component for fading
        Image symbolImage = uiSymbol.GetComponent<Image>();
        Color originalColor = symbolImage ? symbolImage.color : Color.white;
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            Vector2 b = Vector2.Lerp(midPoint, endAnchoredPos, curveValue);
            Vector2 newPos = Vector2.Lerp(a, b, curveValue);
            
            // Update position
            uiSymbolRect.anchoredPosition = newPos;
            
            // Fade out near the end
            if (normalizedTime > 0.8f && symbolImage != null)
            {
                float alpha = 1f - ((normalizedTime - 0.8f) / 0.2f);
                symbolImage.color = new Color(originalColor.r, originalColor.g, originalColor.b, alpha);
            }
            
            // Play sound when close to the nest
        ...
        // Destroy the UI symbol when animation is complete
        Destroy(uiSymbol);
    }
