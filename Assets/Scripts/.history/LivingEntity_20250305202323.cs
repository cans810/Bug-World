...
public class LivingEntity : MonoBehaviour
{
    ...
    [Header("Drops")]
    [SerializeField] private int chitinAmount = 1; // Default: drop 1 chitin
    [SerializeField] private bool randomizeChitinAmount = false;
    [SerializeField] private Vector2Int chitinRange = new Vector2Int(1, 3); // Min/max if randomized
    ...
    public int ChitinAmount 
    { 
        get 
        {
            if (randomizeChitinAmount)
            {
                return Random.Range(chitinRange.x, chitinRange.y + 1); // +1 because Range is exclusive on max
            }
            return chitinAmount;
        }
        set { chitinAmount = value; }
    }
