...
public class AIWandering : MonoBehaviour
{
    ...
    [Header("Player Border Avoidance")]
    [SerializeField] private bool avoidPlayerBorder = true;
    [SerializeField] private float playerBorderAvoidanceDistance = 15f; // Increased from 10f to 15f
    [SerializeField] private string playerBorderName = "PlayerBorder1";
    [SerializeField] private float emergencyTurnForce = 3f; // New parameter for stronger turning when too close
    ...
    private Transform playerBorderTransform;
    private float playerBorderRadius = 0f;
    ...
    private void Update()
    {
        ...
        // Add active border avoidance check in Update for immediate response
        if (!isHittingBoundary && avoidPlayerBorder && playerBorderTransform != null && !IsAlly())
        {
            // Check if we're getting too close to the border
            float currentDistanceToBorder = Vector3.Distance(transform.position, playerBorderTransform.position);
            if (currentDistanceToBorder < (playerBorderRadius + playerBorderAvoidanceDistance))
            {
                // We're too close to the border - immediately turn away
                Vector3 awayDirection = GetDirectionAwayFromBorder(transform.position);
                
                // Force immediate rotation away from border
                transform.rotation = Quaternion.LookRotation(awayDirection);
                
                // Force new waypoint away from border
                currentWaypoint = transform.position + awayDirection * maxWanderDistance;
                
                // Log for debugging
                Debug.LogWarning($"{gameObject.name}: Emergency border avoidance triggered!");
            }
        }
    }
    ...
    private IEnumerator WanderingRoutine()
    {
        while (true)
        {
            ...
            while (moveTimer < moveDuration && isWandering)
            {
                ...
                // ADD THIS: Check for player border proximity during movement
                if (avoidPlayerBorder && playerBorderTransform != null && !IsAlly())
                {
                    float currentDistanceToBorder = Vector3.Distance(transform.position, playerBorderTransform.position);
                    if (currentDistanceToBorder < (playerBorderRadius + playerBorderAvoidanceDistance * 0.8f))
                    {
                        Debug.LogWarning($"{gameObject.name}: Getting too close to player border during movement!");
                        
                        // Emergency direction change
                        Vector3 emergencyDirection = GetDirectionAwayFromBorder(transform.position);
                        transform.rotation = Quaternion.LookRotation(emergencyDirection);
                        
                        // Break out of movement loop to get new waypoint
                        break;
                    }
                }
    ...
    private void FindPlayerBorder()
    {
        // Find the player border by name
        GameObject borderObject = GameObject.Find(playerBorderName);
        if (borderObject != null)
        {
            playerBorderTransform = borderObject.transform;
            
            // Get the sphere collider to determine radius
            SphereCollider borderCollider = borderObject.GetComponent<SphereCollider>();
            BoxCollider boxCollider = borderObject.GetComponent<BoxCollider>();
            
            if (borderCollider != null)
            {
                playerBorderRadius = borderCollider.radius * Mathf.Max(
                    borderObject.transform.lossyScale.x,
                    borderObject.transform.lossyScale.y,
                    borderObject.transform.lossyScale.z
                );
                Debug.Log($"Found player border with sphere radius: {playerBorderRadius}");
            }
            else if (boxCollider != null)
            {
                // For box collider, use the largest dimension as an approximation
                playerBorderRadius = Mathf.Max(
                    boxCollider.size.x * borderObject.transform.lossyScale.x,
                    boxCollider.size.y * borderObject.transform.lossyScale.y,
                    boxCollider.size.z * borderObject.transform.lossyScale.z
                ) * 0.5f; // Half the size since radius is from center
                Debug.Log($"Found player border with approximate radius: {playerBorderRadius}");
            }
            else
            {
                // Default radius if no collider found
                playerBorderRadius = 10f;
                Debug.LogWarning("Player border found but has no SphereCollider or BoxCollider, using default radius");
            }
        }
        else
        {
            Debug.LogWarning($"Player border '{playerBorderName}' not found in scene");
        }
    }
    ...
    // Modify IsPositionTooCloseToNest to also check for player border
    private bool IsPositionTooCloseToNest(Vector3 position)
    {
        ...
        // Original check for nest proximity
        ...
        if (avoidPlayerNest && playerNestTransform != null)
        {
            ...
            tooCloseToNest = distanceToNest < (nestRadius + nestAvoidanceDistance);
        }
        
        // Additional check for player border proximity
        bool tooCloseToBorder = false;
        if (avoidPlayerBorder && playerBorderTransform != null)
        {
            float distanceToBorder = Vector3.Distance(position, playerBorderTransform.position);
            // Use a more aggressive check with 1.2x multiplier for safety
            tooCloseToBorder = distanceToBorder < (playerBorderRadius + playerBorderAvoidanceDistance * 1.2f);
        }
        
        // Return true if too close to either nest or border
        return tooCloseToNest || tooCloseToBorder;
    }
    ...
    // Add a helper method to get direction away from player border
    private Vector3 GetDirectionAwayFromBorder(Vector3 currentPosition)
    {
        if (playerBorderTransform == null)
            return -transform.forward; // Default to opposite of current direction
        
        return (currentPosition - playerBorderTransform.position).normalized;
    }

    // Modify SetRandomWaypoint to consider both nest and border avoidance
    private void SetRandomWaypoint()
    {
        ...
        while (!validWaypointFound && attempts < maxAttempts)
        {
            ...
            // For enemies, check both nest and border proximity
            else if (!IsAlly())
            {
                bool notTooCloseToNestOrBorder = !IsPositionTooCloseToNest(randomWaypoint);
                if (notTooCloseToNestOrBorder)
                {
                    validWaypointFound = true;
                }
            }
    ...
    // Modify SetWaypointInDirection to consider both nest and border
    private void SetWaypointInDirection(Vector3 direction)
    {
        ...
        // For allies, skip proximity checks
        if (!IsAlly())
        {
            // Make sure it's not too close to nest or border
            if (IsPositionTooCloseToNest(newWaypoint))
            {
                // Try to find a point that's away from the nest/border but still in the desired direction
                Vector3 awayFromNest = GetDirectionAwayFromNest(transform.position);
                Vector3 awayFromBorder = GetDirectionAwayFromBorder(transform.position);
                
                // Blend both avoidance directions together
                Vector3 blendedAvoidance = (awayFromNest + awayFromBorder).normalized;
                
                // Blend with the original direction
                Vector3 blendedDirection = (direction + blendedAvoidance).normalized;
                
                newWaypoint = transform.position + blendedDirection * randomDistance;
                
                // Double-check the new waypoint
                if (IsPositionTooCloseToNest(newWaypoint))
                {
                    // If still too close, move directly away from the border/nest
                    newWaypoint = transform.position + blendedAvoidance * randomDistance;
                }
            }
        }
    ...
    // Modify the GetSafeWaypoint method for more aggressive avoidance
    private Vector3 GetSafeWaypoint()
    {
        // Double the number of attempts to find a safe waypoint
        for (int attempt = 0; attempt < 16; attempt++) // Increased from 8 to 16 attempts
        {
            // Generate random direction
            Vector3 randomDirection = new Vector3(Random.Range(-1f, 1f), 0, Random.Range(-1f, 1f)).normalized;
            
            // If we have a player border, bias direction away from it
            if (avoidPlayerBorder && playerBorderTransform != null && !IsAlly())
            {
                // Get direction away from border
                Vector3 awayFromBorder = GetDirectionAwayFromBorder(transform.position);
                
                // Blend with random direction (with bias toward moving away)
                randomDirection = (randomDirection + awayFromBorder * 2f).normalized; // Bias heavily toward moving away
            }
            ...
            // Use a more aggressive check for player border
            if (avoidPlayerBorder && playerBorderTransform != null && !IsAlly())
            {
                float distanceToBorder = Vector3.Distance(potentialWaypoint, playerBorderTransform.position);
                // Use a larger safety margin to ensure we stay far away
                if (distanceToBorder < (playerBorderRadius + playerBorderAvoidanceDistance * 1.2f))
                {
                    // If too close to border, try a different waypoint
                    continue;
                }
            }
        ...
        // If no good waypoint found, explicitly move away from the border
        if (avoidPlayerBorder && playerBorderTransform != null && !IsAlly())
        {
            Vector3 awayFromBorder = GetDirectionAwayFromBorder(transform.position);
            Vector3 safeWaypoint = transform.position + awayFromBorder * maxWanderDistance;
            
            Debug.LogWarning($"{gameObject.name}: No random waypoint found, moving directly away from border");
            return safeWaypoint;
        }
    ...
    private bool IsWaypointSafe(Vector3 waypoint)
    {
        ...
        // Add explicit check for player border distance
        if (avoidPlayerBorder && playerBorderTransform != null)
        {
            float distanceToBorder = Vector3.Distance(waypoint, playerBorderTransform.position);
            if (distanceToBorder < (playerBorderRadius + playerBorderAvoidanceDistance))
            {
                return false; // Too close to player border
            }
        }
