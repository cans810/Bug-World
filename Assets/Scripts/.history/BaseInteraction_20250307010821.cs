...
public class BaseInteraction : MonoBehaviour
{
    ...
    private void DepositPlayerCrumbs()
    {
        ...
        if (currentCrumbs > 0)
        {
            // For egg creation, crumbs are used but no XP is gained
            if (antIncubator != null && currentCrumbs >= antIncubator.GetRequiredCrumbs())
            {
                bool eggCreated = antIncubator.ProcessCrumbDeposit(currentCrumbs);
                
                if (eggCreated)
                {
                    // Only take exactly required crumbs for egg, but don't get XP for them
                    playerInventory.RemoveCrumb(antIncubator.GetRequiredCrumbs());
                    ...
                    // Any leftover crumbs after egg creation should be deposited for XP
                    int leftoverCrumbs = currentCrumbs - antIncubator.GetRequiredCrumbs();
                    if (leftoverCrumbs > 0)
                    {
                        playerInventory.DepositCrumbsAtBase(leftoverCrumbs);
                    }
                    
                    if (showDebugMessages)
                    {
                        Debug.Log($"Used {antIncubator.GetRequiredCrumbs()} crumbs to create an egg!");
                        if (leftoverCrumbs > 0)
                        {
                            Debug.Log($"Deposited {leftoverCrumbs} extra crumbs for XP.");
                        }
                    }
                }
                else
                {
                    // Egg creation failed, deposit all crumbs for XP
                    playerInventory.DepositAllCrumbsAtBase();
                    
                    if (showDebugMessages)
                    {
                        Debug.Log($"Couldn't create an egg, deposited all {currentCrumbs} crumbs for XP instead.");
                    }
                }
            }
            else
            {
                // Not enough crumbs for an egg, deposit them for XP
                playerInventory.DepositAllCrumbsAtBase();
                
                if (showDebugMessages)
                {
                    Debug.Log($"Not enough crumbs for egg, deposited all {currentCrumbs} for XP instead.");
                }
                
                // Show message about needing more crumbs if incubator exists
                if (antIncubator != null)
                {
