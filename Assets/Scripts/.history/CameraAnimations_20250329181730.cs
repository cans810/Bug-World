...
public class CameraAnimations : MonoBehaviour
{
    ...
    // Coroutine to animate camera to newly hatched insect
    private IEnumerator AnimateCameraToNewInsect(Transform insectTransform)
    {
        ...
        // Calculate position to view the insect from the front
        Vector3 insectPosition = insectTransform.position;
        Vector3 insectForward = insectTransform.forward;
        
        // Position camera in front of the insect (opposite of insect's forward direction)
        // and further away (2.5 units instead of 1.5)
        Vector3 viewPosition = insectPosition - insectForward * 2.5f;
        viewPosition.y = insectPosition.y + 0.8f; // Slightly above to see it better
        
        // Calculate rotation to look directly at the insect
        ...
        // Play a sound effect for the new insect
        SoundEffectManager.Instance?.PlaySound("InsectHatched");
        ...
        // Signal that the animation is complete to ensure movement is re-enabled
        SendMessage("OnCameraAnimationComplete", insectTransform.gameObject, SendMessageOptions.DontRequireReceiver);
    }
} 