...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            {
                // Player meets the level requirement - allow entry
                isAtMapBorder = false;
                borderNormal = Vector3.zero;
                
                // No longer setting player's border object name here
                // It will be set in OnTriggerStay instead
            }
        }
    ...
    // Update CheckForMetamorphosis to show Size Up text
    private void CheckForMetamorphosis(int level)
    {
        // Skip checks if metamorphosis is disabled or not enough levels are defined
        if (!enableMetamorphosis || metamorphosisLevels == null || metamorphosisLevels.Length == 0 || level < metamorphosisLevels[0])
            return;
        
        // Determine which metamorphosis stage the player should be at based on level
        int targetStage = 0;
        for (int i = 0; i < metamorphosisLevels.Length; i++)
        {
            if (level >= metamorphosisLevels[i])
        ...
        // If we're already at this stage, do nothing
        if (targetStage <= currentMetamorphosisStage)
            return;
        
        // Player has reached a new metamorphosis stage!
        Debug.Log($"<color=green>METAMORPHOSIS: Player reached stage {targetStage} at level {level}</color>");
        
        // Calculate the target scale based on how many stages we've gone through
        Vector3 targetScale = baseScale * Mathf.Pow(scaleIncreaseMultiplier, targetStage);
        
        // Update the current metamorphosis stage
        currentMetamorphosisStage = targetStage;
        
        // Trigger the metamorphosis effect
        StartCoroutine(MetamorphosisEffect(targetScale));
        
        // Show the Size Up! text notification
        if (uiHelper != null)
        {
            uiHelper.ShowSizeUpText();
        }
        else
        {
            Debug.LogWarning("UIHelper reference is missing in PlayerController");
        }
    }

    // Add the missing MetamorphosisEffect method
    private IEnumerator MetamorphosisEffect(Vector3 targetScale)
    {
        // Get the camera animations component
        CameraAnimations cameraAnimations = FindObjectOfType<CameraAnimations>();
        
        // Flag to track when camera is in position
        bool cameraInPosition = false;
        
        // Start camera animation if available
        if (cameraAnimations != null)
        {
            cameraAnimations.TriggerMetamorphosisAnimation(transform, () => {
                cameraInPosition = true;
            });
            
            // Wait for camera to get in position
            yield return new WaitUntil(() => cameraInPosition);
        }
        ...
        if (SoundEffectManager.Instance != null)
        {
            SoundEffectManager.Instance.PlaySound("PowerUp");
        }
        
        // Create a particle effect if we have a visual effect manager
        VisualEffectManager effectManager = FindObjectOfType<VisualEffectManager>();
        if (effectManager != null)
        {
            effectManager.PlayGrowthEffect(transform.position);
        }
        
        // Growth animation duration
        float duration = 1.5f;
        float elapsed = 0f;
        ...
        while (elapsed < duration)
        {
            ...
            // Use a smooth step function for more natural growth
        ...
        // Log the completed metamorphosis
        Debug.Log($"Metamorphosis complete - New scale: {targetScale}");
        
        // If the camera animation component doesn't handle returning to normal view,
        // we don't need to do anything here as it will return automatically
    }
}