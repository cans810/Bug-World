...
public class LevelUpPanelHelper : MonoBehaviour
{
    ...
    // Add these fields at the top of the class
    private bool isFirstGameSession = true;
    private float gameStartTime;

    // Add this field to detect the revive panel
    private bool wasRevivePanelOpenLastFrame = false;

    // Replace the PlayerRevivalManager references in the OnEnable method
    private void OnEnable()
    {
        ...
        // No need to subscribe to revival events, we'll use our cooldown approach
        
        // Record the time the game started
        gameStartTime = Time.time;
        
        // Make sure lastLevelSeen is properly initialized to avoid false level up
        if (playerInventory != null)
        {
            lastLevelSeen = playerInventory.CurrentLevel;
            Debug.Log($"Initialized lastLevelSeen to {lastLevelSeen} in Awake");
        }
    }

    // Remove the PlayerRevivalManager references in the OnDisable method
    private void OnDisable()
    {
        ...
        // No PlayerRevivalManager to unsubscribe from
    }

    // Modify the IsPlayerReviving method to track revival state changes
    private bool isCurrentlyReviving = false; // Add this field to track state changes
    private bool IsPlayerReviving()
    {
        ...
        bool wasReviving = isCurrentlyReviving;
        isCurrentlyReviving = false;
        
        if (playerInventory != null)
        {
            isCurrentlyReviving = playerInventory.IsReviving;
            
            // Detect when revival transitions from true to false
            if (wasReviving && !isCurrentlyReviving)
            {
                // Player just finished reviving
                NotifyPlayerRevived();
            }
            
            return isCurrentlyReviving;
        }
        ...
        if (inventory != null)
        {
            isCurrentlyReviving = inventory.IsReviving;
    ...
    void Start()
    {
        // Set an initial cooldown to prevent level up panel at game start
        lastRevivalTime = Time.time; // This will use the revival cooldown to prevent early popups
        Debug.Log("Initial startup cooldown activated to prevent level up panel at game start");
    }

    // Update is called once per frame
    void Update()
    {
        // Check for revive panel closing
        CheckForRevivePanelClosing();
    }
    ...
    {
        // Skip if we're in the first few seconds of the game
        if (Time.time - gameStartTime < 3f)
        {
            Debug.Log($"Ignoring level up event during first 3 seconds of game startup. Level: {newLevel}");
            lastLevelSeen = newLevel; // Update the last seen level anyway
            return;
        }
        ...
        // Skip first game session's level up events
        if (isFirstGameSession)
        {
            Debug.Log($"Skipping first game session level up event for level {newLevel}");
    ...
    // Add this method to detect when the revive panel is closed
    private void CheckForRevivePanelClosing()
    {
        // Find the revive panel
        GameObject revivePanel = GameObject.Find("RevivePanel");
        
        // Check if revive panel exists and is active
        bool isRevivePanelOpen = revivePanel != null && revivePanel.activeInHierarchy;
        
        // If revive panel was open last frame and now it's closed, that's a manual revival
        if (wasRevivePanelOpenLastFrame && !isRevivePanelOpen)
        {
            Debug.Log("Detected manual revival: Revive panel was just closed");
            NotifyPlayerRevived();
            
            // Set a longer cooldown for manual revivals
            revivalCooldownTime = 10f; // Longer cooldown for manual revival to be safe
        }
        
        // Store current state for next frame
        wasRevivePanelOpenLastFrame = isRevivePanelOpen;
    }

    // Modify NotifyPlayerRevived to add more debug information and a longer cooldown
    public void NotifyPlayerRevived()
    {
        // Record the time of revival completion
        lastRevivalTime = Time.time;
        
        // Ensure first game session flag is off (treat post-revival as mid-game)
        isFirstGameSession = false;
        
        // Reset panel active flag to be safe
        isPanelActive = false;
        
        // Add more detailed logging
        Debug.Log($"Player revival detected at time {lastRevivalTime}. " +
                  $"Level up panel suppressed for {revivalCooldownTime} seconds. " +
                  $"Current player level: {(playerInventory != null ? playerInventory.CurrentLevel.ToString() : "unknown")}");
