...
public class PlayerInventory : MonoBehaviour
{
    ...
    [SerializeField] private int maxCrumbCapacity = 50;
    ...
    // In the DepositChitin method, update the XP gain calculation and visual effects
    ...
    public void DepositChitin(int amount)
    {
        ...
        // Calculate XP gain - now 2 XP per chitin
        int xpPerChitin = 2; // Changed from experiencePerChitin to explicit value
        int xpGain = amount * xpPerChitin;
        
        // Trigger XP gain visual effect - show correct number of symbols
        VisualEffectManager visualEffectManager = FindObjectOfType<VisualEffectManager>();
        if (visualEffectManager != null)
        {
            // Create two XP symbols per chitin
            for (int i = 0; i < amount; i++)
            {
                // Spawn two XP symbols for each chitin with slight delay between them
                visualEffectManager.PlayXPGainEffect(transform.position, xpPerChitin);
                
                // Optional: Add a small delay between pairs of symbols
                // This would require modifying the method to accept a delay parameter
                // or creating a new method in VisualEffectManager
            }
        }
        ...
        Debug.Log($"XP SYSTEM: Deposited {amount} chitin for {xpGain} XP ({xpPerChitin} XP per chitin)");
    }
