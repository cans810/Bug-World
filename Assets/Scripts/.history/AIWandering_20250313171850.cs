...
public class AIWandering : MonoBehaviour
{
    ...
    {
        // For allies, use a simpler waypoint selection that ignores nest boundaries
        if (IsAlly())
        {
            ...
            Vector3 allyWaypoint = transform.position + offset;
            
            // Only check map boundaries for allies
            if (mapBoundary != null && !mapBoundary.IsWithinBounds(allyWaypoint))
            {
                allyWaypoint = mapBoundary.GetNearestPointInBounds(allyWaypoint);
            }
            
            currentWaypoint = allyWaypoint;
            return;
        }
    ...
    private void MoveTowardsWaypoint()
    {
        ...
        {
            float rotationStep = livingEntity.rotationSpeed * Time.deltaTime;
