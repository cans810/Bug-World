...
public class EntityGenerator : MonoBehaviour
{
    ...
    [SerializeField] private bool restrictEntitiesToSpawnArea = true;
    [SerializeField] private float territoryRadius; // Will default to spawnRadius if 0
    [SerializeField] private bool visualizeTerritoryBoundary = true;
    [SerializeField] private Color territoryBoundaryColor = new Color(0.8f, 0.2f, 0.2f, 0.3f);
    ...
    private void Start()
    {
        ...
        // If territory radius isn't set, use spawn radius
        if (territoryRadius <= 0)
            territoryRadius = spawnRadius;
    ...
    private void TrySpawnEntity()
    {
        ...
        if (TryGetSpawnPosition(out spawnPosition))
        {
            ...
            // Assign this generator as the home territory to the entity
            if (restrictEntitiesToSpawnArea)
            {
                // Add territory info to AI components
                AIWandering wandering = newEntity.GetComponent<AIWandering>();
                if (wandering != null)
                {
                    wandering.SetHomeTerritory(this, useCustomSpawnArea ? spawnAreaCenter : transform.position, territoryRadius);
                }
                
                EnemyAI enemyAI = newEntity.GetComponent<EnemyAI>();
                if (enemyAI != null)
                {
                    enemyAI.SetHomeTerritory(this, useCustomSpawnArea ? spawnAreaCenter : transform.position, territoryRadius);
                }
            }
    ...
    // Public methods for other components to access territory info
    public Vector3 GetTerritoryCenter()
    {
        return useCustomSpawnArea ? spawnAreaCenter : transform.position;
    }
    ...
    public bool IsPositionInTerritory(Vector3 position)
    {
        Vector3 center = GetTerritoryCenter();
        // Ignore Y axis for territory check
        position.y = center.y;
        float distanceToCenter = Vector3.Distance(position, center);
        
        // For ring spawning, check if position is within the ring
        if (useRingSpawning)
        {
            return distanceToCenter >= innerRadius && distanceToCenter <= territoryRadius;
        }
        else
        {
            // For regular spawning, just check if within the radius
            return distanceToCenter <= territoryRadius;
        }
    }
    
    // Add this to inform other components about ring structure
    public bool IsRingTerritory()
    {
        return useRingSpawning;
    }
    
    // Add this to provide the inner radius for ring territories
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        // Visualize territory boundary if different from spawn area
        if (visualizeTerritoryBoundary && territoryRadius > 0 && territoryRadius != spawnRadius)
        {
            Gizmos.color = territoryBoundaryColor;
            DrawWireCircle(center, territoryRadius, 64);
        }
    }
