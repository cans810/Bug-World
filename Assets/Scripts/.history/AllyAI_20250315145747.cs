...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        // Try to get the player controller to access input directly
        ...
        // Only check player movement periodically to avoid jitter
        if (Time.time > lastPlayerVelocityCheckTime + playerVelocityCheckInterval)
        {
            // Try to get player's movement input directly
            if (playerController != null)
            {
                // Get the movement direction directly from the player's input
                playerMovementDirection = playerController.GetMovementDirection();
                
                // If we have a meaningful direction, player is moving
                if (playerMovementDirection.magnitude > playerMovementThreshold)
                {
                    playerIsMoving = true;
                }
            }
            else
            {
                ...
                if (lastPlayerPosition != Vector3.zero)
                {
                    ...
                    float playerMovementAmount = playerMovement.magnitude;
                    
                    if (playerMovementAmount > playerMovementThreshold)
                    {
                        playerIsMoving = true;
                        // Use the player's actual forward direction
            ...
            isPlayerMoving = playerIsMoving;
        }
        else
        {
            // Use cached state between checks
            playerIsMoving = isPlayerMoving;
        }
        
        // If player is not moving, stop immediately
        if (!playerIsMoving)
        {
            // Stop moving
            isMoving = false;
            ...
            return; // Exit early - don't do any more movement logic
        }
        ...
        // If player is moving and we're too far, mimic player's movement
        else
        {
            isMoving = true;
            ...
            // Use the player's movement direction as our movement direction
            Vector3 moveDirection = playerMovementDirection;
            moveDirection.y = 0; // Keep on same Y level
            
            // Rotate to face the same direction as the player
            livingEntity.RotateTowards(moveDirection, rotationMultiplier);
            
            // Move forward using the player's movement direction
