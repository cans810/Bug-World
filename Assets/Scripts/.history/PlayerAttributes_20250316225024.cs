...
public class PlayerAttributes : MonoBehaviour
{
    ...
    // Add a new method to initialize the save path
    private void InitializeSavePath()
    {
        saveFilePath = Path.Combine(Application.persistentDataPath, "playerAttributes.json");
        Debug.Log($"Player attributes will be saved to: {saveFilePath}");
    }
    
    // Create a serializable class to store player attribute data
    [Serializable]
    private class PlayerAttributeData
    {
    ...
    private void Start()
    {
        ...
        // Load saved attributes before applying initial effects
    ...
    private void HandleLevelUp(int newLevel)
    {
        ...
        if (playerInventory != null)
        {
            ...
            playerInventory.IncreaseCrumbCapacity(15);
            Debug.Log($"Level up! Increased chitin and crumb capacity by 15 each.");
        }
        ...
        if (uiHelper != null && uiHelper.onLevelUpThingsGainedText != null)
        {
            ...
            uiHelper.onLevelUpThingsGainedText.text = "You gained:\n+15 Chitin Capacity\n+15 Crumb Capacity\n+1 Attribute Point";
    ...
    {
        // Create a data object to store our values
        PlayerAttributeData data = new PlayerAttributeData
        {
            maxHealth = baseMaxHealth * vitality.GetMultiplier(),
            attackDamage = baseAttackDamage * strength.GetMultiplier(),
            attackCooldown = baseAttackCooldown * AttackCooldownMultiplier,
            strengthPoints = strength.points,
            vitalityPoints = vitality.points,
            agilityPoints = agility.points,
            incubationPoints = incubationPoints,
        ...
        try
        {
            ...
            Debug.Log("Player attributes saved successfully");
        }
        catch (Exception e)
        {
            Debug.LogError($"Failed to save player attributes: {e.Message}");
        }
    }
    ...
    public void LoadPlayerAttributes()
    {
        ...
        try
        {
            ...
            // Convert from JSON
            PlayerAttributeData data = JsonUtility.FromJson<PlayerAttributeData>(jsonData);
            
            // Apply the loaded values to our attributes
            strength.points = data.strengthPoints;
            vitality.points = data.vitalityPoints;
            agility.points = data.agilityPoints;
            incubationPoints = data.incubationPoints;
            availablePoints = data.availablePoints;
            
            // Apply the loaded attributes to the player
            if (livingEntity != null)
            {
                livingEntity.SetMaxHealth(data.maxHealth);
                livingEntity.SetAttackDamage(data.attackDamage);
                livingEntity.SetAttackCooldown(data.attackCooldown);
                
                Debug.Log($"Loaded player attributes: Health={data.maxHealth}, " +
                          $"Damage={data.attackDamage}, Cooldown={data.attackCooldown}");
            }
            else
            {
            ...
            Debug.Log("Player attributes loaded successfully");
        }
        catch (Exception e)
        {
    ...
    // Add a method to save attributes when the game is quitting
