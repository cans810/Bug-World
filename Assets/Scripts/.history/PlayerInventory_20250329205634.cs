...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        // Add XP to total
        int previousXP = _experience; // Use field directly
        _experience += amount; // Use the field directly
        ...
        // Debug log
        Debug.Log($"Added {amount} XP. Total XP: {_experience}, Level: {CurrentLevel}");
        
        // Check if level changed and show UI if needed
        if (newLevel > previousLevel)
        {
            ...
            Debug.Log($"Player leveled up from {previousLevel} to {CurrentLevel}!");
            
            // Let's comment out the levelUpPanel code since it's causing errors
            // LevelUpPanelHelper is likely used through event subscription instead
            /*
            LevelUpPanelHelper levelUpPanel = FindObjectOfType<LevelUpPanelHelper>();
            if (levelUpPanel != null)
            {
                levelUpPanel.OnLevelUp(CurrentLevel);
            }
            */
