...
public class PlayerInventory : MonoBehaviour
{
    ...
    {
        // Forward to the new method with animations
        DepositChitin(amount);
    }
    ...
    public void DepositAllChitinAtBase()
    {
        ...
        {
            // Use the new method with animations instead of the old one
            DepositChitin(chitinToDeposit);
            
            // The DepositChitin method already handles:
            // - XP gain calculation
            // - Visual effects
            // - Chitin count reduction
            // - Debug logs
        }
    }
    ...
    public void DepositChitin(int amount)
    {
        ...
        // Add the experience from deposited chitin
        AddExperience(xpGain);
        
        // Trigger XP gain visual effect - USE THE NEW METHOD NAME
        VisualEffectManager visualEffectManager = FindObjectOfType<VisualEffectManager>();
        if (visualEffectManager != null)
        {
            // Call the new method instead of PlayXPGainEffectFromNest
            visualEffectManager.SpawnXPGainEffect(xpGain);
            Debug.Log("Calling SpawnXPGainEffect");
        }
        else
        {
            Debug.LogError("VisualEffectManager not found for XP effect!");
        }
