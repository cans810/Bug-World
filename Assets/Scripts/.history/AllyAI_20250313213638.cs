...
public class AllyAI : MonoBehaviour
{
    ...
    private void ProcessLootDelivery(Transform loot)
    {
        ...
        if (gameManager != null)
        {
            // Add resources based on loot type
            LootData lootData = loot.GetComponent<LootData>();
            if (lootData != null)
            {
                if (gameManager.resourceManager != null)
                {
                    gameManager.resourceManager.AddResource(lootData.resourceType, lootData.resourceAmount);
                    
                    // Show visual feedback
                    if (gameManager.feedbackManager != null)
                    {
                        gameManager.feedbackManager.ShowResourceGainPopup(lootData.resourceType, lootData.resourceAmount, baseTransform.position);
                    }
                }
            }
        }
