...
public class EnemyAI : MonoBehaviour
{
    ...
    // Helper method to update animations - modified to ensure consistent state
    private void UpdateAnimation(bool isWalking)
    {
        if (animController != null)
        {
            // Ensure animation state is forcefully updated to prevent flickering
            animController.SetWalking(isWalking);
            
            // Double-check that the animation state matches our intent
            if (isWalking && animController.IsAnimationPlaying("idle"))
            {
                // Force walking animation if needed
