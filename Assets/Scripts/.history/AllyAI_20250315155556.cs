...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private bool enableLootCollection = true;
    ...
    private void Update()
    {
        ...
        if (carriedLoot == null && !isInCombat && !isChasing && Time.time > nextLootCollectionTime 
            && currentMode != AIMode.GoingToLoot && lootModeEnabled && enableLootCollection)
        {
            CheckForLoot();
        }
        
        // Only process combat if combat is enabled
        if (enableCombat) {
        ...
        switch (currentMode)
        {
            ...
            case AIMode.Wander:
                // Only wander if wandering is enabled
                if (enableWandering) {
                    WanderBehavior();
                } else {
                    // If wandering is disabled but we're in wander mode, switch to follow
                    SetMode(AIMode.Follow);
                }
                break;
                
            case AIMode.GoingToLoot:
                // Only collect loot if loot collection is enabled
            ...
            case AIMode.Attacking:
                // If we're in attacking mode but have no target or combat is disabled, return to previous mode
                if (currentEnemyTarget == null || currentEnemyTarget.IsDead || !enableCombat)
                {
                    EndChaseAndReturnToMode();
                }
                break;
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (playerCtrl != null)
        {
            ...
            // Debug.Log($"Player moving: {playerIsMoving}, direction: {playerMovementDirection}");
        }
    ...
    // Add these public methods to toggle behavior at runtime
    public void SetEnableWandering(bool enable)
    {
        enableWandering = enable;
        if (!enableWandering && currentMode == AIMode.Wander)
        {
            // If wandering is disabled while in wander mode, switch to follow
            SetMode(AIMode.Follow);
        }
        Debug.Log($"{gameObject.name} wandering set to: {(enableWandering ? "ENABLED" : "DISABLED")}");
    }

    public void SetEnableCombat(bool enable)
    {
        enableCombat = enable;
        if (!enableCombat && (currentMode == AIMode.Attacking || isInCombat))
        {
            // If combat is disabled while attacking, return to original mode
            EndChaseAndReturnToMode();
        }
        Debug.Log($"{gameObject.name} combat set to: {(enableCombat ? "ENABLED" : "DISABLED")}");
    }

    public void SetEnableLootCollection(bool enable)
    {
        enableLootCollection = enable;
        lootModeEnabled = enable; // Update both flags for consistency
        
        if (!enableLootCollection && 
            (currentMode == AIMode.GoingToLoot || currentMode == AIMode.Carrying))
        {
            // If loot collection is disabled while collecting, abort
            if (carriedLoot != null)
            {
                // If carrying loot, drop it where we are
                DropLoot(transform.position);
            }
            ResetLootGoalState();
        }
        Debug.Log($"{gameObject.name} loot collection set to: {(enableLootCollection ? "ENABLED" : "DISABLED")}");
    }
}
