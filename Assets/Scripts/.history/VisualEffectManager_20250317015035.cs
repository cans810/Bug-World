...
public class VisualEffectManager : MonoBehaviour
{
    ...
    // Add this field to track if we should animate to the equipped entity counter
    private bool animateToEquippedEntity = false;
    ...
    public IEnumerator SpawnCrumbCollectEffect(int count, bool animateToEquippedCounter = false)
    {
        if (nestTransform == null) yield break;

        // Store whether we should animate to the counter
        animateToEquippedEntity = animateToEquippedCounter;
    ...
    private IEnumerator AnimateResourceCollection(GameObject symbol, string type)
    {
        ...
        // Ensure it reaches exactly the nest position
        ...
        // For crumbs, check if they should animate to the equipped entity counter
        if (type == "crumb" && animateToEquippedEntity)
        {
            yield return new WaitForSeconds(0.2f); // Short pause at nest
            
            // Find the counter
            DiscoveredEntitiesController entityController = FindObjectOfType<DiscoveredEntitiesController>();
            if (entityController != null)
            {
                // Get the counter transform
                Transform counterTransform = entityController.GetCrumbCounterTransform();
                
                if (counterTransform != null)
                {
                    Debug.Log("Starting second animation phase: nest to counter");
                    
                    // Instead of trying to convert positions, let's create NEW symbols at the nest that will fly to the counter
                    // This ensures a clean, visible animation
                    
                    // Create a new crumb symbol at the nest position
                    GameObject newSymbol = null;
                    if (symbolPools["crumb"].Count > 0)
                    {
                        newSymbol = symbolPools["crumb"].Dequeue();
                        newSymbol.transform.position = nestTransform.position;
                        newSymbol.transform.rotation = cameraTransform.rotation;
                        newSymbol.transform.localScale = baseScale;
                        newSymbol.SetActive(true);
                        
                        SpriteRenderer newSpriteRenderer = newSymbol.GetComponent<SpriteRenderer>();
                        if (newSpriteRenderer != null)
                        {
                            newSpriteRenderer.color = new Color(1f, 1f, 1f, 1f); // Full opacity
                        }
                        
                        // The original symbol can now be recycled
                        spriteRenderer.color = startColor;
                        symbol.transform.localScale = targetScale;
                        symbol.SetActive(false);
                        symbolPools[type].Enqueue(symbol);
                        
                        // Now animate the new symbol directly to the counter's SCREEN position
                        Vector2 counterScreenPos = RectTransformUtility.WorldToScreenPoint(Camera.main, counterTransform.position);
                        
                        Debug.Log($"Counter screen position: {counterScreenPos}");
                        
                        // Start position is the nest in screen space
                        Vector2 nestScreenPos = Camera.main.WorldToScreenPoint(nestTransform.position);
                        
                        float animDuration = 1.0f; // Make this animation more noticeable
                        elapsed = 0f;
                        
                        // Create a curve that goes a bit higher for more visibility
                        Vector2 midScreenPoint = Vector2.Lerp(nestScreenPos, counterScreenPos, 0.5f);
                        // Add a significant Y offset to make the arc higher and more visible
                        midScreenPoint.y += 200f; 
                        
                        while (elapsed < animDuration)
                        {
                            elapsed += Time.deltaTime;
                            float t = elapsed / animDuration;
                            float curveValue = chitinFlyCurve.Evaluate(t);
                            
                            // Calculate the current position on the screen using a quadratic curve
                            Vector2 a = Vector2.Lerp(nestScreenPos, midScreenPoint, curveValue);
                            Vector2 b = Vector2.Lerp(midScreenPoint, counterScreenPos, curveValue);
                            Vector2 currentScreenPos = Vector2.Lerp(a, b, curveValue);
                            
                            // Convert screen position back to world space for the symbol
                            Ray ray = Camera.main.ScreenPointToRay(currentScreenPos);
                            float finalTargetDistance = 10f;  // Changed to a completely different name
                            Vector3 worldPos = ray.GetPoint(finalTargetDistance);
                            
                            // Update symbol position
                            newSymbol.transform.position = worldPos;
                            
                            // Scale the symbol (larger in the middle of the arc, smaller at ends)
                            float scaleMultiplier = 1f;
                            if (t > 0.2f && t < 0.8f)
                            {
                                // Create a bell curve for the scale
                                scaleMultiplier = 1f + 0.5f * Mathf.Sin((t - 0.2f) / 0.6f * Mathf.PI);
                            }
                            newSymbol.transform.localScale = baseScale * scaleMultiplier;
                        ...
                        // Ensure it reaches exactly the target screen position
                        Ray finalRay = Camera.main.ScreenPointToRay(counterScreenPos);
                        float finalTargetDistance = 10f;  // Changed to a completely different name
                        Vector3 finalWorldPos = finalRay.GetPoint(finalTargetDistance);
                        newSymbol.transform.position = finalWorldPos;
                        
                        // Play a sound when reaching the counter
                        SoundEffectManager.Instance.PlaySound("CrumbToCounter", counterTransform.position, true);
                        
                        // Make the counter pulse
                        entityController.PulsateCrumbCounter();
                        
                        // Fade out the new symbol
                        float secondFadeDuration = 0.2f;
                        elapsed = 0f;
                        Color originalColor = newSpriteRenderer.color;
                        
                        while (elapsed < secondFadeDuration)
                        {
                            elapsed += Time.deltaTime;
                            float alpha = Mathf.Lerp(1f, 0f, elapsed / secondFadeDuration);
                            newSpriteRenderer.color = new Color(originalColor.r, originalColor.g, originalColor.b, alpha);
                            yield return null;
                        }
                        
                        // Reset and return to pool
                        newSpriteRenderer.color = originalColor;
                        newSymbol.transform.localScale = baseScale;
                        newSymbol.SetActive(false);
                        symbolPools["crumb"].Enqueue(newSymbol);
                        
                        yield break; // Skip the rest of the original animation
                    }
                }
            }
        }
    ...
    private IEnumerator SpawnXPSymbols(Vector3 nestPosition, int count, int totalXPAmount)
    {
        ...
        // Each symbol now represents 1 XP (since we're spawning 2 symbols per chitin)
