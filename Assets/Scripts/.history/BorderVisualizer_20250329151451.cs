...
public class BorderVisualizer : MonoBehaviour
{
    ...
    // Add these methods to BorderVisualizer.cs
    public void ShowEntityIcons()
    {
        // Only show icons if this is a locked area (hasn't been disabled)
        if (!hasBeenDisabled && entityContainedIcons != null)
        {
            entityContainedIcons.SetActive(true);
            Debug.Log($"Activated entity icons for {gameObject.name} while viewing map");
        }
    }

    public void HideEntityIcons()
    {
        // Always hide icons when map is closed
        if (entityContainedIcons != null)
        {
            entityContainedIcons.SetActive(false);
        }
