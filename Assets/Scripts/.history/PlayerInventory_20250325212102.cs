...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Add to PlayerInventory class
    private bool supressLevelUpEffects = false;

    public void SupressLevelUpEffects(bool supress)
    {
        supressLevelUpEffects = supress;
    }

    // Then in the method that triggers level up effects (likely called LevelUp or similar):
    public void LevelUp()
    {
        // Increase level
        CurrentLevel++;
        
        // Only play effects if not suppressed
        if (!supressLevelUpEffects && !isLoadingData)
        {
            // Play level up visual/sound effects
            // ...
        }
    ...
    private IEnumerator AddExperienceGradually(int amount)
    {
        ...
        for (int level = startLevel + 1; level <= targetLevel; level++)
        {
            ...
            maxChitinCapacity += 15;  // Increase chitin capacity
            maxCrumbCapacity += 2;    // Increase crumb capacity
    ...
    // Update SetExperience to accept a playEffects parameter
    public void SetExperience(int value, bool playEffects = true)
    {
        int oldExperience = _experience;
        _experience = value;
        ...
        Debug.Log($"Set experience directly to {value}, playEffects={playEffects}");
    }
    ...
    public void SetLevel(int level, bool playEffects = true)
    {
        // Use the proper field name with underscore
        _currentLevel = Mathf.Clamp(level, 1, maxLevel);
        
        // Only play effects if explicitly requested and not during loading
        if (playEffects && !isLoadingData)
        {
            // Play level up effects here
        ...
        // Make sure to trigger UI update event from the original implementation
        Debug.Log($"Set player level directly to {_currentLevel}");
        OnLevelDisplayUpdate?.Invoke(_currentLevel);
    }
    ...
    // Modify the OnLevelChanged method to check for suppressed effects
    private void OnLevelChanged(int newLevel)
    {
        ...
        // Only play visual effects if not suppressed and not loading data
        if (!supressLevelUpEffects && !isLoadingData)
        {
