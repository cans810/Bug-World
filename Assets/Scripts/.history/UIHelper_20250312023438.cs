...
public class UIHelper : MonoBehaviour
{
    ...
    private IEnumerator SpawnAndAnimateChitinIcons(int chitinCount)
    {
        ...
        for (int i = 0; i < allIcons.Count; i++)
        {
            ...
            // Use a simpler, more direct animation approach
            float duration = 2.0f;
            float elapsedTime = 0f;
            Vector3 startPosition = icon.transform.position;
            
            // Log the start and target positions
            Debug.Log($"[CRITICAL] Icon {i+1} animating from {startPosition} to {targetPos}");
            ...
            while (elapsedTime < duration && icon != null)
            {
                ...
                {
                    Debug.Log($"[CRITICAL] Icon {i+1} at position {currentPos}, progress: {normalizedTime:F2}");
                }
            ...
            if (icon != null)
            {
                Debug.Log($"[CRITICAL] Icon {i+1} animation complete, final position: {icon.transform.position}");
                Destroy(icon);
            }
