...
public class AllyAntPanelController : MonoBehaviour
{
    ...
    private void CreateButtonForAllyAnt(GameObject allyAnt, int index)
    {
        ...
        // Check if we can get the ally's current state to show in the button
        AllyAI allyAI = allyAnt.GetComponent<AllyAI>();
        if (allyAI != null)
        {
            if (buttonText != null)
            {
                buttonText.text = $"Ally {index + 1}: {GetAIStateText(allyAI)}";
            }
            if (tmpText != null)
            {
                tmpText.text = $"Ally {index + 1}: {GetAIStateText(allyAI)}";
            }
        }
    }
    
    private string GetAIStateText(AIA allyAI)
    {
