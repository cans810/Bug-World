...
public class AllyAI : MonoBehaviour
{
    ...
    private void FaceTarget(Vector3 target)
    {
        ...
        {
            // Use the centralized rotation method in LivingEntity for consistency
    ...
    private void MoveToPosition(Vector3 targetPosition, bool updateAnimation = true)
    {
        ...
        if (direction.magnitude > 0.1f)
        {
            ...
            float rotationStep = livingEntity.rotationSpeed * rotationMultiplier * Time.deltaTime;
            Quaternion targetRotation = Quaternion.LookRotation(direction);
            transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, rotationStep * 60f);
            
            // Calculate movement with enhanced speed
            float adjustedSpeed = livingEntity.moveSpeed * speedMultiplier;
            ...
            if (rb != null)
            {
                ...
                Vector3 targetVelocity = direction * adjustedSpeed;
    ...
    // Add this consistent movement method to standardize movement application
    private void MoveInDirection(Vector3 direction, float speedMultiplier = 1.0f)
    {
        if (direction.magnitude < 0.1f)
            return;
        
        // Calculate proper movement with multiplier
        Vector3 movement = direction * livingEntity.moveSpeed * speedMultiplier * Time.deltaTime;
        
        // Use Rigidbody for movement if available for physics-based movement
        Rigidbody rb = GetComponent<Rigidbody>();
        if (rb != null && !rb.isKinematic)
        {
            // Use velocity-based movement for smoother motion
            Vector3 targetVelocity = direction * livingEntity.moveSpeed * speedMultiplier;
