...
public class PlayerController : MonoBehaviour
{
    ...
    private IEnumerator ReviveAfterDelay(float delay)
    {
        ...
        // Hide the revival panel when player revives
        if (uiHelper != null && uiHelper.revivalPanel != null)
        {
            uiHelper.ShowRevivalPanel(false);
        }
        ...
        // Make sure we have a reference to the LivingEntity
        if (livingEntity == null)
            livingEntity = GetComponent<LivingEntity>();
        ...
        // IMPORTANT: Stop all attack animations and states
        StopAllAttacks();
        ...
        // CRITICAL: Force animation sync with living entity to reset death state
        if (animController != null)
        {
            // Sync with LivingEntity to reset the isDead flag
            animController.SyncWithLivingEntity();
            
            // Now that isDead is reset, these calls will work
            animController.SetWalking(false);
            animController.SetIdle();
            
            // For stubborn animations, try to force play idle directly
