...
public class UIHelper : MonoBehaviour
{
    ...
    {
        // Get reference to CameraAnimations if we don't have one
        if (cameraAnimations == null)
        {
            cameraAnimations = FindObjectOfType<CameraAnimations>();
        }

        // Skip showing panel during camera animation
        if (cameraAnimations != null && cameraAnimations.IsAnimationInProgress())
        {
            ...
            shouldShowAttributesPanel = true; // Flag to show later
            return;
        }

        // Only continue if not during animation
        attributesPanel.SetActive(true);
