...
public class AllyAI : MonoBehaviour
{
    ...
    private void WanderBehavior()
    {
        ...
        // Check if we're actually moving by comparing positions between frames
        Vector3 currentPosition = transform.position;
        
        // If we've moved more than a small threshold since last frame, we're moving
        if (Vector3.Distance(currentPosition, lastFramePosition) > 0.01f)
        {
            isMoving = true;
            UpdateAnimation(true);
        }
