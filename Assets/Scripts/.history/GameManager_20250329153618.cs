...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Find and enable the specific EntityGenerator for OuterBase1
        StartCoroutine(FindAndEnableOuterBase1Generator());
    }

    private IEnumerator FindAndEnableOuterBase1Generator()
    {
        // Wait a short time to ensure all other initialization is complete
        yield return new WaitForSeconds(0.5f);
        
        // Try to find the specific EntityGenerator by name
        GameObject outerBase1Generator = GameObject.Find("EntityGenerator(OuterBase1) (7)");
        
        if (outerBase1Generator != null)
        {
            // Check if it's inactive
            if (!outerBase1Generator.activeSelf)
            {
                Debug.Log($"Found inactive EntityGenerator(OuterBase1) (7), activating it...");
                outerBase1Generator.SetActive(true);
                
                // Also ensure its EntityGenerator component has spawning enabled
                EntityGenerator generator = outerBase1Generator.GetComponent<EntityGenerator>();
                if (generator != null)
                {
                    generator.EnableSpawning(true);
                    Debug.Log("Enabled spawning on EntityGenerator(OuterBase1) (7)");
                }
                else
                {
                    Debug.LogWarning("Found EntityGenerator(OuterBase1) (7) but it doesn't have an EntityGenerator component");
                }
            }
            else
            {
                Debug.Log("EntityGenerator(OuterBase1) (7) is already active");
            }
        }
        else
        {
            // If not found, try to find any EntityGenerator with "OuterBase1" in the name
            Debug.LogWarning("Could not find EntityGenerator(OuterBase1) (7) by exact name, searching for similar objects...");
            
            EntityGenerator[] allGenerators = FindObjectsOfType<EntityGenerator>();
            foreach (var generator in allGenerators)
            {
                if (generator.name.Contains("OuterBase1"))
                {
                    Debug.Log($"Found alternative OuterBase1 generator: {generator.name}, activating it...");
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (playerInventory != null)
        {
            // Suppress level up effects while loading
            bool wasLoadingData = playerInventory.IsLoadingData;
            playerInventory.SetLoadingDataState(true);
            
            // Use the correct overload for SetExperience (one parameter)
            playerInventory.SetExperience(gameData.currentXP);
            
            // Use the correct overload for SetLevel (one parameter)
            playerInventory.SetLevel(gameData.currentLevel);
            
            // Restore original loading state
            playerInventory.SetLoadingDataState(wasLoadingData);
    ...
    private void UpdateBorderVisibilityAfterLoad()
    {
        ...
        foreach (var border in borders)
        {
            ...
            bool shouldDisableBorder = false;
            ...
            // Additionally, check if this border is in the list of previously unlocked borders
            if (gameData != null && gameData.unlockedBorderAreas != null && 
                gameData.unlockedBorderAreas.Contains(borderName))
            {
                Debug.Log($"Border {borderName} was previously unlocked, disabling visualization and enabling spawning");
                shouldDisableBorder = true;
            }
            
            // Apply the changes if needed
            if (shouldDisableBorder)
            {
                border.DisableBorderVisualization(); // This now also deactivates icons
