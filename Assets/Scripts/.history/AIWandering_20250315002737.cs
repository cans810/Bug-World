...
public class AIWandering : MonoBehaviour
{
    ...
    private void FindPlayerNest()
    {
        ...
        if (nestObject != null)
            ...
            // MOVEMENT PHASE - This is where we need to make the change
            // After selecting a random direction, use the BorderVisualizer for waypoint selection

            // Generate distance
            float randomDistance = Random.Range(minWanderDistance, maxWanderDistance);
            
            // IMPORTANT CHANGE: Use BorderVisualizer to get a safe waypoint
            Vector3 targetWaypoint = GetSafeWaypointUsingVisualizer(randomDirection, randomDistance);
            
            // Set as current waypoint
            currentWaypoint = targetWaypoint;
            
            // Log for debugging
            Debug.Log($"{gameObject.name}: Setting safe waypoint at {targetWaypoint}, distance from current: {Vector3.Distance(transform.position, targetWaypoint)}");
            
            // Ensure visualizer safety check is working by drawing a persistent debug line
            Debug.DrawLine(transform.position, targetWaypoint, Color.yellow, 2.0f);
        }
    }
