...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        // Reset border status if not moving against the border
        if (isAtMapBorder && moveDirection.magnitude > 0.1f)
        {
            // Check if movement direction is away from the border
            float movementWithBorder = Vector3.Dot(moveDirection, borderNormal);
            
            // If moving away from the border (positive dot product)
            if (movementWithBorder > 0.1f)
            {
                isAtMapBorder = false;
