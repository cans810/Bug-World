    ...
    private void CreateCircularArrows(GameObject parent)
    {
        ...
        for (int i = 0; i < arrowCount; i++)
        {
            ...
            var sizeField = typeof(PlayerFollowingArrowIndicator).GetField("arrowSize", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (sizeField != null)
                sizeField.SetValue(arrow, Mathf.Clamp(transform.localScale.x * 0.4f, 0.5f, 2f)); // Size relative to player
                
            // Position arrows much lower
            var heightField = typeof(PlayerFollowingArrowIndicator).GetField("floatHeight", 
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            if (heightField != null)
                heightField.SetValue(arrow, Mathf.Clamp(transform.localScale.y * 0.5f, 0.5f, 3f)); // Lower height
            
            // Ensure arrows are visible from camera by positioning them properly
            // Get direction to camera to adjust position
            Vector3 dirToCamera = (mainCamera.transform.position - position).normalized;
            // Move the arrow position slightly toward camera to ensure visibility
            position += dirToCamera * (transform.localScale.x * 0.2f);
