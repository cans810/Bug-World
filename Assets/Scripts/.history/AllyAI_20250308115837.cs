...
public class AllyAI : MonoBehaviour
{
    ...
    [SerializeField] private float detectionRange = 8f; // Range to detect enemies
    [SerializeField] private float attackInterval = 1.5f; // Minimum time between attacks
    [SerializeField] private string[] enemyTags = new string[] { "Enemy", "Insect" }; // Tags of enemies to attack
    ...
    private void Update()
    {
        ...
        // Check if there are enemies in range to attack
        LivingEntity nearestEnemy = FindNearestEnemy();

        // If enemy found within attack range
    ...
    {
        // First check if our LivingEntity has detected enemies
        if (livingEntity.HasTargetsInRange())
        {
            // Get the closest valid target from the LivingEntity
            return livingEntity.GetClosestValidTarget();
        }

        // If no enemies detected by LivingEntity, we can manually search
        // This code wouldn't be needed if we're relying on LivingEntity's detection system
        LivingEntity[] potentialEnemies = FindObjectsOfType<LivingEntity>();
        LivingEntity nearestEnemy = null;
        float closestDistance = detectionRange;

        foreach (LivingEntity entity in potentialEnemies)
        {
            // Skip if this is us, the player, or another ally
            if (entity == livingEntity || entity.gameObject.CompareTag("Player") || 
                entity.gameObject.CompareTag("Ally"))
                continue;
