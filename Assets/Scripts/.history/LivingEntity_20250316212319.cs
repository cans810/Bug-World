...
public class LivingEntity : MonoBehaviour
{
    ...
    private void Start()
    {
        // Find the PlayerAttributes component if not already assigned
        if (playerAttributes == null && gameObject.CompareTag("Player"))
        {
    ...
    public void OnAttackHitFrame()
    {
        ...
        if (targetsInRange.Count > 0)
        {
            ...
            {
                // Calculate damage with strength multiplier for player
                float damageToApply = attackDamage;
                
                // Apply strength multiplier if this is the player
                if (gameObject.CompareTag("Player") && playerAttributes != null)
                {
                    damageToApply = playerAttributes.GetModifiedDamage(attackDamage);
                    Debug.Log($"Player attack: Base damage: {attackDamage}, After strength modifier: {damageToApply}");
                }
                
                // Apply the modified damage
                target.TakeDamage(damageToApply, gameObject);
