...
public class FlyingEntityAI : EnemyAI
{
    ...
    protected new void Start()
    {
        ...
        newPos.y = originalYPosition + flyingHeight; // Now uses 2f instead of 5f
    ...
    private void RunFlyingAI()
    {
        ...
        if (!isWandering && wanderingBehavior != null && !wanderingBehavior.IsCurrentlyMoving())
        {
            ...
            // Ensure we're at flying height when wandering
            Vector3 newPos = transform.position;
            newPos.y = originalYPosition + flyingHeight; // Now uses 2f instead of 5f
            transform.position = newPos;
        }
    ...
    private void HandleFlyingHeight()
    {
        ...
        else if (isAscending)
        {
            ...
            float targetHeight = originalYPosition + flyingHeight; // Now uses 2f instead of 5f
            if (transform.position.y < targetHeight)
            {
                Vector3 newPos = transform.position;
