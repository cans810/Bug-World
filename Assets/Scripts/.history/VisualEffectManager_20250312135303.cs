...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnChitinCollectEffect(int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            symbol.transform.localScale = chitinScale; // Use larger scale for chitin
            symbol.SetActive(true);
    ...
    private IEnumerator AnimateChitinCollection(GameObject symbol)
    {
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            // Scale pulse effect using chitinScale
            float scale = 1f + Mathf.Sin(normalizedTime * Mathf.PI * 2) * 0.2f;
            symbol.transform.localScale = chitinScale * scale;
        ...
        symbol.transform.localScale = chitinScale; // Reset to chitin scale
        symbol.SetActive(false);
