...
public class ArrowIndicator : MonoBehaviour
{
    ...
    // Screen edge padding
    private float screenEdgePadding = 50f;
    ...
    private void UpdateArrowPosition()
    {
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            else
            {
                ...
                // Point toward the target from the player's position
                Vector3 dirToTarget = target.transform.position - playerTransform.position;
                dirToTarget.y = 0; // Keep level horizontally
                if (dirToTarget != Vector3.zero)
                {
                    // For a sprite pointing right, use this calculation
                    float angle = Mathf.Atan2(dirToTarget.z, dirToTarget.x) * Mathf.Rad2Deg;
                    arrowObject.transform.rotation = Quaternion.Euler(0, 0, angle);
                }
            }
            ...
            // Make sure arrow faces the camera
