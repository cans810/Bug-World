...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (!isPlayerMoving && distanceToPlayer <= followDistance * 1.5f)
        {
            ...
            if (wanderWhenPlayerIdle && wanderingBehavior != null)
            {
                ...
                if (!isIdleWandering)
                {
                    ...
                    // Use a new method we'll add to AIWandering
                    wanderingBehavior.SetWanderDistances(idleWanderRadius * 0.5f, idleWanderRadius);
        ...
        else if (isIdleWandering)
        {
            ...
            if (wanderingBehavior != null)
            {
                // Reset to default values - we'll need to add these as public fields
                wanderingBehavior.SetWanderDistances(3f, 10f); // Default values from AIWandering
                wanderingBehavior.SetWanderingEnabled(false);
            }
        }
    ...
    // Add a method to AIWandering.cs to set the wander distances
    public void SetWanderDistances(float min, float max)
    {
        wanderingBehavior.minWanderDistance = min;
        wanderingBehavior.maxWanderDistance = max;
    }
}
