...
public class CameraAnimations : MonoBehaviour
{
    ...
    private struct AnimationRequest
    {
        ...
        public AnimationRequest(Transform target, int level = 0, string areaName = "", bool isEgg = false)
        {
            this.target = target;
            this.level = level;
    ...
    public void AnimateToEgg(Transform eggTransform)
    {
        ...
        AnimationRequest request = new AnimationRequest(eggTransform, 0, "Scorpion Egg", true);
    ...
    // Add this method to animate specifically to newly created eggs
    public void AnimateToNewEgg(Transform eggTransform)
    {
        if (eggTransform == null)
        {
            Debug.LogWarning("Cannot animate to null egg transform");
            return;
        }

        // Create a new animation request marked as an egg animation
        AnimationRequest request = new AnimationRequest(
            eggTransform, 
            0,  // No level needed for eggs
            "Scorpion Egg", 
            true  // This is an egg animation
        );
        
        // Add to queue
        animationQueue.Enqueue(request);
        Debug.Log($"Added egg animation request to queue");
