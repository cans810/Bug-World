...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            // Use the SoundManager to play the ambient sound
            SoundManager soundManager = FindObjectOfType<SoundManager>();
            if (soundManager != null)
            {
                soundManager.PlaySound(backgroundAmbientSound, true, ambientFadeInTime);
                Debug.Log($"Playing background ambient sound: {backgroundAmbientSound}");
            }
            else
            {
                Debug.LogWarning("SoundManager not found. Cannot play ambient sound.");
            }
        }
