...
public class BorderVisualizer : MonoBehaviour
{
    ...
    // Helper method to create a stadium-shaped mesh (capsule projected onto X-Z plane)
    private Mesh CreateStadiumMesh(float radius, float height, bool isXAxis)
    {
        ...
        int totalVertices = circleSegments * 2 + 4 + 1; // Two semicircles + 4 corners of rectangle + center
        ...
        // First semicircle (facing outward, -90° to 90°)
        // First semicircle
        ...
        for (int i = 0; i < circleSegments; i++)
        {
            ...
            if (isXAxis)
                vertices[vertexIndex++] = semicircle1Center + new Vector3(0, 0, z) + new Vector3(x, 0, 0);
            else
                vertices[vertexIndex++] = semicircle1Center + new Vector3(x, 0, 0) + new Vector3(0, 0, z);
        }
        ...
        for (int i = 0; i < circleSegments; i++)
        {
            float angle = Mathf.PI * (1.5f + i / (float)(circleSegments - 1));
            float x = Mathf.Cos(angle) * radius;
            float z = Mathf.Sin(angle) * radius;
            
            // Adjust based on axis
            if (isXAxis)
                vertices[vertexIndex++] = semicircle2Center + new Vector3(0, 0, z) + new Vector3(x, 0, 0);
            else
                vertices[vertexIndex++] = semicircle2Center + new Vector3(x, 0, 0) + new Vector3(0, 0, z);
        }
        
        // Rectangle corner vertices (might be redundant with the semicircle endpoints)
        if (isXAxis)
        {
            vertices[vertexIndex++] = new Vector3(straightLength/2, 0, radius);
            vertices[vertexIndex++] = new Vector3(straightLength/2, 0, -radius);
            vertices[vertexIndex++] = new Vector3(-straightLength/2, 0, radius);
            vertices[vertexIndex++] = new Vector3(-straightLength/2, 0, -radius);
        }
        ...
        {
            vertices[vertexIndex++] = new Vector3(radius, 0, straightLength/2);
            vertices[vertexIndex++] = new Vector3(-radius, 0, straightLength/2);
            vertices[vertexIndex++] = new Vector3(radius, 0, -straightLength/2);
            vertices[vertexIndex++] = new Vector3(-radius, 0, -straightLength/2);
        }
        ...
        int[] triangles = new int[(totalVertices - 1) * 3];
        int triIndex = 0;
        ...
        for (int i = 1; i < totalVertices; i++)
        {
            ...
            if (i < totalVertices - 1)
                ...
                triangles[triIndex++] = 1; // Connect back to first vertex
        }
