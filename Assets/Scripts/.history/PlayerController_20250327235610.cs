...
public class PlayerController : MonoBehaviour
{
    ...
    // Add this public method to enable/disable movement from other scripts
    public void SetCanMove(bool canMove)
    {
        // Store the current movement state
        this.canMove = canMove;
        
        // If movement is disabled, stop all current movement
        if (!canMove)
        {
            moveDirection = Vector3.zero;
            rb.velocity = Vector3.zero;
            
            // Also disable the joystick input if using mobile controls
            OnScreenJoystick joystick = FindObjectOfType<OnScreenJoystick>();
            if (joystick != null)
            {
                joystick.ResetJoystick();
            }
            
            Debug.Log("Player movement disabled externally");
        }
        else
        {
            Debug.Log("Player movement re-enabled externally");
        }
    }
}