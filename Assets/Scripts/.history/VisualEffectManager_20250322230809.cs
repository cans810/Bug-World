    ...
    private IEnumerator FlyChitinToNest(GameObject chitinSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)
        {
            ...
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                ...
                if (uiHelper != null)
                {
                    // Update the chitin count display with animation
                    PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
                    if (playerInventory != null)
                    {
                        // Fix: Use currentChitinCount instead of ChitinAmount
                        uiHelper.UpdateChitinDisplay(playerInventory.currentChitinCount, playerInventory.MaxChitinCapacity);
                    }
                }
    ...
    private IEnumerator FlyCrumbToInventory(GameObject crumbSymbol, Vector3 startPos, int amount = 1)
    {
        ...
        while (elapsed < chitinFlyDuration)  // Using same duration as chitin for consistency
        {
            ...
            if (!effectsTriggered && normalizedTime > 0.9f)
            {
                ...
                if (uiHelper != null)
                {
                    // Update the crumb count display with animation
                    PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
                    if (playerInventory != null)
                    {
                        // Fix: Use currentCrumbCount instead of CrumbAmount
                        uiHelper.UpdateCrumbDisplay(playerInventory.currentCrumbCount, playerInventory.MaxCrumbCapacity);
                    }
                }
                effectsTriggered = true;
            }
