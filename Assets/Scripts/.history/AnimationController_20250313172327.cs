...
public class AnimationController : MonoBehaviour
{
    ...
    // Update animator with current states - prevent animation glitches
    private void UpdateAnimatorParameters()
    {
        if (animator == null) return;
        
        // Only set parameters that have changed to avoid animation glitches
        if (animator.GetBool(idleParam) != isIdle)
        ...
        float currentWalkSpeed = animator.GetFloat(walkSpeedParam);
        float targetWalkSpeed = isWalking ? 1f : 0f;
        
        // Only update if there's a significant change
        if (Mathf.Abs(currentWalkSpeed - targetWalkSpeed) > 0.01f)
            animator.SetFloat(walkSpeedParam, targetWalkSpeed);
        
        if (animator.GetBool(eatingParam) != isEating)
            animator.SetBool(eatingParam, isEating);
        
        if (animator.GetBool(attackingParam) != isAttacking)
            animator.SetBool(attackingParam, isAttacking);
        
        if (animator.GetBool(deadParam) != isDead)
    ...
    // Modify SetWalking to be more robust
