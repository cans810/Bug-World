...
public class GameManager : MonoBehaviour
{
    ...
    public void SaveGame()
    {
        ...
        {
            // Save inventory data
            data.currentChitin = playerInventory.ChitinCount;
            data.currentCrumb = playerInventory.CrumbCount;
            data.currentXP = playerInventory.TotalExperience;
            data.currentLevel = playerInventory.CurrentLevel;
            
            // Save other player data
            LivingEntity playerEntity = FindObjectOfType<PlayerController>()?.GetComponent<LivingEntity>();
            if (playerEntity != null)
            {
                data.currentHP = (int)playerEntity.CurrentHealth;
                data.currentMaxHP = (int)playerEntity.MaxHealth;
            }
            ...
            {
                data.maxEggCapacity = playerAttributes.MaxEggCapacity;
                Debug.Log($"Saved egg capacity: {data.maxEggCapacity}");
            }
        ...
        {
            Debug.LogError("Could not find PlayerInventory to save game data!");
        }
    }
    ...
    private void ApplyLoadedDataToGameState()
    {
        ...
        if (playerAttributes != null && gameData.maxEggCapacity > 0)
        {
            playerAttributes.LoadEggCapacityFromGameData(gameData);
            Debug.Log($"Applied egg capacity from game data: {gameData.maxEggCapacity}");
        }
    }
    ...
    public void LoadGame()
    {
        ...
        if (data != null)
        {
            ...
            if (playerInventory != null)
            {
                // Set the loading flag to prevent animations
                playerInventory.IsLoadingData = true;
                
                // Load inventory data
                playerInventory.SetChitinCount(data.currentChitin);
                playerInventory.SetCrumbCount(data.currentCrumb);
                playerInventory.SetExperience(data.currentXP);
                playerInventory.SetLevel(data.currentLevel);
                
                // Reset the loading flag
                playerInventory.IsLoadingData = false;
            }
            ...
            if (playerEntity != null)
            {
                playerEntity.SetHealth(data.currentHP);
                playerEntity.SetMaxHealth(data.currentMaxHP);
            }
            ...
            if (playerAttributes != null)
            {
                playerAttributes.SetMaxEggCapacity(data.maxEggCapacity);
            }
