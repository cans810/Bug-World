...
public class ChitinCollectible : MonoBehaviour
{
    ...
    [SerializeField] private float floatHeight = 1.5f; // How high the item floats
    [SerializeField] private float floatSpeed = 3.0f; // Speed of floating animation
    [SerializeField] private float dropSpeed = 5.0f; // Speed of dropping onto player
    [SerializeField] private float collectionDelay = 0.5f; // Delay before actually adding to inventory
    ...
    private IEnumerator CollectionAnimation()
    {
        ...
        while (elapsedTime < 1.0f && playerTransform != null)
        {
            ...
            // Scale down slightly as we approach
            transform.localScale = Vector3.Lerp(Vector3.one, Vector3.one * 0.5f, elapsedTime);
