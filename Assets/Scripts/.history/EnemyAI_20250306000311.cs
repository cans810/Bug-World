...
public class EnemyAI : MonoBehaviour
{
    ...
    private void UpdateAnimation(bool isWalking, bool isAttacking)
    {
        ...
        if (isAttacking && isWalking && hasAttackWalkingAnimation)
        {
            ...
            Debug.Log($"{gameObject.name} - ATTEMPTING TO SET ATTACK WALKING");
            
            // First, explicitly reset all animation states in the Animator to avoid conflicts
            if (animController.Animator != null)
            {
                ...
                // Give the animator a frame to process these changes
                animController.Animator.Update(0.01f);
            }
            
            // Now set the attack walking state
            animController.SetAttackWalking(true);
            
            // Verify what animations are active after setting
            if (animController.Animator != null) {
                Debug.Log($"AttackWalking param value: {animController.Animator.GetBool("AttackWalking")}");
                Debug.Log($"Walking param value: {animController.Animator.GetBool("Walk")}");
                Debug.Log($"Attacking param value: {animController.Animator.GetBool("Attack")}");
                Debug.Log($"Idle param value: {animController.Animator.GetBool("Idle")}");
            }
        }
        else
        {
