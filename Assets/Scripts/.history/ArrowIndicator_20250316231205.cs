...
public class ArrowIndicator : MonoBehaviour
{
    ...
    private void UpdateArrowPosition()
    {
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            // IMPROVED ROTATION AND BILLBOARDING
            // First ensure the arrow faces the camera (billboard effect)
            arrowObject.transform.LookAt(arrowObject.transform.position + mainCamera.transform.rotation * Vector3.forward,
                                        mainCamera.transform.rotation * Vector3.up);
                                    
            // Then apply the 2D rotation for direction
