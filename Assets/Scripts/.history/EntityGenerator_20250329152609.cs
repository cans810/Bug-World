...
public class EntityGenerator : MonoBehaviour
{
    ...
    {
        // Add debugging info
        Debug.Log($"<color=yellow>EntityGenerator.EnableSpawning called for {gameObject.name}: {enable}</color>");
        Debug.Log($"<color=yellow>Call stack: {Environment.StackTrace}</color>");
        
        // Set the spawning state
        this.enableSpawning = enable;
        
        // Log the change
        Debug.Log($"Entity spawning for {gameObject.name} set to {enable}");
    }
} 