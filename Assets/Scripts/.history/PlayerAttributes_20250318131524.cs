...
public class PlayerAttributes : MonoBehaviour
{
    ...
    {
        // Get the GameManager and save through it
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null)
        {
            gameManager.SaveGame();
            Debug.Log("Player attributes saved through GameManager");
        }
        else
        {
            Debug.LogError("Could not find GameManager to save player attributes");
        }
    }
    ...
    {
        // Load through GameManager
        GameManager gameManager = FindObjectOfType<GameManager>();
        if (gameManager != null)
        {
            GameData data = gameManager.LoadGame();
            if (data != null)
            {
                ...
                if (livingEntity != null)
                {
                    livingEntity.SetMaxHealth(baseMaxHealth * vitality.GetMultiplier());
                    livingEntity.SetAttackDamage(baseAttackDamage * strength.GetMultiplier());
                    livingEntity.SetAttackCooldown(baseAttackCooldown * AttackCooldownMultiplier);
                }
                ...
                Debug.Log("Player attributes loaded successfully from main save file");
            }
        }
        else
        {
            Debug.LogError("Could not find GameManager to load player attributes");
        }
    }
