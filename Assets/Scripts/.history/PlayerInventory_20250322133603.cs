...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Update this method to ensure attribute points are added properly
    private void ProcessLevelUp(int level)
    {
        ...
        {
            // Always add 2 points per level (or use configurable value)
            int pointsToAdd = 2; // Default value
            
            // Use configured value if available
            if (playerAttributes.pointsPerLevel > 0)
            {
                pointsToAdd = playerAttributes.pointsPerLevel;
            }
            
            // Call the direct method to add points
            playerAttributes.AddAttributePoints(pointsToAdd);
            
            // Don't try to invoke the event directly
            // Instead use a public method to update the UI
            playerAttributes.UpdateUI();
            
            Debug.Log($"Added {pointsToAdd} attribute points for reaching level {level}. " +
                      $"New total: {playerAttributes.AvailablePoints}");
        }
        else
        {
