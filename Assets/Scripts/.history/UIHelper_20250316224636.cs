...
public class UIHelper : MonoBehaviour
{
    ...
    [SerializeField] public GameObject onLevelUpThingsGainedPanel;
    [SerializeField] public GameObject onLevelUpThingsGainedBg;
    [SerializeField] public List<GameObject> awardsPrefabs; // 0 is chitin, 1 is crumb, 2 is attribute point, 3 is area unlocked
    ...
    private void Start()
    {
        // Initialize the player inventory reference
        playerInventory = FindObjectOfType<PlayerInventory>();
        
        // Cache UI element references
        if (chitinCountText != null) countTextRect = chitinCountText.GetComponent<RectTransform>();
        if (crumbCountText != null) crumbTextRect = crumbCountText.GetComponent<RectTransform>();
        if (experienceCountText != null) xpTextRect = experienceCountText.GetComponent<RectTransform>();
        if (eggCountText != null) eggTextRect = eggCountText.GetComponent<RectTransform>();
        
        // Cache original scales
        if (countTextRect != null) originalScale = countTextRect.localScale;
        if (crumbTextRect != null) originalCrumbScale = crumbTextRect.localScale;
        if (xpTextRect != null) originalXpScale = xpTextRect.localScale;
        if (eggTextRect != null) originalEggScale = eggTextRect.localScale;
        ...
        if (playerInventory != null)
        {
            ...
            playerInventory.OnLevelUp += UpdateLevelDisplay;
        }
        
        // Cache camera animations component
        cameraAnimations = Camera.main?.GetComponent<CameraAnimations>();
        
        // Initialize attributes
        cachedPlayerAttributes = FindObjectOfType<PlayerAttributes>();
        
        // Make sure information texts are hidden initially
        if (informPlayerText != null)
        {
            informPlayerText.gameObject.SetActive(false);
        }
        
        // Make sure the level-up rewards panel and background are hidden initially
        if (onLevelUpThingsGainedPanel != null)
        {
            onLevelUpThingsGainedPanel.SetActive(false);
        }
        
        if (onLevelUpThingsGainedBg != null)
        {
            onLevelUpThingsGainedBg.SetActive(false);
        }
        
        // Initialize the current XP display
        if (experienceCountText != null && playerInventory != null)
        {
            currentDisplayedXP = playerInventory.Experience;
        }
    }
    ...
    // Update this method in UIHelper.cs to use prefabs instead of text
    public void ShowLevelUpRewardsText(bool newAreaUnlocked = false)
    {
        // Add debug log at the start of the method
        Debug.Log($"ShowLevelUpRewardsText called with newAreaUnlocked={newAreaUnlocked}");

        // Skip showing messages if we're loading data
        PlayerInventory playerInventory = FindObjectOfType<PlayerInventory>();
        if (playerInventory != null && playerInventory.IsLoadingData)
        {
            Debug.Log("Skipping rewards panel - game is loading data");
            return;
        }
        ...
        if (onLevelUpThingsGainedBg != null)
        {
            Debug.Log("Setting background panel active");
            onLevelUpThingsGainedBg.SetActive(true);
        }
        else
        {
            Debug.LogError("Level-up background panel is null!");
        }

        if (onLevelUpThingsGainedPanel != null)
        {
            // Clear any existing rewards in the panel
            foreach (Transform child in onLevelUpThingsGainedPanel.transform)
            {
                Destroy(child.gameObject);
            }
            
            // Make the panel visible
            Debug.Log("Setting rewards panel active");
            onLevelUpThingsGainedPanel.SetActive(true);
            
            // Check if we have the necessary prefabs
            if (awardsPrefabs != null && awardsPrefabs.Count >= 3)
            {
                Debug.Log($"Found {awardsPrefabs.Count} award prefabs");
                
                // Always instantiate chitin capacity increase (index 0)
                if (awardsPrefabs[0] != null)
                {
                    GameObject chitinReward = Instantiate(awardsPrefabs[0], onLevelUpThingsGainedPanel.transform);
                    Debug.Log($"Instantiated chitin reward: {chitinReward.name}");
                }
                else
                {
                    Debug.LogError("Chitin reward prefab is null!");
                }
                
                // Always instantiate crumb capacity increase (index 1)
                if (awardsPrefabs[1] != null)
                {
                    GameObject crumbReward = Instantiate(awardsPrefabs[1], onLevelUpThingsGainedPanel.transform);
                    // Set position or adjust as needed
                }
                
                // Always instantiate attribute point (index 2)
                if (awardsPrefabs[2] != null)
                {
                    GameObject attributeReward = Instantiate(awardsPrefabs[2], onLevelUpThingsGainedPanel.transform);
                    // Set position or adjust as needed
                }
                
                // If a new area is unlocked, instantiate that reward too (index 3)
                if (newAreaUnlocked && awardsPrefabs.Count >= 4 && awardsPrefabs[3] != null)
                {
                    GameObject areaReward = Instantiate(awardsPrefabs[3], onLevelUpThingsGainedPanel.transform);
                    // Set position or adjust as needed
                }
            }
            else
            {
                Debug.LogWarning($"Award prefabs not properly assigned in UI Helper! Count: {(awardsPrefabs != null ? awardsPrefabs.Count : 0)}");
            }
            
            // Start a coroutine to hide the panel after a delay
            StartCoroutine(HideLevelUpRewardsTextAfterDelay(6.0f));
        }
        else
        {
            Debug.LogError("Level-up rewards panel is null!");
        }
    }

    // Rename this method to match its new functionality better
    private IEnumerator HideLevelUpRewardsTextAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        HideLevelUpRewardsPanel();
    }

    // Rename and update this method to hide the panel
    public void HideLevelUpRewardsPanel()
    {
        if (onLevelUpThingsGainedPanel != null)
        {
            // Hide the panel
