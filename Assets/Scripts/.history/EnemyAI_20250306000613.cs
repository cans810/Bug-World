...
public class EnemyAI : MonoBehaviour
{
    ...
    // Updated helper method to update animations
    private void UpdateAnimation(bool isWalking, bool isAttacking)
    {
        if (animController == null)
            return;
            
        // Add debug log to see what states we're trying to set
        Debug.Log($"{gameObject.name} - Setting animation: Walking={isWalking}, Attacking={isAttacking}, HasAttackWalkingAnim={hasAttackWalkingAnimation}");
        
        if (isAttacking && isWalking && hasAttackWalkingAnimation)
        {
            // Use attack walking animation if available
            Debug.Log($"{gameObject.name} - ATTEMPTING TO SET ATTACK WALKING - DIRECT APPROACH");
            
            // DIRECT APPROACH: Get access to the Animator
            Animator animator = animController.Animator;
            if (animator != null)
            {
                // Force reset ALL animation parameters to ensure clean state
                ...
                // Force update the animator to apply the reset
                animator.Update(0.0f);
                
                // Now ONLY set the AttackWalking parameter
                animator.SetBool("AttackWalking", true);
                
                // Try to directly play the state if parameter approach isn't working
                // Try several variations of the state name
                string[] possibleStateNames = new string[] {
                    "AttackWalking", "Attack Walking", "Attack_Walking", 
                    "Base Layer.AttackWalking", "Base Layer.Attack Walking"
                };
                
                foreach (string stateName in possibleStateNames)
                {
                    try {
                        // Try to force play this state directly
                        animator.Play(stateName, 0, 0f);
                        Debug.Log($"Attempted to directly play state: {stateName}");
                    } catch (System.Exception e) {
                        // State doesn't exist, try the next one
                        Debug.Log($"State {stateName} doesn't exist: {e.Message}");
                    }
                }
                
                // Print all the animation states available for debugging
                // Fix for missing AnimatorController namespace
                try {
                    var runtimeController = animator.runtimeAnimatorController;
                    if (runtimeController != null) {
                        Debug.Log("Animator has a valid runtimeAnimatorController");
                        
                        // Alternative approach that doesn't need AnimatorController cast
                        // Just list the parameters instead
                        Debug.Log("Listing all animator parameters:");
                        foreach (var parameter in animator.parameters) {
                            Debug.Log($"Parameter: {parameter.name}, Type: {parameter.type}");
                        }
                        
                        // Try to output current state info
                        var stateInfo = animator.GetCurrentAnimatorStateInfo(0);
                        Debug.Log($"Current state: {stateInfo.fullPathHash}, normalized time: {stateInfo.normalizedTime}");
                    }
                } catch (System.Exception e) {
                    Debug.LogError($"Error accessing animator controller: {e.Message}");
                }
            }
        }
        else
        {
            // Turn off attack walking animation
            if (hasAttackWalkingAnimation)
                animController.SetAttackWalking(false);
