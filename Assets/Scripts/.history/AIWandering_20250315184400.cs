...
public class AIWandering : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enforceFixedYPosition && livingEntity != null && !livingEntity.IsDead)
        {
            EnforceYPosition();
        }
    }
    ...
    private IEnumerator WanderingRoutine()
    {
        ...
        while (true)
        {
            // Ensure Y position before each loop iteration
    ...
    // Add this method to ensure consistent Y position
    private void EnforceYPosition()
    {
        Vector3 position = transform.position;
        if (Mathf.Abs(position.y - spawnPosition.y) > 0.01f)
        {
