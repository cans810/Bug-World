...
public class VisualEffectManager : MonoBehaviour
{
    ...
    private void InitializeUIPool(string poolName, GameObject prefab, int count = 10)
    {
        ...
        if (!symbolPools.ContainsKey(poolName))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // Configure the sprite renderer to always be visible
                SpriteRenderer renderer = symbol.GetComponent<SpriteRenderer>();
                if (renderer != null)
                {
                    // Set extremely high sorting order to ensure visibility
                    ...
                    // Remove any UI.Image component if it exists (from the prefab)
                    UnityEngine.UI.Image existingImage = symbol.GetComponent<UnityEngine.UI.Image>();
                ...
                {
                    // If no SpriteRenderer, add one
                    renderer = symbol.AddComponent<SpriteRenderer>();
                    renderer.sortingOrder = 9999;
    ...
    public void PlayChitinToXPEffect(Vector3 worldStartPosition, int chitinAmount, int xpAmount)
    {
        ...
        // Get the panel positions in canvas space
        Vector2 chitinPanelPos;
        Vector2 xpPanelPos;
        Vector2 screenCenterPos;
        
        // Get the center of the screen in canvas space
        RectTransformUtility.ScreenPointToLocalPointInRectangle(
            canvasSafeArea.GetComponent<RectTransform>(),
        ...
        // Start the chitin-to-XP conversion animation
        StartCoroutine(ChitinToXPAnimation(chitinPanelPos, screenCenterPos, xpPanelPos, 
            chitinAmount, xpAmount, player));
    }

    // Coroutine for the chitin-to-XP conversion animation
    private IEnumerator ChitinToXPAnimation(Vector2 chitinPanelPos, Vector2 centerPos, Vector2 xpPanelPos, 
        int chitinAmount, int xpAmount, GameObject player)
    {
        if (chitinAmount <= 0 || xpAmount <= 0) yield break;
        
        // Phase 1: Spawn chitin symbols at chitinPanel and animate to center
        List<GameObject> chitinSymbols = new List<GameObject>();
        
        // Spawn all chitin symbols at once
        for (int i = 0; i < chitinAmount; i++)
        {
            if (symbolPools["chitin"].Count == 0)
            {
                Debug.LogWarning("Chitin Symbol pool depleted! Creating more UI symbols.");
                InitializeUIPool("chitin", chitinSymbolPrefab, 10);
            }
            
            GameObject chitinSymbol = symbolPools["chitin"].Dequeue();
            RectTransform rectTransform = chitinSymbol.GetComponent<RectTransform>();
            
            // Add random offset for visual variety
            float randomXOffset = Random.Range(-40f, 40f);
            float randomYOffset = Random.Range(-20f, 20f);
            
            // Set initial position at chitin panel
            rectTransform.anchoredPosition = new Vector2(
                chitinPanelPos.x + randomXOffset,
                chitinPanelPos.y + randomYOffset
            );
            
            // Set up the UI image
            UnityEngine.UI.Image image = chitinSymbol.GetComponent<UnityEngine.UI.Image>();
            if (image != null)
            {
                image.enabled = true;
        ...
        // Short pause for visual clarity
        yield return new WaitForSeconds(0.3f);
        
        // Animate chitin symbols to center with a slight delay between each
        for (int i = 0; i < chitinSymbols.Count; i++)
        {
            StartCoroutine(AnimateChitinToCenter(chitinSymbols[i], centerPos));
            yield return new WaitForSeconds(0.05f);
        }
        
        // Wait for all chitin symbols to reach the center
        yield return new WaitForSeconds(0.8f);
        
        // Play a conversion sound effect
        SoundEffectManager.Instance?.PlaySound("ChitinConvert");
        
        // Deactivate all chitin symbols
        ...
        // Phase 2: Create XP symbols at center and animate to XP panel
        yield return new WaitForSeconds(0.2f); // Brief pause for effect
        
        // Spawn XP symbols at the center position
        for (int i = 0; i < xpAmount; i++)
        {
            ...
            float randomYOffset = Random.Range(-30f, 30f);
            ...
            // Animate to XP panel
            StartCoroutine(AnimateUIResourceSymbol(xpSymbol, "xp", rectTransform.anchoredPosition, 
                Vector2.zero, xpPanelPos, "XPGained"));
    ...
    // Helper coroutine to animate chitin symbols to center
    private IEnumerator AnimateChitinToCenter(GameObject chitinSymbol, Vector2 centerPos)
    {
        ...
        // Ensure final position is exactly at center
