...
public class CameraAnimations : MonoBehaviour
{
    ...
    private IEnumerator AnimateToMapViewCoroutine()
    {
        ...
        // Log the values we're trying to use
        Debug.Log($"Map View Settings - Height: {mapViewHeight}, Angle: {mapCameraAngle}");
        ...
        {
            Debug.LogError("playerCamera reference is null! Map view may not work correctly.");
        }
        ...
        // Add absolute ground offset to ensure correct height regardless of terrain
        float groundLevel = 0f;
        if (player != null)
        {
            // Try to raycast to get ground level
            if (Physics.Raycast(player.transform.position, Vector3.down, out RaycastHit hit, 100f))
            {
                groundLevel = hit.point.y;
                Debug.Log($"Ground level detected at y={groundLevel}");
            }
        }
        
        // Calculate target position above ground level
        Vector3 targetPosition = new Vector3(
            playerPosition.x, 
            groundLevel + mapViewHeight, // Use ground level + height
            playerPosition.z
        );
        ...
        Debug.Log($"Map camera moving to position: {targetPosition}, rotation: {targetRotation.eulerAngles}");
