...
public class VisualEffectManager : MonoBehaviour
{
    ...
    public bool IsTargetingEntityCounter { get; set; }
    ...
    public void SpawnCrumbCollectEffect(int count, bool targetEntityCounter = false)
    {
        if (nestTransform == null) return;

        Vector2 screenTopLeft = new Vector2(Screen.width + crumbSpawnOffsetX, Screen.height - spawnOffsetY);

        for (int i = 0; i < count; i++)
        {
            if (symbolPools["crumb"].Count == 0) return;
