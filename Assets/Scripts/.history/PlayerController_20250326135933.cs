...
public class PlayerController : MonoBehaviour
{
    ...
    // Updated coroutine to hide the cooldown text when timer finishes
    private IEnumerator UpdateCooldownTimerText(float initialCooldown)
    {
        ...
        // Add this line to hide the text when cooldown is complete
        uiHelper.HideAttackCooldownText();
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        BorderVisualizer border = other.GetComponent<BorderVisualizer>();
        if (border != null)
        {
            int requiredLevel = border.GetRequiredLevel();
            int playerLevel = playerInventory != null ? playerInventory.CurrentLevel : 0;
            
            // If player doesn't meet level requirement, prevent entry
            if (requiredLevel > 0 && playerLevel < requiredLevel)
            {
                // Show message to player
                if (uiHelper != null)
                {
                    uiHelper.ShowInformText($"You need to be level {requiredLevel} to enter this area!");
                }
                
                // Create a force to push the player back
                StartCoroutine(PushPlayerFromBorder(other.transform.position));
            }
        }
    }

    private IEnumerator PushPlayerFromBorder(Vector3 borderPos)
    {
        // Calculate direction away from border
        Vector3 pushDir = (transform.position - borderPos).normalized;
        pushDir.y = 0; // Keep push horizontal
        
        // Apply force over a short time
        float pushDuration = 0.5f;
        float pushForce = 15f; // Adjust as needed
        float timer = 0;
        
        while (timer < pushDuration)
        {
            // Apply force to push player back
            Vector3 moveAmount = pushDir * pushForce * Time.deltaTime;
