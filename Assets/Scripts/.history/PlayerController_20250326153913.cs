...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float[] metamorphosisLevels = new float[] { 5f, 10f, 15f, 20f, 25f, 30f, 35f, 40f, 45f, 50f, 55f };
    ...
    public void CheckMetamorphosis(int newLevel)
    {
        ...
        for (int i = 0; i < metamorphosisLevels.Length; i++)
        {
            if (newLevel == metamorphosisLevels[i])
            {
                ...
                // Apply new scale - increase by 10% from current scale
                currentMetamorphosisStage++;
                float cumulativeScale = Mathf.Pow(scaleIncreaseMultiplier, currentMetamorphosisStage);
                Vector3 newScale = baseScale * cumulativeScale;
                transform.localScale = newScale;
                
                Debug.Log($"<color=yellow>METAMORPHOSIS: Level {newLevel} reached! New scale: {cumulativeScale}x</color>");
    ...
    private void UpdateCameraSettings(int level)
    {
        ...
        if (level >= 52)
        {
            cameraController.SetCameraParameters(6.76f, 8.96f);
        }
        else if (level >= 47)
        {
        ...
        else
        {
            ...
            cameraController.SetCameraParameters(1.4f, 1.9f);
        }
    }
