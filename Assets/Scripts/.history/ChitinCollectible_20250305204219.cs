...
public class ChitinCollectible : MonoBehaviour
{
    ...
    // Add this field to track if the chitin is already being collected
    ...
    private void Start()
    {
        ...
        // Add a debug message to confirm this chitin was spawned
        Debug.Log($"Chitin piece spawned at {transform.position}");
    }
    
    private void Update()
    {
        if (playerTransform == null || playerInventory == null || isBeingCollected)
            return;
        ...
        if (distanceToPlayer <= collectRadius)
        {
            ...
            Debug.Log($"Chitin collected at {transform.position}");
