...
public class ArrowIndicator : MonoBehaviour
{
    ...
    [SerializeField] private float arrowSize = 0.18f; // Reduced from 0.25f to make arrows smaller
    [SerializeField] private Color arrowColor = Color.yellow;
    [SerializeField] private float pulseSpeed = 1.5f; // Reduced from 2.0f for smoother pulsing
    [SerializeField] private float pulseAmount = 0.15f; // Reduced from 0.2f for subtler effect
    ...
    [SerializeField] private float positionSmoothTime = 0.15f; // Smoothing time for position
    ...
    private void Update()
    {
        if (isShowing && arrowObject != null && target != null && playerTransform != null)
        {
            ...
            SmoothlyUpdateArrowTransform();
        }
    }
    
    // This method calculates the target position and rotation for the arrow
    private void CalculateArrowTransform()
    {
        ...
        // Calculate direction from target to player in screen space
        Vector2 directionToPlayer = new Vector2(playerScreenPos.x - targetScreenPos.x, 
                                               playerScreenPos.y - targetScreenPos.y);
        ...
        // Calculate the angle - point directly toward the player
        ...
        // Calculate direction from screen center to target (for positioning only)
        ...
        if (!isOnScreen || (distance > reachDistance))
        {
            ...
            if (!isOnScreen)
            {
                ...
                targetPosition = mainCamera.ScreenToWorldPoint(new Vector3(arrowScreenPos.x, arrowScreenPos.y, 1f));
            }
            else
            {
                // If on screen but far away, position near the target
                targetPosition = mainCamera.ScreenToWorldPoint(new Vector3(targetScreenPos.x, targetScreenPos.y, 1f));
            }
            
            // Calculate billboard rotation - facing camera
            targetRotationQuaternion = mainCamera.transform.rotation;
            
            // Store the angle for Z-axis rotation (in local space)
    ...
    // This method smoothly updates the arrow position and rotation
    private void SmoothlyUpdateArrowTransform()
    {
        ...
        arrowObject.transform.rotation = Quaternion.Slerp(
            ...
            Time.deltaTime / rotationSmoothTime * 10f
        );
        
        // Apply the Z rotation smoothly (for directional pointing)
        Vector3 currentRotation = arrowObject.transform.localEulerAngles;
        ...
        arrowObject.transform.localEulerAngles = new Vector3(
            currentRotation.x, 
            currentRotation.y, 
            smoothedZRotation
        );
        
        // Apply pulsing effect to scale only (with smoother pulsing)
