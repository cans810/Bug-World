...
public class PlayerController : MonoBehaviour
{
    ...
    // Add this to ensure continuous level restriction in OnTriggerStay
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("MapBorder"))
        {
            ...
            // CRITICAL: Enforce level restriction EVERY FRAME while inside trigger
            if (playerLevel < requiredLevel)
            {
                ...
                // Apply an extremely strong repulsion force 
                if (livingEntity != null)
                {
                    // Apply much stronger continuous force (2.0f is much stronger than before)
                    livingEntity.MoveInDirection(borderNormal, 2.0f);
                    
                    // Also directly move the transform for immediate effect
                    transform.position += borderNormal * 0.2f;
                }
                else
                {
                    // Direct transform movement as fallback (even stronger)
                ...
                if (Time.time >= lastBorderMessageTime + 2.0f)
                {
                    ...
                    // Log for debugging
                    Debug.LogWarning($"ENFORCING BORDER: Player (Level {playerLevel}) attempting to stay in {borderName} (Level {requiredLevel})");
                }
