    ...
    public void OnPlayerLevelUp(int newLevel)
    {
        ...
        foreach (LevelAreaTarget area in levelAreas)
        {
            ...
            if (area.requiredLevel == newLevel && !area.hasBeenVisited)
            {
                ...
                // Direct method call instead of event
                if (cameraAnimationsRef != null && area.areaTarget != null)
                {
                    Debug.Log($"Directly calling CameraAnimations.OnAreaUnlocked for {area.areaName}");
                    cameraAnimationsRef.OnAreaUnlocked(area.areaTarget, newLevel, area.areaName);
                }
                ...
                if (area.areaTarget != null)
                {
                    ...
                    {
                        Debug.Log($"Firing OnAreaUnlocked event for {area.areaName}");
                        OnAreaUnlocked(area.areaTarget, newLevel, area.areaName);
                    }
                    
                    // Also invoke the Unity Event
                    Debug.Log($"Invoking onAreaUnlockedEvent for {area.areaName}");
                    onAreaUnlockedEvent.Invoke(area.areaTarget, newLevel, area.areaName);
                }
