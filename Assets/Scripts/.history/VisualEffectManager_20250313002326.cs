...
public class VisualEffectManager : MonoBehaviour
{
    ...
    [Header("Nest Pulsation")]
    [SerializeField] private float nestPulseDuration = 0.3f;
    [SerializeField] private float nestPulseScale = 1.2f;
    [SerializeField] private AnimationCurve nestPulseCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);
    ...
    private IEnumerator AnimateResourceCollection(GameObject symbol, string type)
    {
        ...
        // Trigger nest pulsation when the symbol reaches the nest
    ...
    private IEnumerator PulsateNest()
    {
        ...
        Vector3 originalScale = nestTransform.localScale;
        Vector3 targetScale = originalScale * nestPulseScale;
        float elapsed = 0f;
        
        while (elapsed < nestPulseDuration)
        {
            elapsed += Time.deltaTime;
            float normalizedTime = elapsed / nestPulseDuration;
            float curveValue = nestPulseCurve.Evaluate(normalizedTime);
            
            // Scale up and then back down based on the curve
            if (normalizedTime <= 0.5f)
            {
                // Scale up (0 to 0.5)
                nestTransform.localScale = Vector3.Lerp(originalScale, targetScale, curveValue * 2);
            }
            else
            {
                // Scale down (0.5 to 1)
                nestTransform.localScale = Vector3.Lerp(targetScale, originalScale, (curveValue - 0.5f) * 2);
            }
            
            yield return null;
        }
        
        // Ensure the nest returns to its original scale
        nestTransform.localScale = originalScale;
    }
