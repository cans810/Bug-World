...
public class AllyAI : MonoBehaviour
{
    ...
    private void FollowPlayerBehavior()
    {
        ...
        if (distanceToPlayer <= followDistance)
        {
            ...
            if (playerTransform != null)
            {
                ...
                if (playerForward.magnitude > 0.1f)
                {
                    ...
                    livingEntity.RotateTowards(playerForward, rotationMultiplier);
                }
            }
        }
        ...
        else
        {
            ...
            // Calculate a blended rotation direction - mostly toward player but partially matching player's facing direction
            Vector3 playerForward = playerTransform.forward;
            playerForward.y = 0; // Keep on same Y level
            
            // Blend 80% direction to player, 20% player's forward direction
            Vector3 blendedDirection = (directionToPlayer * 0.8f + playerForward * 0.2f).normalized;
            
            // Smoothly rotate to face the blended direction
