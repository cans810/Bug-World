...
public class BorderVisualizer : MonoBehaviour
{
    ...
    private void DrawCapsulePerimeter()
    {
        ...
        // For Y-axis aligned capsule, the projection is just a circle
        ...
        // For X or Z axis capsules, we need a stadium shape
        float halfHeight = (height - 2f * radius) * 0.5f;
        
        // Let's try a different approach with proper angle calculations
        for (int i = 0; i < segments; i++)
        {
            float angle = (float)i / segments * 2f * Mathf.PI;
            float x, z;
            
            // Determine which part of the shape we're in
            if (angle < Mathf.PI / 2 || angle > 3 * Mathf.PI / 2)
            {
                // Top half (connecting the ends)
                if (direction == 0) // X-axis
                {
                    // Calculate position along the straight line
                    float t = (angle < Mathf.PI / 2) ? 
                               (1 - angle / (Mathf.PI / 2)) : 
                               ((angle - 3 * Mathf.PI / 2) / (Mathf.PI / 2));
                    
                    x = Mathf.Lerp(center.x - halfHeight, center.x + halfHeight, t);
                    z = center.z + radius;
                }
                else // Z-axis
                {
                    ...
                    x = center.x + radius;
                    z = Mathf.Lerp(center.z - halfHeight, center.z + halfHeight, t);
                }
            }
            else if (angle < 3 * Mathf.PI / 2 && angle >= Mathf.PI / 2)
            {
                // Bottom half (connecting the ends)
                if (direction == 0) // X-axis
                {
                    // Calculate position along the straight line
                    float t = (angle - Mathf.PI / 2) / Mathf.PI;
            ...
            // Apply the end cap curves at the right locations
            if (direction == 0) // X-axis
            {
                // Right end cap
                if (x > center.x + halfHeight - 0.01f)
                {
                    float localAngle = Mathf.Atan2(z - center.z, x - (center.x + halfHeight));
                    x = center.x + halfHeight;
                    z = center.z + radius * Mathf.Sin(localAngle);
                }
                // Left end cap
                else if (x < center.x - halfHeight + 0.01f)
                {
                    float localAngle = Mathf.Atan2(z - center.z, x - (center.x - halfHeight));
                    x = center.x - halfHeight;
                    z = center.z + radius * Mathf.Sin(localAngle + Mathf.PI);
                }
            }
            else // Z-axis
            {
                // Front end cap
                if (z > center.z + halfHeight - 0.01f)
                {
                    float localAngle = Mathf.Atan2(x - center.x, z - (center.z + halfHeight));
                    z = center.z + halfHeight;
                    x = center.x + radius * Mathf.Sin(localAngle);
                }
                // Back end cap
                else if (z < center.z - halfHeight + 0.01f)
                {
                    float localAngle = Mathf.Atan2(x - center.x, z - (center.z - halfHeight));
                    z = center.z - halfHeight;
                    x = center.x + radius * Mathf.Sin(localAngle + Mathf.PI);
                }
            }
